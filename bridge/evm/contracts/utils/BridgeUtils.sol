...
/// @title BridgeUtils
/// @notice This library defines the message format and constants for the MySocial native bridge. It also
...
library BridgeUtils {
    ...
    /// @notice Converts the provided token amount to the MySocial decimal adjusted amount.
    /// @param erc20Decimal The erc20 decimal value for the token.
    /// @param mysDecimal The mys decimal value for the token.
    /// @param amount The ERC20 amount of the tokens to convert to MySocial.
    /// @return MySocial converted amount.
    function convertERC20ToMysDecimal(uint8 erc20Decimal, uint8 mysDecimal, uint256 amount)
        internal
        pure
        returns (uint64)
    {
        if (erc20Decimal == mysDecimal) {
        ...
        require(erc20Decimal > mysDecimal, "BridgeUtils: Invalid MySocial decimal");

        // Difference in decimal places
        uint256 factor = 10 ** (erc20Decimal - mysDecimal);
    ...
    /// @notice Converts the provided MySocial decimal adjusted amount to the ERC20 token amount.
    /// @param erc20Decimal The erc20 decimal value for the token.
    /// @param mysDecimal The mys decimal value for the token.
    /// @param amount The MySocial amount of the tokens to convert to ERC20.
    /// @return ERC20 converted amount.
    function convertMysToERC20Decimal(uint8 erc20Decimal, uint8 mysDecimal, uint64 amount)
        internal
        pure
        returns (uint256)
    {
        if (mysDecimal == erc20Decimal) {
    ...
    ///     bytes 1-32   : sender address (as we only support MySocial now, it has to be 32 bytes long)
        ...
        returns (TokenTransferPayload memory)
    {
        ...
        require(
            ...
            "BridgeUtils: Invalid sender address length, MySocial address must be 32 bytes"
        );
    ...
    ///     byte m + 1       : number of mys decimals
    ///     bytes m + 2 -> i : mys decimals
    ...
    /// @return mysDecimals the MySocial decimal places of the tokens to be added.
        ...
        )
    {
        ...
        mysDecimals = new uint8[](decimalCount);
        for (uint8 i; i < decimalCount; i++) {
            mysDecimals[i] = uint8(_payload[offset++]);
        }
