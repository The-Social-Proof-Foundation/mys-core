...
/// @title BridgeConfig
/// @notice This contract manages a registry of supported tokens and supported chain IDs for the MysBridge.
/// It also provides functions to convert token amounts to MySocial decimal adjusted amounts and vice versa.
contract BridgeConfig is IBridgeConfig, CommitteeUpgradeable {
    ...
    function initialize(
        ...
        require(
            _supportedTokens.length == _mysDecimals.length, "BridgeConfig: Invalid MySocial decimals"
        );
        ...
        for (uint8 i; i < _tokenIds.length; i++) {
            ...
            supportedTokens[_tokenIds[i]] = Token(_supportedTokens[i], _mysDecimals[i], true);
        }
    ...
    /// @notice Returns the mys decimal places of the token with the given ID.
    /// @param tokenID The ID of the token.
    /// @return amount of mys decimal places of the provided token.
    function tokenMysDecimalOf(uint8 tokenID) public view override returns (uint8) {
        return supportedTokens[tokenID].mysDecimal;
    }
    ...
    /// @notice Returns whether a token is supported in MysBridge with the given ID.
    /// @param tokenID The ID of the token.
    ...
    /// @notice Returns whether a chain is supported in MysBridge with the given ID.
        ...
        verifyMessageAndSignatures(message, signatures, BridgeUtils.ADD_EVM_TOKENS)
    {
        ...
        for (uint8 i; i < tokenIDs.length; i++) {
            _addToken(tokenIDs[i], tokenAddresses[i], mysDecimals[i], _tokenPrices[i], native);
        }

        emit TokensAddedV2(message.nonce, tokenIDs, tokenAddresses, mysDecimals, _tokenPrices);
    }
    ...
    /// @param mysDecimal The decimal places of the token.
    /// @param tokenPrice The price of the token.
    ...
    function _addToken(
        ...
        require(mysDecimal > 0, "BridgeConfig: Invalid MySocial decimal");
        require(tokenPrice > 0, "BridgeConfig: Invalid token price");

        uint8 erc20Decimals = IERC20Metadata(tokenAddress).decimals();
        require(erc20Decimals >= mysDecimal, "BridgeConfig: Invalid MySocial decimal");

        supportedTokens[tokenID] = Token(tokenAddress, mysDecimal, native);
        tokenPrices[tokenID] = tokenPrice;
    }
