...
import "./interfaces/IMysBridge.sol";
...
/// tokens. Bridge operations are managed by a committee of MySocial validators that are responsible
...
contract MysBridge is IMysBridge, CommitteeUpgradeable, PausableUpgradeable {
    /* ========== STATE VARIABLES ========== */
    ...
    uint8 constant MYS_ADDRESS_LENGTH = 32;

    /* ========== INITIALIZER ========== */

    /// @notice Initializes the MysBridge contract with the provided parameters.
        ...
        onlySupportedChain(message.chainID)
    {
        ...
        require(!isTransferProcessed[message.nonce], "MysBridge: Message already processed");
        ...
        require(
            tokenTransferPayload.targetChain == config.chainID(), "MysBridge: Invalid target chain"
        );

        // convert amount to ERC20 token decimals
        uint256 erc20AdjustedAmount = BridgeUtils.convertMysToERC20Decimal(
            IERC20Metadata(config.tokenAddressOf(tokenTransferPayload.tokenID)).decimals(),
            config.tokenMysDecimalOf(tokenTransferPayload.tokenID),
    ...
    function bridgeERC20(
        ...
        require(
            recipientAddress.length == MYS_ADDRESS_LENGTH,
            "MysBridge: Invalid recipient address length"
        );
        ...
        onlySupportedChain(destinationChainID)
    {
        ...
        require(success, "MysBridge: Failed to transfer ETH to vault");
    ...
    modifier limitNotExceeded(uint8 chainID, uint8 tokenID, uint256 amount) {
        require(
            ...
            "MysBridge: Amount exceeds bridge limit"
        );
        _;
    }
    ...
    modifier onlySupportedChain(uint8 targetChainID) {
        require(
            ...
            "MysBridge: Target chain not supported"
        );
        _;
    }
}
