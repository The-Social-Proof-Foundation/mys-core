...
import "../contracts/MysBridge.sol";
...
contract DeployBridge is Script {
    ...
    function run() external {
        ...
        if (isLocal) {
            ...
            deployConfig.mysDecimals = new uint256[](5);
            deployConfig.mysDecimals[0] = 9;
            deployConfig.mysDecimals[1] = 8;
            deployConfig.mysDecimals[2] = 8;
            deployConfig.mysDecimals[3] = 6;
            deployConfig.mysDecimals[4] = 6;
        }
        ...
        require(
            deployConfig.supportedTokens.length == deployConfig.mysDecimals.length,
            "supportedTokens.length != mysDecimals.length"
        );
        ...
        // deploy MySocial Bridge ========================================================================

        address mysBridge = Upgrades.deployUUPSProxy(
            "MysBridge.sol",
            abi.encodeCall(MysBridge.initialize, (bridgeCommittee, address(vault), limiter)),
            opts
        );

        // transfer vault ownership to bridge
        vault.transferOwnership(mysBridge);
        // transfer limiter ownership to bridge
        BridgeLimiter instance = BridgeLimiter(limiter);
        instance.transferOwnership(mysBridge);

        // print deployed addresses for post deployment setup
        console.log("[Deployed] BridgeConfig:", bridgeConfig);
        console.log("[Deployed] MysBridge:", mysBridge);
