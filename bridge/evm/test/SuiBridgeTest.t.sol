...
import "../contracts/interfaces/IMysBridge.sol";
import "./mocks/MockMysBridgeV2.sol";

contract MysBridgeTest is BridgeBaseTest, IMysBridge {
    // This function is called before each unit test
    function setUp() public {
        setUpBridgeTest();
    }

    function testMysBridgeInitialization() public {
    ...
    function testTransferBridgedTokensWithSignaturesTokenDailyLimitExceeded() public {
        ...
        vm.expectRevert(bytes("MysBridge: Amount exceeds bridge limit"));
    ...
    function testTransferBridgedTokensWithSignaturesInvalidTargetChain() public {
        ...
        signatures[2] = getSignature(messageHash, committeeMemberPkC);
        signatures[3] = getSignature(messageHash, committeeMemberPkD);
        vm.expectRevert(bytes("MysBridge: Target chain not supported"));
        bridge.transferBridgedTokensWithSignatures(signatures, message);
    }
    ...
    function testTransferBridgedTokensWithSignaturesInsufficientStakeAmount() public {
        ...
        BridgeUtils.TokenTransferPayload memory payload = BridgeUtils.TokenTransferPayload({
            ...
            // This is MySocial amount (eth decimal 8)
            amount: 100_000_000
        });
        BridgeUtils.Message memory message = BridgeUtils.Message({
            messageType: BridgeUtils.TOKEN_TRANSFER,
    ...
    function testTransferWETHWithValidSignatures() public {
        ...
        address recipientAddress = bridgerA;
        uint8 tokenID = BridgeUtils.ETH;
        uint64 amount = 100000000; // 1 ether in mys decimals
        bytes memory payload = abi.encodePacked(
            senderAddressLength,
            senderAddress,
            targetChain,
        ...
        vm.expectRevert(bytes("MysBridge: Message already processed"));
        bridge.transferBridgedTokensWithSignatures(signatures, message);
    }
    ...
    function testBridgeERC20UnsupportedToken() public {
        vm.expectRevert(bytes("MysBridge: Unsupported token"));
        bridge.bridgeERC20(
    ...
    function testBridgeERC20InsufficientAllowance() public {
        vm.expectRevert(bytes("MysBridge: Insufficient allowance"));
        bridge.bridgeERC20(
            BridgeUtils.ETH,
            type(uint256).max,
    ...
    function testBridgeERC20InvalidRecipientAddress() public {
        vm.expectRevert(bytes("MysBridge: Invalid recipient address length"));
        bridge.bridgeERC20(
            BridgeUtils.ETH,
            1 ether,
    ...
    function testBridgeEth() public {
        ...
        // assert emitted event
        vm.expectEmit(true, true, true, false);
        emit IMysBridge.TokensDeposited(
            chainID,
            0, // nonce
            0, // destination chain id
            BridgeUtils.ETH,
    ...
    function testMysBridgeInvalidERC20DecimalConversion() public {
        IERC20(wETH).approve(address(bridge), 10 ether);
    ...
    function testMysBridgeInvalidEthDecimalConversion() public {
        vm.expectRevert(bytes("BridgeUtils: Insufficient amount provided"));
    ...
    function testMysBridgeInvalidERC20Transfer() public {
        vm.expectRevert(bytes("BridgeUtils: Insufficient amount provided"));
        bridge.bridgeERC20(
            BridgeUtils.USDC,
            0,
    ...
    function testMysBridgeInvalidETHTransfer() public {
        vm.expectRevert(bytes("BridgeUtils: Insufficient amount provided"));
    ...
    function testTransferMysToEthRegressionTest() public {
        ...
        tokenPrices[0] = 1 * USD_VALUE_MULTIPLIER; // MYS PRICE
