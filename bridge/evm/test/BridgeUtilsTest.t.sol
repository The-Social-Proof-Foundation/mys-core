...
contract BridgeUtilsTest is BridgeBaseTest {
    ...
    function testConvertERC20ToMysDecimalAmountTooLargeForUint64() public {
        vm.expectRevert(bytes("BridgeUtils: Amount too large for uint64"));
        BridgeUtils.convertERC20ToMysDecimal(18, 8, type(uint256).max);
    }

    function testConvertERC20ToMysDecimalInvalidMysDecimal() public {
        vm.expectRevert(bytes("BridgeUtils: Invalid MySocial decimal"));
        BridgeUtils.convertERC20ToMysDecimal(10, 11, 100);
    }

    function testconvertMysToERC20DecimalInvalidMysDecimal() public {
        vm.expectRevert(bytes("BridgeUtils: Invalid MySocial decimal"));
        BridgeUtils.convertMysToERC20Decimal(10, 11, 100);
    }

    function testConvertERC20ToMysDecimal() public {
        // ETH
        assertEq(IERC20Metadata(wETH).decimals(), 18);
        uint256 ethAmount = 10 ether;
        uint64 mysAmount = BridgeUtils.convertERC20ToMysDecimal(
            IERC20Metadata(config.tokenAddressOf(BridgeUtils.ETH)).decimals(),
            config.tokenMysDecimalOf(BridgeUtils.ETH),
            ethAmount
        );
        assertEq(mysAmount, 10_000_000_00); // 10 * 10 ^ 8

        // USDC
        assertEq(IERC20Metadata(USDC).decimals(), 6);
        ethAmount = 50_000_000; // 50 USDC
        mysAmount = BridgeUtils.convertERC20ToMysDecimal(
            IERC20Metadata(config.tokenAddressOf(BridgeUtils.USDC)).decimals(),
            config.tokenMysDecimalOf(BridgeUtils.USDC),
            ethAmount
        );
        assertEq(mysAmount, ethAmount);
        ...
        mysAmount = BridgeUtils.convertERC20ToMysDecimal(
            ...
            config.tokenMysDecimalOf(BridgeUtils.USDT),
            ethAmount
        );
        assertEq(mysAmount, ethAmount);
        ...
        mysAmount = BridgeUtils.convertERC20ToMysDecimal(
            ...
            config.tokenMysDecimalOf(BridgeUtils.BTC),
            ethAmount
        );
        assertEq(mysAmount, ethAmount);
    }

    function testconvertMysToERC20Decimal() public {
        // ETH
        assertEq(IERC20Metadata(wETH).decimals(), 18);
        uint64 mysAmount = 11_000_000_00; // 11 eth
        uint256 ethAmount = BridgeUtils.convertMysToERC20Decimal(
            IERC20Metadata(config.tokenAddressOf(BridgeUtils.ETH)).decimals(),
            config.tokenMysDecimalOf(BridgeUtils.ETH),
            mysAmount
        );
        ...
        // USDC
        assertEq(IERC20Metadata(USDC).decimals(), 6);
        mysAmount = 50_000_000; // 50 USDC
        ethAmount = BridgeUtils.convertMysToERC20Decimal(
            IERC20Metadata(config.tokenAddressOf(BridgeUtils.USDC)).decimals(),
            config.tokenMysDecimalOf(BridgeUtils.USDC),
            mysAmount
        );
