...
contract BridgeConfigTest is BridgeBaseTest {
    ...
    function testBridgeConfigInitialization() public {
        ...
        assertEq(config.tokenMysDecimalOf(0), 9);
        assertEq(config.tokenMysDecimalOf(1), 8);
        assertEq(config.tokenMysDecimalOf(2), 8);
        assertEq(config.tokenMysDecimalOf(3), 6);
        assertEq(config.tokenMysDecimalOf(4), 6);
        assertEq(config.tokenPriceOf(0), MYS_PRICE);
    ...
    function testTokenMysDecimalOf() public {
        assertEq(config.tokenMysDecimalOf(1), 8);
    }
    ...
    function testAddTokensWithSignatures() public {
        ...
        assertEq(config.tokenMysDecimalOf(10), 6);
    ...
    function testAddTokensMysDecimalFailure() public {
        MockUSDC _newToken = new MockUSDC();
        ...
        // add token shoudl fail because the mys decimal is greater than the eth decimal
        vm.expectRevert(bytes("BridgeConfig: Invalid MySocial decimal"));
        config.addTokensWithSignatures(signatures, message);
    }
    ...
    function testAddTokensRegressionTest() public {
        ...
        assertEq(mysDecimals.length, 3);
        assertEq(mysDecimals[0], 5);
        assertEq(mysDecimals[1], 6);
        assertEq(mysDecimals[2], 7);
        ...
        assertEq(config.tokenMysDecimalOf(99), 5);
        assertEq(config.tokenMysDecimalOf(100), 6);
        assertEq(config.tokenMysDecimalOf(101), 7);
