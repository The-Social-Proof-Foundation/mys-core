...
use crate::protocol::{ProtocolCommands, ProtocolMetrics};
...
impl<P> Orchestrator<P> {
    ...
    pub fn select_instances(&self) -> TestbedResult<(Vec<Instance>, Option<Instance>)> {
        // Ensure there are enough active instances.
        let available_instances: Vec<_> = self.instances.iter().filter(|x| x.is_active()).collect();
        let minimum_instances = 1 + if self.settings.monitoring { 1 } else { 0 };
        ensure!(
        ...
        for instances in instances_by_regions.into_values() {
            nodes_instances.extend(instances.into_iter().cloned());
        }
...
impl<P: ProtocolCommands + ProtocolMetrics> Orchestrator<P> {
    ...
    pub async fn run(
        ...
        // Kill the nodes.
        let (nodes, _) = self.select_instances()?;
        self.ssh_manager.kill(nodes, "node").await?;
    ...
    pub async fn download_logs(
        ...
        let commit = &self.settings.repository.commit;
        let path: PathBuf = [&self.settings.logs_dir, &format!("logs-{commit}").into()]
            .iter()
            .collect();
