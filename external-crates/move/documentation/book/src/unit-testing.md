...
Unit tests for a Move package can be run with the [`mys move test` command](./packages.md).
...
```bash
$ mys move new test_example; cd test_example 
```
...
You can run a specific test, or a set of tests with `mys move test <str>`. This
...
runtime and gas used for each test. You can additionally add `csv` (`mys move
