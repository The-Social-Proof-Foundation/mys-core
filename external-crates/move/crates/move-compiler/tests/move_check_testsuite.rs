...
const MYS_MODE_DIR: &str = "mys_mode";
...
fn default_testing_addresses(flavor: Flavor) -> BTreeMap<String, NumericalAddress> {
    ...
    if flavor == Flavor::Mys {
        mapping.extend([("mys", "0x2"), ("mys_system", "0x3")]);
    }
    mapping
        .into_iter()
...
fn test_config(path: &Path) -> (TestKind, TestInfo, PackageConfig, Flags) {
    ...
    let flavor = if path_contains(MYS_MODE_DIR) {
        Flavor::Mys
    } else {
        Flavor::default()
    };
...
pub fn run_test(path: &Path) -> datatest_stable::Result<()> {
    ...
    if flavor == Flavor::Mys {
        let (prefix, filters) = mys_mode::linters::known_filters();
        compiler = compiler.add_custom_known_filters(prefix, filters);
        if test_info.lint {
            compiler = compiler.add_visitors(mys_mode::linters::linter_visitors(LintLevel::All))
        }
    }
