...
module a::can_determine_to_be_new {
    ...
    fun make_obj(_: u64, _: vector<vector<u8>>, ctx: &mut mys::tx_context::TxContext): Obj {
        Obj { id: mys::object::new(ctx) }
    }

    public fun transfer(ctx: &mut mys::tx_context::TxContext) {
        let o = make_obj(0, vector[], ctx);
        transfer::transfer(o, @0);
    }

    public fun share(ctx: &mut mys::tx_context::TxContext) {
        let o = make_obj(0, vector[], ctx);
        transfer::share_object(o);
    }
}
...
module b::can_determine_to_be_new_with_struct {
    ...
    fun make_obj<T>(_: X<T>, ctx: &mut mys::tx_context::TxContext): Obj {
        Obj { id: mys::object::new(ctx) }
    }

    public fun transfer(ctx: &mut mys::tx_context::TxContext) {
        let o = make_obj(X<Obj> {}, ctx);
        transfer::transfer(o, mys::tx_context::sender(ctx));
    }
