...
pub struct ProgramInfo<const AFTER_TYPING: bool> {
    ...
    pub mys_flavor_info: Option<MysInfo>,
}
...
impl TypingProgramInfo {
    pub fn new(
        ...
        if env
            .package_configs()
            .any(|(_, config)| config.flavor == Flavor::Mys)
        {
            let mys_flavor_info = MysInfo::new(pre_compiled_lib, modules, &info);
            info.mys_flavor_info = Some(mys_flavor_info);
        };
        info
    }
}
