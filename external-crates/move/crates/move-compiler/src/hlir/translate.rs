...
fn gen_unused_warnings(
    ...
    let is_mys_mode = context.env.package_config(context.current_package).flavor == Flavor::Mys;
    ...
    for (_, sname, sdef) in structs {
        ...
        if let H::StructFields::Defined(fields) = &sdef.fields {
            for (f, _) in fields {
                // skip for Mys ID fields
                if is_mys_mode && has_key && f.value() == ID_FIELD_NAME {
                    continue;
                }
                if !context
                    .used_fields
