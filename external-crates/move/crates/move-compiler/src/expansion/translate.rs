...
use crate::{
    ...
    expansion::{
        ...
        name_validation::{
            ...
            IMPLICIT_STD_MEMBERS, IMPLICIT_STD_MODULES, IMPLICIT_MYS_MEMBERS, IMPLICIT_MYS_MODULES,
        },
        path_expander::{
...
fn default_aliases(context: &mut Context) -> AliasMapBuilder {
    ...
    let mys_address = maybe_make_well_known_address(context, loc, symbol!("mys"));
    ...
    // if mys is defined and the current package is in Mys mode, add implicit mys aliases
    if mys_address.is_some() && context.env().package_config(current_package).flavor == Flavor::Mys
    {
        let mys_address = mys_address.unwrap();
        modules.extend(
            IMPLICIT_MYS_MODULES
                .iter()
                .copied()
                .map(|m| (mys_address, m)),
        );
        members.extend(
            IMPLICIT_MYS_MEMBERS
                .iter()
                .copied()
                .map(|(m, mem, k)| (mys_address, m, mem, k)),
        );
    }
