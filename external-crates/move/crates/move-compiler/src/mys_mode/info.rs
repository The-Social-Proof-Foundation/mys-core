...
//! ProgramInfo extension for Mys Flavor
//! Contains information that may be expensive to compute and is needed only for Mys
...
use crate::{
    ...
    mys_mode::{
        OBJECT_MODULE_NAME, MYS_ADDR_VALUE, TRANSFER_FUNCTION_NAME, TRANSFER_MODULE_NAME,
        UID_TYPE_NAME,
    },
...
pub enum TransferKind {
    ...
    PublicTransfer(Loc),
    /// transferred within the module to an address vis `mys::transfer::transfer`
    PrivateTransfer(Loc),
}
...
impl MysInfo {
    pub fn new(
        ...
        assert!(info.mys_flavor_info.is_none());
    ...
    fn visit_ty(
        ...
        match ty_ {
            ...
            N::Type_::Apply(_, sp!(_, tn_), _)
                if tn_.is(&MYS_ADDR_VALUE, OBJECT_MODULE_NAME, UID_TYPE_NAME) =>
            {
