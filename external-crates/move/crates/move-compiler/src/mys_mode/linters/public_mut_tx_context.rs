...
use crate::{
    ...
    mys_mode::{MYS_ADDR_VALUE, TX_CONTEXT_MODULE_NAME, TX_CONTEXT_TYPE_NAME},
...
simple_visitor!(
    ...
    fn visit_module_custom(&mut self, ident: ModuleIdent, _mdef: &T::ModuleDefinition) -> bool {
        // skip if in 'mys::tx_context'
        ident.value.is(&MYS_ADDR_VALUE, TX_CONTEXT_MODULE_NAME)
    },
    fn visit_function_custom(
        ...
        for (_, _, sp!(loc, param_ty_)) in &fdef.signature.parameters {
            if matches!(
                param_ty_,
                Type_::Ref(false, t) if t.value.is(&MYS_ADDR_VALUE, TX_CONTEXT_MODULE_NAME, TX_CONTEXT_TYPE_NAME),
            ) {
