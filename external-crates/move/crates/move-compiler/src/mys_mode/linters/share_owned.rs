...
use crate::{
    ...
    mys_mode::{
        ...
        MYS_ADDR_VALUE, TX_CONTEXT_MODULE_NAME, TX_CONTEXT_TYPE_NAME,
    },
};
...
const SHARE_FUNCTIONS: &[(AccountAddress, &str, &str)] = &[
    (MYS_ADDR_VALUE, TRANSFER_MOD_NAME, PUBLIC_SHARE_FUN),
    (MYS_ADDR_VALUE, TRANSFER_MOD_NAME, SHARE_FUN),
];
...
impl<'a> ShareOwnedVerifierAI<'a> {
    ...
    fn can_hold_obj_base(&self, sp!(_, bt_): &BaseType) -> bool {
        match bt_ {
            ...
            BaseType_::Apply(_, sp!(_, TypeName_::ModuleType(m, n)), targs) => {
                let m = *m;
                let n = *n;
                if self.mys_info().uid_holders.contains_key(&(m, n)) {
                    return true;
                }
    ...
    fn maybe_warn_share_owned(
        ...
        let Some(transferred_kind) = self.mys_info().transferred.get(&tn) else {
            return;
        };
    ...
    fn mys_info(&self) -> &'a MysInfo {
        self.info.mys_flavor_info.as_ref().unwrap()
    }
}
