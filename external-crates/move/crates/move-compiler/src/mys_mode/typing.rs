...
pub struct MysTypeChecks;

impl TypingVisitorConstructor for MysTypeChecks {
...
pub struct Context<'a> {
    ...
    mys_transfer_ident: Option<ModuleIdent>,
...
impl<'a> Context<'a> {
    fn new(env: &'a CompilationEnv, info: Arc<TypingProgramInfo>) -> Self {
        let mys_module_ident = info
            .modules
            .key_cloned_iter()
            .find(|(m, _)| m.value.is(&MYS_ADDR_VALUE, TRANSFER_MODULE_NAME))
            .map(|(m, _)| m);
        ...
        Context {
            ...
            reporter,
            info,
            mys_transfer_ident: mys_module_ident,
            current_module: None,
            otw_name: None,
...
fn struct_def(context: &mut Context, name: DatatypeName, sdef: &N::StructDefinition) {
    ...
    if !id_field_type
        .value
        .is(&MYS_ADDR_VALUE, OBJECT_MODULE_NAME, UID_TYPE_NAME)
    {
        let actual = format!(
            "But found type: {}",
...
fn invalid_object_id_field_diag(key_loc: Loc, loc: Loc, name: DatatypeName) -> Diagnostic {
    const KEY_MSG: &str = "The 'key' ability is used to declare objects in Mys";
...
fn is_mut_clock(param_ty: &Type) -> bool {
    match &param_ty.value {
        ...
        Type_::Apply(_, sp!(_, n_), _) => {
            n_.is(&MYS_ADDR_VALUE, CLOCK_MODULE_NAME, CLOCK_TYPE_NAME)
        }
        Type_::Unit
        | Type_::Param(_)
        | Type_::Var(_)
...
fn is_entry_receiving_ty(param_ty: &Type) -> bool {
    match &param_ty.value {
        ...
        Type_::Apply(_, sp!(_, n), targs)
            if n.is(&MYS_ADDR_VALUE, TRANSFER_MODULE_NAME, RECEIVING_TYPE_NAME) =>
        {
    ...
    match &param_ty.value {
        ...
        Type_::Apply(_, sp!(_, n), targs)
            if n.is(&STD_ADDR_VALUE, ASCII_MODULE_NAME, ASCII_TYPE_NAME)
                ...
                || n.is(&MYS_ADDR_VALUE, OBJECT_MODULE_NAME, ID_TYPE_NAME) =>
        {
...
fn exp(context: &mut Context, e: &T::Exp) {
    match &e.exp.value {
        T::UnannotatedExp_::ModuleCall(mcall) => {
            ...
            if module.value.is(&MYS_ADDR_VALUE, EVENT_MODULE_NAME)
                && name.value() == EVENT_FUNCTION_NAME
            {
                check_event_emit(context, e.exp.loc, mcall)
            }
            let is_transfer_module = module.value.is(&MYS_ADDR_VALUE, TRANSFER_MODULE_NAME);
...
fn otw_special_cases(context: &Context) -> bool {
    BRIDGE_SUPPORTED_ASSET
        ...
        || context
            .current_module()
            .value
            .is(&MYS_ADDR_VALUE, MYS_MODULE_NAME)
}
...
fn check_private_transfer(context: &mut Context, loc: Loc, mcall: &ModuleCall) {
    ...
    if current_module
        .value
        .is(&MYS_ADDR_VALUE, TRANSFER_FUNCTION_NAME)
    {
