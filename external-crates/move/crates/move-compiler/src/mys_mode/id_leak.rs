...
use crate::{
    ...
    mys_mode::{
        ...
        OBJECT_NEW_UID_FROM_HASH, RANDOMNESS_MODULE_NAME, RANDOMNESS_STATE_CREATE, MYS_ADDR_NAME,
        MYS_ADDR_VALUE, MYS_CLOCK_CREATE, MYS_SYSTEM_ADDR_VALUE, MYS_SYSTEM_CREATE,
        MYS_SYSTEM_MODULE_NAME, TEST_SCENARIO_MODULE_NAME, TS_NEW_OBJECT, UID_TYPE_NAME,
    },
};
...
pub const FRESH_ID_FUNCTIONS: &[(AccountAddress, Symbol, Symbol)] = &[
    (MYS_ADDR_VALUE, OBJECT_MODULE_NAME, OBJECT_NEW),
    (MYS_ADDR_VALUE, OBJECT_MODULE_NAME, OBJECT_NEW_UID_FROM_HASH),
    (MYS_ADDR_VALUE, TEST_SCENARIO_MODULE_NAME, TS_NEW_OBJECT),
];
pub const FUNCTIONS_TO_SKIP: &[(AccountAddress, Symbol, Symbol)] = &[
    (
        MYS_SYSTEM_ADDR_VALUE,
        MYS_SYSTEM_MODULE_NAME,
        MYS_SYSTEM_CREATE,
    ),
    (MYS_ADDR_VALUE, CLOCK_MODULE_NAME, MYS_CLOCK_CREATE),
    (
        MYS_ADDR_VALUE,
    ...
    (MYS_ADDR_VALUE, DENY_LIST_MODULE_NAME, DENY_LIST_CREATE),
...
impl<'a> SimpleAbsInt for IDLeakVerifierAI<'a> {
    ...
    fn exp_custom(
        ...
        if !matches!(first_value, Value::FreshID(_)) {
            ...
            let uid_msg = format!(
                "The UID must come directly from {mys}::{object}::{new}. \
                Or for tests, it can come from {mys}::{ts}::{ts_new}",
                mys = MYS_ADDR_NAME,
...
fn value_for_ty(loc: &Loc, sp!(_, t): &SingleType) -> Value {
    if t.is_apply(&MYS_ADDR_VALUE, OBJECT_MODULE_NAME, UID_TYPE_NAME)
        .is_some()
    {
        Value::NotFresh(*loc)
    } else {
