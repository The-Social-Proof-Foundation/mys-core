...
pub enum PublicForTesting {
    ...
    Entry(Loc),
    // TODO we should allow calling init in unit tests, but this would need Mys bytecode verifier
...
pub fn public_testing_visibility(
    ...
    // is_testing && (is_entry || is_mys_init)
    if !env.flags().is_testing() {
        return None;
    }

    // TODO support mys init functions
    // let flavor = env.package_config(package).flavor;
    // flavor == Flavor::Mys && callee_name.value() == INIT_FUNCTION_NAME
