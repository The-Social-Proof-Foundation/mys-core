...
fn unused_module_members(context: &mut Context, mident: &ModuleIdent_, mdef: &T::ModuleDefinition) {
    ...
    let is_mys_mode = context.env.package_config(mdef.package_name).flavor == Flavor::Mys;
    ...
    for (loc, name, fun) in &mdef.functions {
        ...
        if is_mys_mode && *name == mys_mode::INIT_FUNCTION_NAME {
            // a Mys-specific filter to avoid signaling that the init function is unused
            continue;
        }
