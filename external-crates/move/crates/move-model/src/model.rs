...
const MYS_FRAMEWORK_ADDRESS: AccountAddress = address_from_single_byte(2);
...
/// Return true if `f` is a Mys framework function declared in `module` with a name in `names`
fn is_framework_function(f: &FunctionRef, module: &str, names: Vec<&str>) -> bool {
    *f.module_id.address() == MYS_FRAMEWORK_ADDRESS
