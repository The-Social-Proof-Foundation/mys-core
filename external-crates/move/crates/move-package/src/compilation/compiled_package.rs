...
impl CompiledPackage {
    ...
    pub(crate) fn build_for_driver<W: Write, T>(
        ...
        let mys_mode = resolution_graph
            .build_options
            .default_flavor
            .map_or(false, |f| f == Flavor::Mys);

        let mut compiler = Compiler::from_package_paths(vfs_root, paths, bytecode_deps)
            .unwrap()
            .set_flags(flags);
        if mys_mode {
            let (filter_attr_name, filters) = mys_mode::linters::known_filters();
            compiler = compiler
                .add_custom_known_filters(filter_attr_name, filters)
                .add_visitors(mys_mode::linters::linter_visitors(lint_level))
        }
