...
Clone MySocial repository into `$MYS` directory:

```bash
git clone https://github.com/MystenLabs/mys.git "$MYS"
```

Go to `"$MYS"/external-crates/move/crates/move-analyzer/prettier-plugin` and run the following
command:
...
``` bash
npm install prettier@3.1.1 "$MYS"/external-crates/move/crates/move-analyzer/prettier-plugin
```
