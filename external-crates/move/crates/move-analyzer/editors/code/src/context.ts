...
import {
    ...
    MYS_PATH_OPT, SERVER_PATH_OPT, Configuration,
} from './configuration';
...
export class Context {
    ...
    registerOnDidChangeConfiguration(): void {
        vscode.workspace.onDidChangeConfiguration(async event => {
            ...
            const mys_path_conf = MOVE_CONF_NAME.concat('.').concat(MYS_PATH_OPT);
            ...
            const pathsChanged = event.affectsConfiguration(server_path_conf) ||
                event.affectsConfiguration(mys_path_conf);
    ...
    async installServerBinary(extensionContext: vscode.ExtensionContext): Promise<boolean> {
        ...
        const cliVersionString = version(this.configuration.mysPath, cliVersionArgs);
        ...
        if (cliVersion !== null && (highestVersion === null || semver.gt(cliVersion, highestVersion))) {
            ...
            this.resolvedServerPath = this.configuration.mysPath;
