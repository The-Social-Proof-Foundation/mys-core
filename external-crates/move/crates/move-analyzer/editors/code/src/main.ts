...
async function mysMoveCmd(context: Readonly<Context>, cmd: string): Promise<void> {
    const version = childProcess.spawnSync(
        context.configuration.mysPath, ['--version'], { encoding: 'utf8' },
    );
    if (version.stdout) {
        const pkgRoot = await findPkgRoot();
        if (pkgRoot !== undefined) {
            const terminalName = 'mys move';
            ...
            terminal.sendText(`${context.configuration.mysPath} move ${cmd}`, true);
        }
    } else {
        await vscode.window.showErrorMessage(
            `A problem occurred when executing the Mys command: '${context.configuration.mysPath}'`
            + 'Make sure that Mys CLI is installed and available, either in your global PATH, '
            + 'or on a path set via `move.mys.path` configuration option.',
        );
    }
}
...
async function buildProject(context: Readonly<Context>): Promise<void> {
    return mysMoveCmd(context, 'build');
}
