...
#
# Before running the script, you need to download MySocial binaries to a freshly created directory
# (make sure that there are no other files in this directory). MySocial binary must be a compressed
# tarball (with tgz extension) and its name has to have the following format:
#   mys-VERSION-SHA-PLATFORM.tgz
#
# where:
# - VERSION    is the MySocial binary version (e.g., "v.1.37.0)
# - SHA        is first 10 characgers of commit sha for the version of MySocial repo's main branch
...
usage() {
    ...
    >&2 echo " BINDIR        Directory containing pre-built MySocial binaries"
}
...
do
    if [[ "$cmd" == "-h" ]]; then
        ...
        if [[ ! -d "$BIN_DIR" ]]; then
            echo MySocial binary directory $BIN_DIR does not exist
            usage $0
            exit 1
        fi
    fi
...
if [[ $BIN_DIR == "" ]]; then
    # directory storing MySocial binaries have not been defined
    usage $0
    exit 1
fi

# a map from os version identifiers in MySocial's binary distribution to os version identifiers
...
if (( ${#BIN_FILES[@]} != 4 )); then
    echo "MySocial binary directory $BIN_DIR should only contain binaries for the four supported platforms"
    exit 1
fi


for MYS_ARCHIVE_PATH in "${BIN_FILES[@]}"; do
    # Extract just the file name
    FILE_NAME=${MYS_ARCHIVE_PATH##*/}
    ...
    if [[ ! -v SUPPORTED_OS[$DIST_OS] ]]; then
        echo "Found MySocial binary archive for a platform that is not supported:  $MYS_ARCHIVE_PATH"
    ...
    tar -xf $MYS_ARCHIVE_PATH --directory $TMP_DIR/$DIST_OS
