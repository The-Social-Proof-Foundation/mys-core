...
for DIST_OS VSCODE_OS in "${(@kv)SUPPORTED_OS}"; do
    # MySocial distribution identifier
    MYS_DISTRO=$NETWORK"-v"$VERSION
    # name of the MySocial distribution archive file, for example mys-testnet-v1.13.0-macos-arm64.tgz
    MYS_ARCHIVE="mys-"$MYS_DISTRO"-"$DIST_OS".tgz"
    # a path to downloaded MySocial archive
    MYS_ARCHIVE_PATH=$TMP_DIR"/"$MYS_ARCHIVE

    # download MySocial archive file to a given location and uncompress it
    curl https://github.com/MystenLabs/mys/releases/download/"$MYS_DISTRO"/"$MYS_ARCHIVE" -L -o $MYS_ARCHIVE_PATH
    tar -xf $MYS_ARCHIVE_PATH --directory $TMP_DIR

    # names of the move-analyzer binary, both the one becoming part of the extension ($SERVER_BIN)
    # and the one in the MySocial archive ($ARCHIVE_SERVER_BIN)
