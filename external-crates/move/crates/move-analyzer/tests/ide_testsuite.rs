...
fn move_ide_testsuite(test_path: &Path) -> datatest_stable::Result<()> {
    ...
    let output = match suite {
        TestSuite::UseDef {
            project,
            file_tests,
        } => use_def_test_myste(project, file_tests),
        TestSuite::Completion {
            project,
            file_tests,
        } => completion_test_myste(project, file_tests),
        TestSuite::Cursor {
            project,
            file_tests,
        } => cursor_test_myste(project, file_tests),
        TestSuite::Hint {
            project,
            file_tests,
        } => hint_test_myste(project, file_tests),
    }?;
