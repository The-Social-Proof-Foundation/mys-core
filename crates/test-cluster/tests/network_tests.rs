...
use mys_types::{
    ...
    MYS_FRAMEWORK_PACKAGE_ID, MYS_SYSTEM_ADDRESS, MYS_SYSTEM_PACKAGE_ID,
};
...
async fn test_additional_objects() {
    ...
    assert!(matches!(resp, MysObjectResponse { data: Some(_), .. }));
}
...
async fn test_package_override() {
    ...
    let framework_ref = {
        ...
        let obj = client
            .get_object(MYS_SYSTEM_PACKAGE_ID, None)
            .await
            .unwrap();
    ...
    let modified_ref = {
        let mut framework_modules = BuiltInFramework::get_package_by_id(&MYS_SYSTEM_PACKAGE_ID)
            .modules()
            .to_vec();

        // Create an empty module that is pretending to be part of the mys framework.
        ...
        test_module.address_identifiers[address_idx] = MYS_SYSTEM_ADDRESS;

        // Add the dummy module to the rest of the mys-frameworks.  We can't replace the framework
