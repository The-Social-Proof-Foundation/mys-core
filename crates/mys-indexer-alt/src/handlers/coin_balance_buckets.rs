...
use mys_indexer_alt_framework::FieldCount;
...
use mys_indexer_alt_framework::types::TypeTag;
use mys_indexer_alt_framework::types::base_types::ObjectID;
...
use mys_indexer_alt_framework::types::object::Object;
use mys_indexer_alt_framework::types::object::Owner;
use mys_indexer_alt_schema::objects::StoredCoinBalanceBucket;
use mys_indexer_alt_schema::objects::StoredCoinBalanceBucketDeletionReference;
use mys_indexer_alt_schema::objects::StoredCoinOwnerKind;
use mys_indexer_alt_schema::schema::coin_balance_buckets;
use mys_indexer_alt_schema::schema::coin_balance_buckets_deletion_reference;
...
impl Handler for CoinBalanceBuckets {
    ...
    async fn prune<'a>(
        ...
        //
        // TODO: use mys_sql_macro's query!
        let query = format!(
            "
...
pub(crate) fn get_coin_owner(object: &Object) -> Option<(StoredCoinOwnerKind, MysAddress)> {
    match object.owner() {
...
mod tests {
    ...
    use mys_indexer_alt_framework::types::base_types::MoveObjectType;
    use mys_indexer_alt_framework::types::base_types::ObjectID;
    use mys_indexer_alt_framework::types::base_types::SequenceNumber;
    use mys_indexer_alt_framework::types::base_types::MysAddress;
    use mys_indexer_alt_framework::types::base_types::dbg_addr;
    use mys_indexer_alt_framework::types::digests::TransactionDigest;
    use mys_indexer_alt_framework::types::gas_coin::GAS;
    use mys_indexer_alt_framework::types::object::MoveObject;
    use mys_indexer_alt_framework::types::object::Object;
    ...
    fn test_get_coin_balance_bucket() {
        ...
        let zero_coin = Object::with_id_owner_gas_for_testing(id, MysAddress::ZERO, 0);
        assert_eq!(get_coin_balance_bucket(&zero_coin).unwrap(), 0);

        // Test coin with balance 1 (10^0)
        let one_coin = Object::with_id_owner_gas_for_testing(id, MysAddress::ZERO, 1);
        assert_eq!(get_coin_balance_bucket(&one_coin).unwrap(), 0);

        // Test coin with balance 100 (10^2)
        let hundred_coin = Object::with_id_owner_gas_for_testing(id, MysAddress::ZERO, 100);
        assert_eq!(get_coin_balance_bucket(&hundred_coin).unwrap(), 2);

        // Test coin with balance 1000000 (10^6)
        let million_coin = Object::with_id_owner_gas_for_testing(id, MysAddress::ZERO, 1000000);
        assert_eq!(get_coin_balance_bucket(&million_coin).unwrap(), 6);

        // The type of this object is a staked MYS, not a coin.
        let invalid_coin = unsafe {
            Object::new_move(
                MoveObject::new_from_execution(
                    MoveObjectType::staked_mys(),
                    false,
                    SequenceNumber::new(),
                ...
                .unwrap(),
                Owner::AddressOwner(MysAddress::ZERO),
    ...
    fn test_get_coin_owner() {
        let id = ObjectID::random();
        let addr1 = MysAddress::random_for_testing_only();
    ...
    #[tokio::test]
    async fn test_process_coin_balance_buckets_new_mys_coin() {
        let (indexer, _db) = Indexer::new_for_testing(&MIGRATIONS).await;
        let mut conn = indexer.store().connect().await.unwrap();
        let mut builder = TestCheckpointBuilder::new(0);
        builder = builder
            .start_transaction(0)
            .create_mys_object(0, 0)
            .create_mys_object(1, 100)
            .finish_transaction();
    ...
    async fn test_process_coin_balance_buckets_out_of_order_pruning() {
        ...
        builder = builder
            .start_transaction(0)
            .create_mys_object(0, 100)
            .create_mys_object(1, 1000)
            .create_mys_object(2, 10000)
            .finish_transaction();
