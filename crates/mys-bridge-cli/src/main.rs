...
use mys_bridge::mys_transaction_builder::build_mys_transaction;
use mys_bridge::types::BridgeActionType;
use mys_bridge::utils::{
...
use mys_bridge::utils::{get_eth_contracts, EthBridgeContracts};
use mys_bridge_cli::{
    ...
    match args.command {
        ...
        BridgeCommand::Governance {
            ...
            let mys_bridge_client =
                MysClient::<MysSdkClient>::new(&config.mys_rpc_url, metrics.clone()).await?;
            ...
            // Handle Mys Side
            if chain_id.is_mys_chain() {
                let mys_chain_id = BridgeChainId::try_from(bridge_summary.chain_id).unwrap();
                assert_eq!(
                    mys_chain_id, chain_id,
                    "Chain ID mismatch, expected: {:?}, got from url: {:?}",
                    chain_id, mys_chain_id
                );
                // Create BridgeAction
                let mys_action = make_action(mys_chain_id, &cmd);
                println!("Action to execute on Mys: {:?}", mys_action);
                ...
                let bridge_arg = mys_bridge_client
                    ...
                    .await;
                let rgp = mys_bridge_client
                    .get_reference_gas_price_until_success()
                    .await;
                let id_token_map = mys_bridge_client.get_token_id_map().await.unwrap();
                ...
                let tx = Transaction::from_data(tx, vec![mys_sig]);
                ...
                if resp.status_ok().unwrap() {
                    println!("Mys Transaction succeeded: {:?}", resp.digest);
                } else {
                    println!(
                        "Mys Transaction failed: {:?}. Effects: {:?}",
        ...
        BridgeCommand::ViewBridgeRegistration { mys_rpc_url } => {
            let metrics = Arc::new(BridgeMetrics::new_for_testing());
            let mys_bridge_client = MysClient::<MysSdkClient>::new(&mys_rpc_url, metrics).await?;
            let bridge_summary = mys_bridge_client
                .get_bridge_summary()
                .await
                ...
                .map(|summary| {
                    let protocol_key =
                        AuthorityPublicKeyBytes::from_bytes(&summary.protocol_pubkey_bytes)
                            .unwrap();
                    (summary.mys_address, (protocol_key, summary.name))
                })
                .collect::<HashMap<_, _>>();
            let mut authorities = vec![];
            let mut output_wrapper = Output::<OutputMysBridgeRegistration>::default();
            for (_, member) in move_type_bridge_committee.member_registration {
                ...
                let Ok(pubkey) = BridgeAuthorityPublicKey::from_bytes(&bridge_pubkey_bytes) else {
                    output_wrapper.add_error(format!(
                        ...
                        mys_address, bridge_pubkey_bytes
                    ));
                    continue;
                };
                ...
                let Ok(url) = from_utf8(&http_rest_url) else {
                    output_wrapper.add_error(format!(
                        ...
                        mys_address, http_rest_url
                    ));
                    continue;
                };
                let url = url.to_string();

                let (protocol_key, name) = names.get(&mys_address).unwrap();
                let stake = stakes.get(protocol_key).unwrap();
                authorities.push((name, mys_address, pubkey, eth_address, url, stake));
            }
            let total_stake = authorities
                .iter()
                .map(|(_, _, _, _, _, stake)| **stake)
                .sum::<u64>();
            let mut output = OutputMysBridgeRegistration {
                total_registered_stake: total_stake as f32 / TOTAL_VOTING_POWER as f32 * 100.0,
                ..Default::default()
            };
            for (name, mys_address, pubkey, eth_address, url, stake) in authorities {
        ...
        BridgeCommand::ViewMysBridge {
            ...
            let names = mys_client
                ...
                .active_validators
                .into_iter()
                .map(|summary| (summary.mys_address, summary.name))
                .collect::<HashMap<_, _>>();
            let mut authorities = vec![];
            ...
            let client = reqwest::Client::builder()
                ...
                .build()
                .unwrap();
            let mut output_wrapper = Output::<OutputMysBridge>::default();
            for (_, member) in move_type_bridge_committee.members {
                ...
                let name = names.get(&mys_address).cloned().unwrap_or(url.clone());
            ...
            let total_stake = authorities
                ...
                .sum::<u64>();
            let mut output = OutputMysBridge {
            ...
            for ((name, mys_address, pubkey, eth_address, url, stake, blocklisted), ping_resp) in
        ...
        BridgeCommand::Client { config_path, cmd } => {
            ...
            let metrics = Arc::new(BridgeMetrics::new_for_testing());
            let mys_bridge_client =
                MysClient::<MysSdkClient>::new(&config.mys_rpc_url, metrics).await?;
            cmd.handle(&config, mys_bridge_client).await?;
            return Ok(());
        }
    }
...
#[derive(serde::Serialize, Default)]
struct OutputMysBridgeRegistration {
