...
async fn test_publish_complex_value() {
    ...
    for i in 1..200 {
        ...
        builder.programmable_move_call(
            ...
            module.clone(),
            submit_data.clone(),
            vec![parse_mys_type_tag(&format!("{package}::decimal_value::DecimalValue")).unwrap()],
...
async fn test_consume_oracle_data() {
    ...
    for _ in 0..3 {
        ...
        let ticker = builder
            .input(CallArg::Pure(bcs::to_bytes("MYSUSD".as_bytes()).unwrap()))
            .unwrap();
        let identifier = builder
            .input(CallArg::Pure(
                bcs::to_bytes("identifier".as_bytes()).unwrap(),
...
async fn get_gas(client: &MysClient, sender: MysAddress) -> (ObjectRef, u64) {
    let gas = client
        .coin_read_api()
...
async fn init_test_client() -> (MysClient, Keystore, MysAddress) {
    ...
    let keystore = Keystore::File(
        FileBasedKeystore::new(
            &dirs::home_dir()
                .unwrap()
                .join(".mys/mys_config/mys.keystore"),
        )
        .unwrap(),
    );
    let sender: MysAddress = keystore.addresses()[0];
    let gas = client
        .coin_read_api()
        .get_coins(sender, None, None, Some(1))
        .await
