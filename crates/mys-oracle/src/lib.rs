...
use mys_sdk::rpc_types::MysObjectResponse;
...
use mys_types::base_types::{random_object_ref, ObjectID, ObjectRef};
...
async fn get_gas_obj_ref(
    read_api: &ReadApi,
    gas_obj_id: ObjectID,
    owner_address: MysAddress,
) -> ObjectRef {
    loop {
        match read_api
            .get_object_with_options(gas_obj_id, MysObjectDataOptions::default().with_owner())
            .await
            .map(|resp| resp.data)
        {
            Ok(Some(gas_obj)) => {
...
impl OnChainDataUploader {
    ...
    async fn upload(
        ...
        for data_point in &data_points {
            ...
            builder.command(Command::move_call(
                ...
                vec![
                    parse_mys_type_tag(&format!("{package_id}::decimal_value::DecimalValue"))
                        .unwrap(),
                ],
                arguments,
            ));
    ...
    async fn execute(&mut self, tx: Transaction) -> anyhow::Result<MysTransactionBlockResponse> {
        ...
        loop {
            match self
                ...
                Err(mys_sdk::error::Error::RpcError(err)) => {
                    // jsonrpsee translate every MysError into jsonrpsee::core::Error, so we need to further distinguish 
        ...
        loop {
            ...
            for (feed_name, object_id) in &self.read_configs {
                match self
                    .client
                    .read_api()
                    .get_object_with_options(*object_id, MysObjectDataOptions::default())
                    .await
                {
                    Ok(MysObjectResponse {
                        data: Some(_data), ..
                    }) => {
