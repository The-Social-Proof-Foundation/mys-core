...
use mys_types::transaction::{CallArg, ObjectArg, ProgrammableTransaction};
use mys_types::{MOVE_STDLIB_PACKAGE_ID, MYS_FRAMEWORK_PACKAGE_ID};
...
fn publish_coin_factory(
    ...
    let effects = exec.publish(
        "coin_factory",
        vec![MOVE_STDLIB_PACKAGE_ID, MYS_FRAMEWORK_PACKAGE_ID],
        account,
    );
    let package = effects
        .created()
