...
impl TransactionSponsorship {
    pub fn select_gas(
        ...
        exec: &mut Executor,
        gas_coins: u32,
    ) -> (Vec<ObjectRef>, (u64, Object), MysAddress) {
        match self {
...
impl AUTransactionGen for P2PTransferGenRandomGasRandomPriceRandomSponsorship {
    fn apply(
        ...
        // construct a p2p transfer of a random amount of MYS
        let txn = {
            let mut builder = ProgrammableTransactionBuilder::new();
            builder.transfer_mys(recipient.initial_data.account.address, Some(self.amount));
            builder.finish()
        };
