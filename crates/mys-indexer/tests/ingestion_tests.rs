...
use mys_indexer::schema::events;
use mys_indexer::schema::full_objects_history;
use mys_indexer::schema::objects_history;
use mys_indexer::schema::{checkpoints, objects, objects_snapshot, transactions};
use mys_indexer::store::indexer_store::IndexerStore;
use mys_indexer::test_utils::set_up_on_mvr_mode;
use mys_indexer::test_utils::{
    set_up, set_up_with_start_and_end_checkpoints, wait_for_checkpoint, wait_for_objects_snapshot,
};
use mys_indexer::types::EventIndex;
use mys_indexer::types::IndexedDeletedObject;
use mys_indexer::types::IndexedObject;
use mys_indexer::types::TxIndex;
...
pub async fn test_transaction_table() -> Result<(), IndexerError> {
    ...
    let transfer_recipient = MysAddress::random_for_testing_only();
...
pub async fn test_object_type() -> Result<(), IndexerError> {
    ...
    assert_eq!(
        ...
        Some(MYS_FRAMEWORK_PACKAGE_ID.to_vec())
    );
