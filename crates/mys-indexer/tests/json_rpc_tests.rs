...
use mys_json_rpc_api::{CoinReadApiClient, IndexerApiClient, ReadApiClient};
use mys_json_rpc_types::{
    CoinPage, EventFilter, MysObjectDataOptions, MysObjectResponse, MysObjectResponseQuery,
};
use mys_swarm_config::genesis_config::DEFAULT_GAS_AMOUNT;
use mys_test_transaction_builder::publish_package;
use mys_types::{event::EventID, transaction::CallArg};
...
async fn test_get_owned_objects() -> Result<(), anyhow::Error> {
    ...
    let data_option = MysObjectDataOptions::new().with_owner();
...
async fn test_get_coins() -> Result<(), anyhow::Error> {
    ...
    // We should get all the 5 coins for MYS with the right balance.
    let result: CoinPage = http_client
