...
use mys_json_rpc_types::MysObjectDataFilter;
...
impl DBFilter<ObjectID> for MysObjectDataFilter {
...
fn to_latest_objects_clauses(filter: &MysObjectDataFilter) -> Option<String> {
    match filter {
        MysObjectDataFilter::AddressOwner(a) => Some(format!(
...
fn to_clauses(filter: &MysObjectDataFilter) -> Option<String> {
    match filter {
        MysObjectDataFilter::MatchAll(sub_filters) => {
        ...
        MysObjectDataFilter::MatchAny(sub_filters) => {
            let sub_filters = sub_filters.iter().flat_map(to_clauses).collect::<Vec<_>>();
            if sub_filters.is_empty() {
        ...
        MysObjectDataFilter::MatchNone(sub_filters) => {
            let sub_filters = sub_filters.iter().flat_map(to_clauses).collect::<Vec<_>>();
            if sub_filters.is_empty() {
                None
            } else {
        ...
        MysObjectDataFilter::Package(p) => Some(format!("o.object_type LIKE '{}::%'", p.to_hex_literal())),
        MysObjectDataFilter::MoveModule { package, module } => Some(format!(
            "o.object_type LIKE '{}::{}::%'",
        ...
        MysObjectDataFilter::ObjectId(id) => {
            Some(format!("o.object_id = '{id}'"))
        }
        MysObjectDataFilter::ObjectIds(ids) => {
            if ids.is_empty() {
                None
            } else {
                let ids = ids
        ...
        MysObjectDataFilter::Version(v) => Some(format!("o.version = {v}")),
    }
}

fn to_outer_clauses(filter: &MysObjectDataFilter) -> Option<String> {
    match filter {
        MysObjectDataFilter::MatchNone(sub_filters) => {
        ...
        MysObjectDataFilter::AddressOwner(a) => Some(format!("t1.owner_address = '{a}'")),
        _ => None,
    }
}
...
mod test {
    ...
    fn test_address_filter() {
        let address = MysAddress::from_str(
            "0x92dd4d9b0150c251661d821583ef078024ae9e9ee11063e216500861eec7f381",
        )
        .unwrap();
        let filter = MysObjectDataFilter::AddressOwner(address);
...
LIMIT 100;";
    ...
    fn test_move_module_filter() {
        let filter = MysObjectDataFilter::MoveModule {
...
LIMIT 100;";
    ...
    fn test_empty_all_filter() {
        let filter = MysObjectDataFilter::MatchAll(vec![]);
        let expected_sql = "SELECT t1.*
FROM (SELECT DISTINCT ON (o.object_id) *
      FROM objects_history o
      WHERE o.checkpoint <= $1
...
LIMIT 100;";
    ...
    fn test_empty_any_filter() {
        let filter = MysObjectDataFilter::MatchAny(vec![]);
        let expected_sql = "SELECT t1.*
FROM (SELECT DISTINCT ON (o.object_id) *
      FROM objects_history o
      WHERE o.checkpoint <= $1
...
LIMIT 100;";
    ...
    fn test_all_filter() {
        let filter = MysObjectDataFilter::MatchAll(vec![
            MysObjectDataFilter::ObjectId(
                ObjectID::from_str(
                    "0xef9fb75a7b3d4cb5551ef0b08c83528b94d5f5cd8be28b1d08a87dbbf3731738",
                )
                .unwrap(),
            ),
            MysObjectDataFilter::StructType(parse_mys_struct_tag("0x2::test::Test").unwrap()),
        ]);
