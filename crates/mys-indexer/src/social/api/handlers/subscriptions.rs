...
pub async fn get_subscriptions(
    ...
    let build_query = || {
        ...
        if params.active_only.unwrap_or(false) {
            ...
            query = query.filter(
                schema::profile_subscriptions::cancelled_at
                    .is_null()
...
pub async fn get_subscription_services(
    ...
    let build_query = || {
        ...
        if let Some(profile_owner) = &params.profile_owner {
            query = query
                .filter(schema::profile_subscription_services::profile_owner.eq(profile_owner));
        }
...
async fn calculate_service_performance(
    ...
    if let Some(profile_owner) = &params.profile_owner {
        query =
            query.filter(schema::profile_subscription_services::profile_owner.eq(profile_owner));
    }
