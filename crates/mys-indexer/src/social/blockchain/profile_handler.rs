...
use crate::social::events::profile_event_types::{extract_profile_id, ProfileEventType};
...
use crate::social::models::profile_extras::{NewProfileOffer, NewProfileSaleFee, NewProfileBadge};
use crate::social::models::{VestingWallet, UpdateVestingWallet, NewEcosystemTreasury};
use crate::social::schema;
use crate::social::PROFILE_MODULE_NAME;
...
impl ProfileEventListener {
    ...
    async fn process_profile_updated_with_context(
        ...
        let existing_profile = schema::profiles::table
            ...
            .first::<crate::social::models::profile::Profile>(&mut conn)
            .await
    ...
    async fn process_profile_sale_fee(
        ...
        match crate::social::models::get_current_treasury_address(&mut conn).await {
            Ok(current_treasury) => {
    ...
    async fn process_badge_selected(
        ...
        let badge_exists = schema::profile_badges::table
            ...
            .filter(schema::profile_badges::badge_id.eq(&event.badge_id))
            .filter(schema::profile_badges::revoked.eq(false))
            .first::<crate::social::models::profile_extras::ProfileBadge>(&mut conn)
            .await;
    ...
    async fn process_vesting_wallet_deleted(
        ...
        match wallet_exists {
            Ok(wallet) => {
                ...
                let deletion_event = crate::social::models::NewVestingEvent {
    ...
    async fn handle_generic_profile_event(
        ...
        let fields = crate::social::events::event_utils::extract_event_fields(&blockchain_event.data)?;
        ...
        while let Some(event) = self.receiver.recv().await {
            ...
            if event.event_type.contains("::profile::") {
                ...
                if event.event_type.ends_with("::ProfileCreatedEvent") {
                    ...
                    match crate::social::events::event_utils::extract_event_fields(&event.data).and_then(
                        |fields| {
    ...
    pub async fn process_event(
        ...
        match function_name {
            "create_profile" | "ProfileCreated" => {
                ...
                match crate::social::events::event_utils::extract_event_fields(event_data).and_then(
                    |fields| {
                        serde_json::from_value::<ProfileCreatedEvent>(fields)
                            .map_err(|e| anyhow::anyhow!(e))
                    },
