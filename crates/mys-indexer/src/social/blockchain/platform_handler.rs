...
use crate::social::models::governance::{NewGovernanceRegistry, NewGovernanceEvent};
...
impl PlatformEventHandler {
    ...
    fn validate_platform_categories(primary: &str, secondary: Option<&str>) -> Result<(), anyhow::Error> {
        match crate::social::models::platform::validate_categories(primary, secondary) {
            Ok(()) => Ok(()),
            Err(e) => {
                    ...
                    if has_dao_governance {
                        if let Some(registry_id) = governance_registry_id_for_governance.as_ref() {
                            ...
                            let new_registry = NewGovernanceRegistry {
                                ...
                                voting_period_ms: voting_period_epochs.map(|v| v as i64 * 86400000).unwrap_or(0), // Convert epochs to milliseconds (assuming 1 epoch = 1 day)
                            ...
                            let registry_result = if existing_registry_id.is_some() {
                                ...
                                diesel::update(schema::governance_registries::table)
                                    ...
                                    .set((
                                        ...
                                        schema::governance_registries::voting_period_ms.eq(new_registry.voting_period_ms),
                ...
                Box::pin(async move {
                    ...
                    if platform_exists {
                        ...
                        let updated_at = if let Some(timestamp_ms) = event_timestamp_ms {
                            ...
                            crate::social::models::platform::milliseconds_to_naive_datetime(timestamp_ms)
                        } else {
                            ...
                            if event.updated_at >= min_timestamp_ms && event.updated_at <= max_timestamp_ms {
                                ...
                                crate::social::models::platform::milliseconds_to_naive_datetime(event.updated_at)
                            } else {
            ...
            .run(|mut conn| {
                Box::pin(async move {
                    ...
                    if platform_exists {
                        ...
                        let approval_changed_at = crate::social::models::platform::milliseconds_to_naive_datetime(event.changed_at);
        ...
        conn.build_transaction()
            .run(|mut conn| {
                Box::pin(async move {
                    ...
                    let deleted_at = crate::social::models::platform::milliseconds_to_naive_datetime(event.timestamp);
                    ...
                    if !membership_exists {
                        ...
                        let joined_at = if let Some(timestamp_ms) = event_timestamp_ms {
                            ...
                            if event.timestamp >= min_timestamp_ms && event.timestamp <= max_timestamp_ms {
                                crate::social::models::platform::milliseconds_to_naive_datetime(event.timestamp)
                            } else {
                                // Doesn't look like a timestamp, use current time
                                chrono::DateTime::from_timestamp(now.as_secs() as i64, 0)
                                    .unwrap_or_else(|| chrono::Utc::now())
                        ...
                        if let Some(profile_id) = profile_id_opt {
                            ...
                            let platform_join_event =
                                crate::social::events::profile_event_types::PlatformJoinedEvent {
                            ...
                            let profile_event =
                                crate::social::models::profile_events::NewProfileEvent::from_platform_joined(
                                    &platform_join_event,
                    ...
                    if membership_exists {
                        ...
                        if let Some(profile_id) = profile_id_opt {
                            ...
                            let platform_left_event =
                                crate::social::events::profile_event_types::PlatformLeftEvent {
                                    profile_id: profile_id.clone(),
                                    platform_id: event.platform_id.clone(),
                                    timestamp: profile_event_timestamp,
                                };
                            ...
                            let profile_event =
                                crate::social::models::profile_events::NewProfileEvent::from_platform_left(
                                    &platform_left_event,
                                    event_id_for_profile,
                                );
        ...
        conn.build_transaction()
            .run(|mut conn| {
                Box::pin(async move {
                    ...
                    let airdrop_record = crate::social::models::platform::NewPlatformTokenAirdrop {
                        platform_id: event.platform_id.clone(),
    ...
    async fn process_event(&self, event: BlockchainEvent) -> Result<()> {
        ...
        let event_type = match crate::social::events::platform_events::PlatformEventType::from_str(&event.event_type) {
            Some(et) => {
