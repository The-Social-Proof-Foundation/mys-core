...
use crate::social::events::{event_utils::parse_json_event, parse_event};
...
impl PostEventHandler {
    ...
    async fn process_post_created(&self, event: &PostCreatedEvent, tx_id: &str, timestamp_ms: Option<u64>) -> Result<()> {
        ...
        match diesel::update(crate::social::schema::profiles::table)
            .filter(crate::social::schema::profiles::owner_address.eq(&event.owner))
            .set(crate::social::schema::profiles::post_count.eq(crate::social::schema::profiles::post_count + 1))
            .execute(&mut conn)
            .await
        {
            Ok(updated_rows) => {
    ...
    async fn process_deletion(&self, event: &PostDeletionEvent, tx_id: &str) -> Result<()> {
        ...
        if event.is_post {
            ...
            match diesel::update(crate::social::schema::profiles::table)
                .filter(crate::social::schema::profiles::owner_address.eq(&event.owner))
                .set(
                    crate::social::schema::profiles::post_count.eq(crate::social::schema::profiles::post_count - 1),
                )
                .execute(&mut conn)
                .await
            {
    ...
    async fn process_post_parameters_updated(
        ...
        .get_result::<crate::social::models::post::PostConfig>(&mut conn)
        .await
        ...
        let new_config = crate::social::models::post::NewPostConfig {
...
async fn handle_post_created(
    ...
    // Increment post_count for the profile (with graceful error handling)
    match diesel::update(crate::social::schema::profiles::table)
        .filter(crate::social::schema::profiles::owner_address.eq(&parsed_event.owner))
        .set(crate::social::schema::profiles::post_count.eq(crate::social::schema::profiles::post_count + 1))
        .execute(&mut conn)
        .await
    {
