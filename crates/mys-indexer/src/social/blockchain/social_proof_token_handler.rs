...
use crate::social::blockchain::handler_trait::{
...
impl SocialProofTokenHandler {
    ...
    fn extract_event_fields(data: &serde_json::Value) -> Result<serde_json::Value> {
        crate::social::events::event_utils::extract_event_fields(data)
    }
    ...
    async fn process_reservation_created_event(&mut self, event: &BlockchainEvent) -> Result<()> {
        ...
        let associated_owner = if reservation_event.associated_id.starts_with("post_") {
            ...
            if let Ok(post_owner) = crate::social::schema::posts::table
                .filter(crate::social::schema::posts::post_id.eq(&post_id))
                .select(crate::social::schema::posts::owner)
            ...
            if let Ok(profile_owner) = crate::social::schema::profiles::table
                .filter(crate::social::schema::profiles::profile_id.eq(&profile_id))
                .select(crate::social::schema::profiles::owner_address)
                .first::<String>(&mut conn)
                .await
            {
                Some(profile_owner)
    ...
    async fn process_token_pool_created_event(&mut self, event: &BlockchainEvent) -> Result<()> {
        ...
        let token_pool_event = serde_json::from_value::<
            crate::social::events::social_proof_token_events::TokenPoolCreatedEvent,
        >(fields)
