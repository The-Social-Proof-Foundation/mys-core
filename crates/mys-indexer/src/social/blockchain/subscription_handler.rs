...
use crate::social::models::subscription::*;
use crate::social::schema;
use crate::social::SUBSCRIPTION_MODULE_NAME;
...
impl SubscriptionEventHandler {
        ...
        while let Some(event) = self.receiver.recv().await {
            ...
            if let Err(e) = self.process_event(&mut conn, &event).await {
                ...
                if let Err(log_error) = self
                    .log_failed_event(&mut conn, &event, &e.to_string())
                    .await
                {
    ...
    async fn process_event(&self, conn: &mut DbConnection, event: &BlockchainEvent) -> Result<()> {
        ...
        if let Some(subscription_event_type) = SubscriptionEventType::from_str(&event.event_type) {
            match subscription_event_type {
                SubscriptionEventType::ProfileSubscriptionCreated => {
                    self.process_subscription_created(conn, &event_data, &event.tx_digest)
                        .await?;
                }
                SubscriptionEventType::ProfileSubscriptionRenewed => {
                    self.process_subscription_renewed(conn, &event_data, &event.tx_digest)
                        .await?;
                }
                SubscriptionEventType::ProfileSubscriptionCancelled => {
                    self.process_subscription_cancelled(conn, &event_data, &event.tx_digest)
                        .await?;
                }
        ...
        info!(
            "Successfully processed subscription event: {}",
            event.event_type
        );
        Ok(())
    }
        ...
        conn.transaction(|conn| {
            Box::pin(async move {
                ...
                let new_subscription =
                    event.into_model(Utc::now().timestamp() as u64, tx_id.to_string())?;
                ...
                diesel::update(schema::profile_subscription_services::table)
                    ...
                    .set(
                        schema::profile_subscription_services::subscriber_count
                            .eq(schema::profile_subscription_services::subscriber_count + 1),
                    )
                    .execute(conn)
                    .await?;
                ...
                let profile_owner = self
                    .get_profile_owner_for_service(conn, &event.service_id)
                    .await?;
        ...
        conn.transaction(|conn| {
            Box::pin(async move {
                ...
                let service_info: (String, i64, String) = schema::profile_subscriptions::table
                    .inner_join(
                        schema::profile_subscription_services::table
                            .on(schema::profile_subscriptions::service_id
                                .eq(schema::profile_subscription_services::service_id)),
                    )
                    .filter(
                        schema::profile_subscriptions::subscription_id.eq(&event.subscription_id),
                    )
                ...
                // 3. Create revenue record
                let revenue = event.into_revenue_model(
                    tx_id.to_string(),
                    service_id,
                    monthly_fee as u64,
                    profile_owner,
        ...
        conn.transaction(|conn| {
            Box::pin(async move {
                ...
                if event.refunded_amount > 0 {
                    let profile_owner = self
                        .get_profile_owner_for_service(conn, &service_id)
                        .await?;
                    if let Some(refund_revenue) = event.into_revenue_model(
                        tx_id.to_string(),
                        service_id.clone(),
                        profile_owner,
                    )? {
    ...
    async fn get_profile_owner_for_service(
        &self,
        conn: &mut DbConnection,
        service_id: &str,
    ) -> Result<String> {
...
pub async fn handle_subscription_event(db: &Arc<Database>, event: &BlockchainEvent) -> Result<()> {
...
mod tests {
    ...
    fn test_subscription_business_rules_validation() {
        ...
        assert!(
            validate_business_rules(&invalid_fee_event, "ProfileSubscriptionCreatedEvent").is_err()
        );
    }
}
