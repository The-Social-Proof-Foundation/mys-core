...
use crate::social::events::event_utils::{deserialize_u64_from_string, deserialize_u64_from_string_optional, deserialize_optional_u64_from_string};
...
impl TipEvent {
    ...
    ) -> anyhow::Result<crate::social::models::NewUnifiedRevenue> {
        let revenue_type = if self.is_post {
            crate::social::models::revenue::REVENUE_TYPE_TIPS_POST.to_string()
        } else {
            crate::social::models::revenue::REVENUE_TYPE_TIPS_COMMENT.to_string()
        };

        let content_type = if self.is_post {
            crate::social::models::revenue::CONTENT_TYPE_POST.to_string()
        } else {
            crate::social::models::revenue::CONTENT_TYPE_COMMENT.to_string()
        };

        Ok(crate::social::models::NewUnifiedRevenue::from_tip(
            revenue_type,
            self.to.clone(),
            self.amount as i64,
