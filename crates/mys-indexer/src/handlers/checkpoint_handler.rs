...
use mys_types::effects::{ObjectChange, TransactionEffectsAPI};
use mys_types::event::SystemEpochInfoEvent;
...
impl CheckpointHandler {
    ...
    async fn index_epoch(
        ...
        if *checkpoint_summary.sequence_number() == 0 {
            ...
            let system_state_summary =
                get_mys_system_state(&checkpoint_object_store)?.into_mys_system_state_summary();
        ...
        if epoch_event_opt.is_none() {
            ...
            assert!(
                ...
                "Mys is not in safe mode but no SystemEpochInfoEvent found at end of epoch {}",
                checkpoint_summary.epoch
            );
        }
    ...
    async fn index_checkpoint(
        ...
        let (checkpoint, db_transactions, db_events, db_tx_indices, db_event_indices, db_displays) = {
            ...
            (
                IndexedCheckpoint::from_mys_checkpoint(
                    checkpoint_summary,
                    checkpoint_contents,
        ...
        checkpoint_data
            ...
            .flat_map(|data| {
                ...
                data.transactions
                    ...
                    .flat_map(|tx| &tx.output_objects)
                    .filter_map(|o| {
                        if let mys_types::object::Data::Package(p) = &o.data {
                            Some(IndexedPackage {
                                package_id: o.id(),
