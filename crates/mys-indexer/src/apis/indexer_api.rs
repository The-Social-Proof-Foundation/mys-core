...
impl IndexerApi {
    ...
    async fn get_owned_objects_internal(
        ...
        let MysObjectResponseQuery { filter, options } = query.unwrap_or_default();
                    ...
                    ObjectRead::Exists(object_ref, o, layout) => {
                        if options.show_display {
                            match inner_clone.get_display_fields(&o, &layout).await {
                                Ok(rendered_fields) => Ok(MysObjectResponse::new_with_data(
                                    (object_ref, o, layout, options, Some(rendered_fields))
                                        .try_into()?,
                                )),
                                Err(e) => Ok(MysObjectResponse::new(
...
impl IndexerApiServer for IndexerApi {
    ...
    async fn get_dynamic_field_object(
        ...
        let id = mys_types::dynamic_field::derive_dynamic_field_id(
            parent_object_id,
            &name.type_,
            &name_bcs_value,
        )
        ...
        let options = mys_json_rpc_types::MysObjectDataOptions::full_content();
        match self.inner.get_object_read(id).await? {
            mys_types::object::ObjectRead::NotExists(_)
            | mys_types::object::ObjectRead::Deleted(_) => {}
            mys_types::object::ObjectRead::Exists(object_ref, o, layout) => {
                return Ok(MysObjectResponse::new_with_data(
        ...
        let dynamic_object_field_struct =
            mys_types::dynamic_field::DynamicFieldInfo::dynamic_object_field_wrapper(name.type_);
        let dynamic_object_field_type = TypeTag::Struct(Box::new(dynamic_object_field_struct));
        let dynamic_object_field_id = mys_types::dynamic_field::derive_dynamic_field_id(
            parent_object_id,
        ...
        Ok(MysObjectResponse::new_with_error(
            mys_types::error::MysObjectResponseError::DynamicFieldNotFound { parent_object_id },
        ))
    }
...
impl MysRpcModule for IndexerApi {
    ...
    fn rpc_doc_module() -> Module {
        mys_json_rpc_api::IndexerApiOpenRpc::module_doc()
    }
}
