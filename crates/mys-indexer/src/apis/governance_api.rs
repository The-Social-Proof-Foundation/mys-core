...
use mys_types::{
    ...
    mys_serde::BigInt,
    mys_system_state::{mys_system_state_summary::MysSystemStateSummary, PoolTokenExchangeRate},
};
...
impl GovernanceReadApi {
    ...
    async fn get_latest_mys_system_state(&self) -> Result<MysSystemStateSummary, IndexerError> {
        self.inner.get_latest_mys_system_state().await
    }
    ...
    async fn get_stakes_by_ids(
        ...
        for stored_object in self.inner.multi_get_objects(ids).await? {
            let object = mys_types::object::Object::try_from(stored_object)?;
            let stake_object = StakedMys::try_from(&object)?;
    ...
    async fn get_staked_by_owner(
        ...
        for stored_object in self
            ...
            .get_owned_objects(
                owner,
                Some(MysObjectDataFilter::StructType(
                    MoveObjectType::staked_mys().into(),
                )),
                None,
    ...
    pub async fn get_delegated_stakes(
        ...
        let system_state_summary = self.get_latest_mys_system_state().await?;
        ...
        for (pool_id, stakes) in pools {
            ...
            for stake in stakes {
                ...
                delegations.push(mys_json_rpc_types::Stake {
...
pub async fn exchange_rates(
    ...
    system_state_summary: &MysSystemStateSummary,
    ...
    for df in state
        ...
        .await?
    {
        let pool_id: mys_types::id::ID = bcs::from_bytes(&df.bcs_name).map_err(|e| {
            mys_types::error::MysError::ObjectDeserializationError {
                error: e.to_string(),
            }
        })?;
    ...
    for (address, pool_id, exchange_rates_id, exchange_rates_size, active) in tables {
            ...
            .await?
        {
            let dynamic_field = df
                ...
                .ok_or_else(|| mys_types::error::MysError::ObjectDeserializationError {
...
impl GovernanceReadApiServer for GovernanceReadApi {
    async fn get_stakes_by_ids(
        ...
        self.get_stakes_by_ids(staked_mys_ids)
            .await
            .map_err(Into::into)
    }
    ...
    async fn get_latest_mys_system_state(&self) -> RpcResult<MysSystemStateSummary> {
        self.get_latest_mys_system_state().await.map_err(Into::into)
    }
...
impl MysRpcModule for GovernanceReadApi {
    ...
    fn rpc_doc_module() -> Module {
        mys_json_rpc_api::GovernanceReadApiOpenRpc::module_doc()
    }
}
