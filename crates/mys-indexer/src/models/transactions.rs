...
impl StoredTransaction {
    ...
    pub async fn try_into_mys_transaction_block_response(
        self,
        options: MysTransactionBlockResponseOptions,
        package_resolver: Arc<Resolver<impl PackageStore>>,
    ) -> IndexerResult<MysTransactionBlockResponse> {
        ...
        let transaction = if options.show_input {
            ...
            let tx_block = MysTransactionBlock::try_from_with_package_resolver(
                sender_signed_data,
                &package_resolver,
            )
            .await?;
        ...
        let effects = if options.show_effects {
            let effects = self.try_into_mys_transaction_effects()?;
            Some(effects)
        } else {
            None
        };
        ...
        let events = if options.show_events {
            ...
            tx_events_to_mys_tx_events(tx_events, package_resolver, tx_digest, timestamp).await?
        } else {
            None
        };
        ...
        Ok(MysTransactionBlockResponse {
            digest: tx_digest,
            transaction,
            raw_transaction,
            effects,
    ...
    pub fn try_into_mys_transaction_effects(&self) -> IndexerResult<MysTransactionBlockEffects> {
        ...
        let effects = MysTransactionBlockEffects::try_from(effects)?;
        Ok(effects)
    }
}
...
pub async fn tx_events_to_mys_tx_events(
    tx_events: TransactionEvents,
    package_resolver: Arc<Resolver<impl PackageStore>>,
    tx_digest: TransactionDigest,
    timestamp: u64,
) -> Result<Option<MysTransactionBlockEvents>, IndexerError> {
    ...
    for tx_event in tx_events.data.clone() {
        ...
        mys_event_futures.push(tokio::task::spawn(async move {
    ...
    let event_move_type_layouts = futures::future::join_all(mys_event_futures)
        .await
        .into_iter()
        ...
        .map_err(|e| {
            IndexerError::ResolveMoveStructError(format!(
                "Failed to convert to mys event with Error: {e}",
            ))
        })?;
    ...
    let mys_events = tx_events
        ...
        .collect::<Result<Vec<_>, _>>()?;
    let mys_tx_events = MysTransactionBlockEvents { data: mys_events };
    Ok(Some(mys_tx_events))
}
