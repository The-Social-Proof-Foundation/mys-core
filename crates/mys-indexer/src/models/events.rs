...
impl StoredEvent {
    pub async fn try_into_mys_event(
        self,
        package_resolver: Arc<Resolver<impl PackageStore>>,
    ) -> Result<MysEvent, IndexerError> {
        ...
        // Note: MysEvent only has one sender today, so we always use the first one.
        let sender = {
        ...
        let sender = match sender {
            Some(ref s) => MysAddress::from_bytes(s).map_err(|_e| {
                IndexerError::PersistentStorageDataCorruptionError(format!(
        ...
        let type_ = parse_mys_struct_tag(&self.event_type)?;
