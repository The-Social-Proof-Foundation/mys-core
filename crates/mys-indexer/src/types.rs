...
pub fn owner_to_owner_info(owner: &Owner) -> (OwnerType, Option<MysAddress>) {
    match owner {
...
impl IndexedObject {
    pub fn random() -> Self {
        let mut rng = rand::thread_rng();
        let random_address = MysAddress::random_for_testing_only();
        IndexedObject {
...
pub struct IndexedTransaction {
    ...
    pub balance_change: Vec<mys_json_rpc_types::BalanceChange>,
    pub events: Vec<mys_types::event::Event>,
...
pub struct TxIndex {
    ...
    pub payers: Vec<MysAddress>,
    pub sender: MysAddress,
    pub recipients: Vec<MysAddress>,
...
// MysTransactionBlockResponseWithOptions is only used on the reading path
pub struct MysTransactionBlockResponseWithOptions {
    pub response: MysTransactionBlockResponse,
    pub options: MysTransactionBlockResponseOptions,
}

impl From<MysTransactionBlockResponseWithOptions> for MysTransactionBlockResponse {
    fn from(value: MysTransactionBlockResponseWithOptions) -> Self {
        let MysTransactionBlockResponseWithOptions { response, options } = value;
