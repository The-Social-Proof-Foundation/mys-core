...
//! provides the [`Simulacrum`] type which is a implementation or instantiation of a mys
...
use mys_types::messages_checkpoint::{CheckpointContents, CheckpointSequenceNumber};
use mys_types::mock_checkpoint_builder::{MockCheckpointBuilder, ValidatorKeypairProvider};
use mys_types::{
    gas_coin::GasCoin,
...
/// A `Simulacrum` of Mys.
///
/// This type represents a simulated instantiation of a Mys blockchain that needs to be driven
...
impl<R, S: store::SimulatorStore> Simulacrum<R, S> {
    pub fn new_with_network_config_store(config: &NetworkConfig, rng: R, store: S) -> Self {
        ...
        let epoch_state = EpochState::new(genesis.mys_system_object());
    ...
    /// let address = MysAddress::generate(simulacrum.rng());
    /// simulacrum.request_gas(address, MIST_PER_MYS).unwrap();
    ///
    /// // `account` now has a Coin<MYS> object with single MYS in it.
    /// // ...
    /// # }
    /// ```
    pub fn request_gas(&mut self, address: MysAddress, amount: u64) -> Result<TransactionEffects> {
        ...
        let object = self
            ...
            .find(|object| {
                object.is_gas_coin() && object.get_coin_value_unsafe() > amount + MIST_PER_MYS
            })
            .ok_or_else(|| {
                anyhow!("unable to find a coin with enough to satisfy request for {amount} Mist")
            })?;

        let gas_data = mys_types::transaction::GasData {
        ...
        let pt = {
            let mut builder =
                mys_types::programmable_transaction_builder::ProgrammableTransactionBuilder::new();
            builder.transfer_mys(address, Some(amount));
            builder.finish()
        };

        let kind = mys_types::transaction::TransactionKind::ProgrammableTransaction(pt);
        let tx_data =
            mys_types::transaction::TransactionData::new_with_gas_data(kind, *sender, gas_data);
...
impl<T, V: store::SimulatorStore> ReadStore for Simulacrum<T, V> {
    ...
    fn get_checkpoint_contents_by_digest(
        &self,
        digest: &mys_types::messages_checkpoint::CheckpointContentsDigest,
    ...
    fn get_transaction(
        &self,
        tx_digest: &mys_types::digests::TransactionDigest,
    ...
    fn get_events(
        &self,
        event_digest: &mys_types::digests::TransactionEventsDigest,
    ...
    fn get_full_checkpoint_contents(
        &self,
        _digest: &mys_types::messages_checkpoint::CheckpointContentsDigest,
    ) -> Option<mys_types::messages_checkpoint::FullCheckpointContents> {
        todo!()
    }
}
...
impl<T: Send + Sync, V: store::SimulatorStore + Send + Sync> RpcStateReader for Simulacrum<T, V> {
    ...
    fn get_chain_identifier(
        &self,
    ) -> mys_types::storage::error::Result<mys_types::digests::ChainIdentifier> {
        Ok(self
            .store()
    ...
    fn indexes(&self) -> Option<&dyn mys_types::storage::RpcIndexes> {
        None
    }
}
...
impl Simulacrum {
    ...
    /// existing code for generating transactions in mys-test-transaction-builder by defining a trait
    /// that both WalletContext and Simulacrum implement. Then we can remove this function.
    pub fn transfer_txn(&mut self, recipient: MysAddress) -> (Transaction, u64) {
        let (sender, key) = self.keystore().accounts().next().unwrap();
        let sender = *sender;
        ...
        let pt = {
            ...
            builder.transfer_mys(recipient, Some(transfer_amount));
            builder.finish()
        };
