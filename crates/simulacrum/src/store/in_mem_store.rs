...
impl InMemoryStore {
    ...
    pub fn get_system_state(&self) -> mys_types::mys_system_state::MysSystemState {
        mys_types::mys_system_state::get_mys_system_state(self).expect("system state must exist")
    }

    pub fn get_clock(&self) -> mys_types::clock::Clock {
        self.get_object(&mys_types::MYS_CLOCK_OBJECT_ID)
            .expect("clock should exist")
            .to_rust()
            .expect("clock object should deserialize")
    }

    pub fn owned_objects(&self, owner: MysAddress) -> impl Iterator<Item = &Object> {
        self.live_objects
            .iter()
...
pub struct KeyStore {
    ...
    #[allow(unused)]
    account_keys: BTreeMap<MysAddress, AccountKeyPair>,
}

impl KeyStore {
    pub fn from_network_config(
        network_config: &mys_swarm_config::network_config::NetworkConfig,
    ) -> Self {
    ...
    pub fn accounts(&self) -> impl Iterator<Item = (&MysAddress, &AccountKeyPair)> {
