...
use mys_graphql_rpc::test_infra::cluster::{serve_executor, ExecutorCluster};
...
async fn run_test(path: &Path) -> Result<(), Box<dyn std::error::Error>> {
    ...
    if !cfg!(msim) {
        ...
        // In another crate like `mys-mvr-graphql-e2e-tests`, this would be the place to translate
