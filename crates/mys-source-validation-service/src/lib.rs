...
use mys_sdk::rpc_types::MysTransactionBlockEffects;
...
pub async fn verify_package(
    ...
    let client = MysClientBuilder::default().build(network_url).await?;
...
pub async fn watch_for_upgrades(
    ...
    _network: Network,
    _channel: Option<Sender<MysTransactionBlockEffects>>,
) -> anyhow::Result<()> {
...
async fn check_version_header(
    ...
    let version = headers
        .get(MYS_SOURCE_VALIDATION_VERSION_HEADER)
        .as_ref()
        .and_then(|h| h.to_str().ok())
        .map(|s| s.to_string());

    match version {
        Some(v) if v != MYS_SOURCE_VALIDATION_VERSION => {
            let error = format!(
                "Unsupported version '{v}' specified in header \
		 {MYS_SOURCE_VALIDATION_VERSION_HEADER}"
            );
            let mut headers = HeaderMap::new();
            headers.insert(
                HeaderName::from_static(MYS_SOURCE_VALIDATION_VERSION_HEADER),
                HeaderValue::from_static(MYS_SOURCE_VALIDATION_VERSION),
            );
            return (
                StatusCode::BAD_REQUEST,
                headers,
