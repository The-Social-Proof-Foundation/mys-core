...
pub const MYS_VALIDATOR_SERVER_NAME: &str = "mys";
...
mod tests {
    ...
    fn verify_allowall() {
        ...
        let random_cert_bob =
            SelfSignedCertificate::new(allowed.private(), MYS_VALIDATOR_SERVER_NAME);
        let random_cert_alice =
            SelfSignedCertificate::new(disallowed.private(), MYS_VALIDATOR_SERVER_NAME);

        let verifier = ClientCertVerifier::new(AllowAll, MYS_VALIDATOR_SERVER_NAME.to_string());
    ...
    fn verify_server_cert() {
        ...
        let verifier =
            ServerCertVerifier::new(allowed_public_key, MYS_VALIDATOR_SERVER_NAME.to_string());
    ...
    fn verify_hashset() {
        ...
        let allowed_cert = SelfSignedCertificate::new(allowed.private(), MYS_VALIDATOR_SERVER_NAME);
        ...
        let verifier =
            ClientCertVerifier::new(allowlist.clone(), MYS_VALIDATOR_SERVER_NAME.to_string());
    ...
    fn invalid_server_name() {
        ...
        let cert = SelfSignedCertificate::new(keypair.private(), "not-mys");

        let allowlist = AllowPublicKeys::new(BTreeSet::from([public_key.clone()]));
        let client_verifier =
            ClientCertVerifier::new(allowlist.clone(), MYS_VALIDATOR_SERVER_NAME.to_string());

        // Allowed public key but the server-name in the cert is not the required "mys"
        ...
        let server_verifier =
            ServerCertVerifier::new(public_key, MYS_VALIDATOR_SERVER_NAME.to_string());
    ...
    async fn axum_acceptor() {
        ...
        let client_certificate =
            SelfSignedCertificate::new(client_keypair.private(), MYS_VALIDATOR_SERVER_NAME);
