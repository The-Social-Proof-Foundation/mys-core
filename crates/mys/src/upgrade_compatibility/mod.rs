...
use mys_json_rpc_types::{MysObjectDataOptions, MysRawData};
...
use mys_types::{base_types::ObjectID, execution_config_utils::to_binary_config};
...
pub(crate) async fn check_compatibility(
    ...
    let existing_obj_read = client
        .read_api()
        .get_object_with_options(package_id, MysObjectDataOptions::new().with_bcs())
        .await
    ...
    let existing_package = match existing_obj {
        ...
        MysRawData::MoveObject(_) => Err(anyhow!("Object found when package expected")),
    }?;
