...
/// `mys = { path = ".", features = ["tracing"] }` to our dev-dependencies.
...
async fn test_profiler() {
    ...
    let testnet_url = "https://fullnode.testnet.mysocial.network:8082".to_string();
    ...
    let command_result =
        mys_replay::execute_replay_command(Some(testnet_url), false, false, None, None, cmd).await;
