...
use mys_types::crypto::Secp256k1MysSignature;
use mys_types::crypto::Secp256r1MysSignature;
use mys_types::crypto::Signature;
use mys_types::crypto::SignatureScheme;
use mys_types::crypto::MysKeyPair;
use mys_types::crypto::MysSignatureInner;
...
async fn test_addresses_command() -> Result<(), anyhow::Error> {
    ...
    for _ in 0..3 {
        keystore.add_key(None, MysKeyPair::Secp256k1(get_key_pair().1))?;
    }
...
async fn test_flag_in_signature_and_keypair() -> Result<(), anyhow::Error> {
    ...
    keystore.add_key(None, MysKeyPair::Secp256k1(get_key_pair().1))?;
    keystore.add_key(None, MysKeyPair::Ed25519(get_key_pair().1))?;

    for pk in keystore.keys() {
        let pk1 = pk.clone();
        let sig = keystore.sign_secure(&(&pk).into(), b"hello", Intent::mys_transaction())?;
        match sig {
            Signature::Ed25519MysSignature(_) => {
                // signature contains corresponding flag
                assert_eq!(
                    *sig.as_ref().first().unwrap(),
                    Ed25519MysSignature::SCHEME.flag()
                );
                // keystore stores pubkey with corresponding flag
                assert!(pk1.flag() == Ed25519MysSignature::SCHEME.flag())
            }
            Signature::Secp256k1MysSignature(_) => {
                assert_eq!(
                    *sig.as_ref().first().unwrap(),
                    Secp256k1MysSignature::SCHEME.flag()
                );
                assert!(pk1.flag() == Secp256k1MysSignature::SCHEME.flag())
            }
            Signature::Secp256r1MysSignature(_) => {
                assert_eq!(
                    *sig.as_ref().first().unwrap(),
                    Secp256r1MysSignature::SCHEME.flag()
                );
                assert!(pk1.flag() == Secp256r1MysSignature::SCHEME.flag())
            }
        }
    }
    Ok(())
...
async fn test_read_write_keystore_with_flag() {
    ...
    let kp_secp = MysKeyPair::Secp256k1(get_key_pair().1);
    let addr_secp: MysAddress = (&kp_secp.public()).into();
    ...
    let kp_ed = MysKeyPair::Ed25519(get_key_pair().1);
    let addr_ed: MysAddress = (&kp_ed.public()).into();
...
#[test]
async fn test_mys_operations_config() {
    let temp_dir = TempDir::new().unwrap();
    let path = temp_dir.path().join("mys.keystore");
    let path1 = path.clone();
    // This is the hardcoded keystore in mys-operation: https://github.com/MystenLabs/mys-operations/blob/af04c9d3b61610dbb36401aff6bef29d06ef89f8/docker/config/generate/static/mys.keystore
    // If this test fails, address hardcoded in mys-operations is likely needed be updated.
    let kp = MysKeyPair::decode_base64("ANRj4Rx5FZRehqwrctiLgZDPrY/3tI5+uJLCdaXPCj6C").unwrap();
    ...
    assert!(read.is_ok());
    assert_eq!(
        MysAddress::from_str("7d20dcdb2bca4f508ea9613994683eb4e76e9c4ed371169677c1be02aaf0b58e")
            .unwrap(),
        read.unwrap().addresses()[0]
    );

    // This is the hardcoded keystore in mys-operation: https://github.com/MystenLabs/mys-operations/blob/af04c9d3b61610dbb36401aff6bef29d06ef89f8/docker/config/generate/static/mys-benchmark.keystore
    // If this test fails, address hardcoded in mys-operations is likely needed be updated.
    let path2 = temp_dir.path().join("mys-benchmark.keystore");
    let path3 = path2.clone();
    let kp = MysKeyPair::decode_base64("APCWxPNCbgGxOYKeMfPqPmXmwdNVyau9y4IsyBcmC14A").unwrap();
    let contents = vec![kp.encode_base64()];
    let res = std::fs::write(path2, serde_json::to_string_pretty(&contents).unwrap());
    assert!(res.is_ok());
    let read = FileBasedKeystore::new(&path3);
    assert_eq!(
        MysAddress::from_str("160ef6ce4f395208a12119c5011bf8d8ceb760e3159307c819bd0197d154d384")
            .unwrap(),
        read.unwrap().addresses()[0]
    );
}
...
async fn test_private_keys_import_export() -> Result<(), anyhow::Error> {
    // private key in Bech32, private key in Hex, private key in Base64, derived Mys address in Hex
    const TEST_CASES: &[(&str, &str, &str, &str)] = &[
        (
            "mysprivkey1qzwant3kaegmjy4qxex93s0jzvemekkjmyv3r2sjwgnv2y479pgsywhveae",
        ...
        (
            "mysprivkey1qrh2sjl88rze74hwjndw3l26dqyz63tea5u9frtwcsqhmfk9vxdlx8cpv0g",
        ...
        (
            "mysprivkey1qzg73qyvfz0wpnyectkl08nrhe4pgnu0vqx8gydu96qx7uj4wyr8gcrjlh3",
    ...
    for (private_key, private_key_hex, private_key_base64, address) in TEST_CASES {
        ...
        .execute(&mut keystore)
        .await?;
        let kp = MysKeyPair::decode(private_key).unwrap();
        let kp_from_hex = MysKeyPair::Ed25519(
            Ed25519KeyPair::from_bytes(&Hex::decode(private_key_hex).unwrap()).unwrap(),
        );
        assert_eq!(kp, kp_from_hex);

        let kp_from_base64 = MysKeyPair::decode_base64(private_key_base64).unwrap();
        assert_eq!(kp, kp_from_base64);

        let addr = MysAddress::from_str(address).unwrap();
        assert_eq!(MysAddress::from(&kp.public()), addr);
...
async fn test_mnemonics_ed25519() -> Result<(), anyhow::Error> {
    // Test case matches with /mysten/mys/sdk/typescript/test/unit/cryptography/ed25519-keypair.test.ts
    const TEST_CASES: [[&str; 3]; 3] = [["film crazy soon outside stand loop subway crumble thrive popular green nuclear struggle pistol arm wife phrase warfare march wheat nephew ask sunny firm", "mysprivkey1qrwsjvr6gwaxmsvxk4cfun99ra8uwxg3c9pl0nhle7xxpe4s80y05ctazer", "a2d14fad60c56049ecf75246a481934691214ce413e6a8ae2fe6834c173a6133"],
    ["require decline left thought grid priority false tiny gasp angle royal system attack beef setup reward aunt skill wasp tray vital bounce inflict level", "mysprivkey1qzdvpa77ct272ultqcy20dkw78dysnfyg90fhcxkdm60el0qht9mvzlsh4j", "1ada6e6f3f3e4055096f606c746690f1108fcc2ca479055cc434a3e1d3f758aa"],
    ["organ crash swim stick traffic remember army arctic mesh slice swear summer police vast chaos cradle squirrel hood useless evidence pet hub soap lake", "mysprivkey1qqqscjyyr64jea849dfv9cukurqj2swx0m3rr4hr7sw955jy07tzgcde5ut", "e69e896ca10f5a77732769803cc2b5707f0ab9d4407afb5e4b4464b89769af14"]];
    ...
    for t in TEST_CASES {
        ...
        .execute(&mut keystore)
        .await?;
        let kp = MysKeyPair::decode(t[1]).unwrap();
        let addr = MysAddress::from_str(t[2]).unwrap();
        assert_eq!(MysAddress::from(&kp.public()), addr);
        assert!(keystore.addresses().contains(&addr));
    }
    Ok(())
...
async fn test_mnemonics_secp256k1() -> Result<(), anyhow::Error> {
    // Test case matches with /mysten/mys/sdk/typescript/test/unit/cryptography/secp256k1-keypair.test.ts
    const TEST_CASES: [[&str; 3]; 3] = [["film crazy soon outside stand loop subway crumble thrive popular green nuclear struggle pistol arm wife phrase warfare march wheat nephew ask sunny firm", "mysprivkey1qyqr6yvxdqkh32ep4pk9caqvphmk9epn6rhkczcrhaeermsyvwsg783y9am", "9e8f732575cc5386f8df3c784cd3ed1b53ce538da79926b2ad54dcc1197d2532"],
    ["require decline left thought grid priority false tiny gasp angle royal system attack beef setup reward aunt skill wasp tray vital bounce inflict level", "mysprivkey1q8hexn5m2u36tx39ln5e22hfseadknp7d2qlkhe30ejy7fc6am5aqkqpqsj", "9fd5a804ed6b46d36949ff7434247f0fd594673973ece24aede6b86a7b5dae01"],
    ["organ crash swim stick traffic remember army arctic mesh slice swear summer police vast chaos cradle squirrel hood useless evidence pet hub soap lake", "mysprivkey1qxx6yf53jgxvsmccst8cuwnj0rx4k4uzvn9aalvag7ns0xf0g8j2x246jst", "60287d7c38dee783c2ab1077216124011774be6b0764d62bd05f32c88979d5c5"]];
...
async fn test_mnemonics_secp256r1() -> Result<(), anyhow::Error> {
    // Test case matches with /mysten/mys/sdk/typescript/test/unit/cryptography/secp256r1-keypair.test.ts
    ...
    for [mnemonics, sk, address] in TEST_CASES {
        ...
        let kp = MysKeyPair::decode(sk).unwrap();
        let addr = MysAddress::from_str(address).unwrap();
        assert_eq!(MysAddress::from(&kp.public()), addr);
        assert!(keystore.addresses().contains(&addr));
    }
...
async fn test_invalid_derivation_path() -> Result<(), anyhow::Error> {
    ...
    assert!(KeyToolCommand::Import {
        ...
        input_string: TEST_MNEMONIC.to_string(),
        key_scheme: SignatureScheme::ED25519,
        derivation_path: Some("m/0'/6976'/0'/0/0".parse().unwrap()),
    }
    .execute(&mut keystore)
    .await
    .is_err());
    ...
    assert!(KeyToolCommand::Import {
        ...
        input_string: TEST_MNEMONIC.to_string(),
        key_scheme: SignatureScheme::ED25519,
        derivation_path: Some("m/54'/6976'/0'/0/0".parse().unwrap()),
    }
    .execute(&mut keystore)
    .await
    .is_err());
    ...
    assert!(KeyToolCommand::Import {
        ...
        input_string: TEST_MNEMONIC.to_string(),
        key_scheme: SignatureScheme::Secp256k1,
        derivation_path: Some("m/44'/6976'/0'/0/0".parse().unwrap()),
    }
    .execute(&mut keystore)
    .await
    .is_err());
...
async fn test_valid_derivation_path() -> Result<(), anyhow::Error> {
    ...
    assert!(KeyToolCommand::Import {
        ...
        input_string: TEST_MNEMONIC.to_string(),
        key_scheme: SignatureScheme::ED25519,
        derivation_path: Some("m/44'/6976'/0'/0'/1'".parse().unwrap()),
    }
    .execute(&mut keystore)
    .await
    .is_ok());
    ...
    assert!(KeyToolCommand::Import {
        ...
        input_string: TEST_MNEMONIC.to_string(),
        key_scheme: SignatureScheme::ED25519,
        derivation_path: Some("m/44'/6976'/1'/0'/1'".parse().unwrap()),
    }
    .execute(&mut keystore)
    .await
    .is_ok());
    ...
    assert!(KeyToolCommand::Import {
        ...
        input_string: TEST_MNEMONIC.to_string(),
        key_scheme: SignatureScheme::Secp256k1,
        derivation_path: Some("m/54'/6976'/0'/0/1".parse().unwrap()),
    }
    .execute(&mut keystore)
    .await
    .is_ok());
    ...
    assert!(KeyToolCommand::Import {
        ...
        input_string: TEST_MNEMONIC.to_string(),
        key_scheme: SignatureScheme::Secp256k1,
        derivation_path: Some("m/54'/6976'/1'/0/1".parse().unwrap()),
    }
    .execute(&mut keystore)
    .await
    .is_ok());
...
async fn test_sign_command() -> Result<(), anyhow::Error> {
    ...
    let gas_price = 1;
    let tx_data = TransactionData::new_pay_mys(
        *sender,
        vec![gas],
    ...
    KeyToolCommand::Sign {
        ...
        intent: Some(Intent::mys_app(IntentScope::PersonalMessage)),
    }
    .execute(&mut keystore)
    .await?;
