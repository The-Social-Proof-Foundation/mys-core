...
use crate::client_commands::{MysClientCommandResult, MysClientCommands};
use crate::shell::{
    install_shell_plugins, AsyncHandler, CacheKey, CommandStructure, CompletionCache, Shell,
};

const MYS: &str = "   _____       _    ______                       __
...
pub async fn start_console(
    ...
    let app: Command = MysClientCommands::command();
    writeln!(out, "{}", MYS.cyan().bold())?;
...
async fn handle_command(
    ...
    if let Ok(mut cache) = completion_cache.write() {
        match result {
            MysClientCommandResult::Addresses(ref addresses) => {
            ...
            MysClientCommandResult::Objects(ref objects) => {
                let objects = objects
                    .iter()
