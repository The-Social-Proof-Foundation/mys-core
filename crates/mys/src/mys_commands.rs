...
use crate::validator_commands::MysValidatorCommand;
...
use mys_bridge::mys_transaction_builder::build_committee_register_transaction;
use mys_config::node::Genesis;
use mys_config::p2p::SeedPeer;
use mys_config::{
    genesis_blob_exists, mys_config_dir, Config, PersistedConfig, FULL_NODE_DB_PATH,
    MYS_CLIENT_CONFIG, MYS_FULLNODE_CONFIG, MYS_NETWORK_CONFIG,
};
use mys_config::{
...
use mys_move::{self, execute_move_command};
use mys_move_build::MysPackageHooks;
use mys_sdk::mys_client_config::{MysClientConfig, MysEnv};
use mys_sdk::wallet_context::WalletContext;
use mys_swarm::memory::Swarm;
use mys_swarm_config::genesis_config::{GenesisConfig, DEFAULT_NUMBER_OF_AUTHORITIES};
...
const DEFAULT_FAUCET_NUM_COINS: usize = 5; // 5 coins per request was the default in mys-test-validator
const DEFAULT_FAUCET_MIST_AMOUNT: u64 = 200_000_000_000; // 200 MYS
...
pub struct IndexerArgs {
    ...
    /// DB name for the Indexer Postgres DB. Default DB name is mys_indexer.
    #[clap(long, default_value = "mys_indexer")]
    pg_db_name: String,
...
impl IndexerArgs {
    pub fn for_testing() -> Self {
        Self {
            ...
            pg_db_name: "mys_indexer".to_string(),
...
pub enum MysCommand {
    ...
    ///
    /// By default, mys start will start a local network from the genesis blob that exists in
    /// the Mys config default dir or in the config_dir that was passed. If the default directory
    ...
    /// follows:
    /// - MYS_PROTOCOL_CONFIG_OVERRIDE_ENABLE=1
    /// - Then, to configure an override, use the prefix `MYS_PROTOCOL_CONFIG_OVERRIDE_`
    ///   along with the parameter name. For example, to increase the interval between
    ///   checkpoint creation to >1/s, you might set:
    ///   MYS_PROTOCOL_CONFIG_OVERRIDE_min_checkpoint_interval_ms=1000
    ///
    ...
    Start {
        ...
        /// mys genesis -f --with-faucet generates a genesis config that can be used to start this
        /// proces. Use with caution as the `-f` flag will overwrite the existing config directory.
        /// We can use any config dir that is generated by the `mys genesis`.
        ...
        /// run this command.
        ///
        /// To run with persisted state, do not pass this flag and use the `mys genesis` command
    ...
    Genesis {
        ...
        #[clap(
            ...
            num_args(1..),
            value_delimiter = ',',
            help = "A list of ip addresses to generate a genesis ttttable for benchmarks"
        )]
        benchmark_ips: Option<Vec<String>>,
        #[clap(
            long,
            help = "Creates an extra faucet configuration for mys persisted runs."
        )]
        with_faucet: bool,
    ...
    Validator {
        ...
        config: Option<PathBuf>,
        #[clap(subcommand)]
        cmd: Option<MysValidatorCommand>,
        /// Return command outputs in json format.
        #[clap(long, global = true)]
        json: bool,
        #[clap(short = 'y', long = "yes")]
    ...
    /// Invoke Mys's move-analyzer via CLI
    ...
    pub async fn execute(self) -> Result<(), anyhow::Error> {
        ...
        match self {
            MysCommand::Network {
                ...
                let config_path = config.unwrap_or(mys_config_dir()?.join(MYS_NETWORK_CONFIG));
                let config: NetworkConfig = PersistedConfig::read(&config_path).map_err(|err| {
                    err.context(format!(
                        "Cannot open Mys network config file at {:?}",
                        config_path
                    ))
                })?;
            ...
            MysCommand::GenesisCeremony(cmd) => run(cmd),
            MysCommand::KeyTool {
                keystore_path,
                json,
                cmd,
                ...
                let keystore_path =
                    keystore_path.unwrap_or(mys_config_dir()?.join(MYS_KEYSTORE_FILENAME));
                let mut keystore = Keystore::from(FileBasedKeystore::new(&keystore_path)?);
                cmd.execute(&mut keystore).await?.print(!json);
                Ok(())
            }
            MysCommand::Console { config } => {
                let config = config.unwrap_or(mys_config_dir()?.join(MYS_CLIENT_CONFIG));
            ...
            MysCommand::Client {
                ...
                let config_path = config.unwrap_or(mys_config_dir()?.join(MYS_CLIENT_CONFIG));
                ...
                if let Some(cmd) = cmd {
                    ...
                    // Print help
                    let mut app: Command = MysCommand::command();
                    app.build();
                    app.find_subcommand_mut("client").unwrap().print_help()?;
                }
                Ok(())
            }
            MysCommand::Validator {
                config,
                cmd,
                json,
                accept_defaults,
            ...
            MysCommand::Move {
                ...
                match &mut cmd {
                    mys_move::Command::Build(build) if build.dump_bytecode_as_base64 => {
                        if build.ignore_chain {
                            build.chain_id = None;
                        } else {
                            // `mys move build` does not ordinarily require a network connection.
                            ...
                            let config =
                                client_config.unwrap_or(mys_config_dir()?.join(MYS_CLIENT_CONFIG));
                            prompt_if_no_config(&config, false).await?;
                            let context = WalletContext::new(&config, None, None)?;
                            if let Ok(client) = context.get_client().await {
                                if let Err(e) = client.check_api_version() {
            ...
            MysCommand::BridgeInitialize {
                network_config,
                client_config,
                bridge_committee_config_path,
            } => {
                // Load the config of the Mys authority.
                let network_config_path = network_config
                    .clone()
                    .unwrap_or(mys_config_dir()?.join(MYS_NETWORK_CONFIG));
                ...
                let mys_bridge_client = MysBridgeClient::new(rpc_url, bridge_metrics).await?;
                let bridge_arg = mys_bridge_client
                ...
                {
                    let account_kp = node_config.account_key_pair.keypair();
                    let mys_address = MysAddress::from(&account_kp.public());
            ...
            MysCommand::FireDrill { fire_drill } => run_fire_drill(fire_drill).await,
            MysCommand::Analyzer => {
                analyzer::run();
                Ok(())
            }
...
async fn start(
    ...
    if epoch_duration_ms.is_some() && genesis_blob_exists(config.clone()) && !force_regenesis {
        bail!(
            ...
            there is no genesis configuration in the default Mys configuration folder or the given \
    ...
    let config_dir = if force_regenesis {
        ...
        // the mys config directory for backwards compatibility with `mys-test-validator`.
        let (network_config_path, mys_config_path) = match config {
            Some(config)
                if config.is_file()
                    && config
                        .extension()
                ...
                if committee_size.is_some() {
                    eprintln!(
                        ...
                        "[warning] The committee-size arg wil be ignored as a network \
                            ...
                            with the desired committee size. See `mys genesis --help` for more \
                            information."
                            .yellow()
                            .bold()
                    );
                }
                (config, mys_config_dir()?)
            }
            ...
            Some(config) => {
                ...
                (config.join(MYS_NETWORK_CONFIG), config)
            }

            None => {
                let mys_config = mys_config_dir()?;
                let network_config = mys_config.join(MYS_NETWORK_CONFIG);
                ...
                if !network_config.exists() {
                    ...
                    .map_err(|_| {
                        anyhow!(
                            "Cannot run genesis with non-empty Mys config directory: {}.\n\n\
                                ...
                                genesis, use mys genesis --help to see how to generate a new \
                                genesis.",
                            mys_config.display(),
                        )
                    })?;
    ...
    let mut fullnode_url = mys_config::node::default_json_rpc_address();
    ...
    if let Some(input) = with_faucet {
        ...
        if force_regenesis {
            ...
            let keystore_path = config_dir.join(MYS_KEYSTORE_FILENAME);
            let mut keystore = Keystore::from(FileBasedKeystore::new(&keystore_path).unwrap());
            let address: MysAddress = kp.public().into();
            keystore.add_key(None, MysKeyPair::Ed25519(kp)).unwrap();
            MysClientConfig {
                keystore,
                envs: vec![MysEnv {
            ...
            .persisted(config_dir.join(MYS_CLIENT_CONFIG).as_path())
            .save()
            .unwrap();
        }
...
async fn genesis(
    ...
    let mys_config_dir = &match working_dir {
        // if a directory is specified, it must exist (it
        // will not be created)
        Some(v) => v,
        // create default Mys config dir if not specified
        // on the command line and if it does not exist
        // yet
        None => {
            let config_path = mys_config_dir()?;
            fs::create_dir_all(&config_path)?;
            config_path
        }
    };

    // if Mys config dir is not empty then either clean it
    // up (if --force/-f option was specified or report an
    // error
    let dir = mys_config_dir.read_dir().map_err(|err| {
        anyhow!(err).context(format!("Cannot open Mys config dir {:?}", mys_config_dir))
    })?;
    let files = dir.collect::<Result<Vec<_>, _>>()?;

    let client_path = mys_config_dir.join(MYS_CLIENT_CONFIG);
    let keystore_path = mys_config_dir.join(MYS_KEYSTORE_FILENAME);
    ...
    if write_config.is_none() && !files.is_empty() {
        if force {
            ...
            let is_compatible = FileBasedKeystore::new(&keystore_path).is_ok()
                && PersistedConfig::<MysClientConfig>::read(&client_path).is_ok();
            ...
            if is_compatible {
                ...
                fs::remove_dir_all(mys_config_dir).map_err(|err| {
                    anyhow!(err)
                        .context(format!("Cannot remove Mys config dir {:?}", mys_config_dir))
                })?;
                fs::create_dir(mys_config_dir).map_err(|err| {
                    anyhow!(err)
                        .context(format!("Cannot create Mys config dir {:?}", mys_config_dir))
                })?;
            }
        } else if files.len() != 2 || !client_path.exists() || !keystore_path.exists() {
            bail!("Cannot run genesis with non-empty Mys config directory {}, please use the --force/-f option to remove the existing configuration", mys_config_dir.to_str().unwrap());
        }
    }

    let network_path = mys_config_dir.join(MYS_NETWORK_CONFIG);
    let genesis_path = mys_config_dir.join(MYS_GENESIS_FILENAME);
    ...
    let mut genesis_conf = match from_config {
        ...
        None => {
            if let Some(ips) = benchmark_ips {
                ...
                let path = mys_config_dir.join(MYS_BENCHMARK_GENESIS_GAS_KEYSTORE_FILENAME);
    ...
    let builder = ConfigBuilder::new(mys_config_dir);
    ...
    for key in &network_config.account_keys {
        keystore.add_key(None, MysKeyPair::Ed25519(key.copy()))?;
    }
    let active_address = keystore.addresses().pop();

    network_config.genesis.save(&genesis_path)?;
    for validator in &mut network_config.validator_configs {
        validator.genesis = mys_config::node::Genesis::new_from_file(&genesis_path);
    }
    ...
    let fullnode_config = FullnodeConfigBuilder::new()
        ...
        .with_rpc_addr(mys_config::node::default_json_rpc_address())
        .build(&mut OsRng, &network_config);

    fullnode_config.save(mys_config_dir.join(MYS_FULLNODE_CONFIG))?;
    let mut ssfn_nodes = vec![];
    if let Some(ssfn_info) = ssfn_info {
        for (i, ssfn) in ssfn_info.into_iter().enumerate() {
            let path =
                mys_config_dir.join(mys_config::ssfn_config_file(ssfn.p2p_address.clone(), i));
            ...
            let ssfn_config = FullnodeConfigBuilder::new()
                ...
                .with_db_path(PathBuf::from("/opt/mys/db/authorities_db/full_node_db"))
                ...
                .with_genesis(Genesis::new_from_file("/opt/mys/config/genesis.blob"))
                .build(&mut OsRng, &network_config);
        ...
        for (i, mut validator) in network_config
            ...
            .enumerate()
        {
            let path = mys_config_dir.join(mys_config::validator_config_file(
    ...
    let mut client_config = if client_path.exists() {
        ...
        MysClientConfig::new(keystore.into())
    };
    ...
    client_config.add_env(MysEnv::devnet());
...
async fn prompt_if_no_config(
    ...
    if !wallet_conf_path.exists() {
        let env = match std::env::var_os("MYS_CONFIG_WITH_RPC_URL") {
            Some(v) => Some(MysEnv {
            ...
            None => {
                if accept_defaults {
                    ...
                    print!(
                        "Config file [{:?}] doesn't exist, do you want to connect to a Mys Full node server [y/N]?",
                        wallet_conf_path
                    );
                }
                if accept_defaults
                    || matches!(read_line(), Ok(line) if line.trim().to_lowercase() == "y")
                {
                    let url = if accept_defaults {
                        ...
                        print!(
                            "Mys Full node server URL (Defaults to Mys Testnet if not specified) : "
                        );
                        read_line()?
                    };
