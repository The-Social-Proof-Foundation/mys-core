...
use mys_json_rpc_types::{
    ...
    DynamicFieldPage, MysCoinMetadata, MysData, MysExecutionStatus, MysObjectData,
    MysObjectDataOptions, MysObjectResponse, MysObjectResponseQuery, MysParsedData,
    MysProtocolConfigValue, MysRawData, MysTransactionBlockEffects, MysTransactionBlockEffectsAPI,
...
use mys_package_management::{LockCommand, PublishedAtError};
use mys_replay::ReplayToolCommand;
use mys_sdk::{
    apis::ReadApi,
    mys_client_config::{MysClientConfig, MysEnv},
    wallet_context::WalletContext,
    MysClient, MYS_COIN_TYPE, MYS_DEVNET_URL, MYS_LOCAL_NETWORK_URL, MYS_LOCAL_NETWORK_URL_0,
    MYS_TESTNET_URL,
};
use mys_types::{
...
pub enum MysClientCommands {
    ...
    Balance {
        ...
        /// Show balance for the specified coin (e.g., 0x2::mys::MYS).
    ...
    Call {
        ...
        #[clap(
            long,
            value_parser = parse_mys_type_tag,
            num_args(1..),
        )]
    ...
    /// List all Mys environments
    Envs,

    /// Execute a Signed Transaction. This is useful when the user prefers to sign elsewhere and use this command to execute.
    ExecuteSignedTx {
        /// BCS serialized transaction data bytes without its type tag, as base64 encoded string. This is the output of mys client command using --serialize-unsigned-transaction.
        #[clap(long)]
        tx_bytes: String,
    ...
    ExecuteCombinedSignedTx {
        /// BCS serialized sender signed data, as base64 encoded string. This is the output of mys client command using --serialize-signed-transaction.
        #[clap(long)]
    ...
    Objects {
        ...
        /// objects owned by `mys client active-address`.
        #[clap(name = "owner_address")]
        address: Option<KeyIdentity>,
    },
    ...
    /// Pay all residual MYS coins to the recipient with input coins, after deducting the gas cost.
    ...
    /// Pay MYS coins to recipients following following specified amounts, with input coins.
    /// Length of recipients must be the same as that of amounts.
    /// The input coins also include the coin for gas payment, so no extra gas coin is required.
    PayMys {
        /// The input coins to be used for pay recipients, including the gas coin.
...
pub struct Opts {
    ...
    /// be used to execute transaction with `mys client execute-signed-tx --tx-bytes <TX_BYTES>`.
    ...
    /// `mys client execute-combined-signed-tx --signed-tx-bytes <SIGNED_TX_BYTES>`.
    #[arg(long, required = false)]
...
impl MysClientCommands {
    pub async fn execute(
        self,
        context: &mut WalletContext,
    ) -> Result<MysClientCommandResult, anyhow::Error> {
        let ret = match self {
            MysClientCommands::ProfileTransaction {
                tx_digest,
                profile_output,
            } => {
                ...
                let _command_result =
                    mys_replay::execute_replay_command(Some(rpc), false, false, None, None, cmd)
                        .await?;
                // this will be displayed via trace info, so no output is needed here
                MysClientCommandResult::NoOutput
            }
            MysClientCommands::ReplayTransaction {
                tx_digest,
                gas_info: _,
                ptb_info: _,
                executor_version,
            ...
            MysClientCommands::ReplayCheckpoints {
                start,
                end,
                terminate_early,
            } => {
            ...
            MysClientCommands::Addresses { sort_by_alias } => {
                let active_address = context.active_address()?;
                let mut addresses: Vec<(String, MysAddress)> = context
                    .config
                    .keystore
                ...
                MysClientCommandResult::Addresses(output)
            }
            MysClientCommands::Balance {
                address,
                coin_type,
                with_coins,
            } => {
                ...
                let mys_type_tag = canonicalize_type(MYS_COIN_TYPE)?;

                // show MYS first
                let ordered_coins_mys_first = coins_by_type
                    .remove(&mys_type_tag)
                    .into_iter()
                    .chain(coins_by_type.into_values())
                    .collect();

                MysClientCommandResult::Balance(ordered_coins_mys_first, with_coins)
            }

            MysClientCommands::DynamicFieldQuery { id, cursor, limit } => {
                let client = context.get_client().await?;
                let df_read = client
                    .read_api()
                    .get_dynamic_fields(id, cursor, Some(limit))
                    .await?;
                MysClientCommandResult::DynamicFieldQuery(df_read)
            }

            MysClientCommands::Upgrade {
                package_path,
                upgrade_capability,
                build_config,
                ...
                let package_path =
                    package_path
                        .canonicalize()
                        .map_err(|e| MysError::ModulePublishFailure {
                ...
                let previous_id = if let Some(ref chain_id) = chain_id {
                    mys_package_management::set_package_id(
                        &package_path,
                ...
                if let (Some(chain_id), Some(previous_id)) = (chain_id, previous_id) {
                    let _ = mys_package_management::set_package_id(
                        &package_path,
                        build_config.install_dir.clone(),
                        &chain_id,
                        previous_id,
                ...
                if let MysClientCommandResult::TransactionBlock(ref response) = result {
                    if let Err(e) = mys_package_management::update_lock_file(
                        context,
                        LockCommand::Upgrade,
            ...
            MysClientCommands::Publish {
                ...
                if build_config.test_mode {
                    return Err(MysError::ModulePublishFailure {
                        error:
                            ...
                            compiling the package normally with `mys move build`."
                                .to_string(),
                    }
                    .into());
                }
            ...
            MysClientCommands::VerifyBytecodeMeter {
                protocol_version,
                module_paths,
                package_path,
                build_config,
                ...
                let mut verifier = mys_execution::verifier(
                    &protocol_config,
                    signing_limits,
                ...
                MysClientCommandResult::VerifyBytecodeMeter {
                    success: !exceeded,
            ...
            MysClientCommands::Object { id, bcs } => {
                // Fetch the object ref
                let client = context.get_client().await?;
                if !bcs {
                    let object_read = client
                        .read_api()
                        .get_object_with_options(id, MysObjectDataOptions::full_content())
                        .await?;
                    MysClientCommandResult::Object(object_read)
                } else {
                    let raw_object_read = client
                        .read_api()
                        .get_object_with_options(id, MysObjectDataOptions::bcs_lossless())
                        .await?;
                    MysClientCommandResult::RawObject(raw_object_read)
                }
            }

            MysClientCommands::TransactionBlock { digest } => {
                let client = context.get_client().await?;
                let tx_read = client
                    .read_api()
                    ...
                    .await?;
                MysClientCommandResult::TransactionBlock(tx_read)
            }

            MysClientCommands::Call {
                package,
                module,
                function,
                type_args,
                ...
                // without failing MysJSON's checks.
                let args = args
                    .into_iter()
                    .map(|value| MysJsonValue::new(convert_number_to_string(value.to_json_value())))
                    .collect::<Result<_, _>>()?;
            ...
            MysClientCommands::TransferMys {
                to,
                mys_coin_object_id: object_id,
                amount,
                opts,
            } => {
                let signer = context.get_object_owner(&object_id).await?;
                ...
                let tx_kind = client
                    .transaction_builder()
                    .transfer_mys_tx_kind(to, amount);
                dry_run_or_execute_or_serialize(
                    signer,
                    tx_kind,
                    context,
            ...
            MysClientCommands::Pay {
                ...
                let recipients = recipients
                    ...
                    .collect::<Result<Vec<MysAddress>, anyhow::Error>>()
                    .map_err(|e| anyhow!("{e}"))?;
            ...
            MysClientCommands::PayMys {
                ...
                let tx_kind = client
                    .transaction_builder()
                    .pay_mys_tx_kind(recipients, amounts)?;
            ...
            MysClientCommands::PayAllMys {
                ...
                opts,
                    ...
                    !input_coins.is_empty(),
                    "PayAllMys transaction requires a non-empty list of input coins"
                );
                let recipient = get_identity_address(Some(recipient), context)?;
                let signer = context.get_object_owner(&input_coins[0]).await?;
                let client = context.get_client().await?;
                let tx_kind = client.transaction_builder().pay_all_mys_tx_kind(recipient);
                dry_run_or_execute_or_serialize(
                    signer,
                    tx_kind,
                    context,
            ...
            MysClientCommands::Objects { address } => {
                let address = get_identity_address(address, context)?;
                let client = context.get_client().await?;
                ...
                MysClientCommandResult::Objects(objects)
            }

            MysClientCommands::NewAddress {
                key_scheme,
                alias,
                derivation_path,
                word_length,
                ...
                MysClientCommandResult::NewAddress(NewAddressOutput {
                    alias,
                    address,
                    key_scheme: scheme,
            ...
            MysClientCommands::Gas { address } => {
                ...
                let coins = context
                    ...
                    .collect();
                MysClientCommandResult::Gas(coins)
            }
            MysClientCommands::Faucet { address, url } => {
                let address = get_identity_address(address, context)?;
            ...
            MysClientCommands::ChainIdentifier => {
                let ci = context
                    .get_client()
                    .await?
                    .read_api()
                    ...
                    .await?;
                MysClientCommandResult::ChainIdentifier(ci)
            }
            MysClientCommands::SplitCoin {
                coin_id,
                amounts,
                count,
                opts,
            ...
            MysClientCommands::Switch { address, env } => {
                let mut addr = None;
                ...
                context.config.save()?;
                MysClientCommandResult::Switch(SwitchResponse { address: addr, env })
            }
            MysClientCommands::ActiveAddress => {
                MysClientCommandResult::ActiveAddress(context.active_address().ok())
            }

            MysClientCommands::ExecuteSignedTx {
                tx_bytes,
                signatures,
            } => {
                let data = bcs::from_bytes(
                    ...
                    .to_vec()
                    .map_err(|_| anyhow!("Invalid Base64 encoding"))?
                ).map_err(|_| anyhow!("Failed to parse tx bytes, check if it matches the output of mys client commands with --serialize-unsigned-transaction"))?;
                ...
                MysClientCommandResult::TransactionBlock(response)
            }
            MysClientCommands::ExecuteCombinedSignedTx { signed_tx_bytes } => {
                let data: SenderSignedData = bcs::from_bytes(
                    &Base64::try_from(signed_tx_bytes)
                        .map_err(|_| anyhow!("Invalid Base64 encoding"))?
                        .to_vec()
                        .map_err(|_| anyhow!("Invalid Base64 encoding"))?
                ).map_err(|_| anyhow!("Failed to parse SenderSignedData bytes, check if it matches the output of mys client commands with --serialize-signed-transaction"))?;
                let transaction = Envelope::<SenderSignedData, EmptySignInfo>::new(data);
                let response = context.execute_transaction_may_fail(transaction).await?;
                MysClientCommandResult::TransactionBlock(response)
            }
            MysClientCommands::NewEnv {
                alias,
                rpc,
                ws,
                basic_auth,
                ...
                context.config.save()?;
                MysClientCommandResult::NewEnv(env)
            }
            MysClientCommands::ActiveEnv => {
                MysClientCommandResult::ActiveEnv(context.config.active_env.clone())
            }
            MysClientCommands::Envs => MysClientCommandResult::Envs(
                context.config.envs.clone(),
                context.config.active_env.clone(),
            ),
            MysClientCommands::VerifySource {
                package_path,
                build_config,
                verify_deps,
                skip_source,
                ...
                MysClientCommandResult::VerifySource
            }
            MysClientCommands::PTB(ptb) => {
    ...
    pub fn switch_env(config: &mut MysClientConfig, env: &str) -> Result<(), anyhow::Error> {
        let env = Some(env.into());
        ensure!(config.get_env(&env).is_some(), "Environment config not found for [{env:?}], add new environment config using the `mys client new-env` command.");
...
> {
        ...
        PublishedAtError::Conflict {
            ...
            anyhow!(
                "Conflicting published package address: `Move.toml` contains published-at address \
                 ...
                 - update the `Move.lock` address using the `mys manage-package` command to be the same as the `Move.toml`; OR
                 - check that your `mys active-env` {env_alias} corresponds to the chain on which the package is published (i.e., devnet, testnet, mainnet); OR
    ...
    let resp = read_api
        .get_object_with_options(
            upgrade_capability,
            MysObjectDataOptions::default().with_bcs().with_owner(),
        )
        .await?;
...
> {
    ...
    if let Some(Some(MysProtocolConfigValue::U32(min_version))) = protocol_config
        .attributes
    ...
    // Check that the package's Move version is compatible with the chain's
    if let Some(Some(MysProtocolConfigValue::U32(max_version))) =
    ...
    compiled_package
        ...
        .map_err(|e| MysError::ModuleBuildFailure {
...
impl Display for MysClientCommandResult {
    fn fmt(&self, f: &mut Formatter<'_>) -> std::fmt::Result {
        let mut writer = String::new();
        match self {
            MysClientCommandResult::Addresses(addresses) => {
            ...
            MysClientCommandResult::Balance(coins, with_coins) => {
                if coins.is_empty() {
            ...
            MysClientCommandResult::DynamicFieldQuery(df_refs) => {
            ...
            MysClientCommandResult::Gas(gas_coins) => {
            ...
            MysClientCommandResult::NewAddress(new_address) => {
                let mut builder = TableBuilder::default();
            ...
            MysClientCommandResult::Object(object_read) => match object_read.object() {
                Ok(obj) => {
            ...
            MysClientCommandResult::Objects(object_refs) => {
            ...
            MysClientCommandResult::RawObject(raw_object_read) => {
                let raw_object = match raw_object_read.object() {
                    Ok(v) => match &v.bcs {
                        Some(MysRawData::MoveObject(o)) => {
                            format!("{:?}\nNumber of bytes: {}", o.bcs_bytes, o.bcs_bytes.len())
                        }
                        Some(MysRawData::Package(p)) => {
            ...
            MysClientCommandResult::SerializedUnsignedTransaction(tx_data) => {
                writeln!(
                    writer,
                    "{}",
            ...
            MysClientCommandResult::SerializedSignedTransaction(sender_signed_tx) => {
                writeln!(
                    writer,
                    "{}",
            ...
            MysClientCommandResult::SyncClientState => {
                writeln!(writer, "Client state sync complete.")?;
            }
            MysClientCommandResult::ChainIdentifier(ci) => {
                writeln!(writer, "{}", ci)?;
            }
            MysClientCommandResult::Switch(response) => {
                write!(writer, "{}", response)?;
            }
            MysClientCommandResult::ActiveAddress(response) => {
                match response {
                    Some(r) => write!(writer, "{}", r)?,
                    None => write!(writer, "None")?,
                };
            }
            MysClientCommandResult::ActiveEnv(env) => {
                write!(writer, "{}", env.as_deref().unwrap_or("None"))?;
            }
            MysClientCommandResult::NewEnv(env) => {
                writeln!(writer, "Added new Mys env [{}] to config.", env.alias)?;
            }
            MysClientCommandResult::Envs(envs, active) => {
                let mut builder = TableBuilder::default();
            ...
            MysClientCommandResult::NoOutput => {}
            MysClientCommandResult::DryRun(response) => {
                writeln!(f, "{}", Pretty(response))?;
            }
            MysClientCommandResult::DevInspect(response) => {
                writeln!(f, "{}", Pretty(response))?;
            }
        }
...
impl Debug for MysClientCommandResult {
    fn fmt(&self, f: &mut Formatter<'_>) -> std::fmt::Result {
...
impl MysClientCommandResult {
    pub fn objects_response(&self) -> Option<Vec<MysObjectResponse>> {
        use MysClientCommandResult::*;
        match self {
    ...
    pub fn tx_block_response(&self) -> Option<&MysTransactionBlockResponse> {
        use MysClientCommandResult::*;
        match self {
    ...
    pub async fn prerender_clever_errors(mut self, context: &mut WalletContext) -> Self {
        match &mut self {
            MysClientCommandResult::DryRun(DryRunTransactionBlockResponse { effects, .. })
            | MysClientCommandResult::TransactionBlock(MysTransactionBlockResponse {
            ...
            MysClientCommandResult::ActiveAddress(_)
            | MysClientCommandResult::ActiveEnv(_)
            | MysClientCommandResult::Addresses(_)
            | MysClientCommandResult::Balance(_, _)
            | MysClientCommandResult::ChainIdentifier(_)
            | MysClientCommandResult::DynamicFieldQuery(_)
            | MysClientCommandResult::DevInspect(_)
            | MysClientCommandResult::Envs(_, _)
            | MysClientCommandResult::Gas(_)
            | MysClientCommandResult::NewAddress(_)
            | MysClientCommandResult::NewEnv(_)
            | MysClientCommandResult::NoOutput
            | MysClientCommandResult::Object(_)
            | MysClientCommandResult::Objects(_)
            | MysClientCommandResult::RawObject(_)
            | MysClientCommandResult::SerializedSignedTransaction(_)
            | MysClientCommandResult::SerializedUnsignedTransaction(_)
            | MysClientCommandResult::Switch(_)
            | MysClientCommandResult::SyncClientState
            | MysClientCommandResult::VerifyBytecodeMeter { .. }
            | MysClientCommandResult::VerifySource => (),
        }
...
pub struct AddressesOutput {
    pub active_address: MysAddress,
    pub addresses: Vec<(String, MysAddress)>,
}
...
pub struct ObjectOutput {
    ...
    #[serde(skip_serializing_if = "Option::is_none")]
    pub content: Option<MysParsedData>,
}

impl From<&MysObjectData> for ObjectOutput {
    fn from(obj: &MysObjectData) -> Self {
...
impl From<&GasCoin> for GasCoinOutput {
    fn from(gas_coin: &GasCoin) -> Self {
        Self {
            ...
            mys_balance: format_balance(gas_coin.value() as u128, 9, 2, None),
        }
    }
}
...
impl ObjectsOutput {
    fn from(obj: MysObjectResponse) -> Result<Self, anyhow::Error> {
        let obj = obj.into_object()?;
        // this replicates the object type display as in the mys explorer
        let object_type = match obj.type_ {
            Some(mys_types::base_types::ObjectType::Struct(x)) => {
            ...
            Some(mys_types::base_types::ObjectType::Package) => "Package".to_string(),
            None => "unknown".to_string(),
        };
        Ok(Self {
            object_id: obj.object_id,
    ...
    fn from_vec(objs: Vec<MysObjectResponse>) -> Result<Vec<Self>, anyhow::Error> {
        objs.into_iter()
...
#[derive(Serialize)]
#[serde(untagged)]
pub enum MysClientCommandResult {
    ActiveAddress(Option<MysAddress>),
    ActiveEnv(Option<String>),
    Addresses(AddressesOutput),
    Balance(Vec<(Option<MysCoinMetadata>, Vec<Coin>)>, bool),
    ChainIdentifier(String),
    DynamicFieldQuery(DynamicFieldPage),
    DryRun(DryRunTransactionBlockResponse),
    DevInspect(DevInspectResults),
    Envs(Vec<MysEnv>, Option<String>),
    Gas(Vec<GasCoin>),
    NewAddress(NewAddressOutput),
    NewEnv(MysEnv),
    NoOutput,
    Object(MysObjectResponse),
    Objects(Vec<MysObjectResponse>),
    RawObject(MysObjectResponse),
    SerializedSignedTransaction(SenderSignedData),
    SerializedUnsignedTransaction(TransactionData),
    Switch(SwitchResponse),
    SyncClientState,
    TransactionBlock(MysTransactionBlockResponse),
    VerifyBytecodeMeter {
        success: bool,
    ...
    match resp.status() {
        StatusCode::ACCEPTED | StatusCode::CREATED => {
            ...
            if let Some(err) = faucet_resp.error {
                ...
                println!("Request successful. It can take up to 1 minute to get the coin. Run mys client gas to check your gas coins.");
            }
        }
...
fn pretty_print_balance(
    coins_by_type: &Vec<(Option<MysCoinMetadata>, Vec<Coin>)>,
...
pub async fn execute_dry_run(
    ...
    let resp = MysClientCommandResult::DryRun(response)
...
/// <https://github.com/MystenLabs/mys/blob/3c4369270605f78a243842098b7029daf8d883d9/sdk/typescript/src/transactions/TransactionBlock.ts#L845-L858>
pub async fn estimate_gas_budget(
    ...
    if let Ok(MysClientCommandResult::DryRun(dry_run)) = dry_run {
...
pub async fn max_gas_budget(client: &MysClient) -> Result<u64, anyhow::Error> {
    let cfg = client.read_api().get_protocol_config(None).await?;
    Ok(match cfg.attributes.get("max_tx_gas") {
        Some(Some(mys_json_rpc_types::MysProtocolConfigValue::U64(y))) => *y,
        _ => bail!(
...
pub(crate) async fn dry_run_or_execute_or_serialize(
    ...
    if serialize_unsigned_transaction {
        Ok(MysClientCommandResult::SerializedUnsignedTransaction(
            tx_data,
        ))
    } else {
        ...
        if serialize_signed_transaction {
            Ok(MysClientCommandResult::SerializedSignedTransaction(
                sender_signed_data,
            ))
        } else {
            ...
            let effects = response.effects.as_ref().ok_or_else(|| {
                anyhow!("Effects from MysTransactionBlockResult should not be empty")
            })?;
            if let MysExecutionStatus::Failure { error } = effects.status() {
                return Err(anyhow!(
                    "Error executing transaction '{}': {error}",
                    response.digest
                ));
            }
            Ok(MysClientCommandResult::TransactionBlock(response))
        }
    }
}
...
async fn execute_dev_inspect(
    ...
    let client = context.get_client().await?;
    let gas_budget = gas_budget.map(mys_serde::BigInt::from);
    ...
    let dev_inspect_result = client
        ...
        .dev_inspect_transaction_block(
            signer,
            tx_kind,
            Some(mys_serde::BigInt::from(gas_price)),
            None,
            Some(dev_inspect_args),
        )
        .await?;
    Ok(MysClientCommandResult::DevInspect(dev_inspect_result))
}

pub(crate) async fn prerender_clever_errors(
    effects: &mut MysTransactionBlockEffects,
    read_api: &ReadApi,
) {
    let MysTransactionBlockEffects::V1(effects) = effects;
    if let MysExecutionStatus::Failure { error } = &mut effects.status {
...
async fn check_protocol_version_and_warn(client: &MysClient) -> Result<(), anyhow::Error> {
