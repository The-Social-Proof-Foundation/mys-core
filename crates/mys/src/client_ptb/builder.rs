...
impl<'a> PTBBuilder<'a> {
    ...
    /// Fetch the `MysObjectData` for an object ID -- this is used for object resolution.
    async fn get_object(&self, object_id: ObjectID, obj_loc: Span) -> PTBResult<MysObjectData> {
        let res = self
            .reader
            .get_object_with_options(
                object_id,
                MysObjectDataOptions::new().with_type().with_owner(),
            )
            .await
