...
impl PTB {
    ...
    pub async fn execute(self, context: &mut WalletContext) -> Result<(), Error> {
        ...
        let transaction_response = match transaction_response {
            MysClientCommandResult::DryRun(_) => {
            ...
            MysClientCommandResult::TransactionBlock(response) => response,
...
pub fn ptb_description() -> clap::Command {
    clap::Command::new("mys client ptb")
        .about(
        ...
        .long_help(
            ...
            \n --make-move-vec <mys::coin::Coin<mys::mys::MYS>> [gas]"
        )
