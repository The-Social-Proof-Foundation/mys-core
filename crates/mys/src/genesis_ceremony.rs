...
pub fn run(cmd: Ceremony) -> Result<()> {
    ...
    match cmd.command {
        ...
        CeremonyCommand::AddValidator {
            ...
            let account_keypair: MysKeyPair = read_keypair_from_file(account_key_file)?;
            ...
            builder = builder.add_validator(
                mys_genesis_builder::validator_info::ValidatorInfo {
                    name,
        ...
        CeremonyCommand::Finalize => {
            ...
            genesis.save(dir.join(MYS_GENESIS_FILENAME))?;

            println!("Successfully built {MYS_GENESIS_FILENAME}");
            println!(
                "{MYS_GENESIS_FILENAME} blake2b-256: {}",
...
fn check_protocol_version(builder: &Builder, protocol_version: ProtocolVersion) -> Result<()> {
    ...
    // (e.g. using a `mys` binary built at the wrong commit).
...
mod test {
    ...
    use mys_types::crypto::{get_key_pair_from_rng, AccountKeyPair, AuthorityKeyPair, MysKeyPair};
    ...
    fn ceremony() -> Result<()> {
        ...
        let validators = (0..10)
            .map(|i| {
                ...
                write_keypair_to_file(&MysKeyPair::Ed25519(worker_keypair), &worker_key_file)
                    .unwrap();

                let network_key_file = dir.path().join(format!("{}-1.key", info.name));
                write_keypair_to_file(&MysKeyPair::Ed25519(network_keypair), &network_key_file)
                    .unwrap();

                let account_key_file = dir.path().join(format!("{}-2.key", info.name));
                write_keypair_to_file(&MysKeyPair::Ed25519(account_keypair), &account_key_file)
                    .unwrap();
