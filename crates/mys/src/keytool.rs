...
pub enum KeyToolCommand {
    ...
    ///
    /// Use `mys client new-address` if you want to generate and save the key into mys.keystore.
    Generate {
    ...
    /// List all keys by its Mys address, Base64 encoded public key, key scheme name in
    /// mys.keystore.
    List {
        /// Sort by alias
        #[clap(long, short = 's')]
    ...
    /// [enum MysKeyPair] (Base64 encoded of 33-byte `flag || privkey`) or `type AuthorityKeyPair`
    ...
    /// To MultiSig Mys Address. Pass in a list of all public keys `flag || pk` in Base64.
    ...
    /// result can be used as signature field for `mys client execute-signed-tx`. The sum
    ...
    Show { file: PathBuf },
    /// Create signature using the private key for the given address (or its alias) in mys keystore.
    ...
    /// This takes [enum MysKeyPair] of Base64 encoded of 33-byte `flag || privkey`). It
    /// outputs the keypair into a file at the current directory where the address is the filename,
    /// and prints out its Mys address, Base64 encoded public key, the key scheme, and the key scheme flag.
    ...
    ZkLoginEnterToken {
        ...
        kp_bigint: String,
        #[clap(long)]
        ephemeral_key_identifier: MysAddress,
        #[clap(long, default_value = "devnet")]
        network: String,
        #[clap(long, default_value = "false")]
        test_multisig: bool,
    ...
    /// Example request: mys keytool zk-login-sig-verify --sig $SERIALIZED_ZKLOGIN_SIG --bytes $BYTES --intent-scope 0 --network devnet --curr-epoch 10
    ZkLoginSigVerify {
    ...
    /// e.g. mys keytool zk-login-insecure-sign-personal-message --data "hello" --max-epoch 5
...
pub struct ConvertOutput {
    bech32_with_flag: String, // latest Mys Keystore and Mys Wallet import/export format
    base64_with_flag: String, // Mys Keystore storage format
    hex_without_flag: String, // Legacy Mys Wallet format
    scheme: String,
}
...
pub struct SignData {
    ...
    // serialized Mys signature to be send for executing the transaction.
    mys_signature: String,
}
    ...
    pub async fn execute(self, keystore: &mut Keystore) -> Result<CommandOutput, anyhow::Error> {
        let cmd_result = Ok(match self {
            ...
            KeyToolCommand::DecodeMultiSig {
                ...
                let address = MysAddress::from(multisig.get_pk());
            ...
            KeyToolCommand::Generate {
                ...
                SignatureScheme::BLS12381 => {
                    let (mys_address, kp) = get_authority_key_pair();
                    let file_name = format!("bls-{mys_address}.key");
                ...
                _ => {
                    let (mys_address, skp, _scheme, phrase) =
                        generate_new_key(key_scheme, derivation_path, word_length)?;
                    let file = format!("{mys_address}.key");
            ...
            KeyToolCommand::Import {
                ...
                match MysKeyPair::decode(&input_string) {
                    Ok(skp) => {
                        ...
                        let alias = match alias {
                            Some(x) => x,
                            None => keystore.get_alias_by_address(&key.mys_address)?,
                        };
                    ...
                    Err(_) => {
                        ...
                        let mys_address = keystore.import_from_mnemonic(
                            &input_string,
                            key_scheme,
                            derivation_path,
                            alias.clone(),
                        )?;
                        let skp = keystore.get_key(&mys_address)?;
                        let mut key = Key::from(skp);
            ...
            KeyToolCommand::List { sort_by_alias } => {
                let mut keys = keystore
                    ...
                    .map(|pk| {
                        ...
                        key.alias = keystore.get_alias_by_address(&key.mys_address).ok();
                        key
                    })
                    .collect::<Vec<Key>>();
                if sort_by_alias {
                ...
                let output = match read_keypair_from_file(&file) {
                    Ok(keypair) => {
                        ...
                        let network_worker_keypair = match &keypair {
                            MysKeyPair::Ed25519(kp) => kp.encode_base64(),
                            MysKeyPair::Secp256k1(kp) => kp.encode_base64(),
                            MysKeyPair::Secp256r1(kp) => kp.encode_base64(),
                        };
                        KeypairData {
            ...
            KeyToolCommand::MultiSigAddress {
                ...
                for (pk, w) in pks.into_iter().zip(weights.into_iter()) {
                    output.multisig.push(MultiSigOutput {
                        address: Into::<MysAddress>::into(&pk),
                        public_base64_key: pk.encode_base64(),
                        weight: w,
                    });
                }
                    ...
                    Err(_) => match read_authority_keypair_from_file(&file) {
                        Ok(keypair) => {
                            ...
                            CommandOutput::Show(Key {
                                ...
                                mys_address: (keypair.public()).into(),
                                public_base64_key,
                                key_scheme: SignatureScheme::BLS12381.to_string(),
                                flag: SignatureScheme::BLS12381.flag(),
                                peer_id: None,
            ...
            KeyToolCommand::Sign {
                ...
                let intent = intent.unwrap_or_else(Intent::mys_transaction);
                ...
                CommandOutput::Sign(SignData {
                    ...
                    mys_signature: mys_signature.encode_base64(),
                })
            }
            ...
            KeyToolCommand::SignKMS {
                ...
                let pk_owner = PublicKey::decode_base64(&base64pk)
                    .map_err(|e| anyhow!("Invalid base64 key: {:?}", e))?;
                let address_owner = MysAddress::from(&pk_owner);
            ...
            KeyToolCommand::Unpack { keypair } => {
                let keypair = MysKeyPair::decode_base64(&keypair)
                    .map_err(|_| anyhow!("Invalid Base64 encode keypair"))?;

                let key = Key::from(&keypair);
                let path_str = format!("{}.key", key.mys_address).to_lowercase();
            ...
            KeyToolCommand::ZkLoginSignAndExecuteTx {
                ...
                let skp = if fixed {
                    ...
                    MysKeyPair::Ed25519(Ed25519KeyPair::generate(&mut rand::thread_rng()))
                };
                println!("Ephemeral keypair: {:?}", skp.encode());
                let pk = skp.public();
                let ephemeral_key_identifier: MysAddress = (&skp.public()).into();
...
impl From<&MysKeyPair> for Key {
    fn from(skp: &MysKeyPair) -> Self {
        Key::from(skp.public())
    }
}
...
impl From<PublicKey> for Key {
    fn from(pk: PublicKey) -> Self {
        Key {
            ...
            mys_address: MysAddress::from(&pk),
            public_base64_key: pk.encode_base64(),
...
fn convert_private_key_to_bech32(value: String) -> Result<ConvertOutput, anyhow::Error> {
    let skp = match MysKeyPair::decode(&value) {
        Ok(s) => s,
        Err(_) => match Hex::decode(&value) {
            Ok(decoded) => {
                ...
                MysKeyPair::Ed25519(Ed25519KeyPair::from_bytes(&decoded)?)
            }
            Err(_) => match MysKeyPair::decode_base64(&value) {
                Ok(skp) => skp,
                Err(_) => match Ed25519KeyPair::decode_base64(&value) {
                    Ok(kp) => MysKeyPair::Ed25519(kp),
