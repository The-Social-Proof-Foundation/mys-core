...
pub async fn perform_zk_login_test_tx(
    ...
    let skp1 = MysKeyPair::Ed25519(Ed25519KeyPair::generate(&mut StdRng::from_seed([1; 32])));
    ...
    let sender = if test_multisig {
        ...
        MysAddress::from(&multisig_pk)
    } else {
        println!("Use single zklogin address as sender");
        MysAddress::try_from_unpadded(&zk_login_inputs)?
    };
    println!("Sender: {:?}", sender);

    // Request some coin from faucet and build a test transaction.
    let mys = MysClientBuilder::default().build(fullnode_url).await?;
    ...
    if response.data.len() != 2 {
        panic!("Faucet did not work correctly and the provided Mys address has no coins")
    }
    ...
    let txb_res = mys
        ...
        .transfer_object(
            ...
            Some(gas_coin),
            5000000,
            MysAddress::ZERO, // as a demo, send to a dummy address
        )
        .await?;
    println!(
