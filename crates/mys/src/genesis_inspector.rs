...
use mys_types::{
    base_types::ObjectID,
    coin::CoinMetadata,
    gas_coin::{GasCoin, MIST_PER_MYS, TOTAL_SUPPLY_MIST},
...
const STR_MYS: &str = "Mys";
const STR_STAKED_MYS: &str = "StakedMys";
const STR_PACKAGE: &str = "Package";
const STR_COIN_METADATA: &str = "CoinMetadata";
const STR_OTHER: &str = "Other";
const STR_MYS_DISTRIBUTION: &str = "Mys Distribution";
...
pub(crate) fn examine_genesis_checkpoint(genesis: UnsignedGenesis) {
    ...
    // Prepare Mys distribution info
    let mut mys_distribution = BTreeMap::new();
    let entry = mys_distribution
        .entry("Mys System".to_string())
    ...
    entry.insert(
        ...
        (STR_MYS, system_object.stake_subsidy.balance.value()),
    );

    // Prepare Object Info
    let mut owner_map = BTreeMap::new();
    let mut package_map = BTreeMap::new();
    let mut mys_map = BTreeMap::new();
    let mut staked_mys_map = BTreeMap::new();
    ...
    for object in genesis.objects() {
        ...
        match &object.data {
            mys_types::object::Data::Move(move_object) => {
                if let Ok(gas) = GasCoin::try_from(object) {
                    let entry = mys_distribution
                        .entry(object.owner.to_string())
                        .or_default();
                    entry.insert(object_id_str.clone(), (STR_MYS, gas.value()));
                    mys_map.insert(object.id(), gas);
                } else if let Ok(coin_metadata) = CoinMetadata::try_from(object) {
                    coin_metadata_map.insert(object.id(), coin_metadata);
                } else if let Ok(staked_mys) = StakedMys::try_from(object) {
                    let entry = mys_distribution
                        .entry(object.owner.to_string())
                        .or_default();
                    entry.insert(object_id_str, (STR_STAKED_MYS, staked_mys.principal()));
                    // Assert pool id is associated with a knonw validator.
                    let validator = validator_pool_id_map.get(&staked_mys.pool_id()).unwrap();
                    assert_eq!(validator.staking_pool.id, staked_mys.pool_id());

                    staked_mys_map.insert(object.id(), staked_mys);
                } else {
                    other_object_map.insert(object.id(), move_object);
                }
            }
            mys_types::object::Data::Package(p) => {
    ...
    examine_total_supply(&mys_distribution, false);

    // Main loop for inspection
    let main_options: Vec<&str> = vec![STR_MYS_DISTRIBUTION, STR_VALIDATORS, STR_OBJECTS, STR_EXIT];
    loop {
        let ans = Select::new(
        ...
        .prompt();
        match ans {
            Ok(name) if name == STR_MYS_DISTRIBUTION => {
                examine_total_supply(&mys_distribution, true)
            }
...
fn examine_validators(
    ...
    validator_map: &BTreeMap<&str, &MysValidatorGenesis>,
) {
    loop {
...
fn examine_object(
    ...
    validator_pool_id_map: &BTreeMap<ObjectID, &MysValidatorGenesis>,
    package_map: &BTreeMap<ObjectID, &MovePackage>,
    mys_map: &BTreeMap<ObjectID, GasCoin>,
    staked_mys_map: &BTreeMap<ObjectID, StakedMys>,
    ...
    loop {
        ...
        match ans {
            Ok(name) if name == STR_EXIT => break,
            Ok(name) if name == STR_MYS => {
                for gas_coin in mys_map.values() {
                    display_mys(gas_coin, owner_map);
                }
                print_divider("Mys");
            }
            Ok(name) if name == STR_STAKED_MYS => {
                for staked_mys_coin in staked_mys_map.values() {
                    display_staked_mys(staked_mys_coin, validator_pool_id_map, owner_map);
                }
                print_divider(STR_STAKED_MYS);
            }
...
fn examine_total_supply(
    mys_distribution: &BTreeMap<String, BTreeMap<String, (&str, u64)>>,
    print: bool,
) {
    let mut total_mys = 0;
    let mut total_staked_mys = 0;
    for (owner, coins) in mys_distribution {
        let mut amount_sum = 0;
        for (owner, value) in coins.values() {
            amount_sum += value;
            if *owner == STR_STAKED_MYS {
                total_staked_mys += value;
            }
        }
        total_mys += amount_sum;
        if print {
            println!("Owner {:?}", owner);
            println!(
                "Total Amount of Mys/StakedMys Owned: {amount_sum} MIST or {} MYS:",
                amount_sum / MIST_PER_MYS
            );
            println!("{:#?}\n", coins);
        }
    }
    assert_eq!(total_mys, TOTAL_SUPPLY_MIST);
    // Always print this.
    println!(
        "Total Supply of Mys: {total_mys} MIST or {} MYS",
        total_mys / MIST_PER_MYS
    );
    println!(
        "Total Amount of StakedMys: {total_staked_mys} MIST or {} MYS\n",
        total_staked_mys / MIST_PER_MYS
    );
    if print {
        print_divider("Mys Distribution");
    }
}

fn display_validator(validator: &MysValidatorGenesis) {
    ...
    println!(
        "Staking Pool Mys Balance: {:?}",
        validator.staking_pool.mys_balance
    );
    println!(
        "Rewards Pool: {}",
    ...
    println!(
        "Pending Total Mys Withdraw: {}",
        validator.staking_pool.pending_total_mys_withdraw
    );
    println!(
...
fn display_mys(gas_coin: &GasCoin, owner_map: &BTreeMap<ObjectID, Owner>) {
    println!("ID: {}", gas_coin.id());
    println!("Balance: {}", gas_coin.value());
    println!("Owner: {}\n", owner_map.get(gas_coin.id()).unwrap());
}

fn display_staked_mys(
    ...
    println!("Owner: {}\n", owner_map.get(&staked_mys.id()).unwrap());
}
