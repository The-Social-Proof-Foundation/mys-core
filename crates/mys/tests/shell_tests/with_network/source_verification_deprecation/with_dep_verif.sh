...
FRAMEWORK_DIR=$(echo $CARGO_MANIFEST_DIR | sed 's#/crates/mys#/crates/mys-framework/packages/mys-framework#g')
...
mys client --client.config $CONFIG publish "dependency" --verify-deps \
  --json | jq '.effects.status'

echo "=== publish package v0 (should NOT warn) ===" | tee /dev/stderr
UPGRADE_CAP=$(mys client --client.config $CONFIG publish "example" --verify-deps \
...
mys client --client.config $CONFIG upgrade --upgrade-capability $UPGRADE_CAP example --verify-deps \
  --json | jq '.effects.status'
...
mys client --client.config $CONFIG publish "example" --verify-deps \
