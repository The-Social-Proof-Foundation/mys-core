...
/// of [path], with the `mys` binary on the path.
///
...
async fn test_shell_snapshot(path: &Path) -> datatest_stable::Result<()> {
    ...
    shell
        .env(
            "PATH",
            format!("{}:{}", get_mys_bin_path(), std::env::var("PATH")?),
        )
    ...
    if let Some(ref cluster) = cluster {
        shell.env("CONFIG", cluster.swarm.dir().join(MYS_CLIENT_CONFIG));
    }
...
/// return the path to the `mys` binary that is currently under test
fn get_mys_bin_path() -> String {
    get_cargo_bin("mys")
        .parent()
        .unwrap()
        .to_str()
