...
use mys_indexer_builder::progress::ProgressSavingPolicy;
use mys_types::base_types::ObjectID;
use mys_types::transaction::{Command, TransactionDataAPI};
use tracing::info;

use mys_indexer_builder::indexer_builder::{DataMapper, IndexerProgressStore, Persistent};
use mys_indexer_builder::mys_datasource::CheckpointTxnData;
use mys_indexer_builder::{Task, Tasks, LIVE_TASK_TARGET_CHECKPOINT};
...
use crate::metrics::OrderBookIndexerMetrics;
...
#[derive(Clone)]
pub struct PgOrderbookPersistent {
    pub pool: PgPool,
    save_progress_policy: ProgressSavingPolicy,
}

impl PgOrderbookPersistent {
...
#[async_trait]
impl Persistent<ProcessedTxnData> for PgOrderbookPersistent {
            ...
            .transaction(|conn| {
                async move {
                    ...
                    if !error_transactions_batch.is_empty() {
                        tasks.push(
                            diesel::insert_into(mys_error_transactions::table)
...
#[async_trait]
impl IndexerProgressStore for PgOrderbookPersistent {
...
/// Data mapper impl
#[derive(Clone)]
pub struct MysOrderBookDataMapper {
    pub metrics: OrderBookIndexerMetrics,
    pub package_id: ObjectID,
}

impl DataMapper<CheckpointTxnData, ProcessedTxnData> for MysOrderBookDataMapper {
    fn map(
        &self,
        ...
        self.metrics.total_orderbook_transactions.inc();

        match &data.events {
            Some(events) => {
                let processed_mys_events =
                    events
                        .data
                        .iter()
                        .enumerate()
                        .try_fold(vec![], |mut result, (i, ev)| {
                            if let Some(data) = process_mys_event(
                                ev,
                                i,
                                &data,
                                checkpoint_num,
                ...
                if !processed_mys_events.is_empty() {
                    info!(
                        "MYS: Extracted {} orderbook data entries for tx {}.",
                        processed_mys_events.len(),
                        data.transaction.digest()
                    );
                }
                Ok(processed_mys_events)
            }
            None => {
    ...
    Ok(if ev.type_.address == *package_id {
        match ev.type_.name.as_str() {
            "OrderPlaced" => {
                ...
                info!("Observed orderbook Order Placed {:?}", txn_data);
            ...
            "OrderModified" => {
                ...
                info!("Observed orderbook Order Modified {:?}", txn_data);
            ...
            "OrderCanceled" => {
                ...
                info!("Observed orderbook Order Canceled {:?}", txn_data);
            ...
            "OrderExpired" => {
                ...
                info!("Observed orderbook Order Expired {:?}", txn_data);
            ...
            "OrderFilled" => {
                ...
                info!("Observed orderbook Order Filled {:?}", txn_data);
            ...
            "FlashLoanBorrowed" => {
                ...
                info!("Observed orderbook Flash Loan Borrowed {:?}", txn_data);
            ...
            "PriceAdded" => {
                ...
                info!("Observed orderbook Price Addition {:?}", txn_data);
            ...
            "BalanceEvent" => {
                ...
                info!("Observed orderbook Balance Event {:?}", txn_data);
            ...
            "ProposalEvent" => {
                ...
                info!("Observed orderbook Proposal Event {:?}", txn_data);
            ...
            "RebateEvent" => {
                ...
                info!("Observed orderbook Rebate Event {:?}", txn_data);
            ...
            "StakeEvent" => {
                ...
                info!("Observed orderbook Stake Event {:?}", txn_data);
            ...
            "TradeParamsUpdateEvent" => {
                ...
                info!("Observed orderbook Trade Params Update Event {:?}", txn_data);
            ...
            "VoteEvent" => {
                ...
                info!("Observed orderbook Vote Event {:?}", txn_data);
