...
use mys_json_rpc_types::{
    EventPage, MysEvent, MysExecutionStatus, MysTransactionBlockEffectsAPI,
...
use mys_test_transaction_builder::{
...
use mys_types::messages_grpc::TransactionInfoRequest;
use mys_types::object::{Object, ObjectRead, Owner, PastObjectRead};
...
async fn test_full_node_follows_txes() -> Result<(), anyhow::Error> {
    ...
    let fullnode = test_cluster.spawn_new_fullnode().await.mys_node;

    let context = &mut test_cluster.wallet;

    // TODO: test fails on CI due to flakiness without this. Once https://github.com/MystenLabs/mys/pull/7056 is
...
async fn test_full_node_move_function_index() -> Result<(), anyhow::Error> {
    telemetry_subscribers::init_for_testing();
    let mut test_cluster = TestClusterBuilder::new().build().await;
    let node = &test_cluster.fullnode_handle.mys_node;
    let sender = test_cluster.get_address_0();
    let context = &mut test_cluster.wallet;
...
async fn test_full_node_indexes() -> Result<(), anyhow::Error> {
    ...
    let sender_balance_change = BalanceChange {
        ...
        owner: sender,
        coin_type: parse_struct_tag("0x2::mys::MYS").unwrap(),
    ...
    for _i in 0..5 {
        ...
        tokio::task::spawn(async move {
            ...
            for _ in 0..10 {
                ...
                owned_tx_digest = if let MysClientCommandResult::TransactionBlock(resp) = res {
                    Some(resp.digest)
                } else {
                    panic!(
                        "SplitCoin command did not return MysClientCommandResult::TransactionBlock"
                    );
                };
...
async fn test_full_node_event_read_api_ok() {
    ...
    let events: Vec<MysEvent> = jsonrpc_client
        .request("mys_getEvents", params)
        .await
        .unwrap();
...
async fn test_full_node_event_query_by_module_ok() {
    ...
    let page: EventPage = jsonrpc_client
        .request("mysx_queryEvents", params)
        .await
        .unwrap();
...
async fn test_execute_tx_with_serialized_signature() -> Result<(), anyhow::Error> {
    ...
    context
        .config
        .keystore
        .add_key(None, MysKeyPair::Secp256k1(get_key_pair().1))?;
    context
        .config
        .keystore
    ...
    for txn in txns {
        ...
        let response: MysTransactionBlockResponse = jsonrpc_client
            .request("mys_executeTransactionBlock", params)
            .await
            .unwrap();

        let MysTransactionBlockResponse {
            digest,
...
async fn test_full_node_transaction_orchestrator_rpc_ok() -> Result<(), anyhow::Error> {
    ...
    let _response: MysTransactionBlockResponse = jsonrpc_client
        .request("mys_getTransactionBlock", rpc_params![*tx_digest])
        .await
        .unwrap();
...
async fn test_get_objects_read() -> Result<(), anyhow::Error> {
    ...
    // Transfer some MYS to recipient
...
async fn test_pass_back_no_object() -> Result<(), anyhow::Error> {
    ...
    // TODO: this is publishing the wrong package - we should be publishing the one in `mys-core/src/unit_tests/data` instead.
...
async fn test_access_old_object_pruned() {
    ...
    let effects = test_cluster
        .sign_and_execute_transaction(&tx_builder.transfer_mys(None, sender).build())
        .await
        .effects
        .unwrap();
...
async fn test_full_node_run_with_range_checkpoint() -> Result<(), anyhow::Error> {
    ...
    assert!(test_cluster.fullnode_handle.mys_node.with(|node| {
        node.state()
...
async fn publish_init_events_without_local_execution() {
    ...
    let response = client
        ...
        .execute_transaction_block(
            tx,
            MysTransactionBlockResponseOptions::new().with_events(),
