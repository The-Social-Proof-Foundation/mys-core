...
use mys_swarm_config::genesis_config::{ValidatorGenesisConfig, ValidatorGenesisConfigBuilder};
use mys_test_transaction_builder::{make_transfer_mys_transaction, TestTransactionBuilder};
...
use mys_types::mys_system_state::{
    get_validator_from_table, mys_system_state_summary::get_validator_by_pool_id,
    MysSystemStateTrait,
};
...
async fn test_transaction_expiration() {
    ...
    *expired_data.expiration_mut_for_testing() = TransactionExpiration::Epoch(0);
    ...
    assert!(result
        .unwrap_err()
        .to_string()
        .contains(&MysError::TransactionExpired.to_string()));
...
async fn test_expired_locks() {
    ...
    let t1 = transfer_mys(1);
    // attempt to equivocate
    let t2 = transfer_mys(2);
...
async fn test_create_advance_epoch_tx_race() {
    ...
    let register_wait = |failpoint, node_id, tx: Arc<broadcast::Sender<()>>| {
        let node = mys_simulator::task::NodeId(node_id);
...
async fn test_validator_resign_effects() {
    ...
    // in the new epoch.
    let test_cluster = TestClusterBuilder::new().build().await;
    let tx = make_transfer_mys_transaction(&test_cluster.wallet, None, None).await;
...
async fn test_validator_candidate_pool_read() {
    ...
    let address: MysAddress = (&new_validator.account_key_pair.public()).into();
    let test_cluster = TestClusterBuilder::new()
        .with_validator_candidates([address])
        .build()
        .await;
    add_validator_candidate(&test_cluster, &new_validator).await;
    test_cluster.fullnode_handle.mys_node.with(|node| {
        let system_state = node
            .state()
            .get_mys_system_state_object_for_testing()
            .unwrap();
        let system_state_summary = system_state.clone().into_mys_system_state_summary();
        ...
        .unwrap();
        assert_eq!(validator.mys_address, address);
    });
}
...
async fn test_inactive_validator_pool_read() {
    ...
    let address = validator.with(|node| node.get_config().mys_address());
    let staking_pool_id = test_cluster.fullnode_handle.mys_node.with(|node| {
        node.state()
            .get_mys_system_state_object_for_testing()
            .unwrap()
...
async fn test_epoch_flag_upgrade() {
    ...
    register_fail_point_arg("initial_epoch_flags", move || {
        ...
        let current_node = mys_simulator::current_simnode_id();
...
async fn safe_mode_reconfig_test() {
    ...
    use mys_types::mys_system_state::advance_epoch_result_injection;
...
async fn add_validator_candidate(
    ...
    let cur_validator_candidate_count = test_cluster.fullnode_handle.mys_node.with(|node| {
        node.state()
            .get_mys_system_state_object_for_testing()
            .unwrap()
            .into_mys_system_state_summary()
    ...
    test_cluster.fullnode_handle.mys_node.with(|node| {
        let system_state = node
            ...
            .get_mys_system_state_object_for_testing()
            .unwrap();
        let system_state_summary = system_state.into_mys_system_state_summary();
        assert_eq!(
...
async fn execute_remove_validator_tx(test_cluster: &TestCluster, handle: &MysNodeHandle) {
    let address = handle.with(|node| node.get_config().mys_address());
    let gas = test_cluster
        .wallet
        .get_one_gas_object_owned_by_address(address)
        .await
...
async fn execute_add_validator_transactions(
    ...
    let pending_active_count = test_cluster.fullnode_handle.mys_node.with(|node| {
        let system_state = node
            .state()
            .get_mys_system_state_object_for_testing()
            .unwrap();
    ...
    test_cluster.fullnode_handle.mys_node.with(|node| {
        ...
        assert_eq!(
            pending_active_validators[pending_active_validators.len() - 1].mys_address,
            address
        );
    });
}
