...
mod sim_only_tests {
    ...
    async fn object_pruning_test() {
        ...
        let fullnode = &test_cluster.fullnode_handle.mys_node;
    ...
    async fn publish_package_and_create_parent_object(
        ...
        let package_id = publish_package(
            ...
            PathBuf::from(env!("CARGO_MANIFEST_DIR"))
                .join("../mys-surfer/tests/move_building_blocks"),
        )
        .await
        .0;
    ...
    async fn wait_until_checkpoint_pruned(node: &MysNode, checkpoint: CheckpointSequenceNumber) {
        loop {
            if node
                .state()
