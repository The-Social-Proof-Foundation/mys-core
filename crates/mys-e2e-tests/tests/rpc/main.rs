...
async fn transfer_coin(
    context: &mys_sdk::wallet_context::WalletContext,
) -> mys_sdk_types::TransactionDigest {
    ...
    let txn = context.sign_transaction(
        &mys_test_transaction_builder::TestTransactionBuilder::new(sender, gas_object, gas_price)
...
async fn stake_with_validator(
    ...
    let validator_address = cluster.swarm.config().validator_configs()[0].mys_address();
    let txn = context.sign_transaction(
        &mys_test_transaction_builder::TestTransactionBuilder::new(sender, gas_object, gas_price)
