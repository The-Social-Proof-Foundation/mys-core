...
async fn get_checkpoint() {
    ...
    mys_rpc_api::types::CheckpointResponse::try_from(&response).unwrap();
...
async fn get_full_checkpoint() {
    ...
    let checkpoint = grpc_client
        .get_transaction(mys_rpc_api::proto::node::v2::GetTransactionRequest::new(
            transaction_digest,
        ))
        .await
        .unwrap()
    ...
    mys_rpc_api::types::FullCheckpointResponse::try_from(&response).unwrap();
...
async fn subscribe_checkpoint() {
    use mys_rpc_api::proto::node::v2alpha::subscription_service_client::SubscriptionServiceClient;
    use mys_rpc_api::proto::node::v2alpha::SubscribeCheckpointsRequest;
