...
mod sim_only_tests {
    ...
    use mys_types::{
        ...
        MOVE_STDLIB_PACKAGE_ID, MYS_BRIDGE_OBJECT_ID, MYS_FRAMEWORK_PACKAGE_ID,
    ...
    async fn test_framework_add_struct_ability() {
        ...
        transfer_obj(&cluster, MysAddress::ZERO, to_transfer0).await;
        wrap_obj(&cluster, to_wrap1).await;
        transfer_obj(&cluster, MysAddress::ZERO, to_transfer1).await;
    }
    ...
    async fn test_new_framework_package() {
        ...
        let mys_extra = ObjectID::from_single_byte(0x42);
        framework_injection::set_override(mys_extra, fixture_modules("extra_package"));
        ...
        // initializer
        let effects = get_framework_upgrade_effects(&cluster, &mys_extra).await;
            ...
            .find_map(|(obj, owner)| {
                if let Owner::Shared { .. } = owner {
                    let is_framework_obj = [
                        MYS_SYSTEM_STATE_OBJECT_ID,
                        MYS_CLOCK_OBJECT_ID,
                        MYS_AUTHENTICATOR_STATE_OBJECT_ID,
                        MYS_RANDOMNESS_STATE_OBJECT_ID,
                        MYS_BRIDGE_OBJECT_ID,
                    ]
                    .contains(&obj.0);
    ...
    async fn run_framework_upgrade(from: &str, to: &str) -> TestCluster {
        ...
        override_mys_system_modules(to);
        TestClusterBuilder::new()
            .with_epoch_duration_ms(20000)
            .with_objects([mys_system_package_object(from)])
            .with_supported_protocol_versions(SupportedProtocolVersions::new_for_testing(
                START, FINISH,
            ))
            .build()
    ...
    async fn get_framework_upgrade_versions(
        ...
        let effects = get_framework_upgrade_effects(cluster, &MYS_SYSTEM_PACKAGE_ID).await;

        let modified_at = effects
            .modified_at_versions()
            .iter()
            .find_map(|(id, v)| (id == &MYS_SYSTEM_PACKAGE_ID).then_some(*v));

        let mutated_to = effects
            .mutated()
            .iter()
            .find_map(|((id, v, _), _)| (id == &MYS_SYSTEM_PACKAGE_ID).then_some(*v));
    ...
    async fn get_framework_upgrade_effects(
        ...
        let node_handle = &cluster.fullnode_handle.mys_node;
    ...
    async fn test_framework_compatible_upgrade_no_protocol_version() {
        ...
        override_mys_system_modules("compatible");
        let test_cluster = TestClusterBuilder::new()
            .with_epoch_duration_ms(20000)
            .with_objects([mys_system_package_object("base")])
            .with_supported_protocol_versions(SupportedProtocolVersions::new_for_testing(
                START, START,
            ))
            .build()
    ...
    async fn test_framework_upgrade_conflicting_versions() {
        ...
        override_mys_system_modules_cb(Box::new(move |name| {
            if name == first_name {
                info!("node {:?} using compatible packages", name.concise());
                Some(mys_system_modules("base"))
            } else {
                Some(mys_system_modules("compatible"))
            }
        }));
    ...
    async fn test_safe_mode_recovery() {
        ...
        override_mys_system_modules("mock_mys_systems/base");
        let test_cluster = TestClusterBuilder::new()
            .with_epoch_duration_ms(20000)
            // Overrides with a mys system package that would abort during epoch change txn
            .with_objects([mys_system_package_object("mock_mys_systems/safe_mode")])
            .with_supported_protocol_versions(SupportedProtocolVersions::new_for_testing(
                START, FINISH,
            ))
            .build()
    ...
    #[sim_test]
    async fn mys_system_mock_smoke_test() {
        let _guard = ProtocolConfig::apply_overrides_for_testing(|_, mut config| {
            config.set_disable_bridge_for_testing();
            config
        });
        ...
        let test_cluster = TestClusterBuilder::new()
            ...
            .with_objects([mys_system_package_object("mock_mys_systems/base")])
            .build()
            .await;
    ...
    #[sim_test]
    async fn mys_system_state_shallow_upgrade_test() {
        let _guard = ProtocolConfig::apply_overrides_for_testing(|_, mut config| {
            config.set_disable_bridge_for_testing();
            config
        });

        override_mys_system_modules("mock_mys_systems/shallow_upgrade");
        ...
        assert!(matches!(system_state, MysSystemState::SimTestV1(_)));
        ...
        assert!(matches!(system_state, MysSystemState::SimTestShallowV2(_)));
    }

    #[sim_test]
    async fn mys_system_state_deep_upgrade_test() {
        let _guard = ProtocolConfig::apply_overrides_for_testing(|_, mut config| {
            config.set_disable_bridge_for_testing();
            config
        });

        override_mys_system_modules("mock_mys_systems/deep_upgrade");
        ...
        if let MysSystemState::SimTestV1(inner) = system_state {
        ...
        if let MysSystemState::SimTestDeepV2(inner) = system_state {
            // Make sure we have 1 inactive validator for latter testing.
            assert_eq!(inner.validators.inactive_validators.size, 1);
            get_validator_from_table(
                test_cluster
    ...
    #[sim_test]
    async fn mys_system_state_production_upgrade_test() {
        // Use this test to test a real mys system state upgrade. To make this test work,
        // put the new mys system in a new path and point to it in the override.
        ...
        override_mys_system_modules("../../../mys-framework/packages/mys-system");
        // Wait for the upgrade to finish. After the upgrade, the new framework will be installed,
        // but the system state object hasn't been upgraded yet.
        let system_state = test_cluster.wait_for_epoch(Some(1)).await;
        assert_eq!(system_state.protocol_version(), FINISH);
        ...
        // at epoch boundary.
        let system_state = test_cluster.wait_for_epoch(Some(2)).await;
        if let MysSystemState::V2(inner) = system_state {
            assert_eq!(inner.parameters.min_validator_count, 4);
        } else {
            unreachable!("Unexpected mys system state version");
        }
    }
    ...
    fn override_mys_system_modules(path: &str) {
        framework_injection::set_override(MYS_SYSTEM_PACKAGE_ID, mys_system_modules(path));
    }

    fn override_mys_system_modules_cb(f: framework_injection::PackageUpgradeCallback) {
        framework_injection::set_override_cb(MYS_SYSTEM_PACKAGE_ID, f)
    }

    /// Get compiled modules for Mys System, built from fixture `fixture` in the
    /// `framework_upgrades` directory.
    fn mys_system_modules(fixture: &str) -> Vec<CompiledModule> {
        fixture_package(fixture)
            .get_mys_system_modules()
            .cloned()
            .collect()
    }

    /// Like `mys_system_modules`, but package the modules in an `Object`.
    fn mys_system_package_object(fixture: &str) -> Object {
        Object::new_package(
            &mys_system_modules(fixture),
            TransactionDigest::genesis_marker(),
            u64::MAX,
            VERSION_MAX,
            &[
                BuiltInFramework::get_package_by_id(&MOVE_STDLIB_PACKAGE_ID).genesis_move_package(),
                BuiltInFramework::get_package_by_id(&MYS_FRAMEWORK_PACKAGE_ID)
