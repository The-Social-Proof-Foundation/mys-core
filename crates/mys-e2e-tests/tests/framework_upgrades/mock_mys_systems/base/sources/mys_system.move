...
module mys_system::mys_system {
    ...
    fun load_inner_maybe_upgrade(self: &mut MysSystemState): &mut MysSystemStateInner {
        let version = self.version;
        // TODO: This is where we check the version and perform upgrade if necessary.

        let inner: &mut MysSystemStateInner = dynamic_field::borrow_mut(&mut self.id, version);
