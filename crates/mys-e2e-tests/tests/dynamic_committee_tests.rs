...
struct StressTestRunner {
    ...
    pub accounts: Vec<MysAddress>,
    pub active_validators: BTreeSet<MysAddress>,
    pub preactive_validators: BTreeMap<MysAddress, u64>,
    pub removed_validators: BTreeSet<MysAddress>,
    pub delegation_requests_this_epoch: BTreeMap<ObjectID, MysAddress>,
    pub delegation_withdraws_this_epoch: u64,
    pub delegations: BTreeMap<ObjectID, MysAddress>,
    pub reports: BTreeMap<MysAddress, BTreeSet<MysAddress>>,
    pub rng: StdRng,
}
...
impl StressTestRunner {
    ...
    pub fn pick_random_sender(&mut self) -> MysAddress {
        self.accounts[self.rng.gen_range(0..self.accounts.len())]
    }

    pub fn system_state(&self) -> MysSystemStateSummary {
        self.state()
    ...
    pub fn pick_random_active_validator(&mut self) -> MysValidatorSummary {
    ...
    pub async fn run(&self, sender: MysAddress, pt: ProgrammableTransaction) -> TransactionEffects {
    ...
    pub fn display_effects(&self, effects: &TransactionEffects) {
        ...
        for (obj_ref, _) in effects.created() {
            ...
            println!(">> {struct_tag} TOTAL_MYS: {total_mys}");
        }
    ...
    pub fn state(&self) -> Arc<AuthorityState> {
        self.test_cluster.fullnode_handle.mys_node.state()
    }
...
mod add_stake {
    ...
    impl GenStateChange for RequestAddStakeGen {
        ...
        fn create(&self, runner: &mut StressTestRunner) -> Self::StateChange {
            ...
            let staked_with = runner.pick_random_active_validator().mys_address;
    ...
    impl StatePredicate for RequestAddStake {
        async fn run(&mut self, runner: &mut StressTestRunner) -> Result<TransactionEffects> {
            let pt = {
                ...
                builder.obj(ObjectArg::MYS_SYSTEM_MUT).unwrap();
                builder.pure(self.staked_with).unwrap();
                let coin = StressTestRunner::split_off(&mut builder, self.stake_amount);
                move_call! {
                    builder,
                    (MYS_SYSTEM_PACKAGE_ID)::mys_system::request_add_stake(Argument::Input(0), coin, Argument::Input(1))
                };
                builder.finish()
            };
        ...
        async fn pre_epoch_post_condition(
            ...
            // Assert that a `StakedMys` object matching the amount delegated is created.
            // Assert that this staked mys
            let object = runner
                .get_created_object_of_type_name(effects, "StakedMys")
                .await
                .unwrap();
