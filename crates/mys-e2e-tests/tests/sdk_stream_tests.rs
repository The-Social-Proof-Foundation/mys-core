...
//     let client = MysClientBuilder::default().build(rpc_url).await?;
//     let txs = client
//         .read_api()
//         .get_transactions_stream(MysTransactionBlockResponseQuery::default(), None, true)
//         .collect::<Vec<_>>()
//         .await;
...
//     MysClientCommands::CreateExampleNFT {
//         name: None,
//         description: None,
//         url: None,
//         gas: None,
...
async fn test_coins_stream() -> Result<(), anyhow::Error> {
    ...
    let coins = client
        .coin_read_api()
        .get_coins_stream(address, Some(MYS_COIN_TYPE.to_string()))
        .collect::<Vec<_>>()
        .await;
    ...
    let page = client
        .coin_read_api()
        .get_coins(address, Some(MYS_COIN_TYPE.to_string()), None, None)
        .await?;
