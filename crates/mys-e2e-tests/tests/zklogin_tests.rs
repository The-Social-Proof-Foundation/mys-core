...
async fn do_zklogin_test(address: MysAddress, legacy: bool) -> MysResult {
...
async fn test_zklogin_feature_legacy_address_deny() {
    ...
    let err = do_zklogin_test(get_legacy_zklogin_user_address(), true)
        .await
        .unwrap_err();
    assert!(matches!(err, MysError::SignerSignatureAbsent { .. }));
}
...
async fn test_legacy_zklogin_address_accept() {
    ...
    assert!(matches!(err, MysError::InvalidSignature { .. }));
}
    ...
    for h in &handles {
        h.with(|node| {
            assert!(node
                ...
                .get_latest_object_ref_or_tombstone(MYS_AUTHENTICATOR_STATE_OBJECT_ID)
                .is_none());
        });
    }
