...
async fn test_fullnode_traffic_control_dry_run() -> Result<(), anyhow::Error> {
    ...
    let response: MysTransactionBlockResponse = jsonrpc_client
        .request("mys_executeTransactionBlock", params.clone())
        .await
        .unwrap();
    ...
    for _ in 0..txn_count {
        let response: Result<MysTransactionBlockResponse, _> = jsonrpc_client
...
async fn test_validator_traffic_control_error_blocked() -> Result<(), anyhow::Error> {
    ...
    signatures.push(GenericSignature::Signature(
        mys_types::crypto::Signature::Ed25519MysSignature(Ed25519MysSignature::default()),
    ));
