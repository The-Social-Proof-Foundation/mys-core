...
use mys_types::multisig_legacy::MultiSigLegacy;
use mys_types::{
...
async fn test_upgraded_multisig_feature_allow() {
    ...
    // feature gate.
    assert!(matches!(res.unwrap_err(), MysError::UserInputError { .. }));
}
...
async fn test_multisig_e2e() {
    ...
    .unwrap();
    let wrong_sender = MysAddress::from(&wrong_multisig_pk);
    let gas = test_cluster
...
async fn test_multisig_with_zklogin_scenerios() {
    ...
    let (eph_kp_1, _, _) =
        &load_test_vectors("../mys-types/src/unit_tests/zklogin_test_vectors.json")[2];
    ...
    let wrong_multisig_addr = MysAddress::from(
    ...
    let sender = MysAddress::try_from(&multisig).unwrap();
    ...
    let bad_multisig_addr = MysAddress::from(&bad_multisig_pk);
    let gas = test_cluster
    ...
    let bad_multisig_addr_2 = MysAddress::from(&bad_multisig_pk_2);
    let gas = test_cluster
    ...
    let bad_multisig_addr_3 = MysAddress::from(&small_multisig_pk);
    let gas = test_cluster
    ...
    let bad_multisig_addr_4 = MysAddress::from(&multisig_pk_with_dup);
    let gas = test_cluster
    ...
    let bad_multisig_addr_11 = MysAddress::from(&multisig_pk_11);
    let gas = test_cluster
    ...
    let bad_multisig_addr = MysAddress::from(&multisig_pk_12);
    let gas = test_cluster
        .fund_address_and_return_gas(rgp, Some(20000000000), bad_multisig_addr)
        .await;
    let tx_data = TestTransactionBuilder::new(bad_multisig_addr, gas, rgp)
    ...
    let bad_multisig_addr = MysAddress::from(&bad_multisig_empty_pk);
    let gas = test_cluster
        .fund_address_and_return_gas(rgp, Some(20000000000), bad_multisig_addr)
        .await;
    let tx_data = TestTransactionBuilder::new(bad_multisig_addr, gas, rgp)
...
async fn test_random_zklogin_in_multisig() {
    let test_vectors =
        &load_test_vectors("../mys-types/src/unit_tests/zklogin_test_vectors.json")[1..11];
    let test_cluster = TestClusterBuilder::new()
        .with_epoch_duration_ms(15000)
        .with_default_jwks()
        .build()
...
async fn test_multisig_legacy_works() {
    ...
    let transfer_from_multisig = TestTransactionBuilder::new(multisig_addr, gas, rgp)
        .transfer_mys(Some(1000000), MysAddress::ZERO)
