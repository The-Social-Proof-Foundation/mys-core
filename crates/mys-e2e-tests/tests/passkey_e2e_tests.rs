...
async fn execute_tx(tx: Transaction, test_cluster: &TestCluster) -> MysResult {
    test_cluster
...
async fn create_credential_and_sign_test_tx(
    ...
    // Compute mys address as sender, fund gas and make a test transaction.
    let sender = match sender {
        Some(s) => s,
        None => MysAddress::from(&pk),
    };
...
async fn test_passkey_fails_wrong_author() {
    ...
    let response =
        create_credential_and_sign_test_tx(&test_cluster, Some(MysAddress::ZERO), false, false)
            .await;
    let sig = GenericSignature::PasskeyAuthenticator(
        PasskeyAuthenticator::new_for_testing(
            response.authenticator_data,
