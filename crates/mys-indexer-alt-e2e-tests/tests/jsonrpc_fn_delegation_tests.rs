...
use mys_indexer_alt_jsonrpc::NodeArgs;
use mys_indexer_alt_jsonrpc::RpcArgs;
use mys_indexer_alt_jsonrpc::args::SystemPackageTaskArgs;
use mys_indexer_alt_jsonrpc::config::RpcConfig;
...
impl FnDelegationTestCluster {
    ...
    async fn transfer_transaction(&self) -> anyhow::Result<(String, String, Vec<String>)> {
        ...
        let tx = self
            ...
            .await
            .transfer_mys(Some(1_000), recipient)
            .build();
    ...
    async fn privileged_transaction(&self) -> anyhow::Result<(String, String, Vec<String>)> {
        let tx: mys_types::transaction::TransactionData = self
            .onchain_cluster
            .test_transaction_builder()
            .await
    ...
    async fn get_validator_address(&self) -> MysAddress {
...
async fn test_execution() {
    ...
    let response = test_cluster
        .execute_jsonrpc(
            "mys_executeTransactionBlock".to_string(),
            json!({
                "tx_bytes": tx_bytes,
                "signatures": sigs,
                "options": {
...
async fn test_dry_run() {
    ...
    let response = test_cluster
        .execute_jsonrpc(
            "mys_dryRunTransactionBlock".to_string(),
            json!({
                "tx_bytes": tx_bytes,
            }),
        )
...
async fn test_get_stakes_and_by_ids() {
    ...
    assert!(get_stakes_response["result"][0]["stakes"][0]["stakedMysId"].is_string());
    let stake_id = get_stakes_response["result"][0]["stakes"][0]["stakedMysId"]
        .as_str()
        .unwrap();
    ...
    let get_stakes_by_ids_response = test_cluster
        .execute_jsonrpc(
            "mysx_getStakesByIds".to_string(),
            json!({ "staked_mys_ids": [stake_id] }),
        )
        .await
        .unwrap();
...
async fn test_get_stakes_invalid_params() {
    ...
    let response = test_cluster
        .execute_jsonrpc(
            "mysx_getStakesByIds".to_string(),
            json!({ "staked_mys_ids": ["invalid_stake_id"] }),
        )
        .await
        .unwrap();
...
async fn test_get_validators_apy() {
    ...
    let response = test_cluster
        .execute_jsonrpc("mysx_getValidatorsApy".to_string(), json!({}))
        .await
        .unwrap();
