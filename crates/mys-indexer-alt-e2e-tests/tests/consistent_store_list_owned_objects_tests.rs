...
use mys_indexer_alt_consistent_api::proto::rpc::consistent::v1alpha::ListOwnedObjectsRequest;
use mys_indexer_alt_consistent_api::proto::rpc::consistent::v1alpha::Owner;
use mys_indexer_alt_consistent_api::proto::rpc::consistent::v1alpha::consistent_service_client::ConsistentServiceClient;
use mys_indexer_alt_consistent_api::proto::rpc::consistent::v1alpha::owner::OwnerKind;
use mys_types::MYS_FRAMEWORK_PACKAGE_ID;
use mys_types::TypeTag;
use mys_types::base_types::FullObjectRef;
...
async fn test_coin_balance_change_cleanup() {
    ...
    builder.transfer_mys(a, Some(500));
...
fn create_bag(cluster: &mut FullCluster, owner: MysAddress, ty: TypeTag, size: u64) -> ObjectRef {
    let (sender, kp, gas) = cluster
...
/// owned by `owner` with `size` many elements. The purpose of this is to create an object that
/// isn't a coin. `ty` can be any numeric Move type.
fn create_table(cluster: &mut FullCluster, owner: MysAddress, ty: TypeTag, size: u64) -> ObjectRef {
    let (sender, kp, gas) = cluster
        .funded_account(DEFAULT_GAS_BUDGET)
        .expect("Failed to fund account");
