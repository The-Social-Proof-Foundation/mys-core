...
async fn test_execute_transaction_mutation_schema() {
    ...
    let signed_tx =
        make_transfer_mys_transaction(&validator_cluster.wallet, Some(recipient), Some(1_000_000))
            .await;
...
async fn test_execute_transaction_grpc_errors() {
    ...
    let recipient1 = MysAddress::random_for_testing_only();
    let recipient2 = MysAddress::random_for_testing_only();

    let signed_tx1 =
        make_transfer_mys_transaction(&validator_cluster.wallet, Some(recipient1), Some(1_000_000))
            .await;
    let signed_tx2 =
        make_transfer_mys_transaction(&validator_cluster.wallet, Some(recipient2), Some(2_000_000))
            .await;
...
async fn test_execute_transaction_unchanged_consensus_objects() {
    ...
    let tx_data = {
        ...
        let clock_arg = ptb
            .obj(ObjectArg::SharedObject {
                id: mys_types::MYS_CLOCK_OBJECT_ID,
                initial_shared_version: mys_types::base_types::SequenceNumber::from_u64(1),
                mutability: SharedObjectMutability::Immutable,
            })
            .unwrap();

        ptb.programmable_move_call(
            mys_types::MYS_FRAMEWORK_PACKAGE_ID,
            "clock".parse().unwrap(),
    ...
    assert_eq!(object.address, mys_types::MYS_CLOCK_OBJECT_ID.to_string());
...
async fn test_execute_transaction_object_changes_input_output() {
    ...
    let output_type = output_move_obj
        ...
        .as_str()
        .unwrap();
    let mys_coin_type = GasCoin::type_().to_canonical_string(true);

    // Gas coin versions: 1 → 2 (modified for gas payment)
    assert_eq!(input_state.version, 1);
    assert_eq!(output_state.version, 2);

    // Both should be MYS coins
    assert_eq!(input_type, mys_coin_type);
    assert_eq!(output_type, mys_coin_type);
    ...
    // Created object should be MYS coins
    ...
    let created_type = created_move_obj
        ...
        .as_str()
        .unwrap();
    assert_eq!(created_type, mys_coin_type);
}
