...
async fn test_simulate_transaction_command_results() {
    ...
    let gas_objects = validator_cluster
        ...
        .await
        .unwrap();
    let tx_data = mys_types::transaction::TransactionData::new_programmable(
...
async fn test_simulate_transaction_balance_changes() {
    ...
    let signed_tx = make_transfer_mys_transaction(
...
async fn test_simulate_transaction_with_gas_selection() {
    ...
    let tx_data: mys_types::transaction::TransactionData = bcs::from_bytes(&tx_bytes).unwrap();
