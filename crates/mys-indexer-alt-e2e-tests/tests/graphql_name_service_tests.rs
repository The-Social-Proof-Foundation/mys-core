...
use mys_indexer_alt_graphql::config::RpcConfig;
use mys_indexer_alt_jsonrpc::config::NameServiceConfig;
...
use mys_indexer_alt_e2e_tests::FullCluster;
use mys_indexer_alt_e2e_tests::OffchainClusterConfig;
use mys_indexer_alt_e2e_tests::find;

/// 5 MYS gas budget
const DEFAULT_GAS_BUDGET: u64 = 5_000_000_000;

/// Tests happy path for MysNS resolution (forward lookup).
...
/// Tests MysNS resolution response when the domain doesn't exist or has expired.
...
async fn test_resolve_domain() {
    let mut c = MysNSCluster::new().await;

    let nft = ObjectID::random();
    let target = MysAddress::random_for_testing_only();
    c.add_domain(nft, &["mys", "foo"], Some(target), 1000)
        .await
        .expect("Failed to add domain");

    c.cluster.create_checkpoint().await;

    let resp = c.resolve_address("foo.myso").await.unwrap();
    ...
    assert_reverse!("foo.myso", c.resolve_name(target).await.unwrap());
}
...
async fn test_resolve_domain_no_target() {
    ...
    let nft = ObjectID::random();
    c.add_domain(nft, &["mys", "foo"], None, 1000)
        .await
        .expect("Failed to add domain");
...
async fn test_resolve_domain_expiry() {
    ...
    let target = MysAddress::random_for_testing_only();
    let expiry_ms = 1000;
    c.add_domain(nft, &["mys", "foo"], Some(target), expiry_ms)
        .await
        .expect("Failed to add domain");

    c.cluster.create_checkpoint().await;

    assert_resolved!(target, c.resolve_address("foo.myso").await.unwrap());
    assert_reverse!("foo.myso", c.resolve_name(target).await.unwrap());
    ...
    assert_not_resolved!(c.resolve_address("foo.myso").await.unwrap());
...
async fn test_resolve_subdomain() {
    ...
    c.add_domain(nft, &["mys", "foo", "bar"], Some(target), 0)
        .await
        .expect("Failed to add subdomain");

    c.cluster.create_checkpoint().await;

    let resp = c.resolve_address("bar.foo.myso").await.unwrap();
    assert_resolved!(target, &resp);
    ...
    assert_reverse!("bar.foo.myso", c.resolve_name(target).await.unwrap());
}
...
async fn test_resolve_subdomain_parent_expiry() {
    ...
    c.add_domain(nft, &["mys", "foo"], None, expiry_ms)
        .await
        .expect("Failed to add parent domain");
    ...
    assert_resolved!(target, c.resolve_address("bar.foo.myso").await.unwrap());
    assert_reverse!("bar.foo.myso", c.resolve_name(target).await.unwrap());

    c.cluster.advance_clock(Duration::from_millis(expiry_ms));
    c.cluster.create_checkpoint().await;

    assert_not_resolved!(c.resolve_address("bar.foo.myso").await.unwrap());
    assert_no_reverse!(c.resolve_name(target).await.unwrap());
}
...
async fn test_resolve_subdomain_expiry() {
    ...
    c.add_domain(parent_nft, &["mys", "foo"], None, parent_expiry_ms)
        .await
        .expect("Failed to add parent domain");

    c.add_domain(nft, &["mys", "foo", "bar"], Some(target), expiry_ms)
        .await
        .expect("Failed to add subdomain");
...
async fn test_resolve_subdomain_bad_parent() {
    ...
    c.add_domain(nft0, &["mys", "foo"], None, 1000)
        .await
        .expect("Failed to add parent domain");

    c.add_domain(nft1, &["mys", "foo", "bar"], Some(target), 0)
        .await
        .expect("Failed to add subdomain");
...
/// Test looking up an address by its MysNS name via Query.address(name: ...).
#[tokio::test]
async fn test_address_by_name() {
    ...
    assert_address_by_name!(target, c.address_by_name("foo.myso").await.unwrap());
...
async fn test_address_by_name_no_target() {
    ...
    assert_no_address_by_name!(c.address_by_name("foo.myso").await.unwrap());
}
...
async fn test_address_both_identifiers_error() {
    ...
    let resp = c
        .query(
            r#"
            query($address: MysAddress!, $name: String!) {
...
impl MysNSCluster {
    /// Sets up a full cluster with a mock MysNS package and registries. GraphQL is configured to read
    /// from these packages and registries to resolve MysNS names.
    ///
    ...
    async fn new() -> Self {
        ...
        // (2) Compile the mock MysNS package.
        let mut path = PathBuf::from(env!("CARGO_MANIFEST_DIR"));
        path.extend(["packages", "mysns"]);
        ...
        // (4) Publish the mock MysNS package.
        ...
        // (7) Configure the GraphQL RPC to read from the mock MysNS package.
    ...
    /// Send a GraphQL request to the cluster to resolve the given MysNS name.
    ...
    /// Send a GraphQL request to the cluster to resolve the MysNS name for a given address.
    async fn resolve_name(&self, addr: MysAddress) -> anyhow::Result<Value> {
        self.query(
            r#"
            query($address: MysAddress!) {
    ...
    /// Send a GraphQL request to resolve an address by its MysNS name.
