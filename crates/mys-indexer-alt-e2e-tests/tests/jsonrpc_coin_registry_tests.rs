...
use mys_types::MYS_COIN_REGISTRY_ADDRESS;
...
use mys_types::deny_list_v2::DenyCapV2;
use mys_types::effects::TransactionEffectsAPI;

use mys_indexer_alt_e2e_tests::FullCluster;
use mys_indexer_alt_e2e_tests::coin_registry::LegacyCoinOutputs;
use mys_indexer_alt_e2e_tests::coin_registry::{self};
...
#[tokio::test]
async fn test_mys() {
    // MYS coin is available from genesis, no need to publish
    let mut cluster = FullCluster::new().await.unwrap();
    cluster.create_checkpoint().await;

    let metadata = query_metadata(&cluster, "0x2::mys::MYS").await;
    assert_debug_snapshot!(metadata, @r###"
    CoinMetadataResponse {
        name: "MySocial",
        decimals: 9,
        description: "The official native token of the MySocial blockchain.",
        icon_url: None,
        symbol: "MySo",
    }
    "###);
...
async fn test_fixed_supply() {
    ...
    let currency = find::address_owned_by(&fx, MYS_COIN_REGISTRY_ADDRESS.into()).unwrap();
...
async fn query_owned_outputs(cluster: &FullCluster, owner: MysAddress) -> LegacyCoinOutputs {
...
async fn query_objects(cluster: &FullCluster, owner: MysAddress) -> Vec<(StructTag, ObjectRef)> {
    #[derive(Deserialize)]
    #[serde(rename_all = "camelCase")]
    struct Response {
        result: Page,
