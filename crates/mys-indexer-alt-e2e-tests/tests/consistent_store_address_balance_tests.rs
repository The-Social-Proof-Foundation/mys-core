...
use mys_indexer_alt_consistent_api::proto::rpc::consistent::v1alpha::BatchGetBalancesRequest;
use mys_indexer_alt_consistent_api::proto::rpc::consistent::v1alpha::GetBalanceRequest;
use mys_indexer_alt_consistent_api::proto::rpc::consistent::v1alpha::ListBalancesRequest;
use mys_indexer_alt_consistent_api::proto::rpc::consistent::v1alpha::ListObjectsByTypeRequest;
use mys_indexer_alt_consistent_api::proto::rpc::consistent::v1alpha::consistent_service_client::ConsistentServiceClient;
use mys_test_transaction_builder::FundSource;
...
async fn test_index_address_balance_accumulates() {
    ...
    cluster.send_mys_to_address_balance(a, 100);
    cluster.send_mys_to_address_balance(a, 200);
    cluster.send_mys_to_address_balance(a, 300);

    // Send to B's address balance
    cluster.send_mys_to_address_balance(b, 400);
    cluster.send_mys_to_address_balance(b, 500);
...
async fn test_multiple_coin_types() {
    ...
    cluster.send_mys_to_address_balance(recipient, 500);
    cluster.cluster.create_checkpoint().await;
    ...
    // Recipient should have both MYS and MY_COIN address balances
    assert_eq!(
        cluster
...
async fn test_snapshot_consistency() {
    ...
    cluster.send_mys_to_address_balance(a, 100);
    cluster.send_mys_to_address_balance(a, 200);
    cluster.send_mys_to_address_balance(b, 300);
    cluster.cluster.create_checkpoint().await;
    ...
    cluster.send_mys_to_address_balance(a, 400);
    cluster.send_mys_to_address_balance(b, 500);
    cluster.cluster.create_checkpoint().await;
...
async fn test_address_to_address_transfer() {
    ...
    let (balances, _) = cluster
        ...
        .await
        .unwrap();
    // Single balance entry for MYS/ gas
...
async fn test_edge_cases() {
    ...
    request
        .metadata_mut()
        .insert("x-mys-checkpoint", "10".parse().unwrap());
...
impl BalanceCluster {
    ...
    fn request_gas(&mut self, requester: MysAddress, amount: u64) -> ObjectRef {
    ...
    /// Publisher mints MYS to send to a recipient's address balance.
    fn send_mys_to_address_balance(&mut self, recipient: MysAddress, amount: u64) {
        ...
        let tx =
            TestTransactionBuilder::new(self.publisher, gas, self.cluster.reference_gas_price())
                .with_gas_budget(DEFAULT_GAS_BUDGET)
                .transfer_mys_to_address_balance(FundSource::coin(gas), vec![(amount, recipient)])
                .build();
    ...
    async fn list_balances(
        ...
        if let Some(checkpoint) = checkpoint {
            request
                .metadata_mut()
                .insert("x-mys-checkpoint", checkpoint.to_string().parse().unwrap());
        }
    ...
    async fn batch_get_balances(
        &mut self,
        requests: Vec<(MysAddress, String)>,
        checkpoint: Option<u64>,
...
async fn paginate_list_balances(
    ...
    mys_indexer_alt_consistent_api::proto::rpc::consistent::v1alpha::ListBalancesResponse,
    tonic::Status,
> {
