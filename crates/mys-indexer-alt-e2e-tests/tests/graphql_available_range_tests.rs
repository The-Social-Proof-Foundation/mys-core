...
use mys_indexer_alt_graphql::config::WatermarkConfig;
...
use mys_indexer_alt_e2e_tests::find::address_owned;
...
fn transfer_dust(
    ...
    let mut builder = ProgrammableTransactionBuilder::new();
    builder.transfer_mys(recipient, Some(1));
...
async fn query_transactions(cluster: &FullCluster, affected_address: MysAddress) -> Value {
    execute_graphql_query(
        cluster,
        TRANSACTIONS_QUERY,
        Some(json!({
