...
use mys_indexer_alt_jsonrpc::config::RpcConfig as JsonRpcConfig;
...
/// Tests happy path for MysNS resolution.
...
/// Tests MysNS resolution response when the domain has expired.
...
async fn test_resolve_domain_expiry() {
    ...
    assert_invalid_params!(c.resolve_address("foo.myso").await.unwrap());
...
async fn test_resolve_subdomain_parent_expiry() {
    ...
    assert_invalid_params!(c.resolve_address("bar.foo.myso").await.unwrap());
    assert_no_reverse!(c.resolve_name(target).await.unwrap());
}
...
impl MysNSCluster {
    /// Sets up a full cluster with a mock MysNS package and registries. RPC is configured to read
    ...
    async fn new() -> Self {
        ...
        // (7) Configure the RPC to read from the mock MysNS package. Everything else is configured
    ...
    /// Send a JSON-RPC request to the cluster to resolve the given MysNS name.
    ...
    /// Send a JSON-RPC request to the cluster to resolve the MysNS name for a given address.
