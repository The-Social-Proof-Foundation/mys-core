...
async fn test_aggregation() {
    ...
    // Create multiple MYS coins for address A with different amounts
    create_coin(&mut cluster, a, 1);
    create_coin(&mut cluster, a, 2);
    create_coin(&mut cluster, a, 3);

    // Create MYS coins for address B
...
async fn test_multiple_coin_types() {
    ...
    let mys_balance = DEFAULT_GAS_BUDGET as i64 - fx.gas_cost_summary().net_gas_usage();
    ...
    // P's balances should include MYS (left over from the gas coin) and MY_COIN (1000 + 200 + 30)
    // = 1230 from minting during publish and init.
    let mut balances = vec![
        (gas_type.clone(), mys_balance as u64),
    ...
    assert_eq!(
        ...
        vec![
            ...
            (p.to_string(), gas_type.clone(), mys_balance as u64),
        ]
    );
}
...
async fn test_transfers() {
    ...
    builder.transfer_mys(b, Some(1000));
...
/// `amount` to `owner`.
fn create_coin(cluster: &mut FullCluster, owner: MysAddress, amount: u64) -> ObjectRef {
    let (sender, kp, gas) = cluster
        .funded_account(DEFAULT_GAS_BUDGET + amount)
        .expect("Failed to fund account");

    let mut builder = ProgrammableTransactionBuilder::new();
    builder.transfer_mys(owner, Some(amount));
