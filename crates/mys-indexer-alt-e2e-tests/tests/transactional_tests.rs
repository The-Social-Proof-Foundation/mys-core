...
use mys_transactional_test_runner::create_adapter;
use mys_transactional_test_runner::offchain_state::OffchainStateReader;
use mys_transactional_test_runner::offchain_state::TestResponse;
use mys_transactional_test_runner::run_tasks_with_adapter;
use mys_transactional_test_runner::test_adapter::OffChainConfig;
use mys_transactional_test_runner::test_adapter::PRE_COMPILED;
use mys_transactional_test_runner::test_adapter::MysTestAdapter;
use tokio::join;
...
impl OffchainStateReader for OffchainReader {
    ...
    async fn execute_graphql(
        ...
        if show_usage {
            request = request.header(HeaderName::from_static("x-mys-rpc-show-usage"), "true");
        }
...
async fn run_test(path: &Path) -> Result<(), Box<dyn Error>> {
    ...
    let (output, mut adapter) =
        create_adapter::<MysTestAdapter>(path, Some(Arc::new(PRE_COMPILED.clone()))).await?;
