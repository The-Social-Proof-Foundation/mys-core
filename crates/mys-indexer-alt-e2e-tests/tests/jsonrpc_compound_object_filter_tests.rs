...
use mys_indexer_alt_jsonrpc::config::ObjectsConfig;
...
/// Deserialized successful JSON-RPC response for `mysx_getOwnedObjects`.
#[derive(Deserialize)]
struct Response {
...
/// `amount` to `owner`.
fn create_coin(cluster: &mut FullCluster, owner: MysAddress, amount: u64) -> ObjectID {
...
/// isn't a coin.
fn create_bag(cluster: &mut FullCluster, owner: MysAddress, size: u64) -> ObjectID {
    let (sender, kp, gas) = cluster
...
/// Make a call to `mysx_getOwnedObjects` to the RPC server running on `cluster`.
