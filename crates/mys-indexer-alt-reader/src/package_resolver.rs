...
use mys_package_resolver::PackageStoreWithLruCache;
use mys_package_resolver::Result;
use mys_package_resolver::error::Error;
...
impl Loader<PackageKey> for PgReader {
    ...
    async fn load(&self, keys: &[PackageKey]) -> std::result::Result<HashMap<PackageKey, Arc<Package>>, Self::Error> {
