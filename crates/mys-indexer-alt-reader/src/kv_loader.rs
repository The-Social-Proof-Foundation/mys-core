...
use bcs;
use mys_indexer_alt_schema::transactions::StoredTransaction;
use mys_kvstore::TransactionData as KVTransactionData;
use mys_kvstore::TransactionEventsData as KVTransactionEventsData;
use mys_rpc::proto::mys::rpc::v2 as grpc;
use mys_types::base_types::ObjectID;
use mys_types::crypto::AuthorityQuorumSignInfo;
use mys_types::digests::TransactionDigest;
use mys_types::digests::TransactionEffectsDigest;
...
use mys_types::messages_checkpoint::CheckpointContents;
use mys_types::messages_checkpoint::CheckpointSummary;
...
impl TransactionContents {
    ...
    /// For other sources, converts native effects to proto by setting the bcs field.
    pub fn proto_effects(&self) -> anyhow::Result<grpc::TransactionEffects> {
        match self {
            Self::ExecutedTransaction { proto_effects, .. } => {
                // Use cached proto if available, otherwise construct from BCS
                if let Some(proto) = proto_effects {
                    Ok(proto.clone())
                } else {
                    // proto_effects should always be cached for ExecutedTransaction
                    Err(anyhow::anyhow!("proto_effects not cached for ExecutedTransaction"))
                }
            }
            _ => {
                // For non-ExecutedTransaction sources, proto_effects is not available
                // This is expected as proto_effects is only available from gRPC ExecutedTransaction
                Err(anyhow::anyhow!("proto_effects only available for ExecutedTransaction from gRPC"))
            }
        }
    }
    ...
    /// For other sources, converts native transaction to proto by setting the bcs field.
        ...
        match self {
            Self::ExecutedTransaction {
                ...
                if let Some(proto) = proto_transaction {
                    ...
                    // proto_transaction should always be cached for ExecutedTransaction
                    Err(anyhow::anyhow!("proto_transaction not cached for ExecutedTransaction"))
                }
            }
            _ => {
                // For non-ExecutedTransaction sources, proto_transaction is not available
                // This is expected as proto_transaction is only available from gRPC ExecutedTransaction
                Err(anyhow::anyhow!("proto_transaction only available for ExecutedTransaction from gRPC"))
            }
        }
    }
