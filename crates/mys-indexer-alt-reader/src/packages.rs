...
use mys_indexer_alt_schema::packages::StoredPackageOriginalId;
...
impl Loader<PackageOriginalIdKey> for PgReader {
    ...
    ) -> Result<HashMap<PackageOriginalIdKey, StoredPackageOriginalId>, Self::Error> {
        if keys.is_empty() {
...
impl Loader<CheckpointBoundedOriginalPackageKey> for PgReader {
    ...
    ) -> Result<HashMap<CheckpointBoundedOriginalPackageKey, StoredPackage>, Self::Error> {
        if keys.is_empty() {
            return Ok(HashMap::new());
        }
...
impl Loader<VersionedOriginalPackageKey> for PgReader {
    ...
    ) -> Result<HashMap<VersionedOriginalPackageKey, StoredPackage>, Self::Error> {
        if keys.is_empty() {
            return Ok(HashMap::new());
        }
