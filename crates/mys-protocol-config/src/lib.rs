...
// Version 3: gas model v2, including all mys conservation fixes. Fix for loaded child object
...
//             Add `mys::table_vec::drop` to the framework via a system package upgrade.
...
// Version 19: Changes to mys-system package to enable liquid staking.
...
// Version 25: Add mys::table_vec::swap and mys::table_vec::swap_remove to system packages.
// Version 26: New gas model version.
//             Add support for receiving objects off of other objects in devnet only.
// Version 28: Add mys::zklogin::verify_zklogin_id and related functions to mys framework.
//             Enable transaction effects v2 in devnet.
// Version 29: Add verify_legacy_zklogin_address flag to mys framework, this add ability to verify
...
//             Add support for getting object ID referenced by receiving object in mys framework.
//             Create new execution layer version, and preserve previous behavior in v1.
//             Update semantics of `mys::transfer::receive` and add `mys::transfer::public_receive`.
...
// Version 42: Migrate mys framework and related code to Move 2024
...
//             Modified mys-system package to enable withdrawal of stake before it becomes active.
//             Enable soft bundle in devnet and testnet.
//             Core macro visibility in mys core framework.
...
//             Max gas budget moved to 50_000 MYS
//             Max gas price moved to 50 MYS
...
// Version 74: Enable load_nitro_attestation move function in mys framework in devnet.
...
pub struct ProtocolConfig {
    ...
    /// Max size of objects a transaction can write to disk after completion. Enforce by the Mys adapter.
    ...
    /// Max size of objects a system transaction can write to disk after completion. Enforce by the Mys adapter.
    ...
    /// Maximum size of the `contents` part of an object, in bytes. Enforced by the Mys adapter when effects are produced.
    ...
    /// Maximum size of a Move package object, in bytes. Enforced by the Mys adapter at the end of a publish transaction.
    ...
    // === Execution gas costs ====
    /// Base cost for any Mys transaction
    ...
    /// Per-byte cost of storing an object in the Mys global object store. Some of this cost may be refundable if the object is later freed
    obj_data_cost_refundable: Option<u64>,

    // Per-byte cost of storing an object in the Mys transaction log (e.g., in CertifiedTransactionEffects)
...
impl ProtocolConfig {
    ...
    pub fn get_for_version(version: ProtocolVersion, chain: Chain) -> Self {
        ...
        if std::env::var("MYS_PROTOCOL_CONFIG_OVERRIDE_ENABLE").is_ok() {
            ...
            let overrides: ProtocolConfigOptional =
                serde_env::from_env_with_prefix("MYS_PROTOCOL_CONFIG_OVERRIDE")
        ...
        for cur in 2..=version.0 {
            match cur {
                ...
                3 => {
                    ...
                    // max gas budget is in MIST and an absolute value 50MYS
                    cfg.max_tx_gas = Some(50_000_000_000);
                    // min gas budget is in MIST and an absolute value 2000MIST or 0.000002MYS
                ...
                72 => {
                    ...
                    // max gas budget is in MIST and an absolute value 50_000 MYS
                    cfg.max_tx_gas = Some(50_000_000_000_000);
                    // max gas price is in MIST and an absolute value 50 MYS
...
macro_rules! check_limit_by_meter {
    ($is_metered:expr, $x:expr, $metered_limit:expr, $unmetered_hard_limit:expr, $metric:expr) => {{
        ...
        use mys_protocol_config::check_limit_in_range;
