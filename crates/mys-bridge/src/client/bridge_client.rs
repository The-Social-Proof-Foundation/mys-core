...
impl BridgeClient {
    ...
    fn bridge_action_to_path(event: &BridgeAction) -> String {
        match event {
            BridgeAction::MysToEthBridgeAction(e) => format!(
                "sign/bridge_tx/mys/eth/{}/{}",
                e.mys_tx_digest, e.mys_tx_event_index
            ),
            BridgeAction::EthToMysBridgeAction(e) => format!(
                "sign/bridge_tx/eth/mys/{}/{}",
...
mod tests {
    ...
    async fn test_bridge_client() {
        ...
        let action =
            get_test_mys_to_eth_bridge_action(None, Some(1), Some(1), Some(100), None, None, None);
        ...
        // Ok
        authority.base_url = "https://foo.mysbridge.io".to_string();
        let committee = Arc::new(BridgeCommittee::new(vec![authority.clone()]).unwrap());
    ...
    async fn test_bridge_client_request_sign_action() {
        ...
        mock_handler.add_mys_event_response(tx_digest, event_idx, Ok(signed_event.clone()), None);
        ...
        mock_handler.add_mys_event_response(tx_digest, event_idx, Ok(wrong_signed_action), None);
        let err = client
            .request_sign_bridge_action(action.clone())
            .await
            .unwrap_err();
        ...
        mock_handler.add_mys_event_response(tx_digest, event_idx, Ok(signed_event), None);
        ...
        mock_handler.add_mys_event_response(tx_digest, event_idx, Ok(signed_event2), None);
        let err = client
            .request_sign_bridge_action(action.clone())
            .await
            .unwrap_err();
        ...
        mock_handler.add_mys_event_response(tx_digest, event_idx, Ok(signed_event3), None);
        let err = client
            .request_sign_bridge_action(action.clone())
            .await
            .unwrap_err();
    ...
    fn test_bridge_action_path_regression_tests() {
        ...
        let mys_tx_event_index = 5;
        let action = BridgeAction::MysToEthBridgeAction(crate::types::MysToEthBridgeAction {
            mys_tx_digest,
            mys_tx_event_index,
        ...
        assert_eq!(
            ...
            format!(
                "sign/bridge_tx/mys/eth/{}/{}",
                mys_tx_digest, mys_tx_event_index
            )
        );

        let eth_tx_hash = TxHash::random();
        let eth_event_index = 6;
        let action = BridgeAction::EthToMysBridgeAction(crate::types::EthToMysBridgeAction {
            eth_tx_hash,
            eth_event_index,
        ...
        let action = BridgeAction::AddTokensOnMysAction(crate::types::AddTokensOnMysAction {
            nonce: 3,
            chain_id: BridgeChainId::MysCustom,
            native: false,
