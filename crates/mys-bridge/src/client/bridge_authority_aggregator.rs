...
impl GetSigsState {
    ...
    fn handle_verified_signed_action(
        ...
        if self.total_ok_stake >= self.validity_threshold {
            ...
            let certified_action: mys_types::message_envelope::Envelope<
                BridgeAction,
...
async fn request_sign_bridge_action_into_certification(
    ...
    let preference = match action {
        BridgeAction::MysToEthBridgeAction(_) => Some(SigRequestPrefs {
            ordering_pref: BTreeSet::new(),
            prefetch_timeout,
        }),
        BridgeAction::EthToMysBridgeAction(_) => None,
        _ => {
            if action.chain_id().is_mys_chain() {
                None
            } else {
...
mod tests {
    ...
    use mys_types::committee::VALIDITY_THRESHOLD;
