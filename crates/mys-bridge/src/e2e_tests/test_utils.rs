...
use crate::abi::{EthBridgeCommittee, EthBridgeEvent, EthERC20, EthMysBridge, EthMysBridgeEvents};
...
use crate::types::{BridgeAction, BridgeActionStatus, MysToEthBridgeAction};
...
use mys_types::bridge::get_bridge_obj_initial_shared_version;
use mys_types::bridge::BridgeChainId;
use mys_types::bridge::BridgeSummary;
use mys_types::bridge::BridgeTrait;
use mys_types::bridge::{get_bridge, BRIDGE_MODULE_NAME};
use mys_types::bridge::{TOKEN_ID_BTC, TOKEN_ID_ETH, TOKEN_ID_USDC, TOKEN_ID_USDT};
...
const MYS_BRIDGE_NAME: &str = "MysBridge";
...
impl BridgeTestClusterBuilder {
    pub fn new() -> Self {
        BridgeTestClusterBuilder {
            ...
            mys_chain_id: BridgeChainId::MysCustom,
        }
    }
    ...
    pub fn with_mys_chain_id(mut self, chain_id: BridgeChainId) -> Self {
        self.mys_chain_id = chain_id;
        self
    }
    ...
    pub async fn build(self) -> BridgeTestCluster {
        ...
        let bridge_client =
            MysBridgeClient::new(&test_cluster.inner.fullnode_handle.rpc_url, metrics)
                .await
                .unwrap();
        info!(
            "Bridge committee: {:?}",
        ...
        BridgeTestCluster {
            ...
            mys_chain_id: self.mys_chain_id,
...
impl BridgeTestCluster {
    ...
    pub fn bridge_client(&self) -> &MysBridgeClient {
        &self.bridge_client
    }

    pub fn mys_client(&self) -> &MysClient {
        &self.test_cluster.inner.fullnode_handle.mys_client
    }

    pub fn mys_user_address(&self) -> MysAddress {
        self.test_cluster.inner.get_address_0()
    }

    pub fn mys_chain_id(&self) -> BridgeChainId {
        self.mys_chain_id
    }
    ...
    pub fn mys_bridge_address(&self) -> String {
        self.eth_environment.contracts().mys_bridge_addrress_hex()
    }
    ...
    pub fn mys_rpc_url(&self) -> String {
    ...
    pub async fn new_bridge_transactions(
        &mut self,
        assert_success: bool,
    ) -> Vec<MysTransactionBlockResponse> {
        let resps = self
            .mys_client()
            .read_api()
...
impl DeployedSolContracts {
    ...
    pub fn mys_bridge_addrress_hex(&self) -> String {
        Self::eth_adress_to_hex(self.mys_bridge)
    }
}
...
pub(crate) async fn deploy_sol_contract(
    ...
    let deploy_config = SolDeployConfig {
        ...
        mys_decimals: vec![],     // this is set up in the deploy script
    ...
    let contracts = DeployedSolContracts {
        mys_bridge: deployed_contracts.remove(MYS_BRIDGE_NAME).unwrap(),
...
impl EthBridgeEnvironment {
    ...
    pub async fn get_supported_token(&self, token_id: u8) -> (EthAddress, u8, u64) {
        ...
        let token_mys_decimal = config.token_mys_decimal_of(token_id).call().await.unwrap();
        let token_price = config.token_price_of(token_id).call().await.unwrap();
        (token_address, token_mys_decimal, token_price)
    }
}
...
pub(crate) async fn start_bridge_cluster(
        ...
        .enumerate()
    {
        ...
        let config = BridgeNodeConfig {
            ...
            mys: MysConfig {
                mys_rpc_url: test_cluster.inner.fullnode_handle.rpc_url.clone(),
...
pub async fn get_signatures(
    ...
    let sigs = mys_bridge_client
        .get_token_transfer_action_onchain_signatures_until_success(mys_chain_id, nonce)
        .await
        .unwrap();
...
impl TestClusterWrapperBuilder {
    ...
    pub async fn build(self) -> TestClusterWrapper {
        ...
        let gas_objects_for_authority_keys = self
            ...
            .iter()
            .map(|k| {
                let address = MysAddress::from(k.public());
        ...
        for (node, kp) in reordered_nodes.zip(self.bridge_authority_keys.iter()) {
            let validator_address = node.config().mys_address();
        ...
        if self.deploy_tokens {
            ...
            let action = publish_and_register_coins_return_add_coins_on_mys_action(
                test_cluster.wallet(),
                bridge_arg,
                vec![
        ...
        async fn await_committee_register_tasks(
            ...
            tasks: Vec<
                impl Future<Output = Result<MysTransactionBlockResponse, mys_sdk::error::Error>>,
            >,
        ) {
            ...
            for response in responses {
                if response.unwrap().effects.unwrap().status() != &MysExecutionStatus::Success {
                    has_failure = true;
                }
            }
            if has_failure {
...
pub async fn initiate_bridge_eth_to_mys(
    ...
    let mys_chain_id = bridge_test_cluster.mys_chain_id();
    let eth_chain_id = bridge_test_cluster.eth_chain_id();
    let token_id = TOKEN_ID_ETH;

    let mys_amount = (U256::from(amount) * U256::exp10(8)).as_u64(); // DP for Ether on Mys
    ...
    let eth_bridge_event = tx_receipt
        ...
        .unwrap();
    let EthBridgeEvent::EthMysBridgeEvents(EthMysBridgeEvents::TokensDepositedFilter(
        eth_bridge_event,
    )) = eth_bridge_event
    else {
        unreachable!();
    ...
    assert_eq!(eth_bridge_event.destination_chain_id, mys_chain_id as u8);
    assert_eq!(eth_bridge_event.token_id, token_id);
    assert_eq!(eth_bridge_event.mys_adjusted_amount, mys_amount);
    assert_eq!(eth_bridge_event.sender_address, eth_address);
    assert_eq!(eth_bridge_event.recipient_address, mys_address.to_vec());
    info!(
    ...
    .await
    .tap_ok(|_| {
        info!("Eth to Mys bridge transfer claimed");
    })
}

pub async fn initiate_bridge_mys_to_eth(
    bridge_test_cluster: &BridgeTestCluster,
    eth_address: EthAddress,
    token: ObjectRef,
    nonce: u64,
    mys_amount: u64,
) -> Result<MysToEthBridgeAction, anyhow::Error> {
    let bridge_object_arg = bridge_test_cluster
        .bridge_client()
        .get_mutable_bridge_object_arg_must_succeed()
        .await;
    let mys_client = bridge_test_cluster.mys_client();
    ...
    let resp = match deposit_eth_to_mys_package(
        mys_client,
        mys_address,
    ...
    .await
    {
        Ok(resp) => {
            if !resp.status_ok().unwrap() {
                return Err(anyhow!("Mys TX error"));
            } else {
                resp
            }
        }
    ...
    let mys_events = resp.events.unwrap().data;
    let bridge_event = mys_events
        .iter()
        .filter_map(|e| {
            let mys_bridge_event = MysBridgeEvent::try_from_mys_event(e).unwrap()?;
            mys_bridge_event.try_into_bridge_action(e.id.tx_digest, e.id.event_seq as u16)
        })
        .find_map(|e| {
            if let BridgeAction::MysToEthBridgeAction(a) = e {
                Some(a)
            } else {
                None
            }
    ...
    assert_eq!(bridge_event.mys_bridge_event.nonce, nonce);
    assert_eq!(
        bridge_event.mys_bridge_event.mys_chain_id,
        bridge_test_cluster.mys_chain_id()
    );
    assert_eq!(
        bridge_event.mys_bridge_event.eth_chain_id,
        bridge_test_cluster.eth_chain_id()
    );
    assert_eq!(bridge_event.mys_bridge_event.mys_address, mys_address);
    assert_eq!(bridge_event.mys_bridge_event.eth_address, eth_address);
    assert_eq!(bridge_event.mys_bridge_event.token_id, TOKEN_ID_ETH);
    assert_eq!(
        bridge_event.mys_bridge_event.amount_mys_adjusted,
        mys_amount
    );
    ...
    .await
    .unwrap();
    info!("Mys to Eth bridge transfer approved.");
...
async fn deposit_eth_to_mys_package(
    ...
    mys_token_type_tags: &HashMap<u8, TypeTag>,
) -> Result<MysTransactionBlockResponse, anyhow::Error> {
    ...
    builder.programmable_move_call(
        ...
        vec![mys_token_type_tags.get(&TOKEN_ID_ETH).unwrap().clone()],
...
pub async fn initiate_bridge_erc20_to_mys(
    bridge_test_cluster: &BridgeTestCluster,
    amount_u64: u64,
    ...
    let allowance_call = contract.approve(bridge_test_cluster.contracts().mys_bridge, amount);
    ...
    let mys_recipient_address = bridge_test_cluster.mys_user_address();
    let mys_chain_id = bridge_test_cluster.mys_chain_id();
    let eth_chain_id = bridge_test_cluster.eth_chain_id();
    ...
    let deposit_call = contract.bridge_erc20(
        token_id,
        amount,
        mys_recipient_address.to_vec().into(),
        mys_chain_id as u8,
    );
    ...
    .tap_ok(|_| {
        info!(
            nonce,
            token_id, amount_u64, "Eth to Mys bridge transfer claimed"
        );
    })
}
...
pub(crate) async fn deposit_native_eth_to_sol_contract(
    ...
    let contract = EthMysBridge::new(contract_address, signer.clone().into());
    let mys_recipient_address = mys_recipient_address.to_vec().into();
    let amount = U256::from(amount) * U256::exp10(18); // 1 ETH
    contract
        .bridge_eth(mys_recipient_address, mys_chain_id as u8)
        .value(amount)
}
