...
//! on Mys blockchain from concerned modules of bridge package 0x9.
...
const MYS_EVENTS_CHANNEL_SIZE: usize = 1000;

/// Map from contract address to their start cursor (exclusive)
pub type MysTargetModules = HashMap<Identifier, Option<EventID>>;

pub struct MysSyncer<C> {
    ...
    C: MysClientInner + 'static,
{
    ...
    pub async fn run(
        ...
        let (events_tx, events_rx) = mysten_metrics::metered_channel::channel(
            MYS_EVENTS_CHANNEL_SIZE,
            &mysten_metrics::get_metrics()
                .unwrap()
                .channel_inflight
                .with_label_values(&["mys_events_queue"]),
        );
        ...
        for (module, cursor) in self.cursors {
            ...
            let mys_client_clone = self.mys_client.clone();
    ...
    async fn run_event_listening_task(
        ...
        events_sender: mysten_metrics::metered_channel::Sender<(Identifier, Vec<MysEvent>)>,
        mys_client: Arc<MysClient<C>>,
        query_interval: Duration,
        metrics: Arc<BridgeMetrics>,
    ) {
        tracing::info!(?module, ?cursor, "Starting mys events listening task");
        ...
        let last_synced_mys_checkpoints_metric = metrics
            .last_synced_mys_checkpoints
            .with_label_values(&[&module.to_string()]);
        spawn_logged_monitored_task!(async move {
            loop {
                notify_clone.notified().await;
                let Ok(Ok(latest_checkpoint_sequence_number)) = retry_with_max_elapsed_time!(
                    mys_client_clone.get_latest_checkpoint_sequence_number(),
                    Duration::from_secs(120)
                ) else {
                    tracing::error!("Failed to query latest checkpoint sequence number from mys client after retry");
                    continue;
                };
                last_synced_mys_checkpoints_metric.set(latest_checkpoint_sequence_number as i64);
            }
        });
        ...
        loop {
            ...
            let Ok(Ok(events)) = retry_with_max_elapsed_time!(
                mys_client.query_events_by_module(BRIDGE_PACKAGE_ID, module.clone(), cursor),
                Duration::from_secs(120)
            ) else {
                tracing::error!("Failed to query events from mys client after retry");
                continue;
            };
            ...
            if len != 0 {
                ...
                events_sender
                    ...
                    .await
                    .expect("All Mys event channel receivers are closed");
                if let Some(next) = events.next_cursor {
                    cursor = Some(next);
                }
                tracing::info!(?module, ?cursor, "Observed {len} new Mys events");
            }
        }
    }
}
...
mod tests {
    ...
    #[tokio::test]
    async fn test_mys_syncer_basic() -> anyhow::Result<()> {
        telemetry_subscribers::init_for_testing();
        let registry = Registry::new();
        mysten_metrics::init_metrics(&registry);
        let metrics = Arc::new(BridgeMetrics::new(&registry));
        let mock = MysMockClient::default();
        let client = Arc::new(MysClient::new_for_testing(mock.clone()));
        ...
        let (_handles, mut events_rx) = MysSyncer::new(client, target_modules, metrics.clone())
            .run(interval)
            .await
            .unwrap();
        ...
        let mut event_1: MysEvent = MysEvent::random_for_testing();
        let package_id = BRIDGE_PACKAGE_ID;
        event_1.type_.address = package_id.into();
        event_1.type_.module = module_foo.clone();
        let module_foo_events_1: mys_json_rpc_types::Page<MysEvent, EventID> = EventPage {
        ...
        let mut event_2: MysEvent = MysEvent::random_for_testing();
    ...
    async fn assert_no_more_events(
        interval: Duration,
        events_rx: &mut mysten_metrics::metered_channel::Receiver<(Identifier, Vec<MysEvent>)>,
    ) {
