...
pub fn build_mys_transaction(
    ...
    // TODO: Check chain id?
    match action.data() {
        BridgeAction::EthToMysBridgeAction(_) => build_token_bridge_approve_transaction(
            client_address,
            gas_object_ref,
            action,
            true,
        ...
        BridgeAction::MysToEthBridgeAction(_) => build_token_bridge_approve_transaction(
            client_address,
            gas_object_ref,
            action,
            false,
        ...
        BridgeAction::EvmContractUpgradeAction(_) => {
            // It does not need a Mys tranaction to execute EVM contract upgrade
            unreachable!()
        }
        BridgeAction::AddTokensOnMysAction(_) => build_add_tokens_on_mys_transaction(
            client_address,
            gas_object_ref,
            action,
            bridge_object_arg,
        ...
        BridgeAction::AddTokensOnEvmAction(_) => {
            // It does not need a Mys tranaction to add tokens on EVM
            unreachable!()
        }
    }
}
...
fn build_token_bridge_approve_transaction(
    ...
    let (source_chain, seq_num, sender, target_chain, target, token_type, amount) =
        match bridge_action {
            BridgeAction::MysToEthBridgeAction(a) => {
                let bridge_event = a.mys_bridge_event;
                (
                    ...
                    bridge_event.amount_mys_adjusted,
                )
            }
...
pub fn build_add_tokens_on_mys_transaction(
    client_address: MysAddress,
    gas_object_ref: &ObjectRef,
    action: VerifiedCertifiedBridgeAction,
    bridge_object_arg: ObjectArg,
    ...
    let (source_chain, seq_num, native, token_ids, token_type_names, token_prices) =
        match bridge_action {
            BridgeAction::AddTokensOnMysAction(a) => (
                a.chain_id,
                a.nonce,
                a.native,
                a.token_ids,
    ...
    let message_arg = builder.programmable_move_call(
        ...
        BRIDGE_CREATE_ADD_TOKEN_ON_MYS_MESSAGE_FUNCTION_NAME.into(),
        vec![],
        vec![
            source_chain,
            seq_num,
...
pub fn build_committee_register_transaction(
    ...
    let mut builder = ProgrammableTransactionBuilder::new();
    let system_state = builder.obj(ObjectArg::MYS_SYSTEM_MUT).unwrap();
...
mod tests {
    ...
    async fn test_build_mys_transaction_for_token_transfer() {
        ...
        let mys_client = MysClient::new(&test_cluster.inner.fullnode_handle.rpc_url, metrics)
            .await
            .unwrap();
        let bridge_authority_keys = test_cluster.authority_keys_clone();

        // Note: We don't call `mys_client.get_bridge_committee` here because it will err if the committee
        ...
        // 1. Test Eth -> Mys Transfer approval
        ...
        // 2. Test Mys -> Eth Transfer approval
    ...
    #[tokio::test(flavor = "multi_thread", worker_threads = 8)]
    async fn test_build_mys_transaction_for_emergency_op() {
        telemetry_subscribers::init_for_testing();
        let num_valdiator = 2;
        let mut bridge_keys = vec![];
        ...
        test_cluster
            .trigger_reconfiguration_if_not_yet_and_assert_bridge_committee_initialized()
            .await;
        let summary = mys_client.get_bridge_summary().await.unwrap();
    ...
    #[tokio::test(flavor = "multi_thread", worker_threads = 8)]
    async fn test_build_mys_transaction_for_committee_blocklist() {
        telemetry_subscribers::init_for_testing();
        let mut bridge_keys = vec![];
        for _ in 0..=3 {
            let (_, kp): (_, BridgeAuthorityKeyPair) = get_key_pair();
        ...
        test_cluster
            .trigger_reconfiguration_if_not_yet_and_assert_bridge_committee_initialized()
            .await;
        let committee = mys_client.get_bridge_summary().await.unwrap().committee;
    ...
    #[tokio::test(flavor = "multi_thread", worker_threads = 8)]
    async fn test_build_mys_transaction_for_limit_update() {
        telemetry_subscribers::init_for_testing();
        let mut bridge_keys = vec![];
        for _ in 0..=3 {
            let (_, kp): (_, BridgeAuthorityKeyPair) = get_key_pair();
        ...
        .await;
        let new_transfer_limit = mys_client
            .get_bridge_summary()
            .await
            .unwrap()
            .limiter
        ...
        for limit in new_transfer_limit {
            if limit.0 == BridgeChainId::EthCustom && limit.1 == BridgeChainId::MysCustom {
    ...
    #[tokio::test(flavor = "multi_thread", worker_threads = 8)]
    async fn test_build_mys_transaction_for_price_update() {
        telemetry_subscribers::init_for_testing();
        let mut bridge_keys = vec![];
        for _ in 0..=3 {
            let (_, kp): (_, BridgeAuthorityKeyPair) = get_key_pair();
        ...
        test_cluster
            .trigger_reconfiguration_if_not_yet_and_assert_bridge_committee_initialized()
            .await;
        let notional_values = mys_client.get_notional_values().await.unwrap();
        ...
        .await;
        let new_notional_values = mys_client.get_notional_values().await.unwrap();
