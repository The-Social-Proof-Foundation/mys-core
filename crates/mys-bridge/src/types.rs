...
use mys_types::bridge::{
    ...
    APPROVAL_THRESHOLD_ADD_TOKENS_ON_MYS, BRIDGE_COMMITTEE_MAXIMAL_VOTING_POWER,
...
pub struct MysToEthBridgeAction {
    // Digest of the transaction where the event was emitted
    pub mys_tx_digest: TransactionDigest,
    // The index of the event in the transaction
    pub mys_tx_event_index: u16,
    pub mys_bridge_event: EmittedMysToEthTokenBridgeV1,
}

#[derive(Clone, Debug, Serialize, Deserialize, PartialEq, Eq, Hash)]
pub struct EthToMysBridgeAction {
    // Digest of the transaction where the event was emitted
    pub eth_tx_hash: EthTransactionHash,
    // The index of the event in the transaction
    pub eth_event_index: u16,
    pub eth_bridge_event: EthToMysTokenBridgeV1,
}
...
pub struct LimitUpdateAction {
    ...
    // for the limit update. For example, if chain_id is EthMainnet and sending_chain_id is MysMainnet,
    // it means we want to update the limit for the MysMainnet to EthMainnet route.
    pub chain_id: BridgeChainId,
...
#[derive(Clone, Debug, Serialize, Deserialize, PartialEq, Eq, Hash)]
pub struct AddTokensOnMysAction {
    pub nonce: u64,
    pub chain_id: BridgeChainId,
    pub native: bool,
...
pub enum BridgeAction {
    /// Mys to Eth bridge action
    MysToEthBridgeAction(MysToEthBridgeAction),
    /// Eth to mys bridge action
    EthToMysBridgeAction(EthToMysBridgeAction),
    ...
    AddTokensOnMysAction(AddTokensOnMysAction),
...
impl BridgeAction {
    ...
    pub fn chain_id(&self) -> BridgeChainId {
        match self {
            BridgeAction::MysToEthBridgeAction(a) => a.mys_bridge_event.mys_chain_id,
            BridgeAction::EthToMysBridgeAction(a) => a.eth_bridge_event.eth_chain_id,
            ...
            BridgeAction::AddTokensOnMysAction(a) => a.chain_id,
    ...
    pub fn is_governace_action(&self) -> bool {
        match self.action_type() {
            ...
            BridgeActionType::AddTokensOnMys => true,
    ...
    pub fn action_type(&self) -> BridgeActionType {
        match self {
            BridgeAction::MysToEthBridgeAction(_) => BridgeActionType::TokenTransfer,
            BridgeAction::EthToMysBridgeAction(_) => BridgeActionType::TokenTransfer,
            ...
            BridgeAction::AddTokensOnMysAction(_) => BridgeActionType::AddTokensOnMys,
    ...
    pub fn seq_number(&self) -> u64 {
        match self {
            BridgeAction::MysToEthBridgeAction(a) => a.mys_bridge_event.nonce,
            BridgeAction::EthToMysBridgeAction(a) => a.eth_bridge_event.nonce,
            ...
            BridgeAction::AddTokensOnMysAction(a) => a.nonce,
    ...
    pub fn approval_threshold(&self) -> u64 {
        match self {
            BridgeAction::MysToEthBridgeAction(_) => APPROVAL_THRESHOLD_TOKEN_TRANSFER,
            BridgeAction::EthToMysBridgeAction(_) => APPROVAL_THRESHOLD_TOKEN_TRANSFER,
            ...
            BridgeAction::AddTokensOnMysAction(_) => APPROVAL_THRESHOLD_ADD_TOKENS_ON_MYS,
...
pub fn is_route_valid(one: BridgeChainId, other: BridgeChainId) -> bool {
    if one.is_mys_chain() && other.is_mys_chain() {
        return false;
    }
    if !one.is_mys_chain() && !other.is_mys_chain() {
        return false;
    ...
    if one == BridgeChainId::EthMainnet {
        return other == BridgeChainId::MysMainnet;
    }
    if one == BridgeChainId::MysMainnet {
        return other == BridgeChainId::EthMainnet;
    }
    if other == BridgeChainId::EthMainnet {
        return one == BridgeChainId::MysMainnet;
    }
    if other == BridgeChainId::MysMainnet {
...
mod tests {
    ...
    use crate::test_utils::get_test_eth_to_mys_bridge_action;
    ...
    fn test_bridge_action_approval_threshold_regression_test() -> anyhow::Result<()> {
        let action = get_test_mys_to_eth_bridge_action(None, None, None, None, None, None, None);
        assert_eq!(action.approval_threshold(), 3334);

        let action = get_test_eth_to_mys_bridge_action(None, None, None, None);
        assert_eq!(action.approval_threshold(), 3334);
