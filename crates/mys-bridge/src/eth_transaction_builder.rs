...
use crate::abi::{
    eth_bridge_committee, eth_committee_upgradeable_contract, eth_mys_bridge, EthBridgeCommittee,
...
pub async fn build_eth_transaction(
    ...
    match action.data() {
        ...
        BridgeAction::AddTokensOnMysAction(_) => {
            unreachable!();
        }
...
pub async fn build_emergency_op_approve_transaction(
    ...
    let contract = EthMysBridge::new(contract_address, signer.into());

    let message: eth_mys_bridge::Message = action.clone().into();
    let signatures = sigs
        .signatures
        .values()
