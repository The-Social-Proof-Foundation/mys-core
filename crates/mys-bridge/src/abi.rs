...
use crate::types::{
    ...
    BridgeActionType, EmergencyAction, EthLog, EthToMysBridgeAction, EvmContractUpgradeAction,
    LimitUpdateAction, MysToEthBridgeAction,
};
...
#[rustfmt::skip]
gen_eth_events!(
    EthMysBridge, EthMysBridgeEvents, "abi/mys_bridge.json",
...
impl EthBridgeEvent {
    pub fn try_into_bridge_action(
        ...
        Ok(match self {
            EthBridgeEvent::EthMysBridgeEvents(event) => {
                match event {
                    EthMysBridgeEvents::TokensDepositedFilter(event) => {
                        let bridge_event = match EthToMysTokenBridgeV1::try_from(&event) {
                            Ok(bridge_event) => {
                                if bridge_event.mys_adjusted_amount == 0 {
                            ...
                            Err(e) => {
                                return Err(BridgeError::Generic(format!("Manual intervention is required. Failed to convert TokensDepositedFilter log to EthToMysTokenBridgeV1. This indicates incorrect parameters or a bug in the code: {:?}. Err: {:?}", event, e)));
                            }
                        };

                        Some(BridgeAction::EthToMysBridgeAction(EthToMysBridgeAction {
                            eth_tx_hash,
                            eth_event_index,
                            eth_bridge_event: bridge_event,
                        }))
                    }
                    EthMysBridgeEvents::TokensClaimedFilter(_event) => None,
                    EthMysBridgeEvents::PausedFilter(_event) => None,
                    EthMysBridgeEvents::UnpausedFilter(_event) => None,
                    EthMysBridgeEvents::UpgradedFilter(_event) => None,
                    EthMysBridgeEvents::InitializedFilter(_event) => None,
                    EthMysBridgeEvents::ContractUpgradedFilter(_event) => None,
                    EthMysBridgeEvents::EmergencyOperationFilter(_event) => None,
                }
            }
...
pub struct EthToMysTokenBridgeV1 {
    pub nonce: u64,
    ...
    pub token_id: u8,
    pub mys_adjusted_amount: u64,
}

impl TryFrom<&TokensDepositedFilter> for EthToMysTokenBridgeV1 {
    type Error = BridgeError;
    fn try_from(event: &TokensDepositedFilter) -> BridgeResult<Self> {
        Ok(Self {
            nonce: event.nonce,
            mys_chain_id: BridgeChainId::try_from(event.destination_chain_id)?,
            eth_chain_id: BridgeChainId::try_from(event.source_chain_id)?,
            mys_address: MysAddress::from_bytes(event.recipient_address.as_ref())?,
            eth_address: event.sender_address,
            token_id: event.token_id,
            mys_adjusted_amount: event.mys_adjusted_amount,
        })
    }
}
...
impl From<MysToEthBridgeAction> for eth_mys_bridge::Message {
    fn from(action: MysToEthBridgeAction) -> Self {
        eth_mys_bridge::Message {
            message_type: BridgeActionType::TokenTransfer as u8,
            version: TOKEN_TRANSFER_MESSAGE_VERSION,
            nonce: action.mys_bridge_event.nonce,
            chain_id: action.mys_bridge_event.mys_chain_id as u8,
            payload: action.as_payload_bytes().into(),
        }
    }
}

impl From<ParsedTokenTransferMessage> for eth_mys_bridge::Message {
...
impl From<EmergencyAction> for eth_mys_bridge::Message {
...
mod tests {
    ...
    fn test_eth_message_conversion_emergency_action_regression() -> anyhow::Result<()> {
        ...
        let message: eth_mys_bridge::Message = action.into();
        assert_eq!(
            message,
            eth_mys_bridge::Message {
                message_type: BridgeActionType::EmergencyButton as u8,
    ...
    fn test_eth_message_conversion_update_limit_action_regression() -> anyhow::Result<()> {
        ...
        let action = LimitUpdateAction {
            nonce: 2,
            chain_id: BridgeChainId::EthSepolia,
            sending_chain_id: BridgeChainId::MysTestnet,
    ...
    #[test]
    fn test_token_deposit_eth_log_to_mys_bridge_event_regression() -> anyhow::Result<()> {
        telemetry_subscribers::init_for_testing();
        ...
        assert_eq!(
            event,
            EthBridgeEvent::EthMysBridgeEvents(EthMysBridgeEvents::TokensDepositedFilter(
                TokensDepositedFilter {
                    source_chain_id: 12,
                    nonce: 0,
                    destination_chain_id: 2,
                    token_id: 2,
                    mys_adjusted_amount: 4200000000,
    ...
    #[test]
    fn test_0_mys_amount_conversion_for_eth_event() {
        let e = EthBridgeEvent::EthMysBridgeEvents(EthMysBridgeEvents::TokensDepositedFilter(
            TokensDepositedFilter {
                source_chain_id: BridgeChainId::EthSepolia as u8,
                nonce: 0,
                destination_chain_id: BridgeChainId::MysTestnet as u8,
                token_id: 2,
                mys_adjusted_amount: 1,
                sender_address: EthAddress::random(),
                recipient_address: ethers::types::Bytes::from(
                    MysAddress::random_for_testing_only().to_vec(),
                ),
            },
        ));
        assert!(e
