...
use crate::types::AddTokensOnMysAction;
...
use crate::types::EthToMysBridgeAction;
...
use mys_types::base_types::MYS_ADDRESS_LENGTH;
...
pub const ADD_TOKENS_ON_MYS_MESSAGE_VERSION: u8 = 1;
pub const ADD_TOKENS_ON_EVM_MESSAGE_VERSION: u8 = 1;

pub const BRIDGE_MESSAGE_PREFIX: &[u8] = b"MYS_BRIDGE_MESSAGE";
...
impl BridgeMessageEncoding for MysToEthBridgeAction {
    fn as_bytes(&self) -> Vec<u8> {
        let mut bytes = Vec::new();
        let e = &self.mys_bridge_event;
        // Add message type
        ...
        // Add source chain id
        bytes.push(e.mys_chain_id as u8);
    ...
    fn as_payload_bytes(&self) -> Vec<u8> {
        ...
        bytes.push(MYS_ADDRESS_LENGTH as u8);
        // Add source address
        bytes.extend_from_slice(&e.mys_address.to_vec());
        // Add dest chain id
        ...
        // Add token amount
        bytes.extend_from_slice(&e.amount_mys_adjusted.to_be_bytes());

        bytes
    }
}

impl BridgeMessageEncoding for EthToMysBridgeAction {
    fn as_bytes(&self) -> Vec<u8> {
        let mut bytes = Vec::new();
    ...
    fn as_payload_bytes(&self) -> Vec<u8> {
        ...
        // Add token amount
        bytes.extend_from_slice(&e.mys_adjusted_amount.to_be_bytes());
...
impl BridgeMessageEncoding for AddTokensOnMysAction {
    fn as_bytes(&self) -> Vec<u8> {
        let mut bytes = Vec::new();
        // Add message type
        bytes.push(BridgeActionType::AddTokensOnMys as u8);
        // Add message version
        bytes.push(ADD_TOKENS_ON_MYS_MESSAGE_VERSION);
        // Add nonce
        bytes.extend_from_slice(&self.nonce.to_be_bytes());
        // Add chain id
        bytes.push(self.chain_id as u8);
...
impl BridgeMessageEncoding for AddTokensOnEvmAction {
    ...
    fn as_payload_bytes(&self) -> Vec<u8> {
        ...
        // Add token mys decimals
        // Unwrap: bcs serialization should not fail
        bytes.push(u8::try_from(self.token_mys_decimals.len()).unwrap());
        for token_mys_decimal in &self.token_mys_decimals {
            bytes.push(*token_mys_decimal);
        }
...
mod tests {
    ...
    use crate::events::EmittedMysToEthTokenBridgeV1;
    ...
    fn test_bridge_message_encoding() -> anyhow::Result<()> {
        ...
        let mys_chain_id = BridgeChainId::MysTestnet;
        let mys_tx_event_index = 1u16;
        let eth_chain_id = BridgeChainId::EthSepolia;
        let mys_address = MysAddress::random_for_testing_only();
        let eth_address = EthAddress::random();
        let token_id = TOKEN_ID_USDC;
        let amount_mys_adjusted = 1_000_000;

        let mys_bridge_event = EmittedMysToEthTokenBridgeV1 {
            nonce,
            mys_chain_id,
            eth_chain_id,
            mys_address,
        ...
        let encoded_bytes = BridgeAction::MysToEthBridgeAction(MysToEthBridgeAction {
            mys_tx_digest,
            mys_tx_event_index,
            mys_bridge_event,
        })
        ...
        let source_chain_id_bytes = vec![mys_chain_id as u8]; // len: 1

        let mys_address_length_bytes = vec![MYS_ADDRESS_LENGTH as u8]; // len: 1
        let mys_address_bytes = mys_address.to_vec(); // len: 32
        ...
        let token_amount_bytes = amount_mys_adjusted.to_be_bytes().to_vec(); // len: 8
        ...
        combined_bytes.extend_from_slice(&mys_address_length_bytes);
        combined_bytes.extend_from_slice(&mys_address_bytes);
    ...
    #[test]
    fn test_bridge_message_encoding_regression_emitted_mys_to_eth_token_bridge_v1(
    ) -> anyhow::Result<()> {
        telemetry_subscribers::init_for_testing();
        let registry = Registry::new();
        mysten_metrics::init_metrics(&registry);
        ...
        let mys_chain_id = BridgeChainId::MysTestnet;
        let eth_chain_id = BridgeChainId::EthSepolia;
        let mys_address = MysAddress::from_str(
    ...
    #[test]
    fn test_bridge_message_encoding_regression_eth_to_mys_token_bridge_v1() -> anyhow::Result<()> {
        telemetry_subscribers::init_for_testing();
        let registry = Registry::new();
        mysten_metrics::init_metrics(&registry);
        ...
        let eth_address =
            EthAddress::from_str("0x00000000000000000000000000000000000000c8").unwrap();
        let token_id = TOKEN_ID_USDC;
        let mys_adjusted_amount = 12345;

        let eth_bridge_event = EthToMysTokenBridgeV1 {
            nonce,
            mys_chain_id,
            eth_chain_id,
            mys_address,
        ...
        let encoded_bytes = BridgeAction::EthToMysBridgeAction(EthToMysBridgeAction {
            eth_tx_hash,
            eth_event_index,
            eth_bridge_event,
        })
    ...
    #[test]
    fn test_bridge_message_encoding_regression_add_coins_on_mys() -> anyhow::Result<()> {
        telemetry_subscribers::init_for_testing();

        let action = BridgeAction::AddTokensOnMysAction(AddTokensOnMysAction {
            nonce: 0,
            chain_id: BridgeChainId::MysCustom,
            native: false,
