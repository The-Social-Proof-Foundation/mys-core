...
//! `BridgeMonitor` receives all `MysBridgeEvent` and `EthBridgeEvent`
//! and handles them accordingly.

use crate::abi::{
    EthBridgeCommitteeEvents, EthBridgeConfigEvents, EthBridgeEvent, EthBridgeLimiterEvents,
    EthCommitteeUpgradeableContractEvents, EthMysBridgeEvents,
};
...
pub struct BridgeMonitor<C> {
    ...
    mys_monitor_rx: mysten_metrics::metered_channel::Receiver<MysBridgeEvent>,
    ...
    C: MysClientInner + 'static,
{
    ...
    pub async fn run(self) {
        ...
        let mut latest_token_config = (*mys_token_type_tags.load().clone()).clone();

        loop {
            tokio::select! {
                mys_event = mys_monitor_rx.recv() => {
                    if let Some(mys_event) = mys_event {
                        Self::handle_mys_events(
                            mys_event,
                            &mys_client,
                            &bridge_auth_agg,
                            &bridge_paused_watch_tx,
                        ...
                        panic!("BridgeMonitor mys events channel was closed unexpectedly");
                    }
                }
    ...
    async fn handle_mys_events(
        event: MysBridgeEvent,
        mys_client: &Arc<MysClient<C>>,
        bridge_auth_agg: &Arc<ArcSwap<BridgeAuthorityAggregator>>,
        bridge_paused_watch_tx: &tokio::sync::watch::Sender<IsBridgePaused>,
        mys_token_type_tags: &Arc<ArcSwap<HashMap<u8, TypeTag>>>,
        ...
        info!("Received MysBridgeEvent: {:?}", event);
        macro_rules! bump_mys_counter {
            ($action:expr) => {
                bridge_metrics
                    .observed_governance_actions
                    .with_label_values(&[$action, "mys"])
                    .inc();
            };
        }

        match event {
            MysBridgeEvent::MysToEthTokenBridgeV1(_) => (),
            MysBridgeEvent::TokenTransferApproved(_) => (),
            MysBridgeEvent::TokenTransferClaimed(_) => (),
            MysBridgeEvent::TokenTransferAlreadyApproved(_) => (),
            MysBridgeEvent::TokenTransferAlreadyClaimed(_) => (),
            MysBridgeEvent::TokenTransferLimitExceed(_) => {
                // TODO do we want to do anything here?
            }

            MysBridgeEvent::EmergencyOpEvent(event) => {
                bump_mys_counter!(if event.frozen {
                    "bridge_paused"
                } else {
                    "bridge_unpaused"
                });
            ...
            MysBridgeEvent::CommitteeMemberRegistration(_) => {
                bump_mys_counter!("committee_member_registered");
            }
            MysBridgeEvent::CommitteeUpdateEvent(_) => {
                bump_mys_counter!("committee_updated");
            }

            MysBridgeEvent::CommitteeMemberUrlUpdateEvent(event) => {
                bump_mys_counter!("committee_member_url_updated");
            ...
            MysBridgeEvent::BlocklistValidatorEvent(event) => {
                bump_mys_counter!(if event.blocklisted {
            ...
            MysBridgeEvent::TokenRegistrationEvent(_) => {
                bump_mys_counter!("new_token_registered");
            }

            MysBridgeEvent::NewTokenEvent(event) => {
                bump_mys_counter!("new_token_added");
                    ...
                    latest_token_config.entry(event.token_id)
                {
                    ...
                    mys_token_type_tags.store(Arc::new(latest_token_config.clone()));
                } else {
                    // invariant
            ...
            MysBridgeEvent::UpdateTokenPriceEvent(_event) => {
                bump_mys_counter!("token_price_updated");
            }

            MysBridgeEvent::UpdateRouteLimitEvent(_event) => {
                bump_mys_counter!("limit_updated");
            }
        }
    }
    ...
    fn handle_eth_events(event: EthBridgeEvent, bridge_metrics: &Arc<BridgeMetrics>) {
        ...
        match event {
            ...
            EthBridgeEvent::EthMysBridgeEvents(event) => match event {
                EthMysBridgeEvents::TokensClaimedFilter(_) => (),
                EthMysBridgeEvents::TokensDepositedFilter(_) => (),
                EthMysBridgeEvents::PausedFilter(_) => bump_eth_counter!("bridge_paused"),
                EthMysBridgeEvents::UnpausedFilter(_) => bump_eth_counter!("bridge_unpaused"),
                EthMysBridgeEvents::UpgradedFilter(_) => {
                    bump_eth_counter!("bridge_contract_upgraded")
                }
                EthMysBridgeEvents::InitializedFilter(_) => {
                    bump_eth_counter!("bridge_contract_initialized")
                }
                EthMysBridgeEvents::ContractUpgradedFilter(_) => {
                    bump_eth_counter!("bridge_contract_upgraded")
                }
                EthMysBridgeEvents::EmergencyOperationFilter(e) => {
                    if e.paused {
...
async fn get_latest_bridge_committee_with_url_update_event<C: MysClientInner>(
    mys_client: Arc<MysClient<C>>,
    ...
    loop {
        let Ok(Ok(committee)) = retry_with_max_elapsed_time!(
            mys_client.get_bridge_committee(),
...
async fn get_latest_bridge_committee_with_blocklist_event<C: MysClientInner>(
    mys_client: Arc<MysClient<C>>,
...
async fn get_latest_bridge_pause_status_with_emergency_event<C: MysClientInner>(
    mys_client: Arc<MysClient<C>>,
    ...
    loop {
        let Ok(Ok(summary)) =
            retry_with_max_elapsed_time!(mys_client.get_bridge_summary(), Duration::from_secs(600))
        else {
...
mod tests {
    ...
    async fn test_get_latest_bridge_committee_with_url_update_event() {
        ...
        mys_client_mock.set_bridge_committee(summary.clone());
        ...
        mys_client_mock.set_bridge_committee(old_summary.clone());
        let timer = std::time::Instant::now();
        // update the url to "http://new.url" in 1 second
        let mys_client_mock_clone = mys_client_mock.clone();
        tokio::spawn(async move {
            tokio::time::sleep(Duration::from_secs(1)).await;
            mys_client_mock_clone.set_bridge_committee(summary.clone());
        });
        let committee = get_latest_bridge_committee_with_url_update_event(
            mys_client.clone(),
            event.clone(),
        ...
        mys_client_mock.set_bridge_committee(newer_summary.clone());
        let timer = std::time::Instant::now();
        let committee = get_latest_bridge_committee_with_url_update_event(
            mys_client.clone(),
            event.clone(),
    ...
    async fn test_get_bridge_pause_status_with_emergency_event() {
        ...
        mys_client_mock.set_is_bridge_paused(BRIDGE_PAUSED);
        let timer = std::time::Instant::now();
        assert!(
        ...
        mys_client_mock.set_is_bridge_paused(BRIDGE_UNPAUSED);
        let timer = std::time::Instant::now();
        assert!(
            !get_latest_bridge_pause_status_with_emergency_event(
                mys_client.clone(),
        ...
        tokio::spawn(async move {
            tokio::time::sleep(Duration::from_secs(1)).await;
            mys_client_mock_clone.set_is_bridge_paused(BRIDGE_UNPAUSED);
        });
        assert!(
            !get_latest_bridge_pause_status_with_emergency_event(
                mys_client.clone(),
    ...
    async fn test_update_bridge_authority_aggregation_with_url_change_event() {
        ...
        let mys_token_type_tags = Arc::new(ArcSwap::from(Arc::new(HashMap::new())));
        ...
        mys_client_mock.set_bridge_committee(new_committee_summary.clone());
        mys_monitor_tx
            .send(MysBridgeEvent::CommitteeMemberUrlUpdateEvent(
    ...
    async fn test_update_bridge_authority_aggregation_with_blocklist_event() {
        ...
        mys_client_mock.set_bridge_committee(new_committee_summary.clone());
        mys_monitor_tx
            .send(MysBridgeEvent::BlocklistValidatorEvent(
    ...
    async fn test_update_bridge_pause_status_with_emergency_event() {
        ...
        mys_client_mock.set_is_bridge_paused(event.frozen);
        mys_monitor_tx
            .send(MysBridgeEvent::EmergencyOpEvent(event.clone()))
            .await
            .unwrap();
        // Wait for the monitor to process the event
        tokio::time::sleep(Duration::from_secs(1)).await;
    ...
    #[tokio::test]
    async fn test_update_mys_token_type_tags() {
        let (
            mys_monitor_tx,
            mys_monitor_rx,
            _eth_monitor_tx,
        ...
        let mys_token_type_tags = Arc::new(ArcSwap::from(Arc::new(HashMap::new())));
        let mys_token_type_tags_clone = mys_token_type_tags.clone();
        let _handle = tokio::task::spawn(
            BridgeMonitor::new(
                mys_client.clone(),
                mys_monitor_rx,
        ...
        mys_monitor_tx
            .send(MysBridgeEvent::NewTokenEvent(event.clone()))
            .await
            .unwrap();
        // Wait for the monitor to process the event
        tokio::time::sleep(Duration::from_secs(1)).await;
        // Now expect new token type tags to appear in mys_token_type_tags
        mys_token_type_tags
            .load()
            .clone()
            .get(&event.token_id)
    ...
    fn setup() -> (
        mysten_metrics::metered_channel::Sender<MysBridgeEvent>,
        mysten_metrics::metered_channel::Receiver<MysBridgeEvent>,
        mysten_metrics::metered_channel::Sender<EthBridgeEvent>,
        mysten_metrics::metered_channel::Receiver<EthBridgeEvent>,
        MysMockClient,
        Arc<MysClient<MysMockClient>>,
