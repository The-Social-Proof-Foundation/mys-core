...
pub struct BridgeRequestMockHandler {
    ...
    mys_token_events: Arc<
        Mutex<
            HashMap<(TransactionDigest, u16), (BridgeResult<SignedBridgeAction>, Option<Duration>)>,
        >,
    >,
    mys_token_events_requested: Arc<Mutex<HashMap<(TransactionDigest, u16), u64>>>,
}
...
impl BridgeRequestMockHandler {
    pub fn new() -> Self {
        Self {
            ...
            mys_token_events: Arc::new(Mutex::new(HashMap::new())),
            mys_token_events_requested: Arc::new(Mutex::new(HashMap::new())),
        }
    }

    pub fn add_mys_event_response(
        &self,
        ...
        self.mys_token_events
            .lock()
            .unwrap()
            .insert((tx_digest, idx), (response, delay));
    }

    pub fn get_mys_token_events_requested(
        &self,
        tx_digest: TransactionDigest,
        event_index: u16,
    ) -> u64 {
        *self
            .mys_token_events_requested
            .lock()
            .unwrap()
...
impl BridgeRequestHandlerTrait for BridgeRequestMockHandler {
    ...
    async fn handle_mys_tx_digest(
        ...
        let (result, delay) = {
            let preset = self.mys_token_events.lock().unwrap();
            if !preset.contains_key(&(tx_digest, event_idx)) {
                // Ok to panic in test
                panic!(
                    "No preset handle_mys_tx_digest result for tx_digest: {}, event_idx: {}",
                    tx_digest, event_idx
                );
            }
            let mut requested = self.mys_token_events_requested.lock().unwrap();
        ...
        let signed_action: mys_types::message_envelope::Envelope<
