...
use crate::{
    ...
    types::{
        AddTokensOnEvmAction, AddTokensOnMysAction, AssetPriceUpdateAction,
...
pub(crate) fn make_router(
    ...
    Router::new()
        ...
        .route(MYS_TO_ETH_TX_PATH, get(handle_mys_tx_digest))
        .route(
...
async fn handle_mys_tx_digest(
    ...
    let future = async {
        let sig: Json<SignedBridgeAction> = handler
            .handle_mys_tx_digest(tx_digest_base58, event_idx)
            .await?;
        Ok(sig)
    };
    with_metrics!(metrics.clone(), "handle_mys_tx_digest", future).await
}
...
async fn handle_add_tokens_on_mys(
    ...
    let future = async {
        ...
        if !chain_id.is_mys_chain() {
            return Err(BridgeError::InvalidBridgeClientRequest(
                "handle_add_tokens_on_mys only expects Mys chain id".to_string(),
            ));
        }
    ...
    with_metrics!(metrics.clone(), "handle_add_tokens_on_mys", future).await
}

#[instrument(level = "error", skip_all, fields(chain_id=chain_id, nonce=nonce, native=native, token_ids=token_ids, token_addresses=token_addresses, token_mys_decimals=token_mys_decimals, token_prices=token_prices))]
async fn handle_add_tokens_on_evm(
    Path((chain_id, nonce, native, token_ids, token_addresses, token_mys_decimals, token_prices)): Path<(
        u8,
        u64,
        u8,
        String,
    ...
    let future = async {
        ...
        if chain_id.is_mys_chain() {
            return Err(BridgeError::InvalidBridgeClientRequest(
                "handle_add_tokens_on_evm does not expect Mys chain id".to_string(),
            ));
        }
        ...
        let token_addresses = token_addresses
            ...
            .collect::<Result<Vec<_>, _>>()?;
        let token_mys_decimals = token_mys_decimals
            .split(',')
            .map(|s| {
                s.parse::<u8>().map_err(|err| {
                    BridgeError::InvalidBridgeClientRequest(format!(
                        "Invalid token mys decimals: {:?}",
                        err
                    ))
                })
            })
...
mod tests {
    ...
    #[tokio::test]
    async fn test_bridge_server_handle_add_tokens_on_mys_action_path() {
        let client = setup();
