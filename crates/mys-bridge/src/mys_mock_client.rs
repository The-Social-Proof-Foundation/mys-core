...
pub struct MysMockClient {
    ...
    events_by_tx_digest:
        Arc<Mutex<HashMap<TransactionDigest, Result<Vec<MysEvent>, mys_sdk::error::Error>>>>,
    transaction_responses:
        Arc<Mutex<HashMap<TransactionDigest, BridgeResult<MysTransactionBlockResponse>>>>,
    wildcard_transaction_response: Arc<Mutex<Option<BridgeResult<MysTransactionBlockResponse>>>>,
...
impl MysMockClient {
    ...
    pub fn add_events_by_tx_digest(&self, tx_digest: TransactionDigest, events: Vec<MysEvent>) {
    ...
    pub fn add_events_by_tx_digest_error(&self, tx_digest: TransactionDigest) {
        self.events_by_tx_digest.lock().unwrap().insert(
            tx_digest,
            Err(mys_sdk::error::Error::DataError("".to_string())),
        );
    }
    ...
    pub fn add_transaction_response(
        ...
        response: BridgeResult<MysTransactionBlockResponse>,
    ) {
...
#[async_trait]
impl MysClientInner for MysMockClient {
    ...
    async fn get_events_by_tx_digest(
            ...
            .unwrap_or_else(|| panic!("No preset events found for tx_digest: {:?}", tx_digest))
        {
            ...
            // mys_sdk::error::Error is not Clone
            Err(_) => Err(mys_sdk::error::Error::DataError("".to_string())),
        }
    }
