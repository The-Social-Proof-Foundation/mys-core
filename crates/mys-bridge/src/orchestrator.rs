...
//! 1. monitors Mys and Ethereum events with the help of `MysSyncer` and `EthSyncer`
...
pub struct BridgeOrchestrator<C> {
    ...
    mys_events_rx: mysten_metrics::metered_channel::Receiver<(Identifier, Vec<MysEvent>)>,
    eth_events_rx: mysten_metrics::metered_channel::Receiver<(EthAddress, u64, Vec<EthLog>)>,
    store: Arc<BridgeOrchestratorTables>,
    mys_monitor_tx: mysten_metrics::metered_channel::Sender<MysBridgeEvent>,
    ...
    C: MysClientInner + 'static,
{
    ...
    pub async fn run(
        ...
        task_handles.push(spawn_logged_monitored_task!(Self::run_mys_watcher(
            store_clone,
    ...
    async fn run_mys_watcher(
        store: Arc<BridgeOrchestratorTables>,
        executor_tx: mysten_metrics::metered_channel::Sender<BridgeActionExecutionWrapper>,
        mut mys_events_rx: mysten_metrics::metered_channel::Receiver<(Identifier, Vec<MysEvent>)>,
        monitor_tx: mysten_metrics::metered_channel::Sender<MysBridgeEvent>,
        metrics: Arc<BridgeMetrics>,
    ) {
        info!("Starting mys watcher task");
        while let Some((identifier, events)) = mys_events_rx.recv().await {
            if events.is_empty() {
                continue;
            }
            info!("Received {} Mys events: {:?}", events.len(), events);
            metrics
                .mys_watcher_received_events
                .inc_by(events.len() as u64);
            let bridge_events = events
                .iter()
                .filter_map(|mys_event| {
                    match MysBridgeEvent::try_from_mys_event(mys_event) {
                        ...
                        Err(BridgeError::ZeroValueBridgeTransfer(_)) => {
                            error!("Zero value bridge transfer: {:?}", mys_event);
                            None
                        }
                        Err(e) => {
                            panic!(
                                "Mys Event could not be deserialzed to MysBridgeEvent: {:?}",
                                e
                            );
                        }
                    }
            ...
            let mut actions = vec![];
            for (mys_event, opt_bridge_event) in events.iter().zip(bridge_events) {
                if opt_bridge_event.is_none() {
                    // TODO: we probably should not miss any events, log for now.
                    metrics.mys_watcher_unrecognized_events.inc();
                    error!("Mys event not recognized: {:?}", mys_event);
                    continue;
                }
                // Unwrap safe: checked above
                let bridge_event: MysBridgeEvent = opt_bridge_event.unwrap();
                info!("Observed Mys bridge event: {:?}", bridge_event);
            ...
            if !actions.is_empty() {
                info!("Received {} actions from Mys: {:?}", actions.len(), actions);
                metrics
                    .mys_watcher_received_actions
            ...
            store
                .update_mys_event_cursor(identifier, cursor)
                .expect("Store operation should not fail");
        }
        panic!("Mys event channel was closed unexpectedly");
    }
...
mod tests {
    use crate::{
        test_utils::{get_test_eth_to_mys_bridge_action, get_test_log_and_action},
    ...
    use crate::{events::tests::get_test_mys_event_and_action, mys_mock_client::MysMockClient};

    #[tokio::test]
    async fn test_mys_watcher_task() {
        // Note: this test may fail because of the following reasons:
        // the MysEvent's struct tag does not match the ones in events.rs
        ...
        let identifier = Identifier::from_str("test_mys_watcher_task").unwrap();
        let (mys_event, bridge_action) = get_test_mys_event_and_action(identifier.clone());
        mys_events_tx
            .send((identifier.clone(), vec![mys_event.clone()]))
            .await
            .unwrap();
        ...
        loop {
            ...
            assert_eq!(
                store.get_mys_event_cursors(&[identifier]).unwrap()[0].unwrap(),
                mys_event.id,
            );
            break;
        }
    ...
    async fn test_resume_actions_in_pending_logs() {
        ...
        let action2 = get_test_eth_to_mys_bridge_action(None, None, None, None);
        store
    ...
    fn setup() -> (
        mysten_metrics::metered_channel::Sender<(Identifier, Vec<MysEvent>)>,
        mysten_metrics::metered_channel::Receiver<(Identifier, Vec<MysEvent>)>,
        ...
        let (mys_events_tx, mys_events_rx) = mysten_metrics::metered_channel::channel(
            100,
            &mysten_metrics::get_metrics()
                .unwrap()
                .channel_inflight
                .with_label_values(&["unit_test_mys_events_queue"]),
        );
