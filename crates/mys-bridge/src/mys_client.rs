...
use mys_types::bridge::BridgeTreasurySummary;
use mys_types::bridge::MoveTypeCommitteeMember;
use mys_types::bridge::MoveTypeParsedTokenTransferMessage;
...
pub struct MysClient<P> {
    inner: P,
    bridge_metrics: Arc<BridgeMetrics>,
}

pub type MysBridgeClient = MysClient<MysSdkClient>;

impl MysBridgeClient {
    pub async fn new(rpc_url: &str, bridge_metrics: Arc<BridgeMetrics>) -> anyhow::Result<Self> {
        let inner = MysClientBuilder::default()
            .build(rpc_url)
            .await
            .map_err(|e| {
                anyhow!("Can't establish connection with Mys Rpc {rpc_url}. Error: {e}")
            })?;
        let self_ = Self {
            inner,
            bridge_metrics,
    ...
    pub fn mys_client(&self) -> &MysSdkClient {
        &self.inner
    }
}
    ...
    P: MysClientInner,
{
    ...
    async fn describe(&self) -> anyhow::Result<()> {
        ...
        tracing::info!(
            "MysClient is connected to chain {chain_id}, current block number: {block_number}"
        );
        Ok(())
    }
    ...
    ) -> BridgeResult<Page<MysEvent, EventID>> {
    ...
    /// Returns BridgeAction from a Mys Transaction with transaction hash
    ...
    pub async fn get_bridge_action_by_tx_digest_and_event_idx_maybe(
        ...
        if event.type_.address.as_ref() != BRIDGE_PACKAGE_ID.as_ref() {
            return Err(BridgeError::BridgeEventInUnrecognizedMysPackage);
        }
        let bridge_event = MysBridgeEvent::try_from_mys_event(event)?
            .ok_or(BridgeError::NoBridgeEventsInTxPosition)?;
    ...
    pub async fn get_token_id_map(&self) -> BridgeResult<HashMap<u8, TypeTag>> {
        self.get_bridge_summary()
            ...
            .into_iter()
            .map(|(id, name)| {
                parse_mys_type_tag(&format!("0x{name}"))
                    .map(|name| (id, name))
                    .map_err(|e| {
        ...
        for (_, member) in move_type_bridge_committee.members {
            ...
            let base_url = from_utf8(&http_rest_url).unwrap_or_else(|_e| {
                warn!(
                    ...
                    mys_address, bridge_pubkey_bytes, http_rest_url
                );
                ""
            });
    ...
    pub async fn execute_transaction_block_with_effects(
        &self,
        tx: mys_types::transaction::Transaction,
    ) -> BridgeResult<MysTransactionBlockResponse> {
...
/// Use a trait to abstract over the MysSDKClient and MysMockClient for testing.
#[async_trait]
pub trait MysClientInner: Send + Sync {
    ...
    async fn get_events_by_tx_digest(
        &self,
        tx_digest: TransactionDigest,
    ) -> Result<Vec<MysEvent>, Self::Error>;
    ...
    async fn execute_transaction_block_with_effects(
        &self,
        tx: Transaction,
    ) -> Result<MysTransactionBlockResponse, BridgeError>;
...
#[async_trait]
impl MysClientInner for MysSdkClient {
    type Error = mys_sdk::error::Error;
    ...
    async fn execute_transaction_block_with_effects(
        ...
        match self.quorum_driver_api().execute_transaction_block(
            tx,
            MysTransactionBlockResponseOptions::new().with_effects().with_events(),
            Some(mys_types::quorum_driver_types::ExecuteTransactionRequestType::WaitForEffectsCert),
        ).await {
            Ok(response) => Ok(response),
            Err(e) => return Err(BridgeError::MysTxFailureGeneric(e.to_string())),
        }
    }
...
mod tests {
    ...
    use crate::{
        events::{EmittedMysToEthTokenBridgeV1, MoveTokenDepositedEvent},
        mys_mock_client::MysMockClient,
        test_utils::{
            approve_action_with_validator_secrets, bridge_token, get_test_eth_to_mys_bridge_action,
            get_test_mys_to_eth_bridge_action,
        },
        types::MysToEthBridgeAction,
    };
    ...
    async fn get_bridge_action_by_tx_digest_and_event_idx_maybe() {
        ...
        let mock_client = MysMockClient::default();
        let mys_client = MysClient::new_for_testing(mock_client.clone());
        let tx_digest = TransactionDigest::random();

        // Ensure all struct tags are inited
        init_all_struct_tags();

        let sanitized_event_1 = EmittedMysToEthTokenBridgeV1 {
            nonce: 1,
        ...
        let emitted_event_1 = MoveTokenDepositedEvent {
            ...
            source_chain: sanitized_event_1.mys_chain_id as u8,
            sender_address: sanitized_event_1.mys_address.to_vec(),
            ...
            amount_mys_adjusted: sanitized_event_1.amount_mys_adjusted,
        };
        ...
        mys_event_2.type_.module = Identifier::from_str("unrecognized_module").unwrap();
        mys_event_2.bcs = BcsEvent::new(bcs::to_bytes(&event_2).unwrap());

        // Event 3 is defined in non-bridge package
        let mut mys_event_3 = mys_event_1.clone();
        mys_event_3.type_.address = AccountAddress::random();
        ...
        let expected_action_1 = BridgeAction::MysToEthBridgeAction(MysToEthBridgeAction {
            mys_tx_digest: tx_digest,
            mys_tx_event_index: 0,
            mys_bridge_event: sanitized_event_1.clone(),
        });
        assert_eq!(
            mys_client
        ...
        let expected_action_2 = BridgeAction::MysToEthBridgeAction(MysToEthBridgeAction {
            mys_tx_digest: tx_digest,
            mys_tx_event_index: 2,
            mys_bridge_event: sanitized_event_1.clone(),
        });
        assert_eq!(
            mys_client
        ...
        mys_event_2.type_ = MysToEthTokenBridgeV1.get().unwrap().clone();
        mock_client.add_events_by_tx_digest(tx_digest, vec![mys_event_2]);
        mys_client
            .get_bridge_action_by_tx_digest_and_event_idx_maybe(&tx_digest, 2)
            .await
            .unwrap_err();
    ...
    async fn test_get_action_onchain_status_for_mys_to_eth_transfer() {
        telemetry_subscribers::init_for_testing();
        ...
        let mys_client =
            MysClient::new(&test_cluster.inner.fullnode_handle.rpc_url, bridge_metrics)
                .await
                .unwrap();
        ...
        let bridge_object_arg = mys_client
            ...
            .await;
        let id_token_map = mys_client.get_token_id_map().await.unwrap();

        // 1. Create a Eth -> Mys Transfer (recipient is sender address), approve with validator secrets and assert its status to be Claimed
        let action = get_test_eth_to_mys_bridge_action(None, Some(usdc_amount), Some(sender), None);
        ...
        // 2. Create a Mys -> Eth Transfer, approve with validator secrets and assert its status to be Approved
        ...
        let action =
            get_test_mys_to_eth_bridge_action(None, None, Some(100), None, None, None, None);
        let status = mys_client
            .inner
            .get_token_transfer_action_onchain_status(
                bridge_object_arg,
