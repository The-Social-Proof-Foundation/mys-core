...
const MYS_PKG_NAME: &str = "Mys";

// Use testnet by default. Probably want to add options to make this configurable later
const MYS_PKG_PATH: &str = "{ git = \"https://github.com/MystenLabs/mys.git\", subdir = \"crates/mys-framework/packages/mys-framework\", rev = \"framework/testnet\", override = true }";

#[derive(Parser)]
#[group(id = "mys-move-new")]
pub struct New {
    #[clap(flatten)]
    pub new: new::New,
}
...
impl New {
    pub fn execute(self, path: Option<&Path>) -> anyhow::Result<()> {
        ...
        self.new
            .execute(path, [(MYS_PKG_NAME, MYS_PKG_PATH)], [(name, "0x0")], "")?;
