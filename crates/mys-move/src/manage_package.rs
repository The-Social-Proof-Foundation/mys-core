...
const NO_LOCK_FILE: &str = "Expected a `Move.lock` file to exist in the package path, \
                            but none found. Consider running `mys move build` to \
...
/// Move.lock).
#[derive(Parser)]
#[group(id = "mys-move-manage-package")]
pub struct ManagePackage {
    #[clap(long)]
    /// The environment to associate this package information with (consider using `mys client active-env`).
