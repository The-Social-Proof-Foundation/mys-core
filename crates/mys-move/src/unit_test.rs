    ...
    pub fn execute(
        ...
        if !cfg!(debug_assertions) && compute_coverage {
            return Err(anyhow::anyhow!(
                "The --coverage flag is currently supported only in debug builds. Please build the Mys CLI from source in debug mode."
            ));
        }
