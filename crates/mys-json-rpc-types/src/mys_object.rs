...
use mys_types::error::{
    ExecutionError, MysError, MysObjectResponseError, MysResult, UserInputError, UserInputResult,
};
...
pub struct MysObjectResponse {
    ...
    pub data: Option<MysObjectData>,
    #[serde(skip_serializing_if = "Option::is_none")]
    pub error: Option<MysObjectResponseError>,
}

impl MysObjectResponse {
    pub fn new(data: Option<MysObjectData>, error: Option<MysObjectResponseError>) -> Self {
        Self { data, error }
    }

    pub fn new_with_data(data: MysObjectData) -> Self {
        Self {
            data: Some(data),
            error: None,
        }
    ...
    pub fn new_with_error(error: MysObjectResponseError) -> Self {
        Self {
            data: None,
            error: Some(error),
        }
...
impl Ord for MysObjectResponse {
    fn cmp(&self, other: &Self) -> Ordering {
        match (&self.data, &other.data) {
            ...
            // In this ordering those with data will come before MysObjectResponses that are errors.
            (Some(_), None) => Ordering::Less,
            (None, Some(_)) => Ordering::Greater,
            // MysObjectResponses that are errors are just considered equal.
            _ => Ordering::Equal,
        }
    }
}

impl PartialOrd for MysObjectResponse {
...
impl MysObjectResponse {
    pub fn move_object_bcs(&self) -> Option<&Vec<u8>> {
        match &self.data {
            Some(MysObjectData {
                bcs: Some(MysRawData::MoveObject(obj)),
                ..
            }) => Some(&obj.bcs_bytes),
            _ => None,
        }
    ...
    pub fn object_id(&self) -> Result<ObjectID, anyhow::Error> {
        match (&self.data, &self.error) {
            ...
            (None, Some(MysObjectResponseError::NotExists { object_id })) => Ok(*object_id),
            (
                None,
            ...
            _ => Err(anyhow!("Could not get object_id, something went wrong with MysObjectResponse construction.")),
        }
    }
...
impl TryFrom<MysObjectResponse> for ObjectInfo {
    type Error = anyhow::Error;

    fn try_from(value: MysObjectResponse) -> Result<Self, Self::Error> {
        let MysObjectData {
            object_id,
            version,
            digest,
...
pub struct MysObjectData {
    ...
    /// Move object content or package content in BCS, default to be None unless MysObjectDataOptions.showBcs is set to true
    #[serde(skip_serializing_if = "Option::is_none")]
    pub bcs: Option<MysRawData>,
}
...
impl Display for MysObjectData {
...
impl TryFrom<&MysObjectData> for GasCoin {
    type Error = anyhow::Error;
    fn try_from(object: &MysObjectData) -> Result<Self, Self::Error> {
        match &object
            .content
            .as_ref()
            .ok_or_else(|| anyhow!("Expect object content to not be empty"))?
        {
            MysParsedData::MoveObject(o) => {
                if GasCoin::type_() == o.type_ {
                    return GasCoin::try_from(&o.fields);
                }
            }
            MysParsedData::Package(_) => {}
        }
...
impl TryFrom<&MysMoveStruct> for GasCoin {
    type Error = anyhow::Error;
    fn try_from(move_struct: &MysMoveStruct) -> Result<Self, Self::Error> {
        match move_struct {
            MysMoveStruct::WithFields(fields) | MysMoveStruct::WithTypes { type_: _, fields } => {
                if let Some(MysMoveValue::String(balance)) = fields.get("balance") {
                    if let Ok(balance) = balance.parse::<u64>() {
                        if let Some(MysMoveValue::UID { id }) = fields.get("id") {
...
pub struct MysObjectDataOptions {
...
impl TryFrom<(ObjectRead, MysObjectDataOptions)> for MysObjectResponse {
    type Error = anyhow::Error;

    fn try_from(
        (object_read, options): (ObjectRead, MysObjectDataOptions),
        ...
        match object_read {
            ...
            ObjectRead::Exists(object_ref, o, layout) => {
                ...
                Ok(MysObjectResponse::new_with_data(data))
            }
            ObjectRead::Deleted((object_id, version, digest)) => Ok(
                MysObjectResponse::new_with_error(MysObjectResponseError::Deleted {
                    object_id,
                    version,
                    digest,
                }),
...
impl TryFrom<(ObjectInfo, MysObjectDataOptions)> for MysObjectResponse {
    type Error = anyhow::Error;

    fn try_from(
        (object_info, options): (ObjectInfo, MysObjectDataOptions),
    ) -> Result<Self, Self::Error> {
        let MysObjectDataOptions {
            show_type,
            show_owner,
        ...
        Ok(Self::new_with_data(MysObjectData {
    ...
    )> for MysObjectData
{
    ...
    fn try_from(
        ...
        let content: Option<MysParsedData> = if show_content {
            let data = match o.data {
                Data::Move(m) => {
                    let layout = layout.ok_or_else(|| {
                        anyhow!("Layout is required to convert Move object to json")
                    })?;
                    MysParsedData::try_from_object(m, layout)?
                }
                Data::Package(p) => MysParsedData::try_from_package(p)?,
            };
            Some(data)
        } else {
            None
    ...
    )> for MysObjectData
{
    ...
    fn try_from(
        ...
        let mut data: MysObjectData = (object_ref, o, layout, options).try_into()?;
        if show_display {
...
impl MysObjectResponse {
    ...
    pub fn object(&self) -> Result<&MysObjectData, MysObjectResponseError> {
        if let Some(data) = &self.data {
            Ok(data)
            ...
            Err(MysObjectResponseError::Unknown)
        }
    }

    /// Returns the object value if there is any, otherwise an Err if
    /// the object does not exist or is deleted.
    pub fn into_object(self) -> Result<MysObjectData, MysObjectResponseError> {
        match self.object() {
...
impl TryInto<Object> for MysObjectData {
    ...
    fn try_into(self) -> Result<Object, Self::Error> {
        ...
        let data = match self.bcs {
            ...
            Some(MysRawData::Package(p)) => Data::Package(MovePackage::new(
                p.id,
                self.version,
                p.module_map,
            ...
            _ => Err(anyhow!(
                "BCS data is required to convert MysObjectData to Object"
            ))?,
        };
        Ok(ObjectInner {
            data,
            owner: self
                .owner
                .ok_or_else(|| anyhow!("Owner is required to convert MysObjectData to Object"))?,
            previous_transaction: self.previous_transaction.ok_or_else(|| {
                anyhow!("previous_transaction is required to convert MysObjectData to Object")
            })?,
            storage_rebate: self.storage_rebate.ok_or_else(|| {
                anyhow!("storage_rebate is required to convert MysObjectData to Object")
            })?,
        }
        .into())
    }
...
impl From<ObjectRef> for MysObjectRef {
...
pub enum MysRawData {
    ...
    MoveObject(MysRawMoveObject),
    Package(MysRawMovePackage),
}

impl MysData for MysRawData {
    type ObjectType = MysRawMoveObject;
    type PackageType = MysRawMovePackage;
...
pub enum MysParsedData {
    // Manually handle generic schema generation
    MoveObject(MysParsedMoveObject),
    Package(MysMovePackage),
}

impl MysData for MysParsedData {
    type ObjectType = MysParsedMoveObject;
    type PackageType = MysMovePackage;

    fn try_from_object(
        object: MoveObject,
        layout: MoveStructLayout,
    ) -> Result<Self, anyhow::Error> {
        Ok(Self::MoveObject(MysParsedMoveObject::try_from_layout(
            object, layout,
        )?))
    }
    ...
    fn try_from_package(package: MovePackage) -> Result<Self, anyhow::Error> {
        ...
        for bytecode in package.serialized_module_map().values() {
            ...
            let module = move_binary_format::CompiledModule::deserialize_with_defaults(bytecode)
                .map_err(|error| MysError::ModuleDeserializationFailure {
            ...
            let d = move_disassembler::disassembler::Disassembler::from_module_with_max_size(
                ...
                *mys_types::move_package::MAX_DISASSEMBLED_MODULE_SIZE,
            )
            .map_err(|e| MysError::ObjectSerializationError {
                error: e.to_string(),
            })?;
            let bytecode_str = d
                .disassemble()
        ...
        Ok(Self::Package(MysMovePackage { disassembled }))
    }
...
impl Display for MysParsedData {
    fn fmt(&self, f: &mut Formatter<'_>) -> fmt::Result {
        let mut writer = String::new();
        match self {
            MysParsedData::MoveObject(o) => {
...
pub struct MysParsedMoveObject {
    #[serde(rename = "type")]
    #[serde_as(as = "MysStructTag")]
    #[schemars(with = "String")]
    pub type_: StructTag,
    pub has_public_transfer: bool,
    pub fields: MysMoveStruct,
}

impl MysMoveObject for MysParsedMoveObject {
    fn try_from_layout(
        object: MoveObject,
        layout: MoveStructLayout,
    ) -> Result<Self, anyhow::Error> {
        ...
        Ok(
            if let MysMoveStruct::WithTypes { type_, fields } = move_struct {
                MysParsedMoveObject {
                    type_,
                    has_public_transfer: object.has_public_transfer(),
                    fields: MysMoveStruct::WithFields(fields),
                }
            } else {
                MysParsedMoveObject {
...
impl MysParsedMoveObject {
    pub fn try_from_object_read(object_read: ObjectRead) -> Result<Self, anyhow::Error> {
        let parsed_data = MysParsedData::try_from_object_read(object_read)?;
        match parsed_data {
            MysParsedData::MoveObject(o) => Ok(o),
            MysParsedData::Package(_) => Err(anyhow::anyhow!("Object is not a Move object")),
        }
    }
}
...
) -> MysResult<(StructTag, serde_json::Value)> {
    match event_data.into() {
        MysMoveValue::Struct(move_struct) => match &move_struct {
            MysMoveStruct::WithTypes { type_, .. } => {
                Ok((type_.clone(), move_struct.clone().to_json_value()))
            }
            _ => Err(MysError::ObjectDeserializationError {
                error: "Found non-type MysMoveStruct in MoveValue event".to_string(),
            }),
        },
        MysMoveValue::Variant(v) => Ok((v.type_.clone(), v.clone().to_json_value())),
        MysMoveValue::Vector(_)
        | MysMoveValue::Number(_)
        | MysMoveValue::Bool(_)
        | MysMoveValue::Address(_)
        | MysMoveValue::String(_)
        | MysMoveValue::UID { .. }
        | MysMoveValue::Option(_) => Err(MysError::ObjectDeserializationError {
...
impl From<MoveObject> for MysRawMoveObject {
...
impl MysMoveObject for MysRawMoveObject {
    fn try_from_layout(
        object: MoveObject,
...
impl From<MovePackage> for MysRawMovePackage {
...
pub enum MysPastObjectResponse {
    /// The object exists and is found with this version
    VersionFound(MysObjectData),
    /// The object does not exist
    ObjectNotExists(ObjectID),
    /// The object is found to be deleted with this version
    ObjectDeleted(MysObjectRef),
...
impl MysPastObjectResponse {
    /// Returns a reference to the object if there is any, otherwise an Err
    pub fn object(&self) -> UserInputResult<&MysObjectData> {
        match &self {
    ...
    pub fn into_object(self) -> UserInputResult<MysObjectData> {
        match self {
            Self::ObjectDeleted(oref) => Err(UserInputError::ObjectDeleted {
                object_ref: oref.to_object_ref(),
            }),
...
pub struct MysMovePackage {
    pub disassembled: BTreeMap<String, Value>,
}

pub type QueryObjectsPage = Page<MysObjectResponse, CheckpointedObjectID>;
pub type ObjectsPage = Page<MysObjectResponse, ObjectID>;
...
pub struct MysGetPastObjectRequest {
...
pub enum MysObjectDataFilter {
    MatchAll(Vec<MysObjectDataFilter>),
    MatchAny(Vec<MysObjectDataFilter>),
...
impl MysObjectDataFilter {
    ...
    pub fn matches(&self, object: &ObjectInfo) -> bool {
        match self {
            MysObjectDataFilter::MatchAll(filters) => !filters.iter().any(|f| !f.matches(object)),
            MysObjectDataFilter::MatchAny(filters) => filters.iter().any(|f| f.matches(object)),
            MysObjectDataFilter::MatchNone(filters) => !filters.iter().any(|f| f.matches(object)),
            MysObjectDataFilter::StructType(s) => {
                ...
                // e.g. `0x2::coin::Coin` can match `0x2::coin::Coin<0x2::mys::MYS>`
            ...
            MysObjectDataFilter::MoveModule { package, module } => {
                matches!(&object.type_, ObjectType::Struct(s) if &ObjectID::from(s.address()) == package
                        && s.module() == module.as_ident_str())
            }
            MysObjectDataFilter::Package(p) => {
                matches!(&object.type_, ObjectType::Struct(s) if &ObjectID::from(s.address()) == p)
            }
            MysObjectDataFilter::AddressOwner(a) => {
                matches!(object.owner, Owner::AddressOwner(addr) if &addr == a)
            }
            MysObjectDataFilter::ObjectOwner(o) => {
                matches!(object.owner, Owner::ObjectOwner(addr) if addr == MysAddress::from(*o))
            }
            MysObjectDataFilter::ObjectId(id) => &object.object_id == id,
            MysObjectDataFilter::ObjectIds(ids) => ids.contains(&object.object_id),
            MysObjectDataFilter::Version(v) => object.version.value() == *v,
        }
    }
}
...
pub struct MysObjectResponseQuery {
    /// If None, no filter will be applied
    pub filter: Option<MysObjectDataFilter>,
    /// config which fields to include in the response, by default only digest is included
    pub options: Option<MysObjectDataOptions>,
}

impl MysObjectResponseQuery {
    pub fn new(filter: Option<MysObjectDataFilter>, options: Option<MysObjectDataOptions>) -> Self {
        Self { filter, options }
    }

    pub fn new_with_filter(filter: MysObjectDataFilter) -> Self {
        Self {
            filter: Some(filter),
            options: None,
        }
    ...
    pub fn new_with_options(options: MysObjectDataOptions) -> Self {
        Self {
            filter: None,
