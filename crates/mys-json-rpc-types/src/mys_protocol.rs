...
pub enum MysProtocolConfigValue {
    U16(
...
impl From<ProtocolConfigValue> for MysProtocolConfigValue {
    fn from(value: ProtocolConfigValue) -> Self {
        match value {
            ProtocolConfigValue::u16(y) => MysProtocolConfigValue::U16(y),
            ProtocolConfigValue::u32(y) => MysProtocolConfigValue::U32(y),
            ProtocolConfigValue::u64(x) => MysProtocolConfigValue::U64(x),
            ProtocolConfigValue::bool(z) => MysProtocolConfigValue::Bool(z),
        }
    }
}
...
pub struct ProtocolConfigResponse {
    ...
    pub attributes: BTreeMap<String, Option<MysProtocolConfigValue>>,
}
    ...
    fn from(config: ProtocolConfig) -> Self {
        ProtocolConfigResponse {
            ...
            attributes: config
                .attr_map()
                .into_iter()
                .map(|(k, v)| (k, v.map(MysProtocolConfigValue::from)))
                .collect(),
