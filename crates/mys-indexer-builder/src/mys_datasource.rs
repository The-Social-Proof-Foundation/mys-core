...
pub struct MysCheckpointDatasource {
...
#[async_trait]
impl Datasource<CheckpointTxnData> for MysCheckpointDatasource {
    async fn start_data_retrieval(
        ...
        tracing::info!(
            "Starting Mys checkpoint data retrieval with batch size {}",
...
impl Worker for IndexerWorker<CheckpointTxnData> {
    ...
    async fn process_checkpoint(&self, checkpoint: &MysCheckpointData) -> anyhow::Result<()> {
        tracing::trace!(
            "Received checkpoint [{}] {}: {}",
