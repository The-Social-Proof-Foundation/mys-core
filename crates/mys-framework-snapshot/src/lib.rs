...
use mys_types::{
    ...
    MYS_SYSTEM_PACKAGE_ID, MYS_SOCIAL_PACKAGE_ID, SEAL_PACKAGE_ID, USDC_PACKAGE_ID,
};
...
// [mys_framework::{SystemPackageMetadata, SystemPackage}] types,
// and also to the [mys::framework_versions::{FrameworkVersion, FrameworkPackage}] types.
// They are sort of a stepping stone from one to the other - the [mys_framework] types contain
...
//
// These types serve as a kind of stepping stone; they are constructed from the [mys_framework]
// types and serialized in the manifest, and then the build script for the [mys] crate reads them
// from the manifest file and encodes them in the `mys` binary. A little information is dropped in
// each of these steps.
...
pub struct SnapshotPackage {
    ...
    pub name: String,
    /// Path to the package in the monorepo (e.g. "crates/mys-framework/packages/move-stdlib")
    pub path: String,
