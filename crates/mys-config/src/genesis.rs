...
use mys_types::deny_list_v1::{get_coin_deny_list, PerTypeDenyList};
...
impl Genesis {
    ...
    pub fn validator_set_for_tooling(&self) -> Vec<MysValidatorGenesis> {
        self.mys_system_object()
    ...
    pub fn committee_with_network(&self) -> CommitteeWithNetworkMetadata {
        self.mys_system_object().get_current_epoch_committee()
    }

    pub fn reference_gas_price(&self) -> u64 {
        self.mys_system_object().reference_gas_price()
    }

    // TODO: No need to return MysResult. Also consider return &.
    pub fn committee(&self) -> MysResult<Committee> {
        Ok(self.committee_with_network().committee().clone())
    }

    pub fn mys_system_wrapper_object(&self) -> MysSystemStateWrapper {
        get_mys_system_state_wrapper(&self.objects())
            .expect("Mys System State Wrapper object must always exist")
    }

    pub fn mys_system_object(&self) -> MysSystemState {
        get_mys_system_state(&self.objects()).expect("Mys System State object must always exist")
    }
    ...
    pub fn clock(&self) -> Clock {
        let clock = self
            .objects()
            .iter()
            .find(|o| o.id() == mys_types::MYS_CLOCK_OBJECT_ID)
...
impl UnsignedGenesis {
    ...
    pub fn has_randomness_state_object(&self) -> bool {
        self.objects()
            .get_object(&MYS_RANDOMNESS_STATE_OBJECT_ID)
            .is_some()
    }

    pub fn has_bridge_object(&self) -> bool {
        self.objects().get_object(&MYS_BRIDGE_OBJECT_ID).is_some()
    }
    ...
    pub fn to_genesis_chain_parameters(&self) -> GenesisChainParameters {
        GenesisChainParameters {
            ...
            validator_very_low_stake_threshold:
                mys_types::governance::VALIDATOR_VERY_LOW_STAKE_THRESHOLD_MIST,
            validator_low_stake_grace_period:
                mys_types::governance::VALIDATOR_LOW_STAKE_GRACE_PERIOD,
        }
    }
}
...
impl TokenDistributionSchedule {
    ...
    pub fn check_all_stake_operations_are_for_valid_validators<
        I: IntoIterator<Item = MysAddress>,
    >(
        &self,
        validators: I,
    ) {
        ...
        let mut validators: HashMap<MysAddress, u64> =
        ...
        let minimum_required_stake = mys_types::governance::VALIDATOR_LOW_STAKE_THRESHOLD_MIST;
    ...
    pub fn new_for_validators_with_default_allocation<I: IntoIterator<Item = MysAddress>>(
        validators: I,
    ) -> Self {
...
impl TokenDistributionScheduleBuilder {
    ...
    pub fn default_allocation_for_validators<I: IntoIterator<Item = MysAddress>>(
        &mut self,
        validators: I,
    ) {
        let default_allocation = mys_types::governance::VALIDATOR_LOW_STAKE_THRESHOLD_MIST;
