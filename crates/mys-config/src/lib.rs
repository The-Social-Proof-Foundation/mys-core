...
pub const MYS_NETWORK_CONFIG: &str = "network.yaml";
pub const MYS_FULLNODE_CONFIG: &str = "fullnode.yaml";
pub const MYS_CLIENT_CONFIG: &str = "client.yaml";
pub const MYS_KEYSTORE_FILENAME: &str = "mys.keystore";
...
pub fn mys_config_dir() -> Result<PathBuf, anyhow::Error> {
    match std::env::var_os("MYS_CONFIG_DIR") {
        Some(config_env) => Ok(config_env.into()),
        None => match dirs::home_dir() {
            Some(v) => Ok(v.join(MYS_DIR).join(MYS_CONFIG_DIR)),
...
pub fn genesis_blob_exists(config_dir: Option<PathBuf>) -> bool {
    if let Some(dir) = config_dir {
        dir.join(MYS_GENESIS_FILENAME).exists()
    } else if let Some(config_env) = std::env::var_os("MYS_CONFIG_DIR") {
        Path::new(&config_env).join(MYS_GENESIS_FILENAME).exists()
    } else if let Some(home) = dirs::home_dir() {
        let mut config = PathBuf::new();
        config.push(&home);
        config.extend([MYS_DIR, MYS_CONFIG_DIR, MYS_GENESIS_FILENAME]);
        config.exists()
    } else {
        false
    }
