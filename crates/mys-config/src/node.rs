...
pub const DEFAULT_VALIDATOR_GAS_PRICE: u64 = mys_types::transaction::DEFAULT_VALIDATOR_GAS_PRICE;
...
pub struct NodeConfig {
    ...
    /// In a `mys-node` binary, this is set to SupportedProtocolVersions::SYSTEM_DEFAULT
    /// in mys-node/src/main.rs. It is present in the config so that it can be changed by tests in
...
impl ExecutionCacheConfig {
    pub fn max_cache_size(&self) -> u64 {
        std::env::var("MYS_MAX_CACHE_SIZE")
            .ok()
    ...
    pub fn package_cache_size(&self) -> u64 {
        std::env::var("MYS_PACKAGE_CACHE_SIZE")
            .ok()
            .and_then(|s| s.parse().ok())
            .unwrap_or_else(|| match self {
                ExecutionCacheConfig::PassthroughCache => fatal!("invalid cache config"),
    ...
    pub fn object_cache_size(&self) -> u64 {
        std::env::var("MYS_OBJECT_CACHE_SIZE")
            .ok()
            .and_then(|s| s.parse().ok())
            .unwrap_or_else(|| match self {
                ExecutionCacheConfig::PassthroughCache => fatal!("invalid cache config"),
    ...
    pub fn marker_cache_size(&self) -> u64 {
        std::env::var("MYS_MARKER_CACHE_SIZE")
            .ok()
            .and_then(|s| s.parse().ok())
            .unwrap_or_else(|| match self {
                ExecutionCacheConfig::PassthroughCache => fatal!("invalid cache config"),
    ...
    pub fn object_by_id_cache_size(&self) -> u64 {
        std::env::var("MYS_OBJECT_BY_ID_CACHE_SIZE")
            .ok()
            .and_then(|s| s.parse().ok())
            .unwrap_or_else(|| match self {
                ExecutionCacheConfig::PassthroughCache => fatal!("invalid cache config"),
    ...
    pub fn transaction_cache_size(&self) -> u64 {
        std::env::var("MYS_TRANSACTION_CACHE_SIZE")
            .ok()
            .and_then(|s| s.parse().ok())
            .unwrap_or_else(|| match self {
                ExecutionCacheConfig::PassthroughCache => fatal!("invalid cache config"),
    ...
    pub fn executed_effect_cache_size(&self) -> u64 {
        std::env::var("MYS_EXECUTED_EFFECT_CACHE_SIZE")
            .ok()
            .and_then(|s| s.parse().ok())
            .unwrap_or_else(|| match self {
                ExecutionCacheConfig::PassthroughCache => fatal!("invalid cache config"),
    ...
    pub fn effect_cache_size(&self) -> u64 {
        std::env::var("MYS_EFFECT_CACHE_SIZE")
            .ok()
            .and_then(|s| s.parse().ok())
            .unwrap_or_else(|| match self {
                ExecutionCacheConfig::PassthroughCache => fatal!("invalid cache config"),
    ...
    pub fn events_cache_size(&self) -> u64 {
        std::env::var("MYS_EVENTS_CACHE_SIZE")
            .ok()
            .and_then(|s| s.parse().ok())
            .unwrap_or_else(|| match self {
                ExecutionCacheConfig::PassthroughCache => fatal!("invalid cache config"),
    ...
    pub fn transaction_objects_cache_size(&self) -> u64 {
        std::env::var("MYS_TRANSACTION_OBJECTS_CACHE_SIZE")
            .ok()
            .and_then(|s| s.parse().ok())
            .unwrap_or_else(|| match self {
                ExecutionCacheConfig::PassthroughCache => fatal!("invalid cache config"),
    ...
    pub fn backpressure_threshold(&self) -> u64 {
        std::env::var("MYS_BACKPRESSURE_THRESHOLD")
            .ok()
            .and_then(|s| s.parse().ok())
            .unwrap_or_else(|| match self {
                ExecutionCacheConfig::PassthroughCache => fatal!("invalid cache config"),
    ...
    pub fn backpressure_threshold_for_rpc(&self) -> u64 {
        std::env::var("MYS_BACKPRESSURE_THRESHOLD_FOR_RPC")
            .ok()
            .and_then(|s| s.parse().ok())
            .unwrap_or_else(|| match self {
                ExecutionCacheConfig::PassthroughCache => fatal!("invalid cache config"),
...
impl NodeConfig {
    ...
    pub fn mys_address(&self) -> MysAddress {
...
pub struct ExpensiveSafetyCheckConfig {
    ...
    /// rebate of all live objects, and that the total MYS in the network remains
    /// the same.
    #[serde(default)]
    enable_epoch_mys_conservation_check: bool,

    /// If enabled, we will check that the total MYS in all input objects of a tx
    /// (both the Move part and the storage rebate) matches the total MYS in all
    /// output objects of the tx + gas fees
    #[serde(default)]
    enable_deep_per_tx_mys_conservation_check: bool,

    /// Disable epoch MYS conservation check even when we are running in debug mode.
    #[serde(default)]
    force_disable_epoch_mys_conservation_check: bool,
...
impl ExpensiveSafetyCheckConfig {
    pub fn new_enable_all() -> Self {
        Self {
            enable_epoch_mys_conservation_check: true,
            enable_deep_per_tx_mys_conservation_check: true,
            force_disable_epoch_mys_conservation_check: false,
    ...
    pub fn new_disable_all() -> Self {
        Self {
            enable_epoch_mys_conservation_check: false,
            enable_deep_per_tx_mys_conservation_check: false,
            force_disable_epoch_mys_conservation_check: true,
    ...
    pub fn force_disable_epoch_mys_conservation_check(&mut self) {
        self.force_disable_epoch_mys_conservation_check = true;
    }

    pub fn enable_epoch_mys_conservation_check(&self) -> bool {
        (self.enable_epoch_mys_conservation_check || cfg!(debug_assertions))
            && !self.force_disable_epoch_mys_conservation_check
    }
    ...
    pub fn enable_deep_per_tx_mys_conservation_check(&self) -> bool {
        self.enable_deep_per_tx_mys_conservation_check || cfg!(debug_assertions)
    }
...
/// Wrapper struct for MysKeyPair that can be deserialized from a file path. Used by network, worker, and account keypair.
...
pub struct KeyPairWithPath {
    ...
    #[serde(skip)]
    keypair: OnceCell<Arc<MysKeyPair>>,
}
...
impl KeyPairWithPath {
    pub fn new(kp: MysKeyPair) -> Self {
        let cell: OnceCell<Arc<MysKeyPair>> = OnceCell::new();
    ...
    pub fn keypair(&self) -> &MysKeyPair {
        self.keypair
...
mod tests {
    ...
    fn legacy_validator_config() {
        const FILE: &str = include_str!("../data/mys-node-legacy.yaml");
    ...
    fn load_key_pairs_to_node_config() {
        ...
        write_keypair_to_file(
            &MysKeyPair::Ed25519(worker_key_pair.copy()),
            PathBuf::from("worker.key"),
        )
        .unwrap();
        write_keypair_to_file(
            &MysKeyPair::Ed25519(network_key_pair.copy()),
