...
pub struct TransactionDenyConfig {
    ...
    /// A list of mys addresses that are not allowed to be used as the sender or sponsor.
    #[serde(default, skip_serializing_if = "Vec::is_empty")]
    address_deny_list: Vec<MysAddress>,
    ...
    #[serde(skip)]
    address_deny_set: OnceCell<HashSet<MysAddress>>,
...
impl TransactionDenyConfig {
    ...
    pub fn get_address_deny_set(&self) -> &HashSet<MysAddress> {
...
impl TransactionDenyConfigBuilder {
    ...
    pub fn add_denied_address(mut self, address: MysAddress) -> Self {
