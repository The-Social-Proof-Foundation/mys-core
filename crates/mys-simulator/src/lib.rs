...
// Re-export things used by mys-macros
...
pub mod configs {
    ...
    /// Select from among a number of configs using the MYS_SIM_CONFIG env var.
    pub fn env_config(
        // Config to use when MYS_SIM_CONFIG is not set.
        default: SimConfig,
        // List of (&str, SimConfig) pairs - the SimConfig associated with the value
        // of the MYS_SIM_CONFIG var is chosen.
        ...
        if let Some(env) = std::env::var("MYS_SIM_CONFIG").ok() {
            if let Some(cfg) = env_configs.remove(env.as_str()) {
                info!("Using test config for MYS_SIM_CONFIG={}", env);
                cfg
            } else {
                panic!(
                    "No config found for MYS_SIM_CONFIG={}. Available configs are: {:?}",
                    env,
                    env_configs.keys()
                );
            }
