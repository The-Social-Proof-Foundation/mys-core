...
async fn main() -> Result<(), anyhow::Error> {
    ...
    // use `None` for the default `Coin<MYS>` which is represented as
    // "0x2::mys::MYS"
    let coin_type = Some("0x2::mys::MYS".to_string());
    let coins = mys
        .coin_read_api()
    ...
    let coins_stream = mys.coin_read_api().get_coins_stream(active_address, None);
    ...
    // Select coins based on the provided coin type (MYS in this example). Use `None` for the default Mys coin
    let select_coins = mys
        .coin_read_api()
    ...
    // or if None is passed, it will use Coin<MYS> as the coin type
    let balance = mys
        .coin_read_api()
    ...
    let total_balance = mys.coin_read_api().get_all_balances(active_address).await?;
    println!(" *** Balance + Total Balance *** ");
    println!("Balance: {:?}", balance);
    println!("Total Balance: {:?}", total_balance);
    println!(" *** Balance + Total Balance ***\n ");

    // Return the coin metadata for the Coin<MYS>
