...
use mys_config::{
    mys_config_dir, Config, PersistedConfig, MYS_CLIENT_CONFIG, MYS_KEYSTORE_FILENAME,
};
use mys_json_rpc_types::{Coin, MysObjectDataOptions};
...
/// Return a mys client to interact with the APIs,
...
pub async fn setup_for_write() -> Result<(MysClient, MysAddress, MysAddress), anyhow::Error> {
    let (client, active_address) = setup_for_read().await?;
    // make sure we have some MYS (5_000_000 MIST) on this address
...
/// Return a mys client to interact with the APIs and an active address from the local wallet.
///
/// This function sets up a wallet in case there is no wallet locally,
/// and ensures that the active address of the wallet has MYS on it.
/// If there is no MYS owned by the active address, then it will request
/// MYS from the faucet.
pub async fn setup_for_read() -> Result<(MysClient, MysAddress), anyhow::Error> {
    let client = MysClientBuilder::default().build_testnet().await?;
    println!("Mys testnet version is: {}", client.api_version());
...
pub async fn request_tokens_from_faucet(
    ...
    loop {
        let owner = mys_client
            .read_api()
            .get_object_with_options(
                ObjectID::from_str(&coin_id)?,
                MysObjectDataOptions::new().with_owner(),
            )
            .await?;
...
pub async fn split_coin_digest(
    ...
    let coin = match fetch_coin(mys, sender).await? {
        None => {
            request_tokens_from_faucet(*sender, mys).await?;
            fetch_coin(mys, sender)
                .await?
                .expect("Supposed to get a coin with MYS, but didn't. Aborting")
        }
        Some(c) => c,
    };
    ...
    let gas_price = mys.read_api().get_reference_gas_price().await?;

    // now we programmatically build the transaction through several commands
    let mut ptb = ProgrammableTransactionBuilder::new();
    // first, we want to split the coin, and we specify how much MYS (in MIST) we want
    // for the new coin
    ...
    let keystore = FileBasedKeystore::new(&mys_config_dir()?.join(MYS_KEYSTORE_FILENAME))?;
    let signature = keystore.sign_secure(sender, &tx_data, Intent::mys_transaction())?;

    let transaction_response = mys
        .quorum_driver_api()
...
pub fn retrieve_wallet() -> Result<WalletContext, anyhow::Error> {
    ...
    // check if a wallet exists and if not, create a wallet and a mys client config
    ...
    if !wallet_conf.exists() {
        let keystore = FileBasedKeystore::new(&keystore_path)?;
        let mut client_config = MysClientConfig::new(keystore.into());

        client_config.add_env(MysEnv::testnet());
        client_config.add_env(MysEnv::devnet());
        client_config.add_env(MysEnv::localnet());
    ...
    let mut client_config: MysClientConfig = PersistedConfig::read(&wallet_conf)?;
...
async fn just_for_clippy() -> Result<(), anyhow::Error> {
    let (mys, sender, _recipient) = setup_for_write().await?;
    let _digest = split_coin_digest(&mys, &sender).await?;
    Ok(())
}
