...
pub struct WalletContext {
    pub config: PersistedConfig<MysClientConfig>,
    request_timeout: Option<std::time::Duration>,
    client: Arc<RwLock<Option<MysClient>>>,
...
impl WalletContext {
    pub fn new(
        ...
        let config: MysClientConfig = PersistedConfig::read(config_path).map_err(|err| {
            anyhow!(
    ...
    pub async fn get_client(&self) -> Result<MysClient, anyhow::Error> {
    ...
    // TODO: Ger rid of mut
    pub fn active_address(&mut self) -> Result<MysAddress, anyhow::Error> {
    ...
    pub async fn gas_objects(
        &self,
        address: MysAddress,
    ) -> Result<Vec<(u64, MysObjectData)>, anyhow::Error> {
        let client = self.get_client().await?;
            ...
            let response = client
                ...
                .get_owned_objects(
                    ...
                    Some(MysObjectResponseQuery::new(
                        ...
                        Some(MysObjectDataOptions::full_content()),
                    )),
                    cursor,
                    None,
                )
    ...
    pub async fn get_object_owner(&self, id: &ObjectID) -> Result<MysAddress, anyhow::Error> {
        let client = self.get_client().await?;
        let object = client
            .read_api()
    ...
    pub async fn try_get_object_owner(
        &self,
        id: &Option<ObjectID>,
    ) -> Result<Option<MysAddress>, anyhow::Error> {
        if let Some(id) = id {
    ...
    pub async fn gas_for_owner_budget(
    ...
    ) -> Result<(u64, MysObjectData), anyhow::Error> {
        ...
        Err(anyhow!(
            "No non-argument gas objects found for this address with value >= budget {budget}. Run mys client gas to check for gas objects."
        ))
    }
    ...
    pub async fn get_one_account(&self) -> anyhow::Result<(MysAddress, Vec<ObjectRef>)> {
    ...
    pub async fn get_one_gas_object(&self) -> anyhow::Result<Option<(MysAddress, ObjectRef)>> {
    ...
    pub async fn get_all_accounts_and_gas_objects(
        &self,
    ) -> anyhow::Result<Vec<(MysAddress, Vec<ObjectRef>)>> {
        let mut result = vec![];
        for address in self.get_addresses() {
            let objects = self
                .gas_objects(address)
    ...
    /// Add an account
    pub fn add_account(&mut self, alias: Option<String>, keypair: MysKeyPair) {
