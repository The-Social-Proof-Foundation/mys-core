...
impl ReadApi {
    ...
    ///     let mys = MysClientBuilder::default().build_localnet().await?;
    ...
    pub async fn get_owned_objects(
        &self,
        address: MysAddress,
        query: Option<MysObjectResponseQuery>,
        cursor: Option<ObjectID>,
        limit: Option<usize>,
    ) -> MysRpcResult<ObjectsPage> {
        Ok(self
            .api
            .http
    ...
    ///         "No object data for this MysObjectResponse {:?}",
    ///         object
    ///     ));
    ///     let object_id = object_data.object_id;
    ///     let dynamic_fields = mys
    ///         .read_api()
    ...
    ) -> MysRpcResult<MysObjectResponse> {
        Ok(self
            .api
            .http
    ...
    ) -> MysRpcResult<MysPastObjectResponse> {
        Ok(self
            .api
            .http
    ...
    /// Return a list of [MysPastObjectResponse] objects, or an error upon failure.
    ///
    ...
    /// use mys_sdk::MysClientBuilder;
    /// use mys_types::base_types::{ObjectID, MysAddress};
    /// use mys_json_rpc_types::{MysObjectDataOptions, MysGetPastObjectRequest};
    /// use std::str::FromStr;
    ///
    /// #[tokio::main]
    /// async fn main() -> Result<(), anyhow::Error> {
    ...
    ///     let multi_past_object = mys
    ///         .read_api()
    ///         .try_multi_get_parsed_past_object(
    ///             vec![MysGetPastObjectRequest {
    ...
    pub async fn try_multi_get_parsed_past_object(
        ...
        options: MysObjectDataOptions,
    ) -> MysRpcResult<Vec<MysPastObjectResponse>> {
        Ok(self
            .api
            .http
    ...
    /// Return a [MysObjectResponse] based on the provided [ObjectID] and [MysObjectDataOptions], or an error upon failure.
    ///
    /// The [MysObjectResponse] contains two fields:
    /// 1) `data` for the object's data (see [MysObjectData](mys_json_rpc_types::MysObjectData)),
    /// 2) `error` for the error (if any) (see [MysObjectResponseError](mys_types::error::MysObjectResponseError)).
    ///
    /// # Examples
    ///
    /// ```rust,no_run
    ...
    ///     ));
    ///     let object_id = object_data.object_id;
    ///     let object = mys.read_api().get_object_with_options(object_id,
    ///             MysObjectDataOptions {
    ///                 show_type: true,
    ///                 show_owner: true,
    ///                 show_previous_transaction: true,
    ...
    /// Return a list of [MysObjectResponse] from the given vector of [ObjectID]s and [MysObjectDataOptions], or an error upon failure.
    ///
    ...
    ///     let object = mys.read_api().multi_get_object_with_options(object_ids,
    ///             MysObjectDataOptions {
    ///                 show_type: true,
    ///                 show_owner: true,
    ///                 show_previous_transaction: true,
    ...
    pub async fn multi_get_object_with_options(
        ...
        options: MysObjectDataOptions,
    ) -> MysRpcResult<Vec<MysObjectResponse>> {
        Ok(self
            .api
            .http
    ...
    pub async fn get_move_object_bcs(&self, object_id: ObjectID) -> MysRpcResult<Vec<u8>> {
        let resp = self
            .get_object_with_options(object_id, MysObjectDataOptions::default().with_bcs())
            .await?
            .into_object()
            .map_err(|e| {
    ...
    /// async fn main() -> Result<(), anyhow::Error> {
    ///     let mys = MysClientBuilder::default().build_localnet().await?;
    ///     let total_transaction_blocks = mys
    ///         .read_api()
    ...
    /// }
    /// ```
    pub async fn get_total_transaction_blocks(&self) -> MysRpcResult<u64> {
        Ok(*self.api.http.get_total_transaction_blocks().await?)
    }

    /// Return a transaction and its effects in a [MysTransactionBlockResponse] based on its
    ...
    ) -> MysRpcResult<MysTransactionBlockResponse> {
        Ok(self
            .api
            .http
    ...
    /// Return a list of [MysTransactionBlockResponse] based on the given vector of [TransactionDigest], or an error upon failure.
    ///
    ...
    pub async fn multi_get_transactions_with_options(
        ...
        options: MysTransactionBlockResponseOptions,
    ) -> MysRpcResult<Vec<MysTransactionBlockResponse>> {
        Ok(self
            .api
            .http
    ...
    /// Return the [MysCommittee] information for the provided `epoch`, or an error upon failure.
    ///
    /// The [MysCommittee] contains the validators list and their information (name and stakes).
    ///
    ...
    pub async fn query_transaction_blocks(
        &self,
        query: MysTransactionBlockResponseQuery,
        cursor: Option<TransactionDigest>,
        limit: Option<usize>,
        descending_order: bool,
    ) -> MysRpcResult<TransactionBlocksPage> {
        Ok(self
            .api
            .http
    ...
    pub async fn get_chain_identifier(&self) -> MysRpcResult<String> {
        Ok(self.api.http.get_chain_identifier().await?)
    }

    /// Return a checkpoint, or an error upon failure.
    ///
    /// A Mys checkpoint is a sequence of transaction sets that a quorum of validators
    /// agree upon as having been executed within the Mys system.
    pub async fn get_checkpoint(&self, id: CheckpointId) -> MysRpcResult<Checkpoint> {
    ...
    pub async fn get_latest_checkpoint_sequence_number(
        &self,
    ) -> MysRpcResult<CheckpointSequenceNumber> {
        Ok(*self
            .api
            .http
    ...
    /// Return a stream of [MysTransactionBlockResponse], or an error upon failure.
    pub fn get_transactions_stream(
        &self,
        query: MysTransactionBlockResponseQuery,
        cursor: Option<TransactionDigest>,
        descending_order: bool,
    ) -> impl Stream<Item = MysTransactionBlockResponse> + '_ {
        stream::unfold(
    ...
    ) -> MysRpcResult<impl Stream<Item = MysRpcResult<MysTransactionBlockEffects>>> {
        let Some(c) = &self.api.ws else {
            return Err(Error::Subscription(
                "Subscription only supported by WebSocket client.".to_string(),
            ));
        };
        let subscription: Subscription<MysTransactionBlockEffects> =
    ...
    pub async fn get_normalized_move_modules_by_package(
        &self,
        package: ObjectID,
    ) -> MysRpcResult<BTreeMap<String, MysMoveNormalizedModule>> {
        Ok(self
            .api
            .http
    ...
    pub async fn get_reference_gas_price(&self) -> MysRpcResult<u64> {
    ...
    pub async fn dry_run_transaction_block(
        &self,
        tx: TransactionData,
    ) -> MysRpcResult<DryRunTransactionBlockResponse> {
        Ok(self
            .api
            .http
    ...
    ) -> MysRpcResult<DevInspectResults> {
        Ok(self
            .api
            .http
    ...
    ) -> MysRpcResult<ProtocolConfigResponse> {
...
impl CoinReadApi {
    ...
    /// or use `None` for the default `Coin<MYS>`.
    ///
    /// # Examples
    ///
    /// ```rust,no_run
    ...
    ///
    /// The coins can be filtered by `coin_type` (e.g., 0x168da5bf1f48dafc111b0a488fa454aca95e0b5e::usdc::USDC)
    /// or use `None` to use the default `Coin<MYS>`.
    ///
    /// # Examples
    ///
    /// ```rust,no_run
    ...
    /// it will use the default `Coin<MYS>`.
    ///
    /// # Examples
    ///
    /// ```rust,no_run
    ...
    /// }
    /// ```
    pub async fn get_all_balances(&self, owner: MysAddress) -> MysRpcResult<Vec<Balance>> {
    ...
    /// async fn main() -> Result<(), anyhow::Error> {
    ///     let mys = MysClientBuilder::default().build_localnet().await?;
    ///     let coin_metadata = mys
    ///         .coin_read_api()
    ///         .get_coin_metadata("0x2::mys::MYS".to_string())
    ///         .await?;
    ///     Ok(())
    /// }
    /// ```
    pub async fn get_coin_metadata(
        &self,
        coin_type: String,
    ) -> MysRpcResult<Option<MysCoinMetadata>> {
    ...
    /// async fn main() -> Result<(), anyhow::Error> {
    ///     let mys = MysClientBuilder::default().build_localnet().await?;
    ///     let total_supply = mys
    ///         .coin_read_api()
    ///         .get_total_supply("0x2::mys::MYS".to_string())
    ///         .await?;
    ///     Ok(())
    /// }
    /// ```
    pub async fn get_total_supply(&self, coin_type: String) -> MysRpcResult<Supply> {
        Ok(self.api.http.get_total_supply(coin_type).await?)
    }
}

/// Event API provides the functionality to fetch, query, or subscribe to events on the Mys network.
#[derive(Clone)]
pub struct EventApi {
    api: Arc<RpcClient>,
}
...
impl EventApi {
    ...
    pub async fn subscribe_event(
        &self,
        filter: EventFilter,
    ) -> MysRpcResult<impl Stream<Item = MysRpcResult<MysEvent>>> {
        match &self.api.ws {
            Some(c) => {
                let subscription: Subscription<MysEvent> = c.subscribe_event(filter).await?;
                Ok(subscription.map(|item| Ok(item?)))
            }
    ...
    pub async fn get_events(&self, digest: TransactionDigest) -> MysRpcResult<Vec<MysEvent>> {
...
impl GovernanceApi {
    ...
    pub async fn get_stakes(&self, owner: MysAddress) -> MysRpcResult<Vec<DelegatedStake>> {
        Ok(self.api.http.get_stakes(owner).await?)
    }

    /// Return the [MysCommittee] information for the given `epoch`, or an error upon failure.
    ///
    ...
    /// Return the latest MYS system state object on-chain, or an error upon failure.
    ///
    ...
    /// and much more. See the [MysSystemStateSummary] for all the available fields.
    pub async fn get_latest_mys_system_state(&self) -> MysRpcResult<MysSystemStateSummary> {
        Ok(self.api.http.get_latest_mys_system_state().await?)
    }
