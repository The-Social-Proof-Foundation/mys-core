...
use mys_indexer_alt_schema::schema::watermarks;
use mys_name_service::Domain;
use mys_name_service::NameRecord;
use mys_name_service::NameServiceError;
...
/// Attempt to translate the given MysNS `name` to its address, as long as the mapping exists,
/// and it hasn't expired.
pub(super) async fn resolved_address(
    ctx: &Context,
    name: &str,
) -> Result<Option<MysAddress>, RpcError<Error>> {
    use Error as E;
...
/// Attempt to translate the given `address` to its MysNS name, as long as the reverse mapping
...
pub(super) async fn resolved_name(
    ...
    let reverse_record: Field<MysAddress, Domain> = bcs::from_bytes(
