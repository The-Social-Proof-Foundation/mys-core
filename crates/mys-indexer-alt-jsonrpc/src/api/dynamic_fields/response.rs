...
use anyhow::anyhow;
use mys_json::MoveTypeLayout;
use mys_json::MysJsonValue;
use mys_json_rpc_types::BcsName;
use mys_json_rpc_types::DynamicFieldInfo as DynamicFieldInfoResponse;
...
pub(super) async fn dynamic_field_object(
...
) -> Result<MysObjectResponse, RpcError<Error>> {
    let layout = resolve_type(ctx, &name.type_).await?;
    let bytes = MysJsonValue::new(name.value)
    ...
    let options = MysObjectDataOptions::full_content();
...
/// `mys::dynamic_field::Field<K, V>`, and extract the name and value from it.
