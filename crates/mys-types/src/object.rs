...
impl MoveObject {
    ...
    pub fn get_layout(&self, resolver: &impl GetModule) -> Result<MoveStructLayout, MysError> {
    ...
    pub fn to_move_struct(&self, layout: &MoveStructLayout) -> Result<MoveStruct, MysError> {
    ...
    /// Get the total amount of MYS embedded in `self`. Intended for testing purposes
    pub fn get_total_mys(&self, layout_resolver: &mut dyn LayoutResolver) -> Result<u64, MysError> {
...
impl MoveObject {
    ...
    ) -> Result<BTreeMap<TypeTag, u64>, MysError> {
...
pub enum Owner {
    ...
    /// The object ID is converted to MysAddress as MysAddress is universal.
    ObjectOwner(MysAddress),
...
impl Owner {
    ...
    pub fn get_address_owner_address(&self) -> MysResult<MysAddress> {
        match self {
...
impl PartialEq<ObjectID> for Owner {
    fn eq(&self, other: &ObjectID) -> bool {
        let other_id: MysAddress = (*other).into();
        match self {
...
impl ObjectInner {
    ...
    pub fn get_single_owner(&self) -> Option<MysAddress> {
    ...
    // context: https://github.com/MystenLabs/mys/pull/10679#discussion_r1165877816
    ...
    pub fn transfer(&mut self, new_owner: MysAddress) {
    ...
    pub fn get_layout(
        &self,
        resolver: &impl GetModule,
    ) -> Result<Option<MoveStructLayout>, MysError> {
        match &self.data {
    ...
    pub fn get_move_template_type(&self) -> MysResult<TypeTag> {
        let move_struct = self.data.struct_tag().ok_or_else(|| MysError::TypeError {
...
impl Object {
    /// Get the total amount of MYS embedded in `self`, including both Move objects and the storage rebate
    pub fn get_total_mys(&self, layout_resolver: &mut dyn LayoutResolver) -> Result<u64, MysError> {
        Ok(self.storage_rebate
            + match &self.data {
                Data::Move(m) => m.get_total_mys(layout_resolver)?,
                Data::Package(_) => 0,
            })
    }
    ...
    pub fn with_id_owner_gas_for_testing(id: ObjectID, owner: MysAddress, gas: u64) -> Self {
        let data = Data::Move(MoveObject {
            type_: GasCoin::type_().into(),
            has_public_transfer: true,
            version: OBJECT_START_VERSION,
    ...
    pub fn with_id_owner_for_testing(id: ObjectID, owner: MysAddress) -> Self {
    ...
    pub fn with_owner_for_testing(owner: MysAddress) -> Self {
    ...
    pub fn new_gas_with_balance_and_owner_for_testing(value: u64, owner: MysAddress) -> Self {
