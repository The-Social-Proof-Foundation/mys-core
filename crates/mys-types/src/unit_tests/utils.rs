...
pub fn make_transaction_data(sender: MysAddress) -> TransactionData {
    let object = Object::immutable_with_id_for_testing(ObjectID::random_from_rng(
        &mut StdRng::from_seed([0; 32]),
    ));
    let pt = {
        let mut builder = ProgrammableTransactionBuilder::new();
        builder.transfer_mys(dbg_addr(2), None);
        builder.finish()
    };
...
pub fn make_transaction(sender: MysAddress, kp: &MysKeyPair) -> Transaction {
...
mod zk_login {
    ...
    pub fn load_test_vectors(path: &str) -> Vec<(MysKeyPair, PublicKey, ZkLoginInputs)> {
        ...
        for test in test_datum {
            let kp = MysKeyPair::decode(&test.kp).unwrap();
            let inputs =
    ...
    pub fn get_zklogin_user_address() -> MysAddress {
        thread_local! {
            static USER_ADDRESS: MysAddress = {
                ...
                MysAddress::from_bytes(hasher.finalize().digest).unwrap()
            };
        }
        USER_ADDRESS.with(|a| *a)
    }

    fn get_zklogin_user_key() -> MysKeyPair {
    ...
    pub fn get_legacy_zklogin_user_address() -> MysAddress {
        thread_local! {
            static USER_ADDRESS: MysAddress = {
                let inputs = get_inputs_with_bad_address_seed();
                MysAddress::from(&PublicKey::from_zklogin_inputs(&inputs).unwrap())
            };
        }
        USER_ADDRESS.with(|a| *a)
    }

    pub fn sign_zklogin_personal_msg(data: PersonalMessage) -> (MysAddress, GenericSignature) {
        let inputs = get_zklogin_inputs();
    ...
    pub fn sign_zklogin_tx_with_default_proof(
        data: TransactionData,
        legacy: bool,
    ) -> (MysAddress, Transaction, GenericSignature) {
    ...
    pub fn keys() -> Vec<MysKeyPair> {
        let mut seed = StdRng::from_seed([0; 32]);
        let kp1: MysKeyPair = MysKeyPair::Ed25519(get_key_pair_from_rng(&mut seed).1);
        let kp2: MysKeyPair = MysKeyPair::Secp256k1(get_key_pair_from_rng(&mut seed).1);
        let kp3: MysKeyPair = MysKeyPair::Secp256r1(get_key_pair_from_rng(&mut seed).1);
        vec![kp1, kp2, kp3]
    }
    ...
    pub fn make_upgraded_multisig_tx() -> Transaction {
        ...
        .unwrap();
        let addr = MysAddress::from(&multisig_pk);
        let tx = make_transaction(addr, &keys[0]);

        let msg = IntentMessage::new(Intent::mys_transaction(), tx.transaction_data().clone());
