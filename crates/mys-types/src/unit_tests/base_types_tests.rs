...
use crate::crypto::{
    AccountKeyPair, AuthorityKeyPair, AuthoritySignature, Signature, MysAuthoritySignature,
    MysSignature, get_key_pair, get_key_pair_from_bytes,
};
use crate::digests::Digest;
use crate::id::{ID, UID};
use crate::{MYS_FRAMEWORK_ADDRESS, gas_coin::GasCoin, object::Object};
...
#[test]
fn test_bcs_enum() {
    let address = Owner::AddressOwner(MysAddress::random_for_testing_only());
...
fn test_signatures() {
    ...
    let foo = IntentMessage::new(Intent::mys_transaction(), Foo("hello".into()));
    let foox = IntentMessage::new(Intent::mys_transaction(), Foo("hellox".into()));
    let bar = IntentMessage::new(Intent::mys_transaction(), Bar("hello".into()));
...
fn test_signatures_serde() {
    ...
    let s = Signature::new_secure(&IntentMessage::new(Intent::mys_transaction(), foo), &sec1);
...
fn test_object_id_zero_padding() {
    ...
    assert_eq!(MYS_FRAMEWORK_ADDRESS, obj_id_1.0);
    assert_eq!(MYS_FRAMEWORK_ADDRESS, obj_id_2.0);
    assert_eq!(MYS_FRAMEWORK_ADDRESS, obj_id_3.0);
    assert_eq!(MYS_FRAMEWORK_ADDRESS, obj_id_4.0);
    assert_eq!(MYS_FRAMEWORK_ADDRESS, obj_id_5.0);
    assert_eq!(MYS_FRAMEWORK_ADDRESS, obj_id_6.0);
}

#[test]
fn test_address_display() {
    let hex = SAMPLE_ADDRESS;
    let id = MysAddress::from_str(hex).unwrap();
    assert_eq!(format!("{:?}", id), format!("0x{hex}"));
}
...
fn test_address_serde_not_human_readable() {
    ...
    let deserialized: MysAddress = bincode::deserialize(&serialized).unwrap();
...
fn test_address_serde_human_readable() {
    ...
    let deserialized: MysAddress = serde_json::from_str(&serialized).unwrap();
    assert_eq!(deserialized, address);
}

#[test]
fn test_address_serde_with_expected_value() {
    let address = MysAddress::try_from(SAMPLE_ADDRESS_VEC.to_vec()).unwrap();
...
fn test_authority_signature_serde_not_human_readable() {
    ...
    let sig = AuthoritySignature::new_secure(
        &IntentMessage::new(Intent::mys_transaction(), Foo("some data".to_string())),
        &0,
        &key,
    );
...
fn derive_sample_address() -> (MysAddress, AccountKeyPair) {
...
fn move_object_type_consistency() {
    ...
    let ty = assert_consistent(&StakedMys::type_());
    assert!(ty.is_staked_mys());
