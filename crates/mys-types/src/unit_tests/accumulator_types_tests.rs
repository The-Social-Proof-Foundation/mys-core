...
use crate::base_types::{MoveObjectType, SequenceNumber, MysAddress};
use crate::dynamic_field::{DynamicFieldInfo, DynamicFieldKey};
use crate::gas_coin::GAS;
use crate::object::MoveObject;
use crate::{
    MoveTypeTagTrait, MoveTypeTagTraitGeneric, MYS_ACCUMULATOR_ROOT_OBJECT_ID,
    MYS_FRAMEWORK_ADDRESS,
};
use move_core_types::language_storage::{StructTag, TypeTag};

#[test]
fn test_mys_balance_accumulator_field_recognition() {
    // Create a Field<Key<Balance<MYS>>, U128> type
    let mys_balance = Balance::type_tag(GAS::type_tag());
    let key_type = AccumulatorKey::get_type_tag(&[mys_balance]);
    ...
    assert!(move_type.is_mys_balance_accumulator_field());
...
#[test]
fn test_non_mys_balance_accumulator_field_recognition() {
    ...
    // Should be recognized as a balance accumulator field, but not MYS
    assert!(move_type.is_balance_accumulator_field());
    assert!(!move_type.is_mys_balance_accumulator_field());
...
fn test_accumulator_storage_savings() {
    ...
    // Create the field struct tag for Field<Key<Balance<MYS>>, U128>
    let mys_balance = Balance::type_tag(GAS::type_tag());
    let key_type = AccumulatorKey::get_type_tag(&[mys_balance]);
    let u128_type = U128::get_type_tag();
    ...
    let field_key = DynamicFieldKey(
        MYS_ACCUMULATOR_ROOT_OBJECT_ID,
        key,
    ...
    assert!(
        efficient_move_object
            .type_()
            .is_mys_balance_accumulator_field()
    );
}
...
fn test_accumulator_field_wrong_value_type_not_u128() {
    // Field<Key<Balance<MYS>>, SomeOtherType> should NOT be recognized
    let mys_balance = Balance::type_tag(GAS::type_tag());
    let key_type = AccumulatorKey::get_type_tag(&[mys_balance]);
...
fn test_accumulator_field_key_with_multiple_type_params() {
    // Key<Balance<MYS>, ExtraParam> should not be recognized
    let mys_balance = Balance::type_tag(GAS::type_tag());
    ...
    let key_type = TypeTag::Struct(Box::new(StructTag {
        ...
        module: "accumulator".parse().unwrap(),
        name: "Key".parse().unwrap(),
        type_params: vec![mys_balance, extra_param],
    }));
    let u128_type = U128::get_type_tag();
...
fn test_accumulator_field_with_nested_balance_type() {
    // Balance<Balance<MYS>> - nested balance should still work correctly
    ...
    // Should recognize as accumulator field with Balance<Balance<MYS>> as the extracted type
    assert!(move_type.is_balance_accumulator_field());
    let extracted = move_type.balance_accumulator_field_type_maybe().unwrap();
    // The extracted type should be Balance<MYS>, which is the inner type of Balance<Balance<MYS>>
...
#[test]
fn test_is_method_mys_balance_accumulator_field() {
    let mys_balance = Balance::type_tag(GAS::type_tag());
    let key_type = AccumulatorKey::get_type_tag(&[mys_balance]);
    let u128_type = U128::get_type_tag();
    let field_type = DynamicFieldInfo::dynamic_field_type(key_type, u128_type);
...
#[test]
fn test_is_method_non_mys_balance_accumulator_field() {
    let custom_token = TypeTag::Struct(Box::new(StructTag {
        address: MYS_FRAMEWORK_ADDRESS,
        module: "test".parse().unwrap(),
        name: "CustomToken".parse().unwrap(),
    ...
    // Should not match MYS accumulator field
    let mys_balance = Balance::type_tag(GAS::type_tag());
    let mys_key_type = AccumulatorKey::get_type_tag(&[mys_balance]);
    let mys_field_type = DynamicFieldInfo::dynamic_field_type(mys_key_type, U128::get_type_tag());
    assert!(!move_type.is(&mys_field_type));
