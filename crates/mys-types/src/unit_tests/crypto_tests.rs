...
#[test]
fn serde_keypair() {
    let skp = MysKeyPair::Ed25519(Ed25519KeyPair::generate(&mut StdRng::from_seed([0; 32])));
    let encoded = skp.encode().unwrap();
    assert_eq!(
        encoded,
        "mysprivkey1qzdlfxn2qa2lj5uprl8pyhexs02sg2wrhdy7qaq50cqgnffw4c2477kg9h3"
    );
    let decoded = MysKeyPair::decode(&encoded).unwrap();
    assert_eq!(skp, decoded);
}

#[test]
fn serde_pubkey() {
    let skp = MysKeyPair::Ed25519(get_key_pair().1);
...
fn public_key_equality() {
    let ed_kp1: MysKeyPair = MysKeyPair::Ed25519(get_key_pair().1);
    let ed_kp2: MysKeyPair = MysKeyPair::Ed25519(get_key_pair().1);
    let k1_kp1: MysKeyPair = MysKeyPair::Secp256k1(get_key_pair().1);
    let k1_kp2: MysKeyPair = MysKeyPair::Secp256k1(get_key_pair().1);
...
fn test_proof_of_possession() {
    let address =
        MysAddress::from_str("0x1a4623343cd42be47d67314fce0ad042f3c82685544bc91d8c11d24e74ba7357")
            .unwrap();
...
proptest! {
    ...
    fn test_authority_pk_bytes(
        ...
        let _myssig = Ed25519MysSignature::from_bytes(&bytes);
        let _myssig = Secp256k1MysSignature::from_bytes(&bytes);
    ...
    fn test_deserialize_keypair(
        ...
        let _skp: Result<MysKeyPair, _> = bcs::from_bytes(&bytes);
