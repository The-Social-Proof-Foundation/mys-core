...
use crate::{
    ...
    crypto::{
        get_key_pair, get_key_pair_from_rng, Ed25519MysSignature, PublicKey, Signature, MysKeyPair,
        MysSignatureInner, ZkLoginPublicIdentifier,
    },
...
fn zklogin_in_multisig_works_with_both_addresses() {
    ...
    assert!(
        matches!(res, Err(crate::error::MysError::InvalidSignature { error }) if error.contains("General cryptographic error: Groth16 proof verify failed"))
    );
