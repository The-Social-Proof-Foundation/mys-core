...
use crate::{
    ...
    crypto::{
        Ed25519MysSignature, PublicKey, Signature, MysKeyPair, MysSignatureInner,
...
fn test_combine_sigs() {
    ...
    let kp3: MysKeyPair = MysKeyPair::Secp256r1(get_key_pair().1);
...
fn test_serde_roundtrip() {
    ...
    let sig = Ed25519MysSignature::default();
    ...
    assert_eq!(single_sig_bytes.len(), Ed25519MysSignature::LENGTH);
    assert_eq!(
...
fn test_multisig_address() {
    ...
    assert_eq!(
        MysAddress::from_str("0xe35c69eb504de34afdbd9f307fb3ca152646c92d549fea00065d26fc422109ea")
            .unwrap(),
        address
    );
}
...
fn test_max_sig() {
    ...
    for _ in 0..11 {
        let k = MysKeyPair::Ed25519(get_key_pair_from_rng(&mut seed).1);
        pks.push(k.public());
        keys.push(k);
    }
...
fn multisig_zklogin_scenarios() {
    // consistency test with mys/sdk/typescript/test/unit/cryptography/multisig.test.ts
    let mut seed = StdRng::from_seed([0; 32]);
    let kp: Ed25519KeyPair = get_key_pair_from_rng(&mut seed).1;
    let skp: MysKeyPair = MysKeyPair::Ed25519(kp);
    let pk1 = skp.public();
    ...
    let multisig_addr = MysAddress::from(&multisig_pk);
    assert_eq!(
        multisig_addr,
        MysAddress::from_str("0xb9c0780a3943cde13a2409bf1a6f06ae60b0dff2b2f373260cf627aa4f43a588")
            .unwrap()
    );
    ...
    let intent_msg = &IntentMessage::new(Intent::mys_transaction(), tx.clone());
...
fn zklogin_in_multisig_works_with_both_addresses() {
    ...
    let multisig_address = MysAddress::from(&multisig_pk);
    ...
    assert!(
        matches!(res, Err(crate::error::MysErrorKind::InvalidSignature { error }) if error.contains("General cryptographic error: Groth16 proof verify failed"))
    );
    ...
    let multisig_address_padded = MysAddress::from(&multisig_pk_padded);
...
fn test_derive_multisig_address() {
    ...
    assert_eq!(
        multisig_addr,
        MysAddress::from_str("0x77a9fbf3c695d78dd83449a81a9e70aa79a77dbfd6fb72037bf09201c12052cd")
            .unwrap()
    );
}
...
fn test_zklogin_public_identifier_additional_validation() {
    let ed25519_kp = MysKeyPair::Ed25519(get_key_pair().1);
    ...
    match &err {
        crate::error::MysErrorKind::InvalidSignature { error } => {
