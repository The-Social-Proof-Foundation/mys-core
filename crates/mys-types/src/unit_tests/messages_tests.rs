...
use crate::crypto::{
    ...
    MysAuthoritySignature, get_key_pair,
};
...
fn test_new_with_signatures() {
    ...
    assert_eq!(
        quorum
            ...
            .collect::<MysResult<Vec<&AuthorityName>>>()
            .unwrap(),
...
fn test_handle_reject_malicious_signature() {
        ...
        AuthorityStrongQuorumSignInfo::new_from_auth_sign_infos(signatures, &committee).unwrap();
    {
        ...
        let sig = AuthoritySignature::new_secure(
            &IntentMessage::new(Intent::mys_transaction(), message.clone()),
            &committee.epoch,
            &sec,
        );
...
fn test_sponsored_transaction_message() {
    let epoch = 0;
    let sender_kp = MysKeyPair::Ed25519(get_key_pair().1);
    let sender = (&sender_kp.public()).into();
    let sponsor_kp = MysKeyPair::Ed25519(get_key_pair().1);
    ...
    assert!(matches!(
        Transaction::from_generic_sig_data(tx_data.clone(), vec![sender_sig.clone()],)
            ...
            .unwrap_err()
            .into_inner(),
        MysErrorKind::SignerSignatureNumberMismatch { .. }
    ));
    ...
    let third_party_kp = MysKeyPair::Ed25519(get_key_pair().1);
    ...
    assert!(matches!(
        Transaction::from_generic_sig_data(tx_data, vec![sponsor_sig, third_party_sig],)
            ...
            .unwrap_err()
            .into_inner(),
        MysErrorKind::SignerSignatureAbsent { .. }
    ));
...
fn test_sponsored_transaction_validity_check() {
    ...
    let pt = {
        let mut builder = ProgrammableTransactionBuilder::new();
        builder.transfer_mys(MysAddress::random_for_testing_only(), Some(50000));
        builder.finish()
    };
    let kind = TransactionKind::programmable(pt);
    TransactionData::new_with_gas_data(kind, sender, gas_data.clone())
    ...
    let pt = {
        let mut builder = ProgrammableTransactionBuilder::new();
        builder.pay_mys(vec![], vec![]).unwrap();
        builder.finish()
    };
    let kind = TransactionKind::programmable(pt);
    TransactionData::new_with_gas_data(kind, sender, gas_data.clone())
    ...
    let pt = {
        let mut builder = ProgrammableTransactionBuilder::new();
        builder.pay_all_mys(MysAddress::random_for_testing_only());
        builder.finish()
    };
    let kind = TransactionKind::programmable(pt);
...
fn verify_sender_signature_correctly_with_flag() {
    ...
    let receiver_kp = MysKeyPair::Secp256k1(get_key_pair().1);
    let receiver_address = (&receiver_kp.public()).into();

    // create a sender keypair with Secp256k1
    let sender_kp = MysKeyPair::Secp256k1(get_key_pair().1);
    ...
    let sender_kp_2 = MysKeyPair::Ed25519(get_key_pair().1);
    let mut tx_data_2 = tx_data.clone();
    *tx_data_2.sender_mut_for_testing() = (&sender_kp_2.public()).into();
    ...
    let sender_kp_3 = MysKeyPair::Secp256r1(get_key_pair().1);
    ...
    *tx_data_3.sender_mut_for_testing() = (&sender_kp_3.public()).into();
    ...
    assert_eq!(s.scheme().flag(), Secp256k1MysSignature::SCHEME.flag());
    ...
    assert_eq!(s.scheme().flag(), Ed25519MysSignature::SCHEME.flag());
