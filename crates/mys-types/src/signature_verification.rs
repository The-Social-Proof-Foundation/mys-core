...
impl<D: Hash + Eq + Copy> VerifiedDigestCache<D> {
    ...
    pub fn is_verified<F, G>(&self, digest: D, verify_callback: F, uncached_checks: G) -> MysResult
    where
        F: FnOnce() -> MysResult,
        G: FnOnce() -> MysResult,
    {
...
pub fn verify_sender_signed_data_message_signatures(
    ...
    assert_eq!(intent_message.intent, Intent::mys_transaction());
    ...
    for s in signers {
        if !present_sigs.contains_key(&s) {
            return Err(MysError::SignerSignatureAbsent {
