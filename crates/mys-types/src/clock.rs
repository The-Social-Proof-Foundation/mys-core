...
use crate::{id::UID, MYS_FRAMEWORK_ADDRESS};

pub const CLOCK_MODULE_NAME: &IdentStr = ident_str!("clock");
pub const CLOCK_STRUCT_NAME: &IdentStr = ident_str!("Clock");
pub const RESOLVED_MYS_CLOCK: (&AccountAddress, &IdentStr, &IdentStr) =
    (&MYS_FRAMEWORK_ADDRESS, CLOCK_MODULE_NAME, CLOCK_STRUCT_NAME);
...
impl Clock {
    ...
    /// Detects a `&mut mys::clock::Clock` or `mys::clock::Clock` in the signature.
    pub fn is_mutable(view: &CompiledModule, s: &SignatureToken) -> bool {
        use SignatureToken as S;
        match s {
            S::MutableReference(inner) => Self::is_mutable(view, inner),
            S::Datatype(idx) => resolve_struct(view, *idx) == RESOLVED_MYS_CLOCK,
            _ => false,
        }
    }
}
