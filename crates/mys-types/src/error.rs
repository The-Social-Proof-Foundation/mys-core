...
pub enum UserInputError {
    ...
    #[error("MYS payment transactions use first input coin for gas payment, but found a different gas object")]
...
pub enum MysObjectResponseError {
    ...
    // TODO: also integrate MysPastObjectResponse (VersionNotFound,  VersionTooHigh)
}

/// Custom error type for Mys.
#[derive(
    Eq, PartialEq, Clone, Debug, Serialize, Deserialize, Error, Hash, AsRefStr, IntoStaticStr,
)]
pub enum MysError {
    ...
    MysObjectResponseError { error: MysObjectResponseError },
    ...
    TooManyIncorrectAuthorities {
        errors: Vec<(AuthorityName, MysError)>,
        action: String,
    },
    ...
    #[error("Failed to execute handle_consensus_transaction on Mys: {0}")]
...
pub type MysResult<T = ()> = Result<T, MysError>;
pub type UserInputResult<T = ()> = Result<T, UserInputError>;

impl From<mys_protocol_config::Error> for MysError {
    fn from(error: mys_protocol_config::Error) -> Self {
        MysError::WrongMessageVersion { error: error.0 }
    }
}

impl From<ExecutionError> for MysError {
    fn from(error: ExecutionError) -> Self {
        MysError::ExecutionError(error.to_string())
    }
}

impl From<Status> for MysError {
    fn from(status: Status) -> Self {
        if status.message() == "Too many requests" {
            return Self::TooManyRequests;
        }

        let result = bcs::from_bytes::<MysError>(status.details());
        if let Ok(mys_error) = result {
            mys_error
        } else {
            Self::RpcError(
...
impl From<TypedStoreError> for MysError {
    fn from(e: TypedStoreError) -> Self {
        Self::Storage(e.to_string())
    }
}

impl From<crate::storage::error::Error> for MysError {
    fn from(e: crate::storage::error::Error) -> Self {
        Self::Storage(e.to_string())
    }
}

impl From<MysError> for Status {
...
impl From<String> for MysError {
    fn from(error: String) -> Self {
        MysError::GenericAuthorityError { error }
    }
}

impl TryFrom<MysError> for UserInputError {
    type Error = anyhow::Error;

    fn try_from(err: MysError) -> Result<Self, Self::Error> {
        match err {
            MysError::UserInputError { error } => Ok(error),
            other => anyhow::bail!("error {:?} is not UserInputError", other),
        }
    }
}

impl From<UserInputError> for MysError {
    fn from(error: UserInputError) -> Self {
        MysError::UserInputError { error }
    }
}

impl From<MysObjectResponseError> for MysError {
    fn from(error: MysObjectResponseError) -> Self {
        MysError::MysObjectResponseError { error }
    }
}
...
impl MysError {
    pub fn individual_error_indicates_epoch_change(&self) -> bool {
        matches!(
            self,
            MysError::ValidatorHaltedAtEpochEnd | MysError::MissingCommitteeAtEpoch(_)
        )
    }
    ...
    pub fn is_retryable(&self) -> (bool, bool) {
        let retryable = match self {
            // Network error
            MysError::RpcError { .. } => true,

            // Reconfig error
            MysError::ValidatorHaltedAtEpochEnd => true,
            MysError::MissingCommitteeAtEpoch(..) => true,
            MysError::WrongEpoch { .. } => true,
            MysError::EpochEnded(..) => true,
            ...
            MysError::PotentiallyTemporarilyInvalidSignature { .. } => true,

            // Overload errors
            MysError::TooManyTransactionsPendingExecution { .. } => true,
            MysError::TooManyTransactionsPendingOnObject { .. } => true,
            MysError::TooOldTransactionPendingOnObject { .. } => true,
            MysError::TooManyTransactionsPendingConsensus => true,
            MysError::ValidatorOverloadedRetryAfter { .. } => true,

            // Non retryable error
            MysError::ExecutionError(..) => false,
            MysError::ByzantineAuthoritySuspicion { .. } => false,
            MysError::QuorumFailedToGetEffectsQuorumWhenProcessingTransaction { .. } => false,
            MysError::TxAlreadyFinalizedWithDifferentUserSigs => false,
            MysError::FailedToVerifyTxCertWithExecutedEffects { .. } => false,
            MysError::ObjectLockConflict { .. } => false,
            ...
            MysError::TooManyRequests => false,
    ...
    pub fn is_retryable_overload(&self) -> bool {
        matches!(self, MysError::ValidatorOverloadedRetryAfter { .. })
    }

    pub fn retry_after_secs(&self) -> u64 {
        match self {
            MysError::ValidatorOverloadedRetryAfter { retry_after_secs } => *retry_after_secs,
            _ => 0,
        }
    }
}
