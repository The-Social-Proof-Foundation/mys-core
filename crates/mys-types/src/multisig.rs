...
impl AuthenticatorTrait for MultiSig {
        ...
        T: Serialize,
    {
        ...
        if MysAddress::from(&self.multisig_pk) != multisig_address {
            return Err(MysError::InvalidSignature {
        ...
        for (sig, i) in self.sigs.iter().zip(as_indices(self.bitmap)?) {
            let (subsig_pubkey, weight) =
                self.multisig_pk
                    .pk_map
                    .get(i as usize)
                    .ok_or(MysError::InvalidSignature {
            ...
            let res = match sig {
                CompressedSignature::Ed25519(s) => {
                    ...
                    pk.verify(
                        &digest,
                        &s.try_into().map_err(|_| MysError::InvalidSignature {
            ...
            if res.is_ok() {
                ...
                return res.map_err(|e| MysError::InvalidSignature {
                    error: format!(
...
pub fn as_indices(bitmap: u16) -> Result<Vec<u8>, MysError> {
...
impl MultiSig {
    ...
    pub fn combine(
        ...
        for s in full_sigs {
            ...
            let index = multisig_pk
                .get_index(&pk)
                .ok_or(MysError::IncorrectSigner {
    ...
    pub fn get_zklogin_sigs(&self) -> Result<Vec<ZkLoginAuthenticator>, MysError> {
        ...
        authenticator_as_bytes
            .iter()
            .map(|z| {
                ZkLoginAuthenticator::from_bytes(&z.0).map_err(|_| MysError::InvalidSignature {
                    error: "Invalid zklogin authenticator bytes".to_string(),
                })
            })
            .collect()
    ...
    pub fn get_indices(&self) -> Result<Vec<u8>, MysError> {
...
impl FromStr for MultiSig {
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        let bytes = Base64::decode(s).map_err(|_| MysError::InvalidSignature {
            error: "Invalid base64 string".to_string(),
        })?;
        let sig = MultiSig::from_bytes(&bytes).map_err(|_| MysError::InvalidSignature {
