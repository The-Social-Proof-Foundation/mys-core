...
impl ProgrammableTransactionBuilder {
    ...
    pub fn transfer_arg(&mut self, recipient: MysAddress, arg: Argument) {
        self.transfer_args(recipient, vec![arg])
    }

    pub fn transfer_args(&mut self, recipient: MysAddress, args: Vec<Argument>) {
    ...
    pub fn transfer_mys(&mut self, recipient: MysAddress, amount: Option<u64>) {
        let rec_arg = self.pure(recipient).unwrap();
    ...
    pub fn pay_all_mys(&mut self, recipient: MysAddress) {
        let rec_arg = self.pure(recipient).unwrap();
    ...
    fn pay_impl(
        ...
        let mut recipient_map: IndexMap<MysAddress, Vec<usize>> = IndexMap::new();
