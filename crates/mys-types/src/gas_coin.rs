...
pub const TOTAL_SUPPLY_MIST: u64 = TOTAL_SUPPLY_MYS * MIST_PER_MYS;

pub const GAS_MODULE_NAME: &IdentStr = ident_str!("mys");
pub const GAS_STRUCT_NAME: &IdentStr = ident_str!("MYS");
...
mod checked {
    ...
    /// Rust version of the Move mys::coin::Coin<Mys::mys::MYS> type
    ...
    impl GasCoin {
        ...
        /// Return `true` if `s` is the type of a gas coin (i.e., 0x2::coin::Coin<0x2::mys::MYS>)
        pub fn is_gas_coin(s: &StructTag) -> bool {
            Coin::is_coin(s) && s.type_params.len() == 1 && GAS::is_gas_type(&s.type_params[0])
        }

        /// Return `true` if `s` is the type of a gas balance (i.e., 0x2::balance::Balance<0x2::mys::MYS>)
