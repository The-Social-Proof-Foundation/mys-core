...
use crate::dynamic_field::{
    DynamicFieldInfo, DynamicFieldType, DYNAMIC_FIELD_FIELD_STRUCT_NAME, DYNAMIC_FIELD_MODULE_NAME,
};
...
use crate::error::{ExecutionError, MysResult};
use crate::gas_coin::GasCoin;
use crate::gas_coin::GAS;
use crate::governance::StakedMys;
use crate::governance::STAKED_MYS_STRUCT_NAME;
...
/// Represents an distinct stream of object versions for a Shared or ConsensusAddressOwner object,
...
pub enum MoveObjectType_ {
    ...
    GasCoin,
    /// A record of a staked MYS coin (i.e., `0x3::staking_pool::StakedMys`)
    StakedMys,
...
impl MoveObjectType {
    ...
    pub fn staked_mys() -> Self {
        Self(MoveObjectType_::StakedMys)
    }

    pub fn address(&self) -> AccountAddress {
        match &self.0 {
            MoveObjectType_::GasCoin | MoveObjectType_::Coin(_) => MYS_FRAMEWORK_ADDRESS,
            MoveObjectType_::StakedMys => MYS_SYSTEM_ADDRESS,
            MoveObjectType_::MysBalanceAccumulatorField
            | MoveObjectType_::BalanceAccumulatorField(_) => MYS_FRAMEWORK_ADDRESS,
    ...
    pub fn module(&self) -> &IdentStr {
        match &self.0 {
            ...
            MoveObjectType_::StakedMys => STAKING_POOL_MODULE_NAME,
            MoveObjectType_::MysBalanceAccumulatorField
    ...
    pub fn name(&self) -> &IdentStr {
        match &self.0 {
            ...
            MoveObjectType_::StakedMys => STAKED_MYS_STRUCT_NAME,
            MoveObjectType_::MysBalanceAccumulatorField
    ...
    pub fn coin_type_maybe(&self) -> Option<TypeTag> {
        match &self.0 {
            ...
            MoveObjectType_::StakedMys => None,
            MoveObjectType_::MysBalanceAccumulatorField => None,
    ...
    pub fn size_for_gas_metering(&self) -> usize {
        ...
        match &self.0 {
            ...
            MoveObjectType_::StakedMys => 1,
            MoveObjectType_::Coin(inner) => bcs::serialized_size(inner).unwrap() + 1,
            MoveObjectType_::MysBalanceAccumulatorField => 1,
            MoveObjectType_::BalanceAccumulatorField(inner) => {
    ...
    /// Return true if `self` is `0x2::coin::Coin<T>` for some T (note: T can be MYS)
    pub fn is_coin(&self) -> bool {
        match &self.0 {
            MoveObjectType_::GasCoin | MoveObjectType_::Coin(_) => true,
            MoveObjectType_::StakedMys
            | MoveObjectType_::MysBalanceAccumulatorField
    ...
    /// Return true if `self` is 0x2::coin::Coin<0x2::mys::MYS>
    ...
    pub fn is_staked_mys(&self) -> bool {
        match &self.0 {
            MoveObjectType_::StakedMys => true,
    ...
    pub fn is_upgrade_cap(&self) -> bool {
        self.address() == MYS_FRAMEWORK_ADDRESS
    ...
    pub fn balance_accumulator_field_type_maybe(&self) -> Option<TypeTag> {
        match &self.0 {
            MoveObjectType_::MysBalanceAccumulatorField => Some(GAS::type_tag()),
    ...
    pub fn is_mys_balance_accumulator_field(&self) -> bool {
        matches!(self.0, MoveObjectType_::MysBalanceAccumulatorField)
    }
    ...
    fn balance_accumulator_field_type_params(inner_type: TypeTag) -> Vec<TypeTag> {
        ...
        let key_type = <AccumulatorKey as MoveTypeTagTraitGeneric>::get_type_tag(&[balance_type]);
        let u128_type = <U128 as MoveTypeTagTrait>::get_type_tag();
    ...
    pub fn is_dynamic_field(&self) -> bool {
        match &self.0 {
            MoveObjectType_::GasCoin | MoveObjectType_::StakedMys | MoveObjectType_::Coin(_) => {
                false
            }
            MoveObjectType_::MysBalanceAccumulatorField
    ...
    pub fn is_currency(&self) -> bool {
        match &self.0 {
            ...
            | MoveObjectType_::MysBalanceAccumulatorField
            | MoveObjectType_::BalanceAccumulatorField(_) => false,
            MoveObjectType_::Other(_s) => {
                // Stub implementation: coin_registry module doesn't exist in mys-types yet
                // For now, return false. This can be enhanced when coin_registry is added.
                // In mys-types, this checks: coin_registry::Currency::is_currency(s)
                false
            }
        }
    }

    pub fn try_extract_field_name(&self, type_: &DynamicFieldType) -> MysResult<TypeTag> {
        match &self.0 {
            MoveObjectType_::GasCoin
            | MoveObjectType_::StakedMys
            | MoveObjectType_::Coin(_)
            ...
            | MoveObjectType_::BalanceAccumulatorField(_) => {
                Err(MysError::ObjectDeserializationError {
                    error: "Error extracting dynamic object name from Coin/accumulator object".to_string(),
                })
            }
    ...
    pub fn try_extract_field_value(&self) -> MysResult<TypeTag> {
        match &self.0 {
            MoveObjectType_::GasCoin
            | MoveObjectType_::StakedMys
            | MoveObjectType_::Coin(_)
            ...
            | MoveObjectType_::BalanceAccumulatorField(_) => {
                Err(MysError::ObjectDeserializationError {
                    error: "Error extracting dynamic object value from Coin/accumulator object".to_string(),
                })
            }
    ...
    pub fn is(&self, s: &StructTag) -> bool {
        match &self.0 {
            ...
            MoveObjectType_::StakedMys => StakedMys::is_staked_mys(s),
            MoveObjectType_::Coin(inner) => {
                Coin::is_coin(s) && s.type_params.len() == 1 && inner == &s.type_params[0]
            }
            MoveObjectType_::MysBalanceAccumulatorField => accumulator_value_balance_type_maybe(s)
...
impl From<StructTag> for MoveObjectType {
    fn from(mut s: StructTag) -> Self {
        ...
        } else if StakedMys::is_staked_mys(&s) {
            MoveObjectType_::StakedMys
...
impl From<MoveObjectType> for StructTag {
    fn from(t: MoveObjectType) -> Self {
        match t.0 {
            ...
            MoveObjectType_::StakedMys => StakedMys::type_(),
...
pub fn is_primitive_type_tag(t: &TypeTag) -> bool {
    ...
    match t {
        ...
        T::Struct(st) => {
            ...
            // is id or..
            if resolved_struct == RESOLVED_MYS_ID {
                return true;
            }
            // is utf8 string
...
impl FromStr for ObjectType {
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        if s.to_lowercase() == PACKAGE {
            ...
            let tag = parse_mys_struct_tag(s)?;
...
pub const MYS_ADDRESS_LENGTH: usize = ObjectID::LENGTH;
...
#[cfg_attr(feature = "fuzzing", derive(proptest_derive::Arbitrary))]
pub struct MysAddress(
    #[schemars(with = "Hex")]
    #[serde_as(as = "Readable<Hex, _>")]
    [u8; MYS_ADDRESS_LENGTH],
);

impl MysAddress {
    pub const ZERO: Self = Self([0u8; MYS_ADDRESS_LENGTH]);
    ...
    /// Deserialize into an `Option<MysAddress>`.
    pub fn optional_address_from_hex<'de, D>(
        deserializer: D,
    ) -> Result<Option<MysAddress>, D::Error>
    where
    ...
    /// Return the underlying byte array of a MysAddress.
    pub fn to_inner(self) -> [u8; MYS_ADDRESS_LENGTH] {
        self.0
    }

    /// Parse a MysAddress from a byte array or buffer.
    pub fn from_bytes<T: AsRef<[u8]>>(bytes: T) -> Result<Self, MysError> {
        <[u8; MYS_ADDRESS_LENGTH]>::try_from(bytes.as_ref())
            .map_err(|_| MysError::InvalidAddress)
            .map(MysAddress)
    }
    ...
    pub fn try_from_padded(inputs: &ZkLoginInputs) -> MysResult<Self> {
        Ok((&PublicKey::from_zklogin_inputs(inputs)?).into())
    }

    /// Define as iss_bytes_len || iss_bytes || unpadded_32_byte_address_seed.
    pub fn try_from_unpadded(inputs: &ZkLoginInputs) -> MysResult<Self> {
        ...
        Ok(MysAddress(hasher.finalize().digest))
    }
}

impl From<ObjectID> for MysAddress {
    fn from(object_id: ObjectID) -> MysAddress {
        Self(object_id.into_bytes())
    }
}

impl From<AccountAddress> for MysAddress {
    fn from(address: AccountAddress) -> MysAddress {
        Self(address.into_bytes())
    }
}

impl TryFrom<&[u8]> for MysAddress {
    type Error = MysError;

    /// Tries to convert the provided byte array into a MysAddress.
    fn try_from(bytes: &[u8]) -> Result<Self, MysError> {
...
impl TryFrom<Vec<u8>> for MysAddress {
    ...
    /// Tries to convert the provided byte buffer into a MysAddress.
    fn try_from(bytes: Vec<u8>) -> Result<Self, MysError> {
        Self::from_bytes(bytes)
    }
}

impl AsRef<[u8]> for MysAddress {
    fn as_ref(&self) -> &[u8] {
        &self.0[..]
    }
}
...
impl<T: MysPublicKey> From<&T> for MysAddress {
    fn from(pk: &T) -> Self {
        let mut hasher = DefaultHash::default();
        hasher.update([T::SIGNATURE_SCHEME.flag()]);
        hasher.update(pk);
        let g_arr = hasher.finalize();
        MysAddress(g_arr.digest)
    }
}

impl From<&PublicKey> for MysAddress {
...
impl From<&MultiSigPublicKey> for MysAddress {
    /// Derive a MysAddress from [struct MultiSigPublicKey]. A MultiSig address
    ...
    fn from(multisig_pk: &MultiSigPublicKey) -> Self {
        ...
        MysAddress(hasher.finalize().digest)
    }
}

/// Mys address for [struct ZkLoginAuthenticator] is defined as the black2b hash of
/// [zklogin_flag || iss_bytes_length || iss_bytes || unpadded_address_seed_in_bytes].
impl TryFrom<&ZkLoginAuthenticator> for MysAddress {
    type Error = MysError;
    fn try_from(authenticator: &ZkLoginAuthenticator) -> MysResult<Self> {
        MysAddress::try_from_unpadded(&authenticator.inputs)
    }
}

impl TryFrom<&GenericSignature> for MysAddress {
    type Error = MysError;
    /// Derive a MysAddress from a serialized signature in Mys [GenericSignature].
    fn try_from(sig: &GenericSignature) -> MysResult<Self> {
        match sig {
            ...
            GenericSignature::ZkLoginAuthenticator(zklogin) => {
                MysAddress::try_from_unpadded(&zklogin.inputs)
            }
            GenericSignature::PasskeyAuthenticator(s) => Ok(MysAddress::from(&s.get_pk()?)),
        }
    }
}

impl fmt::Display for MysAddress {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        write!(f, "0x{}", Hex::encode(self.0))
    }
}

impl fmt::Debug for MysAddress {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> Result<(), fmt::Error> {
        write!(f, "0x{}", Hex::encode(self.0))
    }
}

/// Generate a fake MysAddress with repeated one byte.
pub fn dbg_addr(name: u8) -> MysAddress {
    let addr = [name; MYS_ADDRESS_LENGTH];
    MysAddress(addr)
}
...
impl TxContext {
    pub fn new(sender: &MysAddress, digest: &TransactionDigest, epoch_data: &EpochData) -> Self {
    ...
    pub fn kind(view: &CompiledModule, s: &SignatureToken) -> TxContextKind {
        ...
        let is_tx_context_type = module_name == TX_CONTEXT_MODULE_NAME
            && module_addr == &MYS_FRAMEWORK_ADDRESS
    ...
    pub fn sender(&self) -> MysAddress {
        MysAddress::from(ObjectID(self.sender))
    }
    ...
    pub fn random_for_testing_only() -> Self {
        Self::new(
            &MysAddress::random_for_testing_only(),
    ...
    pub fn with_sender_for_testing_only(sender: &MysAddress) -> Self {
...
impl ObjectID {
    ...
    pub fn is_clock(&self) -> bool {
        *self == MYS_CLOCK_OBJECT_ID
    }
}

impl From<MysAddress> for ObjectID {
    fn from(address: MysAddress) -> ObjectID {
...
impl fmt::Display for MoveObjectType {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> std::fmt::Result {
        ...
        write!(
            f,
            "{}",
            to_mys_struct_tag_string(&s).map_err(fmt::Error::custom)?
        )
    }
}
