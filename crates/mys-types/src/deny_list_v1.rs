...
/// It filters out MYS coins since it's known that it's not a regulated coin.
...
fn check_deny_list_v1_impl(
    ...
    let Ok(count) = get_dynamic_field_from_store::<MysAddress, u64>(
        object_store,
    ...
    for coin_type in coin_types {
        let Ok(denied_addresses) = get_dynamic_field_from_store::<Vec<u8>, VecSet<MysAddress>>(
            object_store,
...
pub fn get_deny_list_root_object(object_store: &dyn ObjectStore) -> Option<Object> {
    ...
    match object_store.get_object(&MYS_DENY_LIST_OBJECT_ID) {
        Some(obj) => Some(obj),
        None => {
