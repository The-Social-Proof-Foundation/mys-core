...
impl TryFrom<Vec<u8>> for Digest {
    ...
    fn try_from(bytes: Vec<u8>) -> Result<Self, MysError> {
        let bytes: [u8; 32] =
            <[u8; 32]>::try_from(&bytes[..]).map_err(|_| MysError::InvalidDigestLength {
                expected: 32,
                actual: bytes.len(),
            })?;
...
const MYS_PROTOCOL_CONFIG_CHAIN_OVERRIDE_ENV_VAR_NAME: &str = "MYS_PROTOCOL_CONFIG_CHAIN_OVERRIDE";

static MYS_PROTOCOL_CONFIG_CHAIN_OVERRIDE: Lazy<Option<Chain>> = Lazy::new(|| {
    if let Ok(s) = env::var(MYS_PROTOCOL_CONFIG_CHAIN_OVERRIDE_ENV_VAR_NAME) {
        info!("MYS_PROTOCOL_CONFIG_CHAIN_OVERRIDE: {:?}", s);
        match s.as_str() {
            "mainnet" => Some(Chain::Mainnet),
            "testnet" => Some(Chain::Testnet),
            "" => None,
            _ => panic!("unrecognized MYS_PROTOCOL_CONFIG_CHAIN_OVERRIDE: {s:?}"),
        }
    } else {
        None
    }
...
impl ChainIdentifier {
    ...
    pub fn chain(&self) -> Chain {
        ...
        if let Some(override_chain) = *MYS_PROTOCOL_CONFIG_CHAIN_OVERRIDE {
...
impl TransactionDigest {
    ...
    // TODO(https://github.com/MystenLabs/mys/issues/65): we can pick anything here
...
impl TryFrom<&[u8]> for TransactionDigest {
    type Error = crate::error::MysError;

    fn try_from(bytes: &[u8]) -> Result<Self, crate::error::MysError> {
        let arr: [u8; 32] = bytes
            .try_into()
            .map_err(|_| crate::error::MysError::InvalidTransactionDigest)?;
        Ok(Self::new(arr))
    }
}
...
mod test {
    ...
    fn test_chain_id_mainnet() {
        ...
        assert_eq!(
            ...
            mys_protocol_config::Chain::Mainnet
        );
    }
    ...
    fn test_chain_id_testnet() {
        ...
        assert_eq!(
            chain_id.unwrap().chain(),
            mys_protocol_config::Chain::Testnet
        );
    }
