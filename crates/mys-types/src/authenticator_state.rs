...
use crate::{id::UID, MYS_AUTHENTICATOR_STATE_OBJECT_ID, MYS_FRAMEWORK_ADDRESS};
...
pub const RESOLVED_MYS_AUTHENTICATOR_STATE: (&AccountAddress, &IdentStr, &IdentStr) = (
...
) -> MysResult<Option<AuthenticatorStateInner>> {
    let outer = object_store.get_object(&MYS_AUTHENTICATOR_STATE_OBJECT_ID);
...
pub fn get_authenticator_state_obj_initial_shared_version(
    ...
    Ok(object_store
        .get_object(&MYS_AUTHENTICATOR_STATE_OBJECT_ID)
