...
use crate::{base_types::MysAddress, crypto::Signature, error::MysResult, multisig::MultiSig};
...
impl GenericSignature {
    ...
    /// Parse [enum CompressedSignature] from trait MysSignature `flag || sig || pk`.
    /// This is useful for the MultiSig to combine partial signature into a MultiSig public key.
    pub fn to_compressed(&self) -> Result<CompressedSignature, MysError> {
        match self {
            GenericSignature::Signature(s) => {
                ...
                match s.scheme() {
                    ...
                    _ => Err(MysError::UnsupportedFeatureError {
    ...
    /// Parse [struct PublicKey] from trait MysSignature `flag || sig || pk`.
    /// This is useful for the MultiSig to construct the bitmap in [struct MultiPublicKey].
    pub fn to_public_key(&self) -> Result<PublicKey, MysError> {
        match self {
            GenericSignature::Signature(s) => {
                ...
                match s.scheme() {
                    SignatureScheme::ED25519 => Ok(PublicKey::Ed25519(
                        (&Ed25519PublicKey::from_bytes(bytes).map_err(|_| {
                            MysError::KeyConversionError("Cannot parse ed25519 pk".to_string())
                        })?)
                            .into(),
                    )),
                    SignatureScheme::Secp256k1 => Ok(PublicKey::Secp256k1(
                        (&Secp256k1PublicKey::from_bytes(bytes).map_err(|_| {
                            MysError::KeyConversionError("Cannot parse secp256k1 pk".to_string())
                        })?)
                            .into(),
                    )),
                    SignatureScheme::Secp256r1 => Ok(PublicKey::Secp256r1(
                        (&Secp256r1PublicKey::from_bytes(bytes).map_err(|_| {
                            MysError::KeyConversionError("Cannot parse secp256r1 pk".to_string())
                        })?)
                            .into(),
                    )),
                    _ => Err(MysError::UnsupportedFeatureError {
...
impl AuthenticatorTrait for Signature {
    fn verify_user_authenticator_epoch(&self, _: EpochId, _: Option<EpochId>) -> MysResult {
        Ok(())
    }
