...
/// Rust version of the Move mys::dynamic_field::Field type
...
/// Rust version of the Move mys::dynamic_object_field::Wrapper type
...
pub struct DynamicFieldName {
    ...
    #[serde_as(as = "Readable<MysTypeTag, _>")]
    pub type_: TypeTag,
...
impl DynamicFieldInfo {
    ...
    pub fn try_extract_field_name(tag: &StructTag, type_: &DynamicFieldType) -> MysResult<TypeTag> {
        match (type_, tag.type_params.first()) {
            ...
            (DynamicFieldType::DynamicObject, Some(TypeTag::Struct(s))) => Ok(s
                .type_params
                .first()
                .ok_or_else(|| MysError::ObjectDeserializationError {
    ...
    pub fn try_extract_field_value(tag: &StructTag) -> MysResult<TypeTag> {
        match tag.type_params.last() {
            Some(value_type) => Ok(value_type.clone()),
            None => Err(MysError::ObjectDeserializationError {
    ...
    pub fn parse_move_object(
    ...
    ) -> MysResult<(MoveValue, DynamicFieldType, ObjectID)> {
        let name = extract_field_from_move_struct(move_struct, "name").ok_or_else(|| {
            MysError::ObjectDeserializationError {
                error: "Cannot extract [name] field from mys::dynamic_field::Field".to_string(),
            }
        })?;

        let value = extract_field_from_move_struct(move_struct, "value").ok_or_else(|| {
            MysError::ObjectDeserializationError {
                error: "Cannot extract [value] field from mys::dynamic_field::Field".to_string(),
            }
        })?;
        ...
        Ok(if is_dynamic_object(move_struct) {
            ...
            .ok_or_else(|| MysError::ObjectDeserializationError {
                error: "Cannot extract [name] field from mys::dynamic_object_field::Wrapper."
                    .to_string(),
            })?;
            // ID extracted from the wrapper object
            let object_id =
                extract_id_value(value).ok_or_else(|| MysError::ObjectDeserializationError {
                    error: format!(
                        "Cannot extract dynamic object's object id from \
                        mys::dynamic_field::Field, {value:?}"
                    ),
                })?;
            ...
            let object_id = extract_object_id(move_struct).ok_or_else(|| {
                MysError::ObjectDeserializationError {
                    error: format!(
                        "Cannot extract dynamic object's object id from \
                        mys::dynamic_field::Field, {move_struct:?}",
                    ),
                }
            })?;
...
where
    T: Into<MysAddress>,
{
    let parent: MysAddress = parent.into();
    ...
    K: MoveTypeTagTrait + Serialize + DeserializeOwned + fmt::Debug,
{
    let id = derive_dynamic_field_id(parent_id, &K::get_type_tag(), &bcs::to_bytes(key).unwrap())
        .map_err(|err| MysError::DynamicFieldReadError(err.to_string()))?;
