...
use super::{base_types::*, error::*, MYS_BRIDGE_OBJECT_ID};
...
use crate::{
    MYS_AUTHENTICATOR_STATE_OBJECT_ID, MYS_AUTHENTICATOR_STATE_OBJECT_SHARED_VERSION,
    MYS_CLOCK_OBJECT_ID, MYS_CLOCK_OBJECT_SHARED_VERSION, MYS_FRAMEWORK_PACKAGE_ID,
    MYS_RANDOMNESS_STATE_OBJECT_ID, MYS_SYSTEM_STATE_OBJECT_ID,
    MYS_SYSTEM_STATE_OBJECT_SHARED_VERSION,
};
...
impl CallArg {
    pub const MYS_SYSTEM_MUT: Self = Self::Object(ObjectArg::MYS_SYSTEM_MUT);
    ...
    pub const AUTHENTICATOR_MUT: Self = Self::Object(ObjectArg::SharedObject {
        ...
        initial_shared_version: MYS_AUTHENTICATOR_STATE_OBJECT_SHARED_VERSION,
        mutability: SharedObjectMutability::Mutable,
    });
}
...
pub enum ObjectArg {
    ...
    // SharedObject::mutability controls whether caller asks for a mutable reference to shared object.
    SharedObject {
        id: ObjectID,
...
impl FundsWithdrawalArg {
    ...
    pub fn owner_for_withdrawal(&self, tx: &impl TransactionDataAPI) -> MysAddress {
...
impl EndOfEpochTransactionKind {
        ...
        match self {
            Self::ChangeEpoch(_) => {
                vec![InputObjectKind::SharedMoveObject {
                    id: MYS_SYSTEM_STATE_OBJECT_ID,
                    initial_shared_version: MYS_SYSTEM_STATE_OBJECT_SHARED_VERSION,
                    mutability: SharedObjectMutability::Mutable,
                }]
            }
    ...
    fn shared_input_objects(&self) -> impl Iterator<Item = SharedInputObject> + '_ {
        match self {
            Self::ChangeEpoch(_) => {
                Either::Left(vec![SharedInputObject::MYS_SYSTEM_OBJ].into_iter())
            }
            ...
            Self::BridgeCommitteeInit(bridge_version) => Either::Left(
                vec![
                    ...
                    SharedInputObject::MYS_SYSTEM_OBJ,
                ]
                .into_iter(),
            ),
        }
...
impl ObjectArg {
    pub const MYS_SYSTEM_MUT: Self = Self::SharedObject {
        id: MYS_SYSTEM_STATE_OBJECT_ID,
        initial_shared_version: MYS_SYSTEM_STATE_OBJECT_SHARED_VERSION,
        mutability: SharedObjectMutability::Mutable,
    };
    ...
    pub fn is_argument_used(&self, argument: Argument) -> bool {
        match self {
            ...
            | Command::SplitCoins(arg, args) => {
                args.iter().chain(std::iter::once(arg)).any(|a| a == &argument)
            }
...
impl ProgrammableTransaction {
    pub fn get_funds_withdrawals(&self) -> impl Iterator<Item = &FundsWithdrawalArg> + '_ {
    ...
    fn validity_check(&self, config: &ProtocolConfig) -> UserInputResult {
        ...
        if let Some(random_index) = inputs.iter().position(|obj| {
            matches!(obj, CallArg::Object(ObjectArg::SharedObject { id, .. }) if *id == MYS_RANDOMNESS_STATE_OBJECT_ID)
        }) {
            fp_ensure!(
...
impl SharedInputObject {
    pub const MYS_SYSTEM_OBJ: Self = Self {
        id: MYS_SYSTEM_STATE_OBJECT_ID,
        initial_shared_version: MYS_SYSTEM_STATE_OBJECT_SHARED_VERSION,
        mutable: true,
    };
...
impl TransactionKind {
    ...
    pub fn shared_input_objects(&self) -> impl Iterator<Item = SharedInputObject> + '_ {
        match &self {
            Self::ChangeEpoch(_) => {
                Either::Left(Either::Left(iter::once(SharedInputObject::MYS_SYSTEM_OBJ)))
            }
    ...
    fn get_funds_withdrawals(&self) -> impl Iterator<Item = &FundsWithdrawalArg> + '_ {
        match self {
            TransactionKind::ProgrammableTransaction(pt) => {
                Either::Right(pt.get_funds_withdrawals())
            }
            _ => Either::Left(iter::empty()),
        }
    }
...
pub enum TransactionExpiration {
    ...
    ValidDuring {
        ...
        chain: crate::digests::ChainIdentifier,
...
impl TransactionData {
    ...
    pub fn new_with_gas_data(kind: TransactionKind, sender: MysAddress, gas_data: GasData) -> Self {
        TransactionData::V1(TransactionDataV1 {
            kind,
            sender,
            gas_data,
    ...
    pub fn new_transfer_mys(
        ...
        Self::new_transfer_mys_allow_sponsor(
            recipient,
            sender,
            amount,
            gas_payment,
    ...
    pub fn new_transfer_mys_allow_sponsor(
        recipient: MysAddress,
        sender: MysAddress,
        amount: Option<u64>,
        gas_payment: ObjectRef,
        ...
        let pt = {
            let mut builder = ProgrammableTransactionBuilder::new();
            builder.transfer_mys(recipient, amount);
            builder.finish()
        };
    ...
    pub fn execution_parts(&self) -> (TransactionKind, MysAddress, Vec<ObjectRef>) {
        (
            self.kind().clone(),
            self.sender(),
    ...
    pub fn uses_randomness(&self) -> bool {
        self.shared_input_objects()
            .iter()
            .any(|obj| obj.id() == MYS_RANDOMNESS_STATE_OBJECT_ID)
    }
...
pub trait TransactionDataAPI {
    ...
    fn signers(&self) -> NonEmpty<MysAddress>;

    fn gas_data(&self) -> &GasData;

    fn gas_owner(&self) -> MysAddress;
    ...
    fn sender_mut_for_testing(&mut self) -> &mut MysAddress;
...
impl Serialize for SenderSignedTransaction {
        ...
        S: serde::Serializer,
    {
        ...
        if self.intent_message().intent != Intent::mys_transaction() {
...
impl<'de> Deserialize<'de> for SenderSignedTransaction {
        ...
        D: serde::Deserializer<'de>,
    {
        ...
        if intent_message.intent != Intent::mys_transaction() {
...
impl SenderSignedTransaction {
    ...
    ) -> MysResult<BTreeMap<MysAddress, &GenericSignature>> {
        ...
        for sig in &self.tx_signatures {
            if verify_legacy_zklogin_address {
                ...
                if let GenericSignature::ZkLoginAuthenticator(z) = sig {
                    mapping.insert(MysAddress::try_from_padded(&z.inputs)?, sig);
                };
            }
...
impl SenderSignedData {
    pub fn new(tx_data: TransactionData, tx_signatures: Vec<GenericSignature>) -> Self {
        Self(SizeOneVec::new(SenderSignedTransaction {
            intent_message: IntentMessage::new(Intent::mys_transaction(), tx_data),
            tx_signatures,
        }))
    }
    ...
    pub fn serialized_size(&self) -> MysResult<usize> {
        bcs::serialized_size(self).map_err(|e| MysError::TransactionSerializationError {
            error: e.to_string(),
        })
    }

    fn check_user_signature_protocol_compatibility(&self, config: &ProtocolConfig) -> MysResult {
    ...
    pub fn validity_check(
        ...
        if match &tx_data.expiration() {
            ...
            TransactionExpiration::ValidDuring { max_epoch, .. } => {
                max_epoch.map_or(false, |max| max < epoch)
            }
        } {
            return Err(MysError::TransactionExpired);
        }
...
impl<S> Envelope<SenderSignedData, S> {
    pub fn sender_address(&self) -> MysAddress {
        self.data().intent_message().value.sender()
    }

    pub fn gas_owner(&self) -> MysAddress {
...
impl Transaction {
    pub fn from_data_and_signer(
        ...
        let signatures = {
            let intent_msg = IntentMessage::new(Intent::mys_transaction(), &data);
            signers
                .into_iter()
        ...
        system_transaction
            ...
            .pipe(|data| {
                SenderSignedData::new_from_sender_signature(
                    data,
                    Ed25519MysSignature::from_bytes(&[0; Ed25519MysSignature::LENGTH])
                        .unwrap()
                        .into(),
                )
            })
...
impl SignedTransaction {
    ...
    pub fn try_into_verified_for_testing(
        ...
        committee: &Committee,
        verify_params: &VerifyParams,
    ) -> MysResult<VerifiedSignedTransaction> {
...
impl CertifiedTransaction {
    ...
    pub fn verify_committee_sigs_only(&self, committee: &Committee) -> MysResult {
        self.auth_sig().verify_secure(
            self.data(),
            Intent::mys_app(IntentScope::SenderSignedTransaction),
            committee,
...
impl InputObjectKind {
    ...
    pub fn is_mutable(&self) -> bool {
        match self {
            ...
            Self::SharedMoveObject { mutability, .. } => mutability.is_exclusive(),
        }
    }
}
...
pub enum ObjectReadResultKind {
    ...
    // A consensus object that has ended its stream (deleted at consensus).
...
impl ObjectReadResult {
    ...
    pub fn is_mutable(&self) -> bool {
        match (&self.input_object_kind, &self.object) {
            ...
            (InputObjectKind::SharedMoveObject { mutability, .. }, _) => mutability.is_exclusive(),
        }
    }
    ...
    pub fn to_shared_input(&self) -> Option<SharedInput> {
        match self.input_object_kind {
            ...
            InputObjectKind::SharedMoveObject { id, mutability, .. } => Some(match &self.object {
                ...
                ObjectReadResultKind::DeletedSharedObject(seq, digest) => {
                    SharedInput::Deleted((id, *seq, mutability.is_exclusive(), *digest))
                }
                ObjectReadResultKind::ObjectConsensusStreamEnded(seq, digest) => {
                    SharedInput::Deleted((id, *seq, mutability.is_exclusive(), *digest))
                }
...
// An InputObjects new-type that has been verified by mys-transaction-checks, and can be
// safely passed to execution.
pub struct CheckedInputObjects(InputObjects);

// DO NOT CALL outside of mys-transaction-checks, genesis, or replay.
//
// CheckedInputObjects should really be defined in mys-transaction-checks so that we can
// make public construction impossible. But we can't do that because it would result in circular
// dependencies.
impl CheckedInputObjects {
    // Only called by mys-transaction-checks.
...
impl InputObjects {
    ...
    pub fn contains_consensus_stream_ended_objects(&self) -> bool {
        self.objects
            .iter()
            .any(|obj| matches!(obj.object, ObjectReadResultKind::ObjectConsensusStreamEnded(_, _)))
    }
