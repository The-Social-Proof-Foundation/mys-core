...
use crate::base_types::{AuthorityName, EpochId, MysAddress};
...
pub trait EpochStartSystemStateTrait {
    ...
    fn get_validator_addresses(&self) -> Vec<MysAddress>;
...
/// This type captures the minimum amount of information from MysSystemState needed by a validator
/// to run the protocol. This allows us to decouple from the actual MysSystemState type, and hence
/// do not need to evolve it when we upgrade the MysSystemState type.
...
impl EpochStartSystemStateTrait for EpochStartSystemStateV1 {
    ...
    fn get_validator_addresses(&self) -> Vec<MysAddress> {
        self.active_validators
            .iter()
            .map(|validator| validator.mys_address)
            .collect()
    }
            ...
            .map(|validator| {
                (
                    ...
                    (
                        ...
                        NetworkMetadata {
                            network_address: validator.mys_net_address.clone(),
    ...
    fn get_consensus_committee(&self) -> ConsensusCommittee {
        ...
        // in the Mys committee returned from get_mys_committee().
        authorities.sort_by(|a1, a2| a1.authority_key.cmp(&a2.authority_key));

        for ((i, mysticeti_authority), mys_authority_name) in authorities
            .iter()
            .enumerate()
            .zip(self.get_mys_committee().names())
        {
            if mys_authority_name.0 != mysticeti_authority.authority_key.to_bytes() {
                error!(
                    "Mismatched authority order between Mys and Mysticeti! Index {}, Mysticeti authority {:?}\nMys authority name {}",
                    i, mysticeti_authority, mys_authority_name
                );
            }
        }
...
mod test {
    ...
    fn test_mys_and_mysticeti_committee_are_same() {
        ...
        for i in 0..10 {
            let (mys_address, protocol_key): (MysAddress, AuthorityKeyPair) = get_key_pair();
            ...
            active_validators.push(EpochStartValidatorInfoV1 {
                ...
                mys_net_address: Multiaddr::empty(),
        ...
        // WHEN
        let mys_committee = state.get_mys_committee();
        let consensus_committee = state.get_consensus_committee();

        // THEN
        // assert the validators details
        assert_eq!(mys_committee.num_members(), 10);
        assert_eq!(mys_committee.num_members(), consensus_committee.size());
        assert_eq!(
            mys_committee.validity_threshold(),
            consensus_committee.validity_threshold()
        );
        assert_eq!(
            mys_committee.quorum_threshold(),
        ...
        for (authority_index, consensus_authority) in consensus_committee.authorities() {
            let mys_authority_name = mys_committee
                .authority_by_index(authority_index.value() as u32)
                .unwrap();

            assert_eq!(
                consensus_authority.authority_key.to_bytes(),
                mys_authority_name.0,
                "Mysten & MYS committee member of same index correspond to different public key"
            );
            assert_eq!(
                consensus_authority.stake,
                mys_committee.weight(mys_authority_name),
                "Mysten & MYS committee member stake differs"
            );
        }
    }
}
