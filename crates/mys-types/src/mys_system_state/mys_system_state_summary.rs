...
/// This is the JSON-RPC type for the MYS system state object.
/// It flattens all fields to make them top-level fields such that it as minimum
/// dependencies to the internal data structures of the MYS system state type.

#[serde_as]
#[derive(Debug, Serialize, Deserialize, Clone, JsonSchema)]
#[serde(rename_all = "camelCase")]
pub struct MysSystemStateSummary {
    ...
    pub active_validators: Vec<MysValidatorSummary>,
    ...
    #[schemars(with = "Vec<(MysAddress, BigInt<u64>)>")]
    #[serde_as(as = "Vec<(_, Readable<BigInt<u64>, _>)>")]
    pub at_risk_validators: Vec<(MysAddress, u64)>,
    /// A map storing the records of validator reporting each other.
    pub validator_report_records: Vec<(MysAddress, Vec<MysAddress>)>,
}

impl MysSystemStateSummary {
    pub fn get_mys_committee_for_benchmarking(&self) -> CommitteeWithNetworkMetadata {
        let validators = self
            .active_validators
            .iter()
            .map(|validator| {
...
/// This is the JSON-RPC type for the MYS validator. It flattens all inner structures
/// to top-level fields so that they are decoupled from the internal definitions.
#[serde_as]
#[derive(Debug, Serialize, Deserialize, Clone, JsonSchema)]
#[serde(rename_all = "camelCase")]
pub struct MysValidatorSummary {
    // Metadata
    ...
    #[schemars(with = "BigInt<u64>")]
    #[serde_as(as = "Readable<BigInt<u64>, _>")]
    pub staking_pool_mys_balance: u64,
...
impl Default for MysSystemStateSummary {
    fn default() -> Self {
        Self {
            epoch: 0,
            protocol_version: 1,
...
impl Default for MysValidatorSummary {
    fn default() -> Self {
        Self {
            mys_address: MysAddress::default(),
    ...
    S: ObjectStore + ?Sized,
{
    ...
    let candidate_address: MysAddress = get_dynamic_field_from_store(
        &object_store,
