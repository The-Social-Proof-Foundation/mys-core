...
pub struct TestCheckpointDataBuilder {
    ...
    gas_map: HashMap<MysAddress, ObjectID>,
...
impl TestCheckpointDataBuilder {
    ...
    /// A proper MysAddress will be derived from it.
    ...
    /// The object will be created as a MYS coin object, with default balance,
    /// and the transaction sender as its owner.
    pub fn create_owned_object(self, object_idx: u64) -> Self {
        self.create_mys_object(object_idx, GAS_VALUE_FOR_TESTING)
    }
    ...
    /// Create a new MYS coin object in the transaction.
    /// `object_idx` is a convenient representation of the object's ID.
    /// `balance` is the amount of MYS to be created.
    pub fn create_mys_object(self, object_idx: u64, balance: u64) -> Self {
        let sender_idx = self
            .checkpoint_builder
            .next_transaction
            .as_ref()
    ...
    pub fn advance_epoch(&mut self, safe_mode: bool) -> CheckpointData {
        ...
        let events = if !safe_mode {
            ...
            let struct_tag = StructTag {
                ...
                module: ident_str!("mys_system_state_inner").to_owned(),
                name: ident_str!("SystemEpochInfoEvent").to_owned(),
                type_params: vec![],
            };
            Some(vec![Event::new(
                &MYS_SYSTEM_ADDRESS,
                ident_str!("mys_system_state_inner"),
    ...
    pub fn derive_address(address_idx: u8) -> MysAddress {
...
mod tests {
    ...
    fn test_mys_balance_transfer() {
        ...
        // Verify the original MYS coin now has 90 MIST after the transfer.
        assert!(tx.output_objects.iter().any(|obj| obj.id() == obj_id0
            && obj.is_gas_coin()
            && obj.data.try_as_move().unwrap().get_coin_value_unsafe() == 90));

        // Verify the split out MYS coin has 10 MIST.
