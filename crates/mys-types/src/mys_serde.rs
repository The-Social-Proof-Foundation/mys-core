...
use crate::{
    ...
    MYS_FRAMEWORK_ADDRESS, MYS_SYSTEM_ADDRESS, MYS_SYSTEM_STATE_ADDRESS,
};
...
pub struct MysBitmap;

impl SerializeAs<roaring::RoaringBitmap> for MysBitmap {
...
impl<'de> DeserializeAs<'de, roaring::RoaringBitmap> for MysBitmap {
...
pub struct MysStructTag;

impl SerializeAs<StructTag> for MysStructTag {
    fn serialize_as<S>(value: &StructTag, serializer: S) -> Result<S::Ok, S::Error>
    where
        S: Serializer,
    {
        let f = to_mys_struct_tag_string(value).map_err(S::Error::custom)?;
        f.serialize(serializer)
    }
}

const MYS_ADDRESSES: [AccountAddress; 7] = [
    AccountAddress::ZERO,
    AccountAddress::ONE,
...
pub fn to_mys_struct_tag_string(value: &StructTag) -> Result<String, fmt::Error> {
    ...
    // trim leading zeros if address is in MYS_ADDRESSES
    let address = if MYS_ADDRESSES.contains(&value.address) {
    ...
    if let Some(first_ty) = value.type_params.first() {
        write!(f, "<")?;
        write!(f, "{}", to_mys_type_tag_string(first_ty)?)?;
        for ty in value.type_params.iter().skip(1) {
            write!(f, ", {}", to_mys_type_tag_string(ty)?)?;
        }
        write!(f, ">")?;
    }
    Ok(f)
...
fn to_mys_type_tag_string(value: &TypeTag) -> Result<String, fmt::Error> {
    match value {
        TypeTag::Vector(t) => Ok(format!("vector<{}>", to_mys_type_tag_string(t)?)),
        TypeTag::Struct(s) => to_mys_struct_tag_string(s),
        _ => Ok(value.to_string()),
    }
}

impl<'de> DeserializeAs<'de, StructTag> for MysStructTag {
    fn deserialize_as<D>(deserializer: D) -> Result<StructTag, D::Error>
    where
        D: Deserializer<'de>,
    {
        let s = String::deserialize(deserializer)?;
        parse_mys_struct_tag(&s).map_err(D::Error::custom)
    }
}

pub struct MysTypeTag;

impl SerializeAs<TypeTag> for MysTypeTag {
    fn serialize_as<S>(value: &TypeTag, serializer: S) -> Result<S::Ok, S::Error>
    where
        S: Serializer,
    {
        let s = to_mys_type_tag_string(value).map_err(S::Error::custom)?;
        s.serialize(serializer)
    }
}

impl<'de> DeserializeAs<'de, TypeTag> for MysTypeTag {
    fn deserialize_as<D>(deserializer: D) -> Result<TypeTag, D::Error>
    where
        D: Deserializer<'de>,
    {
        let s = String::deserialize(deserializer)?;
        parse_mys_type_tag(&s).map_err(D::Error::custom)
    }
}
