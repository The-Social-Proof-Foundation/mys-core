...
pub fn generate_proof_of_possession(
    ...
    AuthoritySignature::new_secure(
        &IntentMessage::new(Intent::mys_app(IntentScope::ProofOfPossession), msg),
        &DEFAULT_EPOCH_ID,
        keypair,
    )
}
...
/// * accounts to interact with Mys.
/// * Currently we support eddsa and ecdsa on Mys.
///
...
impl MysKeyPair {
    pub fn public(&self) -> PublicKey {
        match self {
            MysKeyPair::Ed25519(kp) => PublicKey::Ed25519(kp.public().into()),
            MysKeyPair::Secp256k1(kp) => PublicKey::Secp256k1(kp.public().into()),
            MysKeyPair::Secp256r1(kp) => PublicKey::Secp256r1(kp.public().into()),
        }
    }

    pub fn copy(&self) -> Self {
        match self {
            MysKeyPair::Ed25519(kp) => kp.copy().into(),
            MysKeyPair::Secp256k1(kp) => kp.copy().into(),
            MysKeyPair::Secp256r1(kp) => kp.copy().into(),
        }
    }
}

impl Signer<Signature> for MysKeyPair {
    fn sign(&self, msg: &[u8]) -> Signature {
        match self {
            MysKeyPair::Ed25519(kp) => kp.sign(msg),
            MysKeyPair::Secp256k1(kp) => kp.sign(msg),
            MysKeyPair::Secp256r1(kp) => kp.sign(msg),
        }
    }
}

impl EncodeDecodeBase64 for MysKeyPair {
...
impl MysKeyPair {
    ...
    pub fn from_bytes(bytes: &[u8]) -> Result<Self, eyre::Report> {
        ...
        {
            Ok(x) => match x {
                SignatureScheme::ED25519 => Ok(MysKeyPair::Ed25519(Ed25519KeyPair::from_bytes(
                    bytes.get(1..).ok_or_else(|| eyre!("Invalid length"))?,
                )?)),
                SignatureScheme::Secp256k1 => {
                    Ok(MysKeyPair::Secp256k1(Secp256k1KeyPair::from_bytes(
                        bytes.get(1..).ok_or_else(|| eyre!("Invalid length"))?,
                    )?))
                }
                SignatureScheme::Secp256r1 => {
                    Ok(MysKeyPair::Secp256r1(Secp256r1KeyPair::from_bytes(
                        bytes.get(1..).ok_or_else(|| eyre!("Invalid length"))?,
                    )?))
                }
    ...
    pub fn to_bytes_no_flag(&self) -> Vec<u8> {
        match self {
            MysKeyPair::Ed25519(kp) => kp.as_bytes().to_vec(),
            MysKeyPair::Secp256k1(kp) => kp.as_bytes().to_vec(),
            MysKeyPair::Secp256r1(kp) => kp.as_bytes().to_vec(),
        }
    }
...
impl<'de> Deserialize<'de> for MysKeyPair {
        ...
        D: Deserializer<'de>,
    {
        ...
        MysKeyPair::decode_base64(&s).map_err(|e| Error::custom(e.to_string()))
    }
}
...
impl ZkLoginPublicIdentifier {
    ...
    pub fn new(iss: &str, address_seed: &Bn254FrElement) -> MysResult<Self> {
...
impl PublicKey {
    ...
    pub fn scheme(&self) -> SignatureScheme {
        match self {
            PublicKey::Ed25519(_) => Ed25519MysSignature::SCHEME,
            PublicKey::Secp256k1(_) => Secp256k1MysSignature::SCHEME,
            PublicKey::Secp256r1(_) => Secp256r1MysSignature::SCHEME,
            PublicKey::ZkLogin(_) => SignatureScheme::ZkLoginAuthenticator,
            PublicKey::Passkey(_) => SignatureScheme::PasskeyAuthenticator,
        }
    }

    pub fn from_zklogin_inputs(inputs: &ZkLoginInputs) -> MysResult<Self> {
...
pub trait MysAuthoritySignature {
    fn verify_secure<T>(
        &self,
...
impl MysAuthoritySignature for AuthoritySignature {
...
pub fn get_key_pair<KP: KeypairTraits>() -> (MysAddress, KP)
where
    <KP as KeypairTraits>::PubKey: MysPublicKey,
{
...
pub fn random_committee_key_pairs_of_size(size: usize) -> Vec<AuthorityKeyPair> {
    ...
    (0..size)
        .map(|_| {
            ...
            // keys in ConfigBuilder::build (mys-config/src/network_config_builder). This is because
...
pub fn deterministic_random_account_key() -> (MysAddress, AccountKeyPair) {
    let mut rng = StdRng::from_seed([0; 32]);
    get_key_pair_from_rng(&mut rng)
}

pub fn get_account_key_pair() -> (MysAddress, AccountKeyPair) {
    get_key_pair()
}

pub fn get_authority_key_pair() -> (MysAddress, AuthorityKeyPair) {
    get_key_pair()
}

/// Generate a keypair from the specified RNG (useful for testing with seedable rngs).
pub fn get_key_pair_from_rng<KP: KeypairTraits, R>(csprng: &mut R) -> (MysAddress, KP)
where
...
// TODO: C-GETTER
pub fn get_key_pair_from_bytes<KP: KeypairTraits>(bytes: &[u8]) -> MysResult<(MysAddress, KP)>
where
    <KP as KeypairTraits>::PubKey: MysPublicKey,
{
    ...
    if bytes.len() != priv_length + pub_key_length {
        return Err(MysError::KeyConversionError(format!(
    ...
    let sk = <KP as KeypairTraits>::PrivKey::from_bytes(
        bytes
            .get(..priv_length)
            .ok_or(MysError::InvalidPrivateKey)?,
    )
    .map_err(|_| MysError::InvalidPrivateKey)?;
    let kp: KP = sk.into();
...
pub enum Signature {
    Ed25519MysSignature,
    Secp256k1MysSignature,
    Secp256r1MysSignature,
}
...
impl AsRef<[u8]> for Signature {
    fn as_ref(&self) -> &[u8] {
        match self {
            Signature::Ed25519MysSignature(sig) => sig.as_ref(),
            Signature::Secp256k1MysSignature(sig) => sig.as_ref(),
            Signature::Secp256r1MysSignature(sig) => sig.as_ref(),
        }
    }
}
impl AsMut<[u8]> for Signature {
    fn as_mut(&mut self) -> &mut [u8] {
        match self {
            Signature::Ed25519MysSignature(sig) => sig.as_mut(),
            Signature::Secp256k1MysSignature(sig) => sig.as_mut(),
            Signature::Secp256r1MysSignature(sig) => sig.as_mut(),
        }
    }
}
...
impl ToFromBytes for Signature {
    fn from_bytes(bytes: &[u8]) -> Result<Self, FastCryptoError> {
        match bytes.first() {
            Some(x) => {
                if x == &Ed25519MysSignature::SCHEME.flag() {
                    Ok(<Ed25519MysSignature as ToFromBytes>::from_bytes(bytes)?.into())
                } else if x == &Secp256k1MysSignature::SCHEME.flag() {
                    Ok(<Secp256k1MysSignature as ToFromBytes>::from_bytes(bytes)?.into())
                } else if x == &Secp256r1MysSignature::SCHEME.flag() {
                    Ok(<Secp256r1MysSignature as ToFromBytes>::from_bytes(bytes)?.into())
                } else {
                    Err(FastCryptoError::InvalidInput)
                }
            }
...
impl MysPublicKey for BLS12381PublicKey {
    const SIGNATURE_SCHEME: SignatureScheme = SignatureScheme::BLS12381;
}

//
// Ed25519 Mys Signature port
//
...
#[as_ref(forward)]
#[as_mut(forward)]
pub struct Ed25519MysSignature(
    #[schemars(with = "Base64")]
    #[serde_as(as = "Readable<Base64, Bytes>")]
...
impl Default for Ed25519MysSignature {
    fn default() -> Self {
        Self([0; Ed25519PublicKey::LENGTH + Ed25519Signature::LENGTH + 1])
    }
}

impl MysSignatureInner for Ed25519MysSignature {
...
impl MysPublicKey for Ed25519PublicKey {
    const SIGNATURE_SCHEME: SignatureScheme = SignatureScheme::ED25519;
}

impl ToFromBytes for Ed25519MysSignature {
    fn from_bytes(bytes: &[u8]) -> Result<Self, FastCryptoError> {
...
impl Signer<Signature> for Ed25519KeyPair {
    fn sign(&self, msg: &[u8]) -> Signature {
        Ed25519MysSignature::new(self, msg).into()
    }
}

//
// Secp256k1 Mys Signature port
//
#[serde_as]
#[derive(Clone, Debug, Serialize, Deserialize, JsonSchema, PartialEq, Eq, Hash, AsRef, AsMut)]
#[as_ref(forward)]
#[as_mut(forward)]
pub struct Secp256k1MysSignature(
    #[schemars(with = "Base64")]
    #[serde_as(as = "Readable<Base64, Bytes>")]
    [u8; Secp256k1PublicKey::LENGTH + Secp256k1Signature::LENGTH + 1],
);

impl MysSignatureInner for Secp256k1MysSignature {
...
impl MysPublicKey for Secp256k1PublicKey {
    const SIGNATURE_SCHEME: SignatureScheme = SignatureScheme::Secp256k1;
}

impl ToFromBytes for Secp256k1MysSignature {
    fn from_bytes(bytes: &[u8]) -> Result<Self, FastCryptoError> {
        if bytes.len() != Self::LENGTH {
            return Err(FastCryptoError::InputLengthWrong(Self::LENGTH));
        }
...
impl Signer<Signature> for Secp256k1KeyPair {
    fn sign(&self, msg: &[u8]) -> Signature {
        Secp256k1MysSignature::new(self, msg).into()
    }
}

//
// Secp256r1 Mys Signature port
//
#[serde_as]
#[derive(Clone, Debug, Serialize, Deserialize, JsonSchema, PartialEq, Eq, Hash, AsRef, AsMut)]
#[as_ref(forward)]
#[as_mut(forward)]
pub struct Secp256r1MysSignature(
    #[schemars(with = "Base64")]
    #[serde_as(as = "Readable<Base64, Bytes>")]
    [u8; Secp256r1PublicKey::LENGTH + Secp256r1Signature::LENGTH + 1],
);

impl MysSignatureInner for Secp256r1MysSignature {
...
impl MysPublicKey for Secp256r1PublicKey {
    const SIGNATURE_SCHEME: SignatureScheme = SignatureScheme::Secp256r1;
}

impl ToFromBytes for Secp256r1MysSignature {
    fn from_bytes(bytes: &[u8]) -> Result<Self, FastCryptoError> {
        if bytes.len() != Self::LENGTH {
            return Err(FastCryptoError::InputLengthWrong(Self::LENGTH));
        }
...
impl Signer<Signature> for Secp256r1KeyPair {
    fn sign(&self, msg: &[u8]) -> Signature {
        Secp256r1MysSignature::new(self, msg).into()
    }
}
...
//
pub trait MysSignatureInner: Sized + ToFromBytes + PartialEq + Eq + Hash {
    type Sig: Authenticator<PubKey = Self::PubKey>;
    type PubKey: VerifyingKey<Sig = Self::Sig> + MysPublicKey;
    ...
    fn get_verification_inputs(&self) -> MysResult<(Self::Sig, Self::PubKey)> {
        let pk = Self::PubKey::from_bytes(self.public_key_bytes())
            .map_err(|_| MysError::KeyConversionError("Invalid public key".to_string()))?;
    ...
    fn new(kp: &Self::KeyPair, message: &[u8]) -> Self {
        ...
        signature_bytes
            .extend_from_slice(&[<Self::PubKey as MysPublicKey>::SIGNATURE_SCHEME.flag()]);
...
pub trait MysPublicKey: VerifyingKey {
    const SIGNATURE_SCHEME: SignatureScheme;
}

#[enum_dispatch(Signature)]
pub trait MysSignature: Sized + ToFromBytes {
...
impl<S: MysSignatureInner + Sized> MysSignature for S {
    fn signature_bytes(&self) -> &[u8] {
        ...
        T: Serialize,
    {
        ...
        match scheme {
            ...
            _ => {
                let address = MysAddress::from(pk);
                if author != address {
                    return Err(MysError::IncorrectSigner {
                        error: format!(
...
#[serde_as]
#[derive(Clone, Debug, Serialize, Deserialize, JsonSchema)]
pub struct MysAuthorityStrongQuorumSignInfo {
    pub epoch: EpochId,
...
impl From<&AuthorityStrongQuorumSignInfo> for MysAuthorityStrongQuorumSignInfo {
...
impl TryFrom<&MysAuthorityStrongQuorumSignInfo> for AuthorityStrongQuorumSignInfo {
    type Error = FastCryptoError;

    fn try_from(info: &MysAuthorityStrongQuorumSignInfo) -> Result<Self, Self::Error> {
        Ok(Self {
            epoch: info.epoch,
...
//
// see also https://github.com/MystenLabs/mys/issues/266
    ...
    for AuthorityQuorumSignInfo<STRONG_THRESHOLD>
{
    ...
    fn add_to_verification_obligation<'a>(
        ...
        obligation
            .signatures
            .get_mut(message_index)
            .ok_or(MysError::InvalidAuthenticator)?
        ...
        for authority_index in self.signers_map.iter() {
            let authority = committee
                ...
                .ok_or_else(|| MysError::UnknownSigner {
                    signer: None,
        ...
        fp_ensure!(
            ...
            MysError::CertificateRequiresQuorum
        );
...
impl<const STRONG_THRESHOLD: bool> AuthorityQuorumSignInfo<STRONG_THRESHOLD> {
    ...
    pub fn authorities<'a>(
        &'a self,
        committee: &'a Committee,
    ) -> impl Iterator<Item = MysResult<&'a AuthorityName>> {
...
/// * We use `BCS` to generate canonical bytes ttttable for hashing and signing.
///
/// # Safety
...
impl<'a> VerificationObligation<'a> {
    ...
    pub fn verify_all(self) -> MysResult<()> {
...
pub mod bcs_signable_test {
        ...
        T: super::bcs_signable::BcsSignable,
    {
        ...
        let idx = obligation.add_message(
            value,
            0,
            Intent::mys_app(IntentScope::SenderSignedTransaction),
        );
        (obligation, idx)
    }
}
...
pub enum SignatureScheme {
    ...
    Secp256k1,
    Secp256r1,
    BLS12381, // This is currently not supported for user Mys Address.
    MultiSig,
...
impl SignatureScheme {
    pub fn flag(&self) -> u8 {
        match self {
            ...
            SignatureScheme::BLS12381 => 0x04, // This is currently not supported for user Mys Address.
            SignatureScheme::ZkLoginAuthenticator => 0x05,
            SignatureScheme::PasskeyAuthenticator => 0x06,
        }
    }

    pub fn from_flag(flag: &str) -> Result<SignatureScheme, MysError> {
        let byte_int = flag
            .parse::<u8>()
            .map_err(|_| MysError::KeyConversionError("Invalid key scheme".to_string()))?;
        Self::from_flag_byte(&byte_int)
    }

    pub fn from_flag_byte(byte_int: &u8) -> Result<SignatureScheme, MysError> {
        match byte_int {
            ...
            _ => Err(MysError::KeyConversionError(
