...
use crate::{
    error::MysResult, object::Owner, storage::ObjectStore, MYS_FRAMEWORK_ADDRESS,
...
pub const RESOLVED_MYS_RANDOMNESS_STATE: (&AccountAddress, &IdentStr, &IdentStr) = (
...
pub fn is_mutable_random(view: &CompiledModule, s: &SignatureToken) -> bool {
    match s {
        ...
        SignatureToken::Datatype(idx) => {
            resolve_struct(view, *idx) == RESOLVED_MYS_RANDOMNESS_STATE
        }
        _ => false,
    }
}
