...
pub const BRIDGE_CREATE_ADD_TOKEN_ON_MYS_MESSAGE_FUNCTION_NAME: &IdentStr =
    ident_str!("create_add_tokens_on_mys_message");
...
impl BridgeChainId {
    pub fn is_mys_chain(&self) -> bool {
        matches!(
            self,
            BridgeChainId::MysMainnet | BridgeChainId::MysTestnet | BridgeChainId::MysCustom
        )
    }
}
...
pub fn get_bridge_obj_initial_shared_version(
    ...
    Ok(object_store
        .get_object(&MYS_BRIDGE_OBJECT_ID)
...
/// Rust version of the Move mys::bridge::Bridge type
...
pub trait BridgeTrait {
    ...
    fn frozen(&self) -> bool;
    fn try_into_bridge_summary(self) -> MysResult<BridgeSummary>;
}
...
pub fn get_bridge_wrapper(object_store: &dyn ObjectStore) -> Result<BridgeWrapper, MysError> {
    let wrapper = object_store
        .get_object(&MYS_BRIDGE_OBJECT_ID)
        // Don't panic here on None because object_store is a generic store.
        .ok_or_else(|| MysError::MysBridgeReadError("BridgeWrapper object not found".to_owned()))?;
    let move_object = wrapper.data.try_as_move().ok_or_else(|| {
        MysError::MysBridgeReadError("BridgeWrapper object must be a Move object".to_owned())
    })?;
    let result = bcs::from_bytes::<BridgeWrapper>(move_object.contents())
        .map_err(|err| MysError::MysBridgeReadError(err.to_string()))?;
    Ok(result)
}

pub fn get_bridge(object_store: &dyn ObjectStore) -> Result<Bridge, MysError> {
    ...
    match version {
        1 => {
            let result: BridgeInnerV1 = get_dynamic_field_from_store(object_store, id, &version)
                .map_err(|err| {
                    MysError::MysBridgeReadError(format!(
        ...
        _ => Err(MysError::MysBridgeReadError(format!(
            "Unsupported MysBridge version: {}",
            version
        ))),
    }
}
...
pub struct MoveTypeBridgeCommittee {
    ...
    pub member_registrations: VecMap<MysAddress, MoveTypeCommitteeMemberRegistration>,
...
pub struct BridgeCommitteeSummary {
    ...
    pub member_registration: Vec<(MysAddress, MoveTypeCommitteeMemberRegistration)>,
    pub last_committee_update_epoch: u64,
}
...
pub fn is_bridge_committee_initiated(object_store: &dyn ObjectStore) -> MysResult<bool> {
    match get_bridge(object_store) {
        Ok(bridge) => Ok(!bridge.committee().members.contents.is_empty()),
        Err(MysError::MysBridgeReadError(..)) => Ok(false),
        Err(other) => Err(other),
    }
}
