...
use crate::{
    ...
    crypto::DefaultHash,
    error::{ExecutionError, ExecutionErrorKind, MysError, MysResult},
    id::{ID, UID},
...
/// Rust representation of upgrade policy constants in `mys::package`.
#[repr(u8)]
...
/// Rust representation of `mys::package::UpgradeCap`.
...
/// Rust representation of `mys::package::UpgradeTicket`.
#[derive(Debug, Serialize, Deserialize)]
...
/// Rust representation of `mys::package::UpgradeReceipt`.
#[derive(Debug, Serialize, Deserialize)]
...
impl MovePackage {
    ...
    ) -> MysResult<CompiledModule> {
        // TODO use the session's cache
        let bytes = self
            .serialized_module_map()
            .get(module.as_str())
            .ok_or_else(|| MysError::ModuleNotFound {
    ...
    pub fn normalize(
        &self,
        binary_config: &BinaryConfig,
    ) -> MysResult<BTreeMap<String, normalized::Module>> {
