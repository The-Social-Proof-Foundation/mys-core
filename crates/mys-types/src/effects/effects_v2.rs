...
use crate::effects::{InputConsensusObject, InputSharedObject, TransactionEffectsAPI};
...
impl TransactionEffectsAPI for TransactionEffectsV2 {
    ...
    fn input_shared_objects(&self) -> Vec<InputSharedObject> {
        // Effects v2 uses consensus objects instead of shared objects
        // Convert consensus objects to shared objects for compatibility
        self.input_consensus_objects()
            .into_iter()
            .filter_map(|obj| match obj {
                InputConsensusObject::Mutate(oref) => Some(InputSharedObject::Mutate(oref)),
                InputConsensusObject::ReadOnly(oref) => Some(InputSharedObject::ReadOnly(oref)),
    ...
    fn unchanged_shared_objects(&self) -> Vec<(ObjectID, UnchangedSharedKind)> {
        // Effects v2 uses consensus objects, not shared objects
        vec![]
    }
        ...
        let unchanged_consensus_objects = shared_objects
            ...
            .filter_map(|shared_input| match shared_input {
                ...
                SharedInput::Deleted(_) => {
                    // Deleted shared objects are handled elsewhere
                    None
                }
            })
            .chain(
