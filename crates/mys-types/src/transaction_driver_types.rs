...
use crate::error::{ErrorCategory, MysError};
...
pub enum TransactionSubmissionError {
    ...
    TransactionDriverInternalError(MysError),
    #[error("Invalid user signature: {0}.")]
    InvalidUserSignature(MysError),
    #[error(
...
pub type GroupedErrors = Vec<(MysError, StakeUnit, Vec<ConciseAuthorityPublicKeyBytes>)>;
