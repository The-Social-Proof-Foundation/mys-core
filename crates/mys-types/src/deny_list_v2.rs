...
use crate::{MoveTypeTagTrait, MYS_DENY_LIST_OBJECT_ID, MYS_FRAMEWORK_PACKAGE_ID};
...
fn check_new_regulated_coin_owners(
    new_regulated_coin_owners: BTreeMap<String, (Config, BTreeSet<MysAddress>)>,
    cur_epoch: EpochId,
    object_store: &dyn ObjectStore,
...
pub fn get_per_type_coin_deny_list_v2(
    ...
    let config: Config =
        get_dynamic_field_from_store(object_store, MYS_DENY_LIST_OBJECT_ID, &config_key).ok()?;
    Some(config)
}
