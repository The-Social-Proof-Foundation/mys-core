...
use mys_types::messages_checkpoint::EndOfEpochData;
use mys_types::messages_checkpoint::FullCheckpointContents;
use mys_types::messages_checkpoint::VerifiedCheckpoint;
use mys_types::messages_checkpoint::VerifiedCheckpointContents;
use mys_types::object::Object;
use mys_types::storage::error::Error as StorageError;
use mys_types::storage::error::Result;
use mys_types::storage::AccountOwnedObjectInfo;
use mys_types::storage::CoinInfo;
use mys_types::storage::DynamicFieldIndexInfo;
use mys_types::storage::DynamicFieldKey;
use mys_types::storage::ObjectStore;
use mys_types::storage::RpcIndexes;
...
impl ReadStore for RocksDbStore {
            ...
            .and_then(|contents| {
                ...
                for tx in contents.iter() {
                    if let (Some(t), Some(e)) = (
                        ...
                        transactions.push(mys_types::base_types::ExecutionData::new(
                            (*t).clone().into_inner(),
                            e,
                        ))
                    } else {
    ...
    fn get_latest_checkpoint(&self) -> mys_types::storage::error::Result<VerifiedCheckpoint> {
        self.checkpoint_store
            ...
            .expect("db error")
            .ok_or_else(|| {
                mys_types::storage::error::Error::missing("unable to get latest checkpoint")
            })
    }
...
impl ObjectStore for RocksDbStore {
    fn get_object(&self, object_id: &mys_types::base_types::ObjectID) -> Option<Object> {
        self.cache_traits.object_store.get_object(object_id)
    }

    fn get_object_by_key(
        &self,
        object_id: &mys_types::base_types::ObjectID,
...
impl WriteStore for RocksDbStore {
    ...
    ) -> Result<(), mys_types::storage::error::Error> {
    ...
    fn update_highest_synced_checkpoint(
        ...
        self.checkpoint_store
            ...
            .map_err(mys_types::storage::error::Error::custom)?;
...
impl RestReadStore {
    ...
    fn index(&self) -> mys_types::storage::error::Result<&RpcIndexStore> {
        self.state
            .rpc_index
            .as_deref()
            .ok_or_else(|| mys_types::storage::error::Error::custom("rest index store is disabled"))
    }
}
...
impl RpcStateReader for RestReadStore {
    ...
    fn get_chain_identifier(&self) -> Result<mys_types::digests::ChainIdentifier> {
...
impl RpcIndexes for RpcIndexStore {
    ...
    ) -> mys_types::storage::error::Result<Option<CheckpointSequenceNumber>> {
    ...
    fn dynamic_field_iter(
        ...
        parent: ObjectID,
        cursor: Option<ObjectID>,
    ) -> mys_types::storage::error::Result<
    ...
    fn get_coin_info(
        &self,
        coin_type: &StructTag,
    ) -> mys_types::storage::error::Result<Option<CoinInfo>> {
