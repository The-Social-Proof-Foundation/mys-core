...
use crate::authority::authority_store::{ExecutionLockWriteGuard, MysLockResult};
...
use mys_types::digests::{TransactionDigest, TransactionEffectsDigest, TransactionEventsDigest};
...
/// Should only be used for mys-tool or tests. Nodes must use build_execution_cache which
...
pub trait ObjectCacheRead: Send + Sync {
    fn get_package_object(&self, id: &ObjectID) -> MysResult<Option<PackageObject>>;
        ...
        for (object_opt, object_ref) in objects.into_iter().zip(object_refs) {
            match object_opt {
                None => {
                    ...
                    return Err(MysError::UserInputError { error });
                }
                Some(object) => {
                    result.push(object);
                }
    ...
    fn get_lock(&self, obj_ref: ObjectRef, epoch_store: &AuthorityPerEpochStore) -> MysLockResult;

    // This method is considered "private" - only used by multi_get_objects_with_more_accurate_error_return
    fn _get_live_objref(&self, object_id: ObjectID) -> MysResult<ObjectRef>;
    ...
    fn check_owned_objects_are_live(&self, owned_object_refs: &[ObjectRef]) -> MysResult;

    fn get_mys_system_state_object_unsafe(&self) -> MysResult<MysSystemState>;

    fn get_bridge_object_unsafe(&self) -> MysResult<Bridge>;
...
pub trait TransactionCacheRead: Send + Sync {
    ...
    fn get_transactions_and_serialized_sizes(
        &self,
        digests: &[TransactionDigest],
    ) -> MysResult<Vec<Option<(VerifiedTransaction, usize)>>> {
...
pub trait ExecutionCacheReconfigAPI: Send + Sync {
    ...
    fn expensive_check_mys_conservation(
        &self,
        old_epoch_store: &AuthorityPerEpochStore,
    ) -> MysResult;

    fn checkpoint_db(&self, path: &Path) -> MysResult;
    ...
    ($implementor: ident) => {
        ...
        impl BackingPackageStore for $implementor {
            fn get_package_object(
                &self,
                package_id: &ObjectID,
            ) -> MysResult<Option<PackageObject>> {
    ...
    ($implementor: ident) => {
        ...
        impl ExecutionCacheReconfigAPI for $implementor {
            ...
            fn expensive_check_mys_conservation(
                ...
                self.store
                    .expensive_check_mys_conservation(self, old_epoch_store)
            }

            fn checkpoint_db(&self, path: &std::path::Path) -> MysResult {
