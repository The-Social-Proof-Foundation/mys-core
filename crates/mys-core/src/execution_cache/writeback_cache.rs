...
use mys_types::bridge::{get_bridge, Bridge};
...
impl ObjectCacheRead for WritebackCache {
    ...
    fn get_lock(&self, obj_ref: ObjectRef, epoch_store: &AuthorityPerEpochStore) -> MysLockResult {
        ...
        match self.get_object_by_id_cache_only("lock", &obj_ref.0) {
            ...
            CacheResult::NegativeHit => {
                Err(MysError::from(UserInputError::ObjectNotFound {
                    object_id: obj_ref.0,
...
fn do_fallback_lookup_fallible<K: Copy, V: Default + Clone>(
    keys: &[K],
    get_cached_key: impl Fn(&K) -> MysResult<CacheResult<V>>,
    multiget_fallback: impl Fn(&[K]) -> MysResult<Vec<V>>,
) -> MysResult<Vec<V>> {
