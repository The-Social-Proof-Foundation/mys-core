...
use mys_types::{
    base_types::{random_object_ref, MysAddress},
...
impl Scenario {
    ...
    fn new_outputs() -> TransactionOutputs {
        ...
        let (sender, keypair): (MysAddress, AccountKeyPair) = get_key_pair_from_rng(&mut rng);
        let (receiver, _): (MysAddress, AccountKeyPair) = get_key_pair_from_rng(&mut rng);
    ...
    pub async fn commit(&mut self, tx: TransactionDigest) -> MysResult {
        let res = self
            .cache()
