...
impl AuthorityAPI for LocalAuthorityClient {
    async fn handle_transaction(
        ...
        if self.fault_config.fail_before_handle_transaction {
            return Err(MysError::from("Mock error before handle_transaction"));
        }
    ...
    async fn handle_system_state_object(
        ...
        self.state.get_mys_system_state_object_for_testing()
    }
}
...
pub struct MockAuthorityApi {
    delay: Duration,
    count: Arc<Mutex<u32>>,
    handle_object_info_request_result: Option<MysResult<ObjectInfoResponse>>,
}
...
impl MockAuthorityApi {
    ...
    pub fn set_handle_object_info_request(&mut self, result: MysResult<ObjectInfoResponse>) {
...
pub struct HandleTransactionTestAuthorityClient {
    pub tx_info_resp_to_return: MysResult<HandleTransactionResponse>,
    pub cert_resp_to_return: MysResult<HandleCertificateResponseV2>,
...
impl HandleTransactionTestAuthorityClient {
    pub fn new() -> Self {
        Self {
            tx_info_resp_to_return: Err(MysError::Unknown("".to_string())),
            cert_resp_to_return: Err(MysError::Unknown("".to_string())),
    ...
    pub fn set_tx_info_response_error(&mut self, error: MysError) {
        self.tx_info_resp_to_return = Err(error);
    }

    pub fn reset_tx_info_response(&mut self) {
        self.tx_info_resp_to_return = Err(MysError::Unknown("".to_string()));
    }
    ...
    pub fn set_cert_resp_to_return_error(&mut self, error: MysError) {
        self.cert_resp_to_return = Err(error);
    }

    pub fn reset_cert_response(&mut self) {
        self.cert_resp_to_return = Err(MysError::Unknown("".to_string()));
    }
