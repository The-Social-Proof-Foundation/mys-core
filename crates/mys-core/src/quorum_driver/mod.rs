...
impl<A: Clone> QuorumDriver<A> {
    ...
    async fn enqueue_task(&self, task: QuorumDriverTask) -> MysResult<()> {
        self.task_sender
            .send(task.clone())
            .await
            ...
            .map_err(|e| MysError::QuorumDriverCommunicationError {
                error: e.to_string(),
            })
    }
    ...
    A: AuthorityAPI + Send + Sync + 'static + Clone,
{
    ...
    pub async fn submit_transaction(
        &self,
        request: ExecuteTransactionRequestV3,
    ) -> MysResult<Registration<TransactionDigest, QuorumDriverResult>> {
