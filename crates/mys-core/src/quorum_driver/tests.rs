...
use mys_types::crypto::{deterministic_random_account_key, get_key_pair, AccountKeyPair};
use mys_types::effects::TransactionEffectsAPI;
use mys_types::object::{generate_test_gas_objects, Object};
...
async fn test_quorum_driver_object_locked() -> Result<(), anyhow::Error> {
    ...
    let (sender, keypair): (MysAddress, AccountKeyPair) = deterministic_random_account_key();
