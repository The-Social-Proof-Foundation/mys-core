...
use mys_types::messages_checkpoint::{CertifiedCheckpointSummary, CheckpointCommitment};
use mys_types::messages_consensus::ConsensusDeterminedVersionAssignments;
use mys_types::messages_grpc::ObjectInfoRequestKind;
use mys_types::move_package::TypeOrigin;
use mys_types::object::Object;
use mys_types::transaction::{GenesisObject, SenderSignedData, TransactionData};
...
fn get_registry() -> Result<Registry> {
    ...
    let kp1: MysKeyPair =
        MysKeyPair::Ed25519(get_key_pair_from_rng(&mut StdRng::from_seed([0; 32])).1);
    let kp2: MysKeyPair =
        MysKeyPair::Secp256k1(get_key_pair_from_rng(&mut StdRng::from_seed([0; 32])).1);
    let kp3: MysKeyPair =
        MysKeyPair::Secp256r1(get_key_pair_from_rng(&mut StdRng::from_seed([0; 32])).1);
    ...
    // ObjectID and MysAddress are the same length
    ...
    let struct_tag = StructTag::from_str("0x2::coin::Coin<0x2::mys::MYS>").unwrap();
    ...
    tracer
        .trace_type::<base_types::MysAddress>(&samples)
        .unwrap();
    ...
    tracer
        .trace_type::<mys_types::object::Authenticator>(&samples)
        .unwrap();
...
#[derive(Debug, Parser)]
#[clap(
    name = "Mys format generator",
    about = "Trace serde (de)serialization to generate format descriptions for Mys types"
)]
struct Options {
...
const FILE_PATH: &str = "mys-core/tests/staged/mys.yaml";
