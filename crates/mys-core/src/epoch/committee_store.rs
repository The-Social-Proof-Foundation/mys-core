...
impl CommitteeStore {
    ...
    pub fn init_genesis_committee(&self, genesis_committee: Committee) -> MysResult {
    ...
    pub fn insert_new_committee(&self, new_committee: &Committee) -> MysResult {
    ...
    pub fn get_committee(&self, epoch_id: &EpochId) -> MysResult<Option<Arc<Committee>>> {
    ...
    // todo - make use of cache or remove this method
    pub fn get_or_latest_committee(&self, epoch: Option<EpochId>) -> MysResult<Committee> {
        Ok(match epoch {
            Some(epoch) => self
                .get_committee(&epoch)?
                .ok_or(MysError::MissingCommitteeAtEpoch(epoch))
