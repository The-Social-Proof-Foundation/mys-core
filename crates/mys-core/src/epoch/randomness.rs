...
use mys_types::committee::{Committee, EpochId, StakeUnit};
use mys_types::crypto::{AuthorityKeyPair, RandomnessRound};
...
impl RandomnessManager {
    ...
    pub async fn start_dkg(&mut self) -> MysResult {
    ...
    pub(crate) fn reserve_next_randomness(
        ...
        output: &mut ConsensusCommitOutput,
    ) -> MysResult<Option<RandomnessRound>> {
        let epoch_store = self.epoch_store()?;
    ...
    fn epoch_store(&self) -> MysResult<Arc<AuthorityPerEpochStore>> {
        self.epoch_store
            .upgrade()
            .ok_or(MysError::EpochEnded(self.epoch))
    }
...
impl RandomnessReporter {
    ...
    /// the round.
    pub fn notify_randomness_in_checkpoint(&self, round: RandomnessRound) -> MysResult {
    ...
    async fn test_dkg(version: u64) {
        ...
        for validator in network_config.validator_configs.iter() {
            ...
            let randomness_manager = RandomnessManager::try_new(
                ...
                mys_network::randomness::Handle::new_stub(),
