...
use mys_types::messages_consensus::{ConsensusTransaction, ConsensusTransactionKind};
...
use mys_types::traffic_control::{ClientIdSource, PolicyConfig, RemoteFirewallConfig, Weight};
use mys_types::{effects::TransactionEffectsAPI, messages_grpc::HandleTransactionRequestV2};
use mys_types::{error::*, transaction::*};
use mys_types::{
    fp_ensure,
    messages_checkpoint::{
...
impl ValidatorService {
    ...
    // `handle_transaction_v2()` and `MysTxValidator::vote_transaction()` as well.
    async fn handle_transaction(
        ...
        let info = state
            ...
            .await
            .tap_err(|e| {
                if let MysError::ValidatorHaltedAtEpochEnd = e {
    ...
    async fn handle_certificates(
        ...
        fp_ensure!(
            ...
            MysError::FullNodeCantHandleCertificate.into()
        );
    ...
    async fn handle_submit_to_consensus(
        ...
        let consensus_transactions: Vec<_> = consensus_transactions.into();
        {
            ...
            if !reconfiguration_lock.should_accept_user_certs() {
                ...
                return Err(MysError::ValidatorHaltedAtEpochEnd.into());
            }
        ...
        let responses = futures::future::try_join_all(consensus_transactions.into_iter().map(
            |tx| async move {
                ...
                Ok::<_, MysError>(HandleTransactionResponseV2 {
                    effects,
                    events,
                    input_objects,
                    output_objects,
...
impl ValidatorService {
    ...
    async fn soft_bundle_validity_check(
        ...
        // Enforce these checks per [SIP-19](https://github.com/mys-foundation/sips/blob/main/sips/sip-19.md):
    ...
    async fn handle_soft_bundle_certificates_v3_impl(
        ...
        let certificates = NonEmpty::from_vec(request.certificates)
            .ok_or_else(|| MysError::NoCertificateProvidedError)?;
    ...
    ) -> WrappedServiceResponse<MysSystemState> {
        let response = self
            .state
    ...
    async fn handle_traffic_req(&self, client: Option<IpAddr>) -> Result<(), tonic::Status> {
        if let Some(traffic_controller) = &self.traffic_controller {
            if !traffic_controller.check(&client, &None).await {
                // Entity in blocklist
                Err(tonic::Status::from_error(MysError::TooManyRequests.into()))
            } else {
                Ok(())
            }
        } else {
    ...
    fn handle_traffic_resp<T>(
        ...
        let (error, spam_weight, unwrapped_response) = match wrapped_response {
            ...
            Err(status) => (
                Some(MysError::from(status.clone())),
                Weight::zero(),
                Err(status.clone()),
            ),
        };
...
fn normalize(err: MysError) -> Weight {
    match err {
        MysError::UserInputError {
        ...
        | MysError::WrongEpoch { .. } => Weight::one(),
        _ => Weight::zero(),
    }
}
...
impl Validator for ValidatorService {
    ...
    ) -> Result<tonic::Response<MysSystemState>, tonic::Status> {
