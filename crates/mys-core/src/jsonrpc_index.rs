...
use mys_json_rpc_types::{MysObjectDataFilter, TransactionFilter};
...
use mys_types::dynamic_field::{self, DynamicFieldInfo};
use mys_types::effects::TransactionEvents;
use mys_types::error::{MysError, MysResult, UserInputError};
use mys_types::inner_temporary_store::TxCoins;
...
type OwnerIndexKey = (MysAddress, ObjectID);
type CoinIndexKey = (MysAddress, String, ObjectID);
...
impl CoinIndexKey2 {
    ...
    pub fn new(owner: MysAddress, coin_type: String, balance: u64, object_id: ObjectID) -> Self {
        Self {
            owner,
            coin_type,
...
pub struct IndexStoreCaches {
    per_coin_type_balance: ShardedLruCache<(MysAddress, TypeTag), MysResult<TotalBalance>>,
    all_balances: ShardedLruCache<MysAddress, MysResult<Arc<HashMap<TypeTag, TotalBalance>>>>,
    locks: MutexTable<MysAddress>,
}

#[derive(Default)]
pub struct IndexStoreCacheUpdates {
    _locks: Vec<OwnedMutexGuard<()>>,
    per_coin_type_balance_changes: Vec<((MysAddress, TypeTag), MysResult<TotalBalance>)>,
    all_balance_changes: Vec<(MysAddress, MysResult<Arc<AllBalance>>)>,
}
...
pub struct IndexStoreTables {
    ...
    /// Index from mys address to transactions initiated by that address.
    #[default_options_override_fn = "transactions_from_addr_table_default_config"]
    transactions_from_addr: DBMap<(MysAddress, TxSequenceNumber), TransactionDigest>,

    /// Index from mys address to transactions that were sent to that address.
    #[default_options_override_fn = "transactions_to_addr_table_default_config"]
    transactions_to_addr: DBMap<(MysAddress, TxSequenceNumber), TransactionDigest>,
    ...
    /// composite key of the MysAddress of their owner and the object ID of the object.
    ...
    #[default_options_override_fn = "index_table_default_config"]
    event_by_sender: DBMap<(MysAddress, EventId), EventIndex>,
    #[default_options_override_fn = "index_table_default_config"]
...
impl IndexStore {
    ...
    pub fn index_coin(
    ...
    ) -> MysResult<IndexStoreCacheUpdates> {
        ...
        let mut addresses: HashSet<MysAddress> = HashSet::new();
        addresses.extend(
        ...
        let mut balance_changes: HashMap<MysAddress, HashMap<TypeTag, TotalBalance>> =
            HashMap::new();
        // Index coin info
            ...
            .flat_map(|(address, balance_map)| {
                balance_map.iter().map(|(type_tag, balance)| {
                    (
                        ...
                        Ok::<TotalBalance, MysError>(*balance),
                    )
                })
            })
            .collect();
        let all_balance_changes: Vec<_> = balance_changes
            ...
            .map(|(address, balance_map)| {
                (
                    address,
                    Ok::<Arc<HashMap<TypeTag, TotalBalance>>, MysError>(Arc::new(balance_map)),
                )
            })
            .collect();
    ...
    pub fn get_transactions(
        ...
        let cursor = if let Some(cursor) = cursor {
            Some(
                self.get_transaction_seq(&cursor)?
                    .ok_or(MysError::TransactionNotFound { digest: cursor })?,
            )
        } else {
            None
        };
        match filter {
            ...
            Some(_) => Err(MysError::UserInputError {
    ...
    pub fn get_transaction_seq(
        &self,
        digest: &TransactionDigest,
    ) -> MysResult<Option<TxSequenceNumber>> {
    ...
    pub fn all_events(
        ...
        limit: usize,
        descending: bool,
    ) -> MysResult<Vec<(TransactionEventsDigest, TransactionDigest, usize, u64)>> {
        Ok(if descending {
            self.tables
                .event_order
                .unbounded_iter()
    ...
    pub fn events_by_transaction(
        ...
        let seq = self
            ...
            .ok_or(MysError::TransactionNotFound { digest: *digest })?;
        Ok(if descending {
            self.tables
                .event_order
                .unbounded_iter()
    ...
    pub fn insert_genesis_objects(&self, object_index_changes: ObjectIndexChanges) -> MysResult {
    ...
    pub fn checkpoint_db(&self, path: &Path) -> MysResult {
    ...
    #[instrument(skip(self))]
    pub fn get_balance(&self, owner: MysAddress, coin_type: TypeTag) -> MysResult<TotalBalance> {
        ...
        if force_disable_cache {
            Self::get_balance_from_db(metrics_cloned, coin_index_cloned, owner, cloned_coin_type)
                .map_err(|e| {
                MysError::ExecutionError(format!("Failed to read balance frm DB: {:?}", e))
            })?;
        }
    ...
    pub fn get_all_balance(
        &self,
        owner: MysAddress,
    ) -> MysResult<Arc<HashMap<TypeTag, TotalBalance>>> {
        let force_disable_cache = read_size_from_env(ENV_VAR_DISABLE_INDEX_CACHE).unwrap_or(0) > 0;
        let metrics_cloned = self.metrics.clone();
        let coin_index_cloned = self.tables.coin_index_2.clone();
        if force_disable_cache {
            Self::get_all_balances_from_db(metrics_cloned, coin_index_cloned, owner).map_err(
                |e| {
                    MysError::ExecutionError(format!("Failed to read all balance from DB: {:?}", e))
                },
            )?;
        }
    ...
    /// Read balance for a `MysAddress` and `CoinType` from the backend database
    #[instrument(skip_all)]
    ...
    /// Read all balances for a `MysAddress` from the backend database
    #[instrument(skip_all)]
    pub fn get_all_balances_from_db(
        ...
        for (coin_type, coins) in &coins {
            ...
            let coin_type =
                TypeTag::Struct(Box::new(parse_mys_struct_tag(&coin_type).map_err(|e| {
                    MysError::ExecutionError(format!(
    ...
    fn invalidate_per_coin_type_cache(
        &self,
        keys: impl IntoIterator<Item = (MysAddress, TypeTag)>,
    ) -> MysResult {
    ...
    fn invalidate_all_balance_cache(
        &self,
        addresses: impl IntoIterator<Item = MysAddress>,
    ) -> MysResult {
    ...
    fn update_per_coin_type_cache(
        &self,
        keys: impl IntoIterator<Item = ((MysAddress, TypeTag), MysResult<TotalBalance>)>,
    ) -> MysResult {
        self.caches
            .per_coin_type_balance
    ...
    fn merge_balance(
        old_balance: &MysResult<TotalBalance>,
        balance_delta: &MysResult<TotalBalance>,
    ) -> MysResult<TotalBalance> {
    ...
    fn update_all_balance_cache(
        &self,
        keys: impl IntoIterator<Item = (MysAddress, MysResult<Arc<HashMap<TypeTag, TotalBalance>>>)>,
    ) -> MysResult {
        self.caches
            .all_balances
    ...
    fn merge_all_balance(
        old_balance: &MysResult<Arc<HashMap<TypeTag, TotalBalance>>>,
        balance_delta: &MysResult<Arc<HashMap<TypeTag, TotalBalance>>>,
    ) -> MysResult<Arc<HashMap<TypeTag, TotalBalance>>> {
        if let Ok(old_balance) = old_balance {
            if let Ok(balance_delta) = balance_delta {
...
mod tests {
    ...
    async fn test_index_cache() -> anyhow::Result<()> {
        ...
        let address: MysAddress = AccountAddress::random().into();
