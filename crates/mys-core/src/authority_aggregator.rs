...
use mys_types::crypto::{AuthorityPublicKeyBytes, AuthoritySignInfo};
...
pub fn group_errors(errors: Vec<(MysError, Vec<AuthorityName>, StakeUnit)>) -> GroupedErrors {
...
struct ProcessTransactionState {
    ...
    errors: Vec<(MysError, Vec<AuthorityName>, StakeUnit)>,
    // This is exclusively non-retryable stake.
    non_retryable_stake: StakeUnit,
    // This includes both object and package not found mys errors.
...
impl ProcessTransactionState {
    pub fn record_conflicting_transaction_if_any(
        ...
        if let MysError::ObjectLockConflict {
            obj_ref,
    ...
    pub fn check_if_error_indicates_tx_finalized_with_different_user_sig(
        ...
        // and return Mys::FailedToVerifyTxCertWithExecutedEffects.
        ...
        let invalid_sig_stake: StakeUnit = self
            ...
            .filter_map(|(e, _, stake)| {
                if matches!(e, MysError::FailedToVerifyTxCertWithExecutedEffects { .. }) {
                    Some(stake)
                } else {
                    None
                }
...
struct ProcessCertificateState {
    ...
    non_retryable_stake: StakeUnit,
    non_retryable_errors: Vec<(MysError, Vec<AuthorityName>, StakeUnit)>,
    retryable_errors: Vec<(MysError, Vec<AuthorityName>, StakeUnit)>,
    // As long as none of the exit criteria are met we consider the state retryable
    // 1) >= 2f+1 signatures
    // 2) >= f+1 non-retryable errors
    retryable: bool,
...
impl<A: Clone> AuthorityAggregator<A> {
    ...
    ) -> MysResult<AuthorityAggregator<NetworkAuthorityClient>> {
        let network_clients =
...
impl AuthorityAggregator<NetworkAuthorityClient> {
    ...
    pub fn new_from_epoch_start_state(
        ...
        let committee = epoch_start_state.get_mys_committee_with_network_metadata();
    ...
    A: AuthorityAPI + Send + Sync + 'static + Clone,
{
    ...
    async fn quorum_once_inner<'a, S, FMap>(
        ...
        authority_errors: &mut HashMap<AuthorityName, MysError>,
    ) -> Result<S, MysError>
    where
        FMap: Fn(AuthorityName, Arc<SafeClient<A>>) -> AsyncResult<'a, S, MysError>
            + Send
            + Clone
            + 'a,
        S: Send,
    {
        ...
        loop {
            ...
            type RequestResult<S> = Result<Result<S, MysError>, tokio::time::error::Elapsed>;
            ...
            let name = authorities_shuffled.next().ok_or_else(|| {
                ...
                MysError::from("Available authorities list is empty")
            })?;
                    ...
                    Event::Request(name, res) => {
                        match res {
                            ...
                            Err(_) => {
                                ...
                                authority_errors.insert(name, MysError::TimeoutError);
                            }
                            // request completed
                            Ok(inner_res) => {
        ...
        if let Some(t) = timeout_total {
            timeout(t, fut).await.map_err(|_timeout_error| {
                if authority_errors.is_empty() {
                    MysError::TimeoutError
                } else {
                    MysError::TooManyIncorrectAuthorities {
    ...
    pub async fn get_latest_object_version_for_testing(
        ...
        let result = quorum_map_then_reduce_with_timeout(
            ...
            .await.map_err(|_state| MysError::from(UserInputError::ObjectNotFound {
                object_id,
                version: None,
            }))?;
        Ok(result.0)
    ...
    pub async fn get_latest_system_state_object_for_testing(
        &self,
    ) -> anyhow::Result<MysSystemState> {
        #[derive(Debug, Default)]
        struct State {
            latest_system_state: Option<MysSystemState>,
            total_weight: StakeUnit,
        }
        let initial_state = State::default();
        let result = quorum_map_then_reduce_with_timeout(
    ...
    fn record_rpc_error_maybe(
        ...
        if let MysError::RpcError(_message, code) = error {
            metrics
                .total_rpc_err
    ...
    fn handle_process_transaction_response(
        ...
        response: MysResult<PlainTransactionInfoResponse>,
        name: AuthorityName,
        weight: StakeUnit,
    ) -> MysResult<Option<ProcessTransactionResult>> {
        match response {
    ...
    pub async fn process_certificate(
        ...
        .map_err(|state| {
            ...
            for (mys_err, _, _) in state.retryable_errors.iter().chain(state.non_retryable_errors.iter()) {
                self
                    .metrics
                    .total_aggregated_err
                    .with_label_values(&[
    ...
    fn handle_process_certificate_response(
        ...
        response: MysResult<HandleCertificateResponseV3>,
        name: AuthorityName,
    ) -> MysResult<Option<QuorumDriverResponse>> {
        match response {
    ...
    pub async fn handle_transaction_info_request_from_some_validators(
        ...
        timeout_total: Option<Duration>,
    ) -> MysResult<PlainTransactionInfoResponse> {
