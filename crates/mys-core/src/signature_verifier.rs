...
type Sender = oneshot::Sender<MysResult<VerifiedCertificate>>;
...
impl SignatureVerifier {
    ...
    pub async fn verify_cert(&self, cert: CertifiedTransaction) -> MysResult<VerifiedCertificate> {
    ...
    pub async fn multi_verify_certs(
        &self,
        certs: Vec<CertifiedTransaction>,
    ) -> Vec<MysResult<VerifiedCertificate>> {
    ...
    pub fn verify_tx(&self, signed_tx: &SenderSignedData) -> MysResult {
...
fn batch_verify(
    ...
    for cert in certs {
        let idx = obligation.add_message(cert.data(), cert.epoch(), Intent::mys_app(cert.scope()));
        cert.auth_sig()
            .add_to_verification_obligation(committee, &mut obligation, idx)?;
    }

    for ckpt in checkpoints {
        let idx = obligation.add_message(ckpt.data(), ckpt.epoch(), Intent::mys_app(ckpt.scope()));
        ckpt.auth_sig()
            .add_to_verification_obligation(committee, &mut obligation, idx)?;
    }
