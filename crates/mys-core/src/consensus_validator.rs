...
pub struct MysTxValidator {
    ...
    metrics: Arc<MysTxValidatorMetrics>,
}
...
impl MysTxValidator {
    pub fn new(
        ...
        info!(
            "MysTxValidator constructed for epoch {}",
            epoch_store.epoch()
        );
        Self {
            authority_state,
    ...
    fn validate_transactions(&self, txs: &[ConsensusTransactionKind]) -> Result<(), MysError> {
            ...
            match tx {
                ...
                ConsensusTransactionKind::RandomnessDkgMessage(_, bytes) => {
                    if bytes.len() > dkg_v1::DKG_MESSAGES_MAX_SIZE {
                        ...
                        return Err(MysError::InvalidDkgMessageSize);
                    }
                }
                ...
                ConsensusTransactionKind::UserTransaction(_tx) => {
                    if !epoch_store.protocol_config().mysticeti_fastpath() {
                        return Err(MysError::UnexpectedMessage(
                ...
                ConsensusTransactionKind::ExecutionTimeObservation(obs) => {
                    ...
                    if obs.estimates.len()
                        > epoch_store
                            ...
                            .unwrap()
                    {
                        return Err(MysError::UnexpectedMessage(format!(
...
impl TransactionVerifier for MysTxValidator {
...
mod tests {
    ...
    use mys_types::{
        crypto::Ed25519MysSignature, messages_consensus::ConsensusTransaction, object::Object,
    ...
    use crate::{
        ...
        consensus_validator::{MysTxValidator, MysTxValidatorMetrics},
    };
    ...
    async fn accept_valid_transaction() {
        ...
        let network_config =
            mys_swarm_config::network_config_builder::ConfigBuilder::new_with_temp_dir()
        ...
        let metrics = MysTxValidatorMetrics::new(&Default::default());
        let validator = MysTxValidator::new(
            state.clone(),
        ...
        let bogus_transaction_bytes: Vec<_> = certificates
            ...
            .map(|mut cert| {
                ...
                cert.tx_signatures_mut_for_testing()[0] =
                    GenericSignature::Signature(mys_types::crypto::Signature::Ed25519MysSignature(
                        Ed25519MysSignature::default(),
                    ));
                bcs::to_bytes(&ConsensusTransaction::new_certificate_message(&name1, cert)).unwrap()
            })
            .collect();
