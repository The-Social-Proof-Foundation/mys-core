...
use mys_types::{MOVE_STDLIB_ADDRESS, MYS_FRAMEWORK_ADDRESS};
...
fn test_to_json_value() {
    ...
    let mys_move_struct: MysMoveStruct =
        BoundedVisitor::deserialize_struct(&event_bytes, &TestEvent::layout())
            .unwrap()
            .into();
    let json_value = mys_move_struct.to_json_value();
    assert_eq!(
        Some(&json!("1000000")),
...
// TODO: Do we need this in the mys-types lib?
