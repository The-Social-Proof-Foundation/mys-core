...
async fn test_pay_mys_failure_empty_recipients() {
    ...
    let res = execute_pay_mys(vec![coin1], vec![], vec![], sender, sender_key, 2000000).await;
...
#[tokio::test]
async fn test_pay_mys_failure_insufficient_gas_balance_one_input_coin() {
    let (sender, sender_key): (_, AccountKeyPair) = get_key_pair();
...
#[tokio::test]
async fn test_pay_mys_failure_insufficient_total_balance_one_input_coin() {
    let (sender, sender_key): (_, AccountKeyPair) = get_key_pair();
...
#[tokio::test]
async fn test_pay_mys_failure_insufficient_gas_balance_multiple_input_coins() {
    let (sender, sender_key): (_, AccountKeyPair) = get_key_pair();
...
#[tokio::test]
async fn test_pay_mys_failure_insufficient_total_balance_multiple_input_coins() {
    let (sender, sender_key): (_, AccountKeyPair) = get_key_pair();
...
#[tokio::test]
async fn test_pay_mys_success_one_input_coin() -> anyhow::Result<()> {
    let (sender, sender_key): (_, AccountKeyPair) = get_key_pair();
...
#[tokio::test]
async fn test_pay_mys_success_multiple_input_coins() -> anyhow::Result<()> {
    let (sender, sender_key): (_, AccountKeyPair) = get_key_pair();
...
#[tokio::test]
async fn test_pay_all_mys_failure_insufficient_gas_one_input_coin() {
    let (sender, sender_key): (_, AccountKeyPair) = get_key_pair();
    let coin1 = Object::with_id_owner_gas_for_testing(ObjectID::random(), sender, 1800);
    let recipient = dbg_addr(2);

    let res = execute_pay_all_mys(vec![&coin1], recipient, sender, sender_key, 2000000).await;
...
#[tokio::test]
async fn test_pay_all_mys_failure_insufficient_gas_budget_multiple_input_coins() {
    let (sender, sender_key): (_, AccountKeyPair) = get_key_pair();
    ...
    let recipient = dbg_addr(2);
    let res =
        execute_pay_all_mys(vec![&coin1, &coin2], recipient, sender, sender_key, 2500000).await;
...
#[tokio::test]
async fn test_pay_all_mys_success_one_input_coin() -> anyhow::Result<()> {
    let (sender, sender_key): (_, AccountKeyPair) = get_key_pair();
    let object_id = ObjectID::random();
    let coin_obj = Object::with_id_owner_gas_for_testing(object_id, sender, 3000000);
    let recipient = dbg_addr(2);
    let res = execute_pay_all_mys(vec![&coin_obj], recipient, sender, sender_key, 2000000).await;
...
#[tokio::test]
async fn test_pay_all_mys_success_multiple_input_coins() -> anyhow::Result<()> {
    let (sender, sender_key): (_, AccountKeyPair) = get_key_pair();
    let object_id1 = ObjectID::random();
...
struct PayMysTransactionBlockExecutionResult {
    pub authority_state: Arc<AuthorityState>,
    pub txn_result: Result<SignedTransactionEffects, MysError>,
}

async fn execute_pay_mys(
    ...
    gas_budget: u64,
) -> PayMysTransactionBlockExecutionResult {
    ...
    builder.pay_mys(recipients, amounts).unwrap();
