...
impl TestRunner {
    ...
    pub async fn signing_error(&mut self, pt: ProgrammableTransaction) -> MysError {
    ...
    transfer_test_runner! { |mut runner: TestRunner| async move {
        ...
        for (obj_ref, owner) in effects.mutated().iter() {
            if obj_ref.0 == child.0 .0 {
                ...
                assert_eq!(owner, &Owner::AddressOwner(MysAddress::ZERO));
...
async fn test_tto_intersection_input_and_receiving_objects() {
    transfer_test_runner! { |mut runner: TestRunner| async move {
        ...
        let MysError::UserInputError { error } = runner
            .signing_error({
                let mut builder = ProgrammableTransactionBuilder::new();
                let parent = builder.obj(ObjectArg::ImmOrOwnedObject(parent.0)).unwrap();
                let child = builder.obj(ObjectArg::Receiving(child.0)).unwrap();
...
async fn test_tto_invalid_receiving_arguments() {
    transfer_test_runner! { |mut runner: TestRunner| async move {
        ...
        for (i, (mutate, expect)) in mutations.into_iter().enumerate() {
            let MysError::UserInputError { error } = runner.signing_error({
                let mut builder = ProgrammableTransactionBuilder::new();
                let parent = builder.obj(ObjectArg::ImmOrOwnedObject(parent.0)).unwrap();
            ...
            .run_with_gas_object(
                {
                    let mut builder = ProgrammableTransactionBuilder::new();
                    builder
                        .transfer_object(MysAddress::from(parent.0 .0), child.0)
                        .unwrap();
                    builder.finish()
                },
                1,
            ...
            .run_with_gas_object(
                {
                    let mut builder = ProgrammableTransactionBuilder::new();
                    builder
                        .transfer_object(MysAddress::from(parent.0 .0), old_child.0)
                        .unwrap();
                    builder.finish()
                },
                1,
