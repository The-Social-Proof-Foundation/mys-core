...
use mys_json_rpc_types::{
    MysArgument, MysExecutionResult, MysExecutionStatus, MysTransactionBlockEffectsAPI, MysTypeTag,
};
...
use mys_types::execution::SharedInput;
...
use mys_types::{
    ...
    MOVE_STDLIB_PACKAGE_ID, MYS_AUTHENTICATOR_STATE_OBJECT_ID, MYS_CLOCK_OBJECT_ID,
    MYS_FRAMEWORK_PACKAGE_ID, MYS_RANDOMNESS_STATE_OBJECT_ID, MYS_SYSTEM_STATE_OBJECT_ID,
};
use mys_types::{digests::Digest, MYS_CLOCK_OBJECT_SHARED_VERSION};
...
async fn test_dry_run_no_gas_big_transfer() {
    ...
    builder.transfer_mys(recipient, Some(amount));
    ...
    let (dry_run_res, _, _, _) = fullnode
        ...
        .await
        .unwrap();
    assert_eq!(*dry_run_res.effects.status(), MysExecutionStatus::Success);
}
...
async fn test_dev_inspect_gas_coin_argument() {
    ...
    let pt = {
        let mut builder = ProgrammableTransactionBuilder::new();
        builder.pay_mys(vec![recipient], vec![amount]).unwrap();
        builder.finish()
    };
    let kind = TransactionKind::programmable(pt);
    ...
    assert_eq!(arg, &MysArgument::GasCoin);
    check_coin_value(
        arg_value,
        arg_type,
...
fn check_coin_value(actual_value: &[u8], actual_type: &MysTypeTag, expected_value: u64) {
...
async fn test_transfer_package() {
    ...
    let package_object_ref = authority_state
        .get_mys_system_package_object_ref()
        .await
        .unwrap();
...
async fn test_objected_owned_gas() {
    ...
    authority_state
        ...
        .await;
    let rgp = authority_state.reference_gas_price_for_testing().unwrap();
    let data = TransactionData::new_transfer_mys(
        recipient,
        sender,
        None,
...
async fn test_conflicting_transactions() {
    ...
    for _ in 0..100 {
        ...
        assert!(matches!(err, MysError::ObjectLockConflict { .. }));
...
#[tokio::test]
async fn test_handle_transfer_mys_with_amount_insufficient_gas() {
    let (sender, sender_key): (_, AccountKeyPair) = get_key_pair();
    let recipient = dbg_addr(2);
    let object_id = ObjectID::random();
    let authority_state = init_state_with_ids(vec![(sender, object_id)]).await;
...
async fn test_invalid_authenticator_state_parameter() {
    ...
    assert_eq!(
        ...
        UserInputError::InaccessibleSystemObject {
            object_id: MYS_AUTHENTICATOR_STATE_OBJECT_ID
        }
    );
}
...
async fn test_invalid_randomness_parameter() {
    ...
    assert_eq!(
        UserInputError::try_from(e).unwrap(),
        UserInputError::ImmutableParameterExpectedError {
            object_id: MYS_RANDOMNESS_STATE_OBJECT_ID
        }
    );
}
...
#[tokio::test]
async fn test_genesis_mys_system_state_object() {
    // This test verifies that we can read the genesis MysSystemState object.
    ...
    let _mys_system_state =
        bcs::from_bytes::<MysSystemStateWrapper>(move_object.contents()).unwrap();
    assert!(move_object.type_().is(&MysSystemStateWrapper::type_()));
    let mys_system_state = authority_state
...
#[tokio::test]
async fn test_transfer_mys_no_amount() {
    let (sender, sender_key): (_, AccountKeyPair) = get_key_pair();
    let recipient = dbg_addr(2);
    let gas_object_id = ObjectID::random();
    let gas_object = Object::with_id_owner_for_testing(gas_object_id, sender);
    let init_balance = mys_types::gas::get_gas_balance(&gas_object).unwrap();
    ...
    let gas_ref = gas_object.compute_object_reference();
    let tx_data = TransactionData::new_transfer_mys(
        recipient,
        sender,
        None,
        gas_ref,
    ...
    let new_balance =
        mys_types::gas::get_gas_balance(&authority_state.get_object(&gas_object_id).await.unwrap())
            .unwrap();
    assert_eq!(
...
#[tokio::test]
async fn test_transfer_mys_with_amount() {
    let (sender, sender_key): (_, AccountKeyPair) = get_key_pair();
    let recipient = dbg_addr(2);
    let gas_object_id = ObjectID::random();
    let gas_object = Object::with_id_owner_for_testing(gas_object_id, sender);
    ...
    let new_gas = authority_state
        ...
        .await
        .unwrap();
    assert_eq!(mys_types::gas::get_gas_balance(&new_gas).unwrap(), 500);
    assert!(gas_ref.1 < effects.gas_object().0 .1);
...
#[tokio::test]
async fn test_store_revert_transfer_mys() {
    // This test checks the correctness of revert_state_update in MysDataStore.
    let (sender, sender_key): (_, AccountKeyPair) = get_key_pair();
...
async fn create_and_retrieve_df_info(function: &IdentStr) -> (MysAddress, Vec<DynamicFieldInfo>) {
    let (sender, sender_key): (_, AccountKeyPair) = get_key_pair();
    let gas_object_id = ObjectID::random();
    let (authority_state, object_basics) =
        init_state_with_ids_and_object_basics(vec![(sender, gas_object_id)]).await;
...
pub async fn init_state_with_ids_and_object_basics<
    I: IntoIterator<Item = (MysAddress, ObjectID)>,
>(
    objects: I,
) -> (Arc<AuthorityState>, ObjectRef) {
    let state = TestAuthorityBuilder::new().build().await;
...
async fn test_consensus_message_processed() {
    ...
    let network_config = mys_swarm_config::network_config_builder::ConfigBuilder::new(&dir)
...
async fn test_gas_smashing() {
    ...
    fn make_gas_coins(owner: MysAddress, gas_amount: u64, coin_num: u64) -> Vec<Object> {
        let mut objects = vec![];
    ...
    async fn run_and_check(
        ...
        let balance =
            mys_types::gas::get_gas_balance(&state.get_object(&gas_coin_ids[0]).await.unwrap())
                .unwrap();
...
async fn test_publish_missing_dependency() {
    ...
    let mut builder = ProgrammableTransactionBuilder::new();
    builder.publish_immutable(modules, vec![MYS_FRAMEWORK_PACKAGE_ID]);
    let kind = TransactionKind::programmable(builder.finish());
...
async fn test_publish_not_a_package_dependency() {
    ...
    deps.push(MYS_SYSTEM_STATE_OBJECT_ID);
    builder.publish_immutable(modules, deps);
    let kind = TransactionKind::programmable(builder.finish());
    ...
    assert_eq!(
        MysError::UserInputError {
            error: UserInputError::MoveObjectAsPackage {
                object_id: MYS_SYSTEM_STATE_OBJECT_ID
            }
        },
        failure,
    )
...
pub fn create_gas_objects(num: u32, owner: MysAddress) -> Vec<Object> {
    let mut objects = vec![];
    for _ in 0..num {
        let gas_object_id = ObjectID::random();
