...
async fn test_handle_transfer_transaction_bad_signature() {
    do_transaction_test(
        ...
        |err| {
            assert_matches!(err, MysError::SignerSignatureAbsent { .. });
        },
    )
    .await;
}
...
pub fn init_move_call_transaction(
    ...
    let mut data = TransactionData::new_move_call(
        ...
        MYS_SYSTEM_MODULE_NAME.into(),
        ident_str!("request_add_validator").to_owned(),
        vec![],
        gas_object_ref,
        vec![CallArg::MYS_SYSTEM_MUT],
        gas_budget,
        gas_price,
    )
    .unwrap();
...
async fn test_zklogin_transfer_with_large_address_seed() {
    ...
    let sender = MysAddress::generate(StdRng::from_seed([3; 32]));
    let recipient = dbg_addr(2);
...
async fn zklogin_test_caching_scenarios() {
    ...
    let zklogin_json_string =
        &get_one_zklogin_inputs("../mys-types/src/unit_tests/zklogin_test_vectors.json");
    let bad_zklogin_inputs = ZkLoginInputs::from_json(zklogin_json_string, "111").unwrap();
    let sender = MysAddress::try_from_unpadded(&bad_zklogin_inputs).unwrap();
...
async fn setup_zklogin_network(
    ...
    mys_types::message_envelope::Envelope<SenderSignedData, mys_types::crypto::EmptySignInfo>,
    ...
    let sender = MysAddress::try_from_unpadded(zklogin).unwrap();
    ...
    let sender_2 = MysAddress::from(&multisig_pk);
...
async fn zk_multisig_test() {
    ...
    .unwrap();
    let victim_addr = MysAddress::from(&multisig_pk);
    ...
    for (kp, _pk_zklogin, inputs) in test_vectors {
        let intent_message = IntentMessage::new(Intent::mys_transaction(), data.clone());
...
async fn test_very_large_certificate() {
    ...
    let quorum_signature = mys_types::crypto::AuthorityQuorumSignInfo {
        epoch: 0,
        signature: mys_types::crypto::AggregateAuthoritySignature::aggregate(&sigs)
    ...
    let cert = mys_types::message_envelope::Envelope::new_from_data_and_sig(
...
async fn test_handle_soft_bundle_certificates_errors() {
    ...
    println!("Case 0: submit an empty soft bundle.");
    {
        ...
        assert_matches!(
            response.unwrap_err(),
            MysError::NoCertificateProvidedError { .. }
        );
    }
...
fn sender_signed_data_serialized_intent() {
    ...
    assert_eq!(txn.intent_message().intent, Intent::mys_transaction());
