...
pub fn build_upgrade_txn(
    ...
    let upgrade_ticket = move_call! {
        builder,
        (MYS_FRAMEWORK_PACKAGE_ID)::package::authorize_upgrade(Argument::Input(0), upgrade_arg, digest_arg)
    };
    let upgrade_receipt = builder.upgrade(current_pkg_id, upgrade_ticket, vec![], modules);
    move_call! {
        builder,
        (MYS_FRAMEWORK_PACKAGE_ID)::package::commit_upgrade(Argument::Input(0), upgrade_receipt)
    };
    ...
    pub async fn upgrade(
        ...
        let pt = {
            ...
            let ticket = move_call! {
                builder,
                (MYS_FRAMEWORK_PACKAGE_ID)::package::authorize_upgrade(cap, policy, digest)
            };

            let receipt = builder.upgrade(package_id, ticket, dep_ids, modules);
            move_call! { builder, (MYS_FRAMEWORK_PACKAGE_ID)::package::commit_upgrade(cap, receipt) };
...
async fn test_upgrade_introduces_type_then_uses_it() {
    ...
    let effects = runner
        .upgrade(
            ...
            digest,
            modules,
            vec![MYS_FRAMEWORK_PACKAGE_ID, MOVE_STDLIB_PACKAGE_ID],
        )
        .await;
    ...
    let effects = runner
        .run({
            ...
            let cap = builder
                ...
                .unwrap();
            move_call! { builder, (MYS_FRAMEWORK_PACKAGE_ID)::package::only_dep_upgrades(cap) };
            builder.finish()
        })
        .await;
...
async fn test_multiple_upgrades(
    ...
    // Second upgrade: May also adds a dep on the mys framework and stdlib.
