...
use std::sync::Arc;
use mys_config::certificate_deny_config::CertificateDenyConfigBuilder;
use mys_config::transaction_deny_config::{TransactionDenyConfig, TransactionDenyConfigBuilder};
...
use mys_types::utils::get_zklogin_user_address;
use mys_types::utils::{
...
type Account = (MysAddress, Ed25519KeyPair, Vec<ObjectRef>);
...
fn get_accounts_and_coins(
    ...
    let accounts: Vec<_> = network_config
        ...
        .iter()
        .map(|account| {
            let address: MysAddress = account.public().into();
...
async fn transfer_with_account(
    ...
    let data = TransactionData::new_transfer_mys_allow_sponsor(
        sender_account.0,
        sender_account.0,
        None,
        sponsor_account.2[0],
...
fn assert_denied<T: std::fmt::Debug>(result: &MysResult<T>) {
    assert!(matches!(
...
async fn test_shared_object_transaction_disabled() {
    ...
    let tx = TestTransactionBuilder::new(account.0, account.2[0], gas_price)
        .call_staking(account.2[1], MysAddress::default())
