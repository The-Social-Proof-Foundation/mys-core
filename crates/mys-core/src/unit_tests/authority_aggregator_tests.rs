...
use mys_types::crypto::{AuthoritySignature, Signer};
...
async fn test_map_reducer() {
    ...
    let res = quorum_map_then_reduce_with_timeout::<_, _, _, _, _, (), _, _, _>(
        ...
        |_name, _client| {
            Box::pin(async move {
                let res: Result<usize, MysError> = Err(MysError::TooManyIncorrectAuthorities {
                    errors: vec![],
                    action: "".to_string(),
                });
                res
        ...
        |mut accumulated_state, _authority_name, _authority_weight, result| {
            Box::pin(async move {
                assert!(matches!(
                    result,
                    Err(MysError::TooManyIncorrectAuthorities { .. })
                ));
                accumulated_state += 1;
            ...
            |_name, client| {
                ...
                Box::pin(async move {
                    ...
                    match res {
                        ...
                        Err(MysError::TransactionNotFound { .. }) => Ok(()),
                        Err(err) => Err(err),
                    }
                })
            },
...
async fn test_handle_transaction_fork() {
    ...
    let tx = make_transfer_mys_transaction(
        gas_object,
        MysAddress::default(),
        None,
        sender,
...
async fn test_handle_certificate_response() {
    ...
    let (sender, sender_kp): (_, AccountKeyPair) = get_key_pair();
    let gas_object = random_object_ref();
    let tx = VerifiedTransaction::new_unchecked(make_transfer_mys_transaction(
        gas_object,
        MysAddress::default(),
        None,
        sender,
    ...
    assert_resp_err(&agg, tx.clone().into(), |e| matches!(e, AggregatorProcessTransactionError::RetryableTransaction { .. }),
        |e| matches!(e, MysError::WrongEpoch { expected_epoch, actual_epoch } if *expected_epoch == 1 && *actual_epoch == 0)
    ).await;
    ...
    assert_matches!(
        ...
        AggregatorProcessCertificateError::RetryableExecuteCertificate {
            retryable_errors, ..
        } if retryable_errors.iter().any(|(error, _, _)| matches!(error, MysError::WrongEpoch {
...
async fn test_handle_transaction_response() {
    ...
    let tx2 = VerifiedTransaction::new_unchecked(make_transfer_mys_transaction(
        gas_object,
        MysAddress::default(),
        Some(1),
        sender,
    ...
    let package_not_found_error = MysError::UserInputError {
        error: UserInputError::DependentPackageNotFound {
            package_id: gas_object.0,
        },
    };
    let object_not_found_error = MysError::UserInputError {
    ...
    assert_resp_err(
        ...
        |e| matches!(e, MysError::MissingCommitteeAtEpoch(e) if *e == 1),
    )
    .await;
    ...
    assert_resp_err(
        ...
        |e| {
            matches!(
                e,
                MysError::QuorumFailedToGetEffectsQuorumWhenProcessingTransaction { .. }
                    | MysError::RpcError(..)
            )
        },
    )
    ...
    assert_resp_err(
        ...
        |e| matches!(e, MysError::WrongEpoch { expected_epoch, actual_epoch } if *expected_epoch == 0 && *actual_epoch == 1)
    )
    .await;
    ...
    assert_resp_err(
        ...
        |e| matches!(e, MysError::UserInputError { .. } | MysError::RpcError(..)),
    )
    .await;
    ...
    for (name, _) in authority_keys.iter().skip(2) {
        clients
            .get_mut(name)
            .unwrap()
            .set_tx_info_response_error(MysError::EpochEnded(0));
    }
    let agg = get_genesis_agg(authorities.clone(), clients.clone());
    assert_resp_err(
        &agg,
        ...
        |e| matches!(e, MysError::EpochEnded(0)),
    )
    .await;
...
async fn test_handle_conflicting_transaction_response() {
    ...
    let tx1 = VerifiedTransaction::new_unchecked(make_transfer_mys_transaction(
        conflicting_object,
        MysAddress::default(),
        Some(1),
        sender,
    ...
    let conflicting_tx2 = VerifiedTransaction::new_unchecked(make_transfer_mys_transaction(
        conflicting_object,
        MysAddress::default(),
        Some(2),
        sender,
    ...
    let conflicting_error = MysError::ObjectLockConflict {
        obj_ref: conflicting_object,
        pending_transaction: *conflicting_tx2.digest(),
    };
    let retryable_error = MysError::RpcError("RPC".into(), "Error".into());
    let non_retryable_error = MysError::ByzantineAuthoritySuspicion {
    ...
    assert_resp_err(
        ...
        |e| {
            matches!(
                e,
                MysError::ObjectLockConflict { .. } | MysError::RpcError(..)
            )
        },
    )
    .await;
    ...
    assert_resp_err(
        ...
        |e| matches!(e, MysError::ObjectLockConflict { .. }),
    )
    .await;
    ...
    let conflicting_tx3 = make_transfer_mys_transaction(
        conflicting_object,
        MysAddress::default(),
        Some(3),
        sender,
    ...
    let conflicting_error_2 = MysError::ObjectLockConflict {
        obj_ref: conflicting_object,
    ...
    assert_resp_err(
        ...
        |e| {
            matches!(
                e,
                MysError::ObjectLockConflict { .. } | MysError::ByzantineAuthoritySuspicion { .. }
            )
        },
    )
    .await;
    ...
    assert_resp_err(
        ...
        |e| {
            matches!(
                e,
                MysError::ObjectLockConflict { .. } | MysError::UserInputError { .. }
            )
        },
    )
    .await;
    ...
    assert_resp_err(
        ...
        |e| {
            matches!(
                e,
                MysError::MissingCommitteeAtEpoch(..) | MysError::ObjectLockConflict { .. }
            )
        },
    )
    .await;
    ...
    assert_resp_err(
        ...
        |e| {
            matches!(
                e,
                MysError::WrongEpoch { .. } | MysError::ObjectLockConflict { .. }
            )
        },
    )
    .await;
...
async fn test_handle_overload_response() {
    ...
    let (sender, sender_kp): (_, AccountKeyPair) = get_key_pair();
    let gas_object = random_object_ref();
    let txn = make_transfer_mys_transaction(
        gas_object,
        MysAddress::default(),
        None,
        sender,
    ...
    let overload_error = MysError::TooManyTransactionsPendingExecution {
        queue_len: 100,
        threshold: 100,
    };
    let rpc_error = MysError::RpcError("RPC".into(), "Error".into());
    ...
    assert_resp_err(
        ...
        |e| {
            matches!(
                e,
                MysError::TooManyTransactionsPendingExecution { .. } | MysError::RpcError(..)
            )
        },
    )
    .await;
...
// Tests that authority aggregator can aggregate MysError::ValidatorOverloadedRetryAfter into
...
async fn test_handle_overload_retry_response() {
    ...
    assert_resp_err(
        ...
        |e| {
            matches!(
                e,
                MysError::ValidatorOverloadedRetryAfter { .. } | MysError::RpcError(..)
            )
        },
    )
    .await;
    ...
    for (name, secret) in &authority_keys {
        let auth_signature = if byzantines.contains(name) {
            ...
            AuthoritySignInfo::new(
                0,
                tx.clone().data(),
                Intent::mys_app(IntentScope::ProofOfPossession), // bad intent
                *name,
                secret,
            )
        } else {
    ...
    match agg.process_transaction(tx, Some(make_socket_addr())).await {
        Err(received_agg_err) if agg_err_checker(&received_agg_err) => match received_agg_err {
            ...
            AggregatorProcessTransactionError::FatalConflictingTransaction {
                ...
                assert!(errors.iter().map(|e| &e.0).all(mys_err_checker));
            }
...
fn set_cert_response_with_certified_tx(
    ...
    let effects = effects_with_tx(*cert.digest());
    for (name, secret) in authority_keys {
        let resp = mys_types::messages_grpc::HandleCertificateResponseV2 {
...
fn set_retryable_tx_info_response_error(
    ...
    let error = MysError::RpcError("RPC".into(), "Error".into());
