...
use mys_types::{
    base_types::{FullObjectID, ObjectID, ObjectRef, SequenceNumber, MysAddress},
...
async fn test_object_lock_conflict() {
    ...
    assert!(matches!(
        mutate_cert_res.err(),
        Some(MysError::ObjectLockConflict { .. })
    ));
}
