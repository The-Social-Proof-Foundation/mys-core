...
use mys_move_build::{check_unpublished_dependencies, gather_published_ids, BuildConfig};
use mys_types::{
...
async fn test_generate_lock_file() {
    ...
    let expected = expect![[r##"
        ...
        [[move.package]]
        id = "MoveStdlib"
        source = { local = "../../../../../mys-framework/packages/move-stdlib" }

        [[move.package]]
        id = "Mys"
        source = { local = "../../../../../mys-framework/packages/mys-framework" }
...
async fn test_custom_property_check_unpublished_dependencies() {
    ...
    let MysError::ModulePublishFailure { error } = check_unpublished_dependencies(
