...
use mys_types::object::GAS_VALUE_FOR_TESTING;
...
where
    F: FnOnce(&GasCostSummary, u64, u64) -> MysResult,
{
...
// - OOG computation, storage ok
#[tokio::test]
async fn test_oog_computation_storage_ok_one_coin() -> MysResult {
...
#[tokio::test]
async fn test_oog_computation_storage_ok_multi_coins() -> MysResult {
    const GAS_PRICE: u64 = 1_000;
    let budget: u64 = ProtocolConfig::get_for_max_version_UNSAFE().max_tx_gas();
    let (sender, sender_key) = get_key_pair();
    check_oog_transaction(
...
// OOG for computation, OOG for minimal storage (e.g. computation is entire budget)
#[tokio::test]
async fn test_oog_computation_oog_storage_final_one_coin() -> MysResult {
...
// - computation ok, OOG for storage, minimal storage ok
#[tokio::test]
async fn test_computation_ok_oog_storage_minimal_ok_one_coin() -> MysResult {
...
// - computation ok, OOG for storage, minimal storage ok
#[tokio::test]
async fn test_computation_ok_oog_storage_minimal_ok_multi_coins() -> MysResult {
    const GAS_PRICE: u64 = 1001;
    const BUDGET: u64 = 1_100_000;
    let (sender, sender_key) = get_key_pair();
    check_oog_transaction(
...
// - computation ok, OOG for storage, OOG for minimal storage (e.g. computation is entire budget)
#[tokio::test]
async fn test_computation_ok_oog_storage_final_one_coin() -> MysResult {
    const GAS_PRICE: u64 = 1001;
...
#[tokio::test]
async fn test_native_transfer_sufficient_gas() -> MysResult {
...
#[tokio::test]
async fn test_transfer_mys_insufficient_gas() {
...
async fn test_invalid_gas_owners() {
    ...
    let non_sender_owned_object =
        init_object(Object::with_owner_for_testing(MysAddress::ZERO)).await;
...
#[tokio::test]
async fn test_move_call_gas() -> MysResult {
    let (sender, sender_key): (_, AccountKeyPair) = get_key_pair();
    let gas_object_id = ObjectID::random();
...
struct TransferResult {
    ...
    pub response: MysResult<TransactionStatus>,
    pub rgp: u64,
}
