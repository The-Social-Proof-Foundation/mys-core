...
use mys_types::{base_types::*, committee::*, fp_ensure};
use mys_types::{
...
impl<C: Clone> SafeClient<C> {
    ...
    fn get_committee(&self, epoch_id: &EpochId) -> MysResult<Arc<Committee>> {
        self.committee_store
            .get_committee(epoch_id)?
            .ok_or(MysError::MissingCommitteeAtEpoch(*epoch_id))
    }
    ...
    ) -> MysResult<SignedTransactionEffects> {
                ...
                match cert_opt {
                    Some(cert) => {
                        ...
                        ct.verify_committee_sigs_only(&committee).map_err(|e| {
                            MysError::FailedToVerifyTxCertWithExecutedEffects {
    ...
    ) -> MysResult<VerifiedObjectInfoResponse> {
    ...
    C: AuthorityAPI + Send + Sync + Clone + 'static,
{
    ...
    ) -> Result<PlainTransactionInfoResponse, MysError> {
    ...
    ) -> MysResult<HandleCertificateResponseV2> {
        let signed_effects =
    ...
    ) -> MysResult<HandleCertificateResponseV3> {
        ...
        match (&events, effects.events_digest()) {
            ...
            (Some(events), None) => {
                if !events.data.is_empty() {
                    return Err(MysError::ByzantineAuthoritySuspicion {
                        authority: self.address,
    ...
    pub async fn handle_object_info_request(
        &self,
        request: ObjectInfoRequest,
    ) -> Result<VerifiedObjectInfoResponse, MysError> {
    ...
    fn verify_checkpoint_sequence(
        ...
        if let (Some(e), Some(o)) = (expected_seq, observed_seq) {
            fp_ensure!(
                e == o,
                MysError::from("Expected checkpoint number doesn't match with returned")
            );
        }
        Ok(())
    }
    ...
    fn verify_contents_exist<T, O>(
        ...
        match (request_content, checkpoint, contents) {
            ...
            (true, Some(_), None) | (false, _, Some(_)) | (_, None, Some(_)) => Err(
                MysError::from("Checkpoint contents inconsistent with request"),
            ),
            _ => Ok(()),
        }
    }
    ...
    #[instrument(level = "trace", skip_all, fields(authority = ?self.address.concise()))]
    pub async fn handle_system_state_object(&self) -> Result<MysSystemState, MysError> {
        self.authority_client
