...
impl CheckpointStore {
    ...
    fn prune_local_summaries(&self) -> MysResult {
...
impl CheckpointBuilder {
    ...
    ) -> MysResult<Vec<TransactionEffects>> {
        self.metrics
    ...
    ) -> MysResult<Option<(TransactionDigest, TransactionEffects)>> {
    ...
    async fn augment_epoch_last_checkpoint(
        ...
        // TODO: Check whether we must use anyhow::Result or can we use MysResult.
...
impl CheckpointAggregator {
    ...
    async fn run_and_notify(&mut self) -> MysResult {
    ...
    fn run_inner(&mut self) -> MysResult<Vec<CertifiedCheckpointSummary>> {
...
impl CheckpointSignatureAggregator {
    ...
    pub fn try_aggregate(
        ...
        match self.signatures_by_digest.insert(their_digest, envelope) {
            ...
            InsertResult::Failed {
                error:
                    MysError::StakeAggregatorRepeatedSigner {
...
async fn diagnose_split_brain(
    ...
    let committee = epoch_store
        .epoch_start_state()
        .get_mys_committee_with_network_metadata();
...
pub trait CheckpointServiceNotify {
    ...
    fn notify_checkpoint(&self) -> MysResult;
}
...
mod tests {
    ...
    use mys_types::messages_checkpoint::SignedCheckpointSummary;
    ...
    pub async fn checkpoint_builder_test() {
        ...
        for digest in all_digests {
            let signature = Signature::Ed25519MysSignature(Default::default()).into();
            state
