...
pub(crate) fn update_low_scoring_authorities(
    ...
    for (index, score) in scores_per_authority_order_asc {
        let authority_name = mys_committee.authority_by_index(index).unwrap();
...
mod tests {
    ...
    pub fn test_update_low_scoring_authorities() {
        ...
        let (mys_committee, consensus_committee) = generate_committees(8);
        ...
        assert_eq!(
            *low_scoring
                ...
                .get(mys_committee.authority_by_index(2).unwrap())
                .unwrap(),
            50
        );
        assert_eq!(
            *low_scoring
                ...
                .get(mys_committee.authority_by_index(4).unwrap())
                .unwrap(),
            0
        );
    ...
    /// Generate a pair of Mys and consensus committees for the given size.
    fn generate_committees(committee_size: usize) -> (Committee, ConsensusCommittee) {
        ...
        let public_keys = consensus_committee
            ...
            .collect::<Vec<_>>();
        let mys_authorities = public_keys
            .iter()
            .map(|key| (AuthorityPublicKeyBytes::from(*key), 1))
            .collect::<Vec<_>>();
        let mys_committee = Committee::new_for_testing_with_normalized_voting_power(
            0,
            mys_authorities.iter().cloned().collect(),
        );

        (mys_committee, consensus_committee)
    }
}
