...
use mys_storage::mutex_table::{MutexGuard, MutexTable};
...
use mys_types::base_types::{ConciseableName, ObjectRef};
...
use mys_types::digests::{ChainIdentifier, TransactionEffectsDigest};
use mys_types::effects::TransactionEffects;
use mys_types::error::{MysError, MysResult};
use mys_types::executable_transaction::{
...
pub enum ConsensusCertificateResult {
    ...
    MysTransaction(VerifiedExecutableTransaction),
...
impl AuthorityEpochTables {
    ...
    fn load_reconfig_state(&self) -> MysResult<ReconfigState> {
        let state = self
            .reconfig_state
    ...
    pub fn remove_executed_tx_subtle(&self, digest: &TransactionDigest) -> MysResult {
        self.executed_transactions_to_checkpoint.remove(digest)?;
        Ok(())
    }

    pub fn get_last_consensus_index(&self) -> MysResult<Option<ExecutionIndices>> {
        Ok(self
    ...
    pub fn get_last_consensus_stats(&self) -> MysResult<Option<ExecutionIndicesWithStats>> {
    ...
    pub fn get_locked_transaction(&self, obj_ref: &ObjectRef) -> MysResult<Option<LockDetails>> {
        Ok(self
    ...
    ) -> MysResult<Vec<Option<LockDetails>>> {
        Ok(self
            .owned_object_locked_transactions
    ...
    ) -> MysResult<impl IntoIterator<Item = (ObjectID, u64)>> {
...
impl AuthorityPerEpochStore {
    ...
    pub fn tables(&self) -> MysResult<Arc<AuthorityEpochTables>> {
        match self.tables.load_full() {
            Some(tables) => Ok(tables),
            None => Err(MysError::EpochEnded(self.epoch())),
        }
    }
    ...
    pub fn acquire_tx_guard(&self, cert: &VerifiedExecutableTransaction) -> MysResult<CertTxGuard> {
    ...
    pub fn store_reconfig_state(&self, new_state: &ReconfigState) -> MysResult {
        self.tables()?
            .reconfig_state
    ...
    pub fn insert_signed_transaction(&self, transaction: VerifiedSignedTransaction) -> MysResult {
        Ok(self
            .tables()?
            .signed_transactions
    ...
    pub fn revert_executed_transaction(&self, tx_digest: &TransactionDigest) -> MysResult {
        let tables = self.tables()?;
    ...
    pub fn get_effects_signature(
        &self,
        tx_digest: &TransactionDigest,
    ) -> MysResult<Option<AuthoritySignInfo>> {
        let tables = self.tables()?;
    ...
    pub fn get_signed_effects_digest(
        &self,
        tx_digest: &TransactionDigest,
    ) -> MysResult<Option<TransactionEffectsDigest>> {
        let tables = self.tables()?;
    ...
    pub fn get_transaction_cert_sig(
        &self,
        tx_digest: &TransactionDigest,
    ) -> MysResult<Option<AuthorityStrongQuorumSignInfo>> {
                ...
                Ok(match kind {
                    InputObjectKind::SharedMoveObject {
                        ...
                        let assigned_shared_versions = assigned_shared_versions
                            ...
                            .ok_or(MysError::GenericAuthorityError {
    ...
    pub fn get_last_consensus_stats(&self) -> MysResult<ExecutionIndicesWithStats> {
        match self
            .tables()?
    ...
    ) -> MysResult<Vec<(CheckpointSequenceNumber, Accumulator)>> {
        self.tables()?
            .state_hash_by_checkpoint
    ...
    pub fn all_pending_execution(&self) -> MysResult<Vec<VerifiedExecutableTransaction>> {
        Ok(self
            .tables()?
            .pending_execution
            .unbounded_iter()
    ...
    #[instrument(level = "trace", skip_all)]
    pub fn handle_committed_transactions(&self, digests: &[TransactionDigest]) -> MysResult<()> {
        let tables = match self.tables() {
            ...
            Err(MysError::EpochEnded(_)) => return Ok(()),
            Err(e) => return Err(e),
        };
    ...
    ) -> MysResult<HashMap<ConsensusObjectSequenceKey, SequenceNumber>> {
    ...
    pub fn get_assigned_shared_object_versions(
        &self,
        key: &TransactionKey,
    ) -> MysResult<Option<Vec<(ConsensusObjectSequenceKey, SequenceNumber)>>> {
        if self.epoch_start_config().use_version_assignment_tables_v3() {
    ...
    ) -> MysResult<Vec<(DeferralKey, Vec<VerifiedSequencedConsensusTransaction>)>> {
    ...
    pub fn has_sent_end_of_publish(&self, authority: &AuthorityName) -> MysResult<bool> {
        Ok(self
            .end_of_publish
            .try_lock()
    ...
    pub fn user_signatures_for_checkpoint(
        ...
        digests: &[TransactionDigest],
    ) -> MysResult<Vec<Vec<GenericSignature>>> {
    ...
    pub fn clear_override_protocol_upgrade_buffer_stake(&self) -> MysResult {
        warn!(
            epoch = ?self.epoch(),
    ...
    pub fn set_override_protocol_upgrade_buffer_stake(&self, new_stake_bps: u64) -> MysResult {
        warn!(
            ?new_stake_bps,
            epoch = ?self.epoch(),
    ...
    pub fn record_capabilities(&self, capabilities: &AuthorityCapabilitiesV1) -> MysResult {
    ...
    /// Record most recently advertised capabilities of all authorities
    pub fn record_capabilities_v2(&self, capabilities: &AuthorityCapabilitiesV2) -> MysResult {
    ...
    pub(crate) fn get_new_jwks(&self, round: u64) -> MysResult<Vec<ActiveJwk>> {
    ...
    #[instrument(level = "trace", skip_all)]
    pub fn verify_transaction(&self, tx: Transaction) -> MysResult<VerifiedTransaction> {
    ...
    fn verify_consensus_transaction(
        ...
        // Signatures are verified as part of the consensus payload verification in MysTxValidator
    ...
    fn db_batch(&self) -> MysResult<DBBatch> {
    ...
    ) -> MysResult<Vec<VerifiedExecutableTransaction>> {
    ...
    fn add_consensus_commit_prologue_transaction(
    ...
    ) -> MysResult<Option<TransactionKey>> {
        {
        ...
        let consensus_commit_prologue_root = match self.process_consensus_system_transaction(&transaction) {
            ConsensusCertificateResult::MysTransaction(processed_tx) => {
    ...
    pub(crate) async fn process_consensus_transactions<C: CheckpointServiceNotify>(
        ...
        for tx in transactions {
            ...
            match self
                ...
                .await?
            {
                ConsensusCertificateResult::MysTransaction(cert) => {
    ...
    async fn process_consensus_transaction<C: CheckpointServiceNotify>(
        ...
        execution_time_estimator: &ExecutionTimeEstimator,
        authority_metrics: &Arc<AuthorityMetrics>,
    ) -> MysResult<ConsensusCertificateResult> {
        ...
        match &transaction {
            SequencedConsensusTransactionKind::External(ConsensusTransaction {
                ...
                // Safe because signatures are verified when consensus called into MysTxValidator::validate_batch.
            ...
            SequencedConsensusTransactionKind::External(ConsensusTransaction {
                ...
                // We usually call notify_checkpoint_signature in MysTxValidator, but that step can
    ...
    fn process_consensus_system_transaction(
        ...
        ConsensusCertificateResult::MysTransaction(system_transaction.clone())
    }
    ...
    fn process_consensus_user_transaction(
        ...
        Ok(ConsensusCertificateResult::MysTransaction(transaction))
    }
    ...
    ) -> MysResult<Vec<(CheckpointHeight, PendingCheckpointV2)>> {
        let tables = self.tables()?;
    ...
    ) -> MysResult<Option<PendingCheckpointV2>> {
    ...
    pub fn last_built_checkpoint_builder_summary(
        &self,
    ) -> MysResult<Option<BuilderCheckpointSummary>> {
        Ok(self
            .tables()?
            .builder_checkpoint_summary_v2
            .unbounded_iter()
    ...
    pub fn last_built_checkpoint_summary(
        &self,
    ) -> MysResult<Option<(CheckpointSequenceNumber, CheckpointSummary)>> {
        Ok(self
            .tables()?
            .builder_checkpoint_summary_v2
            .unbounded_iter()
    ...
    pub fn get_built_checkpoint_summary(
        &self,
        sequence: CheckpointSequenceNumber,
    ) -> MysResult<Option<CheckpointSummary>> {
        Ok(self
            .tables()?
            .builder_checkpoint_summary_v2
            .get(&sequence)?
    ...
    pub fn get_last_checkpoint_signature_index(&self) -> MysResult<u64> {
        Ok(self
            .tables()?
            .pending_checkpoint_signatures
            .unbounded_iter()
