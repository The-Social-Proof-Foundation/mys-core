...
impl SharedObjVerManager {
    ...
    ) -> MysResult<ConsensusSharedObjVerAssignment> {
    ...
    pub fn assign_versions_from_effects(
        ...
        epoch_store: &AuthorityPerEpochStore,
        cache_reader: &dyn ObjectCacheRead,
    ) -> MysResult<AssignedTxAndVersions> {
            ...
            } in shared_input_objects.iter()
            {
                let assigned_version = match cancellation_info {
                    ...
                    Some(CancelConsensusCertificateReason::DkgFailed) => {
                        if id == &MYS_RANDOMNESS_STATE_OBJECT_ID {
...
mod tests {
    ...
    async fn test_assign_versions_from_consensus_with_randomness() {
        ...
        assert_eq!(
            epoch_store
                .get_next_object_version(&MYS_RANDOMNESS_STATE_OBJECT_ID, randomness_obj_version)
                .unwrap(),
            randomness_obj_version
        );
        ...
        assert_eq!(
            ...
            HashMap::from([(
                (MYS_RANDOMNESS_STATE_OBJECT_ID, randomness_obj_version),
