...
struct AuthorityStoreMetrics {
    mys_conservation_check_latency: IntGauge,
    mys_conservation_live_object_count: IntGauge,
    mys_conservation_live_object_size: IntGauge,
    mys_conservation_imbalance: IntGauge,
    mys_conservation_storage_fund: IntGauge,
    mys_conservation_storage_fund_imbalance: IntGauge,
...
impl AuthorityStoreMetrics {
    pub fn new(registry: &Registry) -> Self {
        Self {
            mys_conservation_check_latency: register_int_gauge_with_registry!(
                "mys_conservation_check_latency",
                "Number of seconds took to scan all live objects in the store for MYS conservation check",
                registry,
            ).unwrap(),
            mys_conservation_live_object_count: register_int_gauge_with_registry!(
                "mys_conservation_live_object_count",
                "Number of live objects in the store",
                registry,
            ).unwrap(),
            mys_conservation_live_object_size: register_int_gauge_with_registry!(
                "mys_conservation_live_object_size",
                "Size in bytes of live objects in the store",
                registry,
            ).unwrap(),
            mys_conservation_imbalance: register_int_gauge_with_registry!(
                "mys_conservation_imbalance",
                "Total amount of MYS in the network - 10B * 10^9. This delta shows the amount of imbalance",
                registry,
            ).unwrap(),
            mys_conservation_storage_fund: register_int_gauge_with_registry!(
                "mys_conservation_storage_fund",
                "Storage Fund pool balance (only includes the storage fund proper that represents object storage)",
                registry,
            ).unwrap(),
            mys_conservation_storage_fund_imbalance: register_int_gauge_with_registry!(
                "mys_conservation_storage_fund_imbalance",
...
/// S is a template on Authority signature state. This allows MysDataStore to be used on either
/// authorities or non-authorities. Specifically, when storing transactions and effects,
/// S allows MysDataStore to either store the authority signed version or unsigned version.
pub struct AuthorityStore {
    ...
    /// Whether to enable expensive MYS conservation check at epoch boundaries.
    enable_epoch_mys_conservation_check: bool,
...
impl AuthorityStore {
    ...
    pub async fn open(
        ...
        let enable_epoch_mys_conservation_check = config
            .expensive_safety_check_config
            .enable_epoch_mys_conservation_check();
        ...
        let epoch_start_configuration = if perpetual_tables.database_is_empty()? {
            ...
            let epoch_start_configuration = EpochStartConfiguration::new(
                genesis.mys_system_object().into_epoch_start_state(),
    ...
    pub fn get_recovery_epoch_at_restart(&self) -> MysResult<EpochId> {
    ...
    ) -> MysResult<Option<TransactionEffects>> {
    ...
    pub fn effects_exists(&self, effects_digest: &TransactionEffectsDigest) -> MysResult<bool> {
        self.perpetual_tables
            .effects
    ...
    ) -> MysResult<Vec<Option<TransactionEffects>>> {
    ...
    ) -> MysResult<Vec<Option<TransactionEffectsDigest>>> {
    ...
    pub fn is_tx_already_executed(&self, digest: &TransactionDigest) -> MysResult<bool> {
        Ok(self
            .perpetual_tables
            .executed_effects
            .contains_key(digest)?)
    ...
    pub fn get_latest_marker(
        ...
        epoch_id: EpochId,
        use_object_per_epoch_marker_table_v2: bool,
    ) -> MysResult<Option<(SequenceNumber, MarkerValue)>> {
        if use_object_per_epoch_marker_table_v2 {
    ...
    pub async fn notify_read_root_state_hash(
        &self,
        epoch: EpochId,
    ) -> MysResult<(CheckpointSequenceNumber, Accumulator)> {
    ...
    ) -> MysResult<Option<(EpochId, CheckpointSequenceNumber)>> {
        Ok(self
            .perpetual_tables
            .executed_transactions_to_checkpoint
            .get(digest)?)
    ...
    pub fn deprecated_multi_get_transaction_checkpoint(
        &self,
        digests: &[TransactionDigest],
    ) -> MysResult<Vec<Option<(EpochId, CheckpointSequenceNumber)>>> {
        Ok(self
            .perpetual_tables
            .executed_transactions_to_checkpoint
            .multi_get(digests)?
    ...
    pub fn database_is_empty(&self) -> MysResult<bool> {
    ...
    pub fn multi_object_exists_by_key(&self, object_keys: &[ObjectKey]) -> MysResult<Vec<bool>> {
        Ok(self
            .perpetual_tables
            .objects
    ...
    fn get_object_ref_prior_to_key(
        ...
        object_id: &ObjectID,
        version: VersionNumber,
    ) -> Result<Option<ObjectRef>, MysError> {
    ...
    ) -> Result<Vec<Option<Object>>, MysError> {
        let wrappers = self
            .perpetual_tables
            .objects
    ...
    /// Get many objects
    pub fn get_objects(&self, objects: &[ObjectID]) -> Result<Vec<Option<Object>>, MysError> {
    ...
    pub(crate) fn insert_genesis_object(&self, object: Object) -> MysResult {
    ...
    fn insert_object_direct(&self, object_ref: ObjectRef, object: &Object) -> MysResult {
    ...
    pub(crate) fn bulk_insert_genesis_objects(&self, objects: &[Object]) -> MysResult<()> {
    ...
    pub fn bulk_insert_live_objects(
        ...
        if *expected_sha3_digest != sha3_digest {
            ...
            return Err(MysError::from("Sha does not match"));
        }
        batch.write()?;
        Ok(())
    }
    ...
    pub fn get_epoch_start_configuration(&self) -> MysResult<Option<EpochStartConfiguration>> {
        ...
        for (live_marker, lock, obj_ref) in izip!(
            ...
            if let Some(previous_tx_digest) = &lock {
                if previous_tx_digest == &tx_digest {
                    ...
                    return Err(MysError::ObjectLockConflict {
                        obj_ref: *obj_ref,
    ...
    pub fn check_owned_objects_are_live(&self, objects: &[ObjectRef]) -> MysResult {
        let locks = self
            .perpetual_tables
            .live_owned_object_markers
            .multi_get(objects)?;
    ...
    /// Returns MysError::ObjectLockAlreadyInitialized if the lock already exists and is locked to a transaction
    ...
    pub fn initialize_live_object_markers(
        ...
        if !is_force_reset {
            ...
            if !existing_live_object_markers.is_empty() {
                ...
                return Err(MysError::ObjectLockAlreadyInitialized {
    ...
    pub fn revert_state_update(&self, tx_digest: &TransactionDigest) -> MysResult {
    ...
    pub fn get_latest_object_or_tombstone(
        &self,
        object_id: ObjectID,
    ) -> Result<Option<(ObjectKey, ObjectOrTombstone)>, MysError> {
    ...
    ) -> MysResult<Vec<Option<VerifiedTransaction>>> {
        Ok(self
            .perpetual_tables
            .transactions
    ...
    /// If the intent is for testing, you can use AuthorityState:: get_mys_system_state_object_for_testing.
    pub fn get_mys_system_state_object_unsafe(&self) -> MysResult<MysSystemState> {
        get_mys_system_state(self.perpetual_tables.as_ref())
    }

    pub fn expensive_check_mys_conservation<T>(
        self: &Arc<Self>,
        type_layout_store: T,
    ...
    {
        if !self.enable_epoch_mys_conservation_check {
            return Ok(());
        }

        let executor = old_epoch_store.executor();
        info!("Starting MYS conservation check. This may take a while..");
        let cur_time = Instant::now();
        let mut pending_objects = vec![];
        let mut count = 0;
        let mut size = 0;
        let (mut total_mys, mut total_storage_rebate) = thread::scope(|s| {
                        ...
                        if count % 1_000_000 == 0 {
                            ...
                            pending_tasks.push(s.spawn(move || {
                                ...
                                for object in task_objects {
                                    ...
                                    // get_total_mys includes storage rebate, however all storage rebate is
                                    // also stored in the storage fund, so we need to subtract it here.
                                    total_mys +=
                                        object.get_total_mys(layout_resolver.as_mut()).unwrap()
                                            - object.storage_rebate;
                                }
                                if count % 50_000_000 == 0 {
                                    info!("Processed {} objects", count);
                                }
                                (total_mys, total_storage_rebate)
                            }));
                        }
                    }
        ...
        for object in pending_objects {
            total_storage_rebate += object.storage_rebate;
            total_mys +=
                object.get_total_mys(layout_resolver.as_mut()).unwrap() - object.storage_rebate;
        }
        info!(
        ...
        let system_state = self
            ...
            .expect("Reading mys system state object cannot fail")
            .into_mys_system_state_summary();
        let storage_fund_balance = system_state.storage_fund_total_object_storage_rebates;
        info!(
            "Total MYS amount in the network: {}, storage fund balance: {}, total storage rebate: {} at beginning of epoch {}",
            total_mys, storage_fund_balance, total_storage_rebate, system_state.epoch
        );
        ...
        self.metrics
            .mys_conservation_imbalance
            .set((total_mys as i128 - TOTAL_SUPPLY_MIST as i128) as i64);
        ...
        if let Some(expected_mys) = self
            .perpetual_tables
            .expected_network_mys_amount
            .get(&())
            .expect("DB read cannot fail")
        {
            fp_ensure!(
                total_mys == expected_mys,
                MysError::from(
                    format!(
                        "Inconsistent state detected at epoch {}: total mys: {}, expecting {}",
                        system_state.epoch, total_mys, expected_mys
                    )
                    .as_str()
                )
            );
...
impl AccumulatorStore for AuthorityStore {
    ...
    fn get_root_state_accumulator_for_epoch(
        &self,
        epoch: EpochId,
    ) -> MysResult<Option<(CheckpointSequenceNumber, Accumulator)>> {
        self.perpetual_tables
    ...
    fn get_root_state_accumulator_for_highest_epoch(
        &self,
    ) -> MysResult<Option<(EpochId, (CheckpointSequenceNumber, Accumulator))>> {
        Ok(self
            .perpetual_tables
            .root_state_hash_by_epoch
            .safe_iter()
...
pub type MysLockResult = MysResult<ObjectLockStatus>;
