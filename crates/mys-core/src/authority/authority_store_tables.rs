...
pub struct AuthorityPerpetualTables {
    ...
    /// Expected total amount of MYS in the network. This is expected to remain constant
    ...
    pub(crate) expected_network_mys_amount: DBMap<(), u64>,
...
impl AuthorityPerpetualTables {
    ...
    // Constructs `mys_types::object::Object` from `StoreObjectWrapper`.
    // Returns `None` if object was deleted/wrapped
    pub fn object(
        &self,
        object_key: &ObjectKey,
        store_object: StoreObjectWrapper,
    ) -> Result<Option<Object>, MysError> {
    ...
    pub fn get_latest_object_or_tombstone(
        &self,
        object_id: ObjectID,
    ) -> Result<Option<(ObjectKey, StoreObjectWrapper)>, MysError> {
        let mut iterator = self
            .objects
            .unbounded_iter()
            .skip_prior_to(&ObjectKey::max_for_id(&object_id))?;
    ...
    pub fn get_highest_pruned_checkpoint(&self) -> MysResult<CheckpointSequenceNumber> {
    ...
    ) -> MysResult<Option<TrustedTransaction>> {
    ...
    pub fn get_effects(&self, digest: &TransactionDigest) -> MysResult<Option<TransactionEffects>> {
    ...
    pub fn reset_db_for_execution_since_genesis(&self) -> MysResult {
        ...
        self.expected_network_mys_amount.unsafe_clear()?;
    ...
    pub fn insert_object_test_only(&self, object: Object) -> MysResult {
    ...
    // fallible get object methods for mys-tool, which may need to attempt to read a corrupted database
    pub fn get_object_fallible(&self, object_id: &ObjectID) -> MysResult<Option<Object>> {
        let obj_entry = self
            .objects
            .unbounded_iter()
            .skip_prior_to(&ObjectKey::max_for_id(object_id))?
