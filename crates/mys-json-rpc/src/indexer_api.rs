...
impl<R: ReadApiServer> IndexerApi<R> {
    ...
    fn extract_values_from_dynamic_field_name(
    ...
    ) -> Result<(TypeTag, Vec<u8>), MysRpcInputError> {
        ...
        let mys_json_value = MysJsonValue::new(value)?;
        let name_bcs_value = mys_json_value.to_bcs_bytes(&layout)?;
...
impl<R: ReadApiServer> IndexerApiServer for IndexerApi<R> {
    ...
    async fn get_owned_objects(
        ...
        with_tracing!(async move {
            let limit =
                validate_limit(limit, *QUERY_MAX_RESULT_LIMIT).map_err(MysRpcInputError::from)?;
            ...
            let data = match options.is_not_in_object_info() {
                ...
                false => objects
                    .into_iter()
                    .map(|o_info| MysObjectResponse::try_from((o_info, options.clone())))
                    .collect::<Result<Vec<MysObjectResponse>, _>>()?,
            };
    ...
    async fn query_transaction_blocks(
        ...
        with_tracing!(async move {
            ...
            let data: Vec<MysTransactionBlockResponse> = if opts.only_digest() {
                digests
                    .into_iter()
    ...
    async fn get_dynamic_field_object(
        ...
        with_tracing!(async move {
            ...
            if let Some(id) = id {
                self.read_api
                    .get_object(id, Some(MysObjectDataOptions::full_content()))
                    .await
                    .map_err(Error::from)
            } else {
                Ok(MysObjectResponse::new_with_error(
                    MysObjectResponseError::DynamicFieldNotFound { parent_object_id },
                ))
            }
        })
    }
