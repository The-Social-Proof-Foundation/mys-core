...
use mys_types::base_types::{ObjectID, SequenceNumber, TransactionDigest};
...
use mys_types::error::{MysError, MysObjectResponseError};
...
struct IntermediateTransactionResponse {
    ...
    events: Option<MysTransactionBlockEvents>,
...
impl ReadApi {
    ...
    async fn multi_get_transaction_blocks_internal(
        ...
        opts: Option<MysTransactionBlockResponseOptions>,
    ) -> Result<Vec<MysTransactionBlockResponse>, Error> {
        trace!("start");
        ...
        if temp_response.len() < num_digests {
            Err(MysRpcInputError::ContainsDuplicates)?
        }
            ...
            for (_, cache_entry) in temp_response.iter_mut() {
                    ...
                    cache_entry.effects.as_ref().and_then(|e| e.events_digest())
                {
                    match events.next() {
                        Some(Some(ev)) => {
                            cache_entry.events =
                                Some(to_mys_transaction_events(self, cache_entry.digest, ev)?)
                        }
                        None | Some(None) => {
                    ...
                    cache_entry.events = Some(MysTransactionBlockEvents::default());
                }
            }
        }
            ...
            for resp in temp_response.values() {
                ...
                results.push(get_balance_changes_from_effect(
                    ...
                    resp.effects.as_ref().ok_or_else(|| {
                        MysRpcInputError::GenericNotFound(
...
impl ReadApiServer for ReadApi {
    ...
    async fn multi_get_objects(
        ...
        with_tracing!(async move {
            if object_ids.len() <= *QUERY_MAX_RESULT_LIMIT {
                ...
                let objects_result: Result<Vec<MysObjectResponse>, String> = results
                    .into_iter()
        ...
        with_tracing!(async move {
            ...
            match past_read {
                PastObjectRead::ObjectNotExists(id) => {
                    Ok(MysPastObjectResponse::ObjectNotExists(id))
                }
                ...
                PastObjectRead::ObjectDeleted(oref) => {
                    Ok(MysPastObjectResponse::ObjectDeleted(oref.into()))
                }
                PastObjectRead::VersionNotFound(id, seq_num) => {
                    Ok(MysPastObjectResponse::VersionNotFound(id, seq_num))
                }
                PastObjectRead::VersionTooHigh {
                    object_id,
                    asked_version,
                    latest_version,
                } => Ok(MysPastObjectResponse::VersionTooHigh {
                    object_id,
                    asked_version,
                    latest_version,
                }),
    ...
    async fn try_get_object_before_version(
        ...
        self.try_get_past_object(
            object_id,
            version,
            Some(MysObjectDataOptions::bcs_lossless()),
        )
        .await
    }
            ...
            if past_objects.len() <= *QUERY_MAX_RESULT_LIMIT {
                ...
                if !errors.is_empty() {
                    let error_string = errors
                        ...
                        .collect::<Vec<String>>()
                        .join("; ");
                    Err(anyhow!("{error_string}").into()) // Collects errors not related to MysPastObjectResponse variants
                } else {
                    Ok(success)
                }
            } else {
        ...
        with_tracing!(async move {
            ...
            if opts.show_events && temp_response.effects.is_some() {
                ...
                let events = spawn_monitored_task!(async move {
                    ...
                    .flatten();
                match events {
                    None => temp_response.events = Some(MysTransactionBlockEvents::default()),
                    Some(events) => match to_mys_transaction_events(self, digest, events) {
            ...
            Ok(match events {
                Some(events) => events
                    ...
                    .map(|(seq, e)| {
                        ...
                        MysEvent::try_from(e, transaction_digest, seq as u64, None, layout)
                    })
                    .collect::<Result<Vec<_>, _>>()
                    .map_err(Error::MysError)?,
                None => vec![],
            })
        }).await.map_err(Error::from)?
        })
    ...
    async fn verify_zklogin_signature(
        ...
        let zklogin_env_native = match self
            ...
            .chain()
        {
            mys_protocol_config::Chain::Mainnet | mys_protocol_config::Chain::Testnet => {
                ZkLoginEnv::Prod
            }
            _ => ZkLoginEnv::Test,
        };
        let GenericSignature::ZkLoginAuthenticator(zklogin_sig) =
            ...
            return Err(MysRpcInputError::GenericNotFound(
...
#[instrument(skip_all)]
fn to_mys_transaction_events(
    fullnode_api: &ReadApi,
    tx_digest: TransactionDigest,
    events: TransactionEvents,
) -> Result<MysTransactionBlockEvents, Error> {
    ...
    Ok(MysTransactionBlockEvents::try_from(
        events,
        tx_digest,
        None,
...
pub fn get_rendered_fields(
    ...
    let mys_move_value: MysMoveValue = MoveValue::Struct(move_struct.clone()).into();
    if let MysMoveValue::Struct(move_struct) = mys_move_value {
        let fields =
            fields
                .contents
                .iter()
...
fn parse_template(template: &str, move_struct: &MysMoveStruct) -> Result<String, Error> {
...
fn get_value_from_move_struct(
    ...
    let mut current_value = &MysMoveValue::Struct(move_struct.clone());
    // iterate over the parts and try to access the corresponding field
    for part in parts {
        match current_value {
            MysMoveValue::Struct(move_struct) => {
                if let MysMoveStruct::WithTypes { type_: _, fields }
                | MysMoveStruct::WithFields(fields) = move_struct
                {
    ...
    match current_value {
        MysMoveValue::Option(move_option) => match move_option.as_ref() {
            Some(move_value) => Ok(move_value.to_string()),
            None => Ok("".to_string()),
        },
        MysMoveValue::Vector(_) => Err(anyhow!(
...
fn convert_to_response(
    ...
    opts: &MysTransactionBlockResponseOptions,
    module_cache: &impl GetModule,
) -> RpcInterimResult<MysTransactionBlockResponse> {
    let mut response = MysTransactionBlockResponse::new(cache.digest);
    ...
    if let Some(transaction) = cache.transaction {
        ...
        if opts.show_input {
            response.transaction = Some(MysTransactionBlock::try_from(
