...
pub enum Error {
    ...
    #[error(transparent)]
    MysObjectResponseError(#[from] MysObjectResponseError),

    #[error(transparent)]
    MysRpcInputError(#[from] MysRpcInputError),
...
impl From<MysError> for Error {
    fn from(e: MysError) -> Self {
        match e {
            MysError::UserInputError { error } => Self::UserInputError(error),
            MysError::MysObjectResponseError { error } => Self::MysObjectResponseError(error),
            MysError::UnsupportedFeatureError { error } => Self::UnsupportedFeature(error),
            MysError::IndexStoreNotAvailable => Self::UnsupportedFeature(
                "Required indexes are not available on this node".to_string(),
            ),
            other => Self::MysError(other),
        }
    }
}
...
impl From<Error> for ErrorObjectOwned {
    ...
    fn from(e: Error) -> ErrorObjectOwned {
        match e {
            ...
            Error::MysObjectResponseError(err) => match err {
            ...
            Error::MysError(mys_error) => match mys_error {
                MysError::TransactionNotFound { .. }
                | MysError::TransactionsNotFound { .. }
                | MysError::TransactionEventsNotFound { .. } => invalid_params(mys_error),
                _ => failed(mys_error),
            },
...
pub enum MysRpcInputError {
    ...
    #[error("{0}")]
    CannotParseMysStructTag(String),
...
impl From<MysRpcInputError> for RpcError {
    fn from(e: MysRpcInputError) -> Self {
        RpcError::Call(invalid_params(e))
    }
}

impl From<MysRpcInputError> for ErrorObjectOwned {
    fn from(e: MysRpcInputError) -> Self {
        invalid_params(e)
    }
}
...
mod tests {
    ...
    use mys_types::crypto::AuthorityPublicKey;
    ...
    mod match_quorum_driver_error_tests {
        ...
        fn test_invalid_user_signature() {
            let quorum_driver_error =
                QuorumDriverError::InvalidUserSignature(MysError::InvalidSignature {
        ...
        fn test_objects_double_used() {
            use mys_types::crypto::VerifyingKey;
        ...
        fn test_non_recoverable_transaction_error_with_transient_errors() {
            let quorum_driver_error = QuorumDriverError::NonRecoverableTransactionError {
                errors: vec![
                    ...
                    (
                        MysError::RpcError("Hello".to_string(), "Testing".to_string()),
                        0,
                        vec![],
                    ),
                ],
        ...
        fn test_quorum_driver_internal_error() {
            let quorum_driver_error = QuorumDriverError::QuorumDriverInternalError(
                MysError::UnexpectedMessage("test".to_string()),
            );
        ...
        fn test_system_overload() {
            let quorum_driver_error = QuorumDriverError::SystemOverload {
                overloaded_stake: 10,
                errors: vec![(MysError::UnexpectedMessage("test".to_string()), 0, vec![])],
            };
