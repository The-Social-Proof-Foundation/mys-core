...
impl GovernanceReadApi {
    ...
    async fn get_staked_mys(&self, owner: MysAddress) -> Result<Vec<StakedMys>, Error> {
        let state = self.state.clone();
        let result =
            spawn_monitored_task!(async move { state.get_staked_mys(owner).await }).await??;

        self.metrics
            .get_stake_mys_result_size
            .observe(result.len() as f64);
        self.metrics
            .get_stake_mys_result_size_total
    ...
    async fn get_stakes_by_ids(
        ...
        let mut stakes: Vec<(StakedMys, bool)> = vec![];
        for stake in stakes_read.into_iter() {
            match stake {
                ObjectRead::Exists(_, o, _) => stakes.push((StakedMys::try_from(&o)?, true)),
                ObjectRead::Deleted(oref) => {
                    match self
                        ...
                        .await?
                    {
                        Some(o) => stakes.push((StakedMys::try_from(&o)?, false)),
                        None => Err(MysRpcInputError::UserInputError(
                ...
                ObjectRead::NotExists(id) => Err(MysRpcInputError::UserInputError(
                    UserInputError::ObjectNotFound {
                        object_id: id,
                        version: None,
                    },
    ...
    async fn get_stakes(&self, owner: MysAddress) -> Result<Vec<DelegatedStake>, Error> {
        let timer = self.metrics.get_stake_mys_latency.start_timer();
        let stakes = self.get_staked_mys(owner).await?;
        if stakes.is_empty() {
            return Ok(vec![]);
        }
        drop(timer);

        let _timer = self.metrics.get_delegated_mys_latency.start_timer();
    ...
    async fn get_delegated_stakes(
        ...
        let system_state_summary: MysSystemStateSummary =
            system_state.clone().into_mys_system_state_summary();
    ...
    fn get_system_state(&self) -> Result<MysSystemState, Error> {
...
impl GovernanceReadApiServer for GovernanceReadApi {
    ...
    async fn get_stakes_by_ids(
        ...
        with_tracing!(async move { self.get_stakes_by_ids(staked_mys_ids).await })
    }
    ...
    #[instrument(skip(self))]
    async fn get_committee_info(&self, epoch: Option<BigInt<u64>>) -> RpcResult<MysCommittee> {
    ...
    async fn get_validators_apy(&self) -> RpcResult<ValidatorApys> {
        ...
        let system_state_summary: MysSystemStateSummary =
            self.get_latest_mys_system_state().await?;
...
async fn exchange_rates(
    ...
    let system_state_summary: MysSystemStateSummary = system_state.into_mys_system_state_summary();
    ...
    for df in state.get_dynamic_fields(
        ...
        let pool_id: ID =
            bcs::from_bytes(&df.1.bcs_name).map_err(|e| MysError::ObjectDeserializationError {
                error: e.to_string(),
            })?;
    ...
    for (address, pool_id, exchange_rates_id, exchange_rates_size, active) in tables {
        let mut rates = state
            ...
            .map(|df| {
                ...
                Ok::<_, MysError>((epoch, exchange_rate))
            })
            .collect::<Result<Vec<_>, _>>()?;
...
impl MysRpcModule for GovernanceReadApi {
