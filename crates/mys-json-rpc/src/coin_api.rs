...
use mys_json_rpc_types::{CoinPage, MysCoinMetadata};
...
pub fn parse_to_struct_tag(coin_type: &str) -> Result<StructTag, MysRpcInputError> {
    parse_mys_struct_tag(coin_type)
        .map_err(|e| MysRpcInputError::CannotParseMysStructTag(format!("{e}")))
}

pub fn parse_to_type_tag(coin_type: Option<String>) -> Result<TypeTag, MysRpcInputError> {
            ...
            let cursor = match cursor {
                Some(c) => {
                    let decoded = CoinCursor::decode(&c).ok_or_else(|| {
                        MysRpcInputError::GenericInvalid("invalid cursor".to_string())
                    })?;

                    if coin_type_tag.to_string() != decoded.coin_type {
                        return Err(
                            MysRpcInputError::GenericInvalid("invalid cursor".to_string()).into(),
                        );
                    }
                    (
                        decoded.coin_type,
...
async fn find_package_object_id(
    ...
    spawn_monitored_task!(async move {
        ...
        Err(MysRpcInputError::GenericNotFound(format!(
...
mod tests {
    ...
    use mys_types::messages_checkpoint::{CheckpointDigest, CheckpointSequenceNumber};
    use mys_types::object::MoveObject;
    use mys_types::object::Object;
    use mys_types::object::Owner;
    use mys_types::utils::create_fake_transaction;
    use mys_types::{parse_mys_struct_tag, TypeTag};
    ...
    mock! {
        ...
        impl TransactionKeyValueStoreTrait for KeyValueStore {
            ...
            ) -> MysResult<KVStoreTransactionData>;
            ...
            ) -> MysResult<KVStoreCheckpointData>;
            ...
            async fn get_object(&self, object_id: ObjectID, version: SequenceNumber) -> MysResult<Option<Object>>;
            ...
            async fn multi_get_events_by_tx_digests(&self,digests: &[TransactionDigest]) -> MysResult<Vec<Option<TransactionEvents>>>;
        }
    }
    ...
    fn get_test_coin_type_tag(coin_type: String) -> TypeTag {
        TypeTag::Struct(Box::new(parse_mys_struct_tag(&coin_type).unwrap()))
    }
    ...
    fn get_test_treasury_cap_peripherals(
        ...
        let input_coin_struct = parse_mys_struct_tag(&coin_name).expect("should not fail");
    ...
    mod get_coins_tests {
        ...
        async fn test_coin_no_cursor() {
            ...
            let coin_type_tag =
                TypeTag::Struct(Box::new(parse_mys_struct_tag(&coin.coin_type).unwrap()));
            let mut mock_state = MockStateRead::new();
            mock_state
                .expect_get_owned_coins()
                .with(
        ...
        async fn test_coin_with_cursor() {
            ...
            let coin_type_tag =
                TypeTag::Struct(Box::new(parse_mys_struct_tag(&coins[0].coin_type).unwrap()));
            let mut mock_state = MockStateRead::new();
            mock_state
                .expect_get_owned_coins()
                .with(
        ...
        async fn test_unrecognized_token() {
            ...
            expected.assert_eq(&error_object.code().to_string());
            let expected =
                expect!["Invalid struct type: 0x2::mys:🤵. Got error: unrecognized token: :🤵"];
            expected.assert_eq(error_object.message());
        }
            ...
            mock_state
                ...
                .returning(move |_, _, _, _| {
                    Err(StateReadError::Client(
                        MysError::IndexStoreNotAvailable.into(),
                    ))
                });
            let coin_read_api = CoinReadApi::new_for_tests(Arc::new(mock_state), None);
            let response = coin_read_api
        ...
        async fn test_get_coins_iterator_typed_store_error() {
            ...
            mock_state
                .expect_get_owned_coins()
                .returning(move |_, _, _, _| {
                    Err(MysError::Storage("mock rocksdb error".to_string()).into())
                });
            let coin_read_api = CoinReadApi::new_for_tests(Arc::new(mock_state), None);
            let response = coin_read_api
                .get_coins(owner, Some(coin_type.to_string()), None, None)
    ...
    mod get_balance_tests {
        ...
        async fn test_get_balance_execution_error() {
            ...
            let mut mock_state = MockStateRead::new();
            mock_state.expect_get_balance().returning(move |_, _| {
                Err(MysError::ExecutionError("mock db error".to_string()).into())
            });
            let coin_read_api = CoinReadApi::new_for_tests(Arc::new(mock_state), None);
            let response = coin_read_api
                .get_balance(owner, Some(coin_type.to_string()))
    ...
    mod get_coin_metadata_tests {
        ...
        async fn test_valid_coin_metadata_object() {
            ...
            let metadata = MysCoinMetadata::try_from(coin_metadata_object.clone()).unwrap();
        ...
        #[tokio::test]
        async fn test_find_package_object_not_mys_coin_metadata() {
            let package_id = get_test_package_id();
            let coin_name = get_test_coin_type(package_id);
            let input_coin_struct = parse_mys_struct_tag(&coin_name).expect("should not fail");
            let coin_metadata_struct = CoinMetadata::type_(input_coin_struct.clone());
