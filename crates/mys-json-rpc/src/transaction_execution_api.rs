...
use mys_json_rpc_api::{JsonRpcMetrics, WriteApiOpenRpc, WriteApiServer};
use mys_json_rpc_types::{
    DevInspectArgs, DevInspectResults, DryRunTransactionBlockResponse, MysTransactionBlock,
...
impl TransactionExecutionApi {
    ...
    > {
        ...
        let transaction = if opts.show_input {
            ...
            Some(MysTransactionBlock::try_from(
                txn.data().clone(),
    ...
    ) -> Result<MysTransactionBlockResponse, Error> {
        let request_type =
    ...
    async fn handle_post_orchestration(
        ...
        transaction: Option<MysTransactionBlock>,
        ...
        let events = if opts.show_events {
            ...
            Some(MysTransactionBlockEvents::try_from(
    ...
    pub fn prepare_dry_run_transaction_block(
        &self,
        tx_bytes: Base64,
    ) -> Result<(TransactionData, TransactionDigest, Vec<InputObjectKind>), MysRpcInputError> {
        let tx_data: TransactionData = self.convert_bytes(tx_bytes)?;
...
impl MysRpcModule for TransactionExecutionApi {
