    ...
    async fn get_owned_objects(
        ...
        Ok(self
            ...
            .get_owner_objects(
                address,
                None,
                Some(MysObjectDataFilter::StructType(object_type)),
            )?)
    }
...
impl TransactionBuilderServer for TransactionBuilderApi {
    ...
    async fn pay_all_mys(
        ...
        let data = self
            .0
            .pay_all_mys(signer, input_coins, recipient, *gas_budget)
            .await
            .map_err(crate::Error::from)?;
        Ok(TransactionBlockBytes::from_data(data).map_err(crate::Error::from)?)
    }
    ...
    async fn move_call(
        ...
        rpc_arguments: Vec<MysJsonValue>,
        gas: Option<ObjectID>,
        gas_budget: BigInt<u64>,
        _txn_builder_mode: Option<MysTransactionBlockBuilderMode>,
    ) -> RpcResult<TransactionBlockBytes> {
    ...
    async fn request_withdraw_stake(
        ...
        Ok(TransactionBlockBytes::from_data(
            self.0
                .request_withdraw_stake(signer, staked_mys, gas, *gas_budget)
                .await
                .map_err(crate::Error::from)?,
        )
        .map_err(crate::Error::from)?)
...
impl MysRpcModule for TransactionBuilderApi {
