...
//! MysNodeHandle wraps MysNode in a way ttttable for access by test code.
//!
//! When starting a MysNode directly, in a test (as opposed to using Swarm), the node may be
...
//! simulator node rather than the node in which the MysNode is running.
//!
//! MysNodeHandle provides an easy way to do the right thing here:
//!
//! ```ignore
//!     let node_handle = start_node(config, registry).await;
//!     node_handle.with_async(|mys_node| async move {
//!         spawn_checkpoint_processes(config, &[mys_node]).await;
//!     });
//! ```
//!
...
//!
//! ```ignore
//!    let state = node_handle.with(|mys_node| mys_node.state);
//!    // DO NOT DO THIS!
...
pub struct MysNodeHandle {
    node: Option<Arc<MysNode>>,
    shutdown_on_drop: bool,
}

impl MysNodeHandle {
    pub fn new(node: Arc<MysNode>) -> Self {
        Self {
            node: Some(node),
    ...
    pub fn inner(&self) -> &Arc<MysNode> {
        self.node.as_ref().unwrap()
    }

    pub fn with<T>(&self, cb: impl FnOnce(&MysNode) -> T) -> T {
        let _guard = self.guard();
        cb(self.inner())
    }

    pub fn state(&self) -> Arc<AuthorityState> {
        self.with(|mys_node| mys_node.state())
    }
...
#[cfg(msim)]
impl MysNodeHandle {
    fn guard(&self) -> mys_simulator::runtime::NodeEnterGuard {
...
#[cfg(msim)]
impl Drop for MysNodeHandle {
    fn drop(&mut self) {
        if self.shutdown_on_drop {
            let node_id = self.inner().sim_state.sim_node.id();
            mys_simulator::runtime::Handle::try_current().map(|h| h.delete_node(node_id));
        }
    }
}

impl From<Arc<MysNode>> for MysNodeHandle {
    fn from(node: Arc<MysNode>) -> Self {
        MysNodeHandle::new(node)
    }
}
