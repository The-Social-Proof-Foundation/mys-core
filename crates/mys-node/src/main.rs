...
use mys_node::metrics;
use mys_telemetry::send_telemetry_event;
...
fn main() {
    ...
    move_vm_profiler::tracing_feature_enabled! {
        panic!("Cannot run the mys-node binary with tracing feature enabled");
    }
    ...
    let runtimes = MysRuntimes::new(&config);
    ...
    // if it deadlocks.
    let node_once_cell = Arc::new(AsyncOnceCell::<Arc<mys_node::MysNode>>::new());
    let node_once_cell_clone = node_once_cell.clone();
    let rpc_runtime = runtimes.json_rpc.handle().clone();

    // let mys-node signal main to shutdown runtimes
    let (runtime_shutdown_tx, runtime_shutdown_rx) = broadcast::channel::<()>(1);

    runtimes.mys_node.spawn(async move {
        match mys_node::MysNode::start_async(config, registry_service, Some(rpc_runtime), VERSION).await {
            Ok(mys_node) => node_once_cell_clone
                .set(mys_node)
        ...
        // when we get a shutdown signal from mys-node, forward it on to the runtime_shutdown_channel here in
        // main to signal runtimes to all shutdown.
        tokio::select! {
           _ = shutdown_rx.recv() => {
                runtime_shutdown_tx.send(()).expect("failed to forward shutdown signal from mys-node to mys-node main");
            }
        }
    ...
    runtimes.metrics.spawn(async move {
        ...
        mys_node::admin::run_admin_server(node, admin_interface_port, filter_handle).await
    });
