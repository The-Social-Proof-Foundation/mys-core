...
module test::m1 {
    ...
    public entry fun add_child(parent: &mut Object, ctx: &mut TxContext) {
        ...
        mys::dynamic_object_field::add(&mut parent.id, 0, child);
    }

    public fun transfer_child(parent: &mut Object, ctx: &TxContext) {
        let child: Child = mys::dynamic_object_field::remove(&mut parent.id, 0);
        mys::dynamic_object_field::add(&mut parent.id, 1, child);
        let child: Child = mys::dynamic_object_field::remove(&mut parent.id, 1);
    ...
    public fun delete_child(parent: &mut Object) {
        let child: Child = mys::dynamic_object_field::remove(&mut parent.id, 0);
        mys::dynamic_object_field::add(&mut parent.id, 1, child);
        let Child { id } = mys::dynamic_object_field::remove(&mut parent.id, 1);
        object::delete(id);
    }
    ...
    public fun wrap_child(parent: &mut Object) {
        let child: Child = mys::dynamic_object_field::remove(&mut parent.id, 0);
        mys::dynamic_object_field::add(&mut parent.id, 1, child);
        let child = mys::dynamic_object_field::remove(&mut parent.id, 1);
