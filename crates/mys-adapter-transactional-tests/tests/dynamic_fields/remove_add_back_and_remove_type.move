...
module test::m1 {
    ...
    public entry fun test_dof(parent: &mut Object, ctx: &mut TxContext) {
        ...
        let C1 { id } = mys::dynamic_object_field::remove(&mut parent.id, 0);
        object::delete(id);
    ...
    public entry fun test_df(parent: &mut Object) {
        mys::dynamic_field::add(&mut parent.id, 0, b"true");
        let _: vector<u8> = mys::dynamic_field::remove(&mut parent.id, 0);
        mys::dynamic_field::add(&mut parent.id, 0, true);
        let _: bool = mys::dynamic_field::remove(&mut parent.id, 0);
    }
}
