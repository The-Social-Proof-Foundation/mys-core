...
module test::m {
    ...
    public entry fun mint(ctx: &mut TxContext) {
        let id = mys::object::new(ctx);
        mys::transfer::public_transfer(S { id }, tx_context::sender(ctx))
    }
    ...
    public entry fun remove_and_wrap(parent: &mut S, idx: u64, ctx: &mut TxContext) {
        let child: S = ofield::remove(&mut parent.id, idx);
        ofield::add(&mut parent.id, idx, R { id: mys::object::new(ctx), s: child })
    }
    ...
    public entry fun wrap(s: S, ctx: &mut TxContext) {
        ...
        mys::transfer::public_transfer(r, tx_context::sender(ctx))
    }

    public entry fun freeze_object(s: S) {
        mys::transfer::public_freeze_object(s)
    }
}
