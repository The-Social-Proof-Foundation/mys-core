...
module t::m {
    ...
    public entry fun create(ctx: &mut TxContext) {
        ...
        mys::dynamic_field::add(&mut o.id, 0, Obj { id: object::new(ctx) });
        mys::dynamic_object_field::add(&mut o.id, 0, Obj { id: object::new(ctx) });
    ...
    public entry fun share_wrapped(o: &mut Obj) {
        let inner: Obj = mys::dynamic_field::remove(&mut o.id, 0);
        transfer::public_share_object(inner)
    }

    public entry fun share_child(o: &mut Obj) {
        let inner: Obj = mys::dynamic_object_field::remove(&mut o.id, 0);
        transfer::public_share_object(inner)
    }
