...
mod generated {
    include!(concat!(env!("OUT_DIR"), "/mys.StateSync.rs"));
}
mod builder;
mod metrics;
mod server;
...
use mys_data_ingestion_core::{ReaderOptions, setup_single_workflow_with_options};
...
impl Iterator for PeerBalancer {
    ...
    fn next(&mut self) -> Option<Self::Item> {
        ...
        if explore {
            if let Some((peer, _)) = self.select_by_rtt() {
                return Some(StateSyncClient::new(peer));
            }
        }

        if has_eligible_known {
            if let Some((peer, _)) = self.select_by_throughput() {
                return Some(StateSyncClient::new(peer));
            }
        }
    ...
    S: WriteStore + Clone + Send + Sync + 'static,
{
    ...
    fn maybe_start_checkpoint_summary_sync_task(&mut self) {
        ...
        if let Some(target_seq) = highest_known_sequence_number {
            if *highest_processed_checkpoint.sequence_number() < target_seq {
...
where
    S: WriteStore,
{
    let _sequence_number = checkpoint.sequence_number;
    let digest = checkpoint.content_digest;
    if let Some(contents) = store.get_full_checkpoint_contents(&digest) {
        debug!("store already contains checkpoint contents");
        return Some(VersionedFullCheckpointContents::V1(contents));
    }
