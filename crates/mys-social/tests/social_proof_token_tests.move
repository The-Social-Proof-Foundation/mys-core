...
module social_contracts::social_proof_token_tests {
    use std::vector;
    use std::string;
    ...
    use social_contracts::social_proof_token::{Self, ExchangeConfig, TokenRegistry, SocialToken};
    ...
    // Token types from social_proof_token module
    ...
    #[test]
    fun test_social_proof_token_initialization() {
        let mut scenario = test_scenario::begin(ADMIN);
        
        // Initialize the social proof token system
        {
            social_proof_token::init_for_testing(test_scenario::ctx(&mut scenario));
        };
        ...
        test_scenario::next_tx(&mut scenario, ADMIN);
        {
            ...
            let admin_cap = test_scenario::take_from_sender<social_proof_token::ExchangeAdminCap>(&scenario);
            test_scenario::return_to_sender(&scenario, admin_cap);
            
            // Check that registry was shared
            let registry = test_scenario::take_shared<social_proof_token::TokenRegistry>(&scenario);
            test_scenario::return_shared(registry);
            
            // Check that config was shared
            let config = test_scenario::take_shared<social_proof_token::ExchangeConfig>(&scenario);
    ...
    fun test_config_update() {
        ...
        {
            let admin_cap = test_scenario::take_from_sender<social_proof_token::ExchangeAdminCap>(&scenario);
            let mut config = test_scenario::take_shared<social_proof_token::ExchangeConfig>(&scenario);
            
            let ecosystem_treasury = @0x67890;
            
            social_proof_token::update_exchange_config(
                &admin_cap,
                &mut config,
                200, // total_fee_bps (2%)
    ...
    fun setup_test_scenario(): Scenario {
        ...
        // Initialize social_proof_token module first
        {
            social_proof_token::init_for_testing(test_scenario::ctx(&mut scenario));
        };
    ...
    fun setup_viral_profile(scenario: &mut Scenario): (address, address) {
        ...
        // to be used with mock check_profile_viral_threshold from social_proof_token
        let registry_id = {
