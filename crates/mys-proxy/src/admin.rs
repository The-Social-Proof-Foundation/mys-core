...
use crate::peers::{AllowedPeer, MysNodeProvider};
use crate::var;
use anyhow::Error;
use anyhow::Result;
...
pub fn app(
    ...
    allower: Option<MysNodeProvider>,
) -> Router {
...
pub fn create_server_cert_default_allow(
    hostname: String,
) -> Result<ServerConfig, mys_tls::rustls::Error> {
    let CertKeyPair(server_certificate, _) = generate_self_cert(hostname);

    ClientCertVerifier::new(AllowAll, MYS_VALIDATOR_SERVER_NAME.to_string()).rustls_server_config(
        vec![server_certificate.rustls_certificate()],
        server_certificate.rustls_private_key(),
    )
}

/// Verify clients against mys blockchain, clients that are not found in mys_getValidators
/// will be rejected
pub fn create_server_cert_enforce_peer(
    dynamic_peers: DynamicPeerValidationConfig,
    static_peers: Option<StaticPeerValidationConfig>,
) -> Result<(ServerConfig, Option<MysNodeProvider>), mys_tls::rustls::Error> {
    let (Some(certificate_path), Some(private_key_path)) =
        (dynamic_peers.certificate_file, dynamic_peers.private_key)
    else {
        return Err(mys_tls::rustls::Error::General(
            "missing certs to initialize server".into(),
        ));
    };
    let static_peers = load_static_peers(static_peers).map_err(|e| {
        mys_tls::rustls::Error::General(format!("unable to load static pub keys: {}", e))
    })?;
    let allower = MysNodeProvider::new(dynamic_peers.url, dynamic_peers.interval, static_peers);
    allower.poll_peer_list();
    let c = ClientCertVerifier::new(allower.clone(), MYS_VALIDATOR_SERVER_NAME.to_string())
