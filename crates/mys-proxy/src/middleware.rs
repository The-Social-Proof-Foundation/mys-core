...
/// we expect mys-node to send us an http header content-length encoding.
...
/// we expect mys-node to send us an http header content-type encoding.
...
/// we expect that calling mys-nodes are known on the blockchain and we enforce
/// their pub key tls creds here
pub async fn expect_valid_public_key(
    Extension(allower): Extension<Arc<MysNodeProvider>>,
