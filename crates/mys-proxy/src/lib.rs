...
mod tests {
    ...
    use crate::{admin::CertKeyPair, config::RemoteWriteConfig, peers::MysNodeProvider};
    ...
    /// mys-node client that posts data to the proxy using the protobuf format.  The server processes this
    ...
    async fn test_axum_acceptor() {
        ...
        let CertKeyPair(client_priv_cert, client_pub_key) = admin::generate_self_cert("mys".into());
        ...
        let mut allower = MysNodeProvider::new("".into(), Duration::from_secs(30), vec![]);
        ...
        allower.get_mys_mut().write().unwrap().insert(
