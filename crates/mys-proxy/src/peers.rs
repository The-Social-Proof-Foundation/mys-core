...
/// MysNodeProvider queries the mys blockchain and keeps a record of known validators based on the response from
/// mys_getValidators.  The node name, public key and other info is extracted from the chain and stored in this
...
#[derive(Debug, Clone)]
pub struct MysNodeProvider {
    mys_nodes: AllowedPeers,
...
impl MysNodeProvider {
    pub fn new(
        ...
        let mys_nodes = Arc::new(RwLock::new(HashMap::new()));
    ...
    pub fn get(&self, key: &Ed25519PublicKey) -> Option<AllowedPeer> {
        ...
        // check mys validators
        if let Some(v) = self.mys_nodes.read().unwrap().get(key) {
            return Some(AllowedPeer {
                name: v.name.to_owned(),
                public_key: v.public_key.to_owned(),
            });
    ...
    /// Get a mutable reference to the allowed mys validator map
    pub fn get_mys_mut(&mut self) -> &mut AllowedPeers {
        &mut self.mys_nodes
    }

    /// get_validators will retrieve known validators
    async fn get_validators(url: String) -> Result<MysSystemStateSummary> {
        let rpc_method = "mysx_getLatestMysSystemState";
        let observe = || {
    ...
    async fn get_bridge_validators(url: String) -> Result<BridgeSummary> {
        let rpc_method = "mysx_getLatestBridge";
    ...
    async fn update_mys_validator_set(&self) {
        match Self::get_validators(self.rpc_url.to_owned()).await {
            Ok(summary) => {
                let validators = extract(summary);
                let mut allow = self.mys_nodes.write().unwrap();
                allow.clear();
                allow.extend(validators);
                info!(
                    "{} mys validators managed to make it on the allow list",
                    allow.len()
                );
            }
            Err(error) => {
    ...
    async fn update_bridge_validator_set(&self, metrics_keys: MetricsPubKeys) {
        let mys_system = match Self::get_validators(self.rpc_url.to_owned()).await {
            Ok(summary) => summary,
            Err(error) => {
                JSON_RPC_STATE
                    .with_label_values(&["update_bridge_peer_count", "failed"])
                    .inc();
                error!("unable to get mys system state: {error}");
                return;
            }
        };
        match Self::get_bridge_validators(self.rpc_url.to_owned()).await {
            Ok(summary) => {
                let names = mys_system
                    .active_validators
                    .into_iter()
                    .map(|v| (v.mys_address, v.name))
                    .collect();
            ...
            Err(error) => {
                JSON_RPC_STATE
                    .with_label_values(&["update_bridge_peer_count", "failed"])
                    .inc();
                error!("unable to refresh mys bridge peer list: {error}");
            }
        };
    }
    ...
    pub fn poll_peer_list(&self) {
        ...
        tokio::spawn(async move {
            ...
            loop {
                ...
                cloned_self.update_mys_validator_set().await;
                cloned_self
...
/// extract will get the network pubkey bytes from a MysValidatorSummary type.  This type comes from a
...
fn extract(
    ...
    summary.active_validators.into_iter().filter_map(|vm| {
        match Ed25519PublicKey::from_bytes(&vm.network_pubkey_bytes) {
            Ok(public_key) => {
                debug!(
                    "adding public key {:?} for mys validator {:?}",
                    public_key, vm.name
                );
                Some((
                    public_key.clone(),
            ...
            Err(error) => {
                error!(
                    "unable to decode public key for name: {:?} mys_address: {:?} error: {error}",
                    vm.name, vm.mys_address
                );
...
async fn extract_bridge(
    ...
    names: Arc<BTreeMap<MysAddress, String>>,
    ...
    let results: Vec<_> = stream::iter(committee_members)
        .filter_map(|(_, cm)| {
            ...
            async move {
                ...
                let bridge_name = names.get(&cm.mys_address).cloned().unwrap_or_else(|| {
                    warn!(
                        address =% cm.mys_address,
                        "Bridge node not found in mys committee, using base URL as the name",
                    );
...
mod tests {
    ...
    #[test]
    fn depend_on_mys_mys_system_state_summary() {
        let CertKeyPair(_, client_pub_key) = generate_self_cert("mys".into());
        // all fields here just satisfy the field types, with exception to active_validators, we use
        // some of those.
        let depends_on = MysSystemStateSummary {
            active_validators: vec![MysValidatorSummary {
        ...
        let r = serde_json::to_string(&ResponseBody { result: depends_on })
            .expect("expected to serialize ResponseBody{MysSystemStateSummary}");

        let deserialized = serde_json::from_str::<ResponseBody>(&r)
            .expect("expected to deserialize ResponseBody{MysSystemStateSummary}");
