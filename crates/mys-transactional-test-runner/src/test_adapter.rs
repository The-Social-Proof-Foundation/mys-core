...
use mys_json_rpc_types::{
    ...
    MysTransactionBlockEffects, MysTransactionBlockEffectsAPI, MysTransactionBlockEvents,
};
...
use mys_types::base_types::{SequenceNumber, VersionNumber};
use mys_types::committee::EpochId;
use mys_types::crypto::{get_authority_key_pair, RandomnessRound};
use mys_types::digests::{ConsensusCommitDigest, TransactionDigest, TransactionEventsDigest};
...
use mys_types::storage::{ObjectStore, RpcStateReader};
...
use mys_types::{
    ...
    MOVE_STDLIB_ADDRESS, MYS_CLOCK_OBJECT_ID, MYS_FRAMEWORK_ADDRESS, MYS_SYSTEM_STATE_OBJECT_ID,
};
use mys_types::{execution_status::ExecutionStatus, transaction::TransactionKind};
use mys_types::{gas::GasCostSummary, object::GAS_VALUE_FOR_TESTING};
use mys_types::{
    move_package::MovePackage,
    transaction::{Argument, CallArg},
};
use mys_types::{
    programmable_transaction_builder::ProgrammableTransactionBuilder, MYS_FRAMEWORK_PACKAGE_ID,
};
...
pub struct OffChainConfig {
    ...
    /// URL for the Mys REST API. To be passed to the offchain indexer if it uses the REST API.
    pub rest_api_url: Option<String>,
}

pub struct MysTestAdapter {
...
impl AdapterInitConfig {
    fn from_args(init_cmd: InitCommand, mys_args: MysInitArgs) -> Self {
...
#[async_trait]
impl<'a> MoveTestAdapter<'a> for MysTestAdapter {
    type ExtraPublishArgs = MysPublishArgs;
    type ExtraRunArgs = MysRunArgs;
    type ExtraInitArgs = MysInitArgs;
    type ExtraValueArgs = MysExtraValueArgs;
    type Subcommand = MysSubcommand<Self::ExtraValueArgs, Self::ExtraRunArgs>;
    ...
    fn render_command_input(
        ...
        match &task.command {
            TaskCommand::Subcommand(MysSubcommand::ProgrammableTransaction(..)) => {
    ...
    async fn init(
        ...
        assert!(
            ...
            "Must populate 'pre_compiled_deps' with Mys framework"
        );
        ...
        let AdapterInitConfig {
            ...
            Some((init_cmd, mys_args)) => AdapterInitConfig::from_args(init_cmd, mys_args),
    ...
    async fn publish_modules(
        ...
        // we are assuming that all packages depend on Move Stdlib and Mys Framework, so these
        // don't have to be provided explicitly as parameters
        dependencies.extend([MOVE_STDLIB_PACKAGE_ID, MYS_FRAMEWORK_PACKAGE_ID]);
        let data = |sender, gas| {
    ...
    async fn call_function(
        ...
        self.next_task();
        let MysRunArgs { summarize, .. } = extra;
    ...
    async fn handle_subcommand(
        ...
        match command {
            MysSubcommand::RunGraphql(RunGraphqlCommand {
                show_usage,
                show_headers,
            ...
            MysSubcommand::RunJsonRpc(RunJsonRpcCommand {
                show_headers,
                cursors,
            }) => {
            ...
            MysSubcommand::CreateCheckpoint(CreateCheckpointCommand { count }) => {
            ...
            MysSubcommand::AdvanceEpoch(AdvanceEpochCommand {
                count,
                create_random_state,
            }) => {
                for _ in 0..count.unwrap_or(1) {
            ...
            MysSubcommand::AdvanceClock(AdvanceClockCommand { duration_ns }) => {
                self.executor
                    .advance_clock(Duration::from_nanos(duration_ns))
                    .await?;
                Ok(None)
            }
            MysSubcommand::SetRandomState(SetRandomStateCommand {
                randomness_round,
                random_bytes,
            ...
            MysSubcommand::ViewObject(ViewObjectCommand { id: fake_id }) => {
            ...
            MysSubcommand::TransferObject(TransferObjectCommand {
                id: fake_id,
                recipient,
                sender,
                gas_budget,
                ...
                let mut builder = ProgrammableTransactionBuilder::new();
                let obj_arg = MysValue::Object(fake_id, None).into_argument(&mut builder, self)?;
                ...
                let transaction = self.sign_txn(sender, |sender, gas| {
                    ...
                    builder.command(mys_types::transaction::Command::TransferObjects(
                        vec![obj_arg],
                        rec_arg,
                    ));
                    let pt = builder.finish();
            ...
            MysSubcommand::ConsensusCommitPrologue(ConsensusCommitPrologueCommand {
                timestamp_ms,
            }) => {
            ...
            MysSubcommand::ProgrammableTransaction(ProgrammableTransactionCommand {
                sender,
                sponsor,
                gas_budget,
                gas_price,
            ...
            MysSubcommand::UpgradePackage(UpgradePackageCommand {
                package,
                upgrade_capability,
                dependencies,
                sender,
            ...
            MysSubcommand::StagePackage(StagePackageCommand {
                syntax,
                dependencies,
            }) => {
                let syntax = syntax.unwrap_or_else(|| self.default_syntax());
            ...
            MysSubcommand::SetAddress(SetAddressCommand { address, input }) => {
                ...
                let (value, package) = match input {
                    MysValue::Object(fake_id, version) => {
                        let id = match self.fake_to_real_object_id(fake_id) {
                            Some(id) => id,
                    ...
                    MysValue::MoveValue(v) => {
                        let bytes = v.simple_serialize().unwrap();
                        let value: AccountAddress = bcs::from_bytes(&bytes)?;
                        (value, None)
                    }
                    MysValue::Digest(_) => bail!("digest is not supported as an input"),
                    MysValue::ObjVec(_) => bail!("obj vec is not supported as an input"),
                    MysValue::Receiving(_, _) => bail!("receiving is not supported as an input"),
                    MysValue::ImmShared(_, _) => {
...
impl MysTestAdapter {
    ...
    async fn upgrade_package(
        ...
        // Argument::Input(0)
        MysValue::Object(upgrade_capability, None).into_argument(&mut builder, self)?;
    ...
    fn sign_txn(
        ...
        txn_data: impl FnOnce(/* sender */ MysAddress, /* gas */ ObjectRef) -> TransactionData,
    ) -> Transaction {
    ...
    fn tx_summary_from_effects(
        &mut self,
        effects: MysTransactionBlockEffects,
        events: MysTransactionBlockEvents,
    ) -> anyhow::Result<TxnSummary> {
    ...
    fn stabilize_str(&self, input: impl AsRef<str>) -> String {
        fn candidate_is_hex(s: &str) -> bool {
            const HEX_STR_LENGTH: usize = MYS_ADDRESS_LENGTH * 2;
            let n = s.len();
...
impl<'a> GetModule for &'a MysTestAdapter {
...
static NAMED_ADDRESSES: Lazy<BTreeMap<String, NumericalAddress>> = Lazy::new(|| {
    ...
    // TODO fix Mys framework constants
    map.insert(
        "mys".to_string(),
        NumericalAddress::new(
            MYS_FRAMEWORK_ADDRESS.into_bytes(),
            move_compiler::shared::NumberFormat::Hex,
        ),
    );
    map.insert(
        "mys_system".to_string(),
        NumericalAddress::new(
            MYS_SYSTEM_ADDRESS.into_bytes(),
            move_compiler::shared::NumberFormat::Hex,
        ),
    );
    map.insert(
...
pub static PRE_COMPILED: Lazy<FullyCompiledProgram> = Lazy::new(|| {
    ...
    let mys_files: &Path = Path::new(DEFAULT_FRAMEWORK_PATH);
    let mys_system_sources = {
        let mut buf = mys_files.to_path_buf();
        buf.extend(["packages", "mys-system", "sources"]);
        buf.to_string_lossy().to_string()
    };
    let mys_sources = {
        let mut buf = mys_files.to_path_buf();
        buf.extend(["packages", "mys-framework", "sources"]);
        buf.to_string_lossy().to_string()
    };
    let mys_deps = {
        let mut buf = mys_files.to_path_buf();
    ...
    let fully_compiled_res = move_compiler::construct_pre_compiled_lib(
        vec![PackagePaths {
            name: Some(("mys-framework".into(), config)),
            paths: vec![
                mys_system_sources,
                mys_sources,
                mys_deps,
    ...
    match fully_compiled_res {
        Err((files, diags)) => {
            eprintln!("!!!Mys framework failed to compile!!!");
...
async fn init_val_fullnode_executor(
    ...
    // For each named Mys account without an address value, create an account with an address
    // and a gas object
...
async fn init_sim_executor(
    ...
    // For each named Mys account without an address value, create a key pair
    for n in account_names {
    ...
    if custom_validator_account {
        ...
        let (a, b): (MysAddress, Ed25519KeyPair) = get_key_pair_from_rng(&mut rng);
...
impl ObjectStore for MysTestAdapter {
...
impl ReadStore for MysTestAdapter {
    ...
    fn get_committee(
        &self,
        epoch: mys_types::committee::EpochId,
    ...
    fn get_lowest_available_checkpoint(
        &self,
    ) -> mys_types::storage::error::Result<CheckpointSequenceNumber> {
        self.executor.get_lowest_available_checkpoint()
    }

    fn get_checkpoint_by_digest(
        &self,
        digest: &mys_types::messages_checkpoint::CheckpointDigest,
