...
use crate::test_adapter::{FakeID, MysTestAdapter};
...
pub struct MysInitArgs {
    ...
    /// URL for the Mys REST API. To be passed to the offchain indexer and reader.
    #[clap(long)]
...
pub struct ProgrammableTransactionCommand {
    ...
    #[clap(
        long = "inputs",
        value_parser = ParsedValue::<MysExtraValueArgs>::parse,
        num_args(1..),
        action = clap::ArgAction::Append,
    )]
    pub inputs: Vec<ParsedValue<MysExtraValueArgs>>,
}
...
pub struct SetAddressCommand {
    pub address: String,
    #[clap(value_parser = ParsedValue::<MysExtraValueArgs>::parse)]
    pub input: ParsedValue<MysExtraValueArgs>,
}
...
#[derive(Debug)]
pub enum MysSubcommand<ExtraValueArgs: ParsableValue, ExtraRunArgs: Parser> {
...
impl<ExtraValueArgs: ParsableValue, ExtraRunArgs: Parser> clap::FromArgMatches
    for MysSubcommand<ExtraValueArgs, ExtraRunArgs>
{
    fn from_arg_matches(matches: &clap::ArgMatches) -> Result<Self, clap::Error> {
        Ok(match matches.subcommand() {
            Some(("view-object", matches)) => {
                MysSubcommand::ViewObject(ViewObjectCommand::from_arg_matches(matches)?)
            }
            Some(("transfer-object", matches)) => {
                MysSubcommand::TransferObject(TransferObjectCommand::from_arg_matches(matches)?)
            }
            Some(("consensus-commit-prologue", matches)) => MysSubcommand::ConsensusCommitPrologue(
                ConsensusCommitPrologueCommand::from_arg_matches(matches)?,
            ),
            Some(("programmable", matches)) => MysSubcommand::ProgrammableTransaction(
                ProgrammableTransactionCommand::from_arg_matches(matches)?,
            ),
            Some(("upgrade", matches)) => {
                MysSubcommand::UpgradePackage(UpgradePackageCommand::from_arg_matches(matches)?)
            }
            Some(("stage-package", matches)) => {
                MysSubcommand::StagePackage(StagePackageCommand::from_arg_matches(matches)?)
            }
            Some(("set-address", matches)) => {
                MysSubcommand::SetAddress(SetAddressCommand::from_arg_matches(matches)?)
            }
            Some(("create-checkpoint", matches)) => {
                MysSubcommand::CreateCheckpoint(CreateCheckpointCommand::from_arg_matches(matches)?)
            }
            Some(("advance-epoch", matches)) => {
                MysSubcommand::AdvanceEpoch(AdvanceEpochCommand::from_arg_matches(matches)?)
            }
            Some(("advance-clock", matches)) => {
                MysSubcommand::AdvanceClock(AdvanceClockCommand::from_arg_matches(matches)?)
            }
            Some(("set-random-state", matches)) => {
                MysSubcommand::SetRandomState(SetRandomStateCommand::from_arg_matches(matches)?)
            }
            Some(("view-checkpoint", _)) => MysSubcommand::ViewCheckpoint,
            Some(("run-graphql", matches)) => {
                MysSubcommand::RunGraphql(RunGraphqlCommand::from_arg_matches(matches)?)
            }
            Some(("run-jsonrpc", matches)) => {
                MysSubcommand::RunJsonRpc(RunJsonRpcCommand::from_arg_matches(matches)?)
            }
            Some(("bench", matches)) => MysSubcommand::Bench(
    ...
    for MysSubcommand<ExtraValueArgs, ExtraRunArgs>
{
    fn command() -> clap::Command {
        clap::Command::new("mys_sub_command")
...
impl MysExtraValueArgs {
    fn parse_object_value<'a, I: Iterator<Item = (ValueToken, &'a str)>>(
        ...
        Ok(MysExtraValueArgs::Object(fake_id, version))
    }
    ...
    fn parse_receiving_value<'a, I: Iterator<Item = (ValueToken, &'a str)>>(
        ...
        Ok(MysExtraValueArgs::Receiving(fake_id, version))
    }
    ...
    fn parse_read_shared_value<'a, I: Iterator<Item = (ValueToken, &'a str)>>(
        ...
        Ok(MysExtraValueArgs::ImmShared(fake_id, version))
    }
    ...
    fn parse_digest_value<'a, I: Iterator<Item = (ValueToken, &'a str)>>(
        ...
        Ok(MysExtraValueArgs::Digest(package.to_owned()))
    }
    ...
    fn parse_receiving_or_object_value<'a, I: Iterator<Item = (ValueToken, &'a str)>>(
        ...
        let fake_id = if let Some(ValueToken::Comma) = parser.peek_tok() {
            ...
            u256_bytes.reverse();
            let address: MysAddress = MysAddress::from_bytes(&u256_bytes).unwrap();
...
impl MysValue {
    fn assert_move_value(self) -> MoveValue {
        match self {
            MysValue::MoveValue(v) => v,
            MysValue::Object(_, _) => panic!("unexpected nested Mys object in args"),
            MysValue::ObjVec(_) => panic!("unexpected nested Mys object vector in args"),
            MysValue::Digest(_) => panic!("unexpected nested Mys package digest in args"),
            MysValue::Receiving(_, _) => panic!("unexpected nested Mys receiving object in args"),
            MysValue::ImmShared(_, _) => panic!("unexpected nested Mys shared object in args"),
        }
    }

    fn assert_object(self) -> (FakeID, Option<SequenceNumber>) {
        match self {
            MysValue::MoveValue(_) => panic!("unexpected nested non-object value in args"),
            MysValue::Object(id, version) => (id, version),
            MysValue::ObjVec(_) => panic!("unexpected nested Mys object vector in args"),
            MysValue::Digest(_) => panic!("unexpected nested Mys package digest in args"),
            MysValue::Receiving(_, _) => panic!("unexpected nested Mys receiving object in args"),
            MysValue::ImmShared(_, _) => panic!("unexpected nested Mys shared object in args"),
    ...
    fn resolve_object(
        ...
        let obj_res = if let Some(v) = version {
            mys_types::storage::ObjectStore::get_object_by_key(&*test_adapter.executor, &id, v)
        } else {
            mys_types::storage::ObjectStore::get_object(&*test_adapter.executor, &id)
        };
    ...
    pub(crate) fn into_call_arg(self, test_adapter: &MysTestAdapter) -> anyhow::Result<CallArg> {
        Ok(match self {
            MysValue::Object(fake_id, version) => {
                CallArg::Object(Self::object_arg(fake_id, version, test_adapter)?)
            }
            MysValue::MoveValue(v) => CallArg::Pure(v.simple_serialize().unwrap()),
            MysValue::Receiving(fake_id, version) => {
                CallArg::Object(Self::receiving_arg(fake_id, version, test_adapter)?)
            }
            MysValue::ImmShared(fake_id, version) => {
    ...
    pub(crate) fn into_argument(
        ...
        match self {
            MysValue::ObjVec(vec) => builder.make_obj_vec(
                vec.iter()
...
impl ParsableValue for MysExtraValueArgs {
    type ConcreteValue = MysValue;
    ...
    fn move_value_into_concrete(v: MoveValue) -> anyhow::Result<Self::ConcreteValue> {
        Ok(MysValue::MoveValue(v))
    }

    fn concrete_vector(elems: Vec<Self::ConcreteValue>) -> anyhow::Result<Self::ConcreteValue> {
        if !elems.is_empty() && matches!(elems[0], MysValue::Object(_, _)) {
            Ok(MysValue::ObjVec(
                elems.into_iter().map(MysValue::assert_object).collect(),
            ))
        } else {
            Ok(MysValue::MoveValue(MoveValue::Vector(
                elems.into_iter().map(MysValue::assert_move_value).collect(),
            )))
        }
    }

    fn concrete_struct(values: Vec<Self::ConcreteValue>) -> anyhow::Result<Self::ConcreteValue> {
        Ok(MysValue::MoveValue(MoveValue::Struct(MoveStruct(
    ...
    fn into_concrete_value(
        ...
        match self {
            MysExtraValueArgs::Object(id, version) => Ok(MysValue::Object(id, version)),
            MysExtraValueArgs::Digest(pkg) => Ok(MysValue::Digest(pkg)),
            MysExtraValueArgs::Receiving(id, version) => Ok(MysValue::Receiving(id, version)),
            MysExtraValueArgs::ImmShared(id, version) => Ok(MysValue::ImmShared(id, version)),
        }
    }
}
