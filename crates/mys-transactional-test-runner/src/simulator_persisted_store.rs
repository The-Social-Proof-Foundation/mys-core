...
pub struct PersistedStoreInner {
    // Checkpoint data
    checkpoints: DBMap<CheckpointSequenceNumber, mys_types::messages_checkpoint::TrustedCheckpoint>,
    ...
    // Transaction data
    transactions: DBMap<TransactionDigest, mys_types::transaction::TrustedTransaction>,
...
impl SimulatorStore for PersistedStore {
    ...
    fn get_system_state(&self) -> mys_types::mys_system_state::MysSystemState {
        mys_types::mys_system_state::get_mys_system_state(self).expect("system state must exist")
    }

    fn get_clock(&self) -> mys_types::clock::Clock {
        SimulatorStore::get_object(self, &mys_types::MYS_CLOCK_OBJECT_ID)
            .expect("clock should exist")
            .to_rust()
            .expect("clock object should deserialize")
    }

    fn owned_objects(&self, owner: MysAddress) -> Box<dyn Iterator<Item = Object> + '_> {
    ...
    fn backing_store(&self) -> &dyn mys_types::storage::BackingStore {
        self
    }
}
...
impl BackingPackageStore for PersistedStore {
    fn get_package_object(
        &self,
        package_id: &ObjectID,
    ) -> mys_types::error::MysResult<Option<PackageObject>> {
...
impl ParentSync for PersistedStore {
    fn get_latest_parent_entry_ref_deprecated(
        &self,
        _object_id: ObjectID,
    ) -> Option<mys_types::base_types::ObjectRef> {
