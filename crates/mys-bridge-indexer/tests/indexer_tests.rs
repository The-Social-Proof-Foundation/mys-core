...
use mys_bridge_indexer::models::{GovernanceAction, TokenTransfer};
use mys_bridge_indexer::postgres_manager::get_connection_pool;
use mys_bridge_indexer::storage::PgBridgePersistent;
use mys_bridge_indexer::{create_mys_indexer, schema};
...
async fn test_indexing_transfer() {
    ...
    let indexer = create_mys_indexer(pool.clone(), metrics.clone(), ingestion_metrics, &config)
        .await
        .unwrap();
    ...
    // transfer eth to mys
    initiate_bridge_eth_to_mys(&cluster, 1000, 0).await.unwrap();
...
async fn setup_bridge_env(with_eth_env: bool) -> (IndexerConfig, BridgeTestCluster, TempDb) {
    ...
    let config = IndexerConfig {
        remote_store_url: format!("{}/rest", bridge_test_cluster.mys_rpc_url()),
        ...
        concurrency: 10,
        mys_bridge_genesis_checkpoint: 0,
        eth_bridge_genesis_block: 0,
        eth_mys_bridge_contract_address: bridge_test_cluster.mys_bridge_address(),
        metric_port: 9001,
    };
