...
use mys_bridge::types::{EthEvent, RawEthLog};
use mys_indexer_builder::indexer_builder::{DataMapper, DataSender, Datasource};
...
impl EthSubscriptionDatasource {
    pub async fn new(
        eth_mys_bridge_contract_addresses: Vec<EthAddress>,
        eth_client: Arc<EthClient<MeteredEthHttpProvier>>,
        eth_ws_url: String,
        metrics: Box<dyn IndexerMetricProvider>,
        genesis_block: u64,
        ...
        Ok(Self {
            addresses: eth_mys_bridge_contract_addresses,
            eth_client,
            eth_ws_url,
            metrics,
            genesis_block,
...
impl EthFinalizedSyncDatasource {
    pub async fn new(
        ...
        Ok(Self {
            bridge_addresses: eth_mys_bridge_contract_addresses,
            eth_http_url,
            eth_client,
            metrics,
            bridge_metrics,
...
impl DataMapper<RawEthData, ProcessedTxnData> for EthDataMapper {
    fn map(
        ...
        match bridge_event {
            EthBridgeEvent::EthMysBridgeEvents(bridge_event) => match &bridge_event {
                EthMysBridgeEvents::TokensDepositedFilter(bridge_event) => {
                    info!(
                    ...
                    processed_txn_data.push(ProcessedTxnData::TokenTransfer(TokenTransfer {
                        ...
                        data: Some(TokenTransferData {
                            ...
                            amount: bridge_event.mys_adjusted_amount,
                            is_finalized,
                        }),
                    }));
                }
                EthMysBridgeEvents::TokensClaimedFilter(bridge_event) => {
                    info!(
                ...
                EthMysBridgeEvents::EmergencyOperationFilter(f) => {
                    info!(
                ...
                EthMysBridgeEvents::ContractUpgradedFilter(f) => {
                    info!(
                        "Observed Eth MysBridge Upgrade at block: {}, tx_hash: {}",
                        log.block_number(),
                        log.tx_hash
                    );
                ...
                | EthMysBridgeEvents::PausedFilter(_)
                | EthMysBridgeEvents::UnpausedFilter(_)
            ...
            EthBridgeEvent::EthBridgeConfigEvents(bridge_event) => match &bridge_event {
                ...
                EthBridgeConfigEvents::TokenAddedFilter(_) => {
                    info!(
                        "Observed Eth AddMysTokens at block: {}, tx_hash: {}",
                        log.block_number(),
                        log.tx_hash
                    );
