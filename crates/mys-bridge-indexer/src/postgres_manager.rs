...
use crate::schema::mys_progress_store::txn_digest;
use crate::schema::{mys_error_transactions, token_transfer_data};
...
const MYS_PROGRESS_STORE_DUMMY_KEY: i32 = 1;
...
pub async fn update_mys_progress_store(
    pool: &PgPool,
    tx_digest: TransactionDigest,
) -> Result<(), anyhow::Error> {
    let mut conn = pool.get().await?;
    diesel::insert_into(schema::mys_progress_store::table)
        .values(&MysProgressStore {
            id: MYS_PROGRESS_STORE_DUMMY_KEY,
            txn_digest: tx_digest.inner().to_vec(),
        })
        .on_conflict(schema::mys_progress_store::dsl::id)
        .do_update()
...
pub async fn read_mys_progress_store(pool: &PgPool) -> anyhow::Result<Option<TransactionDigest>> {
    let mut conn = pool.get().await?;
    let val: Option<MysProgressStore> = crate::schema::mys_progress_store::dsl::mys_progress_store
        .select(MysProgressStore::as_select())
        .first(&mut conn)
        .await
        .optional()?;
    match val {
