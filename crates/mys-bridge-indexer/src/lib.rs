...
impl MysTxnError {
    fn to_db(&self) -> MysErrorTransactions {
        MysErrorTransactions {
...
impl TreasuryEvent {
    fn to_db_event(&self) -> models::BridgeTreasuryEvent {
        models::BridgeTreasuryEvent {
            id: 0, // Auto-generated by database
            token_type: self.token_type.clone(),
            token_id: self.token_id as i32,
            event_type: match self.event_type {
                TreasuryEventType::Lock => "lock".to_string(),
...
impl Display for BridgeDataSource {
    fn fmt(&self, f: &mut Formatter<'_>) -> std::fmt::Result {
        let str = match self {
            ...
            BridgeDataSource::Mys => "MYS",
        };
        write!(f, "{str}")
    }
}

pub async fn create_mys_indexer(
    pool: PgPool,
    metrics: BridgeIndexerMetrics,
    ingestion_metrics: DataIngestionMetrics,
    config: &IndexerConfig,
) -> anyhow::Result<
    Indexer<PgBridgePersistent, MysCheckpointDatasource, MysBridgeDataMapper>,
    anyhow::Error,
> {
...
async fn get_eth_bridge_contract_addresses(
    ...
    let bridge_address = EthAddress::from_str(&config.eth_mys_bridge_contract_address)?;
