...
use crate::postgres_manager::{update_mys_progress_store, write, PgPool};
...
pub async fn handle_mys_transactions_loop(
    pg_pool: PgPool,
    ...
    while let Some(batch) = stream.next().await {
        ...
        if !data.is_empty() {
            ...
            while let Err(err) = write(&pg_pool, data.clone()).await {
                error!("Failed to write mys transactions to DB: {:?}", err);
                tokio::time::sleep(Duration::from_secs(5)).await;
            }
            info!("Wrote {} bridge transaction data to DB", data.len());
            metrics.last_committed_mys_checkpoint.set(last_ckp as i64);
        }

        // update mys progress store using the latest cursor
        if let Some(cursor) = cursor {
            while let Err(err) = update_mys_progress_store(&pg_pool, cursor).await {
                error!("Failed to update mys progress tore DB: {:?}", err);
                tokio::time::sleep(Duration::from_secs(5)).await;
            }
            info!("Updated mys transaction cursor to {}", cursor);
        }
    }
...
pub fn into_token_transfers(
    ...
    for ev in tx.events.data {
        ...
        match ev.type_.name.as_str() {
            "TokenDepositedEvent" => {
                ...
                metrics.total_mys_token_deposited.inc();
            ...
            "TokenTransferApproved" => {
                ...
                metrics.total_mys_token_transfer_approved.inc();
            ...
            "TokenTransferClaimed" => {
                ...
                metrics.total_mys_token_transfer_claimed.inc();
            ...
            _ => {
                metrics.total_mys_bridge_txn_other.inc();
            }
        }
    }
    if !transfers.is_empty() {
        info!(
            ?tx_digest,
            "MYS: Extracted {} bridge token transfer data entries",
            transfers.len(),
        );
    }
    Ok(transfers)
