...
impl ServerBuilder {
    ...
    pub async fn from_config(
        ...
        let mys_sdk_client = if let Some(url) = &config.tx_exec_full_node.node_rpc_url {
            Some(
                MysClientBuilder::default()
                    ...
                    .build(url)
                    .await
                    .map_err(|e| Error::Internal(format!("Failed to create MysClient: {}", e)))?,
            )
        } else {
...
async fn graphql_handler(
    ...
    schema: Extension<MysGraphQLSchema>,
...
pub mod tests {
    ...
    async fn test_timeout() {
        ...
        cluster
            ...
            .await;
        // timeout test includes mutation timeout, which requires a [MysClient] to be able to run
        ...
        async fn test_timeout(
            ...
            let schema = prep_schema(db_url, Some(cfg))
                .await
                .context_data(Some(mys_client.clone()))
                .extension(Timeout)
        ...
        let mys_client = wallet.get_client().await.unwrap();

        test_timeout(delay, timeout, query, &mys_client, db_url.clone())
            .await
            .into_result()
            .expect("Should complete successfully");

        // Should timeout
        let errs: Vec<_> = test_timeout(delay, delay, query, &mys_client, db_url.clone())
            .await
            .into_result()
            .unwrap_err()
            .into_iter()
