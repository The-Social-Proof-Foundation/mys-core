...
fn select_ev(sender: Option<MysAddress>, from: &str) -> RawQuery {
...
pub(crate) fn select_sender(sender: MysAddress) -> RawQuery {
    select_ev(Some(sender), "event_senders")
}

pub(crate) fn select_event_type(event_type: &TypeFilter, sender: Option<MysAddress>) -> RawQuery {
    match event_type {
