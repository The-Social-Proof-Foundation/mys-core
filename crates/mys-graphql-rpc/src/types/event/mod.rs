...
use mys_indexer::models::{events::StoredEvent, transactions::StoredTransaction};
use mys_indexer::schema::{checkpoints, events};
use mys_types::base_types::ObjectID;
use mys_types::Identifier;
use mys_types::{
    base_types::MysAddress as NativeMysAddress, event::Event as NativeEvent, parse_mys_struct_tag,
};
...
/// A Mys node emits one of the following events:
/// Move event
/// Publish event
...
impl Event {
    ...
    async fn sender(&self) -> Result<Option<Address>> {
        if self.native.sender == NativeMysAddress::ZERO {
            return Ok(None);
        }
...
impl Event {
    ...
    fn try_from_stored_event(
        ...
        let sender = NativeMysAddress::from_bytes(sender_bytes)
            .map_err(|e| Error::Internal(e.to_string()))?;
        let package_id =
            ObjectID::from_bytes(&stored.package).map_err(|e| Error::Internal(e.to_string()))?;
        let type_ =
            parse_mys_struct_tag(&stored.event_type).map_err(|e| Error::Internal(e.to_string()))?;
