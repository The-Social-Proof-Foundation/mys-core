...
pub(crate) struct TransactionBlockFilter {
    ...
    pub affected_address: Option<MysAddress>,
    ...
    #[cfg(feature = "staging")]
    pub affected_object: Option<MysAddress>,

    /// Limit to transactions that were sent by the given address.
    pub sent_address: Option<MysAddress>,
    ...
    pub input_object: Option<MysAddress>,
    ...
    /// This filter will be removed with 1.36.0 (2024-10-14), or at least one release after
    /// `affectedObject` is introduced, whichever is later.
    pub changed_object: Option<MysAddress>,
...
impl TransactionBlockFilter {
    ...
    pub(crate) fn explicit_sender(&self) -> Option<MysAddress> {
            ...
            || matches!(
                ...
                (Some(kind), Some(signer))
                    if (kind == TransactionBlockKindInput::SystemTx)
                        != (signer == MysAddress::from(NativeMysAddress::ZERO))
            )
    }
}
