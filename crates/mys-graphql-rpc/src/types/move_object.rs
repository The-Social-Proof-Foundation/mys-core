...
#[graphql(
    ...
    field(
        ...
        desc = "Determines whether a transaction can transfer this object, using the \
                TransferObjects transaction command or `mys::transfer::public_transfer`, both of \
...
impl MoveObject {
    ...
    async fn as_staked_mys(&self) -> Result<Option<StakedMys>> {
        match StakedMys::try_from(self) {
            Ok(coin) => Ok(Some(coin)),
            Err(StakedMysDowncastError::NotAStakedMys) => Ok(None),
            Err(StakedMysDowncastError::Bcs(e)) => Err(Error::Internal(format!(
                "Failed to deserialize StakedMys: {e}"
            )))
            .extend(),
        }
    }
