...
use async_graphql::*;
use mys_indexer::apis::governance_api::exchange_rates;
use mys_types::mys_system_state::mys_system_state_summary::MysValidatorSummary as NativeMysValidatorSummary;
#[derive(Clone, Debug)]
pub(crate) struct Validator {
    pub validator_summary: NativeMysValidatorSummary,
...
impl Loader<u64> for Db {
    type Value = (
        ...
        BTreeMap<NativeMysAddress, Vec<(EpochId, PoolTokenExchangeRate)>>,
    );
    type Error = Error;
...
impl Validator {
    ...
    async fn address(&self) -> Address {
        Address {
            address: MysAddress::from(self.validator_summary.mys_address),
    ...
    async fn staking_pool_id(&self) -> MysAddress {
    ...
    async fn staking_pool_mys_balance(&self) -> Option<BigInt> {
        Some(BigInt::from(
            self.validator_summary.staking_pool_mys_balance,
        ))
    }
    ...
    async fn pending_total_mys_withdraw(&self) -> Option<BigInt> {
        Some(BigInt::from(
            self.validator_summary.pending_total_mys_withdraw,
        ))
    }
...
impl Validator {
    pub fn operation_cap_id(&self) -> MysAddress {
        MysAddress::from_array(**self.validator_summary.operation_cap_id)
    }
}
