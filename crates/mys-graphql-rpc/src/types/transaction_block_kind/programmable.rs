...
use mys_json_rpc_types::MysArgument;
...
impl MoveCallTransaction {
    ...
    async fn package(&self) -> MysAddress {
    ...
    fn from(pt: NativeProgrammableTransaction, checkpoint_viewed_at: u64) -> Self {
        ...
        match pt {
            ...
            N::Publish(modules, dependencies) => P::Publish(PublishTransaction {
                ...
                dependencies: dependencies.into_iter().map(MysAddress::from).collect(),
            }),
...
impl From<MysArgument> for TransactionArgument {
    fn from(argument: MysArgument) -> Self {
        use MysArgument as S;
        use TransactionArgument as A;
        match argument {
