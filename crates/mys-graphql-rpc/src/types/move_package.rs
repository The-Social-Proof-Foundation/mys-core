...
use crate::types::mys_address::addr;
...
impl MovePackage {
    ...
    pub(crate) fn try_from_serialized(
        ...
        let object = Object::new_serialized(
            MysAddress::from_bytes(&history_object.object_id)
...
impl Loader<LatestKey> for Db {
    ...
    fn load(
        &self,
        keys: &[LatestKey],
    ) -> impl Future<Output = Result<HashMap<LatestKey, MysAddress>, Error>> + Send {
        let self_clone = self.clone();
        let keys_vec = keys.to_vec();
        async move {
            use packages::dsl;
