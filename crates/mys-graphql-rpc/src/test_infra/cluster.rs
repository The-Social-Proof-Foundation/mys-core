...
use mys_indexer::test_utils::start_indexer_writer_for_testing;
...
pub async fn start_network_cluster() -> NetworkCluster {
    ...
    let graphql_connection_config = ConnectionConfig {
        ...
        social_indexer_url: "http://localhost:8080".to_owned(),
    };
...
/// Takes in a simulated instantiation of a Mys blockchain and builds a cluster around it. This
...
pub async fn serve_executor(
    ...
    let executor_server_handle = tokio::spawn(async move {
        mys_rpc_api::RpcService::new_without_version(executor)
