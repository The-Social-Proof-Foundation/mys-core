...
use mys_test_transaction_builder::{increment_counter, publish_basics_package_and_make_counter};
...
impl TestCaseImpl for SharedCounterTest {
    ...
    async fn run(&self, ctx: &mut TestContext) -> Result<(), anyhow::Error> {
        ...
        let mys_objs = ctx.get_mys_from_faucet(Some(1)).await;
        assert!(!mys_objs.is_empty());
