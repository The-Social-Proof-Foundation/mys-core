...
impl FullNodeExecuteTransactionTest {
    async fn verify_transaction(fullnode: &MysClient, tx_digest: TransactionDigest) {
        fullnode
            .read_api()
            .get_transaction_with_options(tx_digest, MysTransactionBlockResponseOptions::new())
            .await
            .unwrap_or_else(|e| {
                panic!(
...
impl TestCaseImpl for FullNodeExecuteTransactionTest {
    ...
    async fn run(&self, ctx: &mut TestContext) -> Result<(), anyhow::Error> {
        let txn_count = 4;
        ctx.get_mys_from_faucet(Some(1)).await;
        ...
        if !matches!(effects.status(), MysExecutionStatus::Success { .. }) {
            panic!(
