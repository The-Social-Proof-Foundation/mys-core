...
impl TestCaseImpl for CoinIndexTest {
    ...
    async fn run(&self, ctx: &mut TestContext) -> Result<(), anyhow::Error> {
        ...
        ctx.get_mys_from_faucet(None).await;
        ...
        assert_eq!(
            ...
            "total_balance: {}, old_total_balance: {}, mys_balance_change.amount: {}",
            total_balance,
            old_total_balance,
        ...
        let args = vec![
            MysJsonValue::from_object_id(cap.0),
            MysJsonValue::new(json!("10000"))?,
            MysJsonValue::new(json!(account))?,
        ];
        let txn = client
            .transaction_builder()
            .move_call(
        ...
        let mys_balance_change = balance_changes
            .iter()
            .find(|b| b.coin_type.to_string().contains("MYS"))
            .unwrap();
        ...
        assert_eq!(mys_balance_change.owner, Owner::AddressOwner(account));
        ...
        assert_eq!(
            total_balance,
            (old_total_balance as i128 + mys_balance_change.amount) as u128,
            "total_balance: {}, old_total_balance: {}, mys_balance_change.amount: {}",
            total_balance,
            old_total_balance,
            mys_balance_change.amount
        );
        old_coin_object_count = coin_object_count;
        ...
        let args = vec![MysJsonValue::from_object_id(envelope.0)];
        let txn = client
            .transaction_builder()
            .move_call(
                account,
        ...
        let txn = client
            ...
            .move_call(
                ...
                vec![
                    MysJsonValue::from_object_id(cap.0),
                    MysJsonValue::from_object_id(envelope.0),
                ],
                None,
                rgp * 2_000_000,
                None,
        ...
        let txn = client
            ...
            .move_call(
                ...
                vec![
                    MysJsonValue::from_object_id(cap.0),
                    MysJsonValue::from_object_id(managed_coin_id_10k),
                ],
                None,
                rgp * 2_000_000,
                None,
        ...
        let mys_coins = client
            .coin_read_api()
            .get_coins(account, Some(mys_type_str.into()), None, None)
            .await
            .unwrap()
            .data;
        ...
        assert_eq!(
            // this is only MYS coins at the moment
            mys_coins,
            client
                .coin_read_api()
        ...
        let mys_balance = client
            .coin_read_api()
            .get_balance(account, None)
            .await
            .unwrap();
        assert_eq!(
            mys_balance.total_balance,
            mys_coins.iter().map(|c| c.balance as u128).sum::<u128>()
        );
        ...
        let txn = client
            ...
            .move_call(
                ...
                vec![
                    MysJsonValue::from_object_id(cap.0),
                    MysJsonValue::new(json!("5"))?,  // balance = 5
                    MysJsonValue::new(json!("40"))?, // num = 40
                    MysJsonValue::new(json!(account))?,
                ],
                None,
                rgp * 2_000_000,
        ...
        assert_eq!(
            ...
            client
                .coin_read_api()
                .get_coins(account, None, None, Some(mys_coins.len() + 1))
                .await
                .unwrap()
                .data,
        );
        ...
        assert_eq!(mys_coins.len() + managed_coins.len(), total_coins,);

        let mys_coins_with_managed_coin_1 = client
            .coin_read_api()
            .get_all_coins(account, None, Some(mys_coins.len() + 1))
            .await
            .unwrap();
        assert_eq!(
            mys_coins_with_managed_coin_1.data.len(),
            mys_coins.len() + 1
        );
        assert!(mys_coins_with_managed_coin_1.has_next_page);
        let cursor = mys_coins_with_managed_coin_1.next_cursor;
...
async fn add_to_envelope(
    ...
    let txn = client
        ...
        .move_call(
            ...
            vec![],
            vec![
                MysJsonValue::from_object_id(envelope),
                MysJsonValue::from_object_id(coin),
            ],
            None,
            rgp * 2_000_000,
            None,
