...
impl Cluster for LocalNewCluster {
    async fn start(options: &ClusterTestOpt) -> Result<Self, anyhow::Error> {
        ...
        if let Some(config_dir) = options.config_dir.clone() {
            ...
            let network_config_path = config_dir.join(MYS_NETWORK_CONFIG);
        ...
        let faucet_address = MysAddress::from(faucet_key.public());
...
pub fn new_wallet_context_from_cluster(
    ...
    let address: MysAddress = key_pair.public().into();
    keystore
        .add_key(None, MysKeyPair::Ed25519(key_pair))
        .unwrap();
    MysClientConfig {
        keystore,
        envs: vec![MysEnv {
