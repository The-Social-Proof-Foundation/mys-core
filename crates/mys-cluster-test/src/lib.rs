...
impl TestContext {
    async fn get_mys_from_faucet(&self, minimum_coins: Option<usize>) -> Vec<GasCoin> {
        let addr = self.get_wallet_address();
        let faucet_response = self.faucet.request_mys_coins(addr).await;
        ...
        if gas_coins.len() < minimum_coins {
            panic!(
                "Expect to get at least {minimum_coins} Mys Coins for address {addr}, but only got {}",
                gas_coins.len()
            )
        }
    ...
    fn get_fullnode_client(&self) -> &MysClient {
        self.client.get_fullnode_client()
    }

    fn clone_fullnode_client(&self) -> MysClient {
    ...
    async fn get_latest_mys_system_state(&self) -> MysSystemStateSummary {
        self.client
            .get_fullnode_client()
            .governance_api()
    ...
    fn get_wallet_address(&self) -> MysAddress {
    ...
    async fn get_tx_with_retry_times(
        ...
        match self
            ...
            .get_fullnode_client()
            .read_api()
            .get_transaction_with_options(digest, MysTransactionBlockResponseOptions::new())
            .await
        {
