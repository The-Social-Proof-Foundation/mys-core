...
async fn main() -> Result<()> {
    ...
    // Load environment variables from ~/.mysop/env_vars
    ...
    if let Ok(env_contents) = std::fs::read_to_string(env_file_path) {
        ...
        warn!("Warning: Could not read ~/.mysop/env_vars file. Environment variables not loaded.");
    }
