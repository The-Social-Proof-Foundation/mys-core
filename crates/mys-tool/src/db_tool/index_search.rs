...
use std::fmt::Debug;
use mys_core::jsonrpc_index::IndexStoreTables;
use mys_types::{
    base_types::{ObjectID, MysAddress, TxSequenceNumber},
    Identifier, TypeTag,
};
...
fn from_addr_seq(s: &str) -> Result<(MysAddress, TxSequenceNumber), anyhow::Error> {
    // Remove whitespaces
    let s = s.trim();
    ...
    let address = MysAddress::from_str(tokens[0].trim())?;
...
fn from_addr_oid(s: &str) -> Result<(MysAddress, ObjectID), anyhow::Error> {
    // Remove whitespaces
    let s = s.trim();
    let tokens = s.split(',').collect::<Vec<&str>>();
    if tokens.len() != 2 {
    ...
    let addr = MysAddress::from_str(tokens[0].trim())?;
    let oid = ObjectID::from_str(tokens[1].trim())?;

    Ok((addr, oid))
}

fn from_addr_str_oid(s: &str) -> Result<(MysAddress, String, ObjectID), anyhow::Error> {
    // Remove whitespaces
    let s = s.trim();
    let tokens = s.split(',').collect::<Vec<&str>>();
    if tokens.len() != 3 {
...
fn from_address_and_event_id(
    s: &str,
) -> Result<(MysAddress, (TxSequenceNumber, usize)), anyhow::Error> {
    // Example: "0x1 1234 5"
    let tokens = s.split(' ').collect::<Vec<&str>>();
    if tokens.len() != 3 {
        return Err(anyhow!("Invalid input"));
