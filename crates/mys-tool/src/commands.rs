...
pub enum ToolCommand {
    ...
    /// contains Move bytecode (ttttable for passing into a disassembler).
    ...
    Replay {
        ...
        #[arg(
            ...
            If not specified we will use the default network config file at ~/.mys-replay/network-config.yaml"
        )]
...
async fn check_locked_object(
    ...
    let clients = Arc::new(make_clients(mys_client).await?);
    ...
    pub async fn execute(self, tracing_handle: TracingHandle) -> Result<(), anyhow::Error> {
        match self {
            ToolCommand::LockedObject {
                ...
                let mys_client =
                    Arc::new(MysClientBuilder::default().build(fullnode_rpc_url).await?);
            ...
            ToolCommand::DumpPackages {
                ...
                mys_package_dump::dump(rpc_url, output_dir, before_checkpoint).await?;
            }
                ...
                if !concise {
                    ...
                    for (i, val_info) in genesis.validator_set_for_tooling().iter().enumerate() {
                        ...
                        println!(
                            ...
                            i,
                            metadata.name,
                            metadata.mys_pubkey_bytes().concise(),
            ...
            ToolCommand::RestoreFromDBCheckpoint {
                ...
                let config = mys_config::NodeConfig::load(config_path)?;
