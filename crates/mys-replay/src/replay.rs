...
use mys_json_rpc_types::{
    MysExecutionStatus, MysTransactionBlockEffects, MysTransactionBlockEffectsAPI,
};
...
use mys_types::storage::{get_module, PackageObject};
use mys_types::transaction::TransactionKind::ProgrammableTransaction;
use mys_types::MYS_DENY_LIST_OBJECT_ID;
use mys_types::{
...
pub struct ExecutionSandboxState {
    ...
    pub local_exec_effects: MysTransactionBlockEffects,
...
impl LocalExec {
    ...
    pub async fn multi_download(
        ...
        while num_retries_for_timeout >= 0 {
            match self.fetcher.multi_get_versioned(objs).await {
                ...
                Err(ReplayEngineError::MysRpcRequestTimeout) => {
                    warn!(
    ...
    pub fn download_latest_object(
        ...
        match resp {
            ...
            Err(err) => Err(ReplayEngineError::MysRpcError {
                err: err.to_string(),
            }),
        }
    }
    ...
    pub async fn get_checkpoint_txs(
        ...
        self.fetcher
            ...
            .await
            .map_err(|e| ReplayEngineError::MysRpcError { err: e.to_string() })
    }
    ...
    pub async fn execution_engine_execute_with_tx_info_impl(
        ...
        let effects =
            MysTransactionBlockEffects::try_from(effects).map_err(ReplayEngineError::from)?;
    ...
    pub async fn certificate_execute_with_sandbox_state(
        ...
        let executor = mys_execution::executor(&protocol_config, true, None).unwrap();
    ...
    /// Must be called after `init_for_execution`
    /// This executes from `mys_core::authority::AuthorityState::try_execute_immediately`
    ...
    /// Must be called after `init_for_execution`
    /// This executes from `mys_adapter::execution_engine::execute_transaction_to_effects`
        ...
        match parse_effect_error_for_denied_coins(status) {
            Some(coin_type) => {
                ...
                if !config_id_and_version
                    .iter()
                    .any(|(id, _)| id == &MYS_DENY_LIST_OBJECT_ID)
                {
                    let deny_list_oid_version = self.download_latest_object(&MYS_DENY_LIST_OBJECT_ID)
                        .ok()
                        .flatten()
                        .expect("Unable to download the deny list object for a transaction that requires it")
                        .version();
                    config_id_and_version.push((MYS_DENY_LIST_OBJECT_ID, deny_list_oid_version));
                }
    ...
    async fn resolve_tx_components(
        ...
        let sender = match tx_info.clone().transaction.unwrap().data {
            mys_json_rpc_types::MysTransactionBlockData::V1(tx) => tx.sender,
        };
        let MysTransactionBlockEffects::V1(effects) = tx_info.clone().effects.unwrap();
        ...
        let gas_data = match tx_info.clone().transaction.unwrap().data {
            mys_json_rpc_types::MysTransactionBlockData::V1(tx) => tx.gas_data,
        };
        ...
        Ok(OnChainTransactionInfo {
            ...
            effects: MysTransactionBlockEffects::V1(effects),
            receiving_objs,
            config_objects,
    ...
    async fn resolve_tx_components_from_dump(
        ...
        let effects = MysTransactionBlockEffects::try_from(effects).unwrap();
    ...
    fn read_child_object(
        ...
        fn inner(
            ...
            if child_object.version() > child_version_upper_bound {
                return Err(MysError::Unknown(format!(
            ...
            if child_object.owner != Owner::ObjectOwner(parent.into()) {
                return Err(MysError::InvalidChildObjectAccess {
                    object: *child,
                    given_parent: parent,
...
impl ModuleResolver for LocalExec {
    ...
    /// We do not download
    fn get_module(&self, module_id: &ModuleId) -> MysResult<Option<Vec<u8>>> {
        fn inner(self_: &LocalExec, module_id: &ModuleId) -> MysResult<Option<Vec<u8>>> {
...
impl GetModule for LocalExec {
    ...
    fn get_module_by_id(&self, id: &ModuleId) -> MysResult<Option<Self::Item>> {
...
pub fn get_executor(
    ...
    let silent = true;
    mys_execution::executor(&protocol_config, silent, enable_profiler)
        .expect("Creating an executor should not fail here")
}

fn parse_effect_error_for_denied_coins(status: &MysExecutionStatus) -> Option<String> {
    let MysExecutionStatus::Failure { error } = status else {
        return None;
    };
