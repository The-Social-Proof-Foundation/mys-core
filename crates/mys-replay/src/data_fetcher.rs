...
pub(crate) trait DataFetcher {
    ...
    ) -> Result<MysTransactionBlockResponse, ReplayEngineError>;
    ...
    async fn get_epoch_change_events(
        &self,
        reverse: bool,
    ) -> Result<Vec<MysEvent>, ReplayEngineError>;
...
impl RemoteFetcher {
    pub fn new(rpc_client: MysClient) -> Self {
        Self {
            rpc_client,
...
impl DataFetcher for RemoteFetcher {
    ...
    async fn multi_get_versioned(
        ...
        let options = MysObjectDataOptions::bcs_lossless();

        let objs: Vec<_> = to_fetch
            .iter()
            .map(|(object_id, version)| MysGetPastObjectRequest {
                object_id: *object_id,
                version: *version,
            })
            .collect();
    ...
    async fn get_child_object(
        ...
        let response = self
            ...
            .await
            .map_err(|q| ReplayEngineError::MysRpcError { err: q.to_string() })?;
    ...
    async fn get_transaction(
        ...
        let tx_fetch_opts = MysTransactionBlockResponseOptions::full_content();
    ...
    async fn get_epoch_change_events(
        ...
        let mut epoch_change_events: Vec<MysEvent> = vec![];
...
fn convert_past_obj_response(resp: MysPastObjectResponse) -> Result<Object, ReplayEngineError> {
    match resp {
        MysPastObjectResponse::VersionFound(o) => obj_from_mys_obj_data(&o),
        MysPastObjectResponse::ObjectDeleted(r) => Err(ReplayEngineError::ObjectDeleted {
            id: r.object_id,
            version: r.version,
            digest: r.digest,
        }),
        MysPastObjectResponse::ObjectNotExists(id) => Err(ReplayEngineError::ObjectNotExist { id }),
        MysPastObjectResponse::VersionNotFound(id, version) => {
            Err(ReplayEngineError::ObjectVersionNotFound { id, version })
        }
        MysPastObjectResponse::VersionTooHigh {
            object_id,
            asked_version,
            latest_version,
...
fn obj_from_mys_obj_response(o: &MysObjectResponse) -> Result<Object, ReplayEngineError> {
    let o = o.object().map_err(ReplayEngineError::from)?.clone();
    obj_from_mys_obj_data(&o)
}

fn obj_from_mys_obj_data(o: &MysObjectData) -> Result<Object, ReplayEngineError> {
...
pub fn extract_epoch_and_version(ev: MysEvent) -> Result<(u64, u64), ReplayEngineError> {
