...
const DEFAULT_TLD: &str = "mys";
const ACCEPTED_SEPARATORS: [char; 2] = ['.', '*'];
const MYS_NEW_FORMAT_SEPARATOR: char = '@';
...
pub struct Registry {
    ...
    reverse_registry: Table<MysAddress, Domain>,
}
...
pub struct NameRecord {
    ...
    pub target_address: Option<MysAddress>,
    /// Additional data which may be stored in a record
    pub data: VecMap<String, String>,
}

/// A MysnsRegistration object to manage an SLD
#[derive(Debug, Serialize, Deserialize, Clone, Eq, PartialEq)]
pub struct MysnsRegistration {
    pub id: UID,
    pub domain: Domain,
...
pub struct SubDomainRegistration {
    pub id: UID,
    pub nft: MysnsRegistration,
}

/// Two different view options for a domain.
/// `At` -> `test@example` | `Dot` -> `test.example.mys`
...
/// Rust version of the Move mys::table::Table type.
#[derive(Debug, Serialize, Deserialize, Clone, Eq, PartialEq)]
...
impl Domain {
    pub fn type_(package_address: MysAddress) -> StructTag {
        StructTag {
    ...
    /// E.g. `test.example.mys` -> `example.mys`
    ///
    ...
    pub fn format(&self, format: DomainFormat) -> String {
        ...
        format!("{}{}{}", labels.join(sep), MYS_NEW_FORMAT_SEPARATOR, sld)
    }
}
...
impl NameServiceConfig {
    ...
    pub fn record_field_id(&self, domain: &Domain) -> ObjectID {
        ...
        mys_types::dynamic_field::derive_dynamic_field_id(
            self.registry_id,
    ...
    pub fn mainnet() -> Self {
        ...
        let package_address = MysAddress::from_str(MAINNET_NS_PACKAGE_ADDRESS).unwrap();
    ...
    pub fn testnet() -> Self {
        ...
        let package_address = MysAddress::from_str(TESTNET_NS_PACKAGE_ADDRESS).unwrap();
...
/// E.g.  `example.mys` -> `.` | example*mys -> `@` | `example*mys` -> `*`
...
///
/// E.g. `@example` -> `example.mys` | `test@example` -> `test.example.mys`
fn convert_from_new_format(s: &str, separator: &char) -> Result<String, NameServiceError> {
    let mut splits = s.split(MYS_NEW_FORMAT_SEPARATOR);
...
mod tests {
    ...
    fn test_name_service_outputs() {
        ...
        assert_eq!("@mys".parse::<Domain>().unwrap().to_string(), "mys.myso");
    ...
    fn test_different_wildcard() {
        assert_eq!("test.myso".parse::<Domain>(), "test*mys".parse::<Domain>(),);
