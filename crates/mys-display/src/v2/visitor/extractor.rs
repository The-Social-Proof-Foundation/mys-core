...
use mys_types::base_types::RESOLVED_STD_OPTION;
use mys_types::object::option_visitor::OptionVisitor;
...
impl<'v> AV::Visitor<'v, 'v> for Extractor<'v, '_> {
    ...
    /// Mys does not produce signer values, so we can never extract them.
    fn visit_signer(
        &mut self,
        _: &AV::ValueDriver<'_, 'v, 'v>,
        _: AccountAddress,
