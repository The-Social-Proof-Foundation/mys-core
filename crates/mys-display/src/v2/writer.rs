...
impl RV::Writer for JsonWriter<'_> {
    ...
    fn nest(&mut self) -> Result<<Self as RV::Writer>::Nested<'_>, <Self as RV::Writer>::Error> {
    ...
    fn write_null(&mut self) -> Result<<Self as RV::Writer>::Value, <Self as RV::Writer>::Error> {
        self.debit("null".len())?;
        Ok(serde_json::Value::Null)
    }

    fn write_bool(&mut self, value: bool) -> Result<<Self as RV::Writer>::Value, <Self as RV::Writer>::Error> {
        self.debit(if value { "true".len() } else { "false".len() })?;
        Ok(serde_json::Value::Bool(value))
    }

    fn write_number(&mut self, value: u32) -> Result<<Self as RV::Writer>::Value, <Self as RV::Writer>::Error> {
        self.debit(if value == 0 { 1 } else { value.ilog10() } as usize)?;
        Ok(serde_json::Value::Number(value.into()))
    }

    fn write_str(&mut self, value: String) -> Result<<Self as RV::Writer>::Value, <Self as RV::Writer>::Error> {
        // Account for the quotes around the string.
        self.debit(2 + value.len())?;
        Ok(serde_json::Value::String(value))
    }

    fn write_vec(&mut self, value: <Self as RV::Writer>::Vec) -> Result<<Self as RV::Writer>::Value, <Self as RV::Writer>::Error> {
        // Account for the opening bracket.
        self.debit(1)?;
        Ok(serde_json::Value::Array(value))
    }

    fn write_map(&mut self, value: <Self as RV::Writer>::Map) -> Result<<Self as RV::Writer>::Value, <Self as RV::Writer>::Error> {
    ...
    fn vec_push_element(
        &mut self,
        vec: &mut <Self as RV::Writer>::Vec,
        val: <Self as RV::Writer>::Value,
    ) -> Result<(), <Self as RV::Writer>::Error> {
