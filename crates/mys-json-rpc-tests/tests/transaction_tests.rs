...
use mys_json_rpc_types::{
    MysObjectDataOptions, MysObjectResponseQuery, MysTransactionBlockResponse,
    MysTransactionBlockResponseOptions, TransactionBlockBytes,
};
...
use mys_json_rpc_api::{IndexerApiClient, TransactionBuilderClient, WriteApiClient};
...
async fn test_get_transaction_block() -> Result<(), anyhow::Error> {
    ...
    let mut tx_responses: Vec<MysTransactionBlockResponse> = Vec::new();
    ...
    // let batch_responses: Vec<MysTransactionBlockResponse> = http_client
    //     .multi_get_transaction_blocks(tx, Some(MysTransactionBlockResponseOptions::new()))
    //     .await?;
    ...
    //     assert!(tx_responses
    //         .iter()
    //         .any(|resp| matches!(resp, MysTransactionBlockResponse {digest, ..} if *digest == r.digest)))
    // }

    // // test get_transaction
    // for tx_digest in tx {
    //     let response: MysTransactionBlockResponse = http_client
    //         .get_transaction_block(
    //             tx_digest,
    //             Some(MysTransactionBlockResponseOptions::new().with_raw_input()),
    //         )
    //         .await?;
    //     assert!(tx_responses.iter().any(
    //         |resp| matches!(resp, MysTransactionBlockResponse {digest, ..} if *digest == response.digest)
    //     ));
...
async fn test_get_fullnode_transaction() -> Result<(), anyhow::Error> {
    ...
    let query = MysTransactionBlockResponseQuery {
        options: Some(MysTransactionBlockResponseOptions {
            show_input: true,
            show_effects: true,
            show_events: true,
            ..Default::default()
    ...
    let first_page = client
        ...
        .query_transaction_blocks(
            MysTransactionBlockResponseQuery::default(),
            None,
            Some(5),
            false,
        )
    ...
    let address_txs_asc = client
        .read_api()
        .query_transaction_blocks(
            MysTransactionBlockResponseQuery::new_with_filter(TransactionFilter::FromAddress(
    ...
    for tx_resp in tx.data {
        let response: MysTransactionBlockResponse = client
            .read_api()
            .get_transaction_with_options(tx_resp.digest, MysTransactionBlockResponseOptions::new())
            .await
            .unwrap();
...
async fn test_query_transaction_blocks() -> Result<(), anyhow::Error> {
    ...
    let mys_type_args = type_args![GAS::type_tag()]?;
    let type_args = mys_type_args
        .into_iter()
        .map(|ty| ty.try_into())
        .collect::<Result<Vec<_>, _>>()?;

    let mys_call_args_1 = call_args!(coin.data.clone().unwrap().object_id, 10)?;
    ...
    let mys_call_args_2 = call_args!(coin_2.data.clone().unwrap().object_id, 10)?;
    ...
    let move_call_query = MysTransactionBlockResponseQuery::new_with_filter(filter);
    let tx = client
        .read_api()
        .query_transaction_blocks(move_call_query, None, Some(20), true)
        .await
        .unwrap();
    // verify that only 1 tx is returned and no MysRpcInputError::ContainsDuplicates error
