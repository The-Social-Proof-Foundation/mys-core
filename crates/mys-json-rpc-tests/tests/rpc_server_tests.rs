...
use mys_json::{call_args, type_args};
...
use mys_json_rpc_types::ObjectsPage;
use mys_json_rpc_types::{
    Balance, CoinPage, DelegatedStake, StakeStatus, MysCoinMetadata, MysExecutionStatus,
    MysObjectDataOptions, MysObjectResponse, MysObjectResponseQuery, MysTransactionBlockEffectsAPI,
    MysTransactionBlockResponse, MysTransactionBlockResponseOptions, TransactionBlockBytes,
};
use mys_json_rpc_types::{ObjectChange, ZkLoginIntentScope};
use mys_macros::sim_test;
use mys_move_build::BuildConfig;
use mys_simulator::fastcrypto::encoding::{Base64, Encoding};
...
use mys_types::coin::{TreasuryCap, COIN_MODULE_NAME};
...
use mys_types::{parse_mys_struct_tag, MYS_FRAMEWORK_ADDRESS};
...
async fn test_get_package_with_display_should_not_fail() -> Result<(), anyhow::Error> {
    ...
    let response = http_client
        .get_object(
            ObjectID::from(MYS_FRAMEWORK_ADDRESS),
            Some(MysObjectDataOptions::new().with_display()),
        )
        .await;
    assert!(response.is_ok());
    let response: MysObjectResponse = response?;
    assert!(response
        .into_object()
        .unwrap()
        .display
...
async fn test_public_transfer_object() -> Result<(), anyhow::Error> {
    ...
    let tx_response: MysTransactionBlockResponse = http_client
...
async fn test_publish() -> Result<(), anyhow::Error> {
    ...
    let tx_response = http_client
        .execute_transaction_block(
            tx_bytes,
            signatures,
            Some(MysTransactionBlockResponseOptions::new().with_effects()),
            Some(ExecuteTransactionRequestType::WaitForLocalExecution),
        )
        .await?;
    matches!(tx_response, MysTransactionBlockResponse {effects, ..} if effects.as_ref().unwrap().created().len() == 6);
    Ok(())
}
...
async fn test_move_call() -> Result<(), anyhow::Error> {
    ...
    // now do the call
    let package_id = ObjectID::new(MYS_FRAMEWORK_ADDRESS.into_bytes());
    ...
    let tx_response = http_client
        ...
        .await?;
    matches!(tx_response, MysTransactionBlockResponse {effects, ..} if effects.as_ref().unwrap().created().len() == 1);
    Ok(())
}
...
async fn test_get_object_info() -> Result<(), anyhow::Error> {
    ...
    for obj in objects {
        ...
        let result = http_client
            .get_object(
                oref.object_id,
                Some(MysObjectDataOptions::new().with_owner()),
            )
            .await?;
        assert!(
            matches!(result, MysObjectResponse { data: Some(object), .. } if oref.object_id == object.object_id && object.owner.clone().unwrap().get_owner_address()? == address)
        );
    }
    Ok(())
}
...
async fn test_get_object_data_with_content() -> Result<(), anyhow::Error> {
    ...
    let objects = http_client
        .get_owned_objects(
            address,
            Some(MysObjectResponseQuery::new_with_options(
                MysObjectDataOptions::new().with_content().with_owner(),
            )),
            None,
            None,
        )
    ...
    for obj in objects {
        ...
        let result = http_client
            .get_object(
                oref.object_id,
                Some(MysObjectDataOptions::new().with_content().with_owner()),
            )
            .await?;
        assert!(
            matches!(result, MysObjectResponse { data: Some(object), .. } if oref.object_id == object.object_id && object.owner.clone().unwrap().get_owner_address()? == address)
...
async fn test_get_coins() -> Result<(), anyhow::Error> {
    ...
    let result: CoinPage = http_client
        .get_coins(address, Some("0x2::mys::TestCoin".into()), None, None)
        .await?;
    assert_eq!(0, result.data.len());

    let result: CoinPage = http_client
        .get_coins(address, Some("0x2::mys::MYS".into()), None, None)
        .await?;
    assert_eq!(5, result.data.len());
    assert!(!result.has_next_page);
    ...
    // Test paging
    let result: CoinPage = http_client
        .get_coins(address, Some("0x2::mys::MYS".into()), None, Some(3))
        .await?;
...
async fn test_sorted_get_coin_response() {
    ...
    for amount in amounts {
        let tx = make_transfer_mys_transaction(&cluster.wallet, Some(address), Some(amount)).await;
        let (tx_bytes, signatures) = tx.to_tx_bytes_and_signatures();
...
async fn test_get_balance() -> Result<(), anyhow::Error> {
    ...
    assert_eq!("0x2::mys::MYS", result.coin_type);
    assert_eq!(
...
async fn test_get_metadata() -> Result<(), anyhow::Error> {
    ...
    let result: MysCoinMetadata = http_client
...
async fn test_get_total_supply() -> Result<(), anyhow::Error> {
    ...
    let treasury_cap = object_changes
        ...
        .find_map(|e| {
            ...
            {
                if &TreasuryCap::type_(parse_mys_struct_tag(&coin_name).unwrap()) == object_type {
                    Some(object_id)
                } else {
                    None
                }
    ...
    let MysTransactionBlockResponse { effects, .. } = tx_response;

    assert_eq!(MysExecutionStatus::Success, *effects.unwrap().status());
...
async fn test_staking() -> Result<(), anyhow::Error> {
    ...
    // Check StakedMys object before test
    let staked_mys: Vec<DelegatedStake> = http_client.get_stakes(address).await?;
    assert!(staked_mys.is_empty());
    ...
    http_client
        .execute_transaction_block(
            tx_bytes,
            signatures,
            Some(MysTransactionBlockResponseOptions::new()),
            Some(ExecuteTransactionRequestType::WaitForLocalExecution),
        )
        .await?;
    ...
    let staked_mys: Vec<DelegatedStake> = http_client.get_stakes(address).await?;
    assert_eq!(1, staked_mys.len());
    assert_eq!(1000000000, staked_mys[0].stakes[0].principal);
    assert!(matches!(
        staked_mys[0].stakes[0].status,
        StakeStatus::Pending
    ));
    let staked_mys_copy = http_client
        .get_stakes_by_ids(vec![staked_mys[0].stakes[0].staked_mys_id])
        .await?;
    assert_eq!(
        staked_mys[0].stakes[0].staked_mys_id,
        staked_mys_copy[0].stakes[0].staked_mys_id
    );
    Ok(())
}
...
async fn test_unstaking() -> Result<(), anyhow::Error> {
    ...
    let staked_mys_copy = http_client
        .get_stakes_by_ids(vec![
            staked_mys[0].stakes[0].staked_mys_id,
            staked_mys[0].stakes[1].staked_mys_id,
            staked_mys[0].stakes[2].staked_mys_id,
        ])
        .await?;

    assert!(matches!(
        &staked_mys_copy[0].stakes[0].status,
        StakeStatus::Active {
            estimated_reward: _
        }
    ));
    assert!(matches!(
        &staked_mys_copy[0].stakes[1].status,
        StakeStatus::Active {
            estimated_reward: _
        }
    ));
    assert!(matches!(
        &staked_mys_copy[0].stakes[2].status,
        StakeStatus::Active {
            estimated_reward: _
        }
    ));
    ...
    let transaction_bytes: TransactionBlockBytes = http_client
        .request_withdraw_stake(
            address,
            staked_mys_copy[0].stakes[2].staked_mys_id,
            None,
            1_000_000.into(),
        )
        .await?;
...
async fn test_zklogin_verify() -> Result<(), anyhow::Error> {
    ...
    let (kp, pk_zklogin, inputs) =
        &load_test_vectors("../mys-types/src/unit_tests/zklogin_test_vectors.json")[1];
    ...
    let tx_data = TestTransactionBuilder::new(zklogin_addr, gas, rgp)
        .transfer_mys(None, MysAddress::ZERO)
        .build();
    let msg = IntentMessage::new(Intent::mys_transaction(), tx_data.clone());
