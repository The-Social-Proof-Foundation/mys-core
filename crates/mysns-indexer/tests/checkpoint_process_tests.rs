...
use mysns_indexer::indexer::MysnsIndexer;
...
/// Checkpoint 22279496: Replaces the name added on `22279365` (new.test.mys) by removing it and then adding it as a node name.
/// Checkpoint 22279944: Adds `remove.test.mys`.
/// Checkpoint 22280030: Adds `remove.test.mys` as a replacement (the previous one expired!).
...
fn process_22279187_checkpoint() {
    ...
    assert!(names.contains(&"leaf.test.mys".to_string()));
    assert!(names.contains(&"node.test.mys".to_string()));
    assert!(names.contains(&"test.mys".to_string()));
}
...
fn process_22279365_checkpoint() {
    ...
    assert_eq!(addition.name, "new.test.mys".to_string());
    assert_eq!(addition.parent, "test.mys".to_string());
    assert_eq!(
        addition.nft_id,
...
fn process_22279944_checkpoint() {
    ...
    let addition = updates.first().unwrap();
    assert_eq!(addition.name, "remove.test.mys".to_string());
    assert_eq!(
        addition.nft_id,
...
/// Return a mysns_indexer instance for testing.
/// Uses the testnet demo we used to extract the checkpoints being processed.
fn get_test_indexer() -> MysnsIndexer {
    MysnsIndexer::new(
