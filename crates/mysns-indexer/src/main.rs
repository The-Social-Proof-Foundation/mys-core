...
use mysns_indexer::{
    get_connection_pool,
    indexer::{format_update_field_query, format_update_subdomain_wrapper_query, MysnsIndexer},
    models::VerifiedDomain,
    schema::domains,
    PgConnectionPool,
};

struct MysnsIndexerWorker {
    pg_pool: PgConnectionPool,
    indexer: MysnsIndexer,
}

impl MysnsIndexerWorker {
...
#[async_trait]
impl Worker for MysnsIndexerWorker {
    type Result = ();
...
async fn main() -> Result<()> {
    ...
    let indexer_setup =
        ...
        {
            MysnsIndexer::new(registry_id, subdomain_wrapper_type, name_record_type)
        } else {
            MysnsIndexer::default()
        };
    ...
    let worker_pool = WorkerPool::new(
        ...
        "mysns_indexing".to_string(), /* task name used as a key in the progress store */
        100,                          /* concurrency */
    );
