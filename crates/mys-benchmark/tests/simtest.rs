...
mod test {
    ...
    use mys_benchmark::workloads::expected_failure::ExpectedFailurePayloadCfg;
    use mys_benchmark::workloads::workload::ExpectedFailureType;
    use mys_benchmark::workloads::workload_configuration::{
    ...
    use mys_config::{AUTHORITIES_DB_NAME, MYS_KEYSTORE_FILENAME};
    ...
    use mys_core::checkpoints::{CheckpointStore, CheckpointWatermark};
    ...
    use mys_protocol_config::{PerObjectCongestionControlMode, ProtocolConfig, ProtocolVersion};
    use mys_simulator::tempfile::TempDir;
    use mys_simulator::{configs::*, SimConfig};
    use mys_storage::blob::Blob;
    use mys_surfer::surf_strategy::SurfStrategy;
    use mys_swarm_config::network_config_builder::ConfigBuilder;
    use mys_types::base_types::{ConciseableName, ObjectID, SequenceNumber};
    ...
    use mys_types::traffic_control::{FreqThresholdConfig, PolicyConfig, PolicyType};
    ...
    struct DeadValidator {
        node_id: mys_simulator::task::NodeId,
    ...
    async fn test_simulated_load_with_reconfig() {
        mys_protocol_config::ProtocolConfig::poison_get_for_min_version();
    ...
    fn get_keep_alive_nodes(cluster: &TestCluster) -> HashSet<mys_simulator::task::NodeId> {
        ...
        keep_alive_nodes.insert(mys_simulator::current_simnode_id());
        keep_alive_nodes
    }

    fn handle_failpoint(
        dead_validator: Arc<Mutex<Option<DeadValidator>>>,
        keep_alive_nodes: HashSet<mys_simulator::task::NodeId>,
        ...
        let cur_node = mys_simulator::current_simnode_id();
        ...
        if rng.gen_range(0.0..1.0) < probability {
            ...
            *dead_validator = Some(DeadValidator {
            ...
            mys_simulator::task::kill_current_node(Some(restart_after));
        }
    }
    ...
    async fn test_simulated_load_reconfig_with_prune_and_compact() {
        ...
        let node_state = test_cluster.fullnode_handle.mys_node.clone().state();
    ...
    async fn test_protocol_upgrade_compatibility_impl() {
        ...
        let manifest = mys_framework_snapshot::load_bytecode_snapshot_manifest();
        ...
        let init_framework =
            mys_framework_snapshot::load_bytecode_snapshot(starting_version).unwrap();
        let test_cluster = Arc::new(
        ...
        let _handle = tokio::task::spawn(async move {
            ...
            for version in starting_version..=max_ver {
                ...
                let new_framework = mys_framework_snapshot::load_bytecode_snapshot(next_version);
    ...
    fn handle_bool_failpoint(
        eligible_nodes: &HashSet<mys_simulator::task::NodeId>, // only given eligible nodes may fail
        probability: f64,
    ) -> bool {
        if !eligible_nodes.contains(&mys_simulator::current_simnode_id()) {
    ...
    async fn test_simulated_load_with_test_config(
        ...
        let keystore_path = test_cluster.swarm.dir().join(MYS_KEYSTORE_FILENAME);
        ...
        let surfer_task = tokio::spawn(async move {
            // now do a mys-surfer test
            let mut test_packages_dir = PathBuf::from(env!("CARGO_MANIFEST_DIR"));
            test_packages_dir.extend(["..", "..", "crates", "mys-surfer", "tests"]);
            let test_package_paths: Vec<PathBuf> = std::fs::read_dir(test_packages_dir)
                ...
                .collect();
            info!("using mys_surfer test packages: {test_package_paths:?}");

            let surf_strategy = SurfStrategy::new(Duration::from_millis(400));
            let results = mys_surfer::run_with_test_cluster_and_strategy(
                surf_strategy,
                test_duration,
                test_package_paths,
                test_cluster,
            ...
            .await;
            info!("mys_surfer test complete with results: {results:?}");
