...
/// A Mys account and all of the objects it owns
#[derive(Debug)]
pub struct MysAccount {
...
pub struct InMemoryWallet {
    accounts: BTreeMap<MysAddress, MysAccount>, // TODO: track shared and immutable objects as well
}
...
impl InMemoryWallet {
    ...
    pub fn add_account(
        ...
        self.accounts.insert(addr, MysAccount::new(key, gas, objs));
    }
    ...
    pub fn account_mut(&mut self, addr: &MysAddress) -> Option<&mut MysAccount> {
        self.accounts.get_mut(addr)
    }

    pub fn account(&self, addr: &MysAddress) -> Option<&MysAccount> {
        self.accounts.get(addr)
    }

    pub fn gas(&self, addr: &MysAddress) -> Option<&ObjectRef> {
        self.accounts.get(addr).map(|a| &a.gas)
    }

    pub fn owned_object(&self, addr: &MysAddress, id: &ObjectID) -> Option<&ObjectRef> {
        self.accounts.get(addr).and_then(|a| a.owned.get(id))
    }

    pub fn owned_objects(&self, addr: &MysAddress) -> Option<impl Iterator<Item = &ObjectRef>> {
    ...
    pub fn keypair(&self, addr: &MysAddress) -> Option<Arc<AccountKeyPair>> {
    ...
    pub fn addresses(&self) -> impl Iterator<Item = &MysAddress> {
        self.accounts.keys()
    }
