...
/// as stress, but may not be ttttable in some other cases.
#[derive(Clone)]
pub struct FullNodeReconfigObserver {
    pub fullnode_client: MysClient,
...
impl FullNodeReconfigObserver {
    pub async fn new(
        ...
        Self {
            fullnode_client: MysClientBuilder::default()
                .build(fullnode_rpc_url)
                .await
                .unwrap_or_else(|e| {
                    panic!(
                        "Can't create MysClient with rpc url {fullnode_rpc_url}: {:?}",
                        e
                    )
                }),
            committee_store,
...
impl ReconfigObserver<NetworkAuthorityClient> for FullNodeReconfigObserver {
    ...
    async fn run(&mut self, driver: Arc<dyn AuthorityAggregatorUpdatable<NetworkAuthorityClient>>) {
        loop {
            ...
            {
                Ok(mys_system_state) => {
                    let epoch_id = mys_system_state.epoch;
                    if epoch_id > driver.epoch() {
                        debug!(epoch_id, "Got MysSystemState in newer epoch");
                        let new_committee = mys_system_state.get_mys_committee_for_benchmarking();
                        let _ = self
                            .committee_store
                            .insert_new_committee(new_committee.committee());
                        let auth_agg = AuthorityAggregator::new_from_committee(
                            mys_system_state.get_mys_committee_for_benchmarking(),
                ...
                Err(err) => error!("Can't get MysSystemState from Full Node: {:?}", err,),
            }
        }
    }
}
