...
impl Env {
    ...
    async fn setup_local_env(
        ...
        let (primary_gas_owner, keypair): (MysAddress, AccountKeyPair) =
            deterministic_random_account_key();
        let keypair = Arc::new(keypair);

        // spawn a thread to spin up mys nodes on the multi-threaded server runtime.
        // running forever
    ...
    async fn setup_remote_env(
        ...
        let genesis = mys_config::node::Genesis::new_from_file(genesis_blob_path);
