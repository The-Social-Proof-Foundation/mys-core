...
pub struct ExpectedFailurePayload {
    ...
    transfer_from: MysAddress,
    transfer_to: MysAddress,
    gas: Vec<Gas>,
        ...
        match self.failure_type {
            ExpectedFailureType::InvalidSignature => {
                ...
                signatures.push(GenericSignature::Signature(
                    mys_types::crypto::Signature::Ed25519MysSignature(
...
impl Workload<dyn Payload> for ExpectedFailureWorkload {
    ...
    async fn make_test_payloads(
        ...
        let mut gas_by_address: HashMap<MysAddress, Vec<Gas>> = HashMap::new();
        ...
        let addresses: Vec<MysAddress> = gas_by_address.keys().cloned().collect();
