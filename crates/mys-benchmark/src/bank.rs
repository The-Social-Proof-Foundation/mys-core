...
impl BenchmarkBank {
    ...
    pub async fn generate(
        ...
        for chunk in chunked_coin_configs {
            let gas_coins = self.pay_mys(chunk, &mut init_coin, gas_price).await?;
    ...
    async fn pay_mys(
        ...
        let recipient_addresses: Vec<MysAddress> = coin_configs.iter().map(|g| g.address).collect();
        ...
        let tx = make_pay_mys_transaction(
            init_coin.0,
            vec![],
            recipient_addresses,
            amounts,
        ...
        let address_map: HashMap<MysAddress, Arc<AccountKeyPair>> = coin_configs
            .iter()
