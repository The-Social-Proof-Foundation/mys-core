...
impl TransactionBuilder {
    ...
    async fn select_gas(
        ...
        if let Some(gas) = input_gas {
            ...
            for obj in gas_objs {
                let response = self
                    .0
                    .get_object_with_options(obj.object_id, MysObjectDataOptions::new().with_bcs())
                    .await?;
            ...
            Err(anyhow!("Cannot find gas coin for signer address {signer} with amount sufficient for the required gas budget {gas_budget}. If you are using the pay or transfer commands, you can use pay-mys or transfer-mys commands instead, which will use the only object as gas payment."))
        }
    }
    ...
    pub async fn transfer_mys(
        ...
        let object = self.get_object_ref(mys_object_id).await?;
        let gas_price = self.0.get_reference_gas_price().await?;
        Ok(TransactionData::new_transfer_mys(
    ...
    pub fn pay_mys_tx_kind(
        &self,
        recipients: Vec<MysAddress>,
        amounts: Vec<u64>,
    ) -> Result<TransactionKind, anyhow::Error> {
        let mut builder = ProgrammableTransactionBuilder::new();
        builder.pay_mys(recipients.clone(), amounts.clone())?;
        let pt = builder.finish();
        let tx_kind = TransactionKind::programmable(pt);
        Ok(tx_kind)
    }

    pub async fn pay_mys(
        &self,
        signer: MysAddress,
        input_coins: Vec<ObjectID>,
        recipients: Vec<MysAddress>,
    ...
    pub fn pay_all_mys_tx_kind(&self, recipient: MysAddress) -> TransactionKind {
        let mut builder = ProgrammableTransactionBuilder::new();
    ...
    pub async fn pay_all_mys(
        ...
        let gas_object_ref = coin_refs.remove(0);
        let gas_price = self.0.get_reference_gas_price().await?;
        Ok(TransactionData::new_pay_all_mys(
            signer,
            coin_refs,
            recipient,
            gas_object_ref,
    ...
    pub async fn move_call_tx_kind(
        ...
        module: &str,
        function: &str,
        type_args: Vec<MysTypeTag>,
        call_args: Vec<MysJsonValue>,
    ) -> Result<TransactionKind, anyhow::Error> {
        let mut builder = ProgrammableTransactionBuilder::new();
    ...
    pub async fn resolve_and_checks_json_args(
        ...
        function: &Identifier,
        type_args: &[TypeTag],
        json_args: Vec<MysJsonValue>,
    ) -> Result<Vec<Argument>, anyhow::Error> {
        let object = self
            .0
            .get_object_with_options(package_id, MysObjectDataOptions::bcs_lossless())
            .await?
            .into_object()?;
        let Some(MysRawData::Package(package)) = object.bcs else {
            bail!(
    ...
    pub async fn upgrade_tx_kind(
        ...
        let upgrade_capability = self
            .0
            .get_object_with_options(upgrade_capability, MysObjectDataOptions::new().with_owner())
            .await?
            .into_object()?;
    ...
    pub async fn merge_coins_tx_kind(
        ...
        let coin = self
            .0
            .get_object_with_options(primary_coin, MysObjectDataOptions::bcs_lossless())
            .await?
            .into_object()?;
    ...
    pub async fn request_add_stake(
        ...
        let pt = {
            let mut builder = ProgrammableTransactionBuilder::new();
            let arguments = vec![
                builder.input(CallArg::MYS_SYSTEM_MUT).unwrap(),
    ...
    pub async fn request_withdraw_stake(
        ...
        let staked_mys = self.get_object_ref(staked_mys).await?;
        let gas_price = self.0.get_reference_gas_price().await?;
        let gas = self
        ...
        TransactionData::new_move_call(
            ...
            gas,
            vec![
                CallArg::MYS_SYSTEM_MUT,
                CallArg::Object(ObjectArg::ImmOrOwnedObject(staked_mys)),
            ],
            gas_budget,
            gas_price,
        )
    ...
    async fn get_object_ref_and_type(
        ...
        let object = self
            .0
            .get_object_with_options(object_id, MysObjectDataOptions::new().with_type())
            .await?
            .into_object()?;
