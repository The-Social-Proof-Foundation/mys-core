...
pub enum CommonTransactionCosts {
    ...
    SplitCoin(usize),
    TransferWholeCoin,
    TransferWholeMysCoin,
    TransferPortionMysCoin,
    SharedCounterCreate,
...
// 1. Run `cargo insta test --review` under `./mys-cost`.
...
async fn create_txes(
    ...
    //
    // Transfer Whole Mys Coin and Transfer Portion of Mys Coin
    //
    let whole_mys_coin_tx =
        TestTransactionBuilder::new(sender, gas_objects.pop().unwrap(), gas_price)
            .transfer_mys(None, MysAddress::default())
            .build();
    let partial_mys_coin_tx =
        TestTransactionBuilder::new(sender, gas_objects.pop().unwrap(), gas_price)
            .transfer_mys(Some(10), MysAddress::default())
            .build();
    ret.insert(
        CommonTransactionCosts::TransferWholeMysCoin,
        whole_mys_coin_tx,
    );
    ret.insert(
        CommonTransactionCosts::TransferPortionMysCoin,
        partial_mys_coin_tx,
    );
    ...
    let whole_coin_tx = TestTransactionBuilder::new(sender, gas_objects.pop().unwrap(), gas_price)
        .transfer(gas_objects.pop().unwrap(), MysAddress::default())
        .build();
