...
impl<C> Decode<C> for OwnerKind {
    fn decode<D: Decoder>(d: &mut D) -> Result<Self, DecodeError> {
        ...
        match kind[0] {
            0 => {
                let address = Compat::<MysAddress>::decode(d)?.0;
...
mod tests {
    ...
    fn test_owner_kind_roundtrip() {
        let address = OwnerKind::AddressOwner(MysAddress::random_for_testing_only());
        assert_eq!(address, key::decode(&key::encode(&address)).unwrap());

        let object = OwnerKind::ObjectOwner(MysAddress::random_for_testing_only());
