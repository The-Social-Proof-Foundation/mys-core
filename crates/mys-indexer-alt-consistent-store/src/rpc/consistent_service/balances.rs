...
use mys_indexer_alt_consistent_api::proto::rpc::consistent::v1alpha as grpc;
...
) -> Vec<(Vec<u8>, (MysAddress, TypeTag), i128, u128)> {
...
/// Parse the owner's `MysAddress` from a string. Addresses must start with `0x` followed by
/// between 1 and 64 hexadecimal characters.
///
/// TODO: Switch to using `mys_sdk_types::Address`, once the indexing framework is ported to the
/// new SDK.
fn owner(input: &str) -> Result<MysAddress, Error> {
    ...
    MysAddress::from_str(s.as_ref()).map_err(|_| Error::InvalidOwner(s.to_string()))
}
...
mod tests {
    ...
    fn coin_key(owner: MysAddress, type_str: &str) -> CoinKey {
        CoinKey {
            owner,
    ...
    fn address_key(owner: MysAddress, type_str: &str) -> AddressKey {
        AddressKey {
            owner,
            type_: TypeTag::from_str(type_str).unwrap(),
        }
