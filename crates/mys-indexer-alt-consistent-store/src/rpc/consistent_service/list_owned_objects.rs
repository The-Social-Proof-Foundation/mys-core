...
/// Parse a `MysAddress` from a string. Addresses must start with `0x` followed by between 1 and 64
/// hexadecimal characters.
///
/// TODO: Switch to using `mys_sdk_types::Address`, once the indexing framework is ported to the
/// new SDK.
fn addr(input: &str) -> Result<MysAddress, Error> {
    ...
    MysAddress::from_str(s.as_ref()).map_err(|_| Error::InvalidAddress(s.to_string()))
}
