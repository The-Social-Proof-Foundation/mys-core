...
use mys_indexer_alt_framework::ingestion::streaming_client::StreamingClientArgs;
use mys_indexer_alt_framework::pipeline::concurrent::ConcurrentConfig;
use mys_keys::keystore::AccountKeystore;
use mys_kvstore::BigTableClient;
use mys_kvstore::BigTableIndexer;
use mys_kvstore::BigTableStore;
use mys_kvstore::KeyValueStoreReader;
use mys_kvstore::set_write_legacy_data;
use mys_rpc::client::Client as GrpcClient;
...
use mys_rpc::proto::mys::rpc::v2::Transaction as GrpcTransaction;
use mys_rpc::proto::mys::rpc::v2::UserSignature;
...
const TABLES: &[&str] = &[
    mys_kvstore::tables::objects::NAME,
    mys_kvstore::tables::object_types::NAME,
    mys_kvstore::tables::transactions::NAME,
    mys_kvstore::tables::checkpoints::NAME,
    mys_kvstore::tables::checkpoints_by_digest::NAME,
    mys_kvstore::tables::watermark_alt_legacy::NAME,
    mys_kvstore::tables::epochs::NAME,
];
...
async fn get_all_coins(client: &mut GrpcClient, address: MysAddress) -> Result<Vec<Object>> {
...
impl TestHarness {
    ...
    /// Build, sign, and execute a MYS transfer via gRPC.
    async fn transfer_mys(&mut self, recipient: MysAddress, amount: u64) -> Result<Transaction> {
...
async fn test_indexer_e2e() -> Result<()> {
    ...
    for _ in 0..3 {
        ...
        signed_txns.push(harness.transfer_mys(recipient, 1).await?);
    }
