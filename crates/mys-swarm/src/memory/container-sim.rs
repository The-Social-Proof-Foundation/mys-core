...
pub(crate) struct Container {
    ...
    node_watch: watch::Receiver<Weak<MysNode>>,
}
...
impl Drop for Container {
    fn drop(&mut self) {
        if let Some(handle) = self.handle.take() {
            ...
            mys_simulator::runtime::Handle::try_current().map(|h| h.delete_node(handle.node_id));
        }
    }
}
...
impl Container {
    ...
    pub async fn spawn(config: NodeConfig, _runtime: RuntimeType) -> Self {
        ...
        let handle = mys_simulator::runtime::Handle::current();
        ...
        let node = builder
            ...
            .init(move || {
                ...
                async move {
                    ...
                    let server = MysNode::start(config, registry_service, None)
                        .await
                        .unwrap();
    ...
    /// Get a MysNodeHandle to the node owned by the container.
    pub fn get_node_handle(&self) -> Option<MysNodeHandle> {
        Some(MysNodeHandle::new(self.node_watch.borrow().upgrade()?))
    }
