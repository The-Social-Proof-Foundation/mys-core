...
const MYS_RPC_ATTRS: [&str; 2] = ["deprecated", "version"];
...
pub fn open_rpc(attr: TokenStream, item: TokenStream) -> TokenStream {
    ...
    let methods = rpc_definition.methods.iter().flat_map(|method|{
        ...
        if method.is_pubsub {
            Some(quote! {
                let mut inputs: Vec<mys_open_rpc::ContentDescriptor> = Vec::new();
                #(#inputs)*
    ...
    quote! {
        ...
        impl #open_rpc_name {
            pub fn module_doc() -> mys_open_rpc::Module{
                let mut builder = mys_open_rpc::RpcModuleDocBuilder::default();
                #(#methods)*
                #(#routes)*
                builder.build()
            }
                ...
                find_attr(&mut method.attrs, "method")
            {
                ...
                if let Some(version_attr) = attributes.find("version") {
                    if let (Some(token), Some(version)) = (&version_attr.token, &version_attr.value)
                    {
                        ...
                        attr.tokens = remove_mys_rpc_attributes(attributes);
                        continue;
                    }
                }
                attr.tokens = remove_mys_rpc_attributes(attributes);
...
// Remove Mys rpc specific attributes.
fn remove_mys_rpc_attributes(attributes: Attributes) -> TokenStream2 {
    let attrs = attributes
        .attrs
        .into_iter()
        .filter(|r| !MYS_RPC_ATTRS.contains(&r.key.to_string().as_str()))
