...
module mys::profile_platform {
    ...
    struct ProfilePlatformLink has key {
        id: UID,
        profile_id: ID,
        owner: address,
    ...
    struct ProfilePlatformRegistry has key {
        id: UID,
    ...
    struct UserJoinedPlatformEvent has copy, drop {
        profile_id: ID,
        platform_id: ID,
        user: address,
        timestamp: u64,
    ...
    struct UserPlatformInteractionEvent has copy, drop {
        profile_id: ID,
        platform_id: ID,
    ...
    public entry fun join_platform(
        ...
        let (has_platform_block_list, platform_block_list_id) = block_list::find_block_list(block_list_registry, platform_id);
        if (has_platform_block_list) {
            ...
            assert!(!platform::is_profile_blocked(platform, profile_id), EEntityBlocked);
        };
    ...
    public entry fun record_interaction(
        ...
        if (has_platform_block_list) {
            ...
            assert!(!platform::is_profile_blocked_by_id(platform_id, profile_id), EEntityBlocked);
        };
