...
module mys::user_token {
    ...
    struct UserTokenInfo has store, drop, copy {
        user: address,
    ...
    struct UserTokenOwnership<phantom T> has key {
        id: UID,
    ...
    struct FeeCollector<phantom T> has key {
        id: UID,
    ...
    struct TokenSwapEvent has copy, drop {
        token_type: address,
        sender: address,
        amount_in: u64,
        amount_out: u64,
    ...
    struct TokenConfigUpdatedEvent has copy, drop {
        token_type: address,
        user: address,
        commission_bps: u64,
        creator_split_bps: u64,
    ...
    public entry fun create_user_token<T: drop>(
        ...
        let commission = if (option::is_some(&commission_bps)) {
            let fee = option::destroy_some(commission_bps);
            assert!(fee <= MAX_COMMISSION_BPS, EInvalidCommission);
            fee
        } else {
        ...
        let creator_split = if (option::is_some(&creator_split_bps)) {
            let split = option::destroy_some(creator_split_bps);
            assert!(split <= 10000, EInvalidFeeSplit);
            split
        } else {
        ...
        transfer::transfer(ownership, user);
        
        // Share the fee collector
        transfer::share_object(fee_collector);
    ...
    fun find_token_info(registry: &TokenRegistry, token_type: address): UserTokenInfo {
        let len = vector::length(&registry.tokens);
        let i = 0;
    ...
    /// Make find_token_info public for other modules
    ...
    /// Public struct for events that can be used by other modules
