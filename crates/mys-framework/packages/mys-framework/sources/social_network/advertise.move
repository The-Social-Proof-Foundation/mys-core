module mys_framework::advertise {
    ...
    use mys_framework::post::{Self, Post};
    ...
    struct TargetingCriteria has store, drop, copy {
        /// Type of targeting
        targeting_type: u8,
    ...
    /// Ad creative content
    struct AdCreative has store, drop {
        /// Title of the ad
        title: String,
        /// Main content text
        content: String,
    ...
    struct AdAdminCap has key, store {
        id: UID,
    }
    ...
    struct AdvertiserRegisteredEvent has copy, drop {
    ...
    struct CampaignUpdatedEvent has copy, drop {
        campaign_id: address,
        advertiser: address,
        status: u8,
        updated_at: u64,
    ...
    struct AdEngagementEvent has copy, drop {
        campaign_id: address,
        user: address,
        engagement_type: u8,
        cost: u64,
    ...
    struct PlatformFeesWithdrawnEvent has copy, drop {
        amount: u64,
        recipient: address,
        timestamp: u64,
    }
    ...
    public entry fun record_engagement(
        ...
        // Verify caller is authorized (has admin cap)
        assert!(object::owner(admin_cap) == tx_context::sender(ctx), EUnauthorized);
        ...
        let cost = calculate_engagement_cost(campaign, engagement_type);
    ...
    fun build_targeting_criteria(
        ...
        let result = vector::empty<TargetingCriteria>();
        let i = 0;
