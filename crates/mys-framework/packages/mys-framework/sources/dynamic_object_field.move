...
module mys::dynamic_object_field {
    ...
    public fun exists_with_type<Name: copy + drop + store, Value: key + store>(
        ...
        exists_with_type_impl!<Name, Value>(object, name)
    }
    ...
    public fun id<Name: copy + drop + store>(object: &UID, name: Name): Option<ID> {
        ...
        let (_field_id, value_addr) = field::field_info<Wrapper<Name>>(object, key);
        option::some(object::id_from_address(value_addr))
    }
    ...
    macro fun add_impl<$Name: copy + drop + store, $Value: key>(
        ...
        let (field_id, _) = field::field_info<Wrapper<$Name>>(object, key);
        add_child_object(field_id.to_address(), value);
    }
    ...
    macro fun borrow_impl<$Name: copy + drop + store, $Value: key>(
        ...
        let name = $name;
        let key = Wrapper { name };
        let (field_id, value_id) = field::field_info<Wrapper<$Name>>(object, key);
    ...
    macro fun borrow_mut_impl<$Name: copy + drop + store, $Value: key>(
        ...
        let name = $name;
        let key = Wrapper { name };
        let (field_id, value_id) = field::field_info_mut<Wrapper<$Name>>(object, key);
        borrow_child_object_mut<$Value>(field_id, value_id)
    }
    ...
    macro fun remove_impl<$Name: copy + drop + store, $Value: key>(
        ...
        let key = Wrapper { name };
        let (field_id, value_id) = field::field_info<Wrapper<$Name>>(object, key);
        let value = remove_child_object<$Value>(field_id.to_address(), value_id);
    ...
    macro fun exists_with_type_impl<$Name: copy + drop + store, $Value: key>(
        ...
        let (field_id, value_id) = field::field_info<Wrapper<$Name>>(object, key);
        field::has_child_object_with_ty<$Value>(field_id.to_address(), value_id)
    }
}