...
module orderbook::clob_v2 {
    ...
    use mys::coin::{Self, Coin, join};
    use mys::event;
    use mys::linked_table::{Self, LinkedTable};
    use mys::mys::MYS;
    use mys::table::{Self, Table, contains, add, borrow_mut};

    use orderbook::critbit::{Self, CritbitTree, is_empty, borrow_mut_leaf_by_index, min_leaf, remove_leaf_by_index, max_leaf, next_leaf, previous_leaf, borrow_leaf_by_index, borrow_leaf_by_key, find_leaf, insert_leaf};
    use orderbook::custodian_v2::{Self as custodian, Custodian, AccountCap, mint_account_cap, account_owner};
    ...
    const FEE_AMOUNT_FOR_CREATE_POOL: u64 = 100 * 1_000_000_000; // 100 MYS
    #[test_only]
    ...
    public struct Order has store, drop {
        ...
        // Assuming a maximum TPS of 100K/s of Mys chain, it would take (1<<63) / 100000 / 3600 / 24 / 365 = 2924712 years to reach the full capacity.
    ...
    #[test_only] use mys::coin::mint_for_testing;

    #[test_only] use mys::test_scenario::{Self, Scenario};
    ...
    public fun setup_test_with_tick_lot_and_wrapped_pool(
        ...
        test_scenario::next_tx(scenario, sender);
        {
            let (pool, pool_owner_cap) = create_pool_with_return_<MYS, USD>(
                taker_fee_rate,
                maker_rebate_rate,
                tick_size,
                lot_size,
    ...
    fun test_place_limit_order_with_invalid_restrictions_() {
        ...
        test_scenario::next_tx(&mut test, alice);
        {
            let mut pool = test_scenario::take_shared<Pool<MYS, USD>>(&test);
            ...
            custodian::deposit(
                &mut pool.base_custodian,
                mint_for_testing<MYS>(1000 * 100000000, test_scenario::ctx(&mut test)),
                account_cap_user
            );
            custodian::deposit(
                &mut pool.quote_custodian,
    ...
    fun test_place_limit_order_with_restrictions_FILL_OR_KILL_() {
        ...
        test_scenario::next_tx(&mut test, alice);
        {
            ...
            custodian::assert_user_balance<MYS>(&pool.base_custodian, account_cap_user, 0, 1000 * 100000000);
            test_scenario::return_shared(pool);
            test_scenario::return_shared(clock);
            test_scenario::return_to_address<AccountCap>(alice, account_cap);
        };
        ...
        test_scenario::next_tx(&mut test, bob);
        {
            ...
            custodian::deposit(
                &mut pool.base_custodian,
                mint_for_testing<MYS>(900 * 100000000, test_scenario::ctx(&mut test)),
                account_cap_user
            );
            custodian::assert_user_balance<MYS>(&pool.base_custodian, account_cap_user, 900 * 100000000, 0);
            place_limit_order<MYS, USD>(
                &mut pool,
                CLIENT_ID_BOB,
                4 * FLOAT_SCALING,
    ...
    fun test_place_limit_order_with_restrictions_IMMEDIATE_OR_CANCEL() {
        ...
        test_scenario::next_tx(&mut test, alice);
        {
            ...
            let (base_filled, quote_filled, maker_injected, maker_order_id) = place_limit_order<MYS, USD>(
                &mut pool,
                CLIENT_ID_ALICE,
                10 * FLOAT_SCALING,
                1000 * 100000000,
        ...
        test_scenario::next_tx(&mut test, bob);
        {
            ...
            let (base_filled, quote_filled, maker_injected, _) = place_limit_order<MYS, USD>(
                &mut pool,
                CLIENT_ID_ALICE,
                4 * FLOAT_SCALING,
                800 * 100000000,
            ...
            custodian::assert_user_balance<MYS>(&pool.base_custodian, account_cap_user, 300 * 100000000, 0);
            {
