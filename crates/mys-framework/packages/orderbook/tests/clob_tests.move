...
module orderbook::clob_test {
    use mys::clock::{Self, Clock};
    use mys::coin::{Self, mint_for_testing, burn_for_testing};
    use mys::mys::MYS;
    ...
    fun get_market_price_(mut test: Scenario): TransactionEffects {
        ...
        next_tx(&mut test, alice);{
            let pool = test::take_shared<Pool<MYS, USD>>(&test);
            let (bid_price, ask_price) = clob::get_market_price<MYS, USD>(&pool);
        ...
        next_tx(&mut test, alice);
        {
            let mut pool = test::take_shared<Pool<MYS, USD>>(&test);
            ...
            let alice_deposit_WMYS: u64 = 100000;
            let alice_deposit_USDC: u64 = 100000;
            custodian::test_increase_user_available_balance<MYS>(base_custodian, account_cap_user, alice_deposit_WMYS);
    ...
    fun test_list_open_orders_(mut test: Scenario): TransactionEffects {
        ...
        next_tx(&mut test, alice);
        {
            ...
            let account_cap_user = account_owner(&account_cap);
            let (base_custodian, quote_custodian) = clob::borrow_mut_custodian(&mut pool);
            let alice_deposit_WMYS: u64 = 10000;
    ...
    fun test_deposit_withdraw_(mut test: Scenario): TransactionEffects {
        ...
        next_tx(&mut test, alice);
        {
            ...
            let account_cap_user = account_owner(&account_cap);
            let alice_deposit_WMYS: u64 = 10000;
            let alice_withdraw_WMYS: u64 = 5000;
            let alice_deposit_USDC: u64 = 10000;
            let alice_withdraw_USDC: u64 = 1000;
            clob::deposit_base(&mut pool, mint_for_testing<MYS>(alice_deposit_WMYS, ctx(&mut test)), &account_cap);
            clob::deposit_quote(&mut pool, mint_for_testing<USD>(alice_deposit_USDC, ctx(&mut test)), &account_cap);
            burn_for_testing(clob::withdraw_base(&mut pool, alice_withdraw_WMYS, &account_cap, ctx(&mut test)));
            burn_for_testing(clob::withdraw_quote(&mut pool, alice_withdraw_USDC, &account_cap, ctx(&mut test)));
            let (base_custodian, quote_custodian) = clob::borrow_custodian(&pool);
            custodian::assert_user_balance(
                base_custodian,
                account_cap_user,
                alice_deposit_WMYS - alice_withdraw_WMYS,
                0
            );
            custodian::assert_user_balance(
                quote_custodian,
    ...
    fun test_clean_up_expired_orders_(mut test: Scenario): TransactionEffects {
        ...
        next_tx(&mut test, bob);
        {
            ...
            let account_cap_user = account_owner(&account_cap);
            let (base_custodian, quote_custodian) = clob::borrow_mut_custodian(&mut pool);
            let bob_deposit_WMYS: u64 = 20000;
            let bob_deposit_USDC: u64 = 35000;
            custodian::test_increase_user_available_balance<MYS>(base_custodian, account_cap_user, bob_deposit_WMYS);
        ...
        next_tx(&mut test, owner);
        {
            ...
            let (base_custodian, quote_custodian) = clob::borrow_mut_custodian(&mut pool);
            custodian::assert_user_balance<MYS>(base_custodian, account_cap_owner_alice, 10000, 0);
        ...
        next_tx(&mut test, owner);
        {
            ...
            let (base_custodian, quote_custodian) = clob::borrow_mut_custodian(&mut pool);
            custodian::assert_user_balance<MYS>(base_custodian, account_cap_owner_bob, 20000, 0);
    ...
    fun test_partial_fill_and_cancel_(mut test: Scenario): TransactionEffects {
        ...
        next_tx(&mut test, alice);
        {
            ...
            custodian::deposit(
                base_custodian,
                mint_for_testing<MYS>(1000 * 100000000, ctx(&mut test)),
                account_cap_user
            );
            custodian::deposit(
                quote_custodian,
        ...
        next_tx(&mut test, alice);
        {
            ...
            custodian::assert_user_balance<MYS>(base_custodian, account_cap_user, 0, 1000 * 100000000);
            test::return_shared(pool);
            test::return_to_sender<AccountCap>(&test, account_cap);
        };
        next_tx(&mut test, bob);
        {
            ...
            custodian::deposit(
                base_custodian,
                mint_for_testing<MYS>(300 * 100000000, ctx(&mut test)),
                account_cap_user
            );
            custodian::assert_user_balance<MYS>(base_custodian, account_cap_user, 300 * 100000000, 0);
            test::return_shared(pool);
            test::return_to_address<AccountCap>(bob, account_cap);
        };
        ...
        next_tx(&mut test, bob);
        {
            ...
            let account_cap_user = account_owner(&account_cap);
            let (base_custodian, quote_custodian) = clob::borrow_mut_custodian(&mut pool);
            custodian::assert_user_balance<MYS>(base_custodian, account_cap_user, 0, 0);
        ...
        next_tx(&mut test, alice);
        {
            ...
            clob::cancel_order<MYS, USD>(&mut pool, 2, &account_cap);
            {
                let mut open_orders = vector::empty<Order>();
                vector::push_back(
                    &mut open_orders,
    ...
    fun test_full_transaction_(mut test: Scenario): TransactionEffects {
        ...
        next_tx(&mut test, alice);
        {
            ...
            let account_cap_user = account_owner(&account_cap);
            let (base_custodian, quote_custodian) = clob::borrow_mut_custodian(&mut pool);
            custodian::deposit(base_custodian, mint_for_testing<MYS>(1000, ctx(&mut test)), account_cap_user);
        ...
        next_tx(&mut test, bob);
        {
            ...
            let clock = test::take_shared<Clock>(&test);
            let account_cap = test::take_from_address<AccountCap>(&test, bob);
            let (coin1, coin2) = clob::place_market_order<MYS, USD>(&mut pool, &account_cap, CLIENT_ID_BOB, 600,
                false,
                mint_for_testing<MYS>(600, ctx(&mut test)),
                mint_for_testing<USD>(0, ctx(&mut test)),
                &clock,
                ctx(&mut test));
            assert!(coin::value<MYS>(&coin1) == 0);
        ...
        // bob passes in 600mys, and sells 100mys of it through market ask order
        // Bob should receive the remaining 500mys,
        // and 199 usdt (excluding handling fee) for selling 100mys at a unit price of 2
        next_tx(&mut test, bob);
        {
            let mut pool = test::take_shared<Pool<MYS, USD>>(&test);
            let clock = test::take_shared<Clock>(&test);
            let account_cap = test::take_from_address<AccountCap>(&test, bob);
            let (coin1, coin2) =clob::place_market_order<MYS, USD>(&mut pool, &account_cap, CLIENT_ID_BOB, 100,
                false,
                mint_for_testing<MYS>(600, ctx(&mut test)),
                mint_for_testing<USD>(0, ctx(&mut test)),
                &clock,
                ctx(&mut test));
            assert!(coin::value<MYS>(&coin1) == 500);
    ...
    fun test_place_limit_order_fill_or_kill_(mut test: Scenario): TransactionEffects {
        ...
        next_tx(&mut test, bob);
        {
            ...
            let account_cap = test::take_from_sender<AccountCap>(&test);
            let clock = test::take_shared<Clock>(&test);
            let (base_quantity_filled, quote_quantity_filled, is_placed, order_id) = clob::place_limit_order<MYS, USD>(
                &mut pool,
                CLIENT_ID_BOB,
                4 * FLOAT_SCALING,
                400,
        ...
        next_tx(&mut test, bob);
        {
            ...
            let  (base_avail, base_locked, quote_avail, quote_locked) = account_balance<MYS, USD>(&pool, &account_cap);
    ...
    fun test_absorb_all_liquidity_bid_side_with_customized_tick_(mut test: Scenario): TransactionEffects {
        ...
        next_tx(&mut test, bob);
        {
            ...
            let clock = test::take_shared<Clock>(&test);
            let account_cap = test::take_from_address<AccountCap>(&test, bob);
            let (coin1, coin2) = clob::place_market_order<MYS, USD>(&mut pool, &account_cap, CLIENT_ID_BOB, 2000, false,
                mint_for_testing<MYS>(2000, ctx(&mut test)),
                mint_for_testing<USD>(0, ctx(&mut test)),
                &clock,
                ctx(&mut test));
            assert!(coin::value<MYS>(&coin1) == 500, 0);
    ...
    fun test_absorb_all_liquidity_ask_side_with_customized_tick_(mut test: Scenario): TransactionEffects {
        ...
        next_tx(&mut test, alice);
        {
            ...
            let account_cap_user = account_owner(&account_cap);
            let (base_custodian, quote_custodian) = clob::borrow_mut_custodian(&mut pool);
            custodian::deposit(base_custodian, mint_for_testing<MYS>(10000, ctx(&mut test)), account_cap_user);
            custodian::deposit(quote_custodian, mint_for_testing<USD>(10000, ctx(&mut test)), account_cap_user);
            test::return_shared(pool);
            test::return_to_sender<AccountCap>(&test, account_cap);
        };
        ...
        next_tx(&mut test, bob);
        {
            ...
            let clock = test::take_shared<Clock>(&test);
            let account_cap = test::take_from_address<AccountCap>(&test, bob);
            let (coin1, coin2) = clob::place_market_order<MYS, USD>(&mut pool, &account_cap, CLIENT_ID_BOB, 5000, true,
                mint_for_testing<MYS>(10000, ctx(&mut test)),
                mint_for_testing<USD>(10000, ctx(&mut test)),
                &clock,
                ctx(&mut test));
            assert!(coin::value<MYS>(&coin1) == 12000);
    ...
    fun test_swap_exact_base_for_quote_(mut test: Scenario): TransactionEffects {
        ...
        next_tx(&mut test, bob);
        {
            ...
            let (base_coin, quote_coin, _) = clob::swap_exact_base_for_quote(
                ...
                &account_cap,
                1500,
                mint_for_testing<MYS>(1500, ctx(&mut test)),
                mint_for_testing<USD>(0,  ctx(&mut test)),
                &clock,
                ctx(&mut test)
            );
    ...
    fun test_withdraw_fees_(mut test: Scenario): TransactionEffects {
        ...
        next_tx(&mut test, alice);
        {
            ...
            let account_cap_user = account_owner(&account_cap);
            let (base_custodian, quote_custodian) = clob::borrow_mut_custodian(&mut pool);
            let alice_deposit_WMYS: u64 = 1000000000;
            let alice_deposit_USDC: u64 = 1000000000;
            custodian::test_increase_user_available_balance<MYS>(base_custodian, account_cap_user, alice_deposit_WMYS);
            custodian::test_increase_user_available_balance<USD>(quote_custodian, account_cap_user, alice_deposit_USDC);
            // Example selling 0.1 mys, for the price of .719
        ...
        // Buys some mys from alice
        {
            ...
            let (base_coin, quote_coin, _) = clob::swap_exact_base_for_quote(
                ...
                &account_cap,
                100000,
                mint_for_testing<MYS>(100000000, ctx(&mut test)),
                mint_for_testing<USD>(0,  ctx(&mut test)),
                &clock,
                ctx(&mut test)
            );
    ...
    fun test_cancel_and_remove_(mut test: Scenario): TransactionEffects {
        ...
        next_tx(&mut test, alice);
        {
            ...
            custodian::assert_user_balance<MYS>(base_custodian, account_cap_user, 0, 10);
        ...
        next_tx(&mut test, alice);
        {
            ...
            let (base_custodian, quote_custodian) = clob::borrow_custodian(&pool);
            custodian::assert_user_balance<USD>(quote_custodian, account_cap_user, 35 + 10 + 15, 65 - 10 - 15);
            custodian::assert_user_balance<MYS>(base_custodian, account_cap_user, 10, 0);
            test::return_shared(pool);
            test::return_to_address<AccountCap>(alice, account_cap);
        };
        end(test)
    ...
    fun test_inject_and_match_taker_bid_with_quote_quantity_(mut test: Scenario): TransactionEffects {
        ...
        next_tx(&mut test, bob);
        {
            let mut pool = test::take_shared<Pool<MYS, USD>>(&test);
            let account_cap = test::take_from_address<AccountCap>(&test, bob);
            let (base_quantity_filled, quote_quantity_filled) = clob::test_match_bid_with_quote_quantity<MYS, USD>(&mut pool, &account_cap, CLIENT_ID_BOB, 500, MAX_PRICE, 0);
        ...
        next_tx(&mut test, alice);
        {
            ...
            custodian::assert_user_balance<MYS>(base_custodian, account_cap_user, 8000, 2000);
            let (next_bid_order_id, next_ask_order_id, _, _) = clob::get_pool_stat(&pool);
            assert!(next_bid_order_id == clob::order_id_for_test(1, true));
            assert!(next_ask_order_id == clob::order_id_for_test(3, false));
        ...
        next_tx(&mut test, bob);
        {
            ...
            custodian::assert_user_balance<MYS>(base_custodian, account_cap_user_alice, 8000, 500 + 5);
            {
                let (_, _, _, asks) = get_pool_stat(&pool);
    ...
    fun test_inject_and_match_taker_bid_with_quote_quantity_zero_lot_(mut test: Scenario): TransactionEffects {
        ...
        next_tx(&mut test, bob);
        {
            ...
            custodian::assert_user_balance<MYS>(base_custodian, account_cap_user_alice, 8000, 2000);
            {
                let mut open_orders = vector::empty<Order>();
                vector::push_back(
                    &mut open_orders,
    ...
    fun test_inject_and_match_taker_bid_with_quote_quantity_partial_lot_(mut test: Scenario): TransactionEffects {
        ...
        next_tx(&mut test, bob);
        {
            ...
            custodian::assert_user_balance<MYS>(base_custodian, account_cap_user_alice, 8000, 500 + 10);
            {
                let (_, _, _, asks) = get_pool_stat(&pool);
                clob::check_empty_tick_level(asks, 2 * FLOAT_SCALING);
            };
    ...
    fun test_inject_and_match_taker_bid_(mut test: Scenario): TransactionEffects {
        ...
        next_tx(&mut test, bob);
        {
            ...
            custodian::assert_user_balance<MYS>(base_custodian, account_cap_user_alice, 8000, 500);
            {
                let (_, _, _, asks) = get_pool_stat(&pool);
                clob::check_empty_tick_level(asks, 2 * FLOAT_SCALING);
            };
    ...
    fun test_inject_and_match_taker_bid_with_maker_order_not_fully_filled_(mut test: Scenario): TransactionEffects {
        ...
        next_tx(&mut test, bob);
        {
            ...
            let (base_custodian, quote_custodian) = clob::borrow_custodian(&pool);
            custodian::assert_user_balance<MYS>(base_custodian, account_cap_user_alice, 8000, 750);
    ...
    fun test_inject_and_match_taker_ask_(mut test: Scenario): TransactionEffects {
        ...
        next_tx(&mut test, alice);
        {
            ...
            custodian::assert_user_balance<MYS>(base_custodian, account_cap_user, 0, 10000);
            let (next_bid_order_id, next_ask_order_id, _, _) = clob::get_pool_stat(&pool);
            assert!(next_bid_order_id == clob::order_id_for_test(3, true));
            assert!(next_ask_order_id == clob::order_id_for_test(1, false));
        ...
        next_tx(&mut test, bob);
        {
            ...
            custodian::assert_user_balance<MYS>(base_custodian, account_cap_user, 1500, 10000);
            {
                let (_, _, bids, _) = get_pool_stat(&pool);
                clob::check_empty_tick_level(bids, 5 * FLOAT_SCALING);
            };
    ...
    fun test_inject_and_match_taker_bid_with_quote_quantity_and_expiration_(mut test: Scenario): TransactionEffects {
        ...
        next_tx(&mut test, bob);
        {
            ...
            custodian::assert_user_balance<MYS>(base_custodian, account_cap_user, 8500, 5);
            {
                let (_, _, _, asks) = get_pool_stat(&pool);
                clob::check_empty_tick_level(asks, 2 * FLOAT_SCALING);
            };
    ...
    fun test_inject_and_match_taker_bid_with_expiration_(mut test: Scenario): TransactionEffects {
        ...
        next_tx(&mut test, bob);
        {
            ...
            custodian::assert_user_balance<MYS>(base_custodian, account_cap_user, 8500, 0);
            {
                let (_, _, _, asks) = get_pool_stat(&pool);
                clob::check_empty_tick_level(asks, 2 * FLOAT_SCALING);
            };
    ...
    fun test_inject_and_price_limit_affected_match_taker_bid_(mut test: Scenario): TransactionEffects {
        ...
        next_tx(&mut test, alice);
        {
            ...
            let account_cap_user = account_owner(&account_cap);
            let (base_custodian, quote_custodian) = clob::borrow_mut_custodian(&mut pool);
            let alice_deposit_WMYS: u64 = 100;
        ...
        next_tx(&mut test, alice);
        {
            ...
            custodian::assert_user_balance<MYS>(base_custodian, account_cap_user, 85, 15);
            let (next_bid_order_id, next_ask_order_id, _, _) = clob::get_pool_stat(&pool);
            assert!(next_bid_order_id == clob::order_id_for_test(1, true));
            assert!(next_ask_order_id == clob::order_id_for_test(3, false));
        ...
        next_tx(&mut test, bob);
        {
            ...
            custodian::assert_user_balance<MYS>(base_custodian, account_cap_user, 85, 0);
            {
                let (_, _, _, asks) = get_pool_stat(&pool);
                clob::check_empty_tick_level(asks, 2 * FLOAT_SCALING);
            };
    ...
    fun test_inject_and_price_limit_affected_match_taker_ask_(mut test: Scenario): TransactionEffects {
        ...
        next_tx(&mut test, bob);
        {
            ...
            custodian::assert_user_balance<MYS>(base_custodian, account_cap_user, 5, 10);
            {
                let (_, _, bids, _) = get_pool_stat(&pool);
                clob::check_empty_tick_level(bids, 5 * FLOAT_SCALING);
            };
    ...
    fun test_inject_and_price_limit_affected_match_taker_ask_returned_pool_(mut test: Scenario): TransactionEffects {
        ...
        next_tx(&mut test, alice);
        {
            let mut wrapped_pool = test::take_shared<WrappedPool<MYS, USD>>(&test);
            let pool = borrow_mut_pool<MYS, USD>(&mut wrapped_pool);
            let account_cap = test::take_from_address<AccountCap>(&test, alice);
            let account_cap_user = account_owner(&account_cap);
    ...
    fun test_place_market_sell_order_with_skipping_self_matching_(mut test: Scenario): TransactionEffects {
        ...
        next_tx(&mut test, alice);
        {
            ...
            let clock = test::take_shared<Clock>(&test);
            let account_cap = test::take_from_address<AccountCap>(&test, alice);
            let (coin1, coin2) =clob::place_market_order<MYS, USD>(&mut pool, &account_cap, CLIENT_ID_ALICE, 600,
                false,
                mint_for_testing<MYS>(600, ctx(&mut test)),
                mint_for_testing<USD>(0, ctx(&mut test)),
                &clock,
                ctx(&mut test));
            assert!(coin::value<MYS>(&coin1) == 600);
    ...
    fun test_place_market_buy_order_with_skipping_self_matching_(mut test: Scenario): TransactionEffects {
        ...
        next_tx(&mut test, alice);
        {
            ...
            let account_cap_user = account_owner(&account_cap);
            let (base_custodian, quote_custodian) = clob::borrow_mut_custodian(&mut pool);
            custodian::deposit(base_custodian, mint_for_testing<MYS>(1500, ctx(&mut test)), account_cap_user);
            custodian::deposit(quote_custodian, mint_for_testing<USD>(10000, ctx(&mut test)), account_cap_user);
            test::return_shared(pool);
            test::return_to_sender<AccountCap>(&test, account_cap);
        };
        ...
        next_tx(&mut test, alice);
        {
            ...
            let clock = test::take_shared<Clock>(&test);
            let account_cap = test::take_from_address<AccountCap>(&test, alice);
            let (coin1, coin2) =clob::place_market_order<MYS, USD>(&mut pool, &account_cap, CLIENT_ID_ALICE, 1,
                true,
                mint_for_testing<MYS>(0, ctx(&mut test)),
    ...
    fun test_place_limit_order_with_skipping_self_matching_(mut test: Scenario): TransactionEffects {
        ...
        next_tx(&mut test, alice);
        {
            ...
            let account_cap_user = account_owner(&account_cap);
            let (base_custodian, _) = clob::borrow_mut_custodian(&mut pool);
            custodian::deposit(base_custodian, mint_for_testing<MYS>(400, ctx(&mut test)), account_cap_user);
            test::return_shared(pool);
            test::return_to_sender<AccountCap>(&test, account_cap);
        };
    ...
    fun test_swap_exact_quote_for_base_with_skipping_self_matching_(mut test: Scenario): TransactionEffects {
        ...
        next_tx(&mut test, alice);
        {
            ...
            let account_cap_user = account_owner(&account_cap);
            let (base_custodian, quote_custodian) = clob::borrow_custodian(&pool);
            custodian::assert_user_balance<MYS>(base_custodian, account_cap_user, 10000, 0);
            custodian::assert_user_balance<USD>(quote_custodian, account_cap_user, 0, 10000);
    ...
    fun test_inject_and_match_taker_ask_with_skipping_self_matching_(mut test: Scenario): TransactionEffects {
        ...
        next_tx(&mut test, alice);
        {
            ...
            custodian::assert_user_balance<MYS>(
                base_custodian,
                account_cap_user,
                0,
                10000
    ...
    fun test_cancel_order_no_rounding(): TransactionEffects {
        ...
        next_tx(&mut test, alice);
        {
            ...
            // Example buying 0.1 mys, for the price of .719
            clob::test_inject_limit_order(&mut pool, CLIENT_ID_ALICE, 719000, 1000000000, 1000000000, true,
                CANCEL_OLDEST, &account_cap,ctx(&mut test));

            // Example selling 0.1 mys for the price of .519
        ...
        next_tx(&mut test, alice);
        {
            ...
            let account_cap = test::take_from_address<AccountCap>(&test, alice);
            // cancel all order
            // clob::cancel_order<MYS, USD>(&mut pool, 1, &account_cap);
            // clob::cancel_all_orders<MYS, USD>(&mut pool, &account_cap);
            let (base_custodian, quote_custodian) = clob::borrow_custodian(&pool);
            clob::check_balance_invariants_for_account(&account_cap, quote_custodian, base_custodian, &pool);
            test::return_shared(pool);
            test::return_to_address<AccountCap>(alice, account_cap);
        ...
        // Sells some USDC to alice
        {
            ...
            let (base_coin, quote_coin) = clob::place_market_order<MYS, USD>(
                &mut pool,
                &account_cap,
                CLIENT_ID_BOB,
                100000,
            ...
            let (base_coin, quote_coin) = clob::place_market_order<MYS, USD>(
                ...
                100000,
                false,
                mint_for_testing<MYS>(100000, ctx(&mut test)),
                mint_for_testing<USD>(0, ctx(&mut test)),
                &clock,
                ctx(&mut test)
            );
    ...
    fun test_rounding_lock_order(): TransactionEffects {
        ...
        next_tx(&mut test, alice);
        {
            ...
            let account_cap_user = account_owner(&account_cap);
            let (base_custodian, quote_custodian) = clob::borrow_mut_custodian(&mut pool);
            let alice_deposit_WMYS: u64 = 2000000000;
        ...
        next_tx(&mut test, bob);
        // Sell mys to alice, first rounding token cut in match_ask else clause
        {
            let mut pool = test::take_shared<Pool<MYS, USD>>(&test);
            let account_cap = test::take_from_address<AccountCap>(&test, bob);
            let clock = test::take_shared<Clock>(&test);
    ...
    fun test_swap_exact_base_for_quote_with_metadata_(mut test: Scenario): TransactionEffects {
        ...
        next_tx(&mut test, bob);
        {
            ...
            let (base_coin, quote_coin, _, of_events) = clob::swap_exact_base_for_quote_with_metadata(
                ...
                &account_cap,
                500,
                mint_for_testing<MYS>(500, ctx(&mut test)),
                mint_for_testing<USD>(0, ctx(&mut test)),
                &clock,
                ctx(&mut test)
            );
    ...
    fun test_place_limit_order_with_metadata_(mut test: Scenario): TransactionEffects {
        ...
        next_tx(&mut test, bob);
        {
            ...
            let clock = test::take_shared<Clock>(&test);
            let account_cap = test::take_from_address<AccountCap>(&test, bob);
            let bob_deposit_WMYS: u64 = 10000;
            let (base_custodian, _) = clob::borrow_mut_custodian(&mut pool);
            custodian::test_increase_user_available_balance<MYS>(base_custodian, account_owner(&account_cap), bob_deposit_WMYS);
