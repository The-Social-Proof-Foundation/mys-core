...
#[test_only]
module orderbook::order_query_tests {
    ...
    fun test_order_query_pagination() {
        ...
        let pool = test_scenario::take_shared<Pool<MYS, USD>>(&scenario);
        let page1 = iter_bids(&pool, none(), none(), none(), none(), true);
        assert!(vector::length(order_query::orders(&page1)) == 50);
    ...
    fun test_order_query_pagination_decending() {
        ...
        let orders = order_query::orders(&page1);
        let first_order = vector::borrow(orders, 0);
        assert!(order_query::order_id(first_order) == 50);
        let last_order = vector::borrow(orders, vector::length(orders) - 1);
    ...
    fun test_order_query_start_order_id() {
        ...
        let pool = test_scenario::take_shared<Pool<MYS, USD>>(&scenario);
        // test start order id
        let page = iter_bids(&pool, none(), some(21), none(), none(), true);
        assert!(vector::length(order_query::orders(&page)) == 30);
        assert!(!order_query::has_next_page(&page));

        let orders = order_query::orders(&page);
        let first_order = vector::borrow(orders, 0);
        assert!(order_query::order_id(first_order) == 21);
        let last_order = vector::borrow(orders, vector::length(orders) - 1);
        assert!(order_query::order_id(last_order) == 50);
    ...
    fun test_order_query_pagination_multiple_orders_same_tick_level() {
        let mut scenario = prepare_scenario();
        // orders with same tick level repeated 2 times for lighter test
        add_orders(30, TIMESTAMP_INF, none(), &mut scenario);
        add_orders(30, TIMESTAMP_INF, none(), &mut scenario);

        let pool = test_scenario::take_shared<Pool<MYS, USD>>(&scenario);
        let page1 = iter_bids(&pool, none(), none(), none(), none(), true);
        assert!(vector::length(order_query::orders(&page1)) == 60);
        assert!(!order_query::has_next_page(&page1));
        ...
        assert!(order_query::order_id(first_order) == 1);
        let last_order = vector::borrow(orders, vector::length(orders) - 1);
        assert!(order_query::order_id(last_order) == 60);

        // Query order with tick level >= 20 * FLOAT_SCALING
        let page = iter_bids(&pool, some(20 * FLOAT_SCALING), none(), none(), none(), true);

        // should only contain orders with tick level >= 20 - 30, 22 orders in total
        assert!(vector::length(order_query::orders(&page)) == 22);
        assert!(!order_query::has_next_page(&page));
    ...
    fun test_query_after_insert() {
        ...
        // this page should start from order id 11 and end at order id 50, contains 40 orders
        assert!(vector::length(order_query::orders(&page)) == 40);
        assert!(!order_query::has_next_page(&page));
