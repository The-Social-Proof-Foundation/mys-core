...
fun test_bridge_and_claim() {
    ...
    //
    // move from eth and transfer to mys account
    let transfer_id1 = env.bridge_to_mys<ETH>(
        source_chain,
        eth_address,
        mys_address,
    ...
    let transfer_id = env.bridge_to_mys<ETH>(
        source_chain,
        eth_address,
        mys_address,
        amount,
    );
    let token = env.claim_token<ETH>(mys_address, source_chain, transfer_id);
    env.send_token<ETH>(
        mys_address,
        source_chain,
        eth_address,
