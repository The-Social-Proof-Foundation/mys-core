...
module bridge::bridge_env {
    ...
    public fun create_env(chain_id: u8): BridgeEnv {
        ...
        let mut clock = mys::clock::create_for_testing(ctx);
    ...
    public fun register_committee(env: &mut BridgeEnv) {
        ...
        let mut system_state = test_scenario::take_shared<MysSystemState>(
            scenario,
        );
    ...
    fun add_default_tokens(env: &mut BridgeEnv, sender: address) {
        ...
        let add_token_message = create_add_tokens_on_mys_message(
            env.chain_id,
            bridge.get_seq_num_for(message_types::add_tokens_on_mys()),
            false,
    ...
    const MYS_MESSAGE_PREFIX: vector<u8> = b"MYS_BRIDGE_MESSAGE";

    fun sign_message(
        env: &BridgeEnv,
        message: BridgeMessage,
    ): vector<vector<u8>> {
        let mut message_bytes = MYS_MESSAGE_PREFIX;
    ...
    public fun bridge_to_mys<Token>(
        env: &mut BridgeEnv,
        source_chain: u8,
        source_address: vector<u8>,
        target_address: address,
