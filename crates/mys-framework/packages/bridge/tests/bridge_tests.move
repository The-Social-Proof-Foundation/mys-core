...
fun test_bridge_create_non_system_addr() {
    let mut env = create_env(chain_ids::mys_mainnet());
...
fun test_create_bridge_default() {
    let mut env = create_env(chain_ids::mys_custom());
...
fun test_execute_send_token_frozen() {
    let chain_id = chain_ids::mys_testnet();
...
fun test_system_msg_incorrect_chain_id() {
    ...
    let mut env = create_env(chain_ids::mys_testnet());
    env.create_bridge_default();
    env.execute_blocklist(sender, chain_ids::mys_mainnet(), 0, vector[]);
...
fun test_update_limit() {
    let chain_id = chain_ids::mys_mainnet();
    let mut env = create_env(chain_id);
    env.create_bridge_default();
...
fun test_execute_update_bridge_limit_abort_with_unexpected_chain_id() {
    ...
    // This abort because the receiving_chain (mys_mainnet) is not the same as
    // the bridge's chain_id (mys_devnet)
    env.update_bridge_limit(
        @0x0,
        chain_ids::mys_mainnet(),
        chain_ids::eth_mainnet(),
