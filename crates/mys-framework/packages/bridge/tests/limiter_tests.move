...
module bridge::limiter_tests {
    ...
    fun test_24_hours_windows() {
        ...
        let route = chain_ids::get_route(chain_ids::mys_custom(), chain_ids::eth_sepolia());
    ...
    fun test_24_hours_windows_multiple_route() {
        ...
        let route = chain_ids::get_route(chain_ids::mys_custom(), chain_ids::eth_sepolia());
        let route2 = chain_ids::get_route(chain_ids::eth_sepolia(), chain_ids::mys_custom());
    ...
    fun test_limiter_does_not_limit_receiving_transfers() {
        ...
        let route = chain_ids::get_route(chain_ids::mys_mainnet(), chain_ids::eth_mainnet());
        let mut scenario = test_scenario::begin(@0x1);
        let ctx = scenario.ctx();
        let treasury = treasury::mock_for_test(ctx);
        let mut clock = clock::create_for_testing(ctx);
        clock.set_for_testing(1706288001377);
        // We don't limit mys -> eth transfers. This aborts with `ELimitNotFoundForRoute`
        limiter.check_and_record_sending_transfer<ETH>(
            &treasury,
            &clock,
    ...
    fun test_limiter_basic_op() {
        ...
        let route = chain_ids::get_route(chain_ids::eth_sepolia(), chain_ids::mys_testnet());
    ...
    fun test_update_route_limit() {
        ...
        assert_eq(
            limiter.transfer_limits()[
                &chain_ids::get_route(chain_ids::eth_mainnet(), chain_ids::mys_mainnet())
            ],
        ...
        assert_eq(
            limiter.transfer_limits()[
                &chain_ids::get_route(chain_ids::eth_sepolia(), chain_ids::mys_testnet())
            ],
            max_transfer_limit(),
        );

        // shrink testnet limit
        update_route_limit(&mut limiter, &chain_ids::get_route(chain_ids::eth_sepolia(), chain_ids::mys_testnet()), 1_000 * usd_value_multiplier());
        assert_eq(
            limiter.transfer_limits()[
                &chain_ids::get_route(chain_ids::eth_sepolia(), chain_ids::mys_testnet())
            ],
    ...
    fun test_update_route_limit_all_paths() {
        ...
        update_route_limit(&mut limiter, &chain_ids::get_route(chain_ids::eth_sepolia(), chain_ids::mys_testnet()), new_limit);
        assert_eq(
            limiter.transfer_limits()[
                &chain_ids::get_route(chain_ids::eth_sepolia(), chain_ids::mys_testnet())
            ],
        ...
        update_route_limit(&mut limiter, &chain_ids::get_route(chain_ids::mys_testnet(), chain_ids::eth_sepolia()), new_limit);
        assert_eq(
            limiter.transfer_limits()[
                &chain_ids::get_route(chain_ids::eth_sepolia(), chain_ids::mys_testnet())
            ],
