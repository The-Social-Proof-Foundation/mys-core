...
module bridge::message_tests {
    use bridge::{
        ...
        message::{
            ...
            create_add_tokens_on_mys_message, create_blocklist_message,
            ...
            extract_add_tokens_on_mys, extract_blocklist_payload, extract_token_bridge_payload,
            extract_update_asset_price, extract_update_bridge_limit, make_add_token_on_mys,
    ...
    #[test]
    fun test_message_serialization_mys_to_eth() {
    ...
    #[test]
    fun test_message_serialization_eth_to_mys() {
        ...
        let token_bridge_message = create_token_bridge_message(
            ...
            chain_ids::mys_testnet(), // target_chain
    ...
    fun test_emergency_op_message_serialization() {
        let emergency_op_message = create_emergency_op_message(
            chain_ids::mys_testnet(), // source chain
            10, // seq_num
            emergency_op_pause(),
        );
    ...
    fun test_blocklist_message_serialization_regression() {
        ...
        let blocklist_message = create_blocklist_message(
            chain_ids::mys_custom(), // source chain
            129, // seq_num
            0, // blocklist
            validator_eth_addresses,
        );
    ...
    #[test]
    fun test_add_tokens_on_mys_message_serialization() {
        let mut scenario = test_scenario::begin(@0x1);
        let ctx = test_scenario::ctx(&mut scenario);
        let treasury = treasury::mock_for_test(ctx);

        let add_tokens_on_mys_message = create_add_tokens_on_mys_message(
            chain_ids::mys_custom(),
            1, // seq_num
            false, // native_token
        ...
        let payload = add_tokens_on_mys_message.extract_add_tokens_on_mys();
        ...
        // Test message serialization
        let message = serialize_message(add_tokens_on_mys_message);
        let expected_msg = hex::decode(
        ...
        assert_eq(message, expected_msg);
        assert!(add_tokens_on_mys_message == deserialize_message_test_only(message));
    ...
    #[test]
    fun test_add_tokens_on_mys_message_serialization_2() {
        let mut scenario = test_scenario::begin(@0x1);
        let ctx = test_scenario::ctx(&mut scenario);
        let treasury = treasury::mock_for_test(ctx);
        ...
        let mut message_bytes = b"MYS_BRIDGE_MESSAGE";
        message_bytes.append(message);

        let pubkey = mys::ecdsa_k1::secp256k1_ecrecover(
    ...
    fun test_bad_blocklist() {
        let blocklist_message = create_blocklist_message(
            chain_ids::mys_testnet(), 10, 0, vector[],
        );
    ...
    fun test_bad_blocklist_2() {
        ...
        let validator_eth_addresses = vector[validator_pub_key1, validator_pub_key2];
        create_blocklist_message(chain_ids::mys_testnet(), 10, 0, validator_eth_addresses);
    }
    ...
    fun test_bad_add_token() {
        ...
        let mut add_token_message = create_add_tokens_on_mys_message(
            chain_ids::mys_custom(),
            1, // seq_num
            false, // native_token
            vector[treasury.token_id<BTC>(), treasury.token_id<ETH>()],
        ...
        add_token_message.extract_add_tokens_on_mys();
    ...
    fun test_bad_token_transfer_type() {
        let msg = create_update_asset_price_message(2, chain_ids::mys_testnet(), 10, 12345);
    ...
    fun test_voting_power() {
        ...
        let treasury = treasury::mock_for_test(ctx);
        let message = create_add_tokens_on_mys_message(
            chain_ids::mys_custom(),
            1, // seq_num
            false, // native_token
            vector[treasury.token_id<BTC>(), treasury.token_id<ETH>()],
        ...
        let message = create_update_asset_price_message(2, chain_ids::mys_testnet(), 10, 12345);
        assert!(message.required_voting_power() == 5001);
    ...
    fun test_bad_voting_power_1() {
        let message = create_emergency_op_message(chain_ids::mys_testnet(), 10, 3);
    ...
    fun test_bad_voting_power_2() {
        let message = make_generic_message(
            ...
            1, 10, chain_ids::mys_testnet(), vector[],
        );
        message.required_voting_power();
    }
    ...
    fun test_invalid_chain_id_6() {
        create_update_bridge_limit_message(chain_ids::mys_testnet(), 1, INVALID_CHAIN, 1);
        abort 0
    }
    ...
    fun test_invalid_chain_id_8() {
        create_add_tokens_on_mys_message(INVALID_CHAIN, 1, false, vector[], vector[], vector[]);
        abort 0
    }
