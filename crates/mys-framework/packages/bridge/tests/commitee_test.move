...
module bridge::committee_test {
    ...
    use mys::{hex, test_scenario, test_utils::{Self, assert_eq}};
    ...
    fun test_init_committee_validator_become_inactive() {
        ...
        mys_system::request_remove_validator(&mut system_state, &mut tx(@0xA, 0));
        test_scenario::return_shared(system_state);
        advance_epoch_with_reward_amounts(0, 0, &mut scenario);
        ...
        let ctx = test_scenario::ctx(&mut scenario);
        let voting_powers = mys_system::validator_voting_powers_for_testing(&mut system_state);
