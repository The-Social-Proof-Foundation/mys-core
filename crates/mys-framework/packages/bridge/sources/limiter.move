...
module bridge::limiter {
    ...
    // specify routes from Ethereum to Mys.
    fun initial_transfer_limits(): VecMap<BridgeRoute, u64> {
        ...
        transfer_limits.insert(
            chain_ids::get_route(chain_ids::eth_sepolia(), chain_ids::mys_custom()),
            MAX_TRANSFER_LIMIT
        );

        transfer_limits.insert(
            chain_ids::get_route(chain_ids::eth_custom(), chain_ids::mys_testnet()),
            MAX_TRANSFER_LIMIT
        );

        transfer_limits.insert(
            chain_ids::get_route(chain_ids::eth_custom(), chain_ids::mys_custom()),
            MAX_TRANSFER_LIMIT
        );
