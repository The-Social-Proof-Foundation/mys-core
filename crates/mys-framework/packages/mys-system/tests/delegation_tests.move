...
#[test_only]
module mys_system::stake_tests {
    use mys::coin;
    use mys::test_scenario;
    use mys_system::mys_system::MysSystemState;
    use mys_system::staking_pool::{Self, StakedMys, PoolTokenExchangeRate};
    ...
    #[test]
    fun test_split_join_staked_mys() {
        // All this is just to generate a dummy StakedMys object to split and join later
        ...
        {
            let mut staked_mys = scenario.take_from_sender<StakedMys>();
            let ctx = scenario.ctx();
            staked_mys.split_to_sender(20 * MIST_PER_MYS, ctx);
            scenario.return_to_sender(staked_mys);
        };

        // Verify the correctness of the split and send the join txn
        scenario.next_tx(STAKER_ADDR_1);
        {
            let staked_mys_ids = scenario.ids_for_sender<StakedMys>();
            assert!(staked_mys_ids.length() == 2); // staked mys split to 2 coins

            let mut part1 = scenario.take_from_sender_by_id<StakedMys>(staked_mys_ids[0]);
            let part2 = scenario.take_from_sender_by_id<StakedMys>(staked_mys_ids[1]);

            let amount1 = part1.amount();
            let amount2 = part2.amount();
            assert!(amount1 == 20 * MIST_PER_MYS || amount1 == 40 * MIST_PER_MYS);
            assert!(amount2 == 20 * MIST_PER_MYS || amount2 == 40 * MIST_PER_MYS);
            assert!(amount1 + amount2 == 60 * MIST_PER_MYS);

            part1.join(part2);
            assert!(part1.amount() == 60 * MIST_PER_MYS);
    ...
    #[test]
    #[expected_failure(abort_code = staking_pool::EIncompatibleStakedMys)]
    fun test_join_different_epochs() {
        set_up_mys_system_state();
        let mut scenario_val = test_scenario::begin(STAKER_ADDR_1);
        let scenario = &mut scenario_val;
        // Create two instances of staked mys w/ different epoch activations
        stake_with(STAKER_ADDR_1, VALIDATOR_ADDR_1, 60, scenario);
    ...
    #[test]
    #[expected_failure(abort_code = staking_pool::EStakedMysBelowThreshold)]
    fun test_split_below_threshold() {
        ...
        scenario.next_tx(STAKER_ADDR_1);
        {
            ...
            staked_mys.split_to_sender(1 * MIST_PER_MYS + 1, ctx);
            scenario.return_to_sender(staked_mys);
        };
        scenario_val.end();
    }
    ...
    fun test_split_nonentry_below_threshold() {
        ...
        scenario.next_tx(STAKER_ADDR_1);
        {
            ...
            let ctx = scenario.ctx();
            // The remaining amount after splitting is below the threshold so this should fail.
            let stake = staked_mys.split(1 * MIST_PER_MYS + 1, ctx);
    ...
    fun test_add_remove_stake_flow() {
        ...
        scenario.next_tx(STAKER_ADDR_1);
        {
            ...
            system_state_mut_ref.request_add_stake(
                coin::mint_for_testing(60 * MIST_PER_MYS, ctx), VALIDATOR_ADDR_1, ctx
            );

            assert!(system_state_mut_ref.validator_stake_amount(VALIDATOR_ADDR_1) == 100 * MIST_PER_MYS);
            assert!(system_state_mut_ref.validator_stake_amount(VALIDATOR_ADDR_2) == 100 * MIST_PER_MYS);
        ...
        scenario.next_tx(STAKER_ADDR_1);
        {
            ...
            let staked_mys = scenario.take_from_sender<StakedMys>();
            assert!(staked_mys.amount() == 60 * MIST_PER_MYS);


            let mut system_state = scenario.take_shared<MysSystemState>();
            let system_state_mut_ref = &mut system_state;

            assert!(system_state_mut_ref.validator_stake_amount(VALIDATOR_ADDR_1) == 160 * MIST_PER_MYS);
            assert!(system_state_mut_ref.validator_stake_amount(VALIDATOR_ADDR_2) == 100 * MIST_PER_MYS);

            let ctx = scenario.ctx();

            // Unstake from VALIDATOR_ADDR_1
            system_state_mut_ref.request_withdraw_stake(staked_mys, ctx);
        ...
        {
            let mut system_state = scenario.take_shared<MysSystemState>();
            assert!(system_state.validator_stake_amount(VALIDATOR_ADDR_1) == 100 * MIST_PER_MYS);
            test_scenario::return_shared(system_state);
        };
        scenario_val.end();
    }
    ...
    fun test_remove_stake_post_active_flow(should_distribute_rewards: bool) {
        set_up_mys_system_state_with_storage_fund();
        let mut scenario_val = test_scenario::begin(VALIDATOR_ADDR_1);
        let scenario = &mut scenario_val;
        ...
        assert_validator_total_stake_amounts(
            ...
            vector[200 * MIST_PER_MYS, 100 * MIST_PER_MYS],
            scenario
        );
        ...
        let reward_amt = if (should_distribute_rewards) 15 * MIST_PER_MYS else 0;
        let validator_reward_amt = if (should_distribute_rewards) 10 * MIST_PER_MYS else 0;
        ...
        scenario.next_tx(STAKER_ADDR_1);
        {
            ...
            assert!(!system_state_mut_ref.validators().is_active_validator_by_mys_address(VALIDATOR_ADDR_1));

            let staked_mys = scenario.take_from_sender<StakedMys>();
            assert_eq(staked_mys.amount(), 100 * MIST_PER_MYS);

            // Unstake from VALIDATOR_ADDR_1
            assert_eq(total_mys_balance(STAKER_ADDR_1, scenario), 0);
            let ctx = scenario.ctx();
            system_state_mut_ref.request_withdraw_stake(staked_mys, ctx);

            // Make sure they have all of their stake.
            assert_eq(total_mys_balance(STAKER_ADDR_1, scenario), 100 * MIST_PER_MYS + reward_amt);

            test_scenario::return_shared(system_state);
        };

        // Validator unstakes now.
        assert_eq(total_mys_balance(VALIDATOR_ADDR_1, scenario), 0);
        ...
        assert_eq(total_mys_balance(VALIDATOR_ADDR_1, scenario), 100 * MIST_PER_MYS + reward_amt + validator_reward_amt);
    ...
    fun test_earns_rewards_at_last_epoch() {
        ...
        let reward_amt = 15 * MIST_PER_MYS;
        let validator_reward_amt = 10 * MIST_PER_MYS;
    ...
    fun test_add_preactive_remove_preactive() {
        ...
        assert_eq(total_mys_balance(STAKER_ADDR_1, scenario), 100 * MIST_PER_MYS);
    ...
    fun test_add_preactive_remove_pending_failure() {
        ...
        // Delegate 100 MYS to the pending validator. This should fail because pending active validators don't accept
    ...
    fun test_add_preactive_remove_active() {
        ...
        // Delegate 100 MYS to the preactive validator
        stake_with(STAKER_ADDR_1, NEW_VALIDATOR_ADDR, 100, scenario);
        ...
        // staker 1 and 3 unstake from the validator and earns about 2/5 * (85 - 10) * 1/3 = 10 MYS each.
        ...
        // So they will both get slightly more than 110 MYS in total balance.
        unstake(STAKER_ADDR_1, 0, scenario);
        assert_eq(total_mys_balance(STAKER_ADDR_1, scenario), 110002000000);
        unstake(STAKER_ADDR_3, 0, scenario);
        assert_eq(total_mys_balance(STAKER_ADDR_3, scenario), 110002000000);

        advance_epoch_with_reward_amounts(0, 85, scenario);
        unstake(STAKER_ADDR_2, 0, scenario);
        // staker 2 earns about 5 MYS from the previous epoch and 24-ish from this one
        // so in total she has about 50 + 5 + 24 = 79 MYS.
        assert_eq(total_mys_balance(STAKER_ADDR_2, scenario), 78862939078);
    ...
    fun test_add_preactive_remove_post_active() {
        ...
        // staker 1 earns a bit greater than 30 MYS here. A bit greater because the new validator's voting power
        ...
        unstake(STAKER_ADDR_1, 0, scenario);
        assert_eq(total_mys_balance(STAKER_ADDR_1, scenario), 130006000000);
    ...
    fun test_staking_pool_exchange_rate_getter() {
        ...
        let scenario = &mut scenario_val;
        stake_with(@0x42, @0x2, 100, scenario); // stakes 100 MYS with 0x2
        scenario.next_tx(@0x42);
        let staked_mys = scenario.take_from_address<StakedMys>(@0x42);
        let pool_id = staked_mys.pool_id();
        test_scenario::return_to_address(@0x42, staked_mys);
        advance_epoch(scenario); // advances epoch to effectuate the stake
        // Each staking pool gets 10 MYS of rewards.
        ...
        assert_exchange_rate_eq(rates, 1, 200, 200); // 200 MYS of self + delegate stake at epoch 1
        assert_exchange_rate_eq(rates, 2, 210, 200); // 10 MYS of rewards at epoch 2
        test_scenario::return_shared(system_state);
        scenario_val.end();
    }

    fun assert_exchange_rate_eq(
        rates: &Table<u64, PoolTokenExchangeRate>, epoch: u64, mys_amount: u64, pool_token_amount: u64
    ) {
        let rate = &rates[epoch];
        assert_eq(rate.mys_amount(), mys_amount * MIST_PER_MYS);
        assert_eq(rate.pool_token_amount(), pool_token_amount * MIST_PER_MYS);
    }
    ...
    fun set_up_mys_system_state_with_storage_fund() {
        let mut scenario_val = test_scenario::begin(@0x0);
        let scenario = &mut scenario_val;
        let ctx = scenario.ctx();
        ...
        create_mys_system_state_for_testing(validators, 300, 100, ctx);
        scenario_val.end();
    }
}
