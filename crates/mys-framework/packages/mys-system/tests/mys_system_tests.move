...
// This file contains tests testing functionalities in `mys_system` that are not
// already tested by the other more themed tests such as `stake_tests` or
// `rewards_distribution_tests`.

#[test_only]
module mys_system::mys_system_tests {
    use mys::test_scenario::{Self, Scenario};
    use mys::mys::MYS;
    use mys::coin::Self;
    use mys_system::governance_test_utils::{add_validator_full_flow, advance_epoch, remove_validator, set_up_mys_system_state, create_mys_system_state_for_testing, stake_with, unstake};
    ...
    fun test_validator_ops_by_stakee_ok() {
        let mut scenario_val = test_scenario::begin(@0x0);
        let scenario = &mut scenario_val;
        set_up_mys_system_state(vector[@0x1, @0x2]);
    ...
    #[test]
    #[expected_failure(abort_code = ::mys_system::validator_set::EInvalidCap)]
    ...
    #[test]
    #[expected_failure(abort_code = mys_system_state_inner::ENotValidator)]
    ...
    #[test]
    #[expected_failure(abort_code = mys_system_state_inner::ECannotReportOneself)]
    ...
    #[test]
    #[expected_failure(abort_code = mys_system_state_inner::EReportRecordNotFound)]
    ...
    fun test_validator_address_by_pool_id() {
        ...
        set_up_mys_system_state(vector[@0x1, @0x2, @0x3, @0x4]);
        scenario.next_tx(@0x1);
    ...
    // Note: `pop` MUST be a valid signature using mys_address and protocol_pubkey_bytes.
    ...
    fun test_active_validator_update_metadata() {
        ...
        // Set up MysSystemState with an active validator
        ...
        let validator = validator::new_for_testing(
            ...
            b"/ip4/127.0.0.1/udp/80",
            b"/ip4/127.0.0.1/udp/80",
            option::some(balance::create_for_testing<MYS>(100_000_000_000)),
            1,
            0,
            true,
            ctx
    ...
    fun test_add_validator_candidate_failure_duplicate_with_active() {
        ...
        create_mys_system_state_for_testing(vector[validator], 1000, 0, ctx);
    ...
    fun test_withdraw_inactive_stake() {
...

        {
            ...
            assert!(staking_pool.mys_balance() == 100 * 1_000_000_000, 0);
    ...
    #[test]
    fun test_convert_to_fungible_staked_mys_and_redeem() {
        let mut scenario_val = test_scenario::begin(@0x0);
        let scenario = &mut scenario_val;
        // Epoch duration is set to be 42 here.
        set_up_mys_system_state(vector[@0x1, @0x2]);
        ...
        let staked_mys = system_state.request_add_stake_non_entry(
            coin::mint_for_testing(100_000_000_000, scenario.ctx()),
            @0x1,
            scenario.ctx()
        );

        assert!(staked_mys.amount() == 100_000_000_000, 0);

        test_scenario::return_shared(system_state);
        advance_epoch(scenario);

        let mut system_state = scenario.take_shared<MysSystemState>();
        let fungible_staked_mys = system_state.convert_to_fungible_staked_mys(
            staked_mys,
            scenario.ctx()
        );

        assert!(fungible_staked_mys.value() == 100_000_000_000, 0);

        let mys = system_state.redeem_fungible_staked_mys(
            fungible_staked_mys,
            scenario.ctx()
        );

        assert!(mys.value() == 100_000_000_000, 0);
