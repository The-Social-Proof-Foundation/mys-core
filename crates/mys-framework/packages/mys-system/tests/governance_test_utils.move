...
#[test_only]
module mys_system::governance_test_utils {
    use mys::address;
    use mys::balance;
    use mys::mys::MYS;
    use mys::coin::{Self, Coin};
    use mys_system::staking_pool::{StakedMys, StakingPool};
    ...
    public fun create_validator_for_testing(
        addr: address, init_stake_amount_in_mys: u64, ctx: &mut TxContext
    ): Validator {
        let validator = validator::new_for_testing(
            ...
            b"/ip4/127.0.0.1/udp/80",
            b"/ip4/127.0.0.1/udp/80",
            option::some(balance::create_for_testing<MYS>(init_stake_amount_in_mys * MIST_PER_MYS)),
            1,
            0,
            true,
            ctx
    ...
    public fun create_mys_system_state_for_testing(
        validators: vector<Validator>, mys_supply_amount: u64, storage_fund_amount: u64, ctx: &mut TxContext
    ) {
        let system_parameters = mys_system_state_inner::create_system_parameters(
        ...
        mys_system::create(
            object::new(ctx), // it doesn't matter what ID mys system state has in tests
            validators,
            balance::create_for_testing<MYS>(storage_fund_amount * MIST_PER_MYS), // storage_fund
            1,   // protocol version
    ...
    public fun set_up_mys_system_state(mut addrs: vector<address>) {
    ...
    public fun advance_epoch_with_reward_amounts(
        ...
        let storage_rebate = advance_epoch_with_reward_amounts_return_rebate(storage_charge * MIST_PER_MYS, computation_charge * MIST_PER_MYS, 0, 0, scenario);
    ...
    public fun advance_epoch_with_reward_amounts_and_slashing_rates(
        ...
        let storage_rebate = system_state.advance_epoch_for_testing(
            new_epoch, 1, storage_charge * MIST_PER_MYS, computation_charge * MIST_PER_MYS, 0, 0, 0, reward_slashing_rate, 0, ctx
        );
        test_utils::destroy(storage_rebate);
        test_scenario::return_shared(system_state);
        scenario.next_epoch(@0x0);
    ...
    public fun stake_with(
        ...
        system_state.request_add_stake(coin::mint_for_testing(amount * MIST_PER_MYS, ctx), validator, ctx);
        test_scenario::return_shared(system_state);
    }

    public fun unstake(
        staker: address, staked_mys_idx: u64, scenario: &mut Scenario
    ) {
        scenario.next_tx(staker);
        let stake_mys_ids = scenario.ids_for_sender<StakedMys>();
        let staked_mys = scenario.take_from_sender_by_id(stake_mys_ids[staked_mys_idx]);
        let mut system_state = scenario.take_shared<MysSystemState>();

        let ctx = scenario.ctx();
        system_state.request_withdraw_stake(staked_mys, ctx);
        test_scenario::return_shared(system_state);
    }
    ...
    public fun add_validator_full_flow(validator: address, name: vector<u8>, net_addr: vector<u8>, init_stake_amount: u64, pubkey: vector<u8>, pop: vector<u8>, scenario: &mut Scenario) {
        ...
        system_state.request_add_stake(coin::mint_for_testing<MYS>(init_stake_amount * MIST_PER_MYS, ctx), validator, ctx);
    ...
    /// Return the rewards for the validator at `addr` in terms of MYS.
    public fun stake_plus_current_rewards_for_validator(addr: address, system_state: &mut MysSystemState, scenario: &mut Scenario): u64 {
    ...
    public fun stake_plus_current_rewards(addr: address, staking_pool: &StakingPool, scenario: &mut Scenario): u64 {
        ...
        let mut stake_ids = scenario.ids_for_sender<StakedMys>();
        let current_epoch = scenario.ctx().epoch();

        while (!stake_ids.is_empty()) {
            let staked_mys_id = stake_ids.pop_back();
            let staked_mys = scenario.take_from_sender_by_id<StakedMys>(staked_mys_id);
            sum = sum + staking_pool.calculate_rewards(&staked_mys, current_epoch);
            scenario.return_to_sender(staked_mys);
        };
        sum
    }

    public fun total_mys_balance(addr: address, scenario: &mut Scenario): u64 {
        let mut sum = 0;
        scenario.next_tx(addr);
        let coin_ids = scenario.ids_for_sender<Coin<MYS>>();
        let mut i = 0;
        while (i < coin_ids.length()) {
            let coin = scenario.take_from_sender_by_id<Coin<MYS>>(coin_ids[i]);
            sum = sum + coin.value();
