...
#[test_only]
module mys_system::validator_set_tests {
    use mys::balance;
    use mys::coin;
    use mys_system::staking_pool::StakedMys;
    use mys_system::validator::{Self, Validator, staking_pool_id};
    use mys_system::validator_set::{Self, ValidatorSet, active_validator_addresses};
    use mys::test_scenario::{Self, Scenario};
    use mys::test_utils::{Self, assert_eq};
    use mys::vec_map;

    const MIST_PER_MYS: u64 = 1_000_000_000; // used internally for stakes.
    ...
    fun test_validator_set_flow() {
        ...
        assert!(validator_set.total_stake() == 100 * MIST_PER_MYS);
        ...
        let scenario = &mut scenario_val;
        {
            ...
            let stake = validator_set.request_add_stake(
                @0x1,
                coin::mint_for_testing(500 * MIST_PER_MYS, ctx1).into_balance(),
                ctx1,
            );
        ...
        assert!(validator_set.total_stake() == 1500 * MIST_PER_MYS);
        ...
        assert!(validator_set.total_stake() == 900 * MIST_PER_MYS);
    ...
    fun test_staking_below_threshold() {
        ...
        let validator1 = create_validator(@0x1, 1, 1, true, ctx);
        let mut validator_set = validator_set::new(vector[validator1], ctx);
        assert_eq(validator_set.total_stake(), 100 * MIST_PER_MYS);
        scenario_val.end();
        ...
        let stake = validator_set.request_add_stake(
            @0x1,
            balance::create_for_testing(MIST_PER_MYS - 1), // 1 MIST lower than the threshold
            ctx1,
        );
        transfer::public_transfer(stake, @0x1);
        test_utils::destroy(validator_set);
    ...
    fun test_staking_min_threshold() {
        ...
        let stake = validator_set.request_add_stake(
            @0x1,
            balance::create_for_testing(MIST_PER_MYS), // min possible stake
            ctx1,
        );
        transfer::public_transfer(stake, @0x1);

        advance_epoch_with_dummy_rewards(&mut validator_set, scenario);
        assert!(validator_set.total_stake() == 101 * MIST_PER_MYS);
    ...
    fun test_add_validator_failure_below_min_stake() {
        ...
        scenario.next_tx(@0x42);
        {
            ...
            let stake = validator_set.request_add_stake(
                @0x2,
                balance::create_for_testing(500 * MIST_PER_MYS),
                ctx,
            );
        ...
        // Validator 2 now has 700 MYS in stake but that's not enough because we need 701.
        validator_set.request_add_validator(701 * MIST_PER_MYS, scenario.ctx());
    ...
    fun test_add_validator_with_nonzero_min_stake() {
        ...
        scenario.next_tx(@0x2);
        // Validator 2 now has 700 MYS in stake and that's just enough.
        validator_set.request_add_validator(700 * MIST_PER_MYS, scenario.ctx());
    ...
    fun test_low_stake_departure() {
        ...
        let ctx = scenario.ctx();
        // Create 4 validators.
        let v1 = create_validator(@0x1, 1, 1, true, ctx); // 100 MYS of stake
        let v2 = create_validator(@0x2, 4, 1, true, ctx); // 400 MYS of stake
        let v3 = create_validator(@0x3, 10, 1, true, ctx); // 1000 MYS of stake
        let v4 = create_validator(@0x4, 4, 1, true, ctx); // 400 MYS of stake
        ...
        scenario.next_tx(@0x42);
        {
            let stake = scenario.take_from_sender<StakedMys>();
            let ctx = scenario.ctx();
    ...
    fun create_validator(addr: address, hint: u8, gas_price: u64, is_initial_validator: bool, ctx: &mut TxContext): Validator {
        let stake_value = hint as u64 * 100 * MIST_PER_MYS;
    ...
    fun advance_epoch_with_low_stake_params(
        ...
        validator_set.advance_epoch(
            ...
            &mut vec_map::empty(),
            0, // reward_slashing_rate
            low_stake_threshold * MIST_PER_MYS,
            very_low_stake_threshold * MIST_PER_MYS,
    ...
    fun add_and_activate_validator(validator_set: &mut ValidatorSet, validator: Validator, scenario: &mut Scenario) {
        scenario.next_tx(validator.mys_address());
        let ctx = scenario.ctx();
