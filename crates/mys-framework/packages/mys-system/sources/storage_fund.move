...
module mys_system::storage_fund {
    ...
    public struct StorageFund has store {
        total_object_storage_rebates: Balance<MYS>,
        non_refundable_balance: Balance<MYS>,
    }

    /// Called by `mys_system` at genesis time.
    public(package) fun new(initial_fund: Balance<MYS>) : StorageFund {
        StorageFund {
    ...
    /// Called by `mys_system` at epoch change times to process the inflows and outflows of storage fund.
