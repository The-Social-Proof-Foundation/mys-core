...
module mys_system::validator_set {

    use mys::balance::Balance;
    use mys::mys::MYS;
    use mys_system::validator::{Validator, staking_pool_id, mys_address};
    use mys_system::validator_cap::{Self, UnverifiedValidatorOperationCap, ValidatorOperationCap};
    use mys_system::staking_pool::{PoolTokenExchangeRate, StakedMys, pool_id, FungibleStakedMys, fungible_staked_mys_pool_id};
    use mys::priority_queue as pq;
    use mys::vec_map::{Self, VecMap};
    use mys::vec_set::VecSet;
    use mys::table::{Self, Table};
    use mys::event;
    use mys::table_vec::{Self, TableVec};
    use mys_system::voting_power;
    use mys_system::validator_wrapper::ValidatorWrapper;
    use mys_system::validator_wrapper;
    use mys::bag::Bag;
    use mys::bag;
    ...
    public(package) fun new(init_active_validators: vector<Validator>, ctx: &mut TxContext): ValidatorSet {
        ...
        while (i < num_validators) {
            ...
            staking_pool_mappings.add(staking_pool_id(validator), mys_address(validator));
            i = i + 1;
        };
    ...
    /// Called by `mys_system` to add a new validator candidate.
    public(package) fun request_add_validator_candidate(
        ...
        let validator_address = mys_address(&validator);
        assert!(
        ...
        self.validator_candidates.add(
            mys_address(&validator),
            validator_wrapper::create_v1(validator, ctx),
        );
    }

    /// Called by `mys_system` to remove a validator candidate, and move them to `inactive_validators`.
    ...
    /// Called by `mys_system` to add a new validator to `pending_active_validators`, which will be
    ...
    /// Called by `mys_system`, to remove a validator.
    ...
    /// Called by `mys_system`, to add a new stake to the validator.
    ...
    public(package) fun request_add_stake(
        ...
        assert!(mys_amount >= MIN_STAKING_THRESHOLD, EStakingBelowThreshold);
    ...
    /// Called by `mys_system`, to withdraw some share of a stake from the validator. The share to withdraw
    /// is denoted by `principal_withdraw_amount`. One of two things occurs in this function:
    /// 1. If the `staked_mys` is staked with an active validator, the request is added to the validator's
    ///    staking pool's pending stake withdraw entries, processed at the end of the epoch.
    /// 2. If the `staked_mys` was staked with a validator that is no longer active,
    ...
    public(package) fun request_withdraw_stake(
        ...
        let staking_pool_id = pool_id(&staked_mys);
        let validator =
            if (self.staking_pool_mappings.contains(staking_pool_id)) { // This is an active validator.
                let validator_address = self.staking_pool_mappings[pool_id(&staked_mys)];
        ...
        validator.request_withdraw_stake(staked_mys, ctx)
    }
    ...
    public(package) fun convert_to_fungible_staked_mys(
        ...
        validator.convert_to_fungible_staked_mys(staked_mys, ctx)
    }
    ...
    public(package) fun redeem_fungible_staked_mys(
        ...
        let staking_pool_id = fungible_staked_mys_pool_id(&fungible_staked_mys);
        ...
        validator.redeem_fungible_staked_mys(fungible_staked_mys, ctx)
    }
    ...
    public(package) fun advance_epoch(
        self: &mut ValidatorSet,
        computation_reward: &mut Balance<MYS>,
        storage_fund_reward: &mut Balance<MYS>,
        ...
        let total_slashed_validator_stake = sum_stake_by_addresses(&self.active_validators, &slashed_validators);
    ...
    fun update_and_process_low_stake_departures(
        ...
        while (i > 0) {
            ...
            let validator_address = validator_ref.mys_address();
    ...
    /// Called by `mys_system` to derive reference gas price for the new epoch.
    ...
    public(package) fun is_active_validator_by_mys_address(
        self: &ValidatorSet,
        validator_address: address,
    ): bool {
    ...
    /// It differs from `is_active_validator_by_mys_address` in that the former checks
    ...
    fun find_validator(validators: &vector<Validator>, validator_address: address): Option<u64> {
        ...
        while (i < length) {
            let v = &validators[i];
            if (v.mys_address() == validator_address) {
    ...
    fun process_pending_validators(
        ...
        while (!self.pending_active_validators.is_empty()) {
            ...
            event::emit(
                ValidatorJoinEvent {
                    epoch: new_epoch,
                    validator_address: validator.mys_address(),
                    staking_pool_id: staking_pool_id(&validator),
                }
            );
    ...
    fun compute_slashed_validators(
        ...
        while (!validator_report_records.is_empty()) {
            ...
            assert!(
                is_active_validator_by_mys_address(self, validator_address),
    ...
    fun compute_unadjusted_reward_distribution(
        ...
        let length = validators.length();
        let storage_fund_reward_per_validator = if (length > 0) {
        ...
        while (i < length) {
            ...
            let stake: u128 = validator.total_stake_amount() as u128;
            let reward_amount = if (total_stake > 0) {
                stake * (total_staking_reward as u128) / (total_stake as u128)
            } else {
                0
            };
    ...
    fun compute_adjusted_reward_distribution(
        ...
        // Guard against underflow: if slashed stake exceeds total stake, cap it at total stake.
        let safe_slashed_stake = if (total_slashed_validator_stake > total_stake) {
            total_stake
        } else {
            total_slashed_validator_stake
        };
        let total_unslashed_validator_stake = total_stake - safe_slashed_stake;
        ...
        while (i < length) {
            ...
            // Use u128 to avoid multiplication overflow.
            let stake = validator.total_stake_amount() as u128;
            ...
            let adjusted_staking_reward_amount =
                ...
                if (individual_staking_reward_adjustments.contains(&i)) {
                    ...
                    let adjustment = if (total_unslashed_validator_stake > 0 && total_staking_reward_adjustment > 0) {
                        total_staking_reward_adjustment as u128 * stake
            ...
            let adjusted_storage_fund_reward_amount =
                ...
                if (individual_storage_fund_reward_adjustments.contains(&i)) {
                    ...
                    let adjustment = if (num_unslashed_validators > 0) {
    ...
    fun distribute_reward(
        ...
        staking_rewards: &mut Balance<MYS>,
        storage_fund_reward: &mut Balance<MYS>,
        ctx: &mut TxContext
    ) {
        let length = validators.length();
    ...
    fun emit_validator_epoch_events(
        ...
        while (i < num_validators) {
            let v = &vs[i];
            let validator_address = v.mys_address();
    ...
    /// Sum up the total stake of a given list of validator addresses.
    public fun sum_stake_by_addresses(vs: &vector<Validator>, addresses: &vector<address>): u64 {
        let mut sum = 0;
        let mut i = 0;
        let length = addresses.length();
        while (i < length) {
            let validator = get_validator_ref(vs, addresses[i]);
            sum = sum + validator.total_stake_amount();
            i = i + 1;
        };
        sum
    }
    ...
    public(package) fun active_validator_addresses(self: &ValidatorSet): vector<address> {
        ...
        while (i < length) {
            let validator_address = vs[i].mys_address();
