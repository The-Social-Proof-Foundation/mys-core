...
module mys_system::mys_system_state_inner {
    ...
    use mys_system::validator_set::{Self, ValidatorSet};
    use mys_system::validator_cap::{UnverifiedValidatorOperationCap, ValidatorOperationCap};
    use mys_system::stake_subsidy::StakeSubsidy;
    use mys_system::storage_fund::{Self, StorageFund};
    ...
    public struct MysSystemStateInner has store {
        ...
        safe_mode: bool,
        safe_mode_storage_rewards: Balance<MYS>,
        safe_mode_computation_rewards: Balance<MYS>,
    ...
    public(package) fun v1_to_v2(self: MysSystemStateInner): MysSystemStateInnerV2 {
    ...
    public(package) fun request_withdraw_stake(
        ...
        self.validators.request_withdraw_stake(staked_mys, ctx)
    }
    ...
    public(package) fun convert_to_fungible_staked_mys(
        ...
        self.validators.convert_to_fungible_staked_mys(staked_mys, ctx)
    }
    ...
    public(package) fun redeem_fungible_staked_mys(
        ...
        self.validators.redeem_fungible_staked_mys(fungible_staked_mys, ctx)
    }
    ...
    public(package) fun report_validator(
        ...
        assert!(self.validators.is_active_validator_by_mys_address(reportee_addr), ENotValidator);
    ...
    public(package) fun advance_epoch(
        ...
        mut storage_reward: Balance<MYS>,
        mut computation_reward: Balance<MYS>,
    ...
    public(package) fun epoch(self: &MysSystemStateInnerV2): u64 {
        self.epoch
    }

    public(package) fun protocol_version(self: &MysSystemStateInnerV2): u64 {
        self.protocol_version
    }

    public(package) fun system_state_version(self: &MysSystemStateInnerV2): u64 {
    ...
    public(package) fun epoch_start_timestamp_ms(self: &MysSystemStateInnerV2): u64 {
        self.epoch_start_timestamp_ms
    }

    /// Returns the total amount staked with `validator_addr`.
    /// Aborts if `validator_addr` is not an active validator.
    public(package) fun validator_stake_amount(self: &MysSystemStateInnerV2, validator_addr: address): u64 {
        self.validators.validator_total_stake_amount(validator_addr)
    }

    /// Returns the voting power for `validator_addr`.
    /// Aborts if `validator_addr` is not an active validator.
    public(package) fun active_validator_voting_powers(self: &MysSystemStateInnerV2): VecMap<address, u64> {
    ...
    /// Aborts if `validator_addr` is not an active validator.
    public(package) fun validator_staking_pool_id(self: &MysSystemStateInnerV2, validator_addr: address): ID {

        self.validators.validator_staking_pool_id(validator_addr)
    }

    /// Returns reference to the staking pool mappings that map pool ids to active validator addresses
    public(package) fun validator_staking_pool_mappings(self: &MysSystemStateInnerV2): &Table<ID, address> {

        self.validators.staking_pool_mappings()
    }

    /// Returns all the validators who are currently reporting `addr`
    public(package) fun get_reporters_of(self: &MysSystemStateInnerV2, addr: address): VecSet<address> {
    ...
    public(package) fun get_storage_fund_total_balance(self: &MysSystemStateInnerV2): u64 {
        self.storage_fund.total_balance()
    }

    public(package) fun get_storage_fund_object_rebates(self: &MysSystemStateInnerV2): u64 {
        self.storage_fund.total_object_storage_rebates()
    }

    public(package) fun validator_address_by_pool_id(self: &mut MysSystemStateInnerV2, pool_id: &ID): address {
    ...
    public(package) fun active_validator_addresses(self: &MysSystemStateInnerV2): vector<address> {
    ...
    fun extract_coin_balance(mut coins: vector<Coin<MYS>>, amount: option::Option<u64>, ctx: &mut TxContext): Balance<MYS> {
    ...
    public(package) fun validators(self: &MysSystemStateInnerV2): &ValidatorSet {
        &self.validators
    }

    #[test_only]
    /// Return the currently active validator by address
    public(package) fun active_validator_by_address(self: &MysSystemStateInnerV2, validator_address: address): &Validator {
        self.validators().get_active_validator_ref(validator_address)
    }

    #[test_only]
    /// Return the currently pending validator by address
    public(package) fun pending_validator_by_address(self: &MysSystemStateInnerV2, validator_address: address): &Validator {
        self.validators().get_pending_validator_ref(validator_address)
    }

    #[test_only]
    /// Return the currently candidate validator by address
    public(package) fun candidate_validator_by_address(self: &MysSystemStateInnerV2, validator_address: address): &Validator {
        validators(self).get_candidate_validator_ref(validator_address)
    }

    #[test_only]
    public(package) fun get_stake_subsidy_distribution_counter(self: &MysSystemStateInnerV2): u64 {
        self.stake_subsidy.get_distribution_counter()
    }

    #[test_only]
    public(package) fun set_epoch_for_testing(self: &mut MysSystemStateInnerV2, epoch_num: u64) {
    ...
    #[test_only]
    public(package) fun set_stake_subsidy_distribution_counter(self: &mut MysSystemStateInnerV2, counter: u64) {
        self.stake_subsidy.set_distribution_counter(counter)
    }

    #[test_only]
    public(package) fun epoch_duration_ms(self: &MysSystemStateInnerV2): u64 {
