...
module mys_system::validator {
    ...
    use mys::mys::MYS;
    use mys_system::validator_cap::{Self, ValidatorOperationCap};
    use mys_system::staking_pool::{Self, PoolTokenExchangeRate, StakedMys, StakingPool, FungibleStakedMys};
    ...
    public struct ValidatorMetadata has store {
        /// The Mys Address of the validator. This is the sender that created the Validator object,
    ...
    public struct ConvertingToFungibleStakedMysEvent has copy, drop {
        pool_id: ID,
        stake_activation_epoch: u64,
        staked_mys_principal_amount: u64,
        fungible_staked_mys_amount: u64,
    }

    /// Event emitted when a fungible staked MYS is redeemed.
    public struct RedeemingFungibleStakedMysEvent has copy, drop {
        pool_id: ID,
        fungible_staked_mys_amount: u64,
        mys_amount: u64,
    }
    ...
    public(package) fun request_add_stake(
        ...
        let staked_mys = self.staking_pool.request_add_stake(stake, stake_epoch, ctx);
        ...
        event::emit(
            StakingRequestEvent {
                ...
                validator_address: self.metadata.mys_address,
                staker_address,
                epoch: ctx.epoch(),
                amount: stake_amount,
            }
    ...
    public(package) fun convert_to_fungible_staked_mys(
        ...
        let stake_activation_epoch = staked_mys.stake_activation_epoch();
        let staked_mys_principal_amount = staked_mys.staked_mys_amount();

        let fungible_staked_mys = self.staking_pool.convert_to_fungible_staked_mys(staked_mys, ctx);

        event::emit(
            ConvertingToFungibleStakedMysEvent {
    ...
    public(package) fun redeem_fungible_staked_mys(
        ...
        let fungible_staked_mys_amount = fungible_staked_mys.value();

        let mys = self.staking_pool.redeem_fungible_staked_mys(fungible_staked_mys, ctx);

        self.next_epoch_stake = self.next_epoch_stake - mys.value();

        event::emit(
            RedeemingFungibleStakedMysEvent {
                pool_id: self.staking_pool_id(),
    ...
    public(package) fun request_add_stake_at_genesis(
        ...
        let staked_mys = self.staking_pool.request_add_stake(
            stake,
            0, // epoch 0 -- genesis
            ctx
        );
    ...
    public(package) fun request_withdraw_stake(
        ...
        let principal_amount = staked_mys.staked_mys_amount();
        let stake_activation_epoch = staked_mys.stake_activation_epoch();
        let withdrawn_stake = self.staking_pool.request_withdraw_stake(staked_mys, ctx);
    ...
    public(package) fun request_set_gas_price(
        ...
        assert!(validator_address == self.metadata.mys_address, EInvalidCap);
    ...
    public(package) fun deposit_stake_rewards(self: &mut Validator, reward: Balance<MYS>) {
    ...
    public fun mys_address(self: &Validator): address {
        self.metadata.mys_address
    }
    ...
    public fun is_duplicate(self: &Validator, other: &Validator): bool {
         self.metadata.mys_address == other.metadata.mys_address
    ...
    public(package) fun new_unverified_validator_operation_cap_and_transfer(self: &mut Validator, ctx: &mut TxContext) {
        ...
        assert!(address == self.metadata.mys_address, ENewCapNotCreatedByValidatorItself);
    ...
    fun new_from_metadata(
        ...
        let mys_address = metadata.mys_address;

        let staking_pool = staking_pool::new(ctx);

        let operation_cap_id = validator_cap::new_unverified_validator_operation_cap_and_transfer(mys_address, ctx);
        Validator {
            metadata,
    ...
    // Note: `proof_of_possession` MUST be a valid signature using mys_address and
    ...
    public(package) fun new_for_testing(
        ...
        primary_address: vector<u8>,
        worker_address: vector<u8>,
        mut initial_stake_option: Option<Balance<MYS>>,
        gas_price: u64,
        commission_rate: u64,
        ...
        if (initial_stake_option.is_some()) {
            request_add_stake_at_genesis(
                ...
                mys_address, // give the stake to the validator
                ctx
            );
        };
