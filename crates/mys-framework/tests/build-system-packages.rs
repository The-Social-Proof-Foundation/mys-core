...
fn check_diff(checked_in: &Path, built: &Path) {
    for path in [COMPILED_PACKAGES_DIR, DOCS_DIR, PUBLISHED_API_FILE] {
        ...
        if !output.status.success() {
            let header =
                "Generated and checked-in mys-framework packages and/or docs do not match.\n\
...
fn build_packages_with_move_config(
    ...
    let mys_system_members =
        serialize_modules_to_file(mys_system, &compiled_packages_dir.join(system_dir)).unwrap();
