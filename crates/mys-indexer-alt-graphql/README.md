...
`mys-indexer-alt`, as long as all the required tables are present (all
pipelines are enabled on the indexer). See the indexer's
[README](../mys-indexer-alt/README.md) for details on how to set it up.
...
```sh
cargo nextest run -p mys-indexer-alt-graphql
cargo nextest run -p mys-indexer-alt-reader
cargo nextest run -p mys-indexer-alt-e2e-tests -- graphql
```
...
`mys-indexer-alt`'s stores), the final test is an end-to-end test suite that
...
```sh
cargo nextest run -p mys-indexer-alt-graphql -- test_schema_sdl_export
cargo nextest run -p mys-indexer-alt-graphql --features staging -- test_schema_sdl_export
cargo insta review
```
...
```sh
cargo run --bin mys-indexer-alt-graphql -- generate-config > graphql_config.toml
```
...
```sh
cargo run --bin mys-indexer-alt-graphql -- rpc \
