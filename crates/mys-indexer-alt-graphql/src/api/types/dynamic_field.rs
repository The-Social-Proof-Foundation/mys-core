...
pub(crate) enum Error {
    ...
    Literal(#[from] mys_display::v2::FormatError),
...
impl DynamicField {
    ...
    async fn value(&self, ctx: &Context<'_>) -> Option<Result<DynamicFieldValue, RpcError>> {
        async {
            ...
            let address: MysAddress = bcs::from_bytes(&native.value_bytes)
...
impl DynamicField {
    ...
    pub(crate) async fn by_serialized_name(
        ...
        let field_id: MysAddress = derive_dynamic_field_id(parent, &type_, &bcs.0)?.into();
    ...
    pub(crate) async fn by_literal_name(
        ...
        #[async_trait]
        impl mys_display::v2::Store for NopStore {
            async fn object(
                &self,
                _: AccountAddress,
            ) -> anyhow::Result<Option<mys_display::v2::OwnedSlice>> {
        ...
        let root = mys_display::v2::OwnedSlice {
            layout: MoveTypeLayout::Bool,
            bytes: bcs::to_bytes(&false).unwrap(),
        };

        let parsed =
            mys_display::v2::Name::parse(limits, &literal).map_err(|e| bad_user_input(e.into()))?;

        let interpreter = mys_display::v2::Interpreter::new(root, NopStore);

        let value = match parsed.eval(&interpreter).await {
            Ok(Some(value)) => value,
            Ok(None) => return Err(bad_user_input(Error::FieldAccess)),
            Err(mys_display::v2::FormatError::Store(_)) => {
                return Err(bad_user_input(Error::StoreAccess));
            }
            Err(e) => return Err(bad_user_input(e.into())),
        };

        let field_id: MysAddress = match kind {
