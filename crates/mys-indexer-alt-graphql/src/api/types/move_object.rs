...
use mys_types::object::MoveObject as NativeMoveObject;
...
#[graphql(
    ...
    field(
        ...
        desc = "Whether this object can be transfered using the `TransferObjects` Programmable Transaction Command or `mys::transfer::public_transfer`.\n\nBoth these operations require the object to have both the `key` and `store` abilities.",
    ),
    field(
