...
use mys_indexer_alt_reader::cp_sequence_numbers::CpSequenceNumberKey;
use mys_indexer_alt_reader::epochs::CheckpointBoundedEpochStartKey;
use mys_indexer_alt_reader::epochs::EpochEndKey;
use mys_indexer_alt_reader::epochs::EpochStartKey;
use mys_indexer_alt_reader::pg_reader::PgReader;
use mys_indexer_alt_schema::cp_sequence_numbers::StoredCpSequenceNumbers;
use mys_indexer_alt_schema::epochs::StoredEpochEnd;
...
// Stub constants for system state inner types (not yet available in mys-types)
const MYS_SYSTEM_STATE_INNER_MODULE_NAME: &IdentStr = ident_str!("mys_system_state_inner");
const MYS_SYSTEM_STATE_INNER_V1_STRUCT_NAME: &IdentStr = ident_str!("MysSystemStateInnerV1");
const MYS_SYSTEM_STATE_INNER_V2_STRUCT_NAME: &IdentStr = ident_str!("MysSystemStateInnerV2");
...
/// Activity on Mys is partitioned in time, into epochs.
///
...
impl Epoch {
    ...
    async fn coin_deny_list(&self, ctx: &Context<'_>) -> Option<Result<Object, RpcError>> {
        async {
            ...
            Object::latest(ctx, scope, MYS_DENY_LIST_OBJECT_ID.into()).await
        }
        .await
        .transpose()
    }
...
impl Epoch {
    ...
    async fn system_state_impl(&self, ctx: &Context<'_>) -> Result<Option<MoveValue>, RpcError> {
        ...
        let struct_name = match start.system_state.first() {
            Some(0) => MYS_SYSTEM_STATE_INNER_V1_STRUCT_NAME,
            Some(1) => MYS_SYSTEM_STATE_INNER_V2_STRUCT_NAME,
            _ => {
                return Ok(None);
            }
        };
        ...
        let tag = TypeTag::Struct(Box::new(StructTag {
            ...
            module: MYS_SYSTEM_STATE_INNER_MODULE_NAME.to_owned(),
