...
mod tests {
    ...
    fn test_type_filter_intersection() {
        ...
        let mys = ObjectFilter {
            owner_kind: None,
            owner: None,
            type_: Some("0x2::coin::Coin<0x2::mys::MYS>".parse().unwrap()),
        };
        ...
        // intersection order.
        assert_eq!(coin.clone().intersect(mys.clone()).unwrap(), mys);
        assert_eq!(mys.clone().intersect(coin.clone()).unwrap(), mys);
    ...
    fn test_combined_intersection() {
        ...
        assert_eq!(a_coin.clone().intersect(a_mys.clone()).unwrap(), a_mys);
