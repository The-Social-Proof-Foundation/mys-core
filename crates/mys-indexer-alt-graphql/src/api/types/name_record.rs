...
use mys_name_service::Domain as NativeDomain;
use mys_name_service::NameRecord as NativeNameRecord;
...
async fn reverse_record(
    ...
    scope: Scope,
    address: NativeMysAddress,
) -> Result<Option<Field<NativeMysAddress, NativeDomain>>, RpcError> {
    let config: &NameServiceConfig = ctx.data()?;
