...
use mys_types::base_types::MysAddress as NativeAddress;
use mys_types::coin::COIN_METADATA_STRUCT_NAME;
use mys_types::coin::COIN_MODULE_NAME;
use mys_types::coin::CoinMetadata as NativeMetadata;
use mys_types::deny_list_v1::RegulatedCoinMetadata;
use mys_types::coin::TreasuryCap;

// Stub types for coin_registry functionality that doesn't exist in mys-types yet
mod coin_registry {
    ...
    impl Currency {
        pub fn derive_object_id(_coin_type: mys_types::TypeTag) -> Result<MysAddress, bcs::Error> {
            // Stub implementation - coin registry not available in mys-types
            // Return ZERO address for now until coin_registry is implemented
            Ok(MysAddress::ZERO)
        }
    }
}

use coin_registry::Currency as NativeCurrency;
use coin_registry::RegulatedState as NativeRegulated;
use coin_registry::SupplyState as NativeSupply;
...
impl CoinMetadata {
    ...
    /// Whether this object can be transfered using the `TransferObjects` Programmable Transaction Command or `mys::transfer::public_transfer`.
    ///
...
impl CoinMetadata {
    ...
    pub(crate) async fn by_coin_type(
        ...
        let currency_id: MysAddress = NativeCurrency::derive_object_id(coin_type.clone())
