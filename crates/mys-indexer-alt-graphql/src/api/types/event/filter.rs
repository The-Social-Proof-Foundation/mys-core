...
pub(crate) struct EventFilter {
    ...
    ///
    /// Generic types can be queried by either the generic type name, e.g. `0x2::coin::Coin`, or by the full type name, such as `0x2::coin::Coin<0x2::mys::MYS>`.
...
impl EventFilter {
    ...
    pub(crate) fn matches(&self, event: &NativeEvent) -> bool {
        if let Some(sender) = &self.sender
            && sender != &MysAddress::from(event.sender)
        {
            return false;
        }

        if let Some(module) = &self.module {
            if module.package() != MysAddress::from(event.package_id) {
                return false;
            }
        ...
        if let Some(type_) = &self.type_ {
            if type_.package() != MysAddress::from(event.type_.address) {
                return false;
            }
