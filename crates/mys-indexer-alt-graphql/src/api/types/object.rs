...
use mys_indexer_alt_reader::consistent_reader;
use mys_indexer_alt_reader::consistent_reader::ConsistentReader;
use mys_indexer_alt_reader::kv_loader::KvLoader;
use mys_indexer_alt_reader::object_versions::CheckpointBoundedObjectVersionKey;
use mys_indexer_alt_reader::object_versions::VersionBoundedObjectVersionKey;
use mys_indexer_alt_reader::object_versions::VersionedObjectVersionKey;
use mys_indexer_alt_reader::pg_reader::PgReader;
use mys_indexer_alt_schema::objects::StoredObjVersion;
use mys_indexer_alt_schema::schema::obj_versions;
use mys_pg_db::sql;
...
use mys_types::object::Object as NativeObject;
use mys_types::transaction::GenesisObject;
use tokio::join;
...
#[graphql(
    ...
    field(
        ...
        desc = "The MYS returned to the sponsor or sender of the transaction that modifies or deletes this object."
    ),
    field(
...
/// An Object on Mys is either a typed value (a Move Object) or a Package (modules containing functions and types).
///
/// Every object on Mys is identified by a unique address, and has a version number that increases with every modification. Objects also hold metadata detailing their current owner (who can sign for access to the object and whether that access can modify and/or delete the object), and the digest of the last transaction that modified the object.
#[Object]
impl Object {
    ...
    /// The Object's ID.
    pub(crate) async fn address(&self, ctx: &Context<'_>) -> Result<MysAddress, RpcError> {
    ...
    /// Fetch the total balance for coins with marker type `coinType` (e.g. `0x2::mys::MYS`), owned by this address.
    ///
    ...
    /// Fetch the total balances keyed by coin types (e.g. `0x2::mys::MYS`) owned by this address.
    ///
    ...
    /// The MYS returned to the sponsor or sender of the transaction that modifies or deletes this object.
...
impl Object {
    ...
    pub(crate) fn with_address(scope: Scope, address: NativeMysAddress) -> Self {
    ...
    fn from_stored_version(
        ...
        let address = Address::with_address(
            scope,
            NativeMysAddress::from_bytes(stored.object_id)
                .context("Failed to deserialize MysAddress")?,
        );
