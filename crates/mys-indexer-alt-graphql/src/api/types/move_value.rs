...
use mys_types::object::option_visitor as OV;
use mys_types::object::rpc_visitor as RV;
use tokio::join;
...
pub(crate) enum Error {
    ...
    Format(mys_display::v2::FormatError),

    #[error("Path error: {0}")]
    Path(mys_display::v2::FormatError),
...
impl MoveValue {
    ...
    async fn display(&self, ctx: &Context<'_>) -> Option<Result<Display, RpcError>> {
        async {
            ...
            for (field, value) in
                mys_display::v1::Format::parse(limits.max_display_field_depth, &event.fields)
    ...
    async fn extract(
        ...
        async {
            let limits: &Limits = ctx.data()?;
            let extract = mys_display::v2::Extract::parse(limits.display(), &path)
            ...
            let interpreter = mys_display::v2::Interpreter::new(root, store);
            ...
            let Some(mys_display::v2::OwnedSlice {
                layout,
                bytes: native,
    ...
    async fn format(
        ...
        async {
            let limits: &Limits = ctx.data()?;
            let parsed = mys_display::v2::Format::parse(limits.display(), &format)
...
#[async_trait]
impl<'f, 'r> mys_display::v2::Store for DisplayStore<'f, 'r> {
    async fn object(
        &self,
        id: AccountAddress,
        ...
        Ok(Some(mys_display::v2::OwnedSlice { layout, bytes }))
    }
}
...
fn format_error(
    wrap: impl FnOnce(mys_display::v2::FormatError) -> Error,
    e: mys_display::v2::FormatError,
) -> RpcError<Error> {
    use mys_display::v2::FormatError as FE;
    match &e {
