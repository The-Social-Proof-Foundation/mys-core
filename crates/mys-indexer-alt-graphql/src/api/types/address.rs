...
#[graphql(
    name = "IAddressable",
    field(name = "address", ty = "MysAddress"),
    field(
        name = "address_at",
    ...
    field(
        ...
        desc = "Fetch the total balance for coins with marker type `coinType` (e.g. `0x2::mys::MYS`), owned by this address.\n\nIf the address does not own any coins of that type, a balance of zero is returned.",
    ),
    field(
        name = "balances",
...
pub(crate) struct Address {
    ...
    pub(crate) address: NativeMysAddress,
}
...
pub(crate) struct AddressKey {
    /// The address.
    pub(crate) address: Option<MysAddress>,

    /// A MysNS name to resolve to an address.
...
impl Address {
    ...
    pub(crate) async fn address(&self) -> Result<MysAddress, RpcError> {
...
impl Address {
    ...
    ///
    /// If a name is provided, it is resolved to an address using MysNS. If the name does not
    ...
    /// Construct an address that is represented by just its identifier (`MysAddress`).
