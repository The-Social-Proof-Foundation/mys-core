...
// Stub constants for validator types (not yet exported from mys-types)
const VALIDATOR_MODULE_NAME: &IdentStr = ident_str!("validator");
const VALIDATOR_STRUCT_NAME: &IdentStr = ident_str!("Validator");
...
impl ValidatorSetContents {
    ...
    fn deserialize(
        ...
        type ReportRecords = BTreeMap<NativeMysAddress, Vec<NativeMysAddress>>;
        type AtRiskValidators = BTreeMap<NativeMysAddress, u64>;

        #[derive(Default)]
        struct Contents<'b, 'l> {
            native: Option<(&'b [u8], &'l A::MoveTypeLayout)>,
            active_validators: Vec<(NativeMysAddress, &'b [u8])>,
        ...
        impl AV::Visitor<'_, '_> for AddressVisitor {
            type Value = Option<NativeMysAddress>;
            type Error = Error;
