...
use mys_types::parse_mys_address;
use mys_types::parse_mys_module_id;
...
impl TypeFilter {
    ...
    pub(crate) fn package(&self) -> MysAddress {
        match self {
...
impl FromStr for TypeFilter {
    ...
    fn from_str(s: &str) -> Result<Self, TypeFilterError> {
        ...
        } else if let Ok(module) = parse_mys_module_id(s) {
...
mod tests {
    ...
    fn test_parse_type_filter() {
        ...
        let filter = TypeFilter::from_str("0x2::coin::Coin<0x2::mys::MYS>").unwrap();
        assert!(matches!(filter, TypeFilter::Type(ref t) if !t.type_params.is_empty()));
    }
    ...
    fn test_type_filter_intersect() {
        ...
        let type_with_params = TypeFilter::from_str("0x2::coin::Coin<0x2::mys::MYS>").unwrap();
