...
/// Persistent log of transactions paying out mys from the faucet, keyed by the coin serving the
...
mod tests {
    ...
    fn random_request(coin: ObjectRef) -> (MysAddress, TransactionData) {
        let gas_price = 1;
        let send = MysAddress::random_for_testing_only();
        let recv = MysAddress::random_for_testing_only();
        (
            recv,
