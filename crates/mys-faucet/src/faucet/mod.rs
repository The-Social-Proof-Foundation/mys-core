...
#[async_trait]
pub trait Faucet {
    /// Send `Coin<MYS>` of the specified amount to the recipient
    async fn send(
        &self,
        id: Uuid,
    ...
    /// Send `Coin<MYS>` of the specified amount to the recipient in a batch request
    async fn batch_send(
        &self,
        id: Uuid,
        recipient: MysAddress,
...
#[clap(
    name = "Mys Faucet",
    about = "Faucet for requesting test tokens on Mys",
