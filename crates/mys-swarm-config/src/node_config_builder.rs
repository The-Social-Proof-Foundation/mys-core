...
use mys_config::node::{default_zklogin_oauth_providers, RunWithRange};
use mys_config::p2p::{P2pConfig, SeedPeer, StateSyncConfig};
...
impl ValidatorConfigBuilder {
    ...
    pub fn build(
        ...
        genesis: mys_config::genesis::Genesis,
    ) -> NodeConfig {
        ...
        NodeConfig {
            ...
            network_key_pair: KeyPairWithPath::new(MysKeyPair::Ed25519(validator.network_key_pair)),
            account_key_pair: KeyPairWithPath::new(validator.account_key_pair),
            worker_key_pair: KeyPairWithPath::new(MysKeyPair::Ed25519(validator.worker_key_pair)),
            db_path,
            network_address,
            ...
            genesis: mys_config::node::Genesis::new(genesis),
            grpc_load_shed: None,
...
impl FullnodeConfigBuilder {
    ...
    pub fn with_network_key_pair(mut self, network_key_pair: Option<NetworkKeyPair>) -> Self {
        if let Some(network_key_pair) = network_key_pair {
            self.network_key_pair =
                Some(KeyPairWithPath::new(MysKeyPair::Ed25519(network_key_pair)));
        }
        self
    }
    ...
    pub fn build<R: rand::RngCore + rand::CryptoRng>(
        ...
        NodeConfig {
            ...
            worker_key_pair: KeyPairWithPath::new(MysKeyPair::Ed25519(
                validator_config.worker_key_pair,
            )),
            network_key_pair: self.network_key_pair.unwrap_or(KeyPairWithPath::new(
                MysKeyPair::Ed25519(validator_config.network_key_pair),
            )),
            db_path: self
                .db_path
            ...
            remove_deprecated_tables: false,
            enable_index_processing: default_enable_index_processing(),
            genesis: self.genesis.unwrap_or(mys_config::node::Genesis::new(
