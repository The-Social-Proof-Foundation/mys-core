...
module axelar::validators {
    ...
    #[test]
    /// Tests `ecrecover`, makes sure external signing process works with Mys ecrecover.
    ...
    fun test_ecrecover() {
        ...
        normalize_signature(&mut signature);
        let pubkey = ecdsa::secp256k1_ecrecover(&signature, &to_mys_signed(message), 0);

        assert!(pubkey == SIGNER, 0);
    }

    #[test]
    /// Tests "Mys Signed Message" prefix addition ecrecover.
    ...
    fun test_to_signed() {
        ...
        let pub_key = ecdsa::secp256k1_ecrecover(&signature, &to_mys_signed(message), 0);
