...
///
/// - When call approvals is sent to Mys, it targets an object and not a module;
/// - To support cross-chain messaging, a Channel object has to be created;
/// - Channel can be either owned or shared but not frozen;
/// - Module developer on the Mys side will have to implement a system to support messaging;
...
module axelar::gateway {
    ...
    use axelar::utils::to_mys_signed;
    ...
    /// For when approval chainId is not MYS.
    ...
    /// Emitted when a new message is sent from the MYS network.
    ...
    entry fun process_commands(
        ...
        let mut allow_operatorship_transfer = validate_proof(validators, to_mys_signed(*&data), proof);
    ...
    fun test_execute() {
        use mys::test_scenario::{Self as ts, ctx};
