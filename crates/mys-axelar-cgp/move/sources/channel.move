...
module axelar::channel {
    ...
    ///
    /// This struct is required on the Mys side to be the destination for the
    ...
    ///
    /// - Does not contain direct link to the state in Mys, as some functions
    /// might not take any specific data (eg allow users to create new objects).
    /// If specific object on Mys is targeted by this `Channel`, its reference
    /// should be implemented using the `data` field.
    ///
    /// - The funniest and extremely simple implementation would be a `Channel<ID>`
    /// since it actually contains the data required to point at the object in Mys.
