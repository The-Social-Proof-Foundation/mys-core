...
module axelar::utils {
    ...
    /// Prefix for Mys Messages.
    const PREFIX: vector<u8> = b"\x19Mys Signed Message:\n";

    /// Normalize last byte of the signature. Have it 1 or 0.
    /// See https://tech.mystenlabs.com/cryptography-in-mys-cross-chain-signature-verification/
    ...
    public fun to_mys_signed(bytes: vector<u8>): vector<u8> {
        let mut res = vector[];
