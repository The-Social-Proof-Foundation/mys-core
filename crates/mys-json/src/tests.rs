...
use mys_types::{parse_mys_type_tag, MOVE_STDLIB_ADDRESS};

use crate::ResolvedCallArg;

use super::{check_valid_homogeneous, HEX_PREFIX};
use super::{resolve_move_function_args, MysJsonValue};
...
fn test_json_struct_homogeneous() {
    ...
    assert!(MysJsonValue::new(positive).is_ok());

    let negative = json!({"inner_vec":[1, 2, 3, true, 5, 6, 7]});
    assert!(MysJsonValue::new(negative).is_err());
}

#[test]
fn test_json_is_not_valid_mys_json() {
    let checks = vec![
        // Not homogeneous
        (json!([1, 2, 3, true, 5, 6, 7])),
        // Not homogeneous
    ...
    // Driver
    for arg in checks {
        assert!(MysJsonValue::new(arg).is_err());
    }
}

#[test]
fn test_json_is_valid_mys_json() {
    let checks = vec![
        // Homogeneous
        (json!([1, 2, 3, 4, 5, 6, 7])),
        // String allowed
    ...
    // Driver
    for arg in checks {
        assert!(MysJsonValue::new(arg).is_ok());
    }
}
...
fn test_basic_args_linter_top_level() {
    ...
    let recipient_addr = MysAddress::random_for_testing_only();
    ...
    .into_iter()
    .map(|q| MysJsonValue::new(q.clone()).unwrap())
    .collect();
...
fn test_convert_address_from_bcs() {
    ...
    let value = MysJsonValue::from_bcs_bytes(Some(&MoveTypeLayout::Signer), &bcs_bytes).unwrap();
...
fn test_convert_number_from_bcs() {
    ...
    let value = MysJsonValue::from_bcs_bytes(Some(&MoveTypeLayout::U32), &bcs_bytes).unwrap();
...
fn test_no_address_zero_trimming() {
    ...
    .unwrap();
    let value = MysJsonValue::from_bcs_bytes(Some(&MoveTypeLayout::Address), &bcs_bytes).unwrap();
    assert_eq!(
        "0x0000000000000000000000000000011111111111111111111111111111111111",
        value.0.as_str().unwrap()
    );
...
fn test_convert_number_array_from_bcs() {
    ...
    let value = MysJsonValue::from_bcs_bytes(
...
fn test_from_str() {
    // test number
    let test = MysJsonValue::from_str("10000").unwrap();
    assert!(test.0.is_number());
    // Test array
    let test = MysJsonValue::from_str("[10,10,10,10]").unwrap();
    ...
    // test bool
    let test = MysJsonValue::from_str("true").unwrap();
    assert!(test.0.is_boolean());

    // test id without quotes
    let object_id = ObjectID::random().to_hex_uncompressed();
    let test = MysJsonValue::from_str(&object_id).unwrap();
    assert!(test.0.is_string());
    assert_eq!(object_id, test.0.as_str().unwrap());

    // test id with quotes
    let test = MysJsonValue::from_str(&format!("\"{}\"", &object_id)).unwrap();
    assert!(test.0.is_string());
    assert_eq!(object_id, test.0.as_str().unwrap());

    // test string without quotes
    let test = MysJsonValue::from_str("Some string").unwrap();
    assert!(test.0.is_string());
    ...
    let test = MysJsonValue::from_object_id(
...
#[test]
fn test_mys_call_arg_string_type() {
    ...
    let v = MysJsonValue::from_bcs_bytes(string_layout.as_ref(), &arg1).unwrap();

    assert_eq!(json! {"Some String"}, v.to_json_value());
}

#[test]
fn test_mys_call_arg_option_type() {
    ...
    let v = MysJsonValue::from_bcs_bytes(Some(option_layout).as_ref(), &arg1).unwrap();
    ...
    let s = MysJsonValue::from_str("[test, test2]").unwrap();
    println!("{s:?}");
}
...
fn test_convert_struct() {
    ...
    let mys_json = MysJsonValue::new(value).unwrap();

    println!("JS: {:#?}", mys_json);

    let bcs = mys_json.to_bcs_bytes(&layout).unwrap();
...
fn test_convert_string_vec() {
    ...
    let bcs2 = mys_json.to_bcs_bytes(&layout).unwrap();
...
fn test_string_vec_df_name_child_id_eq() {
    ...
    let mys_json = MysJsonValue::new(name).unwrap();
    let bcs2 = mys_json.to_bcs_bytes(&layout).unwrap();
