...
use mys_types::id::{self, ID, RESOLVED_MYS_ID};
use mys_types::move_package::MovePackage;
use mys_types::object::bounded_visitor::BoundedVisitor;
use mys_types::transfer::RESOLVED_RECEIVING_STRUCT;
use mys_types::MOVE_STDLIB_ADDRESS;
...
pub enum MysJsonValueErrorKind {
...
#[derive(Debug)]
pub struct MysJsonValueError {
    kind: MysJsonValueErrorKind,
    val: JsonValue,
}

impl MysJsonValueError {
    pub fn new(val: &JsonValue, kind: MysJsonValueErrorKind) -> Self {
        Self {
            kind,
            val: val.clone(),
        }
...
impl std::error::Error for MysJsonValueError {}

impl fmt::Display for MysJsonValueError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        let err_str = match self.kind {
            MysJsonValueErrorKind::ValueTypeNotAllowed => {
                format!("JSON value type {} not allowed.", self.val)
            }
            MysJsonValueErrorKind::ArrayNotHomogeneous => {
...
pub struct MysJsonValue(JsonValue);
impl MysJsonValue {
    pub fn new(json_value: JsonValue) -> Result<MysJsonValue, anyhow::Error> {
    ...
    pub fn from_object_id(id: ObjectID) -> MysJsonValue {
    ...
    pub fn to_mys_address(&self) -> anyhow::Result<MysAddress> {
        json_value_to_mys_address(&self.0)
    }
    ...
    pub fn to_move_value(
        ...
        Ok(match (val, ty) {
                ...
                if struct_layout.type_ == ID::type_() =>
            {
                ...
                let addr = MysAddress::from_str(s)?;
            ...
            (v, MoveTypeLayout::Address) => {
                let addr = json_value_to_mys_address(v)?;
                R::MoveValue::Address(addr.into())
            }
...
impl Debug for MysJsonValue {
    fn fmt(&self, f: &mut Formatter<'_>) -> std::fmt::Result {
        write!(f, "{}", self.0)
    }
}

fn json_value_to_mys_address(value: &JsonValue) -> anyhow::Result<MysAddress> {
    match value {
        JsonValue::String(s) => {
        ...
        JsonValue::Array(bytes) => {
            ...
            let bytes = value_to_byte_array(bytes)
                .ok_or_else(|| anyhow!("Invalid input: Cannot parse input into MysAddress."))?;
            Ok(MysAddress::try_from(bytes)?)
        }
...
fn move_value_to_json(move_value: &MoveValue) -> Option<JsonValue> {
    Some(match move_value {
        ...
        MoveValue::Signer(v) | MoveValue::Address(v) => json!(MysAddress::from(*v).to_string()),
...
impl FromStr for MysJsonValue {
    ...
    fn from_str(s: &str) -> Result<Self, anyhow::Error> {
        ...
        MysJsonValue::new(serde_json::from_str(s).unwrap_or_else(|_| try_escape_array(s)))
    }
}
...
pub fn check_valid_homogeneous(val: &JsonValue) -> Result<(), MysJsonValueError> {
...
fn check_valid_homogeneous_rec(curr_q: &mut VecDeque<&JsonValue>) -> Result<(), MysJsonValueError> {
    if curr_q.is_empty() {
        // Nothing to do
        return Ok(());
    }
    ...
    while let Some(v) = curr_q.pop_front() {
        let curr = match v {
            ...
            // Not valid
            _ => {
                return Err(MysJsonValueError::new(
                    v,
                    MysJsonValueErrorKind::ValueTypeNotAllowed,
                ))
            }
...
pub fn primitive_type(
    ...
    Some(match param {
        ...
        SignatureToken::Datatype(struct_handle_idx) => {
            ...
            } else if resolved_struct == RESOLVED_MYS_ID {
...
fn resolve_object_vec_arg(idx: usize, arg: &MysJsonValue) -> Result<Vec<ObjectID>, anyhow::Error> {
    // Every elem has to be a string convertible to a ObjectID
...
pub fn resolve_move_function_args(
    ...
    function: Identifier,
    type_args: &[TypeTag],
    combined_args_json: Vec<MysJsonValue>,
...
macro_rules! call_args {
        ($($value:expr),*) => {
        Ok::<_, anyhow::Error>(vec![$(mys_json::call_arg!($value)?,)*])
    };
    }
...
macro_rules! call_arg {
    ($value:expr) => {{
        ...
        trait MysJsonArg {
            fn to_mys_json(&self) -> anyhow::Result<MysJsonValue>;
        }
        // TODO: anyway to condense this?
        impl MysJsonArg for &str {
            fn to_mys_json(&self) -> anyhow::Result<MysJsonValue> {
                MysJsonValue::from_str(self)
            }
        }
        impl MysJsonArg for String {
            fn to_mys_json(&self) -> anyhow::Result<MysJsonValue> {
                MysJsonValue::from_str(&self)
            }
        }
        impl MysJsonArg for mys_types::base_types::ObjectID {
            fn to_mys_json(&self) -> anyhow::Result<MysJsonValue> {
                MysJsonValue::from_str(&self.to_string())
            }
        }
        impl MysJsonArg for mys_types::base_types::MysAddress {
            fn to_mys_json(&self) -> anyhow::Result<MysJsonValue> {
                MysJsonValue::from_str(&self.to_string())
            }
        }
        impl MysJsonArg for u64 {
            fn to_mys_json(&self) -> anyhow::Result<MysJsonValue> {
                MysJsonValue::from_bcs_bytes(
                    Some(&mys_json::MoveTypeLayout::U64),
                    &bcs::to_bytes(self)?,
                )
            }
        }
        impl MysJsonArg for Vec<u8> {
            fn to_mys_json(&self) -> anyhow::Result<MysJsonValue> {
                MysJsonValue::from_bcs_bytes(None, &self)
            }
        }
        impl MysJsonArg for &[u8] {
            fn to_mys_json(&self) -> anyhow::Result<MysJsonValue> {
                MysJsonValue::from_bcs_bytes(None, self)
            }
        }
...
macro_rules! type_args {
    ($($value:expr), *) => {{
        use mys_json_rpc_types::MysTypeTag;
        use mys_types::TypeTag;
        trait MysJsonTypeArg {
            fn to_mys_json(&self) -> anyhow::Result<MysTypeTag>;
        }
        impl <T: core::fmt::Display> MysJsonTypeArg for T {
            fn to_mys_json(&self) -> anyhow::Result<MysTypeTag> {
                Ok(mys_types::parse_mys_type_tag(&self.to_string())?.into())
            }
        }
        Ok::<_, anyhow::Error>(vec![$($value.to_mys_json()?,)*])
    }};
    }
