...
pub struct RpcCommandProcessor {
    clients: Arc<RwLock<Vec<MysClient>>>,
    ...
    addresses: Arc<DashSet<MysAddress>>,
    data_dir: String,
}
...
impl RpcCommandProcessor {
    ...
    async fn process_command_data(
        ...
        match command {
            ...
            CommandData::PayMys(ref v) => self.process(v, signer_info).await,
    ...
    pub(crate) async fn get_clients(&self) -> Result<Vec<MysClient>> {
    ...
    pub(crate) async fn get_object_ref(
        ...
        match current {
            ...
            None => {
                let resp = client
                    .read_api()
                    .get_object_with_options(*object_id, MysObjectDataOptions::new())
                    .await
    ...
    pub(crate) fn add_addresses_from_response(&self, responses: &[MysTransactionBlockResponse]) {
    ...
    pub(crate) fn add_object_ids_from_response(&self, responses: &[MysTransactionBlockResponse]) {
        for response in responses {
    ...
    pub(crate) fn dump_cache_to_file(&self) {
        ...
        let addresses: Vec<MysAddress> = self.addresses.iter().map(|x| *x).collect();
        if !addresses.is_empty() {
            debug!("dumping addresses to file {:?}", addresses.len());
            write_data_to_file(
                &addresses,
                &format!("{}/{}", &self.data_dir, CacheType::MysAddress),
            )
            .unwrap();
        }
...
impl fmt::Display for CacheType {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            CacheType::MysAddress => write!(f, "MysAddress"),
...
pub fn load_addresses_from_file(filepath: String) -> Vec<MysAddress> {
    let path = format!("{}/{}", filepath, CacheType::MysAddress);
    let addresses: Vec<MysAddress> = read_data_from_file(&path).expect("Failed to read addresses");
    addresses
}
...
async fn prepare_new_signer_and_coins(
    ...
    let (gas_fee_for_split, gas_fee_for_pay_mys) = (
        DEFAULT_LARGE_GAS_BUDGET * num_split_txns as u64,
        DEFAULT_GAS_BUDGET,
    );

    let primary_keypair = MysKeyPair::decode_base64(&signer_info.encoded_keypair)
        .expect("Decoding keypair should not fail");
    let sender = MysAddress::from(&primary_keypair.public());
    let (coin, balance) = get_coin_with_max_balance(client, sender).await;
    // The balance needs to cover `pay_amount` plus
    // 1. gas fee for pay_mys from the primary address to the burner address
    // 2. gas fee for splitting the primary coin into `num_coins`
    let required_balance = pay_amount + gas_fee_for_split + gas_fee_for_pay_mys;
    ...
    let burner_keypair = MysKeyPair::Ed25519(burner_keypair);
    ...
    let coins = get_mys_coin_ids(client, burner_address).await;
...
async fn get_coin_with_max_balance(client: &MysClient, address: MysAddress) -> (ObjectID, u64) {
    let coins = get_mys_coin_ids(client, address).await;
    assert!(!coins.is_empty());
...
async fn get_mys_coin_ids(client: &MysClient, address: MysAddress) -> Vec<(ObjectID, u64)> {
    match client
        .coin_read_api()
        ...
        .await
    {
        Ok(page) => page
            ...
            .collect::<Vec<_>>(),
        Err(e) => {
            panic!("get_mys_coin_ids error for address {address} {e}")
        }
    }
...
async fn pay_mys(
    ...
    let sender = MysAddress::from(&keypair.public());
    let tx = client
        .transaction_builder()
        .pay(sender, input_coins, recipients, amounts, None, gas_budget)
        .await
        .expect("Failed to construct pay mys transaction");
    sign_and_execute(
        client,
        keypair,
        tx,
...
pub(crate) async fn sign_and_execute(
    ...
    let signature = Signature::new_secure(
        &IntentMessage::new(Intent::mys_transaction(), &txn_data),
        keypair,
    );
