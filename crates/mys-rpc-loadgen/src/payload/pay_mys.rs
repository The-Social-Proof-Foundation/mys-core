...
use crate::payload::{PayMys, ProcessPayload, RpcCommandProcessor, SignerInfo};
...
#[async_trait]
impl<'a> ProcessPayload<'a, &'a PayMys> for RpcCommandProcessor {
    async fn process(
        &'a self,
        _op: &'a PayMys,
        ...
        let keypair =
            MysKeyPair::decode_base64(&encoded_keypair).expect("Decoding keypair should not fail");

        debug!(
            "Transfer Mys {} time to {recipient} with {amount} MIST with {gas_payments:?}",
            gas_payments.len()
        );
        ...
        join_all(gas_payments.iter().map(|gas| async {
            let tx = TransactionData::new_transfer_mys(
                recipient,
                sender,
                Some(amount),
