...
pub(crate) async fn check_transactions(
    ...
    cross_validate: bool,
    verify_objects: bool,
) -> Vec<Vec<MysTransactionBlockResponse>> {
    let transactions: Vec<Vec<MysTransactionBlockResponse>> =
        join_all(clients.iter().map(|client| async move {
            client
                .read_api()
                .multi_get_transactions_with_options(
                    digests.to_vec(),
                    MysTransactionBlockResponseOptions::full_content(), // todo(Will) support options for this
                )
                .await
        }))
        .await
...
pub(crate) fn get_all_object_ids(response: &MysTransactionBlockResponse) -> Vec<ObjectID> {
...
pub(crate) async fn multi_get_object(
    clients: &[MysClient],
    object_ids: &[ObjectID],
) -> Vec<Vec<MysObjectResponse>> {
    let objects: Vec<Vec<MysObjectResponse>> = join_all(clients.iter().map(|client| async move {
        ...
        client
            ...
            .multi_get_object_with_options(
                object_ids.to_vec(),
                MysObjectDataOptions::full_content(), // todo(Will) support options for this
            )
            .await
    }))
    .await
