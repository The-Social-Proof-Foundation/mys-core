...
impl<'a> ProcessPayload<'a, &'a QueryTransactionBlocks> for RpcCommandProcessor {
    async fn process(
        ...
        let queries: Vec<MysTransactionBlockResponseQuery> = filters
            .into_iter()
            .map(|filter| MysTransactionBlockResponseQuery {
                filter,
...
) -> Result<Page<MysTransactionBlockResponse, TransactionDigest>> {
