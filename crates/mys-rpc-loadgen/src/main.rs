...
#[derive(Parser)]
#[clap(
    name = "Mys RPC Load Generator",
    version = "0.1",
    about = "A load test application for Mys RPC"
)]
struct Opts {
    ...
    #[clap(long, default_value = "~/.mys/mys_config/logs")]
    logs_directory: String,

    #[clap(long, default_value = "~/.mys/loadgen/data")]
...
fn get_keypair() -> Result<SignerInfo> {
    ...
    // Load keystore from ~/.mys/mys_config/mys.keystore
    let keystore_path = get_mys_config_directory().join("mys.keystore");
    let keystore = Keystore::from(FileBasedKeystore::new(&keystore_path)?);
    let active_address = keystore.addresses().pop().unwrap();
    let keypair: &MysKeyPair = keystore.get_key(&active_address)?;
    println!("using address {active_address} for signing");
    Ok(SignerInfo::new(keypair.encode_base64()))
}

fn get_mys_config_directory() -> PathBuf {
    match dirs::home_dir() {
        Some(v) => v.join(".mys").join("mys_config"),
...
fn get_log_file_path(dir_path: String) -> String {
    ...
    let log_filename = format!("mys-rpc-loadgen.{}.log", timestamp);
...
async fn main() -> Result<(), Box<dyn Error>> {
    ...
    let (command, common, need_keystore) = match opts.command {
        ...
        ClapCommand::PayMys { common } => (Command::new_pay_mys(), common, true),
