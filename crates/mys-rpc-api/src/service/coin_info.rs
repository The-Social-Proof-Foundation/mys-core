...
use mys_sdk_types::{ObjectId, StructTag};
use mys_types::mys_sdk_types_conversions::struct_tag_sdk_to_core;

const MYS_COIN_TREASURY: CoinTreasury = CoinTreasury {
    id: None,
    total_supply: Some(mys_types::gas_coin::TOTAL_SUPPLY_MIST),
};
...
impl RpcService {
    pub fn get_coin_info(&self, request: GetCoinInfoRequest) -> Result<GetCoinInfoResponse> {
        ...
        let mys_types::storage::CoinInfo {
        ...
        let treasury = if let Some(treasury_object_id) = treasury_object_id {
            ...
            .map(mys_types::coin::TreasuryCap::try_from)
            .transpose()
            .map_err(|_| {
                RpcError::new(
                    tonic::Code::Internal,
