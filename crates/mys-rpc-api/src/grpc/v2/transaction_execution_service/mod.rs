...
use mys_rpc::proto::mys::rpc::v2::ExecuteTransactionResponse;
...
use mys_rpc::proto::mys::rpc::v2::transaction_execution_service_server::TransactionExecutionService;
...
pub async fn execute_transaction(
    ...
    let transaction = request
        ...
        .pipe(mys_sdk_types::Transaction::try_from)
        .map_err(|e| {
    ...
    let signatures = request
        ...
        .enumerate()
        .map(|(i, signature)| {
            mys_sdk_types::UserSignature::try_from(signature).map_err(|e| {
    ...
    let signed_transaction = mys_sdk_types::SignedTransaction {
    ...
    let request = mys_types::transaction_driver_types::ExecuteTransactionRequestV3 {
    ...
    let mys_types::quorum_driver_types::ExecuteTransactionResponseV3 {
        effects:
            mys_types::quorum_driver_types::FinalizedEffects {
                effects,
                finality_info: _,
            },
        events,
    ...
    let executed_transaction = {
        ...
        let objects = {
            let mut objects = mys_types::full_checkpoint_content::ObjectSet::default();
        ...
        let balance_changes = if read_mask.contains(ExecutedTransaction::BALANCE_CHANGES_FIELD) {
            derive_balance_changes_2(&effects, &objects)
                .into_iter()
                .map(crate::proto::types::balance::from_core_balance_change)
                .collect()
        } else {
            vec![]
        };
