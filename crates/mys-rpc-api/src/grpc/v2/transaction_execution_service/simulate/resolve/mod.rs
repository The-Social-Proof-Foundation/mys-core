...
use mys_rpc::proto::mys::rpc::v2::Transaction;
...
use mys_types::transaction::FundsWithdrawalArg;
use mys_types::transaction::GasData;
use mys_types::transaction::ObjectArg;
use mys_types::transaction::ProgrammableTransaction;
use mys_types::transaction::Reservation;
use mys_types::transaction::TransactionData;
use mys_types::transaction::WithdrawFrom;
use mys_types::transaction::WithdrawalTypeArg;
use mys_types::execution_config_utils;
use tap::Pipe;
...
pub fn resolve_transaction(
    ...
    let commands = ptb
        .commands
        .iter()
        .map(mys_sdk_types::Command::try_from)
...
pub(super) fn called_packages(
    ...
    let binary_config = execution_config_utils::to_binary_config(protocol_config);
    let pool = normalized::RcPool::new();
    ...
    for move_call in commands.iter().filter_map(|command| {
        ...
        let package = reader
            ...
            .ok_or_else(|| ObjectNotFoundError::new(move_call.package.into()))?
            .data
            .try_as_package()
            .ok_or_else(|| {
                RpcError::new(
        ...
        packages.insert(move_call.package.into(), package);
    }
...
fn resolve_unresolved_transaction(
    ...
    reference_gas_price: u64,
    max_gas_budget: u64,
    sender: mys_sdk_types::Address,
    unresolved_inputs: &[mys_rpc::proto::mys::rpc::v2::Input],
    commands: Vec<Command>,
    gas_payment: Option<&mys_rpc::proto::mys::rpc::v2::GasPayment>,
    expiration: Option<&mys_rpc::proto::mys::rpc::v2::TransactionExpiration>,
) -> Result<TransactionData> {
    ...
    let expiration = expiration
        .map(crate::proto::types::transaction_convert::try_from_proto_transaction_expiration)
        .transpose()
        .map_err(|e| {
        ...
        .unwrap_or(mys_types::transaction::TransactionExpiration::None);
    let ptb = resolve_ptb(reader, called_packages, unresolved_inputs, commands)?;
    Ok(TransactionData::V1(
        mys_types::transaction::TransactionDataV1 {
            kind: mys_types::transaction::TransactionKind::ProgrammableTransaction(ptb),
            sender: sender.into(),
            gas_data,
            expiration,
        },
...
fn resolve_gas_object_reference(
    ...
    let Ok(gas_coin) = mys_types::gas_coin::GasCoin::try_from(&object) else {
...
fn resolve_object_reference_with_object(
    ...
    match object.owner() {
        mys_types::object::Owner::AddressOwner(_) | mys_types::object::Owner::Immutable => {}
        _ => {
...
fn resolve_arg(
    ...
    called_packages: &mut NormalizedPackages,
    commands: &[Command],
    arg: &mys_rpc::proto::mys::rpc::v2::Input,
    arg_idx: usize,
) -> Result<CallArg> {
...
fn resolve_object(
    ...
    arg_idx: usize,
    object_id: Address,
    version: Option<mys_sdk_types::Version>,
    digest: Option<mys_sdk_types::Digest>,
    ...
    match object.owner() {
        mys_types::object::Owner::Immutable => resolve_object_reference_with_object(
            &object,
            UnresolvedObjectReference {
                object_id,
                version,
        ...
        mys_types::object::Owner::AddressOwner(_) => {
...
fn is_input_argument_receiving(
    ...
    let (receiving_package, receiving_module, receiving_struct) =
        mys_types::transfer::RESOLVED_RECEIVING_STRUCT;
    ...
    for (command, idx) in find_arg_uses(arg_idx, commands) {
        if let (Command::MoveCall(move_call), Some(idx)) = (command, idx) {
            ...
            let inner_type = match &*arg_type {
                normalized::Type::Reference(inner) | normalized::Type::MutableReference(inner) => inner,
                _ => &*arg_type,
            };

            if let normalized::Type::Struct { address, module, name, type_arguments: _ } = inner_type
                && receiving_package == address
...
fn arg_type_of_move_call_input(
    called_packages: &NormalizedPackages,
    move_call: &mys_sdk_types::MoveCall,
    idx: usize,
...
fn resolve_shared_input_with_object(
    ...
    for (command, idx) in find_arg_uses(arg_idx, commands) {
        match (command, idx) {
            (Command::MoveCall(move_call), Some(idx)) => {
                ...
                if matches!(
                    &*arg_type,
                    normalized::Type::MutableReference(_)
    ...
    Ok(ObjectArg::SharedObject {
        ...
        mutability: if mutable {
            mys_types::transaction::SharedObjectMutability::Mutable
        } else {
            mys_types::transaction::SharedObjectMutability::Immutable
        },
    })
}
...
impl TryFrom<&mys_rpc::proto::mys::rpc::v2::ObjectReference> for UnresolvedObjectReference {
    type Error = FieldViolation;

    fn try_from(
        value: &mys_rpc::proto::mys::rpc::v2::ObjectReference,
...
struct UnresolvedInput<'a> {
    pub kind: Option<mys_rpc::proto::mys::rpc::v2::input::InputKind>,
    pub pure: Option<&'a Bytes>,
    pub object_id: Option<mys_sdk_types::Address>,
    pub version: Option<mys_sdk_types::Version>,
    pub digest: Option<mys_sdk_types::Digest>,
...
impl<'a> UnresolvedInput<'a> {
    fn from_proto(input: &'a mys_rpc::proto::mys::rpc::v2::Input) -> Result<Self, FieldViolation> {
        Ok(Self {
                ...
                .map(|w| {
                    Ok(FundsWithdrawalArg {
                        ...
                        type_arg: WithdrawalTypeArg::Balance(
                            w.coin_type().parse::<mys_types::TypeTag>().map_err(|e| {
                        ...
                        withdraw_from: match w.source() {
                            mys_rpc::proto::mys::rpc::v2::funds_withdrawal::Source::Sender => {
                                WithdrawFrom::Sender
                            }
                            mys_rpc::proto::mys::rpc::v2::funds_withdrawal::Source::Sponsor => {
