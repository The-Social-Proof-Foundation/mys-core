...
use mys_rpc::proto::mys::rpc::v2::CommandOutput;
use mys_rpc::proto::mys::rpc::v2::CommandResult;
use mys_rpc::proto::mys::rpc::v2::ExecutedTransaction;
use mys_rpc::proto::mys::rpc::v2::ObjectSet;
...
use mys_types::balance_change::derive_balance_changes_2;
...
pub fn simulate_transaction(
    ...
    let checks = crate::proto::types::transaction_convert::from_proto_transaction_checks(request.checks());
    ...
    let mut transaction = match mys_sdk_types::Transaction::try_from(transaction_proto) {
        Ok(sdk_transaction) => mys_types::transaction::TransactionData::try_from(sdk_transaction)
            .map_err(|e| RpcError::new(tonic::Code::Internal, format!("failed to convert transaction: {e}")))?,
    ...
    let transaction = if let Some(submask) = read_mask.subtree("transaction") {
        ...
        let td = mys_types::transaction::TransactionData::try_from(transaction.clone())
            .map_err(|e| RpcError::new(tonic::Code::Internal, format!("failed to convert transaction: {e}")))?;
        let sdk_transaction: mys_sdk_types::Transaction = mys_sdk_types::Transaction::try_from(td)
            .map_err(|e| RpcError::new(tonic::Code::Internal, format!("failed to convert transaction to SDK: {e}")))?;
            ...
            if submask.contains(ExecutedTransaction::BALANCE_CHANGES_FIELD.name) {
                derive_balance_changes_2(&effects, &objects)
                    ...
                    .map(|bc| {
                        ...
                        crate::proto::types::balance::from_core_balance_change(bc)
                    })
                    .collect()
            } else {
                vec![]
        ...
        message.transaction = submask
            ...
            .map(|mask| {
                let transaction_proto: Transaction = sdk_transaction.into();
                Transaction::merge_from(&transaction_proto, &mask)
            });
...
fn to_command_output(
    service: &RpcService,
    arg: Option<mys_types::transaction::Argument>,
    bcs: Vec<u8>,
    ty: mys_types::TypeTag,
) -> CommandOutput {
    let json = service
        .reader
        .inner()
        ...
        .flatten()
        .and_then(|layout| {
            mys_types::proto_value::ProtoVisitor::new(service.config.max_json_move_value_size())
    ...
    message.argument = arg.map(|a| {
        use crate::proto::types::transaction_convert::MysArgument;
        MysArgument::from(a).into()
    });
...
fn estimate_gas_budget_from_gas_cost(
    gas_cost_summary: &mys_types::gas::GasCostSummary,
...
fn select_gas(
    service: &RpcService,
    transaction: &mut mys_types::transaction::TransactionData,
    ...
    use mys_types::transaction::TransactionDataAPI;
    use mys_types::transaction::TransactionExpiration;
    ...
    let address_balance = reader
        ...
        .map(|balance| {
            // Sum up the total MYS reservations for the `owner` so that we can deduct that from the
            // available address balance for determining if an account as sufficient funds.
            let reserved_mys = transaction
                ...
                .filter_map(|w| {
                    ...
                    // Skip if this withdrawal isn't for MYS
            ...
            balance.saturating_sub(reserved_mys)
        });
        ...
        *transaction.expiration_mut() = TransactionExpiration::ValidDuring {
        ...
        let input_objects = transaction
            ...
            .flat_map(|obj| match obj {
                mys_types::transaction::InputObjectKind::ImmOrOwnedMoveObject((id, _, _)) => {
                    Some(*id)
                }
                _ => None,
            })
    ...
    if selected_gas_value >= budget {
        ...
        Err(RpcError::new(
            tonic::Code::InvalidArgument,
            format!(
                "Unable to perform gas selection due to insufficient MYS \
