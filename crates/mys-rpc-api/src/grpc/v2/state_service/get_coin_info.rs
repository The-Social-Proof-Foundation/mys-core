...
use mys_rpc::proto::mys::rpc::v2::CoinTreasury;
use mys_rpc::proto::mys::rpc::v2::GetCoinInfoRequest;
use mys_rpc::proto::mys::rpc::v2::GetCoinInfoResponse;
use mys_rpc::proto::mys::rpc::v2::coin_treasury::SupplyState as RpcSupplyState;
use mys_sdk_types::{Address, StructTag};
use mys_types::base_types::{ObjectID as MysObjectID, MysAddress};
use mys_types::coin::RegulatedCoinMetadata;
use mys_types::coin_registry::{Currency, RegulatedState as CurrencyRegulatedState, SupplyState};
use mys_types::object::Owner::AddressOwner;
use mys_types::object::Owner::ConsensusAddressOwner;
use mys_types::object::Owner::Immutable;
use mys_types::mys_sdk_types_conversions::struct_tag_sdk_to_core;
use crate::proto::types::coin::*;

const MYS_COIN_TREASURY: CoinTreasury = {
    let mut treasury = CoinTreasury::const_default();
    treasury.total_supply = Some(mys_types::gas_coin::TOTAL_SUPPLY_MIST);
...
fn get_coin_info_from_registry(
    ...
    let metadata = Some(crate::proto::types::coin::from_currency(&currency));

    let treasury = if mys_types::gas_coin::GAS::is_gas(core_coin_type) {
        Some(MYS_COIN_TREASURY)
    } else {
        match &currency.supply {
            Some(SupplyState::Fixed(supply)) => {
                ...
                treasury.id = currency
                    .treasury_cap_id
                    .map(crate::proto::types::coin::proto_object_id_from_mys);
    ...
    let regulated_metadata = if !matches!(&currency.regulated, CurrencyRegulatedState::Unknown) {
        Some(crate::proto::types::coin::from_regulated_state(currency.regulated.clone()))
    } else {
        ...
        let indexes = match service.reader.inner().indexes() {
            ...
            None => {
                ...
                response.regulated_metadata = Some(crate::proto::types::coin::from_regulated_state(CurrencyRegulatedState::Unknown));
        ...
        match regulated_id {
            None => {
                ...
                Some(crate::proto::types::coin::from_regulated_state(CurrencyRegulatedState::Unregulated))
            }
            Some(id) => object_store
                ...
                .map(crate::proto::types::coin::from_regulated_coin_metadata)
                .or(Some(crate::proto::types::coin::from_regulated_state(CurrencyRegulatedState::Unregulated))),
        }
    };
...
fn get_coin_info_from_index(
    service: &RpcService,
    indexes: &dyn mys_types::storage::RpcIndexes,
    coin_type: &StructTag,
    core_coin_type: &move_core_types::language_storage::StructTag,
) -> Result<Option<GetCoinInfoResponse>> {
    ...
    let metadata = if let Some(coin_metadata_object_id) = coin_info.coin_metadata_object_id {
        service
            ...
            .map(mys_types::coin::CoinMetadata::try_from)
            .transpose()
            .map_err(|_| {
                RpcError::new(
                    tonic::Code::Internal,
            ...
            .map(crate::proto::types::coin::from_coin_metadata)
    } else {
        None
    };
    ...
    } else if mys_types::gas_coin::GAS::is_gas(core_coin_type) {
        Some(MYS_COIN_TREASURY)
    } else {
        None
    };
...
fn get_treasury_cap_info(
    service: &RpcService,
    treasury_object_id: MysObjectID,
) -> Option<CoinTreasury> {
    ...
    let supply_state = match &obj.owner {
        ...
        AddressOwner(addr) if *addr == MysAddress::ZERO => RpcSupplyState::Fixed,
        ConsensusAddressOwner { owner, .. } if *owner == MysAddress::ZERO => RpcSupplyState::Fixed,
        _ => RpcSupplyState::Unknown,
    };

    mys_types::coin::TreasuryCap::try_from(obj)
        .ok()
        .map(|treasury| {
            let mut coin_treasury: CoinTreasury = crate::proto::types::coin::from_treasury_cap(treasury);
