...
use mys_rpc::proto::mys::rpc::v2::DynamicField;
use mys_rpc::proto::mys::rpc::v2::ErrorReason;
use mys_rpc::proto::mys::rpc::v2::ListDynamicFieldsRequest;
use mys_rpc::proto::mys::rpc::v2::ListDynamicFieldsResponse;
use mys_rpc::proto::mys::rpc::v2::dynamic_field::DynamicFieldKind;
...
pub fn list_dynamic_fields(
    ...
    while let Some(key_result) = iter.next() {
        let key = key_result.map_err(|e| {
            RpcError::new(
                tonic::Code::Internal,
                format!("error iterating dynamic fields: {e}"),
            )
        })?;
    ...
    let next_page_token = iter
        ...
        .map_err(|e| {
            RpcError::new(
                tonic::Code::Internal,
                format!("error getting next page token: {e}"),
            )
        })?
        .map(|cursor| {
...
fn load_dynamic_field(
    ...
    if read_mask.contains(DynamicField::NAME_FIELD) {
        message.name = Some(
            Bcs::default()
                .with_name(mys_types::TypeTag::from(field.name_layout).to_canonical_string(true))
    ...
    if read_mask.contains(DynamicField::VALUE_FIELD) {
        message.value = Some(
            Bcs::default()
                .with_name(mys_types::TypeTag::from(field.value_layout).to_canonical_string(true))
    ...
    match field.value_metadata()? {
        ...
        DFV::ValueMetadata::DynamicObjectField(object_id) => {
                ...
                || read_mask.contains(DynamicField::CHILD_OBJECT_FIELD)
            {
                ...
                let type_tag =
                    mys_types::TypeTag::from(object.struct_tag().ok_or_else(|| {
