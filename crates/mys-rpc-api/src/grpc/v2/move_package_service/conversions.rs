...
// This module contains conversion functions from mys-package-resolver types to their protobuf representations.
...
use mys_package_resolver::{DataDef, FunctionDef, MoveData, VariantDef};
...
pub(crate) fn convert_error(e: mys_package_resolver::error::Error) -> RpcError {
...
pub(crate) fn convert_module(
    module_name: &str,
    resolved_module: &mys_package_resolver::Module,
    package_id: &ObjectID,
) -> Result<Module> {
...
fn convert_open_signature(sig: &mys_package_resolver::OpenSignature) -> OpenSignature {
    let reference = sig.ref_.map(|r| match r {
        mys_package_resolver::Reference::Immutable => Reference::Immutable as i32,
        mys_package_resolver::Reference::Mutable => Reference::Mutable as i32,
    });
...
fn convert_open_signature_body(
    body: &mys_package_resolver::OpenSignatureBody,
) -> OpenSignatureBody {
    let (type_enum, type_name, type_params, type_param_idx) = match body {
        mys_package_resolver::OpenSignatureBody::Bool => (SignatureType::Bool, None, vec![], None),
        mys_package_resolver::OpenSignatureBody::U8 => (SignatureType::U8, None, vec![], None),
        mys_package_resolver::OpenSignatureBody::U16 => (SignatureType::U16, None, vec![], None),
        mys_package_resolver::OpenSignatureBody::U32 => (SignatureType::U32, None, vec![], None),
        mys_package_resolver::OpenSignatureBody::U64 => (SignatureType::U64, None, vec![], None),
        mys_package_resolver::OpenSignatureBody::U128 => (SignatureType::U128, None, vec![], None),
        mys_package_resolver::OpenSignatureBody::U256 => (SignatureType::U256, None, vec![], None),
        mys_package_resolver::OpenSignatureBody::Address => {
            (SignatureType::Address, None, vec![], None)
        }
        mys_package_resolver::OpenSignatureBody::Vector(inner) => {
            let inner_body = convert_open_signature_body(inner.as_ref());
            (SignatureType::Vector, None, vec![inner_body], None)
        }
        mys_package_resolver::OpenSignatureBody::Datatype(key, args) => {
        ...
        mys_package_resolver::OpenSignatureBody::TypeParameter(idx) => {
