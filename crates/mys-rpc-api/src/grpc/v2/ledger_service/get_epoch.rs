...
use mys_protocol_config::ProtocolConfigValue;
...
pub fn get_epoch(service: &RpcService, request: GetEpochRequest) -> Result<GetEpochResponse> {
    ...
    if let Some(system_state) = system_state {
        ...
        if read_mask.contains(Epoch::SYSTEM_STATE_FIELD) {
            let proto_system_state = crate::proto::types::mys_system_state_to_proto(system_state);
            message.system_state = Some(Box::new(proto_system_state));
        }
    }
...
fn get_protocol_config(
    version: u64,
    chain: mys_protocol_config::Chain,
) -> Result<ProtocolConfig, ProtocolVersionNotFoundError> {
    let config =
        mys_protocol_config::ProtocolConfig::get_for_version_if_supported(version.into(), chain)
            .ok_or_else(|| ProtocolVersionNotFoundError::new(version))?;
    Ok(protocol_config_to_proto(config))
}

pub fn protocol_config_to_proto(config: mys_protocol_config::ProtocolConfig) -> ProtocolConfig {
