...
use mys_rpc::proto::mys::rpc::v2::GetTransactionResult;
...
fn transaction_to_response(
    ...
    if mask.contains(ExecutedTransaction::BALANCE_CHANGES_FIELD.name) {
        message.balance_changes = source
            .balance_changes
            .map(|balance_changes| balance_changes.into_iter().map(crate::proto::types::balance::from_core_balance_change).collect())
            .unwrap_or_default();
    }
