...
/// Flag use to disambiguate the signature schemes supported by Mys.
///
...
/// Unique identifier for an account on the Mys blockchain.
///
/// An `Address` is a 32-byte pseudonymous identifier used to uniquely identify an account and
/// asset-ownership on the Mys blockchain. Often, human-readable addresses are encoded in
/// hexadecimal with a `0x` prefix. For example, this is a valid Mys address:
...
/// Unique identifier for an object on the Mys blockchain.
///
/// An `ObjectId` is a 32-byte identifier used to uniquely identify an object on the Mys
/// blockchain.
#[derive(Clone, PartialEq, ::prost::Message)]
pub struct ObjectId {
...
/// 32-byte output of hashing a Mys structure using the Blake2b256 hash function.
#[derive(Clone, PartialEq, ::prost::Message)]
pub struct Digest {
...
/// A header for a checkpoint on the Mys blockchain.
///
/// On the Mys network, checkpoints define the history of the blockchain. They are quite similar to
/// the concept of blocks used by other blockchains like Bitcoin or Ethereum. The Mys blockchain,
...
pub mod failure_status {
    ...
    pub enum ExecutionError {
        ...
        CertificateDenied(()),
        /// Mys Move bytecode verification timed out.
...
pub struct SimpleSignature {
    ...
    /// enum value of [mys.types.SignatureScheme][mys.types.SignatureScheme]
