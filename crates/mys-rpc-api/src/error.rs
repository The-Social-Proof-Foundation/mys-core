...
use mys_types::error::ErrorCategory;
use tonic::Code;

use crate::proto::google::rpc::{BadRequest, ErrorInfo, RetryInfo};
pub use mys_rpc::proto::mys::rpc::v2::ErrorReason;
...
impl From<mys_types::storage::error::Error> for RpcError {
    fn from(value: mys_types::storage::error::Error) -> Self {
        use mys_types::storage::error::Kind;
...
impl From<mys_types::mys_sdk_types_conversions::SdkTypeConversionError> for RpcError {
    fn from(value: mys_types::mys_sdk_types_conversions::SdkTypeConversionError) -> Self {
        Self {
            code: Code::Internal,
            message: Some(value.to_string()),
            details: None,
...
impl From<mys_types::quorum_driver_types::QuorumDriverError> for RpcError {
    fn from(error: mys_types::quorum_driver_types::QuorumDriverError) -> Self {
        ...
        match error {
            InvalidUserSignature(err) => {
                let message = {
                    let err = match &err {
                        MysError::UserInputError { error } => error.to_string(),
                        _ => err.to_string(),
                    };
            ...
            NonRecoverableTransactionError { errors } => {
                let new_errors: Vec<String> = errors
                    ...
                    .filter_map(|(err, _, _)| {
                        match &err {
                            ...
                            MysError::UserInputError { error } => Some(error.to_string()),
                            _ => {
                ...
                let message = if new_errors.is_empty() {
                    "Transaction has non recoverable errors from at least 1/3 of validators".to_string()
                } else {
                    format!(
            ...
            SystemOverload { overloaded_stake, errors } => {
                let message = format!(
                    "Transaction is not processed because {} of validators by stake are overloaded with certificates pending execution. Errors: {:?}",
                    overloaded_stake, errors
                );
                RpcError::new(Code::Unavailable, message)
            }
            SystemOverloadRetryAfter { overload_stake, errors, retry_after_secs } => {
                let message = format!(
                    "Transaction is not processed because {} of validators are overloaded. Please retry after at least {} seconds. Errors: {:?}",
                    overload_stake, retry_after_secs, errors
                );
                RpcError::new(Code::Unavailable, message)
            }
            TxAlreadyFinalizedWithDifferentUserSignatures => {
                RpcError::new(
                    ...
                    "Transaction is already finalized but with different user signatures",
                )
            }
        }
    }
...
impl From<mys_types::transaction_driver_types::TransactionSubmissionError> for RpcError {
    fn from(error: mys_types::transaction_driver_types::TransactionSubmissionError) -> Self {
        use itertools::Itertools;
        use mys_types::error::MysError;
        use mys_types::transaction_driver_types::TransactionSubmissionError::*;
...
impl ObjectNotFoundError {
    pub fn new(object_id: mys_sdk_types::Address) -> Self {
        Self {
            object_id,
            version: None,
        }
...
impl CheckpointNotFoundError {
    ...
    pub fn digest(digest: mys_sdk_types::Digest) -> Self {
        Self {
