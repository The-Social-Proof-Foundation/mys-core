...
fn main() {
    ...
    let mys_proto_dir = crate_dir.join("proto");
    let out_dir = crate_dir.join("src/proto/generated");

    let mys_rpc_proto_dir = find_dependency_proto_dir("mys-rpc", "vendored/proto");

    println!("cargo:rerun-if-changed={}", mys_proto_dir.display());
    println!("cargo:rerun-if-changed={}", mys_rpc_proto_dir.display());

    fs::create_dir_all(&out_dir).expect("create proto out dir");

    // Find all .proto files using walkdir (only from mys_proto_dir, not mys_rpc_proto_dir)
    let proto_ext = OsStr::new("proto");
    let mut proto_files = vec![];
    for entry in WalkDir::new(&mys_proto_dir) {
    ...
    let file_descriptors = protox::compile(proto_files, [mys_proto_dir, mys_rpc_proto_dir])
    ...
    tonic_prost_build::configure()
        ...
        .extern_path(".mys.rpc.v2", "::mys_rpc::proto::mys::rpc::v2")
        .out_dir(&out_dir)
