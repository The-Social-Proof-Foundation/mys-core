...
impl ValidationMode {
    ...
    fn local(&self, package: &CompiledPackage) -> Result<LocalModules, Error> {
        ...
        if self.verify_deps() {
            ...
            for (package, module) in mys_package.bytecode_deps.iter() {
...
impl<'a> BytecodeSourceVerifier<'a> {
    ...
    async fn pkg_for_address(&self, addr: AccountAddress) -> Result<MysRawMovePackage, Error> {
        // Move packages are specified with an AccountAddress, but are
        // fetched from a mys network via mys_getObject, which takes an object ID
        let obj_id = ObjectID::from(addr);

        // fetch the Mys object at the address specified for the package in the local resolution table
        ...
        let obj_read = self
            .rpc_client
            .get_object_with_options(obj_id, MysObjectDataOptions::new().with_bcs())
            .await
            .map_err(Error::DependencyObjectReadFailure)?;

        let obj = obj_read
            .into_object()
            .map_err(Error::MysObjectRefFailure)?
            .bcs
            .ok_or_else(|| {
        ...
        match obj {
            MysRawData::Package(pkg) => Ok(pkg),
            MysRawData::MoveObject(move_obj) => {
