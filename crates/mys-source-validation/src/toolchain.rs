...
const CANONICAL_UNIX_BINARY_NAME: &str = "mys";
const CANONICAL_WIN_BINARY_NAME: &str = "mys.exe";
...
pub(crate) fn current_toolchain() -> ToolchainVersion {
    ToolchainVersion {
        ...
        flavor: Flavor::Mys,      /* does not matter, unused for current_toolchain */
    }
}
...
pub(crate) fn units_for_toolchain(
    ...
    if std::env::var("MYS_RUN_TOOLCHAIN_BUILD").is_err() {
    ...
    for (package, local_unit) in compiled_units {
        ...
        if mys_types::is_system_package(local_unit.unit.address.into_inner()) {
...
fn download_and_compile(
    ...
    if !dest_canonical_binary.exists() {
        ...
        let mainnet_url = format!(
            "https://github.com/MystenLabs/mys/releases/download/mainnet-v{compiler_version}/mys-mainnet-v{compiler_version}-{platform}.tgz",
        );
        ...
        let mut response = match ureq::get(&mainnet_url).call() {
            ...
            Err(ureq::Error::Status(404, _)) => {
                println!(
                    "{} mys mainnet compiler {} not available, attempting to download testnet compiler release...",
                    "WARNING".bold().yellow(),
                    compiler_version.yellow()
                );
                println!(
                ...
                let testnet_url = format!("https://github.com/MystenLabs/mys/releases/download/testnet-v{compiler_version}/mys-testnet-v{compiler_version}-{platform}.tgz");
        ...
        if platform == "windows-x86_64" {
            dest_binary.push(format!("mys-{platform}.exe"));
        } else {
            dest_binary.push(format!("mys-{platform}"));
        }
...
fn detect_platform(
    ...
    let s = match (std::env::consts::OS, std::env::consts::ARCH) {
        ...
        (os, arch) => {
            ...
            bail!(
                "The package {} needs to be built with mys compiler version {compiler_version} but there \
