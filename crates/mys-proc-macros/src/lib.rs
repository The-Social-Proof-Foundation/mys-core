    ...
    input.block = syn::parse2(quote! {
        {
            ...
            std::thread::spawn(|| {
                ...
                ::mys_simulator::telemetry_subscribers::init_for_testing();
                ::mys_simulator::mys_types::execution::get_denied_certificates();
                ::mys_simulator::mys_framework::BuiltInFramework::all_package_ids();
                ::mys_simulator::mys_types::gas::MysGasStatus::new_unmetered();
                ...
                let mut cache = ::mys_simulator::lru::LruCache::new(1.try_into().unwrap());
                cache.put(1, 1);

                {
                    ...
                    use mys_simulator::mys_move_build::{BuildConfig, MysPackageHooks};
                    use mys_simulator::tempfile::TempDir;
                    use mys_simulator::move_package::package_hooks::register_package_hooks;

                    register_package_hooks(Box::new(MysPackageHooks {}));
                ...
                use ::mys_simulator::anemo_tower::callback::CallbackLayer;
                use ::mys_simulator::anemo_tower::trace::DefaultMakeSpan;
                use ::mys_simulator::anemo_tower::trace::DefaultOnFailure;
                use ::mys_simulator::anemo_tower::trace::TraceLayer;
                use ::mys_simulator::fastcrypto::traits::KeyPair;
                use ::mys_simulator::mysten_network::metrics::MetricsMakeCallbackHandler;
                use ::mys_simulator::mysten_network::metrics::NetworkMetrics;
                use ::mys_simulator::rand_crate::rngs::{StdRng, OsRng};
                use ::mys_simulator::rand::SeedableRng;
                use ::mys_simulator::tower::ServiceBuilder;
                ...
                let rt = ::mys_simulator::runtime::Runtime::new();
                rt.block_on(async move {
                    use ::mys_simulator::anemo::{Network, Request};

                    let make_network = |port: u16| {
                        let registry = prometheus::Registry::new();
                        let inbound_network_metrics =
                            NetworkMetrics::new("mys", "inbound", &registry);
                        let outbound_network_metrics =
                            NetworkMetrics::new("mys", "outbound", &registry);
                        ...
                        let service = ServiceBuilder::new()
                            ...
                            .service(::mys_simulator::anemo::Router::new());
                        ...
                        Network::bind(format!("127.0.0.1:{}", port))
                            ...
                            .private_key(
                                ::mys_simulator::fastcrypto::ed25519::Ed25519KeyPair::generate(&mut StdRng::from_rng(OsRng).unwrap())
                                    .private()
                                    .0
                                    .to_bytes(),
                            )
...
/// The mys_test macro will invoke either `#[msim::test]` or `#[tokio::test]`,
...
#[proc_macro_attribute]
pub fn mys_test(args: TokenStream, item: TokenStream) -> TokenStream {
    ...
    let header = if cfg!(msim) {
        quote! {
            #[::mys_simulator::sim_test(crate = "mys_simulator", #(#args)* )]
        }
    } else {
        quote! {
            #[::tokio::test(#(#args)*)]
    ...
    let result = quote! {
        #header
        #[::mys_macros::init_static_initializers]
        #input
    };
    ...
    let result = if cfg!(msim) {
        ...
        quote! {
            ...
            #sig {
                ...
                ::mys_simulator::task::shutdown_all_nodes();
                ...
                assert_eq!(
                    mys_simulator::NodeLeakDetector::get_current_node_count(),
                    0,
                    "MysNode leak detected"
                );
        ...
        quote! {
            ...
            #ignore
            #sig {
                if std::env::var("MYS_SKIP_SIMTESTS").is_ok() {
                    println!("not running test {} in `cargo test`: MYS_SKIP_SIMTESTS is set", stringify!(#fn_name));
...
pub fn enum_variant_order_derive(input: TokenStream) -> TokenStream {
    ...
    if let Data::Enum(DataEnum { variants, .. }) = ast.data {
        ...
        let deriv = quote! {
            impl mys_enum_compat_util::EnumOrderMap for #name {
