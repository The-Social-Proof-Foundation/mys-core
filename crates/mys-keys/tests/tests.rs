...
fn alias_exists_test() {
    ...
    let keystore_path = temp_dir.path().join("mys.keystore");
...
fn create_alias_keystore_file_test() {
    let temp_dir = TempDir::new().unwrap();
    let mut keystore_path = temp_dir.path().join("mys.keystore");
    let mut keystore = Keystore::from(FileBasedKeystore::new(&keystore_path).unwrap());
    keystore
...
fn check_reading_aliases_file_correctly() {
    ...
    let temp_dir = TempDir::new().unwrap();
    let mut keystore_path = temp_dir.path().join("mys.keystore");
    let keystore_path_keep = temp_dir.path().join("mys.keystore");
    let mut keystore = Keystore::from(FileBasedKeystore::new(&keystore_path).unwrap());
    let kp = keystore
        .generate_and_add_new_key(SignatureScheme::ED25519, None, None, None)
        .unwrap();
...
fn keystore_no_aliases() {
    ...
    // mys.aliases file with the existing address in the mys.keystore,
...
fn mnemonic_test() {
    ...
    let keystore_path_2 = temp_dir.path().join("mys2.keystore");
    ...
    let imported_address = keystore2
        ...
        .unwrap();
    assert_eq!(scheme.flag(), Ed25519MysSignature::SCHEME.flag());
    assert_eq!(address, imported_address);
}

/// This test confirms rust's implementation of mnemonic is the same with the Mys Wallet
#[test]
fn mys_wallet_address_mnemonic_test() -> Result<(), anyhow::Error> {
    let phrase = "result crisp session latin must fruit genuine question prevent start coconut brave speak student dismiss";
    let expected_address =
        MysAddress::from_str("0x936accb491f0facaac668baaedcf4d0cfc6da1120b66f77fa6a43af718669973")?;
    ...
    assert_eq!(pubkey.flag(), Ed25519MysSignature::SCHEME.flag());

    let mut hasher = DefaultHash::default();
    hasher.update([pubkey.flag()]);
    hasher.update(pubkey);
    let g_arr = hasher.finalize();
    let mut res = [0u8; MYS_ADDRESS_LENGTH];
    res.copy_from_slice(&AsRef::<[u8]>::as_ref(&g_arr)[..MYS_ADDRESS_LENGTH]);
    let address = MysAddress::try_from(res.as_slice())?;
