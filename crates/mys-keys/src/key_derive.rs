...
/// Ed25519 follows SLIP-0010 using hardened path: m/44'/6976'/0'/0'/{index}'
/// Secp256k1 follows BIP-32/44 using path where the first 3 levels are hardened: m/54'/6976'/0'/0/{index}
/// Secp256r1 follows BIP-32/44 using path where the first 3 levels are hardened: m/74'/6976'/0'/0/{index}
/// Note that the purpose node is used to distinguish signature schemes.
pub fn derive_key_pair_from_path(
    seed: &[u8],
    derivation_path: Option<DerivationPath>,
    key_scheme: &SignatureScheme,
) -> Result<(MysAddress, MysKeyPair), MysError> {
    ...
    match key_scheme {
        SignatureScheme::ED25519 => {
            ...
            let sk = Ed25519PrivateKey::from_bytes(&derived)
                .map_err(|e| MysError::SignatureKeyGenError(e.to_string()))?;
            let kp: Ed25519KeyPair = sk.into();
            Ok((kp.public().into(), MysKeyPair::Ed25519(kp)))
        }
        SignatureScheme::Secp256k1 => {
            ...
            Ok((kp.public().into(), MysKeyPair::Secp256k1(kp)))
        }
        SignatureScheme::Secp256r1 => {
            ...
            Ok((kp.public().into(), MysKeyPair::Secp256r1(kp)))
        }
        SignatureScheme::BLS12381
        | SignatureScheme::MultiSig
        | SignatureScheme::ZkLoginAuthenticator
        | SignatureScheme::PasskeyAuthenticator => Err(MysError::UnsupportedFeatureError {
...
) -> Result<DerivationPath, MysError> {
    match key_scheme {
        SignatureScheme::ED25519 => {
            match path {
                Some(p) => {
                    // The derivation path must be hardened at all levels with purpose = 44, coin_type = 6976
                    if let &[purpose, coin_type, account, change, address] = p.as_ref() {
                            ...
                            && address.is_hardened()
                        {
                            ...
                            Err(MysError::SignatureKeyGenError("Invalid path".to_string()))
                        }
                    } else {
                        Err(MysError::SignatureKeyGenError("Invalid path".to_string()))
                    }
                ...
                .parse()
                .map_err(|_| MysError::SignatureKeyGenError("Cannot parse path".to_string()))?),
            }
        }
        SignatureScheme::Secp256k1 => {
            match path {
                Some(p) => {
                    // The derivation path must be hardened at first 3 levels with purpose = 54, coin_type = 6976
                    if let &[purpose, coin_type, account, change, address] = p.as_ref() {
                        if Some(purpose)
        ...
        SignatureScheme::Secp256r1 => {
            match path {
                Some(p) => {
                    // The derivation path must be hardened at first 3 levels with purpose = 74, coin_type = 6976
                    if let &[purpose, coin_type, account, change, address] = p.as_ref() {
                        if Some(purpose)
...
) -> Result<(MysAddress, MysKeyPair, SignatureScheme, String), anyhow::Error> {
