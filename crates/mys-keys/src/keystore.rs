...
pub trait AccountKeystore: Send + Sync {
    fn add_key(&mut self, alias: Option<String>, keypair: MysKeyPair) -> Result<(), anyhow::Error>;
    fn keys(&self) -> Vec<PublicKey>;
    fn get_key(&self, address: &MysAddress) -> Result<&MysKeyPair, anyhow::Error>;

    fn sign_hashed(&self, address: &MysAddress, msg: &[u8]) -> Result<Signature, signature::Error>;
    ...
    where
        T: Serialize;
    fn addresses(&self) -> Vec<MysAddress> {
        self.keys().iter().map(|k| k.into()).collect()
    }
    fn addresses_with_alias(&self) -> Vec<(&MysAddress, &Alias)>;
    ...
    fn get_alias_by_address(&self, address: &MysAddress) -> Result<String, anyhow::Error>;
    fn get_address_by_alias(&self, alias: String) -> Result<&MysAddress, anyhow::Error>;
    ...
    ) -> Result<(MysAddress, String, SignatureScheme), anyhow::Error> {
...
pub struct FileBasedKeystore {
    keys: BTreeMap<MysAddress, MysKeyPair>,
    aliases: BTreeMap<MysAddress, Alias>,
    path: Option<PathBuf>,
}
...
impl AccountKeystore for FileBasedKeystore {
    ...
    fn add_key(&mut self, alias: Option<String>, keypair: MysKeyPair) -> Result<(), anyhow::Error> {
        let address: MysAddress = (&keypair.public()).into();
...
impl FileBasedKeystore {
        ...
        let aliases = if aliases_path.exists() {
            ...
            aliases
                ...
                .map(|alias| {
                    ...
                    key.map(|k| (Into::<MysAddress>::into(&k), alias))
                })
                .collect::<Result<BTreeMap<_, _>, _>>()
                .map_err(|e| {
                    anyhow!(
    ...
    /// To see Bech32 format encoding, use `mys keytool export $MYS_ADDRESS` where
    /// $MYS_ADDRESS can be found with `mys keytool list`. Or use `mys keytool convert $BASE64_STR`
    ...
    pub fn key_pairs(&self) -> Vec<&MysKeyPair> {
...
impl InMemKeystore {
    pub fn new_insecure_for_tests(initial_key_number: usize) -> Self {
        ...
        let keys = (0..initial_key_number)
            ...
            .map(|(ad, k)| (ad, MysKeyPair::Ed25519(k)))
            .collect::<BTreeMap<MysAddress, MysKeyPair>>();
...
mod tests {
    ...
    fn validate_alias_test() {
        ...
        assert!(validate_alias("abc_123.mys").is_ok());
        // Not allowed
