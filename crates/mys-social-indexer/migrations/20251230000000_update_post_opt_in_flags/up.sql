...
-- Add linked object address columns
...
-- Rename poc_badge_id index to poc_id
...
CREATE INDEX idx_posts_enable_spt ON posts(enable_spt, time) WHERE enable_spt = true;
CREATE INDEX idx_posts_enable_poc ON posts(enable_poc, time) WHERE enable_poc = true;
CREATE INDEX idx_posts_enable_spot ON posts(enable_spot, time) WHERE enable_spot = true;

-- Create indexes for linked object IDs
CREATE INDEX idx_posts_spot_id ON posts(spot_id, time) WHERE spot_id IS NOT NULL;
CREATE INDEX idx_posts_spt_id ON posts(spt_id, time) WHERE spt_id IS NOT NULL;

-- Drop old auto_pool_disabled index
DROP INDEX idx_posts_auto_pool_disabled;
