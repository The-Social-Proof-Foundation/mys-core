...
use crate::events::social_proof_token_events::{
    ...
    SocialProofReservationCreatedEvent, SocialProofReservationWithdrawnEvent,
...
use crate::models::social_proof_token::{
    ...
    NewSptReservationPool, SptReservationPool,
};
...
impl SocialProofTokenHandler {
    ...
    async fn process_sell_event(&mut self, event: &BlockchainEvent) -> Result<()> {
        ...
        let new_circulating_supply = latest_pool.circulating_supply.saturating_sub(sell_event.amount);
        ...
        let mut price_history = sell_event.into_price_history_model(new_circulating_supply, timestamp, event.tx_digest.clone())?;
        price_history.time = datetime;
    ...
    async fn process_reservation_created_event(&mut self, event: &BlockchainEvent) -> Result<()> {
        info!("Processing SPT reservation created event: {}", event.event_id);
        
        // Extract and parse the event
        let fields = Self::extract_event_fields(&event.data)?;
        let reservation_event = serde_json::from_value::<SocialProofReservationCreatedEvent>(fields)
        ...
        let mut reservation_record = reservation_event.into_reservation_model(timestamp, event.tx_digest.clone())?;
        ...
        if let Some(existing) = existing_pool {
            ...
            // Create new pool
            let required_threshold = if reservation_event.associated_id.starts_with("post_") { 1000 } else { 10000 };
            let new_pool = NewSptReservationPool {
                ...
                token_type: if reservation_event.associated_id.starts_with("post_") { 2 } else { 1 },
                total_reserved: reservation_event.total_reserved,
                required_threshold,
                status: if reservation_event.threshold_met {
                    "threshold_met".to_string()
        ...
        info!("Successfully processed reservation created event for pool: {}", pool_id);
        Ok(())
    }

    /// Process reservation withdrawn events
    async fn process_reservation_withdrawn_event(&mut self, event: &BlockchainEvent) -> Result<()> {
        info!("Processing SPT reservation withdrawn event: {}", event.event_id);
        
        // Extract and parse the event
        let fields = Self::extract_event_fields(&event.data)?;
        let withdrawal_event = serde_json::from_value::<SocialProofReservationWithdrawnEvent>(fields)
        ...
        let mut withdrawal_record = withdrawal_event.into_reservation_model(timestamp, event.tx_digest.clone())?;
        ...
        if let Some(existing) = existing_pool {
            ...
            let minimal_pool = NewSptReservationPool {
                ...
                token_type: if withdrawal_event.associated_id.starts_with("post_") { 2 } else { 1 },
                total_reserved: withdrawal_event.total_reserved,
                required_threshold,
        ...
        info!("Successfully processed reservation withdrawn event for pool: {}", pool_id);
        Ok(())
    }
    ...
    async fn process_threshold_met_event(&mut self, event: &BlockchainEvent) -> Result<()> {
        ...
        let mut reservation_pool = threshold_event.into_reservation_pool_model(timestamp, event.tx_digest.clone())?;
    ...
    async fn update_progress(&self) -> Result<()> {
        ...
        if events_processed > 0 && events_processed % 50 == 0 {
            info!(
                ...
                self.base.stats.last_processed_timestamp
                    .map(|ts| format!("{}ms", ts))
...
impl BlockchainEventHandler for SocialProofTokenHandler {
    ...
    async fn process_event(&mut self, event: BlockchainEvent) -> Result<()> {
        ...
        let result = match event_type {
            ...
            t if t.contains("::social_proof_tokens::") && t.ends_with("::ReservationCreatedEvent") => {
                self.process_reservation_created_event(&event).await
            },
            t if t.contains("::social_proof_tokens::") && t.ends_with("::ReservationWithdrawnEvent") => {
