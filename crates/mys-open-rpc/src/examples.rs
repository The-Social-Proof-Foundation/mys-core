...
use mys_types::crypto::{get_key_pair_from_rng, AccountKeyPair, AggregateAuthoritySignature};
use mys_types::digests::TransactionEventsDigest;
use mys_types::dynamic_field::{DynamicFieldInfo, DynamicFieldName, DynamicFieldType};
...
use mys_types::object::MoveObject;
...
use mys_types::transaction::TEST_ONLY_GAS_UNIT_FOR_TRANSFER;
use mys_types::transaction::{CallArg, TransactionData};
use mys_types::utils::to_sender_signed_transaction;
use mys_types::{parse_mys_struct_tag, MYS_FRAMEWORK_PACKAGE_ID};
...
impl RpcExampleProvider {
    ...
    pub fn examples(&mut self) -> BTreeMap<String, Vec<ExamplePairing>> {
        [
            ...
            self.mys_get_committee_info(),
            self.mys_get_reference_gas_price(),
            self.mysx_get_all_balances(),
            self.mysx_get_all_coins(),
            self.mysx_get_balance(),
            self.mysx_get_coin_metadata(),
            self.mys_get_latest_checkpoint_sequence_number(),
            self.mysx_get_coins(),
            self.mysx_get_total_supply(),
            self.mysx_get_dynamic_fields(),
            self.mysx_get_dynamic_field_object(),
            self.mysx_get_owned_objects(),
            self.mys_get_move_function_arg_types(),
            self.mys_get_normalized_move_function(),
            self.mys_get_normalized_move_module(),
            self.mys_get_normalized_move_modules_by_package(),
            self.mys_get_normalized_move_struct(),
            self.multi_get_objects_example(),
            self.multi_get_transaction_blocks(),
            self.mysx_get_validators_apy(),
            self.mysx_get_dynamic_fields(),
            self.mysx_get_dynamic_field_object(),
            self.mysx_get_owned_objects(),
            self.mysx_query_events(),
            self.mysx_get_latest_mys_system_state(),
    ...
    fn batch_transaction_examples(&mut self) -> Examples {
        let signer = MysAddress::from(ObjectID::new(self.rng.gen()));
        let recipient = MysAddress::from(ObjectID::new(self.rng.gen()));
        ...
        let tx_params = vec![
            RPCTransactionRequestParams::MoveCallRequestParams(MoveCallParams {
                package_object_id: MYS_FRAMEWORK_PACKAGE_ID,
                module: "pay".to_string(),
                function: "split".to_string(),
                type_arguments: vec![MysTypeTag::new("0x2::mys::MYS".to_string())],
                arguments: vec![
                    MysJsonValue::new(json!(coin_ref.0)).unwrap(),
                    MysJsonValue::new(json!(random_amount)).unwrap(),
                ],
            }),
        ...
        Examples::new(
            ...
            vec![ExamplePairing::new(
                ...
                vec![
                    ...
                    (
                        "options",
                        json!(MysTransactionBlockResponseOptions::full_content()),
                    ),
                    (
                        "request_type",
    ...
    fn dev_inspect_transaction_block(&mut self) -> Examples {
        ...
        let dev_inspect_results = DevInspectResults {
            ...
            events: MysTransactionBlockEvents { data: vec![] },
            results: None,
            error: None,
            raw_txn_data: vec![],
            raw_effects: vec![],
        ...
        Examples::new(
            ...
            vec![ExamplePairing::new(
                ...
                vec![
                    ("sender_address", json!(MysAddress::from(ObjectID::new(self.rng.gen())))),
                    ("tx_bytes", json!(tx_bytes.tx_bytes)),
                    ("gas_price", json!(1000)),
                    ("epoch", json!(8888)),
        ...
        Examples::new(
            ...
            vec![ExamplePairing::new(
                ...
                vec![
                    ...
                    ("options", json!(MysObjectDataOptions::full_content())),
                ],
                json!(objects),
            )],
        )
    ...
    fn get_object_responses(&mut self, object_count: usize) -> Vec<MysObjectResponse> {
        (0..object_count)
            .map(|_| {
                let object_id = ObjectID::new(self.rng.gen());
                let coin = GasCoin::new(object_id, 100000000);

                MysObjectResponse::new_with_data(MysObjectData {
                    content: Some(
                        MysParsedData::try_from_object(
                            coin.to_object(SequenceNumber::from_u64(1)),
                            GasCoin::layout(),
                        )
                        .unwrap(),
                    ),
                    owner: Some(Owner::AddressOwner(MysAddress::from(ObjectID::new(
                        self.rng.gen(),
                    )))),
    ...
    fn get_past_object_example(&mut self) -> Examples {
        ...
        let result = MysPastObjectResponse::VersionFound(MysObjectData {
            content: Some(
                MysParsedData::try_from_object(
                    coin.to_object(SequenceNumber::from_u64(1)),
                    GasCoin::layout(),
    ...
    fn get_owned_objects(&mut self) -> Examples {
        let owner = MysAddress::from(ObjectID::new(self.rng.gen()));
        let result = (0..4)
            .map(|_| MysObjectData {
        ...
        Examples::new(
            ...
            vec![ExamplePairing::new(
                ...
                vec![
                    ...
                    (
                        "query",
                        json!(MysObjectResponseQuery {
                            filter: Some(MysObjectDataFilter::StructType(
                                StructTag::from_str("0x2::coin::Coin<0x2::mys::MYS>").unwrap()
                            )),
                            options: Some(
        ...
        Examples::new(
            ...
            vec![ExamplePairing::new(
                ...
                vec![
                    ...
                    (
                        "options",
                        json!(MysTransactionBlockResponseOptions::new()
                            .with_input()
                            .with_effects()
                            .with_events()),
                    ),
    ...
    fn query_transaction_blocks(&mut self) -> Examples {
        ...
        let data = data
            .into_iter()
            .map(MysTransactionBlockResponse::new)
            .collect();
        ...
        Examples::new(
            ...
            vec![ExamplePairing::new(
                ...
                vec![
                    (
                        "query",
                        json!(MysTransactionBlockResponseQuery {
    ...
    fn get_transfer_data_response(
        ...
        let object_change = ObjectChange::Transferred {
            ...
            object_type: parse_mys_struct_tag("0x2::example::Object").unwrap(),
        ...
        let result = MysTransactionBlockResponse {
            digest: *tx_digest,
            effects: Some(MysTransactionBlockEffects::V1(
                MysTransactionBlockEffectsV1 {
                    status: MysExecutionStatus::Success,
                    executed_epoch: 0,
                    ...
                    gas_object: OwnedObjectRef {
                        ...
                        reference: MysObjectRef::from(gas_ref),
                    },
            ...
            timestamp_ms: None,
            transaction: Some(MysTransactionBlock {
                data: MysTransactionBlockData::try_from(data1, &&mut NoOpsModuleResolver).unwrap(),
    ...
    fn get_events(&mut self) -> Examples {
        ...
        let event = MysEvent {
            ...
            sender: MysAddress::from(ObjectID::new(self.rng.gen())),
            type_: parse_mys_struct_tag("0x9::test::TestEvent").unwrap(),
    ...
    fn mys_get_reference_gas_price(&mut self) -> Examples {
        let result = 1000;
        Examples::new(
            "mysx_getReferenceGasPrice",
            vec![ExamplePairing::new(
    ...
    fn mysx_get_all_balances(&mut self) -> Examples {
        let address = MysAddress::from(ObjectID::new(self.rng.gen()));
    ...
    fn mysx_get_all_coins(&mut self) -> Examples {
        let limit = 3;
        let owner = MysAddress::from(ObjectID::new(self.rng.gen()));
    ...
    fn mysx_get_balance(&mut self) -> Examples {
        let owner = MysAddress::from(ObjectID::new(self.rng.gen()));
    ...
    fn mysx_get_coin_metadata(&mut self) -> Examples {
        let result = MysCoinMetadata {
            decimals: 9,
            name: "Usdc".to_string(),
    ...
    fn mys_get_latest_checkpoint_sequence_number(&mut self) -> Examples {
        let result = "507021";
        Examples::new(
            "mys_getLatestCheckpointSequenceNumber",
            vec![ExamplePairing::new(
    ...
    fn mysx_get_coins(&mut self) -> Examples {
        let coin_type = "0x2::mys::MYS".to_string();
        let owner = MysAddress::from(ObjectID::new(self.rng.gen()));
        let coins = (0..3)
            .map(|_| Coin {
                coin_type: coin_type.clone(),
        ...
        Examples::new(
            "mysx_getCoins",
            vec![ExamplePairing::new(
                "Gets all MYS coins owned by the address provided. Return a paginated list of `limit` results per page. Similar to `mysx_getAllCoins`, but provides a way to filter by coin type.",
                vec![
                    ("owner", json!(owner)),
    ...
    fn mysx_get_total_supply(&mut self) -> Examples {
    ...
    fn mys_get_move_function_arg_types(&mut self) -> Examples {
        let result = vec![
        ...
        Examples::new(
            ...
            vec![ExamplePairing::new(
                ...
                vec![
                    ...
                    ("module", json!("mysfrens".to_string())),
    ...
    fn mys_get_normalized_move_function(&mut self) -> Examples {
        let ability_set = MysMoveAbilitySet {
            abilities: vec![MysMoveAbility::Store, MysMoveAbility::Key],
        };

        let result = MysMoveNormalizedFunction {
            is_entry: false,
            type_parameters: vec![ability_set],
            parameters: vec![MysMoveNormalizedType::U64],
            visibility: MysMoveVisibility::Public,
            return_: vec![MysMoveNormalizedType::U64],
        };
    ...
    fn mys_get_normalized_move_module(&mut self) -> Examples {
        let result = MysMoveNormalizedModule {
    ...
    fn mys_get_normalized_move_modules_by_package(&mut self) -> Examples {
        let result = MysMoveNormalizedModule {
            address: ObjectID::new(self.rng.gen()).to_string(),
            exposed_functions: BTreeMap::new(),
            file_format_version: 6,
    ...
    fn mys_get_normalized_move_struct(&mut self) -> Examples {
        let abilities = MysMoveAbilitySet {
            abilities: vec![MysMoveAbility::Store, MysMoveAbility::Key],
        };
        let fields = vec![].into_iter().collect::<Vec<_>>();
        let type_parameters = vec![].into_iter().collect::<Vec<_>>();
        let result = MysMoveNormalizedStruct {
            abilities,
            fields,
            type_parameters,
        };
    ...
    fn mysx_get_validators_apy(&mut self) -> Examples {
        let result = vec![
            ValidatorApy {
                address: MysAddress::from(ObjectID::new(self.rng.gen())),
                apy: 0.06,
            },
            ValidatorApy {
                address: MysAddress::from(ObjectID::new(self.rng.gen())),
    ...
    fn mysx_get_dynamic_fields(&mut self) -> Examples {
        let object_id = ObjectID::new(self.rng.gen());
        ...
        Examples::new("mysx_getDynamicFields",
        vec![ExamplePairing::new(
    ...
    fn mysx_get_dynamic_field_object(&mut self) -> Examples {
        ...
        let struct_tag = parse_mys_struct_tag("0x9::test::TestField").unwrap();
        let resp = MysObjectResponse::new_with_data(MysObjectData {
            content: Some(
                MysParsedData::try_from_object(
                    unsafe {
            ...
            type_: Some(ObjectType::Struct(MoveObjectType::from(
                parse_mys_struct_tag("0x9::test::TestField").unwrap(),
            ))),
            bcs: None,
            display: None,
        });
    ...
    fn mysx_get_owned_objects(&mut self) -> Examples {
        let owner = MysAddress::from(ObjectID::new(self.rng.gen()));
        ...
        let filter = Some(MysObjectDataFilter::MatchAll(vec![
            MysObjectDataFilter::StructType(
                StructTag::from_str("0x2::coin::Coin<0x2::mys::MYS>").unwrap(),
            ),
            MysObjectDataFilter::AddressOwner(owner),
            MysObjectDataFilter::Version(version),
        ]));
        let query = json!(MysObjectResponseQuery { filter, options });
        let object_id = ObjectID::new(self.rng.gen());
    ...
    fn mysx_query_events(&mut self) -> Examples {
    ...
    fn mysx_get_latest_mys_system_state(&mut self) -> Examples {
        let result = "some_system_state";
        Examples::new(
            "mysx_getLatestMysSystemState",
            vec![ExamplePairing::new(
                "Gets objects owned by the address in the request.",
                vec![],
                json!(result),
    ...
    fn mys_get_chain_identifier(&mut self) -> Examples {
    ...
    fn mysx_get_stakes(&mut self) -> Examples {
        let principal = 200000000000;
        let owner = MysAddress::from(ObjectID::new(self.rng.gen()));
        let result = vec![
            DelegatedStake {
                validator_address: MysAddress::from(ObjectID::new(self.rng.gen())),
                staking_pool: ObjectID::new(self.rng.gen()),
                stakes: vec![
                    Stake {
                        staked_mys_id: ObjectID::new(self.rng.gen()),
    ...
    fn mysx_get_stakes_by_ids(&mut self) -> Examples {
        let principal = 200000000000;
        ...
        Examples::new(
            ...
            vec![ExamplePairing::new(
                "Returns the staking information for the address the request provides.",
                vec![("staked_mys_ids", json!(vec![stake1, stake2]))],
                json!(result),
            )],
        )
    }
    ...
    fn mys_try_multi_get_past_objects(&mut self) -> Examples {
        ...
        let objects = vec![
            MysGetPastObjectRequest { object_id, version },
            MysGetPastObjectRequest {
        ...
        let result = vec![
            MysPastObjectResponse::VersionFound(MysObjectData {
                content: Some(
                    MysParsedData::try_from_object(
                        coin.to_object(SequenceNumber::from_u64(1)),
                        GasCoin::layout(),
