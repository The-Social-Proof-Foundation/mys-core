...
//!
//! # TODO: Consider moving FieldCount to mys-pg-db
//!
//! Currently, FieldCount lives in this framework crate but is Postgres-specific. Ideally it should
//! live in mys-pg-db. However, this creates a circular dependency:
//! - mys-indexer-alt-framework depends on mys-pg-db (for IndexerCluster and other utilities)
//! - This blanket impl needs FieldCount to implement concurrent::Handler for postgres indexers
//! - Moving FieldCount to mys-pg-db would require framework to depend on mys-pg-db (circular!)
//!
//! To fully decouple, we'd need to move all postgres-specific code (including IndexerCluster) to
//! mys-pg-db, which would be a much larger breaking change. Consider this for a future refactor.
//!
