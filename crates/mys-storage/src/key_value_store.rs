...
impl TransactionKeyValueStore {
    ...
    ) -> MysResult<Vec<Option<CertifiedCheckpointSummary>>> {
    ...
    pub async fn multi_get_checkpoints_contents(
        &self,
        keys: &[CheckpointSequenceNumber],
    ) -> MysResult<Vec<Option<CheckpointContents>>> {
        self.multi_get_checkpoints(&[], keys, &[])
            .await
    ...
    ) -> MysResult<Vec<Option<Transaction>>> {
    ...
    pub async fn get_tx(&self, digest: TransactionDigest) -> MysResult<Transaction> {
        self.multi_get_tx(&[digest])
            .await?
            .into_iter()
            .next()
            .flatten()
            .ok_or(MysError::TransactionNotFound { digest })
    }
    ...
    ) -> MysResult<CertifiedCheckpointSummary> {
