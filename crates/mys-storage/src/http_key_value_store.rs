...
trait IntoMysResult<T> {
    fn into_mys_result(self) -> MysResult<T>;
}

impl<T, E> IntoMysResult<T> for Result<T, E>
where
    E: std::error::Error,
{
    fn into_mys_result(self) -> MysResult<T> {
        self.map_err(|e| MysError::Storage(e.to_string()))
    }
}
...
impl HttpKVStore {
    pub fn new_kv(
        ...
        cache_size: u64,
        metrics: Arc<KeyValueStoreMetrics>,
    ) -> MysResult<TransactionKeyValueStore> {
    ...
    pub fn new(
        ...
        let base_url = Url::parse(&base_url).into_mys_result()?;
    ...
    fn get_url(&self, key: &Key) -> MysResult<Url> {
        let (digest, item_type) = key.to_path_elements();
        let joined = self
            .base_url
            .join(&format!("{}/{}", digest, item_type))
            .into_mys_result()?;
        Url::from_str(joined.as_str()).into_mys_result()
    }

    async fn multi_fetch(&self, uris: Vec<Key>) -> Vec<MysResult<Option<Bytes>>> {
    ...
    async fn fetch(&self, key: Key) -> MysResult<Option<Bytes>> {
        ...
        if resp.status().is_success() {
            let bytes = resp.bytes().await.into_mys_result()?;
...
fn map_fetch<'a, K>(fetch: (&'a MysResult<Option<Bytes>>, &'a K)) -> Option<(&'a Bytes, &'a K)>
where
    K: std::fmt::Debug,
{
    let (fetch, key) = fetch;
