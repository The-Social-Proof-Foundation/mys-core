...
impl ObjectHandler {
    ...
    async fn process_object(
        ...
        let (struct_tag, mys_move_struct) = if let Some(move_struct) = move_struct {
            match move_struct.into() {
                MysMoveStruct::WithTypes { type_, fields } => {
                    (Some(type_), Some(MysMoveStruct::WithFields(fields)))
                }
        ...
        let entry = ObjectEntry {
            ...
            object_json: mys_move_struct.map(|x| x.to_json_value().to_string()),
        };
        state.objects.push(entry);
        Ok(())
    }
