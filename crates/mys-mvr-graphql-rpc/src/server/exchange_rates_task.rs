...
use mys_indexer::apis::{governance_api::exchange_rates, GovernanceReadApi};
        ...
        loop {
            tokio::select! {
                ...
                _ = self.epoch_rx.changed() => {
                    ...
                    let latest_mys_system_state = self.db.inner
                        .get_latest_mys_system_state()
                        .await.map_err(|_| error!("Failed to fetch latest Mys system state"));

                    if let Ok(latest_mys_system_state) = latest_mys_system_state {
                        let db = self.db.clone();
                        let governance_api = GovernanceReadApi::new(db.inner) ;
                        exchange_rates(&governance_api, &latest_mys_system_state)
                            .await
