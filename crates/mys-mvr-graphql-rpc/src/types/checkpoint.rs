...
impl Loader<SeqNumKey> for Db {
    ...
    fn load(&self, keys: &[SeqNumKey]) -> impl Future<Output = Result<HashMap<SeqNumKey, Checkpoint>, Error>> + Send {
        use checkpoints::dsl;
...
impl Loader<DigestKey> for Db {
    ...
    fn load(&self, keys: &[DigestKey]) -> impl Future<Output = Result<HashMap<DigestKey, Checkpoint>, Error>> + Send {
        use checkpoints::dsl;
