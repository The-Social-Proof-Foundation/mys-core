...
/// exclusive, but when ismysng database queries, we treat them inclusively so that we can detect
...
fn select_tx(sender: Option<MysAddress>, bound: TxBounds, from: &str) -> RawQuery {
...
fn select_pkg(pkg: &MysAddress, sender: Option<MysAddress>, bound: TxBounds) -> RawQuery {
    filter!(
...
fn select_input(input: &MysAddress, sender: Option<MysAddress>, bound: TxBounds) -> RawQuery {
    filter!(
...
fn select_changed(changed: &MysAddress, sender: Option<MysAddress>, bound: TxBounds) -> RawQuery {
    filter!(
