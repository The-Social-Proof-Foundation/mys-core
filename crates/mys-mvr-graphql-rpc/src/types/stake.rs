...
use mys_json_rpc_types::{Stake as RpcStakedMys, StakeStatus as RpcStakeStatus};
use mys_types::base_types::MoveObjectType;
use mys_types::governance::StakedMys as NativeStakedMys;
...
pub(crate) enum StakedMysDowncastError {
    NotAStakedMys,
    Bcs(bcs::Error),
}

#[derive(Clone)]
pub(crate) struct StakedMys {
    /// Representation of this StakedMys as a generic Move Object.
    pub super_: MoveObject,

    /// Deserialized representation of the Move Object's contents as a
    /// `0x3::staking_pool::StakedMys`.
    pub native: NativeStakedMys,
}
...
impl StakedMys {
    ...
    async fn pool_id(&self) -> Option<MysAddress> {
...
impl StakedMys {
    /// Query the database for a `page` of Staked MYS. The page uses the same cursor type as is used
    ...
    ) -> Result<Connection<String, StakedMys>, Error> {
        let type_: StructTag = MoveObjectType::staked_mys().into();
        ...
        Object::paginate_subtype(db, page, filter, checkpoint_viewed_at, |object| {
            ...
            let move_object = MoveObject::try_from(&object).map_err(|_| {
                Error::Internal(format!(
                    "Expected {address} to be a StakedMys, but it's not a Move Object.",
                ))
            })?;

            StakedMys::try_from(&move_object).map_err(|_| {
                Error::Internal(format!(
                    "Expected {address} to be a StakedMys, but it is not."
                ))
            })
        })
        .await
    ...
    /// The JSON-RPC representation of a StakedMys so that we can "cheat" to implement fields that
    /// are not yet implemented directly for GraphQL.
    ///
    /// TODO: Make this obsolete
    async fn rpc_stake(&self, ctx: &Context<'_>) -> Result<RpcStakedMys, Error> {
        ctx.data_unchecked::<PgManager>()
            .fetch_rpc_staked_mys(self.native.clone())
            .await
    }
}

impl TryFrom<&MoveObject> for StakedMys {
    type Error = StakedMysDowncastError;

    fn try_from(move_object: &MoveObject) -> Result<Self, Self::Error> {
        if !move_object.native.is_staked_mys() {
            return Err(StakedMysDowncastError::NotAStakedMys);
        }

        Ok(Self {
            super_: move_object.clone(),
            native: bcs::from_bytes(move_object.native.contents())
                .map_err(StakedMysDowncastError::Bcs)?,
        })
    }
}
