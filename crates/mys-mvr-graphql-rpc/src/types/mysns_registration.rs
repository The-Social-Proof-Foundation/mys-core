...
#[graphql(remote = "mys_json_rpc::name_service::DomainFormat")]
...
pub(crate) struct NativeMysnsRegistration {
    pub id: UID,
...
#[derive(Clone)]
pub(crate) struct MysnsRegistration {
    /// Representation of this MysnsRegistration as a generic Move object.
    pub super_: MoveObject,

    /// The deserialized representation of the Move object's contents.
    pub native: NativeMysnsRegistration,
}
...
pub(crate) enum MysnsRegistrationDowncastError {
    NotAMysnsRegistration,
    Bcs(bcs::Error),
}

#[Object]
impl MysnsRegistration {
    pub(crate) async fn address(&self) -> MysAddress {
    ...
    pub(crate) async fn staked_myss(
        &self,
        ctx: &Context<'_>,
        first: Option<u64>,
        after: Option<object::Cursor>,
...
impl NameService {
    /// Lookup the MysNS NameRecord for the given `domain` name. `config` specifies where to find
    ...
    /// Lookup the MysNS Domain for the given `address`. `config` specifies where to find the domain
    ...
    pub(crate) async fn reverse_resolve_to_name(
        ...
        let field: Field<NativeMysAddress, NativeDomain> = object
            .native
            .to_rust()
            .ok_or_else(|| Error::Internal("Malformed Mysns Domain".to_string()))?;
    ...
    async fn query_domain_expiration(
        ...
        // parent's `NameRecord`.
        let mut object_ids = vec![MysAddress::from(config.record_field_id(&domain.0))];
        if domain.0.is_subdomain() {
            object_ids.push(MysAddress::from(config.record_field_id(&domain.0.parent())));
        }
...
impl MysnsRegistration {
    /// Query the database for a `page` of MysNS registrations. The page uses the same cursor type
    ...
    pub(crate) async fn paginate(
        ...
        owner: MysAddress,
        checkpoint_viewed_at: u64,
    ) -> Result<Connection<String, MysnsRegistration>, Error> {
        let type_ = MysnsRegistration::type_(config.package_address.into());
        ...
        Object::paginate_subtype(db, page, filter, checkpoint_viewed_at, |object| {
            ...
            let move_object = MoveObject::try_from(&object).map_err(|_| {
                Error::Internal(format!(
                    "Expected {address} to be a MysnsRegistration, but it's not a Move Object.",
                ))
            })?;

            MysnsRegistration::try_from(&move_object, &type_).map_err(|_| {
                Error::Internal(format!(
                    "Expected {address} to be a MysnsRegistration, but it is not."
                ))
            })
        })
        .await
    ...
    /// Return the type representing a `MysnsRegistration` on chain. This can change from chain to
    /// chain (mainnet, testnet, devnet etc).
    pub(crate) fn type_(package: MysAddress) -> StructTag {
        StructTag {
    ...
    // Because the type of the MysnsRegistration object is not constant,
    // we need to take it in as a param.
    pub(crate) fn try_from(
        move_object: &MoveObject,
        tag: &StructTag,
    ) -> Result<Self, MysnsRegistrationDowncastError> {
        if !move_object.native.is_type(tag) {
            return Err(MysnsRegistrationDowncastError::NotAMysnsRegistration);
        }

        Ok(Self {
            super_: move_object.clone(),
            native: bcs::from_bytes(move_object.native.contents())
                .map_err(MysnsRegistrationDowncastError::Bcs)?,
        })
    }
}
