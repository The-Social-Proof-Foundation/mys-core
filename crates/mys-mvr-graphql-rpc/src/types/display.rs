...
use mys_indexer::{models::display::StoredDisplay, schema::display};
...
pub(crate) fn get_value_from_move_struct(
    ...
    // todo: 10 is a carry-over from the mys-json-rpc implementation
    ...
    // TODO: move off dependency on MysMoveValue
    let mys_move_value: MysMoveValue = result.clone().into();

    match mys_move_value {
        MysMoveValue::Option(move_option) => match move_option.as_ref() {
            Some(move_value) => Ok(move_value.to_string()),
            None => Ok("".to_string()),
        },
        MysMoveValue::Vector(_) => Err(DisplayRenderError::Vector(var_name.to_string())),
        _ => Ok(mys_move_value.to_string()),
    }
}
