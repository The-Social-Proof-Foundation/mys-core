...
use mys_indexer::types::ObjectStatus as NativeObjectStatus;
...
pub(crate) struct ObjectFilter {
    ...
    pub owner: Option<MysAddress>,

    /// Filter for live objects by their IDs.
    pub object_ids: Option<Vec<MysAddress>>,
    ...
    pub(crate) async fn owner(&self) -> Option<ObjectOwner> {
        ...
        match &native.owner {
            O::AddressOwner(address) => {
                let address = MysAddress::from(*address);
...
impl ObjectFilter {
    ...
    /// it).
    fn keys(&self) -> Option<BTreeMap<MysAddress, Option<u64>>> {
...
impl Loader<HistoricalKey> for Db {
    ...
    fn load(&self, keys: &[HistoricalKey]) -> impl Future<Output = Result<HashMap<HistoricalKey, Object>, Error>> + Send {
...
impl Loader<LatestAtKey> for Db {
    ...
    fn load(&self, keys: &[LatestAtKey]) -> impl Future<Output = Result<HashMap<LatestAtKey, Object>, Error>> + Send {
        let self_clone = self.clone();
        let keys_vec = keys.to_vec();
        async move {
...
mod tests {
    ...
    fn test_owner_filter_intersection() {
        let f0 = ObjectFilter {
            owner: Some(MysAddress::from_str("0x1").unwrap()),
            ..Default::default()
        };

        let f1 = ObjectFilter {
            owner: Some(MysAddress::from_str("0x2").unwrap()),
            ..Default::default()
        };
    ...
    fn test_key_filter_intersection() {
        let i1 = MysAddress::from_str("0x1").unwrap();
        let i2 = MysAddress::from_str("0x2").unwrap();
        let i3 = MysAddress::from_str("0x3").unwrap();
        let i4 = MysAddress::from_str("0x4").unwrap();
