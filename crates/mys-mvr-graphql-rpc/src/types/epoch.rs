...
impl Epoch {
    ...
    async fn validator_set(&self, ctx: &Context<'_>) -> Result<Option<ValidatorSet>> {
        let system_state = ctx
            ...
            .fetch_mys_system_state(Some(self.stored.epoch as u64))
            .await?;
...
impl Loader<EpochKey> for Db {
    ...
    fn load(&self, keys: &[EpochKey]) -> impl Future<Output = Result<HashMap<EpochKey, Epoch>, Error>> + Send {
        use epochs::dsl;

        let self_clone = self.clone();
        let keys_vec = keys.to_vec();
        async move {
            let epoch_ids: BTreeSet<_> = keys_vec.iter().map(|key| key.epoch_id as i64).collect();
            let epochs: Vec<QueryableEpochInfo> = self_clone
