...
use mys_types::effects::TransactionEffects as NativeTransactionEffects;
use mys_types::event::Event as NativeEvent;
use mys_types::quorum_driver_types::ExecuteTransactionRequestType;
use mys_types::transaction::SenderSignedData;
use mys_types::{signature::GenericSignature, transaction::Transaction};
pub struct Mutation;
...
impl Mutation {
    ...
    async fn execute_transaction_block(
        ...
        let mys_sdk_client: &Option<MysClient> = ctx
            .data()
            .map_err(|_| Error::Internal("Unable to fetch Mys SDK client".to_string()))
            .extend()?;
        let mys_sdk_client = mys_sdk_client
            .as_ref()
            .ok_or_else(|| Error::Internal("Mys SDK client not initialized".to_string()))
            .extend()?;
