...
use mys_mvr_graphql_rpc::config::ServiceConfig;
use mys_mvr_graphql_rpc::test_infra::cluster::prep_executor_cluster;
use mys_mvr_graphql_rpc::test_infra::cluster::start_cluster;
...
async fn test_simple_client_simulator_cluster() {
    ...
    let cluster = mys_mvr_graphql_rpc::test_infra::cluster::serve_executor(
        Arc::new(sim),
        None,
        None,
...
async fn test_graphql_client_variables() {
    ...
    assert_eq!(
        data.get("obj1")
            ...
            .as_str()
            .unwrap(),
        MYS_FRAMEWORK_ADDRESS.to_canonical_string(true)
    );
    assert_eq!(
        data.get("obj2")
            .unwrap()
...
async fn test_payload_using_vars_mutation_passes() {
    telemetry_subscribers::init_for_testing();
    let cluster = mys_mvr_graphql_rpc::test_infra::cluster::start_cluster(ServiceConfig {
        limits: Limits {
