...
impl<'a, K: DeserializeOwned, V: DeserializeOwned> SafeIter<'a, K, V> {
    ...
    /// Seek to the given key. The iterator will be positioned at the first key >= the given key.
    pub fn seek_to_key(&mut self, key: &[u8]) {
        self.is_initialized = true;
        self.db_iter.seek(key);
    }

    /// Seek to the last key <= the given key (for reverse iteration).
    pub fn seek_for_prev_key(&mut self, key: &[u8]) {
        self.is_initialized = true;
