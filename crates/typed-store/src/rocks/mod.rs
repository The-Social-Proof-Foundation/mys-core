...
use mys_macros::{fail_point, nondeterministic};
#[cfg(tidehunter)]
...
impl<K, V> DBMap<K, V> {
    ...
    /// Alias for `safe_iter_with_bounds` for backward compatibility.
    pub fn iter_with_bounds(
        &self,
        lower_bound: Option<K>,
        upper_bound: Option<K>,
    ) -> crate::DbIteratorWrapper<'_, K, V>
    where
    ...
    /// Returns an iterator over key-value pairs with bounds that unwraps Results for convenience.
    /// This is a convenience method that wraps `safe_iter_with_bounds` and converts it to `UnboundedIter`.
    pub fn unbounded_iter_with_bounds<'a>(
        &'a self,
        lower_bound: Option<K>,
        upper_bound: Option<K>,
    ) -> crate::UnboundedIter<'a, K, V>
    where
        K: Serialize + DeserializeOwned + 'a,
        V: Serialize + DeserializeOwned + 'a,
    {
        crate::UnboundedIter::from_wrapper(self, self.safe_iter_with_bounds(lower_bound, upper_bound))
    }

    /// Returns an iterator over key-value pairs with a range that unwraps Results for convenience.
    /// This is a convenience method that wraps `safe_range_iter` and converts it to `UnboundedIter`.
    pub fn unbounded_range_iter<'a>(
        &'a self,
    ...
    where
        ...
        V: Serialize + DeserializeOwned + 'a,
    {
        crate::UnboundedIter::from_wrapper(self, self.safe_range_iter(range))
    }

    /// Clear all entries in the map.
    /// This is a safe operation that schedules deletion of all keys.
    pub fn unsafe_clear(&self) -> Result<(), TypedStoreError>
    where
        K: Serialize + DeserializeOwned,
        V: Serialize + DeserializeOwned,
    {
        self.schedule_delete_all()
    }

    /// Get all values from the map.
    pub fn values(&self) -> Result<Vec<V>, TypedStoreError>
    where
        K: Serialize + DeserializeOwned,
        V: Serialize + DeserializeOwned,
    {
    ...
    /// Get raw bytes for multiple keys.
    pub fn multi_get_raw_bytes<J>(
        &self,
    ...
    /// Get a reference to the underlying RocksDB instance if available.
    pub fn rocksdb(&self) -> Option<&rocksdb::DBWithThreadMode<MultiThreaded>>
    where
        K: Serialize + DeserializeOwned,
        V: Serialize + DeserializeOwned,
    {
        match &self.db.storage {
            Storage::Rocks(db) => Some(&db.underlying),
            _ => None,
        }
    }

    /// Get the database path if available.
    pub fn path(&self) -> Option<&Path> {
        match &self.db.storage {
            Storage::Rocks(db) => Some(db.underlying.path()),
            _ => None,
        }
    }

    /// Get live files from RocksDB if available.
    pub fn live_files(&self) -> Result<Vec<LiveFile>, TypedStoreError>
    where
        K: Serialize + DeserializeOwned,
        V: Serialize + DeserializeOwned,
    {
        match &self.db.storage {
            Storage::Rocks(db) => {
                let _cf = rocks_cf_from_db(&self.db, &self.cf)?;
                db.underlying
                    .live_files()
                    .map_err(|e| TypedStoreError::RocksDBError(e.to_string()))
            }
            _ => Err(TypedStoreError::RocksDBError(
                "live_files is only available for RocksDB storage".to_string(),
            )),
        }
    }
    ...
    /// Create a transaction handle that wraps a batch and provides a transaction-like API.
    /// Operations can be batched and committed atomically using `commit()`.
    pub fn transaction(&self) -> Result<DBTransaction<K, V>, TypedStoreError> {
        Ok(DBTransaction {
            batch: self.batch(),
            _phantom: PhantomData,
        })
...
impl DBBatch {
    ...
    /// Alias for `partial_merge_batch` for backward compatibility.
    pub fn merge_batch<J: Borrow<K>, K: Serialize, U: Borrow<V>, V: Serialize>(
        &mut self,
        db_map: &DBMap<K, V>,
        new_vals: impl IntoIterator<Item = (J, U)>,
    ) -> Result<&mut Self, TypedStoreError> {
        self.partial_merge_batch(db_map, new_vals)
    }

    /// Merge batch operation that accepts raw bytes directly (for reference counting).
    /// This is useful when you need to pass raw bytes that will be processed by a merge operator
    /// without BCS serialization.
    pub fn partial_merge_batch_raw<K: Serialize, V>(
        &mut self,
        db: &DBMap<K, V>,
        new_vals: impl IntoIterator<Item = (K, Vec<u8>)>,
    ) -> Result<&mut Self, TypedStoreError> {
        if !Arc::ptr_eq(&db.db, &self.database) {
            return Err(TypedStoreError::CrossDBBatch);
        }
...
/// A transaction handle that wraps a batch and provides a transaction-like API.
/// This allows grouping multiple operations that will be committed atomically.
...
impl<K, V> DBTransaction<K, V> {
    /// Get a value from the database (reads from the underlying DBMap, not from pending batch writes).
    pub fn get(&self, db_map: &DBMap<K, V>, key: &K) -> Result<Option<V>, TypedStoreError>
    where
        K: Serialize + DeserializeOwned,
        V: Serialize + DeserializeOwned,
    {
    ...
    /// Insert a batch of key-value pairs into the transaction.
    pub fn insert_batch<J, U>(
        &mut self,
        db_map: &DBMap<K, V>,
    ...
    where
        ...
        V: Serialize + DeserializeOwned,
    {
        self.batch.insert_batch(db_map, key_val_pairs)?;
        Ok(())
    }

    /// Commit the transaction, writing all batched operations atomically.
    ...
    V: Serialize + DeserializeOwned + 'a,
{
    ...
    type Iterator = crate::UnboundedIter<'a, K, V>;
    ...
    fn safe_iter(&'a self) -> crate::DbIteratorWrapper<'a, K, V> {
        let iter = match &self.db.storage {
            Storage::Rocks(db) => {
                let db_iter = db
                    .underlying
        ...
        crate::DbIteratorWrapper::new(iter)
    }
    ...
    fn safe_range_iter(&'a self, range: impl RangeBounds<K>) -> crate::DbIteratorWrapper<'a, K, V> {
    ...
    /// Returns an iterator over all key-value pairs without bounds.
    /// This returns an iterator that unwraps Results for convenience.
    fn unbounded_iter(&'a self) -> crate::UnboundedIter<'a, K, V> {
        // For RocksDB, create SafeIter directly to enable efficient seeking (matching original Iter pattern)
        match &self.db.storage {
            Storage::Rocks(db) => {
                let db_iter = db
                    .underlying
                ...
                crate::UnboundedIter::new_with_safe_iter(self, safe_iter)
            }
            _ => {
                // For non-RocksDB backends, use the trait object
                crate::UnboundedIter::from_wrapper(self, self.safe_iter())
            }
        }
    }
...
/// Opens a database with transactional options enabled, and a number of column families with individual options that are created if they do not exist.
/// This is similar to `open_cf_opts` but enables RocksDB transactional support.
#[instrument(level="debug", skip_all, fields(path = ?path.as_ref()), err)]
pub fn open_cf_opts_transactional<P: AsRef<Path>>(
    ...
    nondeterministic!({
        ...
        options.create_if_missing(true);
        options.create_missing_column_families(true);
        // Note: Full transactional support in RocksDB requires TransactionDB,
        // which is not currently used in this codebase. This function provides
        // the same interface as open_cf_opts for compatibility.
