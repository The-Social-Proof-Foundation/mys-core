...
pub mod traits;
pub use traits::{DbIterator, DbIteratorWrapper, Map};
pub mod iter_ext;
pub use iter_ext::{DbIteratorExt, IteratorExt, SkipPriorToIter, SkipToIter, UnboundedIter, UnwrapIter};
pub mod memstore;
pub mod metrics;
pub mod rocks;
#[cfg(tidehunter)]
...
/// Macro to simplify reopening multiple column families from a database.
/// Usage: `reopen!(&db, CF_NAME;<K, V>, CF_NAME2;<K2, V2>, ...)`
#[macro_export]
macro_rules! reopen {
    ($db:expr, $($cf:expr; <$k:ty, $v:ty>),+ $(,)?) => {
        (
            $(
                $crate::rocks::DBMap::<$k, $v>::reopen(
                    $db,
                    Some($cf),
                    &$crate::rocks::ReadWriteOptions::default(),
                    false,
