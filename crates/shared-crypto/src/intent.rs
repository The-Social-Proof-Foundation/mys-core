...
/// (i.e., Narwhal, Mys, Ethereum etc) should never collide, so that even when a signing
...
impl Intent {
    pub fn mys_app(scope: IntentScope) -> Self {
        Self {
...
/// be any type that implements [trait Serialize]. *ALL* signatures in Mys must commits
...
/// A 1-byte domain separator for hashing Object ID in Mys. It is starting from 0xf0
/// to ensure no hashing collision for any ObjectID vs MysAddress which is derived
/// as the hash of `flag || pubkey`. See `mys_types::crypto::SignatureScheme::flag()`.
#[derive(Serialize_repr, Deserialize_repr, Copy, Clone, PartialEq, Eq, Debug, Hash)]
#[repr(u8)]
