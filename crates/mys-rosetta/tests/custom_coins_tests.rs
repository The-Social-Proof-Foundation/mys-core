...
async fn test_custom_coin_balance() {
    ...
    const MYS_BALANCE: u64 = 150_000_000_000_000_000;
    ...
    let request = AccountBalanceRequest {
        ...
        currencies: Currencies(vec![mys_currency, test_coin_currency]),
    };
    ...
    assert_eq!(response.balances[0].value, MYS_BALANCE as i128);
    assert_eq!(
...
async fn test_custom_coin_without_symbol() {
    ...
    for op in ops {
        if op.type_ == OperationType::MysBalanceChange {
