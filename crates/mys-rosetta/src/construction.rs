...
pub async fn derive(
    Extension(env): Extension<MysEnv>,
    ...
    let address: MysAddress = request.public_key.try_into()?;
...
pub async fn submit(
    ...
    let dry_run = context
        ...
        .await?;
    if let MysExecutionStatus::Failure { error } = dry_run.effects.status() {
    ...
    if let MysExecutionStatus::Failure { error } = response
        .effects
...
/// For Mys, we are returning the latest object refs for all the input objects,
...
pub async fn metadata(
    ...
    let (total_required_amount, objects) = match &option.internal_operation {
        InternalOperation::PayMys { amounts, .. } => {
        ...
        InternalOperation::WithdrawStake { sender, stake_ids } => {
            ...
            let responses = context
                .client
                .read_api()
                .multi_get_object_with_options(stake_ids, MysObjectDataOptions::default())
                .await?;
    ...
    // If required amount is None (all MYS) or failed to select coin (might not have enough MYS), select all coins.
