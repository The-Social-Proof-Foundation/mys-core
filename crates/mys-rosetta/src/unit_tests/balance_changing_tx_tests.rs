...
async fn test_transfer_mys() {
    ...
    let pt = {
        ...
        builder.transfer_mys(recipient, Some(50000));
        builder.finish()
    };
    test_transaction(
        &client,
...
#[tokio::test]
async fn test_transfer_mys_whole_coin() {
    let network = TestClusterBuilder::new().build().await;
    let client = network.wallet.get_client().await.unwrap();
    let keystore = &network.wallet.config.keystore;
    let rgp = network.get_reference_gas_price().await;

    // Test transfer mys whole coin
    let addresses = network.get_addresses();
    let sender = get_random_address(&addresses, vec![]);
    let recipient = get_random_address(&addresses, vec![sender]);
    let pt = {
        let mut builder = ProgrammableTransactionBuilder::new();
        builder.transfer_mys(recipient, None);
        builder.finish()
    };
    test_transaction(
        &client,
...
async fn test_transfer_object() {
    ...
    let sender = get_random_address(&addresses, vec![]);
    let recipient = get_random_address(&addresses, vec![sender]);
    let object_ref = get_random_mys(&client, sender, vec![]).await;
    let pt = {
        let mut builder = ProgrammableTransactionBuilder::new();
...
async fn test_split_coin() {
    ...
    let coin = get_random_mys(&client, sender, vec![]).await;
    let tx = client
        .transaction_builder()
...
async fn test_merge_coin() {
    ...
    let sender = get_random_address(&network.get_addresses(), vec![]);
    let coin = get_random_mys(&client, sender, vec![]).await;
    let coin2 = get_random_mys(&client, sender, vec![coin.0]).await;
    let tx = client
        .transaction_builder()
...
async fn test_pay_multiple_coin_multiple_recipient() {
    ...
    let coin1 = get_random_mys(&client, sender, vec![]).await;
    let coin2 = get_random_mys(&client, sender, vec![coin1.0]).await;
    let pt = {
        let mut builder = ProgrammableTransactionBuilder::new();
        builder
            .pay(
...
#[tokio::test]
async fn test_pay_mys_multiple_coin_same_recipient() {
    let network = TestClusterBuilder::new().build().await;
    let client = network.wallet.get_client().await.unwrap();
    let keystore = &network.wallet.config.keystore;
    let rgp = network.get_reference_gas_price().await;
...
async fn test_pay_mys() {
    ...
    let pt = {
        let mut builder = ProgrammableTransactionBuilder::new();
        builder
            .pay_mys(vec![recipient1, recipient2], vec![1000000, 2000000])
            .unwrap();
        builder.finish()
    };
    test_transaction(
...
#[tokio::test]
async fn test_stake_mys_with_none_amount() {
    let network = TestClusterBuilder::new().build().await;
    let client = network.wallet.get_client().await.unwrap();
    let keystore = &network.wallet.config.keystore;
    let rgp = network.get_reference_gas_price().await;
...
async fn test_delegation_parsing() -> Result<(), anyhow::Error> {
    ...
    let client = network.wallet.get_client().await.unwrap();
    let sender = get_random_address(&network.get_addresses(), vec![]);
    let gas = get_random_mys(&client, sender, vec![]).await;
    let validator = client
        .governance_api()
        .get_latest_mys_system_state()
        .await
...
// Record current Mys balance of an address then execute the transaction,
// and compare the balance change reported by the event against the actual balance change.
async fn test_transaction(
    client: &MysClient,
    keystore: &Keystore,
    addr_to_check: Vec<MysAddress>,
    sender: MysAddress,
    ...
    let gas = if !gas.is_empty() {
        ...
        let input_objects = tx
            ...
            .collect::<Vec<_>>();
        vec![get_random_mys(client, sender, input_objects).await]
    };
    ...
    let signature = keystore
        .sign_secure(&data.sender(), &data, Intent::mys_transaction())
        .unwrap();
...
fn extract_balance_changes_from_ops(ops: Operations) -> HashMap<MysAddress, i128> {
    ops.into_iter()
        .fold(HashMap::<MysAddress, i128>::new(), |mut changes, op| {
            if let Some(OperationStatus::Success) = op.status {
                match op.type_ {
                    OperationType::MysBalanceChange
                    | OperationType::Gas
                    | OperationType::PayMys
...
fn get_random_address(addresses: &[MysAddress], except: Vec<MysAddress>) -> MysAddress {
    *addresses
        .iter()
...
async fn get_balance(client: &MysClient, address: MysAddress) -> u64 {
    let coins = client
        .read_api()
        .get_owned_objects(
            address,
