...
#[derive(Parser)]
#[clap(name = "mys-rosetta", rename_all = "kebab-case", author, version)]
        ...
        match self {
            RosettaServerCommand::GenerateRosettaCLIConfig {
                ...
                let path = keystore_path
                    .unwrap_or_else(|| mys_config_dir().unwrap().join(MYS_KEYSTORE_FILENAME));

                let prefunded_accounts = read_prefunded_account(&path)?;

                info!(
                    "Retrieved {} Mys address from keystore file {:?}",
                ...
                let dsl_path = PathBuf::from(".").join("mys.ros");
                let dsl = include_str!("../resources/mys.ros");
                fs::write(
                    &dsl_path,
                    dsl.replace("{{mys.env}}", json!(env).as_str().unwrap()),
                )?;
            ...
            RosettaServerCommand::StartOnlineRemoteServer {
                ...
                info!(
                    "Starting Rosetta Online Server with remove Mys full node [{full_node_url}]."
                );
                let mys_client = wait_for_mys_client(full_node_url).await;
                let rosetta_path = data_path.join("rosetta_db");
                info!("Rosetta db path : {rosetta_path:?}");
                let rosetta = RosettaOnlineServer::new(env, mys_client);
            ...
            RosettaServerCommand::StartOnlineServer {
                ...
                info!("Starting Rosetta Online Server with embedded Mys full node.");
                info!("Data directory path: {data_path:?}");

                let node_config = node_config.unwrap_or_else(|| {
                    let path = mys_config_dir().unwrap().join(MYS_FULLNODE_CONFIG);
                    info!("Using default node config from {path:?}");
                    path
                });

                let mut config = NodeConfig::load(&node_config)?;
                config.db_path = data_path.join("mys_db");
                info!("Overriding Mys db path to : {:?}", config.db_path);
                ...
                let _node = MysNode::start(config, registry_service, None).await?;

                let mys_client = wait_for_mys_client(rpc_address).await;
...
async fn wait_for_mys_client(rpc_address: String) -> MysClient {
    loop {
        match MysClientBuilder::default().build(&rpc_address).await {
            Ok(client) => return client,
            Err(e) => {
                warn!("Error connecting to Mys RPC server [{rpc_address}]: {e}, retrying in 5 seconds.");
...
/// This method reads the keypairs from the Mys keystore to create the PrefundedAccount objects,
...
fn read_prefunded_account(path: &Path) -> Result<Vec<PrefundedAccount>, anyhow::Error> {
    ...
    let keys = kp_strings
        .iter()
        .map(|kpstr| {
            let key = MysKeyPair::decode_base64(kpstr);
            key.map(|k| (MysAddress::from(&k.public()), k))
        })
