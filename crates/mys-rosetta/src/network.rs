...
pub async fn list(Extension(env): Extension<MysEnv>) -> Result<NetworkListResponse, Error> {
...
pub async fn status(
    ...
    let peers = system_state
        ...
        .iter()
        .map(|validator| Peer {
            peer_id: ObjectID::from(validator.mys_address).into(),
            metadata: Some(json!({
                "public_key": Hex::from_bytes(&validator.protocol_pubkey_bytes),
                "stake_amount": validator.staking_pool_mys_balance,
            })),
        })
        .collect();
