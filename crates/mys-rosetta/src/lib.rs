...
impl RosettaOnlineServer {
    pub fn new(env: MysEnv, client: MysClient) -> Self {
    ...
    pub async fn serve(self, addr: SocketAddr) {
        ...
        info!(
            "Mys Rosetta online server listening on {}",
...
impl RosettaOfflineServer {
    ...
    pub async fn serve(self, addr: SocketAddr) {
        ...
        info!(
            "Mys Rosetta offline server listening on {}",
            listener.local_addr().unwrap()
        );
        axum::serve(listener, app).await.unwrap();
    }
...
impl CoinMetadataCache {
    pub fn new(client: MysClient, size: NonZeroUsize) -> Self {
        Self {
            client,
