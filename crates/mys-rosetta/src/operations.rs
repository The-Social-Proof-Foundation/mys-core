...
use mys_json_rpc_types::MysProgrammableMoveCall;
use mys_json_rpc_types::MysProgrammableTransactionBlock;
use mys_json_rpc_types::{BalanceChange, MysArgument};
use mys_json_rpc_types::{MysCallArg, MysCommand};
...
use crate::{CoinMetadataCache, Error, MYS};
...
impl Operations {
    ...
    pub fn into_internal(self) -> Result<InternalOperation, Error> {
        ...
        match type_ {
            OperationType::PayMys => self.pay_mys_ops_to_internal(),
    ...
    fn pay_mys_ops_to_internal(self) -> Result<InternalOperation, Error> {
    ...
    fn stake_ops_to_internal(self) -> Result<InternalOperation, Error> {
        ...
        // Total issued MySo is less than u64, safe to cast.
    ...
    fn from_transaction(
        tx: MysTransactionBlockKind,
        sender: MysAddress,
        status: Option<OperationStatus>,
    ) -> Result<Vec<Operation>, Error> {
        Ok(match tx {
            MysTransactionBlockKind::ProgrammableTransaction(pt)
    ...
    fn parse_programmable_transaction(
        sender: MysAddress,
        status: Option<OperationStatus>,
        pt: MysProgrammableTransactionBlock,
    ) -> Result<Vec<Operation>, Error> {
        #[derive(Debug)]
        enum KnownValue {
            GasCoin(u64),
            ...
            let amounts = amounts
                ...
                .map(|amount| {
                    let value: u64 = match *amount {
                        ...
                        MysArgument::GasCoin
                        | MysArgument::Result(_)
                        | MysArgument::NestedResult(_, _) => return None,
                    };
        ...
        fn transfer_object(
            aggregated_recipients: &mut HashMap<MysAddress, u64>,
            inputs: &[MysCallArg],
            known_results: &[Vec<KnownValue>],
            objs: &[MysArgument],
            recipient: MysArgument,
        ) -> Option<Vec<KnownValue>> {
            let addr = match recipient {
                MysArgument::Input(i) => inputs.get(i as usize)?.pure()?.to_mys_address().ok()?,
                MysArgument::GasCoin | MysArgument::Result(_) | MysArgument::NestedResult(_, _) => {
                    return None
                }
            };
            for obj in objs {
                let value = match *obj {
                    ...
                    MysArgument::GasCoin | MysArgument::Input(_) => return None,
                };
        ...
        fn stake_call(
            inputs: &[MysCallArg],
            known_results: &[Vec<KnownValue>],
            call: &MysProgrammableMoveCall,
        ) -> Result<Option<(Option<u64>, MysAddress)>, Error> {
            let MysProgrammableMoveCall { arguments, .. } = call;
            let (amount, validator) = match &arguments[..] {
                [_, coin, validator] => {
                    ...
                    let (some_amount, validator) = match validator {
                        ...
                        MysArgument::Input(i) => (*i==1, inputs.get(*i as usize).and_then(|input| input.pure()).map(|v|v.to_mys_address()).transpose()),
                        _=> return Ok(None),
                    };
        ...
        let MysProgrammableTransactionBlock { inputs, commands } = &pt;
        let mut known_results: Vec<Vec<KnownValue>> = vec![];
        let mut aggregated_recipients: HashMap<MysAddress, u64> = HashMap::new();
        ...
        for command in commands {
            let result = match command {
                ...
                MysCommand::TransferObjects(objs, addr) => transfer_object(
                ...
                MysCommand::MoveCall(m) if Self::is_stake_call(m) => {
                ...
                MysCommand::MoveCall(m) if Self::is_unstake_call(m) => {
                ...
                aggregated_recipients
                    ...
                    .map(|(recipient, amount)| {
                        currency = inputs.iter().last().and_then(|arg| {
                            if let MysCallArg::Pure(value) = arg {
                                let bytes = value
                                    .value()
                                    .to_json_value()
                                    .as_array()?
                        ...
                        match currency {
                            ...
                            None => Operation::pay_mys(status, recipient, amount.into()),
                        }
                    }),
            );
            match currency {
                ...
                _ => operations.push(Operation::pay_mys(status, sender, -(total_paid as i128))),
            }
    ...
    fn is_stake_call(tx: &MysProgrammableMoveCall) -> bool {
        tx.package == MYS_SYSTEM_PACKAGE_ID
            && tx.module == MYS_SYSTEM_MODULE_NAME.as_str()
            && tx.function == ADD_STAKE_FUN_NAME.as_str()
    }

    fn is_unstake_call(tx: &MysProgrammableMoveCall) -> bool {
        tx.package == MYS_SYSTEM_PACKAGE_ID
            && tx.module == MYS_SYSTEM_MODULE_NAME.as_str()
    ...
    fn process_balance_change(
        gas_owner: MysAddress,
        gas_used: i128,
        balance_changes: Vec<(BalanceChange, Currency)>,
        status: Option<OperationStatus>,
        balances: HashMap<(MysAddress, Currency), i128>,
        ...
        *balances.entry((gas_owner, MYS.clone())).or_default() -= gas_used;
...
impl Operations {
    pub async fn try_from_response(
        ...
        let staking_balance = if principal_amounts != 0 {
            *accounted_balances.entry((sender, MYS.clone())).or_default() -= principal_amounts;
            *accounted_balances.entry((sender, MYS.clone())).or_default() -= reward_amounts;
            vec![
        ...
        // This is a workaround for the payCoin cases that are mistakenly considered to be payMys operations
        // In this case we remove any irrelevant, MYS specific operation entries that sum up to 0 balance changes per address
        ...
        let mutually_cancelling_balances: HashMap<_, _> = ops
            ...
            .fold(
                HashMap::new(),
                |mut balances: HashMap<(MysAddress, Currency), i128>, op| {
                    if let (Some(acc), Some(amount), Some(OperationStatus::Success)) =
                        (&op.account, &op.amount, &op.status)
                    {
...
fn is_unstake_event(tag: &StructTag) -> bool {
    tag.address == MYS_SYSTEM_ADDRESS
...
impl TryFrom<TransactionData> for Operations {
    ...
    fn try_from(data: TransactionData) -> Result<Self, Self::Error> {
        ...
        Ok(Operations::try_from_data(
            MysTransactionBlockData::try_from(data, &&mut NoOpsModuleResolver)?,
            None,
        )?)
    }
}
...
pub enum OperationMetadata {
    GenericTransaction(MysTransactionBlockKind),
    Stake { validator: MysAddress },
...
impl Operation {
    ...
    pub fn genesis(index: u64, sender: MysAddress, coin: GasCoin) -> Self {
        Operation {
    ...
    fn pay_mys(status: Option<OperationStatus>, address: MysAddress, amount: i128) -> Self {
        Operation {
            operation_identifier: Default::default(),
            type_: OperationType::PayMys,
            status,
    ...
    fn balance_change(
        ...
        Self {
            operation_identifier: Default::default(),
            type_: OperationType::MysBalanceChange,
            status,
    ...
    fn gas(addr: MysAddress, amount: i128) -> Self {
        Self {
            operation_identifier: Default::default(),
    ...
    fn stake_reward(status: Option<OperationStatus>, addr: MysAddress, amount: i128) -> Self {
        Self {
            operation_identifier: Default::default(),
    ...
    fn stake_principle(status: Option<OperationStatus>, addr: MysAddress, amount: i128) -> Self {
        Self {
            operation_identifier: Default::default(),
