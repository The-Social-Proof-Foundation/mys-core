...
use mys_move_build::{CompiledPackage, MysPackageHooks};
...
fn test_metered_move_bytecode_verifier() {
    ...
    let path =
        PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("../mys-framework/packages/mys-framework");
    ...
    let mut meter = MysVerifierMeter::new(meter_config.clone());
    ...
    let mut meter = MysVerifierMeter::new(meter_config);
    let timer_start = Instant::now();
    let r = run_metered_move_bytecode_verifier(
        &compiled_modules,
        &verifier_config,
