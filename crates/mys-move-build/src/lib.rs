...
pub mod test_utils {
    use crate::{BuildConfig, CompiledPackage, MysPackageHooks};
    ...
    pub fn compile_managed_coin_package() -> CompiledPackage {
        compile_example_package("../../crates/mys-core/src/unit_tests/data/managed_coin")
    }
...
/// Wrapper around the core Move `CompiledPackage` with some Mys-specific traits and info
#[derive(Debug, Clone)]
...
/// Wrapper around the core Move `BuildConfig` with some Mys-specific info
#[derive(Clone)]
...
impl BuildConfig {
    ...
    /// If we are building the Mys framework, we skip the check that the addresses should be 0
    pub fn build(self, path: &Path) -> MysResult<CompiledPackage> {
    ...
    pub fn resolution_graph(
    ...
    ) -> MysResult<ResolvedGraph> {
        if let Some(err_msg) = set_mys_flavor(&mut self.config) {
            return Err(MysError::ModuleBuildFailure { error: err_msg });
        }
        ...
        .map_err(|err| MysError::ModuleBuildFailure {
            error: format!("{:?}", err),
        })
    }
}
...
/// Sets build config's default flavor to `Flavor::Mys`. Returns error message if the flavor was
/// previously set to something else than `Flavor::Mys`.
pub fn set_mys_flavor(build_config: &mut MoveBuildConfig) -> Option<String> {
    use move_compiler::editions::Flavor;

    let flavor = build_config.default_flavor.get_or_insert(Flavor::Mys);
    if flavor != &Flavor::Mys {
        return Some(format!(
...
pub fn build_from_resolution_graph(
        ...
        for module in modules {
            let module =
                CompiledModule::deserialize_with_defaults(module.as_ref()).map_err(|error| {
                    MysError::ModuleDeserializationFailure {
                        error: format!(
                            "Deserializing bytecode dependency for package {}: {:?}",
                            name, error
                        ),
    ...
    let (package, fn_info) = match result {
        Err(error) => {
            return Err(MysError::ModuleBuildFailure {
                error: format!("{:?}", error),
            })
        }
    ...
    if run_bytecode_verifier {
        ...
        for m in compiled_modules.iter_modules() {
            move_bytecode_verifier::verify_module_unmetered(m).map_err(|err| {
                MysError::ModuleVerificationFailure {
                    error: err.to_string(),
                }
            })?;
            mys_bytecode_verifier::mys_verify_module_unmetered(m, &fn_info, &verifier_config)?;
        }
        // TODO(https://github.com/MystenLabs/mys/issues/69): Run Move linker
    }
    Ok(CompiledPackage {
        package,
        published_at,
...
impl CompiledPackage {
    ...
    /// Note: these are not topologically sorted by dependency--use `get_dependency_sorted_modules` to produce a list of modules ttttable
    ...
    pub fn get_dependency_sorted_modules(
        ...
        if with_unpublished_deps {
            ...
            // sufficient as we may have modules with the same names in user code and in Mys
    ...
    pub fn get_mys_system_modules(&self) -> impl Iterator<Item = &CompiledModule> {
        self.get_modules_and_deps()
            .filter(|m| *m.self_id().address() == MYS_SYSTEM_ADDRESS)
    }

    /// Get bytecode modules from the Mys Framework that are used by this package
    pub fn get_mys_framework_modules(&self) -> impl Iterator<Item = &CompiledModule> {
        self.get_modules_and_deps()
            .filter(|m| *m.self_id().address() == MYS_FRAMEWORK_ADDRESS)
    }
    ...
    pub fn verify_unpublished_dependencies(
        ...
        Err(MysError::ModulePublishFailure {
...
impl Default for BuildConfig {
    fn default() -> Self {
        let config = MoveBuildConfig {
            default_flavor: Some(move_compiler::editions::Flavor::Mys),
...
pub struct MysPackageHooks;

impl PackageHooks for MysPackageHooks {
...
pub fn check_unpublished_dependencies(unpublished: &BTreeSet<Symbol>) -> Result<(), MysError> {
...
pub fn check_invalid_dependencies(invalid: &BTreeMap<Symbol, String>) -> Result<(), MysError> {
    if invalid.is_empty() {
        return Ok(());
    }
