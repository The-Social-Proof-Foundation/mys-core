...
use mys_types::crypto::{
    ...
    AuthoritySignature, DefaultHash, MysAuthoritySignature,
};
...
impl Builder {
    ...
    pub fn add_validator_signature(mut self, keypair: &AuthorityKeyPair) -> Self {
        ...
        let checkpoint_signature = {
            let intent_msg = IntentMessage::new(
                Intent::mys_app(IntentScope::CheckpointSummary),
                checkpoint.clone(),
            );
        ...
        let token_distribution_schedule =
            if let Some(token_distribution_schedule) = &self.token_distribution_schedule {
                ...
                TokenDistributionSchedule::new_for_validators_with_default_allocation(
                    validators.iter().map(|v| v.info.mys_address()),
                )
            };
    ...
    fn committee(objects: &[Object]) -> Committee {
        let mys_system_object =
            get_mys_system_state(&objects).expect("Mys System State object must always exist");
        mys_system_object
    ...
    fn validate_inputs(&self) -> anyhow::Result<(), anyhow::Error> {
        ...
        if let Some(token_distribution_schedule) = &self.token_distribution_schedule {
            ...
            token_distribution_schedule.check_all_stake_operations_are_for_valid_validators(
                self.validators.values().map(|v| v.info.mys_address()),
            );
        }
    ...
    fn validate_output(&self) {
        ...
        let system_state = match unsigned_genesis.mys_system_object() {
            MysSystemState::V1(inner) => inner,
            MysSystemState::V2(_) => unreachable!(),
            #[cfg(msim)]
            _ => {
            ...
            .zip(system_state.validators.active_validators.iter())
        {
            ...
            assert!(address_to_pool_id
                .insert(metadata.mys_address, onchain_validator.staking_pool.id)
                .is_none());
            assert_eq!(validator.info.mys_address(), metadata.mys_address);
            assert_eq!(validator.info.protocol_key(), metadata.mys_pubkey_bytes());
        ...
        let mut gas_objects: BTreeMap<ObjectID, (&Object, GasCoin)> = unsigned_genesis
            ...
            .collect();
        let mut staked_mys_objects: BTreeMap<ObjectID, (&Object, StakedMys)> = unsigned_genesis
            .objects()
            .iter()
            .filter_map(|o| StakedMys::try_from(o).ok().map(|s| (o.id(), (o, s))))
            .collect();
        ...
        for allocation in token_distribution_schedule.allocations {
            if let Some(staked_with_validator) = allocation.staked_with_validator {
                ...
                let staked_mys_object_id = staked_mys_objects
                    .iter()
                    .find(|(_k, (o, s))| {
                ...
                let staked_mys_object = staked_mys_objects.remove(&staked_mys_object_id).unwrap();
                assert_eq!(
                    staked_mys_object.0.owner,
                    Owner::AddressOwner(allocation.recipient_address)
                );
                assert_eq!(staked_mys_object.1.principal(), allocation.amount_mist);
                assert_eq!(staked_mys_object.1.pool_id(), staking_pool_id);
                assert_eq!(staked_mys_object.1.activation_epoch(), 0);
            } else {
        ...
        if !self.parameters.allow_insertion_of_extra_objects {
            ...
            assert!(staked_mys_objects.is_empty());
        }
...
fn get_genesis_protocol_config(version: ProtocolVersion) -> ProtocolConfig {
    ...
    // MYS_PROTOCOL_CONFIG_CHAIN_OVERRIDE if necessary
...
fn build_unsigned_genesis_data(
    ...
    token_distribution_schedule.validate();
    token_distribution_schedule.check_all_stake_operations_are_for_valid_validators(
        genesis_validators.iter().map(|v| v.mys_address),
    );
    ...
    let mut system_packages =
        mys_framework_snapshot::load_bytecode_snapshot(parameters.protocol_version.as_u64())
...
// on `mys_system` which is mocked in some tests, but would be in the loader
    ...
    let genesis_transaction = {
        let genesis_objects = objects
            ...
            .map(|mut object| {
                ...
                mys_types::transaction::GenesisObject::RawObject {
                    data: object.data,
                    owner: object.owner,
                }
            })
        ...
        mys_types::transaction::VerifiedTransaction::new_genesis_transaction(genesis_objects)
            .into_inner()
    };
...
pub fn generate_genesis_system_object(
    ...
    let pt = {
        let mut builder = ProgrammableTransactionBuilder::new();
        // Step 1: Create the MysSystemState UID
        let mys_system_state_uid = builder.programmable_move_call(
            MYS_FRAMEWORK_ADDRESS.into(),
            ident_str!("object").to_owned(),
            ident_str!("mys_system_state").to_owned(),
            vec![],
            vec![],
        );
        ...
        if protocol_config.enable_bridge() {
            let bridge_uid = builder
                .input(CallArg::Pure(UID::new(MYS_BRIDGE_OBJECT_ID).to_bcs_bytes()))
                .unwrap();
            // TODO(bridge): this needs to be passed in as a parameter for next testnet regenesis
            // Hardcoding chain id to MysCustom
            let bridge_chain_id = builder.pure(BridgeChainId::MysCustom).unwrap();
        ...
        // Step 5: Run genesis.
        // The first argument is the system state uid we got from step 1 and the second one is the MYS supply we
        // got from step 3.
        let mut arguments = vec![mys_system_state_uid, mys_supply];
    ...
    {
        let object = written.get_mut(&mys_types::MYS_CLOCK_OBJECT_ID).unwrap();
        object
            .data
            .try_as_move_mut()
            .unwrap()
...
mod test {
    ...
    use mys_config::node::DEFAULT_COMMISSION_RATE;
