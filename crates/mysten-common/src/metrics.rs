...
pub struct MetricsPushClient {
    certificate: std::sync::Arc<mys_tls::SelfSignedCertificate>,
    client: reqwest::Client,
}

impl MetricsPushClient {
    pub fn new(metrics_key: mys_types::crypto::NetworkKeyPair) -> Self {
        use fastcrypto::traits::KeyPair;
        let certificate = std::sync::Arc::new(mys_tls::SelfSignedCertificate::new(
            metrics_key.private(),
            mys_tls::MYS_VALIDATOR_SERVER_NAME,
        ));
    ...
    pub fn certificate(&self) -> &mys_tls::SelfSignedCertificate {
        &self.certificate
    }
