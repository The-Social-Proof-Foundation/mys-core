...
use mys_sdk::rpc_types::{
    get_new_package_obj_from_response, MysObjectDataOptions, MysTransactionBlockEffectsAPI,
...
impl TestTransactionBuilder {
    pub fn new(sender: MysAddress, gas_object: ObjectRef, gas_price: u64) -> Self {
        Self {
    ...
    pub fn call_staking(self, stake_coin: ObjectRef, validator: MysAddress) -> Self {
        self.move_call(
            MYS_SYSTEM_PACKAGE_ID,
            MYS_SYSTEM_MODULE_NAME.as_str(),
            "request_add_stake",
            vec![
    ...
    pub fn transfer(mut self, object: ObjectRef, recipient: MysAddress) -> Self {
        self.test_data = TestTransactionData::Transfer(TransferData { object, recipient });
        self
    }

    pub fn transfer_mys(mut self, amount: Option<u64>, recipient: MysAddress) -> Self {
        self.test_data = TestTransactionData::TransferMys(TransferMysData { amount, recipient });
        self
    }
    ...
    pub fn build(self) -> TransactionData {
        match self.test_data {
            ...
            TestTransactionData::TransferMys(data) => TransactionData::new_transfer_mys(
                data.recipient,
                self.sender,
                data.amount,
                self.gas_object,
    ...
    pub fn build_and_sign_multisig(
        ...
        let data = self.build();
        let intent_msg = IntentMessage::new(Intent::mys_transaction(), data.clone());
    ...
    pub fn build_and_sign_multisig_legacy(
        ...
        let data = self.build();
        let intent = Intent::mys_transaction();
...
pub async fn make_transfer_mys_transaction(
    context: &WalletContext,
    recipient: Option<MysAddress>,
    amount: Option<u64>,
) -> Transaction {
    ...
    context.sign_transaction(
        &TestTransactionBuilder::new(sender, gas_object, gas_price)
            .transfer_mys(amount, recipient.unwrap_or(sender))
            .build(),
    )
}
...
pub async fn create_nft(
    context: &WalletContext,
    package_id: ObjectID,
) -> (MysAddress, ObjectID, TransactionDigest) {
    let (sender, gas_object) = context.get_one_gas_object().await.unwrap().unwrap();
    let rgp = context.get_reference_gas_price().await.unwrap();
