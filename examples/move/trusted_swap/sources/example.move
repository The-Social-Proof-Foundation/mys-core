...
use mys::{balance::{Self, Balance}, coin::{Self, Coin}, mys::MYS};
...
public fun request_swap(object: Object, fee: Coin<MYS>, service: address, ctx: &mut TxContext) {
...
public fun execute_swap(s1: SwapRequest, s2: SwapRequest): Balance<MYS> {
...
fun successful_swap() {
    ...
    let i1 = {
        ...
        let c1 = coin::mint_for_testing<MYS>(MIN_FEE, ts::ctx(&mut ts));
        let i = object::id(&o1);
    ...
    let i2 = {
        ...
        let c2 = coin::mint_for_testing<MYS>(MIN_FEE, ts::ctx(&mut ts));
        let i = object::id(&o2);
    ...
    {
        ts::next_tx(&mut ts, custodian);
        let fee: Coin<MYS> = ts::take_from_sender(&ts);
...
fun swap_too_cheap() {
    ...
    let o1 = new(1, 0, ts::ctx(&mut ts));
    let c1 = coin::mint_for_testing<MYS>(MIN_FEE - 1, ts::ctx(&mut ts));
    request_swap(o1, c1, custodian, ts::ctx(&mut ts));
