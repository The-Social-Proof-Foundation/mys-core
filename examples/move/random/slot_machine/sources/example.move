...
/// A betting game that depends on Mys randomness.
///
/// Anyone can create a new game for the current epoch by depositing MYS as the initial balance. The creator can
/// withdraw the remaining balance after the epoch is over.
///
/// Anyone can play the game by betting on X MYS. They win X with probability 49% and lose the X MYS otherwise.
///
module slot_machine::example;

use mys::{balance::Balance, coin::{Self, Coin}, random::{Random, new_generator}, mys::MYS};
...
public fun create(reward: Coin<MYS>, ctx: &mut TxContext) {
...
public fun close(game: Game, ctx: &mut TxContext): Coin<MYS> {
...
entry fun play(game: &mut Game, r: &Random, coin: &mut Coin<MYS>, ctx: &mut TxContext) {
    assert!(ctx.epoch() == game.epoch, EInvalidEpoch);
