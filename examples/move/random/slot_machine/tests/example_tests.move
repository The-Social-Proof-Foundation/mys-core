...
fun mint(addr: address, amount: u64, scenario: &mut ts::Scenario) {
    transfer::public_transfer(coin::mint_for_testing<MYS>(amount, scenario.ctx()), addr);
...
fun test_game() {
    ...
    let coin = ts.take_from_sender<Coin<MYS>>();
    ...
    let mut coin: Coin<MYS> = ts.take_from_sender();
