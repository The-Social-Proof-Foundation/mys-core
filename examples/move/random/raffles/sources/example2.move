...
/// Basic raffles game that depends on Mys randomness.
///
...
public fun create(end_time: u64, cost_in_mys: u64, ctx: &mut TxContext) {
    let game = Game {
        id: object::new(ctx),
        cost_in_mys,
        participants: 0,
...
entry fun close(game: Game, r: &Random, clock: &Clock, ctx: &mut TxContext) {
    ...
    let Game { id, cost_in_mys: _, participants, end_time: _, balance, participants_table } = game;
    if (participants > 0) {
...
/// Anyone can play.
public fun play(game: &mut Game, coin: Coin<MYS>, clock: &Clock, ctx: &mut TxContext) {
    assert!(game.end_time > clock.timestamp_ms(), EGameAlreadyCompleted);
    assert!(coin.value() == game.cost_in_mys, EInvalidAmount);
...
#[test_only]
public fun cost_in_mys(game: &Game): u64 {
    game.cost_in_mys
}
