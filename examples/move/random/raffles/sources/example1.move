...
/// Basic raffle game that depends on Mys randomness.
///
...
public fun redeem(ticket: Ticket, game: Game, ctx: &mut TxContext): Coin<MYS> {
    assert!(object::id(&game) == ticket.game_id, EGameMismatch);
    assert!(game.winner.contains(&ticket.participant_index), ENotWinner);
    destroy_ticket(ticket);

    let Game { id, cost_in_mys: _, participants: _, end_time: _, winner: _, balance } = game;
    object::delete(id);
