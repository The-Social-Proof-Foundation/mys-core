...
use mys::{balance::{Self, Balance}, coin::{Self, Coin}};
...
fun test_flash_loan() {
    ...
    // Admin creates a flash lender with 100 coins and a fee of 1 coin.
    {
        ...
        let coin = coin::mint_for_testing<MYS>(100, ts::ctx(&mut ts));
    ...
    // Alice requests and repays a loan of 10 coins and the fee
    {
        ...
        let mut profit = coin::mint_for_testing<MYS>(1, ts::ctx(&mut ts));
    ...
    // Admin withdraws 1 coin profit
    {
        ...
        let mut lender: FlashLender<MYS> = ts::take_shared(&ts);
