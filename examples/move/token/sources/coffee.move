...
public struct CoffeeShop has key {
    ...
    /// The MYS balance of the shop; the shop can sell Coffee for MYS.
    balance: Balance<MYS>,
}
...
fun init(otw: COFFEE, ctx: &mut TxContext) {
    ...
    mys::transfer::share_object(CoffeeShop {
        coffee_points,
        id: object::new(ctx),
...
public fun buy_coffee(app: &mut CoffeeShop, payment: Coin<MYS>, ctx: &mut TxContext) {
    // Check if the customer has enough MYS to pay for the coffee.
    ...
    mys::event::emit(CoffeePurchased {})
}
