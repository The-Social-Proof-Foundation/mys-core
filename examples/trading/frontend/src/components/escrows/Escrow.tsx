...
import { useCurrentAccount, useMysClientQuery } from "@mysten/dapp-kit";
import { MysObjectDisplay } from "@/components/MysObjectDisplay";
...
export function Escrow({ escrow }: { escrow: ApiEscrowObject }) {
  ...
  const mysObject = useMysClientQuery("getObject", {
    id: escrow?.itemId,
    options: {
      showDisplay: true,
      showType: true,
  ...
  const { data: mysLockedObject } = useGetLockedObject({
      ...
      <div className="p-4 flex gap-3 flex-wrap">
        ...
        {isToggled && lockedData.data && (
          <div className="min-w-[340px] w-full justify-self-start text-left">
            {mysLockedObject?.data && (
              <LockedObject
                object={mysLockedObject.data}
