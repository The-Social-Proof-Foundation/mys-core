...
#[test_only]
fun test_coin(ts: &mut Scenario): Coin<MYS> {
    coin::mint_for_testing<MYS>(42, ts::ctx(ts))
}
...
fun test_successful_swap() {
    ...
    // Alice gives the custodian their object to hold in escrow.
    {
        ...
        let l1: Locked<Coin<MYS>> = ts.take_from_sender();
    ...
    // Bob does the same.
    {
        ...
        let l2: Locked<Coin<MYS>> = ts.take_from_sender();
    ...
    {
        let c: Coin<MYS> = ts.take_from_address_by_id(ALICE, i2);
        ts::return_to_address(ALICE, c);
    };

    // Bob gets the object from Alice
    {
        let c: Coin<MYS> = ts.take_from_address_by_id(BOB, i1);
...
fun test_object_tamper() {
    ...
    // with it.
    {
        ...
        let l: Locked<Coin<MYS>> = ts.take_from_sender();
...
fun test_return_to_sender() {
    ...
    {
        ts.next_tx(CUSTODIAN);
        return_to_sender<Coin<MYS>>(ts.take_from_sender());
    };
    ...
    {
        let c: Coin<MYS> = ts.take_from_address_by_id(ALICE, cid);
        ts::return_to_address(ALICE, c)
    };
