...
export const MYS_BIN = `mys`;

export const getActiveAddress = () => {
	return execSync(`${MYS_BIN} client active-address`, { encoding: 'utf8' }).trim();
};

/** Returns a signer based on the active address of system's mys. */
export const getSigner = () => {
	const sender = getActiveAddress();

	const keystore = JSON.parse(
		readFileSync(path.join(homedir(), '.mys', 'mys_config', 'mys.keystore'), 'utf8'),
	);
	...
	for (const priv of keystore) {
		...
		if (pair.getPublicKey().toMysAddress() === sender) {
			return pair;
		}
	}
...
export const getClient = (network: Network) => {
	return new MysClient({ url: getFullnodeUrl(network) });
};
...
export const publishPackage = async ({
	...
	const { modules, dependencies } = JSON.parse(
		execSync(`${MYS_BIN} move build --dump-bytecode-as-base64 --path ${packagePath}`, {
			encoding: 'utf-8',
		}),
	);
