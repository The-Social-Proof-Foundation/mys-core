...
import { getClient } from '../mys-utils';
import { handleEscrowObjects } from './escrow-handler';
import { handleLockObjects } from './locked-handler';

type MysEventsCursor = EventId | null | undefined;

type EventExecutionResult = {
	cursor: MysEventsCursor;
	hasNextPage: boolean;
};
...
type EventTracker = {
	...
	type: string;
	filter: MysEventFilter;
	callback: (events: MysEvent[], type: string) => any;
};
...
const runEventJob = async (client: MysClient, tracker: EventTracker, cursor: MysEventsCursor) => {
