...
// Define the USDC token type on Mys Testnet
// This is the unique identifier for the USDC token on Mys
...
function HomeContent() {
  ...
  // Get the Mys client for interacting with the Mys network
  ...
  const handleSendTokens = async () => {
    ...
    try {
      ...
      // This uses the MysClient to get coins of the specified type owned by the current address
      const { data: coins } = await mysClient.getCoins({
      ...
      // Transaction is used to construct and execute transactions on Mys
  ...
  return (
    <main className="mainwrapper">
      <div className="outerwrapper">
        <h1 className="h1">Mys USDC Sender (Testnet)</h1>
        <ConnectButton />
