...
use mys_sdk::{
    rpc_types::{
        DevInspectArgs, DevInspectResults, DryRunTransactionBlockResponse, ObjectChange, MysData,
        MysExecutionStatus, MysObjectData, MysObjectDataFilter, MysObjectDataOptions,
        MysObjectResponse, MysObjectResponseQuery, MysProtocolConfigValue,
        MysTransactionBlockEffectsAPI, MysTransactionBlockResponse,
    },
...
pub struct Connection {
    /// The Mys CLI config file, (default: ~/.mys/mys_config/client.yaml)
    #[clap(long)]
...
impl Client {
    ...
    pub(crate) fn new(conn: Connection) -> Result<Self> {
        let Some(config) = conn.config.or_else(|| {
            ...
            default.extend([".mys", "mys_config", "client.yaml"]);
            Some(default)
        }) else {
            bail!(
                "Cannot find wallet config. No config was supplied, and the default path \
                 (~/.mys/mys_config/client.yaml) does not exist.",
            );
        };
    ...
    pub(crate) async fn turn_cap(&mut self, game: &Game) -> Result<ObjectRef> {
        ...
        let query = Some(MysObjectResponseQuery::new(
            Some(MysObjectDataFilter::StructType(turn_cap_type.clone())),
            Some(MysObjectDataOptions::new().with_bcs()),
        ));
        ...
        loop {
            ...
            for MysObjectResponse { data, error } in response.data {
    ...
    pub(crate) async fn new_shared_game(&mut self, opponent: MysAddress) -> Result<ObjectID> {
        let player = self.wallet.active_address()?;
    ...
    pub async fn new_owned_game(&mut self, opponent_key: PublicKey) -> Result<ObjectID> {
        ...
        let opponent = MysAddress::from(&opponent_key);
        ...
        let admin = MysAddress::from(&admin_key);
        let admin_bytes =
    ...
    async fn max_gas_budget(&self) -> Result<u64> {
        ...
        let Some(Some(MysProtocolConfigValue::U64(max))) = cfg.attributes.get("max_tx_gas") else {
    ...
    async fn multi_sig_transaction(
        ...
        let sponsor_sig: GenericSignature = self
            ...
            .config
            .keystore
            .sign_secure(&sender, &data, Intent::mys_transaction())
    ...
    async fn execute_transaction(&self, tx: Transaction) -> Result<MysTransactionBlockResponse> {
        let response = self
            .wallet
    ...
    async fn client(&self) -> Result<MysClient> {
        self.wallet
            .get_client()
            .await
