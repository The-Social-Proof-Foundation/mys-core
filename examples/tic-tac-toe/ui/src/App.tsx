...
function App() {
	...
	const chain = account?.chains?.find((c) => c.startsWith('mys:'))?.replace(/^mys:/, '');
	if (chain) {
...
function Content() {
	...
	const addr = normalizeMysObjectId(path);
	...
	if (packageId === null) {
		...
		return <Root />;
	} else if (isValidMysObjectId(addr)) {
		return <Game id={addr} />;
	} else {
		return (
			<Error title="Invalid Game ID">
				<code>"{path}"</code> is not a valid MYS object ID.
			</Error>
		);
	}
}
