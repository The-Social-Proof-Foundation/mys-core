...
pub fn mys_verify_module_metered_check_timeout_only(
    ...
    if let Err(error) = mys_verify_module_metered(module, fn_info_map, meter, verifier_config) {
        if matches!(
            error.kind(),
...
pub fn mys_verify_module_unmetered(
    ...
    mys_verify_module_metered(module, fn_info_map, &mut DummyMeter, verifier_config).inspect_err(
        |err| {
