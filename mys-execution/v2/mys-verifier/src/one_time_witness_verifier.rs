...
pub fn verify_module(
    ...
    // In Mys's framework code there is an exception to the one-time witness type rule - we have a
    // MYS type in the mys module but it is instantiated outside of the module initializer (in fact,
    // the module has no initializer). The reason for it is that the MYS coin is only instantiated
    ...
    if ModuleId::new(MYS_FRAMEWORK_ADDRESS, ident_str!("mys").to_owned()) == self_id {
        return Ok(());
    }
