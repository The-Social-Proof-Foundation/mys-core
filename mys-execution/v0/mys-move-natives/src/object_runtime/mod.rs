...
pub(crate) struct TestInventories {
    ...
    pub(crate) address_inventories: BTreeMap<MysAddress, BTreeMap<Type, Set<ObjectID>>>,
...
impl<'a> ObjectRuntime<'a> {
    ...
    pub fn transfer(
        ...
        //   child object
        let is_framework_obj = [MYS_SYSTEM_STATE_OBJECT_ID, MYS_CLOCK_OBJECT_ID].contains(&id);
            ...
            .map(|(id, (owner, type_, value))| {
                let write_kind =
                    if input_objects.contains_key(&id) || loaded_child_objects.contains_key(&id) {
                        ...
                        WriteKind::Mutate
                    } else if id == MYS_SYSTEM_STATE_OBJECT_ID || new_ids.contains_key(&id) {
                        // MYS_SYSTEM_STATE_OBJECT_ID is only transferred during genesis
