...
//! sure that the id field of Mys objects never get leaked.
//! Unpack is the only bytecode that could extract the id field out of
//! a Mys object. From there, we track the flow of the value and make
...
use mys_types::{
    ...
    MYS_FRAMEWORK_ADDRESS, MYS_SYSTEM_ADDRESS,
};
...
const MYS_SYSTEM_CREATE: FunctionIdent = (
    &MYS_SYSTEM_ADDRESS,
    MYS_SYSTEM_MODULE_NAME,
    ident_str!("create"),
);
const MYS_CLOCK_CREATE: FunctionIdent = (
    &MYS_FRAMEWORK_ADDRESS,
    CLOCK_MODULE_NAME,
    ident_str!("create"),
);
...
#[cfg(msim)]
const MYS_AUTHENTICATOR_STATE_CREATE: FunctionIdent = (
    &MYS_FRAMEWORK_ADDRESS,
    AUTHENTICATOR_STATE_MODULE_NAME,
    ident_str!("create"),
);
#[cfg(msim)]
const MYS_RANDOMNESS_STATE_CREATE: FunctionIdent = (
    &MYS_FRAMEWORK_ADDRESS,
    RANDOMNESS_MODULE_NAME,
    ident_str!("create"),
);
#[cfg(msim)]
const MYS_DENY_LIST_OBJECT_CREATE: FunctionIdent = (
    &MYS_FRAMEWORK_ADDRESS,
    COIN_MODULE_NAME,
...
#[cfg(not(msim))]
const FUNCTIONS_TO_SKIP: &[FunctionIdent] = &[MYS_SYSTEM_CREATE, MYS_CLOCK_CREATE];

#[cfg(msim)]
const FUNCTIONS_TO_SKIP: &[FunctionIdent] = &[
    MYS_SYSTEM_CREATE,
    MYS_CLOCK_CREATE,
    MYS_AUTHENTICATOR_STATE_CREATE,
    MYS_RANDOMNESS_STATE_CREATE,
    MYS_DENY_LIST_OBJECT_CREATE,
];
    ...
    if handle.abilities.has_key() && last_value != AbstractValue::Fresh {
        ...
        let msg = format!(
            "Invalid object creation in {cur_package}::{cur_module}::{cur_function}. \
                ...
                The UID must come directly from mys::{}::{}. \
                Or for tests, it can come from mys::{}::{}",
