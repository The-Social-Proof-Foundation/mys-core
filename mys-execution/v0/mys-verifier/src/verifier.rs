...
pub fn mys_verify_module_metered_check_timeout_only(
    ...
    if let Err(error) = mys_verify_module_metered(config, module, fn_info_map, meter) {
        if matches!(
            error.kind(),
...
pub fn mys_verify_module_unmetered(
    ...
    mys_verify_module_metered(config, module, fn_info_map, &mut DummyMeter).inspect_err(|err| {
