...
impl<T> MysResolver for T
where
    T: ResourceResolver<Error = MysError>,
...
pub trait ExecutionState: StorageView + MysResolver {
    fn as_mys_resolver(&self) -> &dyn MysResolver;
...
impl TryFromValue for MysAddress {
    fn try_from_value(value: Value) -> Result<Self, CommandArgumentError> {
