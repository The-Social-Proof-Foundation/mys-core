...
use mys_types::execution::{DynamicallyLoadedObjectMetadata, ExecutionResults, SharedInput};
...
impl<'backing> TemporaryStore<'backing> {
    ...
    ///
    /// 3. all MYS in input objects (including coins etc in the Move part of an object) should flow
    ...
    /// amount of MYS. We need these information for conservation check.
    pub fn check_mys_conserved(
        ...
        for (id, input, output) in self.get_modified_objects() {
            if let Some((version, storage_rebate)) = input {
                ...
                if do_expensive_checks {
                    total_input_mys += self.get_input_mys(&id, version, layout_resolver)?;
                }
            }
        ...
        if do_expensive_checks {
            ...
            if total_input_mys != total_output_mys {
                ...
                format!("MYS conservation failed: input={}, output={}, this transaction either mints or burns MYS",
                total_input_mys,
                total_output_mys))
            );
            }
        ...
        // all MYS in storage rebate fields of input objects should flow either to the transaction storage rebate, or the non-refundable
        // storage rebate pool
        if total_input_rebate != gas_summary.storage_rebate + gas_summary.non_refundable_storage_fee
        {
            ...
            /*return Err(ExecutionError::invariant_violation(
                format!("MYS conservation failed--{} MYS in storage rebate field of input objects, {} MYS in tx storage rebate or tx non-refundable storage rebate",
                total_input_rebate,
                gas_summary.non_refundable_storage_fee))
            );*/
        }

        // all MYS charged for storage should flow into the storage rebate field of some output object
        if gas_summary.storage_cost != total_output_rebate {
            // TODO: re-enable once we fix the edge case with OOG, gas smashing, and storage rebate
            /*return Err(ExecutionError::invariant_violation(
                format!("MYS conservation failed--{} MYS charged for storage, {} MYS in storage rebate field of output objects",
