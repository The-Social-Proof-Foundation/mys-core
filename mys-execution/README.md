...
The `mys-execution` crate is responsible for abstracting access to the
...
- The adapter, that integrates Move into MySocial.
...
- `./mys-execution`, (latest version and cuts/copies of mys-specific
  crates).
...
code must be via `mys-execution`, and not by directly depending a
constituent crate.
...
and elsewhere, **default to accessing execution via `mys-execution`.**
...
`mys-execution tests::test_encapsulation` is a test that detects
...
`./crates/mys-types` are shared across all versions.  Changes to
...
## `mys-execution/src/lib.rs`

The entry-point to the execution crate -- `mys-execution/src/lib.rs`
...
- `mys-execution/src/lib.template.rs` -- a template file with
