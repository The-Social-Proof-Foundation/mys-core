...
use mys_types::deny_list_v2::check_coin_deny_list_v2_during_execution;
                ...
                match &old_obj.owner {
                    ...
                    Owner::Immutable => {
                        assert!(
                            ...
                            Mys Framework/Move stdlib upgrades at epoch change boundaries"
                        );
...
impl<'backing> Storage for TemporaryStore<'backing> {
    ...
    fn check_coin_deny_list(&self, written_objects: &BTreeMap<ObjectID, Object>) -> DenyListResult {
        ...
        if result.num_non_gas_coin_owners > 0
            && !self.input_objects.contains_key(&MYS_DENY_LIST_OBJECT_ID)
        {
