...
mod checked {
    ...
    use mys_verifier::check_for_verifier_timeout;
    ...
    use mys_types::{
        ...
        error::{ExecutionErrorKind, MysError},
        execution_config_utils::to_binary_config,
        metrics::LimitsMetrics,
        storage::ChildObjectResolver,
    };
    use mys_verifier::verifier::mys_verify_module_metered_check_timeout_only;

    pub fn new_move_vm(
        natives: NativeFunctionTable,
        protocol_config: &ProtocolConfig,
        _enable_profiler: Option<PathBuf>,
    ) -> Result<MoveVM, MysError> {
        ...
        .map_err(|_| MysError::ExecutionInvariantViolation)
    }
    ...
    /// Run both the Move verifier and the Mys verifier, checking just for timeouts. Returns Ok(())
    ...
    fn verify_module_timeout_only(
        ...
        if let Err(e) = verify_module_with_config_metered(verifier_config, module, meter) {
            ...
            if check_for_verifier_timeout(&e.major_status()) {
                return Err(MysError::ModuleVerificationFailure {
                    error: format!("Verification timed out: {}", e),
                });
            }
        } else if let Err(err) = mys_verify_module_metered_check_timeout_only(
            module,
            &BTreeMap::new(),
            meter,
            verifier_config,
