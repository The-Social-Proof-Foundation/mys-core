...
mod checked {
    ...
    use mys_types::randomness_state::{
        RANDOMNESS_MODULE_NAME, RANDOMNESS_STATE_CREATE_FUNCTION_NAME,
        RANDOMNESS_STATE_UPDATE_FUNCTION_NAME,
    };
    use mys_types::{BRIDGE_ADDRESS, MYS_BRIDGE_OBJECT_ID, MYS_RANDOMNESS_STATE_OBJECT_ID};
    ...
    use mys_types::bridge::BRIDGE_COMMITTEE_MINIMAL_VOTING_POWER;
    use mys_types::bridge::{
        BridgeChainId, BRIDGE_CREATE_FUNCTION_NAME, BRIDGE_INIT_COMMITTEE_FUNCTION_NAME,
        BRIDGE_MODULE_NAME,
    };
    use mys_types::clock::{CLOCK_MODULE_NAME, CONSENSUS_COMMIT_PROLOGUE_FUNCTION_NAME};
    ...
    use mys_types::error::{ExecutionError, ExecutionErrorKind};
    use mys_types::execution::{is_certificate_denied, ExecutionTiming, ResultWithTimings};
    use mys_types::execution_config_utils::to_binary_config;
    use mys_types::execution_status::{CongestedObjects, ExecutionStatus};
    ...
    #[cfg(msim)]
    use mys_types::mys_system_state::advance_epoch_result_injection::maybe_modify_result;
    ...
    pub fn execute_transaction_to_effects<Mode: ExecutionMode>(
        ...
        let status = if let Err(error) = &execution_result {
            ...
            match error.kind() {
                ...
                K::MysMoveVerificationError | K::VMVerificationOrDeserializationError => {
                    #[skip_checked_arithmetic]
                    tracing::debug!(
                        kind = ?error.kind(),
                        tx_digest = ?transaction_digest,
    ...
    fn execute_transaction<Mode: ExecutionMode>(
        ...
        // information provided to check_mys_conserved, because we mint rewards, and burn
    ...
    fn run_conservation_checks<Mode: ExecutionMode>(
        ...
        let mut result: std::result::Result<(), mys_types::error::ExecutionError> = Ok(());
        if !is_genesis_tx && !Mode::skip_conservation_checks() {
            // ensure that this transaction did not create or destroy MYS, try to recover if the check fails
            let conservation_result = {
                temporary_store
                    .check_mys_conserved(simple_conservation_checks, cost_summary)
                    .and_then(|()| {
                        if enable_expensive_checks {
                            // ensure that this transaction did not create or destroy MYS, try to recover if the check fails
                            let mut layout_resolver =
                                TypeLayoutResolver::new(move_vm, Box::new(&*temporary_store));
                            temporary_store.check_mys_conserved_expensive(
                                cost_summary,
                                advance_epoch_gas_summary,
                                &mut layout_resolver,
                            )
            ...
            if let Err(conservation_err) = conservation_result {
                ...
                if let Err(recovery_err) = {
                    ...
                    // we will create or destroy MYS otherwise
                    panic!(
                        "MYS conservation fail in tx block {}: {}\nGas status is {}\nTx was ",
                        tx_digest,
                        recovery_err,
                        gas_charger.summary()
                    )
        ...
        } // else, we're in the genesis transaction which mints the MYS supply, and hence does not satisfy MYS conservation, or
            ...
            TransactionKind::Genesis(GenesisTransaction { objects }) => {
                ...
                for genesis_object in objects {
                    match genesis_object {
                        mys_types::transaction::GenesisObject::RawObject { data, owner } => {
