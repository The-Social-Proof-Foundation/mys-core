...
use mys_types::randomness_state::is_mutable_random;
use mys_types::{
...
fn verify_param_type(
    ...
    // Only `mys::mys_system` is allowed to expose entry functions that accept a mutable clock
    // parameter.
    ...
    // Only `mys::mys_system` is allowed to expose entry functions that accept a mutable Random
    // parameter.
