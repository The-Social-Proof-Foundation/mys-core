...
use mys_types::{error::ExecutionError, MYS_FRAMEWORK_ADDRESS};
...
/// All transfer functions (the functions in `mys::transfer`) are "private" in that they are
...
pub fn verify_module(
    ...
    if *module.address() == MYS_FRAMEWORK_ADDRESS
        && module.name() == IdentStr::new(TEST_SCENARIO_MODULE_NAME).unwrap()
    {
        // exclude test_module which is a test-only module in the Mys framework which "emulates"
        // transactional execution and needs to allow test code to bypass private generics
        return Ok(());
    }
    // do not need to check the mys::transfer module itself
...
fn verify_function(
    ...
    for instr in &code.code {
        if let Bytecode::CallGeneric(finst_idx) = instr {
            ...
            if ident == (MYS_FRAMEWORK_ADDRESS, TRANSFER_MODULE) {
                verify_private_transfer(view, fhandle, type_arguments, allow_receiving_object_id)?
            } else if ident == (MYS_FRAMEWORK_ADDRESS, EVENT_MODULE) {
...
fn verify_private_transfer(
    ...
    if addr_module(view, self_handle) == (MYS_FRAMEWORK_ADDRESS, TRANSFER_MODULE) {
        return Ok(());
    }
    ...
    if !is_defined_in_current_module(view, type_arg) {
        return Err(format!(
            "Invalid call to '{mys}::transfer::{f}' on an object of type '{t}'. \
            ...
            instead, i.e. '{mys}::transfer::public_{f}'",
            mys = MYS_FRAMEWORK_ADDRESS,
            f = fident,
