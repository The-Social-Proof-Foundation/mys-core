...
//! - The first field has type `mys::object::UID`
    ...
    for def in struct_defs {
        ...
        let uid_field_type = match uid_field_type {
            ...
            _ => {
                return Err(verification_failure(format!(
                    ...
                    {:?} type found",
                    name, MYS_FRAMEWORK_ADDRESS, uid_field_type
                )))
            }
        };
        // check that the struct type for "id" field must be MYS_FRAMEWORK_ADDRESS::object::UID.
        ...
        fp_ensure!(
            uid_type_struct_name == UID_STRUCT_NAME
                && uid_type_module_address == &MYS_FRAMEWORK_ADDRESS
