...
impl<'backing> TemporaryStore<'backing> {
    ...
    fn into_effects_v1(
        ...
        let updated_gas_object_info = if let Some(coin_id) = gas_charger.gas_coin() {
            ...
            (
                ...
                Owner::AddressOwner(MysAddress::default()),
            )
        };
    ...
    pub fn conserve_unmetered_storage_rebate(&mut self, unmetered_storage_rebate: u64) {
        ...
        let mut system_state_wrapper = self
            .read_object(&MYS_SYSTEM_STATE_OBJECT_ID)
        ...
        // If not, we would be dropping MYS on the floor by overriding it.
...
impl<'backing> TemporaryStore<'backing> {
    ...
    fn get_objects_to_authenticate(
    ...
    ) -> MysResult<(Vec<ObjectID>, HashSet<ObjectID>)> {
        ...
        for id in &self.execution_results.modified_objects {
            ...
            match &old_obj.owner {
                ...
                Owner::Immutable => {
                    assert!(is_epoch_change, "Immutable objects cannot be written, except for Mys Framework/Move stdlib upgrades at epoch change boundaries");
...
impl<'backing> TemporaryStore<'backing> {
    ...
    /// `MysGasStatus` `storage_rebate` and `storage_gas_units` track the transaction
...
impl<'backing> TemporaryStore<'backing> {
    pub fn advance_epoch_safe_mode(
        ...
        let wrapper = get_mys_system_state_wrapper(self.store.as_object_store())
...
impl<'backing> TemporaryStore<'backing> {
    fn get_input_mys(
        ...
        if let Some(obj) = self.input_objects.get(id) {
            ...
            obj.get_total_mys(layout_resolver).map_err(|e| {
                make_invariant_violation!(
                    "Failed looking up input MYS in MYS conservation checking for input with \
                         type {:?}: {e:#?}",
                    obj.struct_tag(),
                )
            })
            ...
            let Some(obj) = self.store.get_object_by_key(id, expected_version) else {
                invariant_violation!(
                    "Failed looking up dynamic field {id} in MYS conservation checking"
                );
            };
            obj.get_total_mys(layout_resolver).map_err(|e| {
                make_invariant_violation!(
                    "Failed looking up input MYS in MYS conservation checking for type \
                         {:?}: {e:#?}",
                    obj.struct_tag(),
                )
            })
    ...
    /// Check that this transaction neither creates nor destroys MYS. This should hold for all txes
    ...
    ///
    /// 1. all MYS in storage rebate fields of input objects should flow either to the transaction
    ///    storage rebate, or the transaction non-refundable storage rebate
    /// 2. all MYS charged for storage should flow into the storage rebate field of some output
    ///    object
    ///
    ...
    pub fn check_mys_conserved(
        ...
        // total amount of MYS in storage rebate of input objects
        let mut total_input_rebate = 0;
        // total amount of MYS in storage rebate of output objects
        ...
        if gas_summary.storage_cost == 0 {
                    ...
                    + gas_summary.non_refundable_storage_fee
            {
                return Err(ExecutionError::invariant_violation(format!(
                    "MYS conservation failed -- no storage charges in gas summary \
            ...
            // all MYS in storage rebate fields of input objects should flow either to
            ...
            {
                return Err(ExecutionError::invariant_violation(format!(
                    "MYS conservation failed -- {} MYS in storage rebate field of input objects, \
                        {} MYS in tx storage rebate or tx non-refundable storage rebate",
                    total_input_rebate, gas_summary.non_refundable_storage_fee,
                )));
            }

            // all MYS charged for storage should flow into the storage rebate field
            // of some output object
            if gas_summary.storage_cost != total_output_rebate {
                return Err(ExecutionError::invariant_violation(format!(
                    "MYS conservation failed -- {} MYS charged for storage, \
                        {} MYS in storage rebate field of output objects",
    ...
    /// Check that this transaction neither creates nor destroys MYS.
    /// This more expensive check will check a third invariant on top of the 2 performed
    /// by `check_mys_conserved` above:
    ///
    /// * all MYS in input objects (including coins etc in the Move part of an object) should flow
    ...
    /// advance epoch transaction would mint `epoch_fees` amount of MYS, and burn `epoch_rebates`
    /// amount of MYS. We need these information for this check.
    pub fn check_mys_conserved_expensive(
        &self,
        gas_summary: &GasCostSummary,
        advance_epoch_gas_summary: Option<(u64, u64)>,
        layout_resolver: &mut impl LayoutResolver,
    ) -> Result<(), ExecutionError> {
        // total amount of MYS in input objects, including both coins and storage rebates
        let mut total_input_mys = 0;
        // total amount of MYS in output objects, including both coins and storage rebates
        let mut total_output_mys = 0;
        for (id, input, output) in self.get_modified_objects() {
            if let Some(input) = input {
                total_input_mys += self.get_input_mys(&id, input.version, layout_resolver)?;
            }
            if let Some(object) = output {
                total_output_mys += object.get_total_mys(layout_resolver).map_err(|e| {
                    make_invariant_violation!(
                        "Failed looking up output MYS in MYS conservation checking for \
                         mutated type {:?}: {e:#?}",
                        object.struct_tag(),
                    )
                })?;
        ...
        total_output_mys += gas_summary.computation_cost + gas_summary.non_refundable_storage_fee;
        if let Some((epoch_fees, epoch_rebates)) = advance_epoch_gas_summary {
            total_input_mys += epoch_fees;
            total_output_mys += epoch_rebates;
        }
        if total_input_mys != total_output_mys {
            return Err(ExecutionError::invariant_violation(format!(
                "MYS conservation failed: input={}, output={}, \
                    this transaction either mints or burns MYS",
                total_input_mys, total_output_mys,
            )));
        }
        Ok(())
    }
    ...
    fn get_resource(
        ...
        let object = match self.read_object(&ObjectID::from(*address)) {
            ...
            None => match self.read_object(&ObjectID::from(*address)) {
                ...
                Some(x) => {
                    if !x.is_immutable() {
                        fp_bail!(MysError::ExecutionInvariantViolation);
                    }
                    x
                }
            },
