...
mod checked {
    ...
    use mys_types::execution_status::{CommandArgumentError, PackageUpgradeError};
    ...
    fn publish_and_verify_modules(
        ...
        // TODO(https://github.com/MystenLabs/mys/issues/69): avoid this redundant serialization by exposing VM API that allows us to run the linker directly on `Vec<CompiledModule>`
        ...
        // run the Mys verifier
        for module in modules {
            // Run Mys bytecode verifier, which runs some additional checks that assume the Move
            // bytecode verifier has passed.
            mys_verifier::verifier::mys_verify_module_unmetered(module, &BTreeMap::new())?;
        }
    ...
    fn check_visibility_and_signature<Mode: ExecutionMode>(
        ...
        let data_store = MysDataStore::new(&context.linkage_view, &no_new_packages);
        let module = context
            .vm
            .get_runtime()
    ...
    fn check_private_generics(
        ...
        if module_ident == (&MYS_FRAMEWORK_ADDRESS, EVENT_MODULE) {
            return Err(ExecutionError::new_with_source(
                ExecutionErrorKind::NonEntryFunctionInvoked,
                format!("Cannot directly call functions in mys::{}", EVENT_MODULE),
            ));
        }

        if module_ident == (&MYS_FRAMEWORK_ADDRESS, TRANSFER_MODULE)
            && PRIVATE_TRANSFER_FUNCTIONS.contains(&function)
        {
            let msg = format!(
                "Cannot directly call mys::{m}::{f}. \
                Use the public variant instead, mys::{m}::public_{f}",
                m = TRANSFER_MODULE,
                f = function
            );
            return Err(ExecutionError::new_with_source(
        ...
        match value {
            ...
            Value::Receiving(_, _, assigned_type) => {
                ...
                let Some(s) = context.vm.get_runtime().get_type(*sidx) else {
                    invariant_violation!("mys::transfer::Receiving struct not found in session")
                };
    ...
    pub fn is_tx_context(
        ...
        let is_tx_context_type = module_addr == &MYS_FRAMEWORK_ADDRESS
        ...
        fn deserialize<D: serde::de::Deserializer<'d>>(
            ...
            match self {
                ...
                PrimitiveArgumentLayout::Address => {
                    MysAddress::deserialize(deserializer)?;
                    Ok(())
                }
            }
        }
