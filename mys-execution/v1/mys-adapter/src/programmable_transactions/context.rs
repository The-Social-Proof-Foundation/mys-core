...
mod checked {
    ...
    pub struct ExecutionContext<'vm, 'state, 'a> {
        ...
        additional_transfers: Vec<(/* new owner */ MysAddress, ObjectValue)>,
    ...
    impl<'vm, 'state, 'a> ExecutionContext<'vm, 'state, 'a> {
        ...
        pub fn new(
            ...
            let mut linkage_view = LinkageView::new(Box::new(state_view.as_mys_resolver()));
        ...
        pub fn convert_type_argument_error(&self, idx: usize, error: VMError) -> ExecutionError {
            use move_core_types::vm_status::StatusCode;
            use mys_types::execution_status::TypeArgumentError;
        ...
        pub(crate) fn execute_function_bypass_visibility(
            ...
            let mut data_store = MysDataStore::new(&self.linkage_view, &self.new_packages);
    ...
    pub fn load_type_from_struct(
        ...
        let data_store = MysDataStore::new(linkage_view, new_packages);
    ...
    pub(crate) struct MysDataStore<'state, 'a> {
        linkage_view: &'a LinkageView<'state>,
        new_packages: &'a [MovePackage],
    }

    impl<'state, 'a> MysDataStore<'state, 'a> {
        pub(crate) fn new(
            linkage_view: &'a LinkageView<'state>,
            new_packages: &'a [MovePackage],
        ) -> Self {
