...
use crate::execution_value::MysResolver;
use move_core_types::{
...
pub struct LinkageView<'state> {
    ...
    resolver: Box<dyn MysResolver + 'state>,
...
impl<'state> LinkageView<'state> {
    pub fn new(resolver: Box<dyn MysResolver + 'state>) -> Self {
        Self {
            resolver,
            linkage_info: None,
    ...
    pub(crate) fn relocate(&self, module_id: &ModuleId) -> Result<ModuleId, MysError> {
