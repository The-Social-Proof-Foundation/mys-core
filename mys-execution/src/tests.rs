...
#[test]
/// Make sure that all accesses to execution layer crates in the `mys-node` and `mys-replay` crates
/// go via the `mys-execution` crate (in other words, the `mys-execution` crate dominates execution
/// layer crates in the dependency graphs of `mys-node` and `mys-replay`).
///
...
fn test_encapsulation() {
    ...
    let mut exec_crates: BTreeSet<_> = packages.normal_deps("mys-execution").collect();

    // Remove the crates that the execution layer depends on but which are not directly part of the
    // execution layer -- these don't need to be accessed exclusively via `mys-execution`.
    exec_crates.remove("mys-protocol-config");
    exec_crates.remove("mys-types");
    ...
    for root in ["mys-node", "mys-replay"] {
        let mut graph = packages.graph(root);

        // If we can still create a path from `root` to an execution crate after removing these
        // nodes then we know that we can potential bypass "mys-execution".
        graph.remove_node("mys-execution");
    ...
    panic!(
        "protocol-sensitive binaries depend on execution crates outside of 'mys-execution', e.g.:\n\
         \n  {}\n\
         \n\
         This can cause execution to fork by not respecting the execution layer version set in the \
         protocol config.  Fix this by depending on these crates via 'mys-execution'.\n\
         \n\
         P.S. if you believe one of these crates should not be part of 'mys-execution' then update \
         the test to exclude this crate.",
        examples.join("\n  "),
    );
}

/// Parse `cargo metadata` for the `mys` repo.
fn cargo_metadata() -> cargo_metadata::Result<Metadata> {
    let mys_execution = PathBuf::from(env!("CARGO_MANIFEST_DIR"));

    MetadataCommand::new()
        .manifest_path(mys_execution.join("../Cargo.toml"))
        .current_dir(mys_execution.join(".."))
        .no_deps()
        .exec()
}
