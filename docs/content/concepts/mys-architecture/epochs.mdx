...
In a simplified view, epochs are to the MySocial network what individual days are to a calendar. Reconfiguration, then, is a process at the end of each epoch where that day's activity is tallied and the next day's parameters are set.

## Epoch

An epoch is a duration of time where the MySocial validator set and their stakes remain unchanged. On both Mainnet and Testnet, an epoch is about 24 hours. This timeframe allows validators to process transactions efficiently without worrying about ad hoc validator changes during the middle of an epoch.

Epoch values are included in the metadata of transactions on the MySocial network, but the average user is unaffected by its value. The only time a user might need to know about epochs is in the case of expiring transactions, where a transaction is only valid if executed before a set epoch. See [Transactions](../transactions.mdx) for more information about transactions on MySocial.
...
1. **Distribution of gas rewards**
    ...
    - Storage fees are allocated to a storage fund, playing a vital role in the MySocial tokenomics, as explained in [Tokenomics](../tokenomics.mdx).
