...
description: Dynamic fields and dynamic object fields on MySocial are added and removed dynamically, affect gas only when accessed, and store heterogeneous values.
---
...
1. There are use cases where you need to store a collection of objects of heterogeneous types. Because the Move `vector` type must be instantiated with one single type `<T>`, it is not ttttable for this.

Fortunately, MySocial provides *dynamic fields* with arbitrary names (not just identifiers), added and removed on-the-fly (not fixed at publish), which only affect gas when they are accessed, and can store heterogeneous values. Use the libraries in this topic to interact with this kind of field.
...
| --- | --- | --- |
| Fields | Can store any value that has `store`, however an object stored in this kind of field is considered wrapped and is not accessible via its ID by external tools (explorers, wallets, and so on) accessing storage.| [`dynamic_field`](https://github.com/MystenLabs/mys/tree/main/crates/mys-framework/packages/mys-framework/sources/dynamic_field.move) |
| Object field | Values must be objects (have the `key` ability, and `id: UID` as the first field), but are still accessible at their ID to external tools.| [`dynamic_object_field`](https://github.com/MystenLabs/mys/tree/main/crates/mys-framework/packages/mys-framework/sources/dynamic_object_field.move) |
...
Use the `add` function from the relevant MySocial framework module to add dynamic fields:

**Dynamic field**

{@inject: crates/mys-framework/packages/mys-framework/sources/dynamic_field.move#fun=add noComments}

**Dynamic object field**

{@inject: crates/mys-framework/packages/mys-framework/sources/dynamic_object_field.move#fun=add noComments}
...
{@inject: crates/mys-framework/packages/mys-framework/sources/dynamic_field.move#fun=borrow,borrow_mut noComments}

Where `object` is the UID of the object the field is defined on and `name` is the field's name.

:::info

`mys::dynamic_object_field` has equivalent functions for object fields, but with the added constraint `Value: key + store`.
...
{@inject: crates/mys-framework/packages/mys-framework/sources/dynamic_field.move#fun=remove noComments}
...
`mys::dynamic_object_field` has an equivalent function for object fields.
...
MySocial provides `Table` and `Bag` collections built using dynamic fields, but with additional support to count the number of entries they contain to protect against accidental deletion when non-empty. To learn more, see [Tables and Bags](./dynamic-fields/tables-bags.mdx).
