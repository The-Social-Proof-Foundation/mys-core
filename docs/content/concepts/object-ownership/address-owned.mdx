...
description: Address-owned objects are owned by a MySocial 32-byte address, which can either be an account address or an object ID. Learn how to create and access these objects.
---
...
You can use one of the following functions (defined in the [transfer module](https://github.com/MystenLabs/mys/blob/main/crates/mys-framework/packages/mys-framework/sources/transfer.move)) to create address-owned objects.
...
You should use the `mys::transfer::transfer` function if you are defining a [custom transfer policy](../transfers/custom-rules.mdx) for the object. Use the `mys::transfer::public_transfer` function to create an address-owned object if the object has the `store` capability.
...
An example of an object that is frequently address-owned is that of a [Coin object](https://github.com/MystenLabs/mys/blob/main/crates/mys-framework/packages/mys-framework/sources/coin.move). If address `0xA11CE` had a coin `C` with 100 MYS and wanted to pay address `0xB0B` 100 MYS, `0xA11CE` could do so by transferring `C` to `0xB0B`.

```move
mys::transfer::public_transfer(C, @0xB0B);
```

This results in `C` having a new address owner of `0xB0B`, and `0xB0B` can later use that 100 MYS coin.
