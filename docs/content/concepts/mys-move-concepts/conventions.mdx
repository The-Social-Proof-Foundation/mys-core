...
description: Recommended Move 2024 best practices for MySocial development.
---

This guide outlines recommended conventions and best practices for writing Move smart contracts on MySocial. Following these guidelines helps create more maintainable, secure, and composable code that aligns with ecosystem standards.

While these conventions are recommendations rather than strict rules, they represent patterns that have proven effective across many MySocial projects. They help create consistency across the ecosystem and make code easier to understand and maintain.

## Organization principles

### Package

A MySocial package consists of: 
...
- a `Move.lock` file that the MySocial Move toolchain automatically generates to lock the versions of the dependencies and track the different published and upgraded versions of the package that exist on the different networks

For this reason, the `Move.lock` file should always be part of the package (don't add it to the `.gitignore` file). Use the [automated address management](https://docs.mysocial.network/concepts/mys-move-concepts/packages/automated-address-management) instead of the old `published-at` field in the manifest file.
...
The following section covers common patterns and best practices specific to Move development on MySocial, including object ownership models and function design principles.
...
public fun withdraw(state: &mut State, account: &mut Account, ctx: &mut TxContext): Coin<MYS> {
...
public fun wrong_withdraw(state: &mut State, ctx: &mut TxContext): Coin<MYS> {
...
public fun new(deposit: Coin<MYS>, ctx: &mut TxContext): OwnedWallet {
    // Implementation omitted.
    abort(0)
}
...
public fun new_shared(deposit: Coin<MYS>, ctx: &mut TxContext) {
    // Implementation omitted.
...
public fun start_battle(
    // user asset container would go here
