...
To address the security risk of single-key upgrade ownership poses while still providing the opportunity to [upgrade live packages](./upgrade.mdx), MySocial offers _custom upgrade policies_. These policies protect `UpgradeCap` access and issue `UpgradeTicket` objects that authorize upgrades on a case-by-case basis.

## Compatibility {#compatibility}

MySocial comes with a set of built-in package compatibility policies, listed here from most to least strict:
...
You can change the current policy by calling one of the functions in `mys::package` (`only_additive_upgrades`, `only_dep_upgrades`, `make_immutable`) on the package's `UpgradeCap` and a policy can become only more restrictive. For example, after you call `mys::package::only_dep_upgrades` to restrict the policy to become additive, calling `mys::package::only_additive_upgrades` on the `UpgradeCap` of the same package results in an error.
...
While step 2 is a built-in command, steps 1 and 3 are implemented as Move functions. The MySocial framework provides their most basic implementation:

{@inject: crates/mys-framework/packages/mys-framework/sources/package.move#fun=authorize_upgrade,commit_upgrade noComments}

These are the functions that `mys client upgrade` calls for authorization and commit. Custom upgrade policies work by guarding
...
{@inject: crates/mys-framework/packages/mys-framework/sources/package.move#struct=UpgradeCap noComments}
...
{@inject: crates/mys-framework/packages/mys-framework/sources/package.move#struct=UpgradeTicket noComments}
...
bytecode or source code for ahead of time. MySocial calculates the digest as follows:
...
Refer to the [implementation for digest calculation](https://github.com/MystenLabs/mys/blob/d8cb153d886d54752763fbdab631b062da7d894b/crates/mys-types/src/move_package.rs#L232-L251) for more information, but in most cases, you can rely on the Move toolchain to output the digest as part of the build, when passing the `--dump-bytecode-as-base64` flag:

```sh
$ mys move build --dump-bytecode-as-base64
```
```sh
FETCHING GIT DEPENDENCY https://github.com/MystenLabs/mys.git
...
{@inject: crates/mys-framework/packages/mys-framework/sources/package.move#struct=UpgradeReceipt noComments}

The `UpgradeReceipt` is proof that the `Upgrade` command ran successfully, and MySocial added the new package to the set of created
...
After MySocial creates an `UpgradeReceipt`, you must use it to update its `UpgradeCap` within the same transaction (you cannot store it for later, drop it, or burn it), or the transaction fails.
...
The signature of a custom `authorize_upgrade` can be different from the signature of `mys::package::authorize_upgrade` as long as it returns an `UpgradeTicket`.

Finally, provide implementations of `commit_upgrade` and `make_immutable` that delegate to their respective functions in `mys::package`:
...
module policy::day_of_week {
    ...
    fun week_day(ctx: &TxContext): u8 {
        let days_since_unix_epoch =
            mys::tx_context::epoch_timestamp_ms(ctx) / MS_IN_DAY;
        // The unix epoch (1st Jan 1970) was a Thursday so shift days
        // since the epoch by 3 so that 0 = Monday.
        ((days_since_unix_epoch + 3) % 7 as u8)
    }
...
Use the `mys client publish` command to publish the policy.
...
If you don't have a package available, use the `mys move new` command to create the template for a new package called `example`.
...
Open a terminal or console to the root of your Node.js project. Run the following command to add the MySocial TypeScript SDK as a dependency:

```sh
$ npm install @mysten/mys
```
...
- `MYS`: the location of the `mys` CLI binary.
...
Next, add boilerplate code to get the signer key pair for the currently active address in the MySocial Client CLI:
...
const sender = execSync(`${MYS} client active-address`, { encoding: 'utf8' }).trim();
const signer = (() => {
	...
	for (const priv of keystore) {
		...
		if (pair.getPublicKey().toMySocialAddress() === sender) {
			return pair;
		}
	}
...
const { modules, dependencies } = JSON.parse(
	execSync(`${MYS} move build --dump-bytecode-as-base64 --path ${packagePath}`, {
		encoding: 'utf-8',
	}),
);
```
...
```js
import { getFullnodeUrl, MySocialClient } from '@socialproof/mys/client';

const client = new MySocialClient({ url: getFullnodeUrl('localnet') });
...
```sh
$ mys client call --gas-budget 10000000 \
...
Array [
    Object {
        ...
        "objectType": String("0x2::coin::Coin<0x2::mys::MYS>"),
        "objectId": String("<GAS>"),
        "version": String("12"),
...
import { getFullnodeUrl, MySocialClient } from '@socialproof/mys/client';
import { Ed25519Keypair } from '@socialproof/mys/keypairs/ed25519';
import { Transaction, UpgradePolicy } from '@socialproof/mys/transactions';
import { fromBase64 } from '@socialproof/mys/utils';
...
Use the MySocial Client CLI to test the upgraded package (the package ID is **different** from the original version of your example package):
