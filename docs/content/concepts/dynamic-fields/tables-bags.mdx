---
title: Table and Bag
description: MySocial provides `Table` and `Bag` collections built using dynamic fields, but with additional support to count the number of entries they contain to protect against accidental deletion when non-empty.
---
...
The types and function discussed in this section are built into the MySocial framework in modules [`table`](https://github.com/MystenLabs/mys/tree/main/crates/mys-framework/packages/mys-framework/sources/table.move) and [`bag`](https://github.com/MystenLabs/mys/tree/main/crates/mys-framework/packages/mys-framework/sources/bag.move). As with dynamic fields, there is also an `object_` variant of both: `ObjectTable` in [`object_table`](https://github.com/MystenLabs/mys/tree/main/crates/mys-framework/packages/mys-framework/sources/object_table.move) and `ObjectBag` in [`object_bag`](https://github.com/MystenLabs/mys/tree/main/crates/mys-framework/packages/mys-framework/sources/object_bag.move). The relationship between `Table` and `ObjectTable`, and `Bag` and `ObjectBag` is the same as between a field and an object field: The former can hold any `store` type as a value, but objects stored as values are hidden when viewed from external storage. The latter can only store objects as values, but keeps those objects visible at their ID in external storage.
...
`Table<K, V>` is a *homogeneous* map, meaning that all its keys have the same type as each other (`K`), and all its values have the same type as each other as well (`V`). It is created with `mys::table::new`, which requires access to a `&mut TxContext` because `Table`s are objects themselves, which can be transferred, shared, wrapped, or unwrapped, just like any other object.

See `mys::object_table::ObjectTable` for the object-preserving version of `Table`.
...
`Bag` is a *heterogeneous* map, so it can hold key-value pairs of arbitrary types (they don't need to match each other). Note that the `Bag` type does not have any type parameters for this reason. Like `Table`, `Bag` is also an object, so creating one with `mys::bag::new` requires supplying a `&mut TxContext` to generate an ID.

See `mys::bag::ObjectBag` for the object-preserving version of `Bag`.

---

The following sections explain the collection APIs. They use `mys::table` as the basis for code examples, with explanations where other modules differ.
...
This function takes the collection by value. If it contains no entries, it is deleted, otherwise the call fails. `mys::table::Table` also has a convenience function:
...
```move
let t1 = mys::table::new<u64, u64>(ctx);
let t2 = mys::table::new<u64, u64>(ctx);
