...
In MySocial, an intent is a compact struct that serves as the domain separator for a message that a signature commits to. The data that the signature commits to is an intent message. All signatures in MySocial must commit to an intent message, instead of the message itself.

## Motivation

In previous releases, MySocial used a special `Signable` trait that attached the Rust struct name as a prefix to the serialized data. This is not ideal because it's:
...
To see a detailed definition for each field, see each enum definition [in the source code](https://github.com/MystenLabs/mys/blob/0dc1a38f800fc2d8fabe11477fdef702058cf00d/crates/mys-types/src/intent.rs).
...
1.  Passes the digest to the signing API for each corresponding scheme of the signer. The supported signature schemes are pure Ed25519, ECDSA Secp256k1 and ECDSA Secp256r1. See [MySocial Signatures](./signatures.mdx#signature-requirements) for requirements of each scheme.

## Authority Signature

The authority signature is created using the protocol key. The data that it commits to is also an intent message `intent || message`. See all available intent scopes [in the source code](https://github.com/MystenLabs/mys/blob/0dc1a38f800fc2d8fabe11477fdef702058cf00d/crates/mys-types/src/intent.rs#L66)
...
The proof of possession is a BLS signature created using the authority's protocol private key, committed over the following message: `intent || pubkey || address || epoch`. Here `intent` is serialized to `[5, 0, 0]` representing an intent with scope as "Proof of Possession", version as "V0" and app_id as "MySocial". `pubkey` is the serialized public key bytes of the authority's BLS protocol key. `address` is the account address associated with the authority's account key. `epoch` is serialized to `[0, 0, 0, 0, 0, 0, 0, 0]`.
...
1. [Struct and enum definitions](https://github.com/MystenLabs/mys/blob/0dc1a38f800fc2d8fabe11477fdef702058cf00d/crates/mys-types/src/intent.rs)
2. [Test](https://github.com/MystenLabs/mys/blob/d009e82fa35bda4f2b3e7a86a9529d36c32a8159/crates/mys-types/src/unit_tests/intent_tests.rs)
3. User transaction intent signing [PR 1](https://github.com/MystenLabs/mys/pull/6445), [PR 2](https://github.com/MystenLabs/mys/pull/8321)
4. Authority intent signing [PR 1](https://github.com/MystenLabs/mys/pull/8154), [PR 2](https://github.com/MystenLabs/mys/pull/8726)
