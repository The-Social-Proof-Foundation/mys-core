---
title: MySocial-Related Research Papers
sidebar_label: Research Papers
---

This document contains a list of research papers that are relevant to MySocial and have been co-authored by at least one member of the team.
Some of the ideas of these papers are currently being integrated into MySocial, others are in our roadmap, and others are not in our roadmap
but could be integrated in the future. Start with the [MySocial Smart Contract Platform](/doc/mys.pdf) white paper, which contains
our latest design inspired by previous works below.

<div class="bg-mys-blue-light dark:bg-mys-gray-95 p-8 rounded-lg border border-solid border-mys-blue-dark">

<p class="text-mys-gray-80 text-xl font-bold italic">Latest</p>
...
- **Relevance:** Provides a performant leader election algorithm for partially-synchronous consensus protocol (such as Bullshark). MySocial may want to use it
...
- **Relevance:** Provides a partially-synchronous consensus protocol running over Narwhal. MySocial may want to use it instead of Tusk.
...
- **Relevance:** FastPay describes the core protocol at the heart of MySocial.
- **Summary:** FastPay allows a set of distributed validators, some of which are Byzantine, to maintain a high-integrity and availability
  ...
  side-infrastructure to support retail payments in fiat currencies. This is not the protocol MySocial uses, yet it proposes the basic safety mechanism
  that MySocial extends. FastPay is based on Byzantine Consistent Broadcast as its core primitive, foregoing the expenses of full atomic commit channels
...
MySocial blockchain protocol uses a variant of the Bullshark consensus
...
- **Relevance:** The consensus system that we will likely use to support shared-objects in MySocial.
...
- **Publication:** Not published
- **Relevance:** Less related to MySocial than the other papers, and the paper is in its early stages. It presents an algorithm to strengthen proof-of-Stake systems (like MySocial). The paper is, however, theoretical and not on our roadmap.
...
- **Relevance:** Less related to MySocial than the other papers, this provides a way to test implementations of consensus systems, such as Tusk and Bullshark.
...
- **Publication:** Not published yet (under submission)
- **Relevance:** Extends the FastPay design to support objects (rather than accounts), what MySocial actually uses. An additional contribution of this paper is
  to add strong privacy to FastPay transactions (but MySocial does not plan to do this).
