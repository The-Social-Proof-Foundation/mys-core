...
- `x-mys-rpc-version` to specify which RPC version to use (currently only one version is supported),
- `x-mys-rpc-show-usage` returns the response with extra query complexity information.

By default, each request returns the service's version in the response header: `x-mys-rpc-version`.

```sh
$ curl -i -X POST https://mys-mainnet.mystenlabs.com/graphql \
     --header 'x-mys-rpc-show-usage: true'                 \
...
```sh
$ curl -X POST https://mys-testnet.mystenlabs.com/graphql \
    --header 'Content-Type: application/json' \
    --data '{
...
- `pageInfo`, of type <a href="/references/mys-api/mys-graphql/reference/objects/page-info" data-noBrokenLinkCheck='true'>PageInfo</a>, which indicates whether there are more pages before or after the page returned.
...
The GraphQL service for MySocial RPC is rate-limited on all available instances to keep network throughput optimized and to protect against excessive or abusive calls to the service.
...
- [Querying MySocial RPC with GraphQL](../guides/developer/getting-started/graphql-rpc.mdx): Gets you started using GraphQL to query the MySocial RPC for on-chain data.  
- [Migrating to GraphQL](../guides/developer/advanced/graphql-migration.mdx): Guides you through migrating MySocial RPC projects from JSON-RPC to GraphQL. 
- [GraphQL for MySocial RPC](../references/mys-graphql.mdx): Auto-generated GraphQL reference for MySocial RPC.
