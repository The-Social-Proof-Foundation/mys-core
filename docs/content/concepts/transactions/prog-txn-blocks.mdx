...
description: Programmable transaction blocks are a group of commands that complete a transaction on MySocial.
---

On MySocial, a transaction is more than a basic record of the flow of assets. Transactions on MySocial are composed of a number of commands that execute on inputs to define the result of the transaction. Termed programmable transaction blocks (PTBs), these groups of commands define all user transactions on MySocial. PTBs allow a user to call multiple Move functions, manage their objects, and manage their coins in a single transaction--without publishing a new Move package. Designed with automation and transaction builders in mind, PTBs are a lightweight and flexible way of generating transactions. More intricate programming patterns, such as loops, are not supported, however, and in those cases you must publish a new Move package.
...
The remainder of this topic covers the semantics of the execution of the transaction commands. It assumes familiarity with the MySocial object model and the Move language. For more information on those topics, see the following documents:

- [Object model](../object-model.mdx)
- [Move Concepts](../mys-move-concepts.mdx)
...
- The `commands` value is a vector of commands, `[Command]`. The possible commands are:
  ...
  - `SplitCoins` splits off multiple (one or more) coins from a single coin. It can be any `mys::coin::Coin<_>` object.
  - `MergeCoins` merges multiple (one or more) coins into a single coin. Any `mys::coin::Coin<_>` objects can be merged, as long as they are all of the same type.
  ...
  - `Upgrade` upgrades an existing package. The upgrade is gated by the `mys::package::UpgradeCap` for that package.
...
- `SplitCoins`: produces (one or more) coins from a single coin. The type of each coin is `mys::coin::Coin<T>` where the specific coin type `T` matches the coin being split.
- `Publish`: returns the upgrade capability, `mys::package::UpgradeCap`, for the newly published package.
- `Upgrade`: returns the upgrade receipt, `mys::package::UpgradeReceipt`, for the upgraded package.
...
The most important thing to note at this stage is the effects on the gas coin. At the beginning of execution, the maximum gas budget (in terms of `MYS`) is withdrawn from the gas coin. Any unused gas is returned to the gas coin at the end of execution, even if the coin has changed owners.
...
- If the signature expects a `T`, the runtime checks the argument has type `T` and it is copied if `T: copy` and moved otherwise. No object in MySocial has `copy` because the unique ID field `mys::object::UID` present in all objects does not have the `copy` ability.
...
Object inputs have the type of their object `T` as you might expect. However, for `ObjectArg::Receiving` inputs, the object type `T` is instead wrapped as `mys::transfer::Receiving<T>`. This is because the object is not owned by the sender, but instead by another object. And to prove ownership with that parent object, you call the `mys::transfer::receive` function to remove the wrapper.
...
- The coin argument `CoinArg: Argument` must be a coin of type `mys::coin::Coin<T>` where `T` is the type of the coin being split. It can be any coin type and is not limited to `MYS` coins.
...
- The result of the command is a vector of coins, `mys::coin::Coin<T>`. The coin type `T` is the same as the coin being split, and the number of results matches the number of arguments
- For a rough signature expressed in Move, it is similar to a function `<T: key + store>(coin: &mut mys::coin::Coin<T>, amounts: vector<u64>): vector<mys::coin::Coin<T>>` where the result `vector` is guaranteed to have the same length as the `amounts` vector.
...
- The coin argument `CoinArg: Argument` must be a coin of type `mys::coin::Coin<T>` where `T` is the type of the coin being merged. It can be any coin type and is not limited to `MYS` coins.
- The coin arguments `ToMergeArgs: [Argument]` must be `mys::coin::Coin<T>` values where the `T` is the same type as the `CoinArg`.
- The coin argument `CoinArg` is taken by mutable reference.
- The merge arguments `ToMergeArgs` are taken by value (moved).
- The command does not produce any results (an empty result vector).
- For a rough signature expressed in Move, it is similar to a function `<T: key + store>(coin: &mut mys::coin::Coin<T>, to_merge: vector<mys::coin::Coin<T>>): ()`
...
The command produces a single result of type `mys::package::UpgradeCap`, which is the upgrade capability for the newly published package.
...
The `UpgradeTicket: mys::package::UpgradeTicket` is the upgrade ticket for the package being upgraded and is generated from the `mys::package::UpgradeCap`. The ticket is taken by value (moved).

The command produces a single result type `mys::package::UpgradeReceipt` which provides proof for that upgrade. For more details on upgrades, see [Upgrading Packages](../mys-move-concepts/packages/upgrade.mdx).
...
Any remaining MYS deducted from the gas coin at the beginning of execution is returned to the coin, even if the owner has changed. In other words, the maximum possible gas is deducted at the beginning of execution, and then the unused gas is returned at the end of execution (all in MYS). Because you can take the gas coin only by-value with `TransferObjects`, it will not have been wrapped or deleted.

The total effects (which contain the created, mutated, and deleted objects) are then passed out of the execution layer and are applied by the MySocial network.
...
```rust
Gas Coin: Coin<MYS> { id: gas_coin, ... value: 500_000u64 ... } // The maximum gas is deducted
Inputs: [
  Pure(/* @0x808 BCS bytes */ ...),
...
```rust
Gas Coin: Coin<MYS> { id: gas_coin, ... value: 499_900u64 ... }
Inputs: [
  Pure(/* @0x808 BCS bytes */ ...),
  Marketplace { id: market_id, ... },
  Pure(/* 100u64 BCS bytes */ ...),
...
Results: [
  [Coin<MYS> { id: new_coin, value: 100u64 ... }], // The result of SplitCoins
],
```
...
Assume that `buy_two` deletes its `Coin<MYS>` object argument and transfers the `Balance<MYS>` into the `Marketplace` object.
...
Make another Move call, this one to `mys::tx_context::sender` with the signature
