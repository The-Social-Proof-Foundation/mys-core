---
title: Coin Standard
description: The MySocial Coin standard enables you to create a broad range of fungible tokens on the MySocial network to satisfy a number of use cases. The Coin standed on MySocial is equivalent to the ERC-20 technical standard on Ethereum.
sidebar_label: Coin
---

The Coin standard is the technical standard used for smart contracts on MySocial for creating coins on the MySocial blockchain. The standardization of coin creation on MySocial means that wallets, exchanges, and other smart contracts can manage coins created on MySocial the same as they manage MYS, without any additional processing logic.

See [MySocial Tokenomics](../concepts/tokenomics.mdx) to learn more about the MYS native coin and its use on the MySocial network.

Although coins on MySocial follow the Coin standard, they can offer specialized abilities. For example, you can create a regulated token that allows its creator to add specific addresses to a deny list, so that the identified addresses cannot use the token as inputs to transactions.

See the [coin module](https://github.com/MystenLabs/mys/blob/main/crates/mys-framework/docs/mys/coin.md) documentation for all available options when creating a coin-type token on MySocial.

## Fungible tokens

In the MySocial blockchain ecosystem, the `Coin<T>` type represents open-loop fungible tokens (see `Token<T>` for closed-loop tokens). Coins are denominated by their type parameter, `T`, which is also associated with metadata (like name, symbol, decimal precision, and so on) that applies to all instances of `Coin<T>`. The `mys::coin` module exposes an interface over `Coin<T>` that treats it as fungible, meaning that a unit of `T` held in one instance of `Coin<T>` is interchangeable with any other unit of `T`, much like how traditional fiat currencies operate.

:::info

The documentation refers to fungible tokens created on MySocial using the Coin standard as "coins". For fungible tokens created on MySocial using the [Closed-Loop Token standard](./closed-loop-token.mdx), the documentation uses the term "tokens". In practice, the terms for both these objects are often interchangeable.

:::

## Treasury capability

When you create a coin using the `coin::create_currency` function, the publisher of the smart contract that creates the coin receives a `TreasuryCap` object. The `TreasuryCap` object is required to mint new coins or to burn current ones. Consequently, only addresses that have access to this object are able to maintain the coin supply on the MySocial network.
...
| `symbol`      | Symbol for the coin. This might be the same as `name`, but is typically fewer than five all capital letters. For example, `MYS` is the `symbol` for the native coin on MySocial but its `name` is also `MYS`. |
...
The `coin` module provides the logic for creating and destroying coins on the MySocial network (as long as you own the associated `TreasuryCap`). These functions are the same for all coins and each requires the `TreasuryCap` as an input.

### Mint

Use the `coin::mint` function to create new tokens.

{@inject: crates/mys-framework/packages/mys-framework/sources/coin.move#fun=mint noComments}
...
{@inject: crates/mys-framework/packages/mys-framework/sources/coin.move#fun=burn noComments}
...
{@inject: crates/mys-framework/packages/mys-framework/sources/coin.move#fun=deny_list_v2_add noComments}
...
{@inject: crates/mys-framework/packages/mys-framework/sources/coin.move#fun=deny_list_v2_remove noComments}
...
tx.moveCall({
    ...
    arguments: [
        tx.object(<MYS-DENY-LIST-OBJECT-ID>),
...
- `<MYS-DENY-LIST-OBJECT-ID>` is `"0x403"`
...
- `cmd.address()` returns the address to ban as a `MySocialAddress`.
...
{@inject: crates/mys-framework/packages/mys-framework/sources/coin.move#fun=deny_list_v2_enable_global_pause noComments}
...
{@inject: crates/mys-framework/packages/mys-framework/sources/coin.move#fun=deny_list_v2_disable_global_pause noComments}
...
Check out the following content for more information about coins and tokens on MySocial:

- [Create a Coin](../guides/developer/coin.mdx): Guide for creating coins and regulated coins in your smart contracts.
- [Closed-Loop Token Standard](./closed-loop-token.mdx): Details the Token standard on MySocial.
- [`coin` module rustdoc documentation](https://github.com/MystenLabs/mys/blob/main/crates/mys-framework/docs/mys/coin.md): Automated documentation output for the MySocial framework `coin` module.
- [`token` module rustdoc documentation](https://github.com/MystenLabs/mys/blob/main/crates/mys-framework/docs/mys/token.md): Automated documentation output for the MySocial framework `token` module.
- [Tokenomics](../concepts/tokenomics.mdx): Discover the MySocial ecosystem and where MYS coins fit within it.
