---
title: Kiosk Apps
description: Kiosk apps are a way to extend the functionality of MySocial Kiosk while keeping the core functionality intact. You can develop apps to add new features to a kiosk without having to modify the core code or move the assets elsewhere.
---

Kiosk apps are a way to extend the functionality of MySocial Kiosk while keeping the core functionality intact. You can develop apps to add new features to a kiosk without having to modify the core code or move the assets elsewhere.
...
Kiosk has an `id: UID` field like all objects on MySocial, which allows this object to be uniquely identified and carry custom dynamic fields and dynamic object fields. The Kiosk itself is built around dynamic fields and features like place and list are built around dynamic object fields.
...
In addition to limited and constrained access to storage, app permissions are also owner dependent. In the default setup, no party can place or lock items in a kiosk without its owner's consent. As a result, some cases such as collection bidding (offering X MYS for any item in a collection) requires the kiosk owner to approve the bid.
...
These are the key points in the lifecycle of a MySocial Kiosk app:
...
Currently, MySocial Kiosk has only two permissions: `place` (first bit) and `lock` and `place` (second bit). The remaining bits are reserved for future use.
...
- [NFT Rental repository](https://github.com/MystenLabs/mys/tree/main/examples/move/nft-rental): GitHub repo that contains the source code for the NFT Rental app.
