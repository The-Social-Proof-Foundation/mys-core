...
When you create a pool in DeepBook, you need to specify the `BaseAsset`, `QuoteAsset`, `tick_size`, and `lot_size`. You also need to pay a transaction fee (in MYS) to create a pool.
...
///   3. `[coin]` Object ID of the mys coin, to pay fee for create pool (100 MIST mys charged)

/// Returns the AccountCap
public fun create_pool<BaseAsset, QuoteAsset, MYS>(
...
Tokens are created with different decimal precision. For example, MYS has nine decimals of precision, while the wormhole wrapped ethereum USDC has
...
Let's walk through an example between MYS/USDC. For this example, assume MySocial price is $1.10 and is the base asset
of the MYS/USDC trading pair.

Thus, the calculation is `10^9 \* (1.10/ P_d) = 1000`, where `P_d` should be the price of mys in the DeepBook MYS/USDC trading pair. You can solve for `P_d = 1100000`,
so if the user wants to sell MYS at the market price in DeepBook, they should enter a price with six decimal points of precision.
...
flowchart LR
 ...
 subgraph MySocial_blockchain
    direction TB
    B["account_balances: Table&lt;address, Account&lt;T>>"]
    C("<table><tbody><tr><td>0xABCD</td><td>5 MYS</td></tr><tr><td>0xBCDE</td><td>12 MYS</td></tr><tr><td>...</td><td>...</td></tr></tbody></table>")
 end

 A ----> MySocial_blockchain
```
...
public placeLimitOrder(
    ...
    overrides.txb.moveCall({
        ...
        arguments: [
            ...
            overrides.txb.object(normalizeMySocialObjectId(this.configs.clock)),
