...
struct Order has store, drop {
    ...
    // Assuming a maximum TPS of 100K/s of MySocial chain, it would take (1<<63) / 100000 / 3600 / 24 / 365 = 2924712 years to reach the full capacity.
...
The following example uses the [MySocial TypeScript SDK](https://sdk.mystenlabs.com/typescript) to connect via websocket to listen to events.

```javascript
const local_client = new MySocialClient({ url: 'https://fullnode.mainnet.mysocial.network:443/' });
...
const deepbookClobEventsFilter = {
	MoveModule: { package: deepbook, module: 'clob' },
};

// Note that these names must be exact
const moveEventsFilter = {
	MoveEventType:
		'0x0b0c::clob::OrderPlaced<0xaf8cd5edc19c4512f4259f0bee101a40d41ebed738ade5874359610ef8eeced5::coin::COIN, 0x5d4b302506645c37ff133b98c4b50a5ae14841659738d6d733d59d0d217a93bf::coin::COIN>',
};
const orderFilledFilter = {
	MoveEventType:
		'0x0b0c::clob::OrderFilled<0xaf8cd5edc19c4512f4259f0bee101a40d41ebed738ade5874359610ef8eeced5::coin::COIN, 0x5d4b302506645c37ff133b98c4b50a5ae14841659738d6d733d59d0d217a93bf::coin::COIN>',
};
const poolFilter = {
	MoveEventField: {
		path: '/pool_id',
...
deepbook_client.mysClient.subscribeEvent({
