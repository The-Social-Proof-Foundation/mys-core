---
title: MySocial Object Display
description: The MySocial Object Display standard is a template engine that enables on-chain management of off-chain representation (display) for a type.
---

The MySocial Object Display standard is a template engine that enables on-chain management of off-chain representation (display) for a type. With it, you can substitute data for an object into a template string. The standard doesn’t limit the fields you can set. You can use the  `{property}` syntax to access all object properties, and then insert them as a part of the template string.

Use a `Publisher` object that you own to set `mys::display` for a type. For more information about `Publisher` objects, see [Publisher](https://examples.mysocial.network/basics/publisher.html) topic in *MySocial Move by Example*.

In MySocial Move, `Display<T>` represents an object that specifies a set of named templates for the type `T`. For example, for a type `0x2::capy::Capy` the display syntax is:  `Display<0x2::capy::Capy>`.

MySocial Full nodes process all objects of the type `T` by matching the `Display` definition, and return the processed result when you query an object with the `{ showDisplay: true }` setting in the query.
...
### An example MySocial Hero module
...
{
    "name": "{name}",
    "link": "https://mys-heroes.io/hero/{id}",
    "image_url": "https://mys-heroes.io/hero/{image_url}",
    "description": "A true Hero of the MySocial ecosystem!",
    "project_url": "https://mys-heroes.io",
    "creator": "Unknown MySocial Fan"
}
```

```move
/// Example of an unlimited "MySocial Hero" collection - anyone can
...
fun init(otw: MY_HERO, ctx: &mut TxContext) {
    ...
    let values = vector[
        ...
        b"https://mys-heroes.io/hero/{id}".to_string(),
        // For `image_url` use an IPFS template + `image_url` property.
        b"ipfs://{image_url}".to_string(),
        // Description is static for all `Hero` objects.
        b"A true Hero of the MySocial ecosystem!".to_string(),
        // Project URL is usually static
        b"https://mys-heroes.io".to_string(),
        // Creator field can be any
        b"Unknown MySocial Fan".to_string(),
    ];
...
MySocial Capys use dynamic image generation. When a Capy is born, its attributes determine the Capy’s appearance, such as color or pattern. When a user puts an item on a Capy, the Capy’s appearance changes. When users put multiple items on a Capy, there’s a chance of a bonus for a combination of items.
