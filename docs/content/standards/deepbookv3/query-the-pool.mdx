...
The `Pool` shared object represents a market, such as a MYS/USDC market. That `Pool` is the only one representing that unique pairing (MYS/USDC) and the pairing is the only member of that particular `Pool`. See [DeepBook Design](./design.mdx#pool) to learn more about the structure of pools.
