...
Browser extension wallets built for MySocial use the [Wallet standard](https://github.com/wallet-standard/wallet-standard/). This is a cross-chain standard that defines how dApps can automatically discover and interact with wallets.
...
```tsx
import { MYS_DEVNET_CHAIN, Wallet } from '@socialproof/wallet-standard';
...
class YourWallet implements Wallet {
	...
	// Return the MySocial chains that your wallet supports.
	get chains() {
		return [MYS_DEVNET_CHAIN];
	}
}
```
...
Features are standard methods consumers can use to interact with a wallet. To be listed in the MySocial
...
- `mys:signPersonalMessage` - Use to prompt the user to sign a personal message and return the
  message signature back to the dApp. Use this to verify the user’s public key.
- `mys:signTransaction` - Use to prompt the user to sign a transaction and return the serialized
  transaction and signature back to the dApp. This method does not submit the transaction for
  execution.
- `mys:signAndExecuteTransaction` - Use to prompt the user to sign a transaction, then submit it
  for execution to the blockchain.
- `mys:reportTransactionEffects` - Use to report the effects of a transaction executed in the dApp
  to the wallet. This allows the wallet to update its internal state to reflect the changes the transaction makes.
- `mys:signTransactionBlock` - The previous version of `mys:signTransaction`. Still
  implemented for compatibility with dApps that have not updated to the new feature.
- `mys:signAndExecuteTransactionBlock` - The previous version of `mys:signAndExecuteTransaction`. Still implemented for compatibility with dApps that have not updated to the new feature.
...
import {
  ...
  EventsOnMethod,
  MySocialFeatures,
  MySocialSignPersonalMessageMethod,
  MySocialSignTransactionMethod,
  MySocialSignAndExecuteTransactionMethod,
  MySocialReportTransactionEffectsMethod
} from "@mysten/wallet-standard";

class YourWallet implements Wallet {
	/* ... existing code from above ... */

  get features(): ConnectFeature & EventsFeature & MySocialFeatures {
    return {
      "standard:connect": {
        version: "1.0.0",
  ...
  #connect: ConnectMethod = () => {
	...
	#signPersonalMessage: MySocialSignPersonalMessageMethod = () => {
    // Your wallet's signTransaction implementation
  };

  #signTransaction: MySocialSignTransactionMethod = () => {
    // Your wallet's signTransaction implementation
  };

  #signAndExecuteTransaction: MySocialSignAndExecuteTransactionMethod = () => {
    // Your wallet's signAndExecuteTransaction implementation
  };

  #reportTransactionEffects: MySocialReportTransactionEffectsMethod = () => {
		...
		return someWalletAccounts.map(
			(walletAccount) =>
				...
				new ReadonlyWalletAccount({
					address: walletAccount.mysAddress,
					publicKey: walletAccount.pubkey,
					// The MySocial chains that your wallet supports.
					chains: [MYS_DEVNET_CHAIN],
...
```tsx
import { registerWallet } from '@socialproof/wallet-standard';
...
The Wallet standard has been updated from its original design to better support changes in the MySocial ecosystem. For example, the GraphQL service was introduced after Mainnet launched. The `mys:signAndExecuteTransactionBlock` feature is closely tied to the JSON RPC options and data structures, so its continued maintenance becomes increasingly difficult as the GraphQL service becomes more ubiquitous.

Consequently, the Wallet standard introduced the `mys:signAndExecuteTransaction` feature. The features of this method are more useful, regardless of which API you use to execute transactions. This usefulness comes at the expense
of flexibility in what `mys:signAndExecuteTransaction` returns.

To solve this problem, use the `mys:signTransaction` feature to sign transactions, and
...
To mitigate this, wallets can use the `mys:reportTransactionEffects` feature so that dApps can report
...
The `@mysten/mys/transaction` SDK exports the `SerialTransactionExecutor` class, which you can use
...
Using the combination of `mys:signTransaction` and `mys:reportTransactionEffects`, dApps can use
...
Construct the transaction separately with the `@mysten/mys` library and then sign it with the private key of the user. Use the `mys:signTransaction` feature to achieve this:

```tsx
wallet.features[mys:signTransaction].singTransaction(<Transaction>, <WalletAccount>);
```
...
To execute the transaction, use `MySocialClient` from `@mysten/mys`:

```tsx
const client: MySocialClient
...
wallet.features['mys:reportTransactionEffects'].reportTransactionEffects(
...
Many wallets abstract the above flow into one feature: `mys:signAndExecuteTransaction`. The required arguments for this feature are the raw transaction and the options with the desired information to be included in the response:
