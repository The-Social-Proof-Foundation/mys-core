...
`ActionRequest` is defined in the `mys::token` module and contains the following fields:
...
An example of a transaction implemented in TypeScript with mys.js, confirming an action request with a `TreasuryCap`. Here the admin account owns the `TreasuryCap`, which is used to mint and confirm the transfer request for the token:
