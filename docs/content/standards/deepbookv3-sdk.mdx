...
To work with DeepBook, you must create a `DeepBookClient`. To construct the `DeepBookClient`, pass in a `MySocialClient`, the sender address, and environment. The [MySocial TypeScript SDK](https://sdk.mystenlabs.com/typescript) provides the `MySocialClient` and key functionality necessary to process transactions. The following example imports those libraries, as well.

```tsx
import { DeepBookClient } from '@socialproof/deepbook-v3';
import { getFullnodeUrl, MySocialClient } from '@socialproof/mys/client';
import { decodeMySocialPrivateKey } from '@socialproof/mys/cryptography';
...
class DeepBookMarketMaker {
	...
	mysClient: MySocialClient; // For executing transactions
	...
	constructor(privateKey: string, env: 'testnet' | 'mainnet') {
		...
		this.mysClient = new MySocialClient({
	...
	getSignerFromPK = (privateKey: string): Ed25519Keypair => {
		const { schema, secretKey } = decodeMySocialPrivateKey(privateKey);
	...
	getActiveAddress() {
		return this.keypair.toMySocialAddress();
	}
}
```
...
class DeepBookMarketMaker {
	...
	async createBalanceManagerAndReinitialize() {
		...
		const res = await this.mysClient.signAndExecuteTransaction({
			transaction: tx,
			signer: this.keypair,
			options: {
				showEffects: true,
...
```tsx
import { decodeMySocialPrivateKey } from '@socialproof/mys.js/cryptography';
import { getFullnodeUrl, MySocialClient } from '@socialproof/mys/client';
...
export class DeepBookMarketMaker extends DeepBookClient {
	...
	constructor(
		...
		const address = resolvedKeypair.toMySocialAddress();

		super({
			address: address,
			env: env,
			client: new MySocialClient({
				url: getFullnodeUrl(env),
			}),
			balanceManagers: balanceManagers,
			adminCap: adminCap,
	...
	getActiveAddress() {
		return this.keypair.getPublicKey().toMySocialAddress();
	}
}
```
