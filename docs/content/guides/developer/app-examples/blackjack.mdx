...
The following documentation goes through an example implementation of the popular casino game blackjack on MySocial. This guide walks through its components, providing a detailed look at the module's functions, structures, constants, and their significance in the overall gameplay mechanism.

A deployed version of the blackjack game is online at [Mysten Blackjack](https://blackjack-mys.vercel.app/).
...
For more details on building a backend and deploying to MySocial, check out the [Coin Flip app example](./coin-flip.mdx).

You can also find the [full repository for this example here](https://github.com/MystenLabs/blackjack-mys).
...
The [`single_player_blackjack.move`](https://github.com/MystenLabs/blackjack-mys/blob/main/move/blackjack/sources/single_player_blackjack.move) module includes several constants that define game statuses and help track the game's progress:
...
- [health/](https://github.com/MystenLabs/blackjack-mys/blob/main/app/src/app/api/health): A simple healthcheck endpoint to check the API availability
- [helpers/](https://github.com/MystenLabs/blackjack-mys/blob/main/app/src/app/api/helpers): Multiple helper functions used in various endpoints
- [services/](https://github.com/MystenLabs/blackjack-mys/blob/main/app/src/app/api/services): The core logic of the backend, that signs and executes the transactions on the MySocial blockchain
- [utils/](https://github.com/MystenLabs/blackjack-mys/blob/main/app/src/app/api/utils): Reusable methods for signing the transactions as the dealer, and sponsoring them with Shinami, to avoid gas coins equivocation
...
An interesting aspect of developing a dApp on MySocial, that is coupled to using a full node with/without a load balancer, and requires attention by the developer, is the occurrence of read-after-write and write-after-write cases.
...
The main code of the frontend lies under the [app/src/](https://github.com/MystenLabs/blackjack-mys/blob/main/app/src/app/) directory.
The main sub-directories are:

- [app/](https://github.com/MystenLabs/blackjack-mys/blob/main/app/src/app/): The main code of the pages and the API endpoints.
- [components/](https://github.com/MystenLabs/blackjack-mys/blob/main/app/src/components): The reusable components of the app, organized in sub-directories.
- [hooks/](https://github.com/MystenLabs/blackjack-mys/blob/main/app/src/hooks): The custom hooks used in the app.
- [helpers/](https://github.com/MystenLabs/blackjack-mys/blob/main/app/src/helpers), [utils/](https://github.com/MystenLabs/blackjack-mys/blob/main/app/src/utils), [lib/](https://github.com/MystenLabs/blackjack-mys/blob/main/app/src/lib): Multiple helper functions and utilities.
- [types/](https://github.com/MystenLabs/blackjack-mys/blob/main/app/src/types): The types/interfaces used in the app.
- [styles/](https://github.com/MystenLabs/blackjack-mys/blob/main/app/src/styles): The global css files to style our app.

### Components and custom hooks for state management

- **Custom Hooks:** To keep the code as structured as possible, multiple custom hooks are utilized to manage the complex state of the game board at each step. The [useBlackjackGame](https://github.com/MystenLabs/blackjack-mys/blob/main/app/src/hooks/useBlackjackGame.ts) custom hook encapsulates the game state and logic, exposing all the required information (with fields such as `game`, `isInitialDealLoading`), and the required functionality (with methods such as `handleCreateGame` and `handleHit`) to display and play the game. Multiple additional custom hooks, such as [useCreateBlackjackGame](https://github.com/MystenLabs/blackjack-mys/blob/main/app/src/hooks/useCreateBlackjackGame.ts), and [useMakeMoveInBlackjackGame](https://github.com/MystenLabs/blackjack-mys/blob/main/app/src/hooks/useMakeMoveInBlackjackGame.ts) are encapsulating their own piece of state and logic to make the code readable and maintainable.

- **Component for Game Initialization:** The [StartGame](https://github.com/MystenLabs/blackjack-mys/blob/main/app/src/components/home/StartGame.tsx) component is implemented to facilitate the creation of a new game. It renders the [CollectMouseRandomness](https://github.com/MystenLabs/blackjack-mys/blob/main/app/src/components/home/CollectMouseRandomness.tsx) to capture the randomness and uses the `handleCreateGame` function of the [useBlackjackGame](https://github.com/MystenLabs/blackjack-mys/blob/main/app/src/hooks/useBlackjackGame.ts) hook to execute the create-game transaction.

- **Randomness Generation:** Fair game outcomes are also ensured by the [CollectMouseRandomness](https://github.com/MystenLabs/blackjack-mys/blob/main/app/src/components/home/CollectMouseRandomness.tsx) component. This component is using the [useMouseRandomness](https://github.com/MystenLabs/blackjack-mys/blob/main/app/src/hooks/useMouseRandomness.ts) custom hook, and is in charge of capturing some user's mouse movements and generating a random bytes array. This array is converted to a hexadecimal string (`randomness`) and used in the create-game transaction.

- **Card Displaying and Management:** The [DealerCards](https://github.com/MystenLabs/blackjack-mys/blob/main/app/src/components/home/DealerCards.tsx) and the [PlayerCards](https://github.com/MystenLabs/blackjack-mys/blob/main/app/src/components/home/PlayerCards.tsx) components are used to display the total points and the cards owned by the dealer and the player respectively.

- **Game Actions:** The [GameActions](https://github.com/MystenLabs/blackjack-mys/blob/main/app/src/components/home/GameActions.tsx) component is used to display the Hit and Stand buttons, and trigger the corresponding actions, as they are exported by the [useBlackjackGame](https://github.com/MystenLabs/blackjack-mys/blob/main/app/src/hooks/useBlackjackGame.ts) hook to execute the corresponding transactions.

- **BlackjackBanner**: The [BlackjackBanner](https://github.com/MystenLabs/blackjack-mys/blob/main/app/src/components/home/BlackjackBanner.tsx) component is used as a custom view to display when the player wins with a blackjack.

## Comparison: Blackjack and Coin Flip

### Similarities

- **Blockchain-Based Logic:** Both games are built on MySocial, leveraging its capabilities for decentralized applications. The core game logic for each resides in Move modules, ensuring secure and verifiable gameplay.
...
The complete app example can be found in [the blackjack-mys repo](https://github.com/MystenLabs/blackjack-mys).
