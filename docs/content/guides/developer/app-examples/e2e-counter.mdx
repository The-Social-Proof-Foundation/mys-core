...
This example walks you through building a basic distributed counter app, covering the full end-to-end flow of building your MySocial Move module and connecting it to your React MySocial dApp. The app allows users to create counters that anyone can increment, but only the owner can reset. 
...
[Example source code](https://github.com/MystenLabs/mys/blob/60bb8bdc274b9e5706fd916cd84c13f81e832529/sdk/create-dapp/templates/react-e2e-counter)
...
Learn more about taking [object references as input](../../../concepts/mys-move-concepts.mdx#entry-points)
...
Your smart contract is complete. You should be able to run the `mys move build` command from `react-e2e-counter/move/counter` and receive a response similar to the following:
```sh
...
BUILDING counter
```
You always run `mys move build` at the same level as your `Move.toml` file. After a successful build, you now have a `build` folder inside `react-e2e-counter/move/counter`.
...
The `create` function now creates a new MySocial `Transaction` and calls the `create` function from your Move module. The PTB is then signed and executed via the `useSignAndExecuteTransaction` hook. The `onCreated` callback is called with the new counter's ID when the transaction is successful.
...
<details>
  ...
  ```tsx
    ...
    import { isValidMySocialObjectId } from "@mysten/mys/utils";
    ...
    function App() {
      ...
      const [counterId, setCounter] = useState(() => {
        ...
        return isValidMySocialObjectId(hash) ? hash : null;
      });
...
<details>
  ...
  ```tsx
    ...
    export function Counter({ id }: { id: string }) {
      const currentAccount = useCurrentAccount();
      const { data, isPending, error, refetch } = useMySocialClientQuery("getObject", {
        id,
        options: {
          showContent: true,
          showOwner: true,
    ...
    function getCounterFields(data: MySocialObjectData) {
