...
This example walks you through building a coin flip dApp, covering the full end-to-end flow of building your MySocial Move module and connecting it to your React MySocial dApp. This coin flip dApp utilizes verifiable random functions (VRFs) to create a fair coin game on the MySocial blockchain. The user (human) plays against the house (module) and places a bet on either heads or tails. The user then either receives double their bet, or gets nothing, depending on the outcome of the game.
...
- [Frontend repository](https://github.com/mys-foundation/satoshi-coin-flip-frontend-example)
...
- **One-time witnesses:** The guide teaches you how to use [one-time witnesses](concepts/mys-move-concepts.mdx#one-time-witness) to ensure only a single instance of the `HouseData` object ever exists.
...
- **Events:** The guide teaches you how to emit events in your contracts, which can be used to track on-chain activity. For more information on events, see [Using Events](../mys-101/using-events.mdx) for practical usage of events on MySocial or [Events in The Move Book](https://move-book.com/programmability/events.html) to learn about event structure and how to emit them in Move.
...
1. Seven lines begin with the `use` keyword, which enables this module to use types and functions declared in other modules (in this case, they are all coming from the MySocial standard library).
...
```move title='house_data.move'
  public fun initialize_house_data(house_cap: HouseCap, coin: Coin<MYS>, public_key: vector<u8>, ctx: &mut TxContext) {
    ...
    let house_data = HouseData {
      ...
      house: ctx.sender(),
      public_key,
      max_stake: 50_000_000_000, // 50 MYS, 1 MYS = 10^9.
      min_stake: 1_000_000_000, // 1 MYS.
      fees: balance::zero(),
...
```move title='house_data.move'
  public fun top_up(house_data: &mut HouseData, coin: Coin<MYS>, _: &mut TxContext) {
...
- `top_up`: Add to the balance of the house to ensure that there is enough MYS for future games.
...
```move title='house_data.move'
  ...
  public(package) fun borrow_balance_mut(house_data: &mut HouseData): &mut Balance<MYS> {
    &mut house_data.balance
  }

  public(package) fun borrow_fees_mut(house_data: &mut HouseData): &mut Balance<MYS> {
    &mut house_data.fees
  }
...
module satoshi_flip::single_player_satoshi {
  ...
  use mys::mys::MYS;
  use mys::bls12381::bls12381_min_pk_verify;
  use mys::event::emit;
  use mys::hash::{blake2b256};
  use mys::dynamic_object_field::{Self as dof};
...
```move title='single_player_satoshi.move'
  ...
  fun internal_start_game(guess: String, counter: &mut Counter, coin: Coin<MYS>, house_data: &mut HouseData, fee_bp: u16, ctx: &mut TxContext): (ID, Game) {
...
Next, configure the MySocial CLI to use `testnet` as the active environment, as well. If you haven't already set up a `testnet` environment, do so by running the following command in a terminal or console:
...
To skip building the frontend and test out your newly deployed package, use the provided [Satoshi Coin Flip Frontend Example repository](https://github.com/mys-foundation/satoshi-coin-flip-frontend-example) and follow the instructions in the example's `README.md` file
...
- Tooling: [MySocial TypeScript SDK](https://sdk.mystenlabs.com/typescript) for basic usage on how to interact with MySocial using TypeScript.
...
- Tooling: [`@mysten/dapp`](https://sdk.mystenlabs.com/dapp-kit/create-dapp), used within this project to quickly scaffold a React-based MySocial dApp.
...
The following frontend code snippets include only the most relevant sections. Refer to the [Satoshi Coin Flip Frontend Example repository](https://github.com/mys-foundation/satoshi-coin-flip-frontend-example) for complete source code.
...
<form
  onSubmit={(e) => {
    ...
    const [houseStakeCoin] = txb.splitCoins(txb.gas, [
      MIST_PER_MYS * BigInt(houseStake),
    ]);
    ...
    execInitializeHouse(
      ...
      },
      {
        ...
        onSuccess: (result: MySocialTransactionBlockResponse) => {
          let houseDataObjId;
...
To use a [programmable transaction block](/concepts/transactions/prog-txn-blocks.mdx) (PTB) in MySocial, create a `Transaction`. To initiate a Move call, you must know the global identifier of a public function in your smart contract. The global identifier usually takes the following form:
...
import { useCurrentAccount, useMySocialClientQuery } from '@socialproof/dapp-kit';
...
// besides using MySocialClient directly
export function useFetchCounterNft() {
  ...
  const { data, isLoading, isError, error, refetch } = useMySocialClientQuery(
    'getOwnedObjects',
    {
...
This hook logic is very basic: if there is no current connected wallet, return empty data; otherwise, fetch the `Counter` object and return it. dApp Kit provides a React hook, `useMySocialClientQuery()`, that enables interaction with [MySocial RPC](references/mys-api.mdx) methods. Different RPC methods require different parameters. To fetch the object owned by a known address, use the [`getOwnedObjects` query](/mys-api-ref#mysx_getownedobjects).
...
// Player stake
const [stakeCoin] = txb.splitCoins(txb.gas, [MIST_PER_MYS * BigInt(stake)]);
...
Event subscriptions are deprecated. To learn future-safe methods to work with events, see [Using Events](../mys-101/using-events.mdx).
...
export function HouseFinishGame() {
  const mysClient = useMySocialClient();
  ...
  useEffect(() => {
    ...
    const unsub = mysClient.subscribeEvent({
      filter: {
  ...
  }, [housePrivHex, houseDataId, mysClient]);

  return null;
}
```

To get the underlying `MySocialClient` instance from the SDK, use `useMySocialClient()`. You want to subscribe to events whenever the `HouseFinishGame` component loads. To do this, use the React hook `useEffect()` from the core React library.

`MySocialClient` exposes a method called `subscribeEvent()` that enables you to subscribe to a variety of event types. `MySocialClient::subscribeEvent()` is actually a thin wrapper around the RPC method [`mysx_subscribeEvent`](/mys-api-ref#mysx_subscribeevent).
...
Congratulations, you completed the frontend. You can carry the lessons learned here forward when using the dApp Kit to build your next MySocial project.
