...
sequenceDiagram
    ...
    participant Blockchain as MySocial Blockchain
...
The `plinko::plinko` module combines various MySocial blockchain features, such as coin handling, event emissions, and dynamic object fields, to create on-chain Plinko games.
...
public fun start_game(counter: &mut Counter, num_balls: u64, coin: Coin<MYS>, house_data: &mut HouseData, ctx: &mut TxContext): ID {
...
public fun finish_game(game_id: ID, bls_sig: vector<u8>, house_data: &mut HouseData, num_balls: u64, ctx: &mut TxContext): (u64, address, vector<u8>) {
    ...
    let stake_per_ball = stake.value<MYS>() / num_balls;
    ...
    // Divide by 100 to adjust for multiplier scale and MYS units
...
let payout_coin: Coin<MYS> = coin::take(payout_balance_mut, total_funds_amount, ctx);
...
fun internal_start_game(counter: &mut Counter, num_balls: u64, coin: Coin<MYS>, house_data: &HouseData, fee_bp: u16, ctx: &mut TxContext): (ID, Game) {
...
public fun initialize_house_data(house_cap: HouseCap, coin: Coin<MYS>, public_key: vector<u8>, multiplier: vector<u64>, ctx: &mut TxContext) {
    ...
    let mut house_data = HouseData {
        ...
        house: ctx.sender(),
        public_key,
        max_stake: 10_000_000_000, // 10 MYS = 10^9.
