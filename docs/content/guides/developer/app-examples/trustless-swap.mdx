...
description: An app that performs atomic swaps on MySocial. Atomic swaps are similar to escrows but without requiring a trusted third party.
effort: large
---

{@include: ../../../snippets/app-examples-swap-source.mdx}

This guide demonstrates how to make an app that performs atomic swaps on MySocial. Atomic swaps are similar to escrows but without requiring a trusted third party.
...
- [Installed the latest version of MySocial](../getting-started/mys-install.mdx).
...
- [Acquired Devnet or Testnet](../getting-started/get-coins.mdx) tokens for development purposes.
  ...
  :::
- Read the basics of [shared versus owned objects](../mys-101/shared-owned.mdx).
...
- You have the latest version of MySocial installed. If you run `mys --version` in your terminal or console, it responds with the currently installed version.
- Your active environment is pointing to the expected network. Run `mys client active-env` to make sure. If you receive a warning about a client and server API version mismatch, update MySocial using the version in the relevant branch (`mainnet`, `testnet`, `devent`) of the MySocial repo.
- Your active address has MYS. Run `mys client balance` in your terminal or console. If there is no balance, [acquire MYS](../getting-started/get-coins.mdx) from the faucet (not available in Mainnet).
...
In this part of the guide, you write the Move contracts that perform the trustless swaps. The guide describes how to create the package from scratch, but you can use a fork or copy of the example code in the MySocial repo to follow along instead. See [Write a Move Package](../first-app/write-package.mdx) to learn more about package structure and how to use the MySocial CLI to scaffold a new project.
...
If you are targeting a network other than Testnet, be sure to update the `rev` value for the MySocial dependency.
...
The test starts with a helper function for creating an object, it doesn't matter what kind of object it is, as long as it has the `store` ability. The test uses `Coin<MYS>`, because it comes with a `#[test_only]` function for minting:
...
- The `test_scenario` module is used to provide access to a `&mut TxContext` in the test (necessary for creating new objects). Tests that don't need to simulate multiple transactions but still need access to a `TxContext` can use `mys::tx_context::dummy` to create a test context instead.
...
From your `escrow` folder, run `mys move test` in your terminal or console. If successful, you get a response similar to the following that confirms the package builds and your tests pass:
...
- [Full source code](https://github.com/MystenLabs/mys/blob/705ee1ed3ce8cfadc4597c6facb6769d7dfb5896/examples/trading/contracts/escrow/sources/shared.move)
- Concepts: [Shared Objects](../../../concepts/object-ownership/shared)
- Concepts: [Shared Object Deletion](https://blog.mysocial.network/ephemeral-shared-objects/)
...
`Locked` and `Key` objects are typically owned by the transaction sender, and so can be queried through the MySocial RPC, but `Escrow` objects are shared, and it is useful to be able to query them by their sender and recipient (so that users can see the trades they have offered and received).
...
{@inject: examples/trading/contracts/escrow/sources/shared.move#use=mys::event}
...
- Guide: [Using Events](../mys-101/using-events.mdx)

:::

:::checkpoint

You now have `shared.move` and `locked.move` files in your `sources` folder. From the parent `escrow` folder, run `mys move test` in your terminal or console. If successful, you get a response similar to the following that confirms the package builds and your tests pass: 
...
- [Full source code](https://github.com/MystenLabs/mys/tree/705ee1ed3ce8cfadc4597c6facb6769d7dfb5896/examples/trading/api)
- Reference: [JSON-RPC](https://docs.mysocial.network/mys-api-ref)
...
3. Deploy the MySocial package

  <details>
    <summary>
      Deployment instructions
    </summary>
    {@include: ../../../snippets/initialize-mys-client-cli.mdx}

  Next, configure the MySocial CLI to use `testnet` as the active environment. 
  ...
  ```sh
  $ mys client new-env --alias testnet --rpc https://fullnode.testnet.mysocial.network:443
  ```
  ...
  Before being able to publish your package to Testnet, you need Testnet MYS tokens. To get some, run the following command: 

  ```sh
  $ mys client faucet
  ```

  For other ways to get MYS in your Testnet account, see [Get MYS Tokens](/guides/developer/getting-started/get-coins).

  Now that you have an account with some Testnet MYS, you can deploy your contracts. 

  There are some helper functions to publish the smart contracts so you can create some demo data (for Testnet). The helper function to publish the smart contrqcts expects built smart contracts in both the `escrow` and `demo` directories. Run `mys move build` in both directories, if necessary. Be sure to update the MySocial dependency in the manifest to point to the correct source based on your environment. 
...
- Tooling: [MySocial Typescript SDK](https://sdk.mystenlabs.com/typescript). For basic usage on how to interact with MySocial with TypeScript.
- Tooling: [MySocial dApp Kit](https://sdk.mystenlabs.com/dapp-kit). To learn basic building blocks for developing a dApp in the MySocial ecosystem with React.js.
- Tooling: [`@mysten/dapp`](https://sdk.mystenlabs.com/dapp-kit/create-dapp). This is used within this project to quickly scaffold a React-based MySocial dApp.
...
<details>
  <summary>
    `MySocialObjectDisplay.tsx`
  </summary>
  {@inject: examples/trading/frontend/src/components/MySocialObjectDisplay.tsx}
</details>
...
Note that `dApp Kit` provides a set of hooks for making query and mutation calls to the MySocial blockchain. These hooks are thin wrappers around query and mutation hooks from `@tanstack/react-query`.
...
```tsx
import { useCurrentAccount, useMySocialClientInfiniteQuery } from "@mysten/dapp-kit";
import { MySocialObjectDisplay } from "@/components/MySocialObjectDisplay";
...
export function LockOwnedObjects() {
  ...
  const { data, fetchNextPage, isFetchingNextPage, hasNextPage, refetch } =
    useMySocialClientInfiniteQuery(
      "getOwnedObjects",
      {
  ...
  return (
    ...
    >
      {data?.map((obj) => (
        <MySocialObjectDisplay object={obj.data!}>
        </MySocialObjectDisplay>
      ))}
    </InfiniteScrollArea>
  );
}
...
Fetch the owned objects directly from MySocial blockchain using the `useMySocialClientInfiniteQuery()` hook from `dApp Kit`. This hook is a thin wrapper around MySocial blockchain RPC calls, reference the documentation to learn more about these [RPC hooks](https://sdk.mystenlabs.com/dapp-kit/rpc-hooks). Basically, supply the RPC endpoint you want to execute, in this case it's the [`getOwnedObjects` endpoint](https://docs.mysocial.network/mys-api-ref#mysx_getownedobjects). Supply the connected wallet account as the `owner`. The returned data is stored inside the cache at query key `getOwnedObjects`. In a future step you invalidate this cache after a mutation succeeds, so the data will be re-fetched automatically.
...
Use the `useMySocialClient()` hook from `dApp Kit` to retrieve the MySocial client instance configured in `src/main.tsx`. The `useSignTransaction()` function is another hook from `dApp kit` that helps to sign the transaction block using the currently connected wallet. It displays the UI for users to review and sign their transactions with their selected wallet. To execute a transaction block, use the `executeTransaction()` on the MySocial client instance of the MySocial TypeScript SDK.

### Generate demo data

:::info

The full source code of the demo bear smart contract is available at [Trading Contracts Demo directory](https://github.com/MystenLabs/mys/tree/main/examples/trading/contracts/demo)
...
As previously mentioned, this example uses `@tanstack/react-query` to query, cache, and mutate server state. Server state is data only available on remote servers, and the only way to retrieve or update this data is by interacting with these remote servers. In this case, it could be from an API or directly from MySocial blockchain RPC.

When you execute a transaction call to mutate data on the MySocial blockchain, use the `useMutation()` hook. The `useMutation()` hook accepts several inputs, however, you only need two of them for this example. The first parameter, `mutationFn`, accepts the function to execute the main mutating logic, while the second parameter, `onSuccess`, is a callback that runs when the mutating logic succeeds.
...
import { MySocialObjectData } from "@mysten/mys/client";
...
export function useLockObjectMutation() {
  ...
  return useMutation({
    mutationFn: async ({ object }: { object: MySocialObjectData }) => {
      if (!account?.address)
...
```tsx
import { useCurrentAccount, useMySocialClientQuery } from "@mysten/dapp-kit";
import { MySocialObjectDisplay } from "@/components/MySocialObjectDisplay";
...
export function Locked({
  ...
  const mysObject = useMySocialClientQuery(
    "getObject",
    {
      id: locked.itemId,
      options: {
...
This component uses tanstack's `useInfiniteQuery` instead of `useMySocialClientInfiniteQuery` since the data is being fetched from the example's API rather than MySocial. 
...
export function Escrow({ escrow }: { escrow: ApiEscrowObject }) {
  ...
  const mysObject = useMySocialClientQuery("getObject", {
    id: escrow?.itemId,
    options: {
      showDisplay: true,
      showType: true,
...
import { useTransactionExecution } from "@/hooks/useTransactionExecution";
import { ApiEscrowObject, ApiLockedObject } from "@/types/types";
import { useCurrentAccount, useMySocialClient } from "@mysten/dapp-kit";
import { MySocialObjectData } from "@mysten/mys/client";
import { Transaction } from "@mysten/mys/transactions";
import { useMutation, useQueryClient } from "@tanstack/react-query";
...
export function useAcceptEscrowMutation() {
  const currentAccount = useCurrentAccount();
  const client = useMySocialClient();
  const executeTransaction = useTransactionExecution();
  const queryClient = useQueryClient();
