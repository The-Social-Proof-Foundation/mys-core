...
description: Using the MySocial TypeScript SDK, you can create programmable transaction blocks to perform multiple commands in a single transaction.
---

This guide explores creating a programmable transaction block (PTB) on MySocial using the TypeScript SDK. For an overview of what a PTB is, see [Programmable Transaction Blocks](/concepts/transactions/prog-txn-blocks.mdx) in the Concepts section. If you don't already have the MySocial TypeScript SDK, follow the [install instructions](https://sdk.mystenlabs.com/typescript/install) on the MySocial TypeScript SDK site.

This example starts by constructing a PTB to send MySocial. If you are familiar with the legacy MySocial transaction types, this is similar to a `payMySocial` transaction. To construct transactions, import the `Transaction` class, and construct it:
...
tx.transferObjects([coin], tx.pure('0xSomeMySocialAddress'));
```
...
// Procure a list of some MySocial transfers to make:
...
After you have the Transaction defined, you can directly execute it with a `MySocialClient` and `KeyPair` using `client.signAndExecuteTransaction`.
...
Inputs are how you provide external values to PTBs. For example, defining an amount of MySocial to transfer, or which object to pass into a Move call, or a shared object.
...
MySocial supports following transaction commands:
...
- `tx.moveCall({ target, arguments, typeArguments })`: Executes a Move call. Returns whatever the MySocial Move call returns.
...
```ts
import { Inputs } from '@socialproof/mys/transactions';
...
The gas budget is represented in MySocial, and should take the gas price of the PTB into account.
...
To serialize a PTB for sending to a wallet, MySocial recommends using the `tx.serialize()` function, which returns an opaque string representation of the PTB that can be passed from the wallet standard dApp context to your wallet. This can then be converted back into a `Transaction` using `Transaction.from()`.
