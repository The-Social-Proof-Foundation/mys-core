...
The MySocial network stores countless objects on chain where Move code can perform actions using those objects. Tracking this activity is often desired, for example, to discover how many times a module mints an NFT or to tally the amount of MYS in transactions that a smart contract generates.

To support activity monitoring, Move provides a structure to emit events on the MySocial network. You can then leverage a custom indexer to process checkpoint data that includes events that have been emitted. See the [custom indexer](../advanced/custom-indexer.mdx) topic in the Advanced section to learn how to stream checkpoints and filter events continuously. 

If you don't want to run a custom indexer, you can poll the MySocial network to query for emitted events instead. This approach typically includes a database to store the data retrieved from these calls. The [Poll events](#poll-events) section provides an example of using this method.

## Move event structure

An event object in MySocial consists of the following attributes:
...
- `sender`: The MySocial network address that triggered the event.
...
To create an event in your Move modules, add the `mys::event` dependency.
...
The MySocial RPC provides a [queryEvents](/mys-api-ref#mysx_queryEvents) method to query on-chain packages and return available events. As an example, the following `curl` command queries the Deepbook package on Mainnet for a specific type of event:

```sh
$ curl -X POST https://fullnode.mainnet.mysocial.network:443 \
  ...
  "params": [
    {
      "MoveModule": {
        ...
        "type": "0x0b0c::clob_v2::DepositAsset<0x5d4b302506645c37ff133b98c4b50a5ae14841659738d6d733d59d0d217a93bf::coin::COIN>"
      }
    },
    null,
    3,
		...
		"data": [
			{
				...
				"type": "0x0b0c::clob_v2::WithdrawAsset<0x5d4b302506645c37ff133b98c4b50a5ae14841659738d6d733d59d0d217a93bf::coin::COIN>",
				"parsedJson": {
			...
			},
			{
				...
				"transactionModule": "deepbook_utils",
				"sender": "0x8b35e67a519fffa11a9c74f169228ff1aa085f3a3d57710af08baab8c02211b9",
				"type": "0x0b0c::clob_v2::OrderFilled<0x2::mys::MYS, 0x5d4b302506645c37ff133b98c4b50a5ae14841659738d6d733d59d0d217a93bf::coin::COIN>",
				"parsedJson": {
...
The TypeScript SDK provides a wrapper for the `mysx_queryEvents` method: [`client.queryEvents`](https://sdk.mystenlabs.com/typedoc/classes/_mysten_mys.client.MySocialClient.html#queryEvents).
...
```
import { useCurrentAccount, useSignAndExecuteTransaction, useMySocialClient } from "@mysten/dapp-kit";
...
| ----------------- | -------------------------------------------------------- | ----------------------------------------------------------------------------------------------------- |
| `All`             | All events                                               | `{"All": []}`                                                                                         |
| `Any`             | Events emitted from any of the given filter              | `{"Any": MySocialEventFilter[]}`                                                                           |
...
The [MySocial by Example](https://github.com/gdanezis/mys-by-example/blob/main/src/05_reading_events/bin/main.rs) repo on GitHub contains a code sample that demonstrates how to query events using the `query_events` function. The package that `PACKAGE_ID_CONST` points to exists on Mainnet, so you can test this code using Cargo. To do so, clone the `mys-by-example` repo locally and follow the [Example 05 directions](https://github.com/gdanezis/mys-by-example/tree/main/src/05_reading_events). 

```rust
use mys_sdk::{rpc_types::EventFilter, types::Identifier, MySocialClientBuilder};

const PACKAGE_ID_CONST: &str = "0x279525274aa623ef31a25ad90e3b99f27c8dbbad636a6454918855c81d625abc";

#[tokio::main]
async fn main() -> Result<(), anyhow::Error> {
    let mys_mainnet = MySocialClientBuilder::default()
        .build("https://fullnode.mainnet.mysocial.network:443")
        .await?;

    let events = mys_mainnet
        .event_api()
        .query_events(
...
You can use GraphQL to query events instead of JSON RPC. The following example queries are in the [`mys-graphql-rpc` crate](https://github.com/MystenLabs/mys/tree/main/crates/mys-graphql-rpc/examples/event_connection) in the MySocial repo. 

<details>
<summary>
Event connection
</summary>
{@inject: crates/mys-graphql-rpc/examples/event_connection/event_connection.graphql}
</details>
...
{@inject: crates/mys-graphql-rpc/examples/event_connection/filter_by_sender.graphql}
</details>

The [TypeScript SDK](https://sdk.mystenlabs.com/typedoc/modules/_mysten_mys.graphql.html) provides functionality to interact with the MySocial GraphQL service.
...
- Incorporate a [custom indexer](../advanced/custom-indexer.mdx) to take advantage of MySocial's micro-data ingestion framework.
- Poll the MySocial network on a schedule to query events.
...
To monitor events, you need a database to store checkpoint data. The [Trustless Swap](../app-examples/trustless-swap.mdx) example uses a Prisma database to store checkpoint data from the MySocial network. The database is populated from polling the network to retrieve emitted events. 
