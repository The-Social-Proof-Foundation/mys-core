...
Equivocation is when validators send conflicting information about objects on the network. Sometimes this is because of bad actors trying to subvert the integrity of the MySocial network. The network has measures in place to punish validators that might engage in such behavior.

The other source of equivocation does not originate from bad intentions. There are logic traps that smart contract code can inadvertently trigger, like when dealing with [sponsored transactions](https://docs.mysocial.network/guides/developer/mys-101/sponsor-txn#client-equivocation-risk). The effects of this type of equivocation can lock the objects your code interacts with until the end of the current epoch. This can lead to frustration for you, and more importantly, your users.
...
It's not just objects that can cause equivocation. Any object used in multiple transactions can be a source of equivocation if not handled properly. MySocial uses object versioning to track the status of objects across transactions and epochs. If a transaction modifies an object, then one of the results of that transaction is to update the version of that same object.
...
You can install the `myso-tool` (https://github.com/MystenLabs/mys/tree/main/crates/mys-tool) utility and use the `locked-object` command to check the locked status of the passed asset on a specific RPC network (`--fullnode-rpc-url` value). If you provide an address, locked-object checks if all the gas objects owned by that address are locked. Pass an object ID to check if that specific object is locked.

```sh
$ cargo run --bin myso-tool
```
```sh
$ myso-tool locked-object --address 0x02a212de6a9dfa3a69e22387acfbafbb1a9e591bd9d636e7895dcfc8de05f331 --fullnode-rpc-url <https://rpc.mainnet.mysocial.network:443>
```
```sh
$ myso-tool locked-object --id 0xd4c3ecf5eaa211da58c36495613899e70349f6048baaeca99596f1682e89c837 --fullnode-rpc-url <https://rpc.mainnet.mysocial.network:443>
```
...
- [Building Programmable Transaction Blocks](./building-ptb.mdx): Using the MySocial TypeScript SDK to create programmable transaction blocks, which perform multiple commands in a single transaction.
- [Object and Package Versioning](../../../concepts/versioning.mdx): Versioning provides the ability to upgrade packages and objects on the MySocial network.
- [Sponsored Transactions](./sponsor-txn.mdx): Sponsored transactions are a primitive on the MySocial blockchain that enable the execution of a transaction where you pay the gas fee for your users transactions.
- [MySocial TypeScript SDK](https://sdk.mystenlabs.com/typescript): The MySocial TypeScript SDK is a modular library of tools for interacting with the MySocial blockchain.
