...
description: On MySocial, you can use shared objects, owned objects, or both in your transactions. Weigh the benefits of each to decide the best approach for your project.
---

Objects on MySocial can be shared (accessible for reads and writes by any transaction) or owned (accessible for reads and writes by transactions signed by their owner).  Many applications can be built using a solution that either uses shared objects or only owned objects, with trade-offs for each that need to be weighed.
...
For more information on the types of objects that MySocial supports, see [Object Ownership](/concepts/object-ownership.mdx).
...
[Code Sample](https://github.com/MystenLabs/mys/blob/93e6b4845a481300ed4a56ab4ac61c5ccb6aa008/examples/move/escrow/sources/lock.move)
