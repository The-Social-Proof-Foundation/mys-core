...
description: Use the borrow module in the MySocial framework to include objects by reference in your programmable transaction blocks.
---

Everything on the MySocial blockchain is an object. When you develop Move packages for the MySocial network, you're typically manipulating or using on-chain objects in some way through functionality available in the MySocial API. For most API functions, you provide an object by reference.

References are a key construct when programming in Move and on MySocial. Most of the functionality available in the MySocial API takes objects by reference.
