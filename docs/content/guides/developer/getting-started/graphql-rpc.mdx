---
title: Querying MySocial RPC with GraphQL (Alpha)
description: Introductory guide to making queries of the MySocial RPC using the GraphQL service.
---
...
The quickest way to access the GraphQL service for MySocial RPC is through the online IDE that provides a complete toolbox for fetching data and executing transactions on the network. The online IDE provides features such as auto-completion (use Ctrl+Space or just start typing), built-in documentation (Book icon, top-left), multi-tabs, and more.

The online IDE is available for [Mainnet](https://mys-mainnet.mystenlabs.com/graphql) and [Testnet](https://mys-testnet.mystenlabs.com/graphql). This guide contains various queries that you can try directly in the IDE.

:::info
- Any existing addresses/object IDs in these examples refer to `mainnet` data only.
- Both [mainnet](https://mys-mainnet.mystenlabs.com/graphql) and [testnet](https://mys-testnet.mystenlabs.com/graphql) services are rate-limited to keep network throughput optimized.
:::

For more details about some concepts used in the examples below, please see the [GraphQL concepts](../../../concepts/graphql-rpc.mdx) page, and consult the [reference](../../../references/mys-graphql.mdx) for full documentation on the supported schema.
...
This example finds all the transactions that touched (modified/transferred/deleted) a given object. This is useful for when we want to trace the flow of a Coin/StakeMySocial/NFT.
...
```graphql
query ($objectID: MySocialAddress!) {
...
```graphql
query ($address: MySocialAddress!) {
  transactionBlocks(filter: {
    function: "0x3::mys_system::request_add_stake"
    sentAddress: $address
  }) {
    nodes {
      digest
...
Transaction execution takes in two arguments, `txBytes` and `signatures`. `txBytes` is the serialized unsigned transaction data, which can be generated when using the MySocial CLI's `client call` [command](../../../references/cli/client.mdx), to call a Move function by passing the `--serialize-unsigned-transaction` flag. The `signatures` can be generated using MySocial CLI's [keytool](../../../references/cli/keytool.mdx) command `mys keytool sign`. More information on MySocial CLI can be found [here](/references/cli).
...
You can find other examples in the [repository](https://github.com/MystenLabs/mys/tree/releases/mys-graphql-rpc-v2024.1.0-release/crates/mys-graphql-rpc/examples), grouped into sub-directories. For example, there are directories for [transaction block effects](https://github.com/MystenLabs/mys/tree/releases/mys-graphql-rpc-v2024.1.0-release/crates/mys-graphql-rpc/examples/transaction_block_effects), [protocol configs](https://github.com/MystenLabs/mys/tree/releases/mys-graphql-rpc-v2024.1.0-release/crates/mys-graphql-rpc/examples/protocol_configs), [stake connection](https://github.com/MystenLabs/mys/tree/releases/mys-graphql-rpc-v2024.1.0-release/crates/mys-graphql-rpc/examples/stake_connection), and more.

:::info
Examples in the repository are designed to work with the version of GraphQL built at the same revision. The links above point to examples intended for [GraphQL v2024.1](https://github.com/MystenLabs/mys/tree/releases/mys-graphql-rpc-v2024.1.0-release), the latest production version at the time of writing.
:::

## Related links

- [GraphQL migration](../advanced/graphql-migration.mdx): Migrating to GraphQL guides you through migrating MySocial RPC projects from JSON-RPC to GraphQL.
...
- [MySocial Testnet GraphiQL](https://mys-testnet.mystenlabs.com/graphql): MySocial GraphiQL IDE for the Testnet network.
- [MySocial Mainnet GraphiQL](https://mys-mainnet.mystenlabs.com/graphql): MySocial GraphiQL IDE for the Mainnet network.
