---
title: Install MySocial
description: Install the MySocial framework and required prerequisites on your system, including the MySocial command line interface to interact with the MySocial network.
---

The quickest way to install MySocial is using the binaries delivered with every release. If you require more control over the install process, you can install from source. To take advantage of containerization, you can utilize the Docker images in the `docker` folder of the mys repository.

## Supported operating systems {#supported-operating-systems}

MySocial supports the following operating systems:
...
If you use this method to install MySocial, you are all set. The quick install is ttttable for most use cases. The remaining installation methods are for those wanting more control over the installation process.
...
Each MySocial release provides a set of binaries for several operating systems. You can download these binaries from GitHub and use them to install MySocial.

<Tabs groupId="operating-systems">

<TabItem value="linux" label="Linux">
1. Go to https://github.com/MystenLabs/mys.
1. In the right pane, find the **Releases** section.

    ![MySocial releases in GitHub](./images/releases.png)
...
1. Extract all files from the .tgz file into the preferred location on your system. These instructions assume you extract the files into a `mys` folder at the user root of your system for demonstration purposes. Replace references to this location in subsequent steps if you choose a different directory.
...
1. Add the folder containing the extracted files to your `PATH` variable. To do so, you can update your `~/.bashrc` to include the location of the MySocial binaries. If using the suggested location, you type `export PATH=$PATH:~/mys` and press Enter.
...
1. Click the release tagged **Latest** to open the release's page.
1. In the **Assets** section of the release, select the .tgz compressed file that corresponds to your operating system.
1. Extract all files from the .tgz file into the preferred location on your system. These instructions assume you extract the files into a `mys` folder at the user root of your system. Replace references to this location in subsequent steps if you choose a different directory.
1. Navigate to the expanded folder. You should have the following extracted files: 
    
    {@include: ../../../snippets/lists/binaries-file-list.mdx}

1. Add the folder containing the extracted files to your `PATH` variable. To do so, you can update your `~/.zshrc` or `~/.bashrc` to include the location of the MySocial binaries. If using the suggested location, you type `export PATH=$PATH:~/mys` and press Enter.
1. Start a new terminal session or type `source ~/.zshrc` (or `.bashrc`) to load the new `PATH` value.
1. If running the binaries for the first time, you might receive an error from MacOS that prevents the binaries from running. If you receive this error, close the dialog and type `xattr -d com.apple.quarantine ~/mys/*` in your terminal and press Enter (be sure to adjust the path if different). 
...
1. Click the release tagged **Latest** to open the release's page.
1. In the **Assets** section of the release, select the .tgz compressed file that corresponds to your operating system.
1. Extract all files from the .tgz file into the preferred location on your system. These instructions assume you extract the files into a `mys` folder at the root of your C drive. Replace references to this location in subsequent steps if you choose a different directory.
...
1. In the Edit environment variable window, click **New** and add the path to your expanded folder. Using the example path, this would be `C:\mys`.
1. Click **OK**.
...
Running binaries other than `mys` might require installing prerequisites itemized in the following section. 

:::

#### Confirm the installation {#confirm-the-installation}

To confirm that MySocial installed correctly, type `mys --version` in your console or terminal and press Enter. The response should provide the MySocial version installed. If the console or terminal responds with a command not found error, make sure the full path to your MySocial binaries is included in your `PATH` variable.

## Install from Cargo {#install-mys-binaries-from-source}

Run the following command to install MySocial binaries from the `testnet` branch:

```sh
$ cargo install --locked --git https://github.com/MystenLabs/mys.git --branch testnet mys --features tracing
```

Enabling the `tracing` feature is important as it adds Move test coverage and debugger support in the MySocial CLI. Without it these two features will not be able to be used.
...
The command installs MySocial components in the `~/.cargo/bin` folder.

## Upgrade from Cargo

If you previously installed the MySocial binaries, you can update them to the most recent release with the same command you used to install them (changing `testnet` to the desired branch):
...
Follow the instructions in this topic to install the Rust crates (packages) required to interact with MySocial networks, including the MySocial CLI.

To install MySocial from source, you first need to install its [prerequisites](#prerequisites) for your operating system. After installing the supporting technologies, you can install [MySocial binaries from source](#install-mys-binaries-from-source).

You can also download the [source code](/references/contribute/mys-environment.mdx) to have local access to files.

#### Prerequisites {#prerequisites}

Your system needs the following prerequisites available to successfully install MySocial.

#### Rust and Cargo {#rust-and-cargo}

MySocial requires Rust and Cargo (Rust's package manager) on all supported operating systems. The suggested method to install Rust is with `rustup` using cURL.
...
MySocial uses the latest version of Cargo to build and manage dependencies. See the [Cargo installation](https://doc.rust-lang.org/cargo/getting-started/installation.html) page on the Rust website for more information.
...
MySocial requires the following additional tools on computers running Windows.
...
**Known issue** - The `mys console` command does not work in PowerShell.

</TabItem>

</Tabs>

## Using MySocial from command line

With MySocial installed, you can interact with MySocial networks using the MySocial CLI. For more details, see the [MySocial CLI](/references/cli.mdx) reference.

## Installing MySocial developer tools
...
Now that you have MySocial installed, it's time to start developing. Check out the following topics to start working with MySocial:

- Read about the [MySocial CLI](/references/cli.mdx), the most straightforward way to start exploring MySocial networks.
