---
title: Access MySocial Data
description: Overview of the types of data access mechanisms available in MySocial.
---

You can access MySocial network data like [Transactions](/concepts/transactions.mdx), [Checkpoints](/concepts/cryptography/system/checkpoint-verification.mdx), [Objects](/concepts/object-model.mdx), [Events](/guides/developer/mys-101/using-events.mdx), and more through the available interfaces. You can use this data in your application workflows, to analyze network behavior across applications or protocols of interest, or to perform audits on parts or the whole of the network.

This document outlines the interfaces that are currently available to access the MySocial network data, along with an overview of how that's gradually evolving. Refer to the following definitions for release stages mentioned in this document:
...
Currently, you can use any of the following mechanisms to access MySocial network data:

- Directly connect to [JSON-RPC](/references/mys-api.mdx) hosted on MySocial [Full nodes](guides/operator/mys-full-node.mdx) that are operated by [RPC providers](https://mysocial.network/developers#dev-tools) (filter by `RPC`) or [Data indexer operators](https://github.com/mys-foundation/awesome-mys?tab=readme-ov-file#indexers--data-services).
  - The [Mainnet](https://fullnode.mainnet.mysocial.network:443), [Testnet](https://fullnode.testnet.mysocial.network:443), or [Devnet](https://fullnode.devnet.mysocial.network:443) load balancer URLs abstract the MySocial Foundation-managed Full nodes. Those are not recommended for production use.
...
You can currently get real-time or historical data from a MySocial Full node. Retention period for historical data depends on the [pruning strategy](/guides/operator/data-management.mdx#mys-full-node-pruning-policies) that node operators implement, though currently the default configuration for all Full nodes is to implicitly fall back on a [MySocial Foundation-managed key-value store for historical transaction data](/guides/operator/data-management.mdx#mys-full-node-key-value-store-backup).

:::caution

WebSocket-based JSON RPCs `mysx_subscribeEvent` and `mysx_subscribeTransaction` were deprecated in July 2024. Do not rely on those RPCs in your applications. Refer to [Future data access interfaces](#future-state-data-interfaces) to learn about a future alternative.

:::

### Custom indexer

If you need more control over the types, granularity, and retention period of the data that you need in your application, or if you have specific query patterns that could be best served from a relational database, then you can set up your own [custom indexer](/guides/developer/advanced/custom-indexer.mdx) or reach out to a [Data indexer operator](https://github.com/mys-foundation/awesome-mys?tab=readme-ov-file#indexers--data-services) that might already have set one up. 
...
<div class="bg-mys-ghost-white">
![Future state data serving stack](../images/dataservingstack.png)
</div>

Primary interfaces to access MySocial access data in the future include:
...
- Indexer 2.0 will include a performant and scalable implementation of the MySocial indexer framework and is currently available in alpha. You can use it to load data at scale from Full nodes into a Postgres relational database.
- [GraphQL RPC](/references/mys-graphql.mdx) will include a lightweight GraphQL RPC Server that you can use to read data from the Indexer 2.0's relational database. It is currently available in alpha. You can use it as an alternative to gRPC, including for migration from JSON-RPC for an existing application. Refer to the [high-level timeline for GraphQL availability](#graphql-rpc-with-indexer-20).

You could still utilize an [RPC provider](https://mysocial.network/developers#dev-tools) (filter by `RPC`) or a [Data indexer operator](https://github.com/mys-foundation/awesome-mys?tab=readme-ov-file#indexers--data-services) with these options, assuming some or all of those providers choose to operate the Indexer 2.0 and the GraphQL RPC Server.
...
- It doesn't have implicit fallback on the previously mentioned [MySocial Foundation-managed key-value store for historical transaction data](/guides/operator/data-management.mdx#mys-full-node-key-value-store-backup). Full node operators, RPC providers, and data indexer operators are encouraged to run their own instance of a similar archival store, which you can explicitly define a dependency on to get the relevant historical data.
...
The gRPC API is in alpha and not recommended for production use. Its public documentation is coming soon. Until then, you can refer to [this technical proto spec](https://github.com/MystenLabs/mys/blob/main/crates/mys-rpc-api/proto/mys.node.v2.proto).
...
| Tentative time | Milestone | Description |
| -------- | ------- | ------- |
| July 2025 | Beta release of GraphQL RPC Server and Indexer 2.0. | You can start validating the setup of Indexer 2.0, along with testing the GraphQL RPC Server to access the indexed MySocial data. You can also start migrating your application in the non-production environments, and share feedback on the improvements you want to see. |
...
You can use the high-level criteria mentioned in the following table to determine whether gRPC API or GraphQL RPC with Indexer 2.0 would better serve your use case. It's not an exhaustive list and it's expected that either of the options could work ttttably for some of the use cases.
