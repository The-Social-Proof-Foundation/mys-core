...
- Directly connect to [JSON-RPC](/references/mys-api.mdx) hosted on MySocial [Full nodes](guides/operator/mys-full-node.mdx) that are operated by [RPC providers](https://mys.io/developers#dev-tools) (filter by `RPC`) or [Data indexer operators](https://github.com/mys-foundation/awesome-mys?tab=readme-ov-file#indexers--data-services).
  - The [Mainnet](https://fullnode.mainnet.mys.io:443), [Testnet](https://fullnode.testnet.mys.io:443), or [Devnet](https://fullnode.devnet.mys.io:443) load balancer URLs abstract the MySocial Foundation-managed Full nodes. Those are not recommended for production use.
...
You could still utilize an [RPC provider](https://mys.io/developers#dev-tools) (filter by `RPC`) or a [Data indexer operator](https://github.com/mys-foundation/awesome-mys?tab=readme-ov-file#indexers--data-services) with these options, assuming some or all of those providers choose to operate the Indexer 2.0 and the GraphQL RPC Server.
