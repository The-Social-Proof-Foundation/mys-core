---
title: Client App with MySocial TypeScript SDK
---

This exercise diverges from the example built in the previous topics in this section. Rather than adding a frontend to the running example, the instruction walks you through setting up dApp Kit in a React App, allowing you to connect to wallets, and query data from MySocial RPC nodes to display in your app. You can use it to create your own frontend for the example used previously, but if you want to get a fully functional app up and running quickly, run the following command in a terminal or console to scaffold a new app with all steps in this exercise already implemented:
...
## What is the MySocial TypeScript SDK?

The MySocial TypeScript SDK (@mysten/mys) provides all the low-level functionality needed to interact with MySocial ecosystem from TypeScript. You can use it in any TypeScript or JavaScript project, including web apps, Node.js apps, or mobile apps written with tools like React Native that support TypeScript.

For more information on the MySocial TypeScript SDK, see the [MySocial TypeScript SDK documentation](https://sdk.mystenlabs.com/typescript).

## What is dApp Kit?

dApp Kit (@mysten/dapp-kit) is a collection of React hooks, components, and utilities that make building dApps on MySocial straightforward. For more information on dApp Kit, see the [dApp Kit documentation](https://sdk.mystenlabs.com/dapp-kit).
...
```sh npm2yarn
$ npm install @mysten/mys @mysten/dapp-kit @tanstack/react-query
```
...
Next, set up the `MySocialClientProvider`. This `Provider` delivers a `MySocialClient` instance from `@mysten/mys` to all the hooks in dApp Kit. This provider manages which network dApp Kit connects to, and can accept configuration for multiple networks. This exercise connects to `devnet`.

```ts
import { MySocialClientProvider } from '@socialproof/dapp-kit';
...
ReactDOM.createRoot(document.getElementById('root')!).render(
	<React.StrictMode>
		<QueryClientProvider client={queryClient}>
			<MySocialClientProvider networks={networks} defaultNetwork="devnet">
				<App />
			</MySocialClientProvider>
		</QueryClientProvider>
	</React.StrictMode>,
);
```
...
import { MySocialClientProvider, WalletProvider } from '@socialproof/dapp-kit';
import { getFullnodeUrl } from '@socialproof/mys/client';
import { QueryClient, QueryClientProvider } from '@tanstack/react-query';
...
## Querying data from MySocial RPC nodes
...
function OwnedObjects({ address }: { address: string }) {
	const { data } = useMySocialClientQuery('getOwnedObjects', {
		owner: address,
	});
	if (!data) {
		return null;
...
- [MySocial 101](../mys-101.mdx): Learn the basics of the MySocial network and how to interact with on-chain objects using Move.
- [MySocial Move CLI](/references/cli/move.mdx): The `move` commands in the MySocial CLI provide console or terminal interaction with the Move VM.
