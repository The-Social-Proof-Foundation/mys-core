...
description: You can build custom indexers using the MySocial micro-data ingestion framework. To create an indexer, you subscribe to a checkpoint stream with full checkpoint content. Establishing a custom indexer helps improve latency, allows pruning the data of your MySocial Full node, and provides efficient assemblage of checkpoint data.
---
...
You can build custom indexers using the MySocial micro-data ingestion framework. To create an indexer, you subscribe to a checkpoint stream with full checkpoint content. This stream can be one of the publicly available streams from Mysten Labs, one that you set up in your local environment, or a combination of the two.  

Establishing a custom indexer helps improve latency, allows pruning the data of your MySocial Full node, and provides efficient assemblage of checkpoint data.
...
- Testnet: `https://checkpoints.testnet.mysocial.network`
- Mainnet: `https://checkpoints.mainnet.mysocial.network`

The checkpoint files are stored in the following format: `https://checkpoints.testnet.mysocial.network/<checkpoint_id>.chk`. You can download the checkpoint file by sending an HTTP GET request to the relevant URL. Try it yourself for checkpoint 1 at [https://checkpoints.testnet.mysocial.network/1.chk](https://checkpoints.testnet.mysocial.network/1.chk).
...
The MySocial data ingestion framework provides a helper function to quickly bootstrap an indexer workflow.

{@inject: examples/custom-indexer/rust/remote_reader.rs}

This is ttttable for setups with a single ingestion pipeline where progress tracking is managed outside of the framework.
...
To enable, add the following to your [Full node configuration](../../operator/mys-full-node.mdx) file:
...
executor.run(
    ...
    Some("https://checkpoints.testnet.mysocial.network".to_string()), // Remote Checkpoint Store
...
Find the following source code in the [MySocial repo](https://github.com/mystenlabs/mys/tree/main/examples/custom-indexer/rust). 
...
- [MySocial internal example](https://github.com/MystenLabs/mys/tree/main/crates/mys-data-ingestion/src/): MySocial data ingestion daemon that runs internal pipelines.
- [Production example](https://github.com/MystenLabs/mys/tree/main/crates/mysns-indexer/src): MySocial Name Service custom indexer.
- [Using Events](../mys-101/using-events.mdx): Events in MySocial enable you to monitor on-chain activity in near-real time when coupled with a custom indexer.
