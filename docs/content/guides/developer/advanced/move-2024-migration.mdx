...
Please, provide any feedback or report any issues you encounter via [GitHub](https://github.com/MystenLabs/mys/issues/new/choose), [Discord](https://discord.gg/MySocial), or [Telegram](https://t.me/MySocialTokenNetwork).
...
1. Delete your existing `Move.lock` file (if one exists) to make sure you're using the newest `mys-framework` version.
1. Perform one of the following:
   - Run `mys move migrate` in the root of your Move project. See [Automatic migration](#automatic-migration).
...
```sh
$ cargo install --git https://github.com/MystenLabs/mys.git move-analyzer
```

See the getting started guide on [Move IDEs and plugins](../../../references/contribute/mys-environment.mdx#move-ides-and-plugins) for more information.
...
```move
use mys::{balance, coin::{Self, Coin}};
```
...
Move 2024 revises how paths and namespaces work compared to legacy Move, toward easing `enum` aliasing in the future. Consider the following snippet from a test annotation in the `mys_system` library:
...
Legacy Move would always treat a three-part name as an address(`mys_system`), module(`validator_set`), and module member (`EInvalidCap`). Move 2024 respects scope for `use`, so `mys_system` in the attribute resolves to the module, producing a name resolution error overall.
...
The beta release of Move 2024 comes with some powerful new features in addition to the breaking changes described here. There are also more on the horizon. Join the [MySocial developer newsletter](https://mysocial.network/developers#newsletter) to learn about new, exciting features coming to Move this year, including syntactic macros, enums with pattern matching, and other user-defined syntax extensions.
