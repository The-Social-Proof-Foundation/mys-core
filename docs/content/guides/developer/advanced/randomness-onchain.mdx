...
`Random` has a reserved address `0x8`. See <a href="/references/framework/mys-framework/random" data-noBrokenLinkCheck='true'>random.move</a> for the Move APIs for accessing randomness on MySocial.
...
entry fun insecure_play(r: &Random, payment: Coin<MYS>, ...) {
  ...
...
See [this](https://github.com/MystenLabs/mys/blob/main/examples/move/random/random_nft/sources/example.move#L117-L142) for example implementation.
...
module games::dice {
  ...
  public fun play_dice(guess: u8, fee: Coin<MYS>, r: &Random, ctx: &mut TxContext): Ticket {
    // Pay for the turn
...
For example, consider the `entry play_dice(guess: u8, fee: Coin<MYS>, r: &Random, ctx: &mut TxContext): Ticket { … }` function defined earlier, the attacker can publish the function
...
To protect against PTB-based composition attacks, MySocial rejects PTBs that have commands that are not `TransferObjects` or `MergeCoins` following a `MoveCall` command that uses `Random` as an input.
...
- [MySocial Framework Reference](../../../references/framework)
  - <a href="/references/framework/mys-framework/random" data-noBrokenLinkCheck="true">
    	random.move
    </a>
- [Raffle example](https://github.com/MystenLabs/mys/blob/main/examples/move/random/raffles)
- [MySocial Client CLI](../../../references/cli/client)
