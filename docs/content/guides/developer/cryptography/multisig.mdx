...
Use `mys keytool` to list the addresses you created in the previous section.
...
```sh
$ mys client transfer --to $ACTIVE --object-id $COIN --gas-budget <GAS-AMOUNT> --serialize-unsigned-transaction
```
...
```sh
$ mys keytool sign --address $ADDRESS1 --data $TXBYTES
```
```sh
$ mys keytool sign --address $ADDRESS2 --data $TXBYTES
```
