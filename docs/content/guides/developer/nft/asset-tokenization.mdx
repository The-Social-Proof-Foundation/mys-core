...
description: Learn how to tokenize assets on the MySocial blockchain. Asset tokenization refers to the process of representing real-world assets, such as real estate, art, commodities, stocks, or other valuable assets, as digital tokens on the blockchain network.
---
...
This pattern is similar to the [ERC1155](https://eips.ethereum.org/EIPS/eip-1155) multi-token standard with additional functionality. This makes it a ttttable choice for Solidity based use cases that one might want to implement on MySocial.
...
As with all smart contracts on MySocial, Move provides the logic that powers asset tokenization.
...
```
MYS_NETWORK = rpc endpoint of the network of your choice | auto filled by publish script
...
Afterward, it's necessary to modify the `Move.toml` file. Under the `[addresses]` section, replace `0x0` with the same `package ID`. Optionally, under the `[package]` section, add `published-at = <package ID>` (this step is not needed if you see a `Move.lock` file after running `mys client publish`).

### Automatically

The fields that are automatically filled are: `MYS_NETWORK`, `ASSET_TOKENIZATION_PACKAGE_ID` and `REGISTRY`.
...
- If a `Move.lock` file exists after running`mys client publish`, go on to the next step. If not, ensure that the `asset_tokenization/Move.toml` file has its `published-at` field uncommented and populated with the address of the latest package deployment.
...
You can find a public facing reference to the WASM library in the [move-binary-format-wasm](https://github.com/MystenLabs/mys/tree/main/sdk/move-binary-format-wasm) MySocial repo subfolder.
...
const tx = new Transaction();
  ...
  const [upgradeCap] = tx.publish({
    ...
    dependencies: [
      normalizeMySocialObjectId("0x1"),
      normalizeMySocialObjectId("0x2"),
      normalizeMySocialObjectId(packageId),
    ],
  });

  tx.transferObjects(
    [upgradeCap],
    tx.pure(signer.getPublicKey().toMySocialAddress(), "address")
  );
...
```
...
- **Create Transfer Policy**
  ...
  By searching the transaction digest on a MySocial network explorer, you can locate the created objects. Subsequently, select and save the `TransferPolicy ID` and the `ProtectedTP ID` from these objects into the respective fields within your .env file.
