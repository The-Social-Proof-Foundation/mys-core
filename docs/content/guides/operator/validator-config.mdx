---
title: MySocial Validator Node Configuration
description: Learn how to set up, configure, and manage a MySocial Validator node, including staking, reference gas price, and tallying rules.
---

Validators on the MySocial network run special nodes and have additional tasks and responsibilities beyond those of Full node operators.

## Requirements to run a validator on MySocial 

To run a MySocial validator, you must set up and configure a MySocial Validator node. After you have a running node, you must have a minimum of 30 million MYS in your staking pool to join the validator set on the MySocial network.

To learn how to set up and configure a MySocial Validator node, see [MySocial for Node Operators](https://github.com/MystenLabs/mys/blob/main/nre/mys_for_node_operators.md) on GitHub. The guide includes all of the information you need to configure your Validator node. It also provides guidance on the tasks you must perform after you join the validator set.

Specific steps you must take include:

- Install and configure MySocial
...
Suggested minimum hardware specifications to run a MySocial Validator node:
...
The total voting power on MySocial is always 10,000, regardless of the amount staked. Therefore, the quorum threshold is 6,667. There is no limit to the amount of MYS users can stake with a validator. Each validator has consensus voting power proportional to MYS in its staking pool, with one exception: the voting power of an individual validator is capped at 1,000 (10% of the total). If a validator accumulates more than 10% of total stake, the validator's voting power remains fixed at 10%, and the remaining voting power is spread across the rest of the validator set.

### User staking and withdrawals

When users stake MYS tokens, these MYS objects are wrapped into StakedMYS objects. The calculation to determine each user's relative ownership of the staking pool is done directly with the timestamp of the StakedMYS object (which determines the moment at which the deposit took place) and the change in the exchange rates between the deposit epoch and the withdrawal epoch. Each staking pool's data structure contains a time series with that pool's exchange rates. These exchange rates can be used to determine the withdrawals of any of the pool's stakers.
...
MYS withdrawn at E' = ( MYS deposited at E ) * ( Exchange Rate at E'-1 / Exchange Rate at E )
...
Stake subsidies are intended to subsidize the network during its early phases and are funded by a 10% allocation of MYS tokens. After this allocation depletes, the entirety of stake rewards will be made up of gas fees collected through regular network operations.
...
- **Gas Fees:** Each epoch's amount depends on the total gas fees collected throughout the epoch. Each MySocial transaction pays gas fees depending on two variables, the amount of executed gas units and the gas price:
...
A stake deposit request goes into a pending state immediately in the staking pool as soon as it is made. MySocial Wallet reflects any pending stake deposit requests for the user's account. However, pending stake deposit requests do not take effect until the end of the epoch during which the request is made.

A withdrawal (un-stake) request is processed immediately as soon as it is received. The staker obtains the originally deposited MYS together with all accrued stake rewards up to the previous epoch boundary – in other words, they do not include stake rewards for the current epoch.

Users can't withdraw a portion of their active stake. They must withdraw all staked MYS at the same time. Users can, however, stake using multiple `StakedMySocial` objects by splitting their MYS into multiple coins. They can then perform a partial withdrawal from a validator by un-staking only some of the `StakedMYS` objects.

## Reference gas price 

MySocial is designed such that end-users can expect the gas price to be stable and predictable during regular network operations. This is achieved by having validators set the network's reference gas price at the beginning of each epoch.
...
```sh
$ mys client call --package <PACKAGE-ID> --module mys_system --function request_set_gas_price --args 0x5 \"42\" --gas-budget <GAS-AMOUNT>
```
...
For example, to report a validator whose MySocial address is `0x44840a79dd5cf1f5efeff1379f5eece04c72db13512a2e31e8750f5176285446` as bad or non-performant, run:

```sh
$ mys client call --package <PACKAGE-ID> --module mys_system --function report_validator --args 0x5 0x44840a79dd5cf1f5efeff1379f5eece04c72db13512a2e31e8750f5176285446 --gas-budget <GAS-AMOUNT>
```
