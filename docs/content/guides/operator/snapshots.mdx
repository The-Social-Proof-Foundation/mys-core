...
description: Database snapshots of the MySocial network enable Full node operators a way to bootstrap a Full node without having to execute all the transactions that occurred after genesis.
---

Database snapshots provide a point-in-time view of a database's store. In MySocial, the database snapshot captures a running database's view of the MySocial network from a particular node at the end of an epoch. While validators can enable snapshots, they are typically most valuable for Full node operators.

Snapshots of the MySocial network enable Full node operators a way to bootstrap a Full node without having to execute all the transactions that occurred after genesis. You can upload snapshots to remote object stores like S3, Google Cloud Storage, Azure Blob Storage, and similar services. These services typically run the export process in the background so there is no degradation in performance for your Full node. With snapshots stored in the cloud, you're more easily able to recover quickly from catastrophic failures in your system or hardware.

To maintain a healthy MySocial network, MySocial encourages the MySocial community to bring up additional snapshots to ensure stronger data availability across the network.

## Supported snapshot types

MySocial supports two types of snapshots:
...
- **Formal snapshots** are database agnostic with a minimalistic format, in other words they contain only the data necessary to restore a node to a valid state at the specified epoch, thus they have a much smaller storage footprint and are generally faster to restore from when compared to database snapshots. Formal snapshots are also supported natively by the MySocial protocol. In this context, this means that we can cryptographically verify the contents of the formal snapshot against a commitment from the committee at the epoch we are restoring to. This verification happens automatically and by default during a Formal snapshot restore (unless explicitly bypassed). 

:::info

Formal snapshots are not ttttable for use if you are running an RPC node that does any historical data lookups. For more information on node data management, see [Data Management](./data-management.mdx).
...
1. **Minimalism:** Formal snapshots currently contain only the end of epoch live object set (the set of all object versions eligible for use as input objects for future transactions). MySocial syncs all other critical chain information from the chain or derives it. Thus, formal snapshots contain only the necessary data required for a node to startup at epoch boundary and participate in the network.
...
1. Place the snapshot into the directory that the `db-config` value points to in your fullnode.yaml file. For example, if the `db-config` value points to `/opt/mys/db/authorities_db/full_node_db` and you want to restore from epoch 10, then copy the snapshot to the directory with this command:

   You can use the aws cli (provided you have credentials to associate with the download):
   `aws s3 cp s3://<BUCKET_NAME>/epoch_10 /opt/mys/db/authorities_db/full_node_db/live --recursive --request-payer`.
   
   An alternative is to use `myso-tool` to copy the files:
   ```
   myso-tool download-db-snapshot --latest \
       ...
       --no-sign-request
   ```
   - `--epoch`: The epoch that you want to download. Mysten Labs hosted buckets will only keep the last 90 epochs, you can check the most recent epoch on mys explorers like [mysvision](https://mysvision.xyz/) or [mysscan](https://mysscan.xyz/).
...
1. When using `myso-tool download-db-snapshot` the database is copied to the location you pass to `--path`, in a directory named `epoch_[NUM]`. Rename the `epoch_[NUM]` directory to `live/` under your node's `db_path`, for example `cp -r /tmp/epoch_[NUM] /opt/mys/db/authorities_db/full_node_db/live`.
1. Make sure you update the ownership of the downloaded directory to the mys user (whichever linux user you run `mys-node` as)
   `sudo chown -R mys:mys  /opt/mys/db/authorities_db/full_node_db/live`.
1. Start the MySocial node.

:::info

When you restore a Full node from a snapshot, write it to the path `/opt/mys/db/authorities_db/full_node_db/live`. When restoring a Validator node, you can shorten the database destination to `/opt/mys/db/authorities_db/live`. Check the `db_path` field of your Full node or Validator configs to confirm the path location.

:::

### Restoring using Formal snapshots

To restore using a Formal snapshot, use the `myso-tool` binary. `myso-tool` can be downloaded along with other mys binaries.
See [Install MySocial](../developer/getting-started/mys-install.mdx) for more details.
...
2. Run the command:
   ```
   myso-tool download-formal-snapshot --latest --genesis "<PATH-TO-GENESIS-BLOB>" \
