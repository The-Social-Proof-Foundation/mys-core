---
title: MySocial Exchange Integration Guide
description: Learn the primary tasks necessary to integrate MYS, the token native to the MySocial network, into a cryptocurrency exchange.
---
...
## Requirements to configure a MYS integration
...
The requirements to configure a MYS integration include:
 ...
 - Suggested hardware requirements to run a MySocial Full node:
...
## Configure a MySocial Full node

You can set up and configure a [MySocial Full node](./mys-full-node.mdx) using Docker or directly from source code in the MySocial GitHub repository.

## Set up MySocial addresses
...
MySocial is a DAG-based blockchain and uses checkpoints for node synchronization and global transaction ordering. Checkpoints differ from blocks in the following ways:
 ...
 - MySocial creates about four checkpoints every second. Find the most up-to-date statistic on the [MySocial public dashboard](https://metrics.mysocial.network/public-dashboards/4ceb11cc210d4025b122294586961169).
...
MySocial Checkpoint API operations include:
 ...
 - [mys_getCheckpoints](/mys-api-ref#mys_getCheckpoints) - Retrieves a paginated list of checkpoints that occurred during the specified interval. Pending a future release.

## MYS balance transfer
...
## MySocial API operations for transfers
...
Refer to [MySocial Signatures](/concepts/cryptography/transaction-auth/signatures.mdx) for more details on signature validity requirements.
