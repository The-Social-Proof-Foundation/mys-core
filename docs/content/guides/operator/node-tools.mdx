...
This guide focuses on using the MySocial CLI `validator` commands. 
...
1. Make sure you have completed all the [prerequisites](../developer/getting-started/mys-install.mdx#prerequisites).

2. Build the `mys` binary, which you need for the genesis ceremony. This step can be done on any machine you like. It does not have to be done on the machine on which you will run the validator.
...
3. Run the following command to set up your MySocial account and CLI environment. 
    ...
    2. If you already set it up, simply make sure 
      ...
      b. `mys.keystore` contains your account key pair.
        ...
        If at this point you can't find where `client.yaml` or `mys.keystore` is or have other questions, read [MySocial Client CLI tutorial](/references/cli/client.mdx).
...
``` bash
$ mys validator display-metadata
```
...
``` bash
$ mys validator display-metadata <validator-address>
```
...
``` bash
$ mys validator update-metadata --help
```
...
```sh
$ mys validator update-gas-price <gas-price>
```

if the account itself is a validator and holds the Operation Cap. Or 

```sh
$ mys validator update-gas-price --operation-cap-id <operation-cap-id> <gas-price>
```
...
```sh
$ mys validator report-validator <reportee-address>
```
...
```sh
$ mys validator make-validator-info <name> <description> <image-url> <project-url> <host-name> <gas_price>
```
...
``` sh
$ mys validator become-candidate {path-to}validator.info
```
...
```sh
$ mys validator serialize-payload-pop --account-address $ACCOUNT_ADDRESS --protocol-public-key $BLS_PUBKEY
```
```sh
