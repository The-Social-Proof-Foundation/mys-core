...
MySocial Docs uses the MDX format for its network documentation, which allows [JSX in markdown content](https://mdxjs.com/). MySocial uses features provided by Docusaurus, plugins from its community, and custom plugins in an effort to improve reader experience. None of these features are required in your markdown contributions to the MySocial documentation, but the MySocial community might include them if it improves experience.
...
---
title: A Guides
description: A guide on how to do this thing in MySocial.
effort: medium
---
```
...
`Tabs` and child `TabItem` elements allow for the segmentation of content so that users see only the content that is relevant to them. MySocial documentation uses the default Docusaurus tab components. Check the [official documentation](https://docusaurus.io/docs/next/markdown-features/tabs) for all available options.  
...
1. Check your page in the Vercel preview of your PR (`mys-core` preview link in Vercel comment) to make sure the content displays as expected.
...
1. Check each page in the Vercel preview of your PR (`mys-core` preview link in Vercel comment) to make sure the content displays. If the relative link you provide does not resolve correctly, the page displays error text instead of the expected content.

:::warning

Do not use links relative to the file structure in snippets (for example, `../concepts/file.mdx`). If your snippet requires a link internal to the MySocial docs site, use the relative HTML path instead (`/concepts/file`). 

:::

## Code inject

MySocial documentation uses a custom Docusaurus plugin to include code from the MySocial repo in its pages. The plugin is based on the docusaurus-plugin-includes plugin used for [snippets](#snippets). 

### When to use

In an active repo, source code changes frequently. If you copy and paste that source code in a document, then the two sources can diverge quickly and drastically. In MySocial Docs, you can use the `{@inject}` directive to make a copy of the source when the site builds (merge PR), ensuring the source used in the content always matches the source code in the repo. If the code you inject is tested, this directive ensures the code in your document is also tested.
...
To include the entire source file, use the `{@inject}` directive with a path to the file, based on the MySocial repo root. For example, to include the source file at `mys/examples/move/hero/sources/example.move`, write `{@inject: examples/move/hero/sources/example.move`. The file location is automatically used as the title for the codeblock.

You can use source code from other GitHub repositories, but should do so sparingly and purposefully. There is almost no way to track those projects to ensure the code is valid, unlike the CI processes running on the mys repo. To use, format the directive as `{@inject: github:<GITHUB_ORG_NAME>/<GITHUB_REPO_NAME>/<PATH_TO_FILE>}`.
