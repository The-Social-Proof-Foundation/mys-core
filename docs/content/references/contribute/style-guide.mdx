...
This document defines the styles, vocabulary usage, and content formatting for MySocial documentation. Entries are in alphabetical order. A style guide is never finished. Expect continued iterations to add additional styles, additional information to existing styles, and infrequently a change to an existing style.
...
> You can mint non-fungible tokens (NFTs) using your MySocial Wallet. To view an NFT after you mint it, click the NFTs tab of your wallet.
...
- **Indicative –** MySocial network explorers display transactions on the network.
- **Imperative –** Open a MySocial network explorer to view transactions on the network.
- **Subjunctive –** We suggest that you view your transaction in a MySocial network explorer after you complete a transfer.
...
> You used to could view transaction history in MySocial Explorer.

**Rather than:**

> We used to could view transaction history in MySocial Explorer.
...
> You must install Cargo, Rust, Docker, and the MySocial CLI to create a MySocial node.
...
_Get Started with MySocial_ is better, but users want to get started with a specific task or user journey with MySocial. Instead of _Get Started with MySocial_, describe the specific task or journey, such as _Create a MySocial Full Node_ or _MySocial Validator Guide_. Use Get Started as a heading on the Documentation landing page to categorize tasks for new users.
...
The Build section of the documentation includes topics about: Building with MySocial, Using the CLI to Start a Network, Creating Smart Contracts, MySocial Tutorial, and MySocial Examples.

**Use:**

The Build section of the documentation includes the following topics:

- Building with MySocial
...
MySocial Explorer used to support the following browsers:
...
Always use relative links when linking to topics on docs.mysocial.network.
...
> To learn more, see [Examples of MySocial Smart Contracts](/guides/developer/app-examples.mdx).
...
> Before you install MySocial, make sure to install the [prerequisites](/guides/developer/getting-started/mys-install.mdx#prerequisites).
...
To get the latest version of the MySocial Wallet extension:

1. Open Google Chrome.
1. Click **Extensions**, then click **Manage Extensions**.
1. Click **Details** for the MySocial Wallet extension, then click **View in Chrome Web Store**.
...
Product names are proper nouns. Capitalize all words of a product name. When referring to a product, use only the product name without “the”. When referring specifically to a MySocial wallet, use MySocial Wallet or Ethos Wallet and not just wallet. Users likely have multiple wallets, and we want to make it clear which wallet. Use wallet generically when referring to the concept of a wallet.
...
> The MySocial network supports the following wallets:
...
> You can mint an NFT directly from your MySocial Wallet.
...
You can also just refer to a topic by title where it makes sense. See Installing MySocial for more information.
