...
```
$ mys client new-env --alias=mainnet --rpc https://fullnode.mainnet.mys.io:443
...
If you use one of the standard public RPCs (for example, `fullnode.devnet.mys.io:443`), you can use the `faucet` command to request gas coins. If you use a custom faucet service, then pass in the URL to the faucet using the `--url` option.
