---
title: MySocial Client CLI
description: The MySocial Client CLI provides command-level access to interact with the MySocial network.
---

The MySocial CLI `client` command provides command-level access to interact with the MySocial network. Typical uses for `mys client` include publishing Move smart contracts, getting the information of an object, executing transactions, or managing addresses.

{@include: ../../snippets/cli-check-install.mdx}

## Commands

The following list itemizes all the available subcommands for the `mys client` command.

```
Usage: mys client [OPTIONS] [COMMAND]
...
Commands:
  ...
  envs                        List all MySocial environments
  ...
  new-address                 Generate new address and keypair with keypair scheme flag {ed25519 | secp256k1 | secp256r1} with optional derivation path, default to m/44'/6976'/0'/0'/0' for ed25519 or m/54'/6976'/0'/0/0
                                  for secp256k1 or m/74'/6976'/0'/0/0 for secp256r1. Word length can be { word12 | word15 | word18 | word21 | word24} default to word12 if not specified
  new-env                     Add new MySocial environment
  ...
  pay-all-mys                 Pay all residual MYS coins to the recipient with input coins, after deducting the gas cost. The input coins also include the coin for gas payment, so no extra gas coin is required
  pay-mys                     Pay MYS coins to recipients following following specified amounts, with input coins. Length of recipients must be the same as that of amounts. The input coins also include the coin for
  ...
  transfer-mys                Transfer MYS, and pay gas with the same MYS coin object. If amount is specified, only the amount is transferred; otherwise the entire object is transferred
...
Append the `--json` flag to commands to format responses in JSON instead of the more human-friendly default MySocial CLI output. This can be useful for extremely large datasets, for example, as those results can have a troublesome display on smaller screens. In these cases, the `--json` flag is useful.
...
Use the `mys client envs` command to find the network environments set up in the CLI. The information for these environments is also stored in the client.yaml file in the MySocial configuration directory (`~/.mys/mys_config`).
...
Use `client new-env` to add details for a new network environment. This example creates an environment pointer to MySocial Mainnet. Setting the `alias` value makes referencing the environment less prone to typographical errors. After running this command, MySocial updates your client.yaml file in `~/.mys/mys_config` with the new information.

```
$ mys client new-env --alias=mainnet --rpc https://fullnode.mainnet.mysocial.network:443

Added new MySocial env [mainnet] to config.
```

### Set current environment

Use the `mys client switch` command to change the current network. This example switches the current network to `mainnet`.

```sh
$ mys client switch --env mainnet
```
```sh
Active environment switched to [mainnet]
```

If you run `mys client envs` after this command, you see the asterisk in the `active` column on the `mainnet` row of the table.

### Get current active address

Use the `mys client active-address` command to reveal the current address. The CLI uses the current active address to execute address-specific CLI commands (like `mys client objects`) when you don't provide them with a MySocial address value.

```sh
$ mys client active-address
```
```sh
0x514692f08249c3e9951234ce29074695840422564bff85e424b56de462913e0d
```

### Request a MYS coin from faucet

If you use one of the standard public RPCs (for example, `fullnode.devnet.mysocial.network:443`), you can use the `faucet` command to request gas coins. If you use a custom faucet service, then pass in the URL to the faucet using the `--url` option.
...
```
```sh
Request successful. It can take up to 1 minute to get the coin. Run mys client gas to check your gas coins.
```

### Get objects owned by an address

Use `mys client objects` to list summary information about the objects the current active address owns. You can provide a MySocial address value to the command to list objects for a particular address. This example lists objects for the current active address.

```sh
$ mys client objects 0x36df11369cf00ecf0be68d6ba965b0abe2e883bc5245911e3a29ebfa0aaf6b69
```
```sh
╭───────────────────────────────────────────────────────────────────────────────────────╮
| ╭────────────┬──────────────────────────────────────────────────────────────────────╮ │
...
Use `mys client object <OBJECT-ID>` to list object information for the ID you provide. This example displays the information for a Coin object.

```sh
$ mys client object 0xfffbb30ccb631f15f6cd36700589fc9c31cb04af28a95f3ed26d62daf3acb57f
```
```sh
╭───────────────┬─────────────────────────────────────────────────────────────────────────────────────────────────────────────────────╮
│ objectId  	  │  0xfffbb30ccb631f15f6cd36700589fc9c31cb04af28a95f3ed26d62daf3acb57f                                             	  │
...
│ objType   	  │  0x2::coin::Coin<0x2::mys::MYS>                                                                                 	  │
...
Use the `mys client dynamic-field <DYNAMIC-FIELD-ID>` command to list the details of the dynamic field with the ID you provide.

```sh
$ mys client dynamic-field 0x5
```
```sh
╭─────────────┬───────────────────────────────────────────────────────────────────────────────────────────╮
│ hasNextPage │  false                                                                                	  │
...
│         	  │ │ │ bcsName	   │  LQM2cdzDY3                                                      	  │ │ │
│         	  │ │ │ type   	   │  DynamicField                                                    	  │ │ │
│         	  │ │ │ objectType │  0x3::mys_system_state_inner::MysSystemStateInnerV2              	  │ │ │
...
In this example, let's see how to transfer MYS or transfer an object from one address to another. First of all, there two main commands for sending MYS or transferring objects: `pay` and `transfer`. 
Both `pay` and `transfer` have a few sister commands: `pay-mys`, `pay-all-mys`, `transfer-mys`. 

The differences between these commands are:
- commands that end in `-mys` deal with MySocial's native coin, and they use the input coints to pay for gas and for transferring MYS or the object.
...
- `pay-all-mys` is a special case of `pay-mys` that offers a way to transfer the entire balance after smashing.
- `transfer-mys` is a legacy command and has been entirely superseded by `pay-mys` or `pay-all-mys` depending on whether an amount is specified or not.
...
```sh
$ mys client gas hungry-spodumene
```
```sh
╭────────────────────────────────────────────────────────────────────┬────────────────────┬──────────────────╮
│ gasCoinId                                                          │ mistBalance (MIST) │ mysBalance (MYS) │
├────────────────────────────────────────────────────────────────────┼────────────────────┼──────────────────┤
...
You want to send 0.5 MYS to `eloquent-amber`. Given that you have a few gas coins, you can use `pay`. If only one gas coin exists, then you need to use `transfer-mys` or `pay-mys`, or you would need to split the coin first to have another coin to use for paying gas. In this case, let's use the `pay-mys` command as you do not need to provide a separate gas coin to be used for the gas fees. In the command below, you set the recipient to be `eloquent-amber`, which coin to use to transfer MYS from, and the amount of MYS to transfer.

{@include: ../../snippets/info-gas-budget.mdx}

```sh
$ mys client pay-mys --recipients eloquent-amber --input-coins 0xc9b447fff18f13fa035e028534b8344d5fc8a8760248fad10155e78f44dc3a52 --amounts 500000000 --gas-budget 5000000
```
...
│  │ Sender: 0x0fe375fff0ee40d20c54a7f2478b9b5c7eaa3625b7611f9661ec5faefb4a6fea                    │
│  │ Owner: Account Address ( 0x3d78abc33ccd173c521b4f9e78b21ea2c989960b631732c539efaa38bda30976 ) │
│  │ ObjectType: 0x2::coin::Coin<0x2::mys::MYS>                                                    │
│  │ Version: 3                                                                                    │
│  │ Digest: Fq3maqq3pShgKw58Ssm8cS2j1pspfhanedVK2MMxfQWQ                                          │
│  └──                                                                                             │
│ Mutated Objects:                                                                                 │
...
Check the gas for the sender and the recipient's coins using `mys client gas <address>`. The sender now has 199.49 MYS for that gas coin that was used. 0.5 MYS was transferred, and the remaining 0.01 MYS paid the gas fees.
```sh
$ mys client gas
```
```sh
...
If you want to transfer the whole object, you can use `mys client pay-all-mys` or `mys client transfer-mys` (without passing the amount): 
```sh
$ mys client pay-mys --recipient eloquent-amber --input-coins 0xc9b447fff18f13fa035e028534b8344d5fc8a8760248fad10155e78f44dc3a52 --gas-budget 5000000
```
or
```sh
$ mys client transfer-mys --to eloquent-amber --mys-coin-object-id 0xc9b447fff18f13fa035e028534b8344d5fc8a8760248fad10155e78f44dc3a52 --gas-budget 5000000
```
...
Use the `mys client replay-transaction --tx-digest <TRANSACTION-DIGEST>` to re-execute a transaction locally and show the transaction effects.
...
```sh
$ mys client replay-transaction --tx-digest 51MzJP2Uesvza8vXGpPCGbfLrY6UCfdvdoErN1z4oXPW
```
```sh
╭───────────────────────────────────────────────────────────────────────────────────────────────────╮
│ Transaction Effects                                                                               │
...
Use `mys client replay-batch --path <FILEPATH>` to replay several transactions listed in a newline-separated file. This
will verify that all transactions local execution results match the effects on-chain.

### Profile a transaction

Use the `mys client profile-transaction --tx-digest <TRANSACTION-DIGEST>` command to re-execute a transaction locally
...
To enable the profiler, you must either install or build the MySocial Client binary locally with the `--features tracing` flag if it has not already been built with that feature enabled.
...
One of the main uses of the `mys client` command is to publish smart contracts on the MySocial network. This example switches the current environment to the Devnet network, then builds, tests, and publishes one of the existing Move examples available in the MySocial repository: [mys/examples/move](https://github.com/MystenLabs/mys/tree/main/examples/move)

This example also makes use of `mys move` commands. To learn more about those commands, see [MySocial Move CLI](./move.mdx).

1. Open a terminal or console to the root of your local MySocial repository and navigate to the `move_tutorial` example.
    ```sh
    $ cd examples/move/first_package
    ```
1. Switch to the Devnet network. This command uses an alias, so the `devnet` value might be different for you, depending on the alias name set in your configuration (use `mys client envs` for a list of your defined networks and their aliases).
    ```sh
    $ mys client switch --env devnet
    ```
1. Use `mys move build` to build the package. You must run this command at the same level as the package manifest file ([Move.toml](https://move-book.com/concepts/manifest.html)).
    ```sh
    $ mys move build
    ```
    ...
    ```
1. Use `mys move test` to run the unit tests.
    ```sh
    $ mys move test
    ```
    ...
    Test result: OK. Total tests: 2; passed: 2; failed: 0
     ```
1. Use the `mys client verify-bytecode-meter` to check if the module passes the bytecode meter.
    ```sh
    $ mys client verify-bytecode-meter
    ```
    ...
    ╰────────┴────────────┴────────────╯
    ```
1. Use `mys client gas` to verify that the active address has a gas coin for paying gas. 
    ```sh
    $ mys client gas
    ```
    ...
    ```
1. If you need coins, use `mys client faucet` (not available for Mainnet). For more information on getting gas tokens, see [Get MySocial Tokens](/guides/developer/getting-started/get-coins.mdx).
    ```sh
    $ mys client faucet
    ```
1. Use `mys client gas` to verify the current active address received the coins.
    ```sh
    $ mys client gas
    ```
    ```sh
    ...
    ╰────────────────────────────────────────────────────────────────────┴─────────────╯
    ```
1. Use `mys client publish` to publish the package, being sure to set an appropriate value for the `gas-budget` flag if you're using an older version of MySocial. The console responds with the details of the publish. You can use `mys client object <OBJECT-ID>` to check the details of any of the objects from the process.
    ```sh
    $ mys client publish --gas-budget 100000000 .
    ```
    ```sh
...
Each command has its own help section. For example, `mys client call --help` displays the following prompt:

```sh
Call Move function

Usage: mys client call [OPTIONS] --package <PACKAGE> --module <MODULE> --function <FUNCTION>
...
Options:
      ...
      --serialize-unsigned-transaction  Instead of executing the transaction, serialize the bcs bytes of the unsigned transaction data (TransactionData) using
                                        base64 encoding, and print out the string <TX_BYTES>. The string can be used to execute transaction with `mys client
      ...
      --serialize-signed-transaction    Instead of executing the transaction, serialize the bcs bytes of the signed transaction data (SenderSignedData) using
                                        base64 encoding, and print out the string <SIGNED_TX_BYTES>. The string can be used to execute transaction with `mys
