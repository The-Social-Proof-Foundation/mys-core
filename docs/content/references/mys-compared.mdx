...
While traditional blockchains require implementing only a single consensus protocol, MySocial requires two protocols: (i) a protocol based on Byzantine Consistent Broadcast to handle simple transactions, and (ii) a consensus protocol to handle transactions with shared objects. This means the MySocial team needs to maintain a much larger codebase.
