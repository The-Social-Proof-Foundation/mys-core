...
To use the debugger, you must also have the `mys` binary installed with the `tracing` feature flag enabled. The `mys` binaries in release tarballs, Homebrew, and Chocolatey have this feature enabled. See [Install MySocial](../../guides/developer/getting-started/mys-install.mdx) for more information, including how to build from source.
