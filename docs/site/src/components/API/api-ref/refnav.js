          ...
          <div key={`${api.replaceAll(/\s/g, "-").toLowerCase()}`}>
            <Link
              ...
              data-to-scrollspy-id={`${api
                .replaceAll(/\s/g, "-")
                .toLowerCase()}`}
              className="hover:no-underline pt-4 block text-black dark:text-white hover:text-mys-blue dark:hover:text-mys-blue"
            >
              {api}
            </Link>
            {json["methods"]
              ...
              .map((method) => {
                return (
                  <Link
                    className="my-1 pl-4 block text-mys-gray-95 dark:text-mys-grey-35 hover:no-underline dark:hover:text-mys-blue"
