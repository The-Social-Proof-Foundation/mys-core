...
const protocolInject = async function (source) {
  ...
  for (const file of spec.files) {
    ...
    for (const message of file.messages) {
      ...
      if (allFields.length > 0) {
        const attrStyle =
          "text-lg before:pr-2 before:mr-2 before:text-sm before:border before:border-solid before:border-transparent before:border-r-mys-gray-65";
        const fieldStyle =
          "p-2 font-medium text-lg rounded-lg bg-mys-ghost-white dark:bg-mys-ghost-dark";
        const borderStyle =
          "border border-solid border-y-transparent border-r-transparent border-l-mys-gray-65";
        const leftArrowStyle =
          "relative inline-flex items-center before:content-[''] before:border-t-transparent before:border-b-transparent before:border-solid before:border-y-5 before:border-r-0 before:border-l-8 before:border-l-mys-gray-65";
  ...
  content.push("\n## Scalar Value Types");
  const cellStyle =
    "m-2 min-w-24 max-w-[13rem] rounded-lg border border-solid align-center text-center relative border-mys-gray-65";
  const titleStyle =
    "p-4 pb-2 font-bold text-mys-ghost-dark dark:text-mys-ghost-white bg-mys-ghost-white dark:bg-mys-ghost-dark border border-solid border-transparent rounded-t-lg";
  const valStyle =
    "p-4 pt-2 border border-solid border-transparent border-t-mys-gray-65 whitespace-break-spaces";
