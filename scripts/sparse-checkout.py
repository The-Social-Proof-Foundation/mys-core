...
def read_sparse_config(sparse_file=".sparse"):
    ...
    if os.path.isfile(sparse_file):
        ...
        # mys-benchmark tests use mys-surfer which requires move sources to be checked out
        if "crates/mys-surfer" not in crates and "crates/mys-benchmark" in crates:
            crates.append("crates/mys-surfer")
...
def update_git_sparse_checkout(crates_to_checkout):
    ...
    # if we don't have mys-framework, we probably need to add the move sources in order for tests
    # to run
    if "crates/mys-framework" not in crates_to_checkout:
        default_directories.append("crates/mys-framework/packages")
...
def create_sparse_checkout_worktree():
    ...
    if choice == "y" or choice == "":
        ...
        # are `crates/mys-core`. First, write the defaults
        with open(".sparse", "w") as f:
            f.write("# Directories to include in the sparse checkout\n")
            f.write("crates/mys-core\n")
        # now launch $EDITOR
