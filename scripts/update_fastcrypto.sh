...
# This script attempts to update the Fastcrypto pointer in MySocial
...
function current_fc_revision() {
	...
	[[ -n "$not_equal" ]] && echo "Different values found for the current Fastcrypto revision in MySocial, aborting" && exit 1
	echo "$watermark"
}
...
if [[ "$CURRENT_FC" != "$LATEST_FC" ]]; then
	echo "About to replace $CURRENT_FC with $LATEST_FC as the Narwhal pointer in MySocial"
else
	exit 0
fi
