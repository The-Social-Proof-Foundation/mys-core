...
def get_current_network_epoch(env='testnet'):
    for i in range(NUM_RETRIES):
        ...
        try:
            ...
            if 'error' in result:
                print(
                    f'mysx_getCurrentEpoch rpc request failed: {result["error"]}')
                time.sleep(3)
                continue
            return int(result['result']['epoch'])
        except (KeyError, IndexError, json.JSONDecodeError):
            print(f'mysx_getCurrentEpoch rpc request failed: {result}')
            time.sleep(RETRY_BASE_TIME_SEC * 2**i)  # exponential backoff
            continue
...
def await_started(start_checkpoint):
    for i in range(STARTUP_TIMEOUT_SEC):
        if get_local_metric(Metric.CHECKPOINT) != start_checkpoint:
            print(f"mys-node started successfully after {i} seconds")
            return
        print("Awaiting mys-node startup...")
        time.sleep(1)
    print(f"mys-node failed to start after {STARTUP_TIMEOUT_SEC} seconds")
