...
if [ -f "$WORKING_DIR/mys-node-release" ] && [ -f "$WORKING_DIR/mys-release" ] && [ -f "$WORKING_DIR/mys-node-candidate" ]; then
  echo "Binaries already built, skipping build"
else
  echo "Building mys-node and mys at $RELEASE_COMMIT"

  # remember current commit
  CURRENT_COMMIT=$(git rev-parse HEAD)

  git checkout $RELEASE_COMMIT || exit 1
  cargo build --bin mys-node --bin mys || exit 1
  cp ./target/debug/mys-node "$WORKING_DIR/mys-node-release"
  cp ./target/debug/mys "$WORKING_DIR/mys-release"

  echo "Building mys-node at $RELEASE_CANDIDATE_COMMIT"
  git checkout $RELEASE_CANDIDATE_COMMIT || exit 1
  cargo build --bin mys-node || exit 1
  cp ./target/debug/mys-node "$WORKING_DIR/mys-node-candidate"

  echo "returning to $CURRENT_COMMIT"
  git checkout $CURRENT_COMMIT || exit 1
fi

export MYS_CONFIG_DIR="$WORKING_DIR/config"
rm -rf "$MYS_CONFIG_DIR"

"$WORKING_DIR/mys-release" genesis --epoch-duration-ms 20000
...
while IFS= read -r -d '' file; do
  CONFIGS+=("$file")
done < <(find "$MYS_CONFIG_DIR" -name "127.0.0.1*.yaml" -print0)

export RUST_LOG=mys=debug,info

# 2 release nodes
"$WORKING_DIR/mys-node-release" --config-path "${CONFIGS[0]}" > "$LOG_DIR/node-0.log" 2>&1 &
"$WORKING_DIR/mys-node-release" --config-path "${CONFIGS[1]}" > "$LOG_DIR/node-1.log" 2>&1 &

# 2 candidate nodes
"$WORKING_DIR/mys-node-candidate" --config-path "${CONFIGS[2]}" > "$LOG_DIR/node-2.log" 2>&1 &
"$WORKING_DIR/mys-node-candidate" --config-path "${CONFIGS[3]}" > "$LOG_DIR/node-3.log" 2>&1 &

# and a fullnode
"$WORKING_DIR/mys-node-release" --config-path "$MYS_CONFIG_DIR/fullnode.yaml" > "$LOG_DIR/fullnode.log" 2>&1 &
