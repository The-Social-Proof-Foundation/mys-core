...
# check out all files in crates/mys-protocol-config/src/snapshots at origin commit
echo "Checking out $NETWORK snapshot files"
git checkout $RELEASED_COMMIT -- crates/mys-protocol-config/src/snapshots || exit 1
...
cargo test --package mys-protocol-config snapshot_tests || exit 1
