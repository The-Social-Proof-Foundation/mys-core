...
def extract_notes_for_pr(pr):
    ...
    url = f"https://api.github.com/repos/MystenLabs/mys/pulls/{pr}"
    curl_command = [
        "curl", "-s",
...
def extract_protocol_version(commit):
    ...
    Assumes that it is being called from the root of the mys repository."""
    for line in git(
        "show", f"{commit}:crates/mys-protocol-config/src/lib.rs"
    ).splitlines():
...
def print_changelog(pr, log):
    if pr:
        print(f"https://github.com/MystenLabs/mys/pull/{pr}:")
    print(log)
...
def do_generate(from_, to):
    ...
    for impacted in NOTE_ORDER:
        ...
        if impacted == "Protocol":
            print(f"#### MySocial Protocol Version in this release: `{protocol_version}`")
        print()
