...
def parse_args():
    ...
    generate_lib = subparsers.add_parser(
        "generate-lib",
        help=(
            "Generate `mys-execution/src/lib.rs` based on the current set of "
...
def do_cut(args):
    ...
    update_toml(args.feature, Path() / "mys-execution" / "Cargo.toml")
    generate_impls(args.feature, impl_module)

    with open(Path() / "mys-execution" / "src" / "lib.rs", mode="w") as lib:
        generate_lib(lib)
...
def do_generate_lib(args):
    if args.dry_run:
        ...
        lib_path = Path() / "mys-execution" / "src" / "lib.rs"
...
def cut_command(f):
    ...
    return [
        ...
        *["-d", f"mys-execution/latest:mys-execution/{f}:-latest"],
        *["-d", f"external-crates/move:external-crates/move/move-execution/{f}"],
        *["-p", "mys-adapter-latest"],
        *["-p", "mys-move-natives-latest"],
        *["-p", "mys-verifier-latest"],
...
def cut_directories(f):
    ...
    mys_base = Path() / "mys-execution"
    external = Path() / "external-crates"

    crates = [
        mys_base / f / "mys-adapter",
        mys_base / f / "mys-move-natives",
        mys_base / f / "mys-verifier",
    ]
...
def impl(feature):
    ...
    return Path() / "mys-execution" / "src" / (feature.replace("-", "_") + ".rs")
...
def update_toml(feature, toml_path):
    """Add dependencies for 'feature' to mys-execution's manifest."""
...
def generate_impls(feature, copy):
    ...
    orig = Path() / "mys-execution" / "src" / "latest.rs"
...
def generate_lib(output_file: TextIO):
    ...
    Generates the contents of mys-execution/src/lib.rs to assign a numeric
    ...
    template_path = Path() / "mys-execution" / "src" / "lib.template.rs"
    cuts = discover_cuts()
...
# Modules in `mys-execution` that don't count as "cuts" (they are
...
def discover_cuts():
    ...
    Finds all modules within the `mys-execution` crate that count as
