...
usage() {
    ...
    echo "field of the mys-node policy-config config for traffic controller."
    echo "To use, do the following:"
    echo
    echo "1. Set the following mys-node config:"
    echo
    echo "  client-id-source:"
    echo "    x-forwarded-for: 0"
    echo
    echo "2. Start the node"
    echo "3. Run this script, piping mys-node logs to it and providing the known client IP address as an argument."
    ...
    echo
    echo "Example 1: journalctl -fu mys-node | $0 1.2.3.4"
