...
def replace_protocol_version_in_repo(repo_path, old_version, new_version, yes_to_all, dry_run):
    for root, dirs, files in os.walk(repo_path):
        for file in files:
            if "mys-graphql-e2e-tests" in root.split(os.sep):
                if file.endswith('.move'):
                    file_path = os.path.join(root, file)
                    replace_protocol_version_in_file(file_path, old_version, new_version, yes_to_all, dry_run)

if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Replace protocol version in mys-graphql-e2e-tests tests.')
    ...
    if not args.dry_run:
        print(f"Next step. Running `env UB=1 cargo nextest run` in `crates/mys-graphql-e2e-tests` to update all the snapshots.")
