...
/// ConsensusAuthority is used by MySo to manage the lifetime of AuthorityNode.
    ...
    N: NetworkManager<AuthorityService<ChannelCoreThreadDispatcher>>,
{
    ...
    pub(crate) async fn start(
        ...
        // Only start ProposedBlockHandler if fastpath is enabled
        let proposed_block_handler = if context.protocol_config.mysticeti_fastpath() {
            info!("Starting ProposedBlockHandler (fastpath enabled)");
            let mut handler = ProposedBlockHandler::new(
                context.clone(),
                signals_receivers.block_broadcast_receiver(),
                transaction_certifier.clone(),
            );
            Some(spawn_logged_monitored_task!(handler.run(), "proposed_block_handler"))
        } else {
            info!("Skipping ProposedBlockHandler (fastpath disabled)");
            None
        };
    ...
    pub(crate) async fn stop(mut self) {
        ...
        if let Some(handler) = &self.proposed_block_handler {
            handler.abort();
        }
