...
impl ChannelPool {
    ...
    async fn get_channel(
        ...
        let client_tls_config = mys_tls::create_rustls_client_config(
            self.context
                .committee
                .authority(peer)
                .network_key
...
impl<S: NetworkService> NetworkManager<S> for TonicManager {
    ...
    async fn install_service(&mut self, service: Arc<S>) {
        ...
        let tls_server_config = mys_tls::create_rustls_server_config_with_client_verifier(
...
// TODO: improve mys-http to allow for providing a MakeService so that this can be done once per
// connection
