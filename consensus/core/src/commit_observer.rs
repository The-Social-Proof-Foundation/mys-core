...
/// When MySo recovers, it blocks until the commits it knows about are recovered. So consensus
/// must be able to quickly recover the commits it has sent to MySo.
    ...
    async fn recover_and_send_commits(&mut self, commit_consumer: &CommitConsumerArgs) {
            ...
            .step_by(COMMIT_RECOVERY_BATCH_SIZE as usize)
        {
            ...
            for commit in unsent_commits.into_iter() {
                ...
                // MySo so that the current scores are available for submission.
