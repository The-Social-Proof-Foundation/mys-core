...
import { MysArgument, MysCallArg, MysTransaction, TransactionBlockData } from '@socialproof/mys/client';
...
const getCallArgDisplay = (argument: MysCallArg | undefined) => {
...
const getMysArgumentDisplay = (argument: MysArgument, inputs: MysCallArg[]) => {
	if (typeof argument === 'string') return argument;
...
const renderArguments = (callArgs: MysArgument[], inputs: MysCallArg[]) => {
	return (
		<div className="flex overflow-x-auto gap-3 my-3">
			{callArgs.map((arg, index) => (
				<div key={index} className="flex-shrink-0">
					{getMysArgumentDisplay(arg, inputs)}
				</div>
			))}
		</div>
	);
