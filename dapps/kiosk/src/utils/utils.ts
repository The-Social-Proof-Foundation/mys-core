...
export const mistToMys = (mist: bigint | string | undefined) => {
	if (!mist) return 0;
	return Number(mist || 0) / Number(MIST_PER_MYS);
};

export const formatMys = (amount: number) => {
...
export const findActiveCap = (
	...
	return caps.find((x) => normalizeMysAddress(x.kioskId) === normalizeMysAddress(kioskId));
};
