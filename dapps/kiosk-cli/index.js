...
const KNOWN_TYPES = {
  mysfren:
    '0x80d7de9c4a56194087e0ba0bf59492aa8e6a5ee881606226930827085ddf2332::mysfrens::MysFren<0x80d7de9c4a56194087e0ba0bf59492aa8e6a5ee881606226930827085ddf2332::capy::Capy>',
};

/** JsonRpcProvider for the Testnet */
const client = new MysClient({ url: getFullnodeUrl('testnet') });
...
program
  .command('list')
  .description('list an item in the Kiosk for the specified amount of MYS')
  .argument('<item ID>', 'The ID of the item to list')
  .argument('<amount MIST>', 'The amount of MYS to list the item for')
  .action(listItem);
...
async function newKiosk() {
  const sender = keypair.getPublicKey().toMysAddress();
...
async function showInventory({ address, onlyDisplay, cursor, filter }) {
  const owner = address || keypair.getPublicKey().toMysAddress();

  if (!isValidMysAddress(owner)) {
    throw new Error(`Invalid MYS address: "${owner}"`);
  }
...
async function showKioskContents({ id, address }) {
  ...
  if (id) {
    if (!isValidMysObjectId(id)) {
      throw new Error(`Invalid Kiosk ID: "${id}"`);
    }

    kioskId = id;
  } else {
    const sender = address || keypair.getPublicKey().toMysAddress();

    if (!isValidMysAddress(sender)) {
      throw new Error(`Invalid MYS address: "${sender}"`);
    }
  ...
  const tabledItems = items
    .map((item) => ({
      ...
      'price (MYS)': item.listing ? formatAmount(item.listing.price) : 'N/A',
    }))
...
async function placeItem(itemId) {
  const kioskCap = await findKioskCap().catch(() => null);
  const owner = keypair.getPublicKey().toMysAddress();

  if (kioskCap === null) {
    throw new Error('No Kiosk found for sender; use `new` to create one');
  }

  if (!isValidMysObjectId(itemId)) {
...
async function takeItem(itemId, { address }) {
  const cap = await findKioskCap().catch(() => null);
  const receiver = address || keypair.getPublicKey().toMysAddress();

  if (!isValidMysAddress(receiver)) {
...
/**
 * Command: `list`
 * Description: Lists an item in the Kiosk for the specified amount of MYS
 */
...
async function purchaseItem(itemId, opts) {
  ...
  if (inputKioskId && !isValidMysObjectId(inputKioskId)) {
...
function formatAmount(amount) {
  ...
  if (amount <= MIST_PER_MYS) {
    return Number(amount) / Number(MIST_PER_MYS);
  }
